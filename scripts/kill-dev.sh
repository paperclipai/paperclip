#!/usr/bin/env bash
#
# Kill all local Ironworks dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all ironworks dev processes
...
# Collect PIDs of node processes running from any ironworks directory.
# Matches paths like /Users/*/ironworks/... or /Users/*/ironworks-*/...
...
while IFS= read -r line; do
  ...
  pids+=("$pid")
  lines+=("$line")
done < <(ps aux | grep -E '/ironworks(-[^/]+)?/' | grep node | grep -v grep || true)

if [[ ${#pids[@]} -eq 0 ]]; then
  echo "No Ironworks dev processes found."
  exit 0
fi

echo "Found ${#pids[@]} Ironworks dev process(es):"
echo ""

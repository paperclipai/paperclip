...
#
# Usage:
#   scripts/kill-dev.sh        # kill all odysseus dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/odysseus(-[^/]+)?/' | grep node | grep -v grep || true)
...
candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.odysseus/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.odysseus/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.odysseus/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/odysseus*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.odysseus/instances/*/db/postmaster.pid
    "$sibling_root"/.odysseus/runtime-services/instances/*/db/postmaster.pid
  )
done

...
AITEAMCORP_API_URL="${AITEAMCORP_API_URL:-http://127.0.0.1:3100}"
...
AITEAMCORP_API_URL_FOR_OPENCLAW="${AITEAMCORP_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
if [[ -n "${AITEAMCORP_COOKIE:-}" ]]; then
  ...
  AITEAMCORP_BROWSER_ORIGIN="${AITEAMCORP_BROWSER_ORIGIN:-${AITEAMCORP_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${AITEAMCORP_BROWSER_ORIGIN}" -H "Referer: ${AITEAMCORP_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set AITEAMCORP_COOKIE or AITEAMCORP_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg aiteamcorpApiUrl "$AITEAMCORP_API_URL_FOR_OPENCLAW" \
    ...
    '{
      ...
      agentDefaultsPayload: {
        ...
        sessionKey: "aiteamcorp",
        timeoutSec: $timeoutSec,
        waitTimeoutMs: $waitTimeoutMs,
        aiteamcorpApiUrl: $aiteamcorpApiUrl
      }
    }')"
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/aiteamcorp"
  local claimed_file="${workspace_dir}/aiteamcorp-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/aiteamcorp-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/aiteamcorp-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/aiteamcorp-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/aiteamcorp"
    assert_status "200"
    {
      echo "AITEAMCORP_API_URL=${AITEAMCORP_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set AITEAMCORP_API_KEY=${AGENT_API_KEY} in your run context before running AiTeamCorp heartbeat steps."
...
main() {
  ...
  wait_http_ready "${AITEAMCORP_API_URL%/}/api/health" 15 || fail "AiTeamCorp API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "aiteamcorp health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"

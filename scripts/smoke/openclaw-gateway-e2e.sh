...
VALADRIEN_OS_API_URL_FOR_OPENCLAW="${VALADRIEN_OS_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set VALADRIEN_OS_COOKIE or VALADRIEN_OS_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg valadrienOsApiUrl "$VALADRIEN_OS_API_URL_FOR_OPENCLAW" \
    ...
    '{
      ...
      agentDefaultsPayload: {
        ...
        valadrienOsApiUrl: $valadrienOsApiUrl
      }
    }')"
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/valadrien-os"
  local claimed_file="${workspace_dir}/valadrien-os-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/valadrien-os-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/valadrien-os-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/valadrien-os-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/valadrien-os"
    assert_status "200"
    {
      echo "VALADRIEN_OS_API_URL=${VALADRIEN_OS_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set VALADRIEN_OS_API_KEY=${AGENT_API_KEY} in your run context before running ValAdrien OS heartbeat steps."
...
main() {
  ...
  wait_http_ready "${VALADRIEN_OS_API_URL%/}/api/health" 15 || fail "ValAdrien OS API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "valadrien-os health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"

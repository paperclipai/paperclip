...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-$OPENCLAW_TMP_DIR/openclaw-odysseus-smoke}"
...
ODYSSEUS_HOST_PORT="${ODYSSEUS_HOST_PORT:-3100}"
ODYSSEUS_HOST_FROM_CONTAINER="${ODYSSEUS_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.odysseus-openclaw.override.yml"
...
detect_odysseus_base_url() {
  ...
  for candidate in "$ODYSSEUS_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${ODYSSEUS_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${ODYSSEUS_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
odysseus_base_url="$(detect_odysseus_base_url || true)"
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${ODYSSEUS_HOST_PORT}
  (Do not use http://127.0.0.1:${ODYSSEUS_HOST_PORT} inside the container.)

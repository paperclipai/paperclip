...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-$OPENCLAW_TMP_DIR/openclaw-valadrien-os-smoke}"
...
VALADRIEN_OS_HOST_PORT="${VALADRIEN_OS_HOST_PORT:-3100}"
VALADRIEN_OS_HOST_FROM_CONTAINER="${VALADRIEN_OS_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.valadrien-os-openclaw.override.yml"
...
detect_valadrien_os_base_url() {
  ...
  for candidate in "$VALADRIEN_OS_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${VALADRIEN_OS_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${VALADRIEN_OS_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
valadrien_os_base_url="$(detect_valadrien_os_base_url || true)"

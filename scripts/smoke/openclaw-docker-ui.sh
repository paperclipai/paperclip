...
PAPERCLIP_HOST_PORT="${PAPERCLIP_HOST_PORT:-3102}"
...
PrivateClip URL for OpenClaw container:
EOF
...
EOF
  ...
  If PrivateClip rejects the host, run on host machine:
    pnpm paperclipai allowed-hostname host.docker.internal
  Then restart PrivateClip and re-run this script.
EOF
  fi
  cat <<EOF

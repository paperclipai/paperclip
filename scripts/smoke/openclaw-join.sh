...
API_BASE="${ODYSSEUS_API_URL%/}/api"
COMPANY_ID="${COMPANY_ID:-${ODYSSEUS_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-odysseus-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-odysseus-openclaw-smoke}"
SMOKE_PORT="${SMOKE_PORT:-19091}"
SMOKE_TIMEOUT_SEC="${SMOKE_TIMEOUT_SEC:-45}"

AUTH_HEADERS=()
if [[ -n "${ODYSSEUS_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=(-H "Authorization: ${ODYSSEUS_AUTH_HEADER}")
fi
if [[ -n "${ODYSSEUS_COOKIE:-}" ]]; then
  AUTH_HEADERS+=(-H "Cookie: ${ODYSSEUS_COOKIE}")
fi
...
Provide one of:
  ODYSSEUS_AUTH_HEADER=\"Bearer <board-token>\"
  ODYSSEUS_COOKIE=\"<board-session-cookie>\"
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.odysseus.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"

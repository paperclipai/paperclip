...
API_BASE="${IRONWORKS_API_URL%/}/api"
COMPANY_ID="${COMPANY_ID:-${IRONWORKS_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-ironworks-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-ironworks-openclaw-smoke}"
SMOKE_PORT="${SMOKE_PORT:-19091}"
SMOKE_TIMEOUT_SEC="${SMOKE_TIMEOUT_SEC:-45}"

AUTH_HEADERS=()
if [[ -n "${IRONWORKS_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=(-H "Authorization: ${IRONWORKS_AUTH_HEADER}")
fi
if [[ -n "${IRONWORKS_COOKIE:-}" ]]; then
  AUTH_HEADERS+=(-H "Cookie: ${IRONWORKS_COOKIE}")
fi
...
Provide one of:
  IRONWORKS_AUTH_HEADER=\"Bearer <board-token>\"
  IRONWORKS_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Ironworks OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.ironworks.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"

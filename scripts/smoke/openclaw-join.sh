...
AITEAMCORP_API_URL="${AITEAMCORP_API_URL:-http://localhost:3100}"
API_BASE="${AITEAMCORP_API_URL%/}/api"
COMPANY_ID="${COMPANY_ID:-${AITEAMCORP_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-aiteamcorp-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-aiteamcorp-openclaw-smoke}"
SMOKE_PORT="${SMOKE_PORT:-19091}"
SMOKE_TIMEOUT_SEC="${SMOKE_TIMEOUT_SEC:-45}"

AUTH_HEADERS=()
if [[ -n "${AITEAMCORP_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=(-H "Authorization: ${AITEAMCORP_AUTH_HEADER}")
fi
if [[ -n "${AITEAMCORP_COOKIE:-}" ]]; then
  AUTH_HEADERS+=(-H "Cookie: ${AITEAMCORP_COOKIE}")
fi
...
Provide one of:
  AITEAMCORP_AUTH_HEADER=\"Bearer <board-token>\"
  AITEAMCORP_COOKIE=\"<board-session-cookie>\"
...
log "checking AiTeamCorp health"
...
assert_status "200"
if ! grep -q "AiTeamCorp OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.aiteamcorp.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"

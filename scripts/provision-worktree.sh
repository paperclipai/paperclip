...
base_cwd="${IRONWORKS_WORKSPACE_BASE_CWD:?IRONWORKS_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${IRONWORKS_WORKSPACE_CWD:?IRONWORKS_WORKSPACE_CWD is required}"
ironworks_home="${IRONWORKS_HOME:-$HOME/.ironworks}"
ironworks_instance_id="${IRONWORKS_INSTANCE_ID:-default}"
ironworks_dir="$worktree_cwd/.ironworks"
worktree_config_path="$ironworks_dir/config.json"
worktree_env_path="$ironworks_dir/.env"
worktree_name="${IRONWORKS_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${IRONWORKS_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.ironworks/config.json" || -L "$base_cwd/.ironworks/config.json" ) ]]; then
  source_config_path="$base_cwd/.ironworks/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$ironworks_home/instances/$ironworks_instance_id/config.json"
fi
source_env_path="$(dirname "$source_config_path")/.env"

mkdir -p "$ironworks_dir"

run_isolated_worktree_init() {
  if command -v pnpm >/dev/null 2>&1 && pnpm ironworksai --help >/dev/null 2>&1; then
    pnpm ironworksai worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi

  if command -v ironworksai >/dev/null 2>&1; then
    ironworksai worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  IRONWORKS_DIR="$ironworks_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  IRONWORKS_WORKTREES_DIR="${IRONWORKS_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const ironworksDir = process.env.IRONWORKS_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.IRONWORKS_WORKTREES_DIR) ?? "~/.ironworks-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(ironworksDir, "config.json");
  const envPath = path.resolve(ironworksDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "ironworks",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.IRONWORKS_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.IRONWORKS_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.IRONWORKS_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "IRONWORKS_HOME=" + JSON.stringify(worktreeHome),
    "IRONWORKS_INSTANCE_ID=" + JSON.stringify(instanceId),
    "IRONWORKS_CONFIG=" + JSON.stringify(configPath),
    "IRONWORKS_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "IRONWORKS_IN_WORKTREE=true",
    "IRONWORKS_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.IRONWORKS_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("IRONWORKS_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if ! run_isolated_worktree_init; then
  echo "ironworksai CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
  write_fallback_worktree_config
fi

...
base_cwd="${ODYSSEUS_WORKSPACE_BASE_CWD:?ODYSSEUS_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${ODYSSEUS_WORKSPACE_CWD:?ODYSSEUS_WORKSPACE_CWD is required}"
odysseus_home="${ODYSSEUS_HOME:-$HOME/.odysseus}"
odysseus_instance_id="${ODYSSEUS_INSTANCE_ID:-default}"
odysseus_dir="$worktree_cwd/.odysseus"
worktree_config_path="$odysseus_dir/config.json"
worktree_env_path="$odysseus_dir/.env"
worktree_name="${ODYSSEUS_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${ODYSSEUS_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.odysseus/config.json" || -L "$base_cwd/.odysseus/config.json" ) ]]; then
  source_config_path="$base_cwd/.odysseus/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$odysseus_home/instances/$odysseus_instance_id/config.json"
fi
...
run_isolated_worktree_init() {
  ...
  if command -v pnpm >/dev/null 2>&1 && pnpm odysseus --help >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      pnpm odysseus worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi

  if command -v odysseus >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      odysseus worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  ODYSSEUS_WORKTREES_DIR="${ODYSSEUS_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const odysseusDir = process.env.ODYSSEUS_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.ODYSSEUS_WORKTREES_DIR) ?? "~/.odysseus-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(odysseusDir, "config.json");
  const envPath = path.resolve(odysseusDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "odysseus",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.ODYSSEUS_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.ODYSSEUS_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.ODYSSEUS_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "ODYSSEUS_HOME=" + JSON.stringify(worktreeHome),
    "ODYSSEUS_INSTANCE_ID=" + JSON.stringify(instanceId),
    "ODYSSEUS_CONFIG=" + JSON.stringify(configPath),
    "ODYSSEUS_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "ODYSSEUS_IN_WORKTREE=true",
    "ODYSSEUS_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.ODYSSEUS_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("ODYSSEUS_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if [[ -e "$worktree_config_path" && -e "$worktree_env_path" ]]; then
  ...
  if odysseus_command_available; then
    run_isolated_worktree_init
  else
    echo "odysseus CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
    write_fallback_worktree_config
  fi
fi
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".odysseus-backup-${BASHPID:-$$}"
    moved_symlink_paths=()

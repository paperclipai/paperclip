...
["speko","Speko","Answer and place phone calls, then read transcripts and recordings.","communication","speko.ai",["https://mcp.speko.ai/*"],method("mcp-key","mcp_remote","api_key",{serverUrl:"https://mcp.speko.ai/mcp"},"S4","Create a Speko API key at https://platform.speko.ai/api-keys, then paste it here. A Speko key belongs to one Speko organization, so the key is the boundary: use a key from an organization that holds only the agents and phone numbers these agents may reach. Keep approvals on, because calls cost money and reach real people.",{credentialFields:[field("authorization","Speko API key","Paste your Speko API key")],keyPlacement:{location:"header",name:"Authorization",prefix:"Bearer "},requiredResourceFilters:["organization","agent","phone_number"]})],

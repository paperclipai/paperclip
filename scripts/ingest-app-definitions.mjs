...
["notion","Notion","Read and update pages in your Notion workspace.","content","notion.so",["https://mcp.notion.com/*","https://api.notion.com/*"],[method("mcp-oauth","mcp_remote","oauth",{serverUrl:"https://mcp.notion.com/mcp",authorizationEndpoint:"https://api.notion.com/v1/oauth/authorize",tokenEndpoint:"https://api.notion.com/v1/oauth/token",scopesHint:["read_content","update_content"]},"S3","Connect Notion for workspace content. Share only the pages and databases agents should use.",{whenToUse:"Use Notion's hosted MCP server for the quickest OAuth setup.",requiredResourceFilters:["workspace","page","database"]}),method("api-key","rest_api","api_key",{serviceHost:"api.notion.com"},"S3","Create a Notion internal integration, copy its secret, and share only the pages and databases agents should use.",{whenToUse:"Use a Notion integration token for direct REST API access.",credentialFields:[field("apiKey","Integration token","secret_...")],keyPlacement:{location:"header",name:"Authorization",prefix:"Bearer "},requiredResourceFilters:["workspace","page","database"]})]],
...
].map(([slug,name,description,category,domain,urlPatterns,m])=>({schemaVersion:1,slug,name,description,categories:[category],featured:["zapier","github","slack","notion","linear"].includes(slug),branding:{logoUrl:favicon(domain)},urlPatterns,methods:Array.isArray(m)?m:[m]}));

...
const chatProviderName=(provider)=>({github:"GitHub","microsoft-teams":"Microsoft Teams",slack:"Slack",telegram:"Telegram"})[provider];
const channelMethod=(provider,credentialFields,requiredResourceFilters,guidanceMd,consoleLinks)=>({key:"chat-agent",label:"Chat with an agent",purpose:"channel",provider,transport:"chat_sdk",auth:"api_key",ownershipModes:["customer"],whenToUse:`Let people in ${chatProviderName(provider)} start and continue work with one Paperclip agent.`,credentialFields,guidanceMd,consoleLinks,riskTier:"S3",requiredResourceFilters});
...
["github","GitHub","Give agents repository tools or let people work with an agent from GitHub issues and pull requests.","developer","github.com",["https://api.githubcopilot.com/mcp/*","https://github.com/*"],[method("managed","mcp_remote","oauth",{serverUrl:"https://api.githubcopilot.com/mcp/"},"S3","Authorize Paperclip, then choose selected repositories in GitHub. You can edit repository access later from GitHub's installation settings.",{label:"Use this connection as an agent tool",purpose:"tool",oauthStrategy:"paperclip_cloud_connector",connectorProfile:"github.code",grantKinds:["user","agent"],ownershipModes:["platform_shared"],whenToUse:"Connect your GitHub account for durable MCP, shell Git, gh, and repository access.",warnings:["Shell Git and gh receive this identity for the run and are not constrained by per-tool Ask-first controls."],requiredResourceFilters:["organization","repository"]}),method("mcp-key","mcp_remote","api_key",{serverUrl:"https://api.githubcopilot.com/mcp/"},"S3","Create a fine-grained token limited to the repositories agents should use.",{label:"Personal access token (advanced)",purpose:"tool",credentialFields:[field("authorization","GitHub token","github_pat_...")],keyPlacement:{location:"header",name:"Authorization",prefix:"Bearer "},requiredResourceFilters:["organization","repository"]}),channelMethod("github",[{...field("appId","GitHub App ID","123456"),type:"text",secret:false},{...field("privateKey","Private key (PEM)","-----BEGIN RSA PRIVATE KEY-----"),type:"textarea"},field("webhookSecret","Webhook secret","Paste the secret configured on the GitHub App webhook")],["organization","repository"],"Create one private GitHub App with active SSL-verified webhooks, Issues and Pull requests read/write permission, and the issue_comment and pull_request_review_comment events. Install it only on the repositories where people may mention the agent.",{register:"https://github.com/settings/apps/new",docs:"https://docs.github.com/en/apps/creating-github-apps/registering-a-github-app/registering-a-github-app"})]],
["slack","Slack","Give agents Slack tools or let people start and continue Paperclip work from Slack.","communication","slack.com",["https://mcp.slack.com/*","https://app.slack.com/client/*"],[method("mcp-oauth","mcp_remote","oauth",{serverUrl:"https://mcp.slack.com/mcp",authorizationEndpoint:"https://slack.com/oauth/v2/authorize",tokenEndpoint:"https://slack.com/api/oauth.v2.access",scopesHint:["channels:read","chat:write","search:read"]},"S3","Connect a Slack workspace and limit access to the channels agents need.",{label:"Use this connection as an agent tool",purpose:"tool",ownershipModes:["customer"],requiredResourceFilters:["workspace","channel"]}),channelMethod("slack",[field("botToken","Bot User OAuth Token","xoxb-..."),field("signingSecret","Signing Secret","Paste the Slack App signing secret")],["workspace","channel"],"Create and install one Slack App for this agent. Paperclip receives verified Events API requests and interactive callbacks, acknowledges with reactions, responds in direct messages, and starts one Paperclip task per new mentioned channel thread.",{register:"https://api.slack.com/apps",docs:"https://api.slack.com/start/quickstart"})]],
["microsoft-teams","Microsoft Teams","Let people start and continue Paperclip work with an agent from Microsoft Teams.","communication","teams.microsoft.com",["https://teams.microsoft.com/*"],channelMethod("microsoft-teams",[{...field("clientId","Application / Client ID","00000000-0000-0000-0000-000000000000"),type:"text",secret:false},{...field("tenantId","Directory / Tenant ID","00000000-0000-0000-0000-000000000000"),type:"text",secret:false},field("clientSecret","Client secret","Paste the client-secret value")],["team","channel","chat"],"Register one single-tenant Azure Bot, then create a Teams app whose manifest enables personal, team, and groupchat bot scopes and the ChannelMessage.Read.Group and ChatMessage.Read.Chat resource-specific permissions. Install it in each standard channel or chat where it may work.",{register:"https://dev.teams.microsoft.com/apps",docs:"https://learn.microsoft.com/en-us/microsoftteams/platform/bots/how-to/create-a-bot-for-teams"})],
["telegram","Telegram","Let people start and continue Paperclip work with an agent from Telegram.","communication","telegram.org",["https://t.me/*","https://telegram.me/*","https://api.telegram.org/*"],channelMethod("telegram",[field("botToken","Bot token","123456789:AA...")],["chat","group","topic"],"Create one dedicated bot with BotFather. Paperclip replaces any existing webhook for that token. The bot works in private chats, groups, supergroups, and forum topics; keep privacy mode enabled so group work starts from a command, mention, or reply addressed to the bot.",{register:"https://t.me/BotFather",docs:"https://core.telegram.org/bots/tutorial"})],
...
const validateApp=(app)=>{
 ...
 for(const connectionMethod of app.methods){
  if(connectionMethod.auth==="api_key"&&!connectionMethod.keyPlacement&&(connectionMethod.purpose??"tool")!=="channel") throw new Error(`${app.slug}/${connectionMethod.key}: tool api_key requires keyPlacement`);

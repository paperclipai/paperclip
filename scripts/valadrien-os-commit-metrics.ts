...
const DEFAULT_QUERY = "\"Co-Authored-By: ValadrienOs <noreply@TODO_DOMAIN>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/valadrien-os-commit-metrics-cache.json");
...
const VALADRIEN_OS_EMAIL = "noreply@TODO_DOMAIN";
const VALADRIEN_OS_NAME = "valadrien-os";
...
function printHelp() {
  console.log(`Usage: tsx scripts/valadrien-os-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === VALADRIEN_OS_EMAIL) || (displayName && displayName.toLowerCase() === VALADRIEN_OS_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("ValadrienOs commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "valadrien-os-commit-metrics",

#!/usr/bin/env node
/**
 * Screenshot utility for AiTeamCorp UI.
 *
 * Reads the board token from ~/.aiteamcorp/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/aiteamcorp-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".aiteamcorp/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.aiteamcorp/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the AiTeamCorp origin only

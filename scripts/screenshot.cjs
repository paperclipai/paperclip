#!/usr/bin/env node
/**
 * Screenshot utility for Odysseus UI.
 *
 * Reads the board token from ~/.odysseus/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/odysseus-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".odysseus/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.odysseus/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the Odysseus origin only

...
const LANG_DIR = path.join(ROOT, "ui", "src", "i18n", "lang");
const RESOURCES_TS = path.join(ROOT, "ui", "src", "i18n", "resources.ts");
...
function generateResourcesTs(availableLangs) {
  const imports = availableLangs.map((l) => `import ${l} from "./lang/${l}.json";`).join("\n");
  const resourcesObj = availableLangs.map((l) => `    ${l},`).join("\n");
...
`;
  fs.writeFileSync(RESOURCES_TS, content, "utf-8");
  console.log(`📝 resources.ts updated with languages: ${availableLangs.join(", ")}`);
}

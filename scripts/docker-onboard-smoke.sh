...
IMAGE_NAME="${IMAGE_NAME:-aiteamcorp-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
AITEAMCORP_VERSION="${AITEAMCORP_VERSION:-latest}"
...
AITEAMCORP_DEPLOYMENT_MODE="${AITEAMCORP_DEPLOYMENT_MODE:-authenticated}"
AITEAMCORP_DEPLOYMENT_EXPOSURE="${AITEAMCORP_DEPLOYMENT_EXPOSURE:-private}"
AITEAMCORP_PUBLIC_URL="${AITEAMCORP_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@aiteamcorp.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-aiteamcorp-smoke-password}"
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_AITEAMCORP_VERSION=%q\n' "$AITEAMCORP_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e AITEAMCORP_DEPLOYMENT_MODE="$AITEAMCORP_DEPLOYMENT_MODE" \
      -e AITEAMCORP_DEPLOYMENT_EXPOSURE="$AITEAMCORP_DEPLOYMENT_EXPOSURE" \
      -e AITEAMCORP_PUBLIC_URL="$AITEAMCORP_PUBLIC_URL" \
      -e AITEAMCORP_HOME="/aiteamcorp" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "aiteamcorp@${AITEAMCORP_VERSION}" auth bootstrap-ceo --data-dir "$AITEAMCORP_HOME" --base-url "$AITEAMCORP_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
post_json_with_cookies() {
  ...
  curl -sS \
    ...
    -H "Origin: $AITEAMCORP_PUBLIC_URL" \
    -X POST \
    "$url" \
    --data "$body"
}
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$AITEAMCORP_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$AITEAMCORP_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$AITEAMCORP_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$AITEAMCORP_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$AITEAMCORP_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$AITEAMCORP_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg AITEAMCORP_VERSION="$AITEAMCORP_VERSION" \
...
echo "    Public URL: $AITEAMCORP_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $AITEAMCORP_DEPLOYMENT_MODE/$AITEAMCORP_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/aiteamcorp-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$AITEAMCORP_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $AITEAMCORP_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$AITEAMCORP_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $AITEAMCORP_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"

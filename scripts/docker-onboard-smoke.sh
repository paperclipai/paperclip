...
IMAGE_NAME="${IMAGE_NAME:-valadrien-os-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
VALADRIEN OS_VERSION="${VALADRIEN OS_VERSION:-latest}"
...
VALADRIEN_OS_DEPLOYMENT_MODE="${VALADRIEN_OS_DEPLOYMENT_MODE:-authenticated}"
VALADRIEN_OS_DEPLOYMENT_EXPOSURE="${VALADRIEN_OS_DEPLOYMENT_EXPOSURE:-private}"
VALADRIEN_OS_PUBLIC_URL="${VALADRIEN_OS_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@valadrien-os.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-valadrien-os-smoke-password}"
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_VALADRIEN OS_VERSION=%q\n' "$VALADRIEN OS_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e VALADRIEN_OS_DEPLOYMENT_MODE="$VALADRIEN_OS_DEPLOYMENT_MODE" \
      -e VALADRIEN_OS_DEPLOYMENT_EXPOSURE="$VALADRIEN_OS_DEPLOYMENT_EXPOSURE" \
      -e VALADRIEN_OS_PUBLIC_URL="$VALADRIEN_OS_PUBLIC_URL" \
      -e VALADRIEN_OS_HOME="/valadrien-os" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "valadrien-os@${VALADRIEN OS_VERSION}" auth bootstrap-ceo --data-dir "$VALADRIEN_OS_HOME" --base-url "$VALADRIEN_OS_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
post_json_with_cookies() {
  ...
  curl -sS \
    ...
    -H "Origin: $VALADRIEN_OS_PUBLIC_URL" \
    -X POST \
    "$url" \
    --data "$body"
}
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$VALADRIEN_OS_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$VALADRIEN_OS_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$VALADRIEN_OS_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$VALADRIEN_OS_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$VALADRIEN_OS_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$VALADRIEN_OS_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg VALADRIEN OS_VERSION="$VALADRIEN OS_VERSION" \
...
echo "    Public URL: $VALADRIEN_OS_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $VALADRIEN_OS_DEPLOYMENT_MODE/$VALADRIEN_OS_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/valadrien-os-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$VALADRIEN_OS_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $VALADRIEN_OS_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$VALADRIEN_OS_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $VALADRIEN_OS_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"

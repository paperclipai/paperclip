...
IMAGE_NAME="${IMAGE_NAME:-odysseus-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
ODYSSEUS_AI_VERSION="${ODYSSEUS_AI_VERSION:-latest}"
...
ODYSSEUS_DEPLOYMENT_MODE="${ODYSSEUS_DEPLOYMENT_MODE:-authenticated}"
ODYSSEUS_DEPLOYMENT_EXPOSURE="${ODYSSEUS_DEPLOYMENT_EXPOSURE:-private}"
ODYSSEUS_PUBLIC_URL="${ODYSSEUS_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@odysseus.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-odysseus-smoke-password}"
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_ODYSSEUS_AI_VERSION=%q\n' "$ODYSSEUS_AI_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e ODYSSEUS_DEPLOYMENT_MODE="$ODYSSEUS_DEPLOYMENT_MODE" \
      -e ODYSSEUS_DEPLOYMENT_EXPOSURE="$ODYSSEUS_DEPLOYMENT_EXPOSURE" \
      -e ODYSSEUS_PUBLIC_URL="$ODYSSEUS_PUBLIC_URL" \
      -e ODYSSEUS_HOME="/odysseus" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "odysseus@${ODYSSEUS_AI_VERSION}" auth bootstrap-ceo --data-dir "$ODYSSEUS_HOME" --base-url "$ODYSSEUS_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$ODYSSEUS_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$ODYSSEUS_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$ODYSSEUS_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$ODYSSEUS_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$ODYSSEUS_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$ODYSSEUS_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg ODYSSEUS_AI_VERSION="$ODYSSEUS_AI_VERSION" \
...
echo "    Public URL: $ODYSSEUS_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $ODYSSEUS_DEPLOYMENT_MODE/$ODYSSEUS_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/odysseus-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$ODYSSEUS_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $ODYSSEUS_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$ODYSSEUS_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $ODYSSEUS_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"

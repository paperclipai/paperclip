...
Environment overrides:
  ...
  ODYSSEUS_HOST=127.0.0.1    Host passed to the onboarded server
  ODYSSEUS_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/odysseus-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/odysseus-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export ODYSSEUS_HOST="${ODYSSEUS_HOST:-127.0.0.1}"
export ODYSSEUS_PORT="${ODYSSEUS_PORT:-3100}"
export ODYSSEUS_OPEN_ON_LISTEN="${ODYSSEUS_OPEN_ON_LISTEN:-false}"
...
echo "ODYSSEUS_HOST: $ODYSSEUS_HOST"
echo "ODYSSEUS_PORT: $ODYSSEUS_PORT"
...
env \
  ...
  HOST="$ODYSSEUS_HOST" \
  PORT="$ODYSSEUS_PORT" \
  ODYSSEUS_OPEN_ON_LISTEN="$ODYSSEUS_OPEN_ON_LISTEN" \

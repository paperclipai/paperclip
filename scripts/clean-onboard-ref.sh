...
Environment overrides:
  ...
  PC_DATA=/tmp/data           AiTeamCorp data dir to use
  AITEAMCORP_HOST=127.0.0.1    Host passed to the onboarded server
  AITEAMCORP_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/aiteamcorp-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/aiteamcorp-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export AITEAMCORP_HOST="${AITEAMCORP_HOST:-127.0.0.1}"
export AITEAMCORP_PORT="${AITEAMCORP_PORT:-3100}"
export AITEAMCORP_OPEN_ON_LISTEN="${AITEAMCORP_OPEN_ON_LISTEN:-false}"
...
echo "AITEAMCORP_HOST: $AITEAMCORP_HOST"
echo "AITEAMCORP_PORT: $AITEAMCORP_PORT"
...
env \
  ...
  HOST="$AITEAMCORP_HOST" \
  PORT="$AITEAMCORP_PORT" \
  AITEAMCORP_OPEN_ON_LISTEN="$AITEAMCORP_OPEN_ON_LISTEN" \

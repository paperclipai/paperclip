...
usage() {
  cat <<'EOF'
Usage: configure-cloud-reviewer-default.sh [--apply] [--agent-id <id>]

Dry-run is the default. --apply is intentionally required because switching the
Cloud Reviewer lane is a Tier-3 model-routing change.
...
if [[ -n "$agent_id" ]]; then
  ...
  matches="$(jq '[.[] | select(.name | startswith("Cloud Reviewer"))]' <<<"$agents_json")"
  match_count="$(jq 'length' <<<"$matches")"
  if [[ "$match_count" != "1" ]]; then
    echo "Expected exactly one Cloud Reviewer agent, found $match_count; pass --agent-id to disambiguate." >&2
    exit 1
  fi
...
if [[ "$current_name" != Cloud\ Reviewer* ]]; then
  echo "Refusing to update non-Cloud-Reviewer agent '$current_name'." >&2
  exit 1
fi
...
desired_payload='{"adapterType":"claude_local","adapterConfig":{"model":"claude-opus-4-8"}}'

echo "Cloud Reviewer: $current_name ($current_id)"
echo "Current adapter/model: $current_adapter / $current_model"
echo "Desired adapter/model: claude_local / claude-opus-4-8"
...
jq -e '
  .adapterType == "claude_local" and
  .adapterConfig.model == "claude-opus-4-8"
' <<<"$updated_json" >/dev/null

echo "Applied and verified: claude_local / claude-opus-4-8"

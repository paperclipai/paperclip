...
const DEFAULT_QUERY = "\"Co-Authored-By: Odysseus <noreply@odysseus.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/odysseus-commit-metrics-cache.json");
...
const ODYSSEUS_EMAIL = "noreply@odysseus.ing";
const ODYSSEUS_NAME = "odysseus";
...
function printHelp() {
  console.log(`Usage: tsx scripts/odysseus-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === ODYSSEUS_EMAIL) || (displayName && displayName.toLowerCase() === ODYSSEUS_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Odysseus commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "odysseus-commit-metrics",

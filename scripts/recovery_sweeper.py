...
Levers (governance-safe, per SAG-3162 / SAG-3082 3-invariants):
  ...
  - Transient agent `error`: the server-side routine handler performs the
    board-authorized exact error->active mutation, then passes per-agent outcomes
    here for the digest. This artifact never holds a privileged agent API key.
...
# --- nudge primitives --------------------------------------------------------
# Agent error recovery is deliberately server-side; this routine remains a
# least-privilege reporting/nudge artifact and never mutates agent routes.

def error_recovery_outcomes():
    """Read server-side exact error->active outcomes for digest inclusion.
    Malformed input is surfaced once; it never triggers a retry loop."""
    raw = os.environ.get("PAPERCLIP_RECOVERY_SWEEPER_ERROR_OUTCOMES_JSON", "")
    if not raw:
        return {"reactivated": [], "failures": []}
    try:
        parsed = json.loads(raw)
    except Exception:
        return {"reactivated": [], "failures": ["server supplied malformed error recovery outcomes"]}
    if not isinstance(parsed, dict):
        return {"reactivated": [], "failures": ["server supplied invalid error recovery outcomes"]}
    return {
        "reactivated": [item for item in parsed.get("reactivated", []) if isinstance(item, str)],
        "failures": [item for item in parsed.get("failures", []) if isinstance(item, str)],
    }
...
def sweep():
    out = {"mode": MODE, "at": iso(),
           "errorAgentsReactivated": [], "errorAgentFailures": [],
           "runsNudged": [], "handoffsRequested": [],
    ...
    # The server handler performs bounded, atomic exact error->active updates before
    # this script runs. Include every outcome in this digest, and surface any error
    # that appeared after that bounded snapshot without attempting a second mutation.
    outcomes = error_recovery_outcomes()
    out["errorAgentsReactivated"].extend(outcomes["reactivated"])
    out["errorAgentFailures"].extend(outcomes["failures"])
    for agent_id in outcomes["reactivated"]:
        esc_doc.append(f"Agent `{agent_id}` reactivated (`error` -> `active`)")
    for failure in outcomes["failures"]:
        esc_doc.append(f"Agent error recovery failed: {failure}")
    ...
    for a in agents:
        ...
        err_surfaced += 1
        esc_doc.append(f"Agent **{nm}** remains in error (outside this bounded recovery snapshot; surfaced-only)")
    if err_surfaced:
        out["surfacedOnly"].append(f"{err_surfaced} error agents surfaced-only after recovery snapshot")

...
export const IGNORED_SPECS = ["in-feed-native/**", "multi-user.spec.ts", "multi-user-authenticated.spec.ts", "runtime-service*.spec.ts"];
...
export function listE2eSpecs(e2eDir = E2E_DIR, repoRoot = REPO_ROOT) {
  return readdirSync(e2eDir)
    .filter((entry) => entry.endsWith(".spec.ts") && !IGNORED_SPECS.some(pattern => path.matchesGlob(entry, pattern)))

...
- A pre-existing immutable Linear origin with no import state has one bounded first-link adoption window. It must already be unassigned: preview records `first_link_assignment_conflict` for an agent- or user-assigned origin, and apply rejects it before creating origin state or changing assignment. Operators must explicitly unassign it through Paperclip before retrying. Apply may adopt a declared source parent only when the current Paperclip parent is null, and may adopt declared source blockers only when the current Paperclip blocker set is empty. Any established parent or blocker state, including Paperclip-native relations, is preserved; mismatches record `parent_relation_drift` or `blocker_relations_drift` rather than overwriting current state.
...
- Apply creates new imported issues unassigned and accepts first-link only for already-unassigned issues. Later reconciliation preserves Paperclip-owned assignment state and never invokes the wake path. Its `assignments=0` and `wakes=0` report describes import-owned behavior rather than a racy company-wide assignment or wake-count delta, so unrelated concurrent processing cannot invalidate an import.
...
- Run and item records retain source version/timestamp, proposed/current/applied values, conflicts, failures, relations, comments, assignment count, and wake count. Current and applied item snapshots report the persisted agent/user assignment columns; `staged` is true only while both are null. Reconciliation preserves assignments made in Paperclip after origin state exists and still records zero import-owned assignments and wakes. Request schemas do not accept provider credentials, and persisted failures use a bounded generic message.

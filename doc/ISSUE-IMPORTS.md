...
- Preview reports proposed blocker source IDs alongside current blocker issue/source IDs. Every existing Linear-linked origin is checked, including a first import-state link, and a mismatch records `blocker_relations_drift`. Paperclip-native blocker issue IDs count as current relation state even though they have no Linear source ID. Apply preserves the current Paperclip blocker set and records the reconciliation result.

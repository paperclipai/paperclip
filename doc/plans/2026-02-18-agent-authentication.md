...
Agents need API keys to authenticate with Ironworks. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Ironworks
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Ironworks generates a token and passes it directly to the agent
...
`IRONWORKS_API_URL`, `IRONWORKS_AGENT_ID`, `IRONWORKS_COMPANY_ID`. We need to
add a `IRONWORKS_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Ironworks CLI
...
```
ironworks auth login
# Opens browser -> user confirms -> token stored at ~/.ironworks/credentials
```
...
managed by the Ironworks server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Ironworks.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Ironworks SKILL.md (or a link to it)
   - What information Ironworks needs from the agent (e.g. webhook URL, adapter
...
5. Ironworks stores the pending registration.
...
7. On approval, Ironworks provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Ironworks issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.PLACEHOLDER_IRONWORKS_DOT_ING/skills/ironworks/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Ironworks should send you heartbeats",
...
1. Generate a Ironworks invite link for the company.
...
4. A Ironworks company member approves the new agent.
5. Ironworks begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `IRONWORKS_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `ironworksai auth login` for developer-managed remote agents.                                      |

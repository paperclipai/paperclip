...
`VALADRIEN_OS_API_URL`, `VALADRIEN_OS_AGENT_ID`, `VALADRIEN_OS_COMPANY_ID`. We need to
add a `VALADRIEN_OS_API_KEY` (JWT) to the set of injected env vars.
...
```
valadrien-os auth login
# Opens browser -> user confirms -> token stored at ~/.valadrien-os/credentials
```
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `VALADRIEN_OS_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `valadrien-os auth login` for developer-managed remote agents.                                      |

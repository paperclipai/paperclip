...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Ironworks plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Ironworks.
...
- package: `@ironworksai/plugin-kitchen-sink-example`
- plugin id: `ironworks.kitchen-sink-example` or `ironworks-kitchen-sink-example`

Recommendation: use `ironworks-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Ironworks event such as `issue.created`
...
Recommendation: default to a Ironworks-local endpoint or a stable public echo endpoint to avoid flaky docs.

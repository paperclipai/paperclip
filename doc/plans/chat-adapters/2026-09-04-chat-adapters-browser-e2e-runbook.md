...
- **Final implementation revision:** `83018c688` contains the Discord log-redaction and setup-copy corrections. Its parent merge `da8f83d6c9befe7bf958f6d9cf12a95fc7e59e88` passed the five-provider deterministic suite and the focused merged-build live checks described below; the implementation revision then passed the 42-test Discord adapter/runtime subset, the 34-test Discord/OpenAPI/UI contract subset, and server/UI typechecks.
- **Slack:** broad live evidence covers one-root/one-thread/one-task behavior, DM answer continuation, ordered follow-ups, exact final presentation, reaction add/remove, lifecycle edits/deletes, files, pause/resume, and disabled-resource recovery. On the merged build, the exact `SLACK-MERGED-C-0906` and `SLACK-MERGED-D-0906` responses passed on healthy ingress, while delayed-event recovery delivered the earlier `SLACK-MERGED-A-0906` and `SLACK-MERGED-B-0906` pair in order after tunnel rotation. It is not a complete S1–S7 pass; governance, injected ambiguous delivery, reinstall, and cleanup remain incomplete.
- **Telegram:** broad live evidence covers private chat, group/topic isolation, FIFO/bursts, commands, edits/reactions, documents, native confirmation continuation, and exact once-only final presentation. After rotating the expired test tunnel on the merged build, a fresh task returned exact `TELEGRAM-MERGED-A-0906` once and edited its working placeholder in place. It is not a complete TG1–TG6 pass; identity governance, media boundaries, rate-limit recovery, token rotation, and cleanup remain incomplete.

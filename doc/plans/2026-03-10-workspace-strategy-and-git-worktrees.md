...
`PAP-447` asks how Valadrien OS should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Valadrien OS instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Valadrien OS:

- some operators use Valadrien OS to manage Valadrien OS and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Valadrien OS
- Claude and Codex expose different built-in affordances, so Valadrien OS should not overfit to one tool

## Core Product Decision

Valadrien OS should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Valadrien OS
...
- Valadrien OS should not assume that every adapter can see or use a host filesystem path directly
...
If Valadrien OS makes worktrees agent-first, it will blur:
...
- Valadrien OS core resolves and records execution workspace state
...
That gives Valadrien OS room to support:
...
- Valadrien OS handles workspace runtime services under the hood using project/workspace policy
...
Once Valadrien OS is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Valadrien OS should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Valadrien OS cannot infer
...
- when issue moves to done, should Valadrien OS auto-commit?
- should it auto-push?
...
Valadrien OS already has the right foundation for a project-first model.
...
For server-spawned local adapters, Valadrien OS already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Valadrien OS should mirror directly

Implication:

- for `codex_local`, Valadrien OS should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Valadrien OS should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Valadrien OS should separate:
...
Valadrien OS should be able to express intentions such as:
...
These are not specific to Valadrien OS. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".valadrien-os/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Valadrien OS-specific `devServer` flag.
...
Valadrien OS should distinguish between:
...
Valadrien OS should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Valadrien OS should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Valadrien OS
...
For runtime services specifically, Valadrien OS should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Valadrien OS-only dev-server flags
...
3. Inject resolved execution workspace details into `context.valadrienOsWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Valadrien OS instance can be reused through env/config injection when appropriate
...
- Valadrien OS can decide whether to reuse or start a fresh service deterministically
...
3. Test against Valadrien OS-on-Valadrien OS workflows first.
...
7. Workspace runtime services are modeled generically, not as Valadrien OS-specific dev-server toggles.

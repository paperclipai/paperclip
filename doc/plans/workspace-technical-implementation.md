...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/valadrien-os-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `VALADRIEN_OS_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `VALADRIEN_OS_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `provider text not null`
  - `valadrien-os | github | vercel | s3 | custom`
...
- use `VALADRIEN_OS_EXECUTION_TOPOLOGY` as a defaulting guardrail

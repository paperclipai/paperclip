...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/odysseus-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `ODYSSEUS_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `ODYSSEUS_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `provider text not null`
  - `odysseus | github | vercel | s3 | custom`
...
- use `ODYSSEUS_EXECUTION_TOPOLOGY` as a defaulting guardrail

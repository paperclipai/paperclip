...
- Contract comparison ignores server-managed revision metadata, so editing human-readable text around an unchanged legacy inline contract does not create a revision or fail against the freeze. Adding a first contract after execution starts is still a contract mutation and is rejected.
- Contract updates lock and re-read the issue row before checking execution state. Child creation and topology-changing updates lock the affected issue/parent rows so service-mediated concurrent writes cannot cross the one-level or ten-lane limits.
- Existing `suggest_tasks` rows with pre-schema contract payloads remain readable through list/get. Acceptance reparses the stored proposal against the current schema and topology rules before claiming it, so legacy readability does not weaken new execution.

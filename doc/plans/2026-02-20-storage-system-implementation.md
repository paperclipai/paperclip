...
Add a single storage subsystem for ValAdrien OS that supports:
...
- API serves attachment bytes through authenticated ValAdrien OS endpoints.
...
- Default local path is under instance root: `~/.valadrien-os/instances/<instanceId>/data/storage`.
...
- `valadrien-os onboard` writes a valid `storage` config block by default.
- `valadrien-os configure --section storage` can switch between local and s3 modes.
...
- `valadrien-os doctor` reports actionable storage status.

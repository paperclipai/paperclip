...
Today release smoke testing for published Ironworks packages is manual and shell-driven:
...
- Docker
- `npx ironworksai@canary`
- `npx ironworksai@latest`
...
- runs `ironworksai@${IRONWORKSAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@ironworks.local`
- password: `ironworks-smoke-password`
...
- the suite passes locally against both:
  - `IRONWORKSAI_VERSION=canary`
  - `IRONWORKSAI_VERSION=latest`
...
1. A published `ironworksai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `ironworksai@latest` Docker install can be smoke-tested by Playwright in CI.

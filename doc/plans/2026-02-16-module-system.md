...
ValAdrien OS's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends ValAdrien OS's API, UI, and data model | Yes |
...
modules/
  observability/
    valadrien-os.module.json     # manifest (required)
    src/
...
### Manifest (`valadrien-os.module.json`)
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@valadrien-os/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@valadrien-os/core";
...
```
1. Scan modules/ directory for valadrien-os.module.json manifests
...
```jsonc
// valadrien-os.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@valadrien-os/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@valadrien-os/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:valadrien-os/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.TODO_DOMAIN/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm valadrien-os store list                    # browse available modules and templates
pnpm valadrien-os store install <module-id>     # install a module
pnpm valadrien-os store import <template-id>    # import a company template
pnpm valadrien-os store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one ValAdrien OS instance | `server:started` |
...
4. **Module config** — `valadrien-os.config.json` with per-module config, env var interpolation
...
10. **`@valadrien-os/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema

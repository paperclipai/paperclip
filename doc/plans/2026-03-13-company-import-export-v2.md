...
`valadrien-os.manifest.json` is not part of the future package direction.
...
- omit duplicated inline prompt content from `.valadrien-os.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.valadrien-os.yaml` alongside the base package
...
- `valadrien-os company export <company-id> --out <path>`
- `valadrien-os company import <path-or-url> --dry-run`
- `valadrien-os company import <path-or-url> --target existing -C <company-id>`

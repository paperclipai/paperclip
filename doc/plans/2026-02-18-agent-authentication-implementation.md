...
- New config/env settings:
  ...
  - `VALADRIEN_OS_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `VALADRIEN_OS_AGENT_JWT_ISSUER` (default: `valadrien-os`)
  - `VALADRIEN_OS_AGENT_JWT_AUDIENCE` (default: `valadrien-os-api`)
...
1. In:
   ...
   inject `VALADRIEN_OS_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `VALADRIEN_OS_API_KEY`, do not overwrite it.
- Continue injecting:
...
- Local adapters authenticate without manual `VALADRIEN_OS_API_KEY` config.

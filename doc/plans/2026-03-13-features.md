...
## 2) Operator command surface, not generic chat
...
- On approval page: “Why are you asking to hire?” → appends a operator comment to the approval context.
...
- comment metadata: `comment.intent = hint | correction | operator_question | operator_decision`
...
- Operator can tell what is happening without reading shell commands.
...
- multi-operator governance logic in first cut
...
Add operator interjections as resumable input events:
...
- A operator comment can resume an active session instead of spawning a fresh one.
...
- auto-pause at 100%
- visible audit trail
- explicit operator override to continue
...
**P1** 5. Operator command surface 6. Visibility/explainability layer 7. Auto mode + interrupt/resume 8. Minimal multi-user collaboration
...
- **Keep** Paperclip as the operator-level control plane.

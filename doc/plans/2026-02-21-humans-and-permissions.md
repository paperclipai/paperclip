...
- local mode that still feels instant (`npx valadrien-os run` and go)
...
2. CLI command `pnpm valadrien-os auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm valadrien-os onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm valadrien-os onboard`).
...
- expose key instance settings in API + CLI (`valadrien-os configure` / `valadrien-os onboard`)
...
7. `pnpm valadrien-os onboard` outputs a CEO onboarding invite URL when bootstrap is pending.

...
Productivity-review issues are visible ops/monitoring alerts linked to the source issue. When a same-company agent has a trusted board-managed `productivityReviewRouting` permission (or the equivalent ops-routing permission), is invokable, is budget-unblocked, and is not the source issue assignee, Paperclip routes the review there; otherwise it falls back to the normal manager/creator/project-lead/CTO/CEO owner chain. Free-text agent names, roles, and capabilities are not trusted routing signals. The review title and `ops: monitoring alert` label should make clear that it is not noise. The assigned Foreman/pi-orchestrator path must investigate and fix the source issue, handoff, blocker, or loop, then leave a disposition comment with the source outcome before closing the productivity review.

Default productivity-review thresholds are: 10 terminal issue-linked runs without an assignee run comment; 1 hour long-active duration; high churn at 10 runs or assignee-run comments in 1 hour or 30 in 6 hours; refresh comments at most once per 1 hour and at most 3 times per open review; resolved-review snooze for 6 hours; and at most 3 created productivity-review issues per source issue per 24 hours. The service runs during the shared heartbeat recovery pass, whose default scheduler interval is 30 seconds (`HEARTBEAT_SCHEDULER_INTERVAL_MS`, minimum 10 seconds).

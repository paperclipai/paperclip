...
- **Cross-family review** is an invariant: Anthropic workers get OpenAI reviewers, OpenAI workers get Anthropic reviewers, mechanical (meta/deepseek) workers get one of the two authority families. A reviewer is never an agent that authored any attempt on the issue. Mandatory-review classes cannot be waived by a rule or an override. When no valid reviewer exists, the review is not created and the issue is parked `blocked` with a board-owned unblock descriptor; it is never self-reviewed or silently waived.
...
- **Dispatch** claims one pool slot under the profile row lock, assigns the issue to the selected agent, and wakes it through the ordinary heartbeat scheduler with `routeDecisionId`, `executionProfileId`, and `routeRole` in the run context. Dispatch refuses while a live run exists. Claims are released when their run reaches a terminal status or by an explicit board release, never by elapsed time.

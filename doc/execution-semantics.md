...
### Dispatch contention is a resource wait, not a failure

Two pieces of work must never mutate one working tree at the same time, and contention between them is not a defect in either. Paperclip therefore models every form of "the writer is already taken" as one resource wait with one terminal shape, and never as a failed run that spends a bounded retry budget.

The wait has one native shape: the run is cancelled with `errorCode: workspace_busy`, the wait is recorded in its `resultJson.workspaceBusy` evidence, and a scheduled retry with `retryReason: workspace_busy` is queued through the ordinary retry machinery. The retry keeps rescheduling while a live owner exists. A holder is never evicted because of its age, and there is no attempt ceiling: an owner stops counting only when it is no longer a live path — a native holder that has been silent past recovery's own `RECOVERY_ACTIVE_RUN_OUTPUT_SUSPICION_THRESHOLD_MS` bar, or a contained runner whose container self-terminates when its launcher dies (PID1 tears the namespace down on control-stream EOF or heartbeat expiry).

The wait is produced in three places, all of which converge on that shape:

- **Native shared-workspace gate (pre-dispatch).** Another running run holds the issue's shared project workspace. Existing behaviour, unchanged in kind.
- **Native canonical writer-root admission (pre-dispatch, opt-in).** When an agent's adapter config provisions an `executionResourceResolver`, Paperclip invokes that fixed resolver before dispatch and receives a writer-resource receipt: `access` (`isolated`, `read_only`, or `exclusive`) plus the canonical writer-root identity of the lane the run will touch. Every non-isolated receipt reserves that root atomically — a cross-company advisory lock plus the run's own persisted context — so two runs sharing one physical writer root are serialized by native scheduling even when they belong to different projects or companies. Compatibility follows the physical rule: an `exclusive` run waits for any live holder of the root, a `read_only` run waits only for a live writer, and two `read_only` runs on one root run concurrently. An `isolated` run touches no canonical lane and reserves nothing. A live holder whose receipt has no canonical identity is treated as touching the root, because assuming otherwise would let an unidentified holder be overlapped. An adapter without the resolver keeps the previous behaviour exactly. Resolution is per dispatch attempt, and a changed `configIdentity` is recorded rather than silently reused.
- **Contained-runner refusal (race backstop).** The framework's launcher can still refuse after native admission — another container owns the writer-root name, or the supervisor's canonical target lease is already held or the admitted revision moved before that lease was taken. The runner reports that as a structured envelope with a reserved exit code, and the legacy `process` adapter maps it onto the same native deferral. The classification requires the reserved code, the envelope, a known reason and the matching run identity: a worker cannot manufacture a deferral, and an unrelated failure that merely exits with the same code stays a failure.

Because a resolver is operator provisioning, a resolver that cannot produce a valid receipt stops the run as a configuration-incomplete blocker for its human owner instead of starting work in an unverified workspace.

### Bounded continuation of a timed-out contained run

A contained run can hit its execution timeout with real work behind it. Restarting the task from scratch redoes that work and can repeat external effects whose receipts already exist; raising the timeout moves the same wall without bounding anything. A run that timed out after reaching the model and making progress therefore continues the **same worker session** from its checkpoint, bounded by its own small attempt budget, with an instruction that forbids repeating already-recorded external actions and states that the timeout is unchanged. A timeout without that evidence keeps the ordinary timeout handling.
...
## 13.1 Task-Attempt Routing

Task-attempt routing selects which model-bound executor identity performs an attempt. It never changes an agent's adapter model, never switches models inside a session, and never reinterprets whether a PR is ready or merged. The tables are `execution_profiles`, `route_rules`, `route_decisions`, and `route_pool_claims`; the deterministic policy is `server/src/services/routing/policy.ts`. Issue JSON (`executionPolicy`/`executionState`) and run JSON were rejected as carriers because they are mutable and run-scoped, while route decisions must be append-only, revision-linked, and exist before any run.

- An **execution profile** names an existing agent, a typed provider family (`anthropic | openai | meta | deepseek`), the requested model/effort, the roles it may fill (`worker | advisor | reviewer | rescuer`), and its own concurrency cap. Capacity belongs to the profile pool, never to a project. The profile model must match the agent's configured model; a later drift refuses dispatch (`execution_profile_model_drift`) instead of silently running a different model.
- A **route rule** binds one task class to a worker, optional advisor, reviewer, reviewer fallback, rescue profile, and bounds. Rule writes reject same-family or self reviewers. The default matrix is seeded from explicit operator bindings and never overwrites tuned rules.
- A **route decision** is immutable. Revision 1 is `initial`; only a typed `escalation`, `fallback`, `override`, or `rescue` appends a new revision that names the decision it supersedes. Ordinary retries reuse the current decision. Every selected role is snapshotted (profile, agent, family, model, effort), so profile edits cannot rewrite history. Refusals persist as explicit states: `classification-required`, `no-capable-worker`, `reviewer-family-conflict`, `reviewer-unavailable`, `budget-limited`, `escalation-required`.
- **Cross-family review** is an invariant: Anthropic workers get OpenAI reviewers, OpenAI workers get Anthropic reviewers, mechanical (meta/deepseek) workers get one of the two authority families. A reviewer or advisor is never an agent that authored any attempt on the issue, and no agent holds two authority roles on one attempt. Mandatory-review classes cannot be waived by a rule or an override. Dispatch installs the review as the issue's existing execution-policy review stage whose participants are the routed reviewer and its opposite-family fallback only, so `in_review`/`done` cannot complete until that participant records a decision; an operator-authored execution policy is left untouched and the omission is audited. Executing the fallback reviewer first appends a typed `fallback` revision naming it. When no valid reviewer exists at route time or review time, the decision is persisted in its refusal state and the issue is parked `blocked` with a board-owned unblock descriptor whenever no live run exists; it is never self-reviewed or silently waived.
- **Roles are separate authorities.** The worker writes; the advisor answers one bounded read-only question per attempt and its answer is evidence, never approval; the reviewer owns its own review child issue (origin kind `route_review`, parent blocked by it, reviewer posts findings and marks it done per §6); the rescuer is a fresh writable attempt after a typed escalation. A rescue recomputes the reviewer against the rescuer's family and excludes every prior author.
- **Dispatch** claims one pool slot under the profile row lock, assigns the issue to the selected agent, and wakes it through the ordinary heartbeat scheduler with `routeDecisionId`, `executionProfileId`, `routeRole`, and the attempt bounds in the run context. The native `bindWake` hook re-checks the decision revision and claim under the issue lock inside the admission transaction, and `bindRun` links the claim to the run atomically. Dispatch refuses while a live run exists or when the agent's configured model drifted from the snapshot. A wake parked on a durable carrier keeps its slot; a refused wake releases it. Claims are otherwise released when their run reaches a terminal status or by a typed board release, never by elapsed time.
- **Escalation** is typed (repeated failure fingerprint, architectural review rejection, dirty or unverified terminal attempt, scope crossing a critical risk, no artifact within wall time, no durable action path, repeated correction against a rejected invariant). A single timeout, formatter failure, or compiler error is not an escalation. The ladder is one advisor round, then one rescue, then human attention.
- **Authority.** Board operators configure profiles and rules and hold dispatch, escalate, rescue, override, and release. Agents may submit classification facts and request the review a decision already requires; they cannot choose or waive a worker, reviewer, or advisor, and an agent re-route can never lower the risk class or review requirement an earlier decision established (`route_downgrade_requires_board`). Every revision is fenced on the revision the policy was evaluated against (`route_revision_conflict`); overrides preserve the current worker unless a worker is named, are invariant-checked, and audited.
- Every mutation writes activity (`execution_profile.*`, `route_rule.*`, `route_decision.*`, `route_attempt.*`, `route_review.*`, `route_claim.*`) with ids, families, models, and reason codes only. Dispatch writes a `route.decision` run-log event on the woken run.

Routing composes with, and never replaces, the existing recovery, advisor, review, and delivery lifecycles. The bounded advisor round itself is delivered through the native advisor interaction contract; routing only selects the advisor profile and bounds the round count.

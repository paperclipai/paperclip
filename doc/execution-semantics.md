...
A review created straight as an `unreported_completion` carries the same recovery marker: its wake is recorded as intended before delivery and confirmed after, so a first wake that never lands is retried on a later pass instead of leaving finished work with an untriggered assignee. This is scoped to `unreported_completion`; a stall review's wake behaviour is unchanged.

Evidence can also arrive *after* a review was written — the run that finished the work commits, then dies, and a stall review already exists. On the next reconcile pass, an open `stall` review whose issue now has completion evidence is rewritten in place: new title and body, priority lowered, reassigned from the manager to the source assignee, and the assignee woken. Otherwise a manager would keep owning a review whose decision menu says reassign/decompose against work that demonstrably exists. This runs before the refresh-comment throttle, which is there to limit comment spam rather than to defer a wrong instruction. The reclassification counts as finished only once that wake exists: if it cannot be delivered, the review is restored to the stall it was, because a review that already reads `unreported_completion` would never meet the retry condition again and the finished work would sit assigned to an agent nobody triggers. That rollback only covers a wake that fails while the process is alive; a crash between the review update and the wake cannot be caught, so it is made recoverable instead. Deciding to reclassify and claiming the cycle happen under a row lock on the review, so two overlapping reconcile passes cannot each mint their own cycle and enqueue a wake under a different key — the second reads the first's marker and resumes that cycle. The reclassification records its intent in the activity log (`wakeDelivered: false`) *before* the review is touched and confirms it (`wakeDelivered: true`) *after* the wake. A later pass retries whenever the newest such marker is unconfirmed, whatever killed the previous attempt; re-running is idempotent, and the explanatory comment is written only on the first, genuine reclassification. The marker carries a cycle id that the resumed attempt reuses as the wake's idempotency key, so a crash *after* a successful wake and before its confirmation cannot produce a second report-and-close run. `agent_wakeup_requests.idempotency_key` carries no unique constraint and the enqueue path does not read it, so the resume looks that row up itself rather than relying on the key being enforced downstream. Existence of the row is not enough: a wake can be persisted and then dropped (`skipped`, `cancelled`, `failed`) without ever producing a run, so delivery is claimed only for a request that is still live, was coalesced into another wake, or already has a run. An unrecognised state re-wakes — at worst a duplicate the agent no-ops on, against stranding finished work for good. If no invokable owner can be resolved there is nobody to tell, so the reclassification rolls back rather than recording a delivery that never happened — the manager-owned review stands until a candidate becomes invokable again. The reverse direction is deliberately *not* automatic: if evidence disappears, handing the review back to a manager with the destructive actions re-enabled is a call for a human reading the refresh comment.

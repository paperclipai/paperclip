...
Because that reachability is a precondition and not a per-run choice, the server validates it once at startup rather than letting each run discover it. As soon as the listener is up, Paperclip probes `GET /api/health` against the resolved runtime API URL and requires a response carrying that route's own shape — a `status` of `ok` or `unhealthy` plus at least one field only that route serves. Any HTTP status is acceptable, including `503 unhealthy` — the probe establishes that the board API is on the other end, not that it is healthy. A redirect or an HTML body means the URL names a UI origin or an auth proxy rather than the API, and the server logs the rejected URL with the reason and falls back to the first runtime API candidate that does answer, ending with the loopback origin. If nothing answers, the server keeps the configured value and logs the failure: external clients may reach an origin the server process cannot, so taking the API down is the worse outcome. Runs spawned in the window before the listener comes up can still inherit an unvalidated URL.

The configured URL and a fallback are held to different strictness, because a spawned run attaches its bearer run token to whatever origin wins the probe. The configured URL is the operator's explicit choice and is what every run inherited before this check existed, so probing it can only reject it — never widen where credentials go — and the board health shape is signal enough to keep it. A fallback is chosen by the server, so promoting one is the only way this check could send a run somewhere new; every fallback candidate is the server's own bound port on one of its own hostnames, so a fallback must additionally report the same build `commit` this process is running before it may win. `commit` rides every variant of the health route, including the redacted one an unauthenticated probe receives, so checking it needs no credential. When the server cannot read its own commit, fallbacks are accepted on the shape check alone — still narrower than the unconditional trust that preceded startup validation.

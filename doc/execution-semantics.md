...
- **Write-time:** setting an issue to `blocked` requires at least one unresolved blocker edge or a first-class external owner/action recorded on the issue. A bare `blocked` with neither is invalid.
- **Transition-time:** any transition that leaves a `blocked` issue with zero unresolved blocker edges must queue the idempotent `issue_blockers_resolved` wake for that issue.

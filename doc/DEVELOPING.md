...
## Zenova Release Promotion Gate

For the HenkDz production fork, promotion is staging-first and always follows
this exact order:

1. Build and record one immutable candidate SHA.
2. Deploy that exact SHA only to `paper-dev.zenova.id`.
3. Verify `paper-dev.zenova.id` health reports the candidate and complete the
   required smoke checks there.
4. Stop and wait for explicit written sign-off from Henk that identifies the
   validated candidate and its health/smoke evidence.

`paper.zenova.id` is production. It must never be auto-deployed by CI, an
agent, a script, or a release workflow. Written sign-off authorizes only a
separate, deliberate human promotion; it does not turn a staging deployment
into an automatic production rollout.
...
Hermes is external-only in this fork. Install
`@henkey/hermes-paperclip-adapter` (or an approved `file:` development path)
through **Board → Adapter manager**; core has no Hermes dependency, built-in
registration, or dedicated onboarding flow. Once the package is loaded, its
adapter type remains `hermes_local`. Do not add a built-in `hermes_gateway`
path to `server/` or `ui/`.

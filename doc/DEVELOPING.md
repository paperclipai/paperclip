...
An isolated repair is not a live repair. After applying the exact independently verified commit, the operator records `repairCommit` and `activationEvidence` at `PUT /api/companies/:companyId/recovery-engineer/incidents/:incidentId/activation`. Only then can recovery claim a resume for each unchanged source generation. Dispatch is idempotent and recoverable after interruption; replay rechecks the original owner and all live gates. Queueing or starting a resume does not mark the source recovered, and recovery never marks a source task Done.

Recovery closes a source generation only when subsequent evidence proves that its original operation advanced. Incident outcome uses the newest generation of each source, so an older superseded failure cannot prevent a genuinely recovered incident from closing. A newly admitted failure prevents maintenance closure even if it races with reconciliation. Repeated unchanged failures remain explicit rather than being hidden by resume attempts.

Reviewed procedures remain diagnostic data. Record applicability and evidence when applying one, then record its observed outcome; a success cannot bypass applicability checks, and conflicting terminal outcomes are refused. Reuse evidence remains addressable after the source closes.

Verify with `pnpm exec vitest run server/src/services/recovery-engineer-policy.test.ts server/src/__tests__/recovery-engineer.test.ts` against embedded PostgreSQL; skipped database tests are not acceptance evidence. Apply all pending migrations, including recovery outcome migration 0252, with their generated snapshots and journal. Disabling the company configuration releases ordinary source recovery suppression without granting repair agents any additional authority.

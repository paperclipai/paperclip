...
Hermes is external-only in the HenkDz fork. Core does not depend on, import, or
register a Hermes adapter. Operators install
`@henkey/hermes-paperclip-adapter` (or an approved `file:` development path)
through **Board → Adapter manager**; the plugin supplies `hermes_local` after
it loads.
...
## 15.5 Zenova Staging-First Release Gate

For the HenkDz production fork, release promotion is a hard operational gate:

1. Record an immutable candidate SHA.
2. Deploy that SHA to `paper-dev.zenova.id` only.
3. Confirm health for that SHA and complete the required smoke checks on the
   staging host.
4. Stop for explicit written sign-off from Henk before any production action.

`paper.zenova.id` must never be auto-deployed. The written sign-off is tied to
the validated candidate and authorizes only a separate, deliberate human
promotion.

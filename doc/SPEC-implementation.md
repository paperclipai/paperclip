...
- source-scoped recovery routing is cause-keyed: bounded continuity and disposition repair may retry only the original agent; retryable provider-quota failures with explicit pre-provider bootstrap evidence and remaining incident budget create/reuse a normal-model `scheduled_retry` heartbeat run at the reset time without changing the issue monitor policy or denormalized monitor fields; a later quota failure for the same issue and agent updates that pending retry's `retryOfRunId`, reset time, and incident retry count instead of suppressing recovery; pre-existing duplicate quota waits reconcile under the issue lock to one eligible retry with the latest deadline and highest attempt count, choosing the newest scoped predecessor failure independently of retry creation/deadline order and retaining it in both the run and queued wake, cancelling redundant unclaimed rows/wakeups with audit history even when the current failure is already covered; uncertain or exhausted legacy executions require reconciliation before quota handling; every other exhausted or unsafe path creates/reuses a board-owned recovery action with `routingPolicy: board_escalation_no_takeover_v1` and no substitute-agent wake
- quota failures bearing `continue_conversation_v1` retain the two-retry automatic incident cap without creating a legacy reconciliation hold; their custom retry producer reuses scheduled-retry admission gates under the issue lock, propagates conversation policy to the surviving retry, and cancels unclaimed historical waits that become gated or exhausted. Explicit new conversations retain upstream continuation semantics.

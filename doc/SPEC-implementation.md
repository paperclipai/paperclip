...
- project env is merged into run environment for issues in that project and overrides conflicting agent env keys before Bizbox runtime-owned keys are injected
...
- external Postgres runtime pool is bounded and configurable with `BIZBOX_DB_POOL_MAX`, `BIZBOX_DB_IDLE_TIMEOUT_SECONDS`, and `BIZBOX_DB_CONNECT_TIMEOUT_SECONDS`
...
- `.paperclip.yaml` sidecar for Bizbox-specific fidelity
...
- recurring `TASK.md` entries use `recurring: true` in the base package and Bizbox routine fidelity in `.paperclip.yaml`
- Bizbox imports recurring task packages as routines instead of downgrading them to one-time issues

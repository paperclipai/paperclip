...
- source-scoped recovery routing is cause-keyed: bounded continuity and disposition repair may retry only the original agent; provider-quota failures create/reuse a normal-model `scheduled_retry` heartbeat run at the reset time without changing the issue monitor policy or denormalized monitor fields; a later quota failure for the same issue and agent updates that pending retry's `retryOfRunId` and reset time instead of suppressing recovery; every other exhausted or unsafe path creates/reuses a board-owned recovery action with `routingPolicy: board_escalation_no_takeover_v1` and no substitute-agent wake

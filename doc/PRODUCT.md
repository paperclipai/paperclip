...
AiTeamCorp is the control plane for autonomous AI companies. One instance of AiTeamCorp can run multiple companies. A **company** is a first-order object.
...
- **Adapter type + config** — how this agent runs and what defines its identity/behavior. This is adapter-specific (e.g., an OpenClaw agent might use SOUL.md and HEARTBEAT.md files; a Claude Code agent might use CLAUDE.md; a bare script might use CLI args). AiTeamCorp doesn't prescribe the format — the adapter does.
...
1. **Run a command** — AiTeamCorp kicks off a process (shell command, Python script, etc.) and tracks it. The heartbeat is "execute this and monitor it."
2. **Fire and forget a request** — AiTeamCorp sends a webhook/API call to an externally running agent. The heartbeat is "notify this agent to wake up." (OpenClaw hooks work this way.)
...
5. **Control plane, not execution plane.** AiTeamCorp orchestrates. Agents run wherever they run and phone home.
...
There are two runtime modes AiTeamCorp must support:
...
- Do not build a complete Jira/GitHub replacement. The repo/docs already position AiTeamCorp as organization orchestration, not focused on pull-request review.

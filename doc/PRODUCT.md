...
### Optional Apps Administration

Named MCP Gateways and Access Profile administration are Paperclip EE product surfaces. Core owns their stored data, runtime policy evaluation, and internal tool-gateway sessions so disabling or uninstalling EE cannot erase configuration or weaken enforcement. The canonical Paperclip EE plugin may make these administration surfaces available only while its lifecycle state is ready. In Community Edition, and during every non-ready EE lifecycle state, the server fails closed: navigation and direct routes are hidden, and management APIs return a stable feature-unavailable response. Re-enabling EE restores access to the preserved rows.

...
AiTeamCorp supports two runtime modes:
...
AiTeamCorp now treats **bind** as a separate concern from auth:
...
pnpm aiteamcorp onboard --yes --bind lan
pnpm aiteamcorp run --bind tailnet
```
...
When running `authenticated` mode, if the only instance admin is `local-board`, AiTeamCorp emits a startup warning with a one-time high-entropy claim URL.

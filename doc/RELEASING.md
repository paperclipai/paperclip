...
2. **npm** — `valadrien-os` and public workspace packages are published
...
```bash
npx valadrien-os@canary onboard
# or
npx valadrien-os@canary onboard --data-dir "$(mktemp -d /tmp/valadrien-os-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/ValDola-stack/valadrien-os/actions/workflows/release.yml)
...
```bash
VALADRIEN OS_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
VALADRIEN OS_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
```bash
gh workflow run release-smoke.yml -f valadrien_os_version=canary
gh workflow run release-smoke.yml -f valadrien_os_version=latest
```

Minimum checks:

- `npx valadrien-os@canary onboard` installs

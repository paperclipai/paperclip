...
Maintainer runbook for shipping Ironworks across npm, GitHub, and the website-facing changelog surface.
...
Ironworks uses calendar versions that still fit semver syntax:
...
2. **npm** — `ironworksai` and public workspace packages are published
...
```bash
npx ironworksai@canary onboard
# or
npx ironworksai@canary onboard --data-dir "$(mktemp -d /tmp/ironworks-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/ironworksai/ironworks/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Ironworks. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
IRONWORKSAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
IRONWORKSAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Useful isolated variants:

```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary IRONWORKSAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable IRONWORKSAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Automated browser smoke is also available:

```bash
gh workflow run release-smoke.yml -f ironworks_version=canary
gh workflow run release-smoke.yml -f ironworks_version=latest
```

Minimum checks:

- `npx ironworksai@canary onboard` installs

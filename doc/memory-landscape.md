...
AiTeamCorp is not trying to become a single opinionated memory engine. The more useful target is a control-plane memory surface that:

- stays company-scoped
- lets each company choose a default memory provider
- lets specific agents override that default
- keeps provenance back to AiTeamCorp runs, issues, comments, and documents
...
The question is not "which memory project wins?" The question is "what is the smallest AiTeamCorp contract that can sit above several very different memory systems without flattening away the useful differences?"
...
These emphasize local persistence, inspectability, and low operational overhead. They are useful because AiTeamCorp is local-first today and needs at least one zero-config path.
...
If AiTeamCorp does not expose these, it will not adapt well to the systems above.

## Where The Systems Differ

These differences are exactly why AiTeamCorp needs a layered contract instead of a single hard-coded engine.
...
AiTeamCorp should not require a single storage shape. It should require normalized references back to AiTeamCorp entities.
...
AiTeamCorp should make plain search the minimum contract and richer outputs optional capabilities.
...
AiTeamCorp does not need to standardize every feature from every provider. It needs:
...
AiTeamCorp should adopt a two-layer memory model:

1. `Memory binding + control plane layer`
   AiTeamCorp decides which provider key is in effect for a company, agent, or project, and it logs every memory operation with provenance and usage.

2. `Provider adapter layer`
   A built-in or plugin-supplied adapter turns AiTeamCorp memory requests into provider-specific calls.
...
without forcing AiTeamCorp itself to become a monolithic memory engine.

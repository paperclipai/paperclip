...
1. Start PrivateClip in auth mode.
```bash
...
5. Approve the join request in PrivateClip UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from PrivateClip invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in PrivateClip with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in PrivateClip UI that new issue exists.

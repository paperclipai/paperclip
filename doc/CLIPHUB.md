...
ClipHub is the public registry where people share, discover, and download Ironworks company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Ironworks what a package registry is to a programming language. Ironworks already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Ironworks — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Ironworks instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Ironworks spec. It contains:
...
**Install (fresh start):**
```
ironworks install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Ironworks instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Ironworks, export your company as a template, then publish:

```
ironworks export --template my-company
ironworks publish cliphub my-company
```
...
ClipHub is a **separate service** from Ironworks itself. Ironworks is self-hosted; ClipHub is a hosted registry that Ironworks instances talk to.
...
| **Ironworks CLI** | `ironworksai install`, `ironworksai publish`, `ironworksai cliphub sync` — built into Ironworks |
| **Ironworks UI** | "Browse ClipHub" panel in the Ironworks web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Ironworks) |
| Backend | TypeScript + Hono (consistent with Ironworks) |
| Database | PostgreSQL |
...
4. Run `ironworksai install cliphub:acme/lean-saas-shop`
5. Ironworks creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Ironworks until it works well
2. Export: `ironworksai export --template my-agency`
3. Publish: `ironworksai publish cliphub my-agency`
...
3. Install just that agent: `ironworksai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Ironworks

ClipHub is **not required** to use Ironworks. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Ironworks what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `ironworksai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Ironworks web UI
- [ ] `ironworksai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Ironworks's own philosophy)

...
All persisted under your bind mount (`./data/docker-odysseus` in the example above).
...
- host port: `3100`
- persistent data dir: `./data/docker-odysseus`
...
If you change host port or use a non-local domain, set `ODYSSEUS_PUBLIC_URL` to the external URL you will use in browser/auth flows.
...
PostgreSQL data persists in a named Docker volume (`pgdata`). Paperclip data persists in `odysseus-data`.
...
services:
  odysseus:
    environment:
      ...
      ODYSSEUS_PUBLIC_URL: https://desk.koker.net
```

`ODYSSEUS_PUBLIC_URL` is used as the primary source for:
...
Granular overrides remain available if needed (`ODYSSEUS_AUTH_PUBLIC_BASE_URL`, `BETTER_AUTH_URL`, `BETTER_AUTH_TRUSTED_ORIGINS`, `ODYSSEUS_ALLOWED_HOSTNAMES`).

Set `ODYSSEUS_ALLOWED_HOSTNAMES` explicitly only when you need additional hostnames beyond the public URL host (for example Tailscale/LAN aliases or multiple private hostnames).
...
| File | Purpose |
|------|---------|
| `docker/quadlet/odysseus.pod` | Pod definition — groups containers into a shared network namespace |
| `docker/quadlet/odysseus.container` | Paperclip server — joins the pod, connects to Postgres at `127.0.0.1` |
| `docker/quadlet/odysseus-db.container` | PostgreSQL 17 — joins the pod, health-checked |
...
3. Create a secrets env file (keep out of version control):
   ...
   ```sh
   cat > ~/.config/containers/systemd/odysseus.env <<EOL
   BETTER_AUTH_SECRET=$(openssl rand -hex 32)
   POSTGRES_USER=odysseus
   POSTGRES_PASSWORD=odysseus
   POSTGRES_DB=odysseus
   DATABASE_URL=postgres://odysseus:odysseus@127.0.0.1:5432/odysseus
   # OPENAI_API_KEY=sk-...
...
4. Create the data directory and start:
   ...
   ```sh
   mkdir -p ~/.local/share/odysseus
   systemctl --user daemon-reload
   systemctl --user start odysseus-pod
   ```

### Quadlet management

```sh
journalctl --user -u odysseus -f        # App logs
journalctl --user -u odysseus-db -f     # DB logs
systemctl --user status odysseus-pod    # Pod status
systemctl --user restart odysseus-pod   # Restart all
systemctl --user stop odysseus-pod      # Stop all
```

### Quadlet notes

- **First boot**: Unlike Docker Compose's `condition: service_healthy`, Quadlet's `After=` only waits for the DB unit to *start*, not for PostgreSQL to be ready. On a cold first boot you may see one or two restart attempts in `journalctl --user -u odysseus` while PostgreSQL initialises — this is expected and resolves automatically via `Restart=on-failure`.
- Containers in a pod share `localhost`, so Paperclip reaches Postgres at `127.0.0.1:5432`.
- PostgreSQL data persists in the `odysseus-pgdata` named volume.
- Paperclip data persists at `~/.local/share/odysseus`.
...
- `npx odysseus onboard --yes` completes
...
```sh
HOST_PORT=3200 ODYSSEUS_AI_VERSION=latest ./scripts/docker-onboard-smoke.sh
ODYSSEUS_DEPLOYMENT_MODE=authenticated ODYSSEUS_DEPLOYMENT_EXPOSURE=private ./scripts/docker-onboard-smoke.sh
SMOKE_DETACH=true SMOKE_METADATA_FILE=/tmp/odysseus-smoke.env ODYSSEUS_AI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
- Smoke script also defaults `ODYSSEUS_PUBLIC_URL` to `http://localhost:<HOST_PORT>` so bootstrap invite URLs and auth callbacks use the reachable host port instead of the container's internal `3100`.
- In authenticated mode, the smoke script defaults `SMOKE_AUTO_BOOTSTRAP=true` and drives the real bootstrap path automatically: it signs up a real user, runs `odysseus auth bootstrap-ceo` inside the container to mint a real bootstrap invite, accepts that invite over HTTP, and verifies board session access.
...
- Paperclip data persists via Docker volumes/bind mounts (compose) or at `~/.local/share/odysseus` (quadlet).

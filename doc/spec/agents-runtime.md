...
For `claude_local` and `codex_local`, ValAdrien OS assumes the CLI is already installed and authenticated on the host machine.
...
ValAdrien OS stores resumable session state per `(agent, taskKey, adapterType)`.

...
For `claude_local` and `codex_local`, Ironworks assumes the CLI is already installed and authenticated on the host machine.
...
Ironworks stores resumable session state per `(agent, taskKey, adapterType)`.

...
For `claude_local` and `codex_local`, AiTeamCorp assumes the CLI is already installed and authenticated on the host machine.
...
AiTeamCorp stores resumable session state per `(agent, taskKey, adapterType)`.

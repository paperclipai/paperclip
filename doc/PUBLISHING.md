...
Example for `@paperclipai/acpx-engine` from the repo root:

```bash
# safe preview
pnpm run release:bootstrap-package -- @paperclipai/acpx-engine

# one-time first publish from an authenticated maintainer machine
pnpm run release:bootstrap-package -- @paperclipai/acpx-engine --publish --otp 123456
```
...
1. open `https://www.npmjs.com/package/@paperclipai/acpx-engine`

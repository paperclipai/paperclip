...
This guide describes the current, implemented way to create a PrivateClip plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main PrivateClip app.
...
For a plugin that lives outside the PrivateClip repo:
...
Outside this monorepo, the scaffold snapshots `@paperclipai/plugin-sdk` from the local PrivateClip checkout into a `.paperclip-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into PrivateClip from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.

...
This guide describes the current, implemented way to create a AiTeamCorp plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main AiTeamCorp app.
...
node packages/plugins/create-aiteamcorp-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the AiTeamCorp repo:
...
Outside this monorepo, the scaffold snapshots `@aiteamcorp/plugin-sdk` from the local AiTeamCorp checkout into a `.aiteamcorp-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into AiTeamCorp from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@aiteamcorp/plugin-sdk/ui`

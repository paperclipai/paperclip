...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@valadrien-os/plugin-sdk/ui`) with design tokens and common components.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@valadrien-os/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
- **Single SDK package**: `@valadrien-os/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@valadrien-os/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
```ts
import { definePlugin, z } from "@valadrien-os/plugin-sdk";

export default definePlugin({
  id: "@valadrien-os/plugin-linear",
  version: "0.1.0",
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@valadrien-os/plugin-sdk/ui";
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@valadrien-os/plugin-linear`)
...
Package idea: `@valadrien-os/plugin-workspace-files`
...
Package idea: `@valadrien-os/plugin-terminal`
...
Package idea: `@valadrien-os/plugin-git`
...
- optional `events.emit` (e.g. `plugin.@valadrien-os/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@valadrien-os/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@valadrien-os/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@valadrien-os/plugin-runtime-processes`
...
- plugin UI bundle loading, host bridge, `@valadrien-os/plugin-sdk/ui`
...
- `@valadrien-os/plugin-test-harness` and `create-valadrien-os-plugin` starter template
...
8. Ship `@valadrien-os/plugin-test-harness` and `create-valadrien-os-plugin` starter template.

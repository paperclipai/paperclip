...
odysseus plugin init @acme/hello-plugin --output ~/dev/odysseus-plugins

# 3. Install dependencies and start the watch build
cd ~/dev/odysseus-plugins/hello-plugin
pnpm install
pnpm dev

# 4. In another terminal, install the plugin from its absolute path
odysseus plugin install ~/dev/odysseus-plugins/hello-plugin

# 5. Confirm it loaded
odysseus plugin list
odysseus plugin inspect acme.hello-plugin
```
...
This creates `~/dev/odysseus-plugins/hello-plugin/` with `src/manifest.ts`, `src/worker.ts`, `src/ui/index.tsx`, an esbuild watch config, a Vitest config, and a snapshot of `@odysseus/plugin-sdk` from your local Paperclip checkout. You can run the package and tests without publishing anything to npm.
...
```
Installing plugin from local path: /Users/you/dev/odysseus-plugins/hello-plugin
✓ Installed acme.hello-plugin v0.1.0 (ready)
Local plugin installs run trusted local code from your machine.
Keep `pnpm dev` running in /Users/you/dev/odysseus-plugins/hello-plugin;
Paperclip watches rebuilt dist output and reloads the plugin worker.
```

Relative paths are resolved against the current working directory, so `odysseus plugin install .` from inside the plugin folder works too.
...
Paperclip watches the on-disk plugin package after a local install. The watcher targets the runtime entrypoints declared in the package's `odysseusPlugin` field (`dist/manifest.js`, `dist/worker.js`, `dist/ui/`).
...
- **`node_modules`, `.git`, `.odysseus-sdk`, and other dotfolders are ignored.** Adding a dependency requires the new code to actually be imported and rebuilt before the worker sees it.
...
- **npm packages are the deployable artifact.** `odysseus plugin install @acme/plugin-foo` (optionally `--version 1.2.3`) installs from your configured npm registry, version-pins, and produces an install record that other operators can reproduce. Ship plugins this way.
...
- **Restart cleanly:** `odysseus plugin disable <key>` pauses the plugin without removing it. `odysseus plugin enable <key>` brings it back. `odysseus plugin uninstall <key>` removes the install record; add `--force` to also purge plugin state and settings.
- **Browse examples:** `odysseus plugin examples` lists the bundled example plugins that ship with the repo, each with a ready-to-run `odysseus plugin install <path>` line.
- **Go deeper:** [`PLUGIN_AUTHORING_GUIDE.md`](./PLUGIN_AUTHORING_GUIDE.md) covers worker capabilities, managed agents/projects/routines, plugin database namespaces, scoped API routes, and the shared UI components in `@odysseus/plugin-sdk/ui`. [`PLUGIN_SPEC.md`](./PLUGIN_SPEC.md) is the longer-form specification, including future ideas that are not yet implemented.

## Troubleshooting

- **`Plugin install returned no plugin record` or `error` status.** Run `odysseus plugin inspect <key>` for the last error. The most common causes are (1) the plugin has not built yet — run `pnpm dev` or `pnpm build` first, (2) the `odysseusPlugin` entries in `package.json` point at files that do not exist on disk, or (3) the manifest failed validation. The Paperclip server log has the full validation error.
- **Edits do not seem to reload.** Confirm `pnpm dev` is still running and writing to `dist/`. If you renamed entry files, update the `odysseusPlugin.manifest` / `odysseusPlugin.worker` / `odysseusPlugin.ui` fields in `package.json` so the watcher targets them.

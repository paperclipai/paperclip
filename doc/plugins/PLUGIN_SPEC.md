...
- The current runtime ships a small host-provided plugin UI component kit through `@odysseus/plugin-sdk/ui`, but does not support plugin asset uploads/reads yet. Treat plugin asset APIs as future-scope ideas, not current implementation promises.
...
4. Depend on project-local plugin folders such as `.odysseus/plugins`.
...
- `~/.odysseus/instances/default/plugins/package.json`
- `~/.odysseus/instances/default/plugins/node_modules/`
- `~/.odysseus/instances/default/plugins/.cache/`
- `~/.odysseus/instances/default/data/plugins/<plugin-id>/`
...
- `pnpm odysseus plugin list`
- `pnpm odysseus plugin install <package[@version]>`
- `pnpm odysseus plugin uninstall <plugin-id>`
- `pnpm odysseus plugin upgrade <plugin-id> [version]`
- `pnpm odysseus plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@odysseus/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@odysseus/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@odysseus/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@odysseus/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@odysseus/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@odysseus/plugin-sdk/ui`
...
The `@odysseus/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
6. The operator may force-purge immediately via CLI: `pnpm odysseus plugin purge <plugin-id>`.
...
### 27.1 `@odysseus/plugin-test-harness`
...
```ts
import { createTestHarness } from "@odysseus/plugin-test-harness";
...
- The operator installs the plugin from a local path: `pnpm odysseus plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-odysseus-plugin`) that scaffolds:

- `package.json` with correct `odysseusPlugin` keys
...
- `@odysseus/plugin-workspace-files`
- `@odysseus/plugin-terminal`
- `@odysseus/plugin-git`
- `@odysseus/plugin-linear`
- `@odysseus/plugin-github-issues`
- `@odysseus/plugin-grafana`
- `@odysseus/plugin-runtime-processes`
- `@odysseus/plugin-stripe`
...
- `@odysseus/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@odysseus/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@odysseus/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@odysseus/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@odysseus/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@odysseus/plugin-sdk/ui`
...
- `@odysseus/plugin-test-harness`
- `create-odysseus-plugin` starter template

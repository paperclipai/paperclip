...
| Gateway calls | `GET /api/tool-gateway/tools`, `POST /api/tool-gateway/tools/call` | `X-Paperclip-Tool-Gateway-Token` header is canonical and wins when both are sent; a client with a single credential slot may send the same `pcgt_` token as `Authorization: Bearer` instead. Call body uses `tool` + `parameters`. Approval-required calls respond `409` with `reasonCode: approval_required` and an `actionRequestId`; the agent retries with `approvedActionRequestId`. |
| Named gateway protocol | `POST /mcp/gateways/:gatewayPublicId`, `POST /api/tool-gateway/gateways/:gatewayId/mcp` | MCP streamable-HTTP surface for a named gateway. `Authorization: Bearer <pcgw_…>` only — the client token minted above. These paths and the two session data-plane paths are the route-owned authentication surfaces: they validate their own credential family, so the global actor middleware leaves their bearer alone (see `server/src/middleware/route-owned-auth.ts`). An invalid credential answers `401` with `reasonCode: gateway_token_invalid`; repeated failures answer `429` with `gateway_auth_throttled`. |

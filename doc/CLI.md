...
Markdown, pasted snippets, and model output. `npx` passes the argument as an
inert `argv` value. It does not run a shell over the value.

Do not use `pnpm paperclipai` for a content-bearing argument. `pnpm` runs the
argument through `/bin/sh` first. The shell interprets these metacharacters
...
- `npx paperclipai <command> <args>` — the documented default
- `pnpm exec paperclipai <command> <args>` — secondary safe form

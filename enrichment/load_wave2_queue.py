"""
Wave-2 queue loader for SAG-2845 Gate 2.

Inserts ~150 SSI-HP catalog rows as 'pending' into enrichment_staging.enrichment_queue,
skipping any source_row_id already present (idempotent). Rows use the same payload
format as the SAG-2842 canary: {sku, category_tag, product_name, price_per_sqft,
raw_description}.

Usage:
  DATABASE_URL=postgresql://... python3 enrichment/load_wave2_queue.py [--dry-run]
"""
...
def main() -> None:
    ...
    db_url = os.environ.get("DATABASE_URL", "postgresql://paperclip:paperclip@localhost:54329/paperclip")
    ...
    cur.execute("SELECT source_row_id FROM enrichment_staging.enrichment_queue")
    ...
    for row in rows_to_load:
        ...
        cur.execute(
            """
            INSERT INTO enrichment_staging.enrichment_queue
              (source_row_id, payload_json, status)
            VALUES (%s, %s, 'pending')
            """,
            (sku, json.dumps(row)),
        )

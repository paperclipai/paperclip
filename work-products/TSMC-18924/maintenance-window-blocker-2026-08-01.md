...
- `adapter_failed` (547) was mainly lane/configuration-specific: 357 Codex managed-home TOML duplicate-key failures, 86 Hermes credential/provider failures, 56 Codex unsupported-model/configuration failures, 35 shell-handler failures, and 13 other adapter failures.
...
Rollback source, database, config, Hermes state, and exact agent-status manifests are under `/Users/glad0s/paperclip-maintenance-backups/20260802-1125IST`. The database backup, Paperclip bundle, and Hermes bundle passed integrity verification. The old generated ACPX tree and stale Hermes update cache were moved into that directory rather than destroyed.

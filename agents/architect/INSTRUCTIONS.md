...
1. **One cargo build alive at a time, *machine-wide* — enforced by a global `flock`.** Each worktree has its *own* `target/`, so cargo's per-target `target/.cargo-lock` does **not** serialize sibling Architects anymore (that implicit serialization died when the profile moved to per-worktree targets — 8 concurrent runs then meant 8 cargos thrashing the same 8 cores). The replacement is an explicit machine-wide build lock: **every cargo build runs under `flock /tmp/cargo-global.lock`** (baked into the canonical commands below). Exactly one cargo holds the lock and gets all cores; every other launch *blocks* on the lock at zero CPU cost until it's free, then runs at full speed. This is strictly better than a shared `CARGO_TARGET_DIR` (which serialized builds but thrashed incremental state on every branch switch), and it makes the Architect's `maxConcurrentRuns` non-critical for CPU — runs can pipeline on cheap work (rebase, PR-open) while their builds queue on the lock. Also: never start a second `cargo` *within your own run* before the previous exits. **The one running build is also CPU/IO-capped so it cannot starve the Paperclip API server on the same host:** it is pinned to 6 of the 8 cores (`taskset -c 0-5`) at lowest scheduling + idle IO priority (`nice -n19 ionice -c3`), leaving 2 cores + IO headroom for the API. This cap is baked into the canonical commands below — do **not** drop it (AA-1751: an un-capped serialized build saturated all 8 cores and drove API TTFB to ~24s, causing `http=000` timeouts on Coordinator PATCH/POST calls). **Every canonical command also pre-starts sccache *outside* the flock** (`sccache --start-server >/dev/null 2>&1 || true;` immediately before `flock`) — do **not** drop this either (AA-1811: cargo cold-starting the sccache daemon *under* the lock makes the persistent daemon inherit the flock's open file description, so the lock stays held after flock exits and wedges the global build lock for hours; pre-starting outside the lock means under-lock cargo connects to the existing singleton and never cold-starts). `--start-server` is a no-op when a server already runs, so the prefix is idempotent and ~free.
2. **DETACH the build so it survives your run dying, then land it on a later wake — do NOT block in-run waiting for cargo.** The real killer of this pipeline is **not** turn count or wall-clock — it is the **Claude session/usage limit**, which terminates your run at *any* unpredictable point (a verify run ran 57min before dying on `session limit`). No in-run wait survives that. So the verify flow is now a **sentinel-driven state machine across runs** (AA-1609/**AA-1610**): one run launches a detached build that outlives the run, a later run observes the result via a sentinel file and lands it.
   - **Launch (detached, durable).** Start cargo under `setsid`/`nohup` so it is NOT in your run's process group and survives the run ending. Chain the stages with `&&` (this preserves the staged gate — clippy only runs if check passed, test only if clippy passed) and write the final exit code to a sentinel **after** the chain. Then **exit your run** (a normal short run — you are not abandoning work, the detached build owns it now). Canonical:
     ...
     # Orphan guard (AA-1751): if this task's PR has already merged, the work landed.
     # Launching/relaunching now would only queue a pointless cargo on the global flock,
     # starving the API while the freshness gate later refuses to land it anyway. Bail
     # BEFORE the rebase + launch, and clean up any stale sentinels we own.
     if gh pr list --head "task/{task-id}" --state merged --json number -q '.[0].number' | grep -q .; then
       rm -f "$EXIT" "$BASE" "$LOG" /tmp/verify-{task-id}.freshness
       echo "task/{task-id} PR already merged — orphan verify, no build launched; cleaned sentinels, exiting"; exit 0
     fi
     rm -f "$EXIT"
   ...
   - **Every verify wake decides by sentinel FIRST — never start cargo before checking it:**
     ...
     - **Sentinel absent AND no build running** → either first dispatch or a launch that died before writing the sentinel → **launch** (above; the launch block self-aborts before queuing if this task's PR has already merged — the AA-1751 orphan guard). Relaunch is safe/idempotent.

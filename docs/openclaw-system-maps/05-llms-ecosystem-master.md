...
- [Katailyst graph-discovery rule](#katailyst-graph-discovery-rule) — semantic search + graph traversal before local invention
- [Axon rule](#axon-rule) — when and how to use Axon for repo comprehension
- [Key Katailyst entity types](#key-katailyst-entity-types) — use the ontology, not hardcoded paths
- [Official canon docs](#official-canon-docs)
- [Current truths and decisions](#current-truths-and-decisions)
- [Repo inventory](#repo-inventory) — active repos, roles, inspect-first files
...
## Key Katailyst entity types

Use Katailyst discovery to find relevant entities before doing serious work. Each `type:code` is a canonical registry ref; names are stable across revisions. Do not hardcode a small “load first” list into new workflows; query `registry_capabilities`, search by task, and traverse from the best-matching refs.

### Operating canon
- `kb:katailyst-vault-access-pattern` — canonical secrets access. Read BEFORE hunting in env files or vault backups. Lists active secrets by `secret_key` name when the live vault inventory is queried.
...
### Hubs (graph context clusters — `traverse` when relevant)
- `hub:hub-research` — research lane (firecrawl, brave, tavily, perplexity, gpt-researcher).
- `hub:hub-social` — social channels + playbooks + schemas.
...
- hubs and orientation indexes are useful, but the system should not rely only on shallow hub navigation. Deep search, region finding, and graph traversal matter.
...
### Agentic infrastructure posture (Vercel agentic infrastructure frame, 2026-04)

HLT's ecosystem maps well to the agentic infrastructure frame Vercel articulated on 2026-04-09. Naming our posture in that frame makes it easier to reason about what we have, what we're missing, and what we're buying / building next without introducing a new HLT ontology term.
...
- **Coordination**: multi-step orchestration, pause/resume/retry. **HLT thin** — see the durable-workflows gap above.
- **Execution**: `generateObject` / `tool_execute` / `dispatchSpecialist`. Strong.
- **Integration**: Katailyst MCP bridge, per-tool call_specs. Strong.
...
## Katailyst graph-discovery rule

Use Katailyst before guessing at capabilities or inventing workflows. Do not treat any hub, playbook, or doc as a fixed route. Start with semantic discovery for the actual task, traverse the graph from the best-matching refs, and compose the returned atomic building blocks.

### Standard discovery pattern
1. `registry_capabilities`
2. `registry_session`
3. `discover` by task
4. `traverse` from the best-matching refs when relationships matter
5. `get_entity` / `registry_artifact_body` for only the selected atomic units
6. `tool_describe`
7. `tool_execute`
...
- **[katailyst](https://github.com/Awhitter/katailyst)** — capability canon + streamable-http MCP server at `/mcp` + dashboard-cms admin. Live registry/vault counts come from `registry_capabilities` and vault tooling. Stack: Next.js + pnpm 9.15.9. Local: `~/hlt/katailyst`. Live: https://www.katailyst.com (200). Key dirs: `lib/mcp/`, `lib/docs/`, `app/dashboard-cms/`, `registry-packs/`, `integrations/`, `plugins/`.
- **[sidecar-system](https://github.com/Awhitter/sidecar-system)** — content factory chat: articles/social/ads/recruiting specialists via Vercel AI SDK (Opus 4.7) + destination fan-out + Katailyst MCP bridge. Stack: Next.js 15 + pnpm 8.15.9, 102 deps incl @ai-sdk/{anthropic,google,groq,openai,mcp,replicate,xai}. Local: `~/hlt/sidecar`. Live: https://sidecar-system.vercel.app (200) + https://www.katailyst.com chat surface. Key dirs: `app/(apps)/chat/`, `domains/{articles,social,ads,recruiting}/`, `lib/integrations/`, `lib/publish/`, `lib/mcp/`, `docs/ads-methodology/`, `docs/recruiting-corpus/`.
...
Hubs are graph context clusters in the Katailyst registry. They are not prescribed paths. Use them when semantic discovery or an explicit relationship points there, then traverse outward to find the relevant skills, KBs, tools, playbooks, rubrics, and assets for the current task.
...
- **[hub:hub-registry]** — registry operations: entity ontology, health checks, taxonomy coverage, and bootstrap tool posture. Pull live counts from `registry_capabilities`, never from this file.
- **[hub:hub-research]** — cross-domain research: people, companies, markets, competitors, topics, trends, finance. Use when discovery says the job needs deep external research.
...
## Canonical KB examples

- These examples are not a startup checklist. Use discovery first, then load the specific entities the task calls for.
- **[kb:katailyst-vault-access-pattern]** — canonical secrets access. Read BEFORE hunting in `.env.local` or vault backup files. Pull live secret inventory from the registry/vault tools, not this file.
...
- **[kb:registry-reference-bible]** — registry state reference with examples; verify any count through live tools before quoting it.
...
- Additional agents are listed in `content_type:agent-doc-map`; fetch it live before quoting roster size.
...
- `Core Maps/` — working-copy detail maps (agents-and-runtime-fleet, content-factory-and-testing-strategy, contradictions-and-cleanup-cuts, drift-reports, orientation-indexes, layer-map, repo-doc-hygiene-audit, taxonomy-map, etc.). **Status: ⚠ per-file audit pending; several flagged stale by operator.**
...
- `Archive/Old Core Maps/`, `Archive/Old Orientation Indexes/` — fully archived. No updates needed.
...
- **Main purpose:** the Katailyst registry and MCP repo, with Supabase-canonical atomic units, discovery APIs, CMS and operator surfaces, portability mirrors, and export layers for downstream runtimes
...
- **Key shapes and contracts:** registry entities, tool refs, integration contracts, runtime context, prompts, resources, toolsets
...
Run live `registry_health` and service health checks before making readiness claims. Old morning-brief reports are not canonical evidence.
...
- **Vercel, "Agentic Infrastructure"** (Tom Occhino, 2026-04-09) — sharpened the infrastructure framing above. The principle (design for agent primacy, not human-command-line primacy) is ours; their source language is not an HLT ontology term.
...
- **Build time audits across active repos.** Not measured systematically; a one-off audit is pending.
...
- **Integration readiness must be live-verified.** Do not reuse dated readiness scores or deleted morning-brief reports. Run current `registry_health` plus service health checks before claiming Paperclip, Hermes, OpenClaw, or downstream publishing surfaces are healthy.
...
| Repo | Katailyst MCP client | Vault secrets used | Publishes to | Consumed by |
|---|---|---|---|---|
...
**Meta Ads (Facebook + Instagram)** — current state: Manus dashboard live at `https://hltadspdash-gqtedncp.manus.space`. Native surface: sidecar `/ads` (shipped Ads Factory v1), additive not replacement. Reads via `tool:meta-ads.insights` (Katailyst MCP) once `http_multi_action` executor ships.
...
**Ads + social isolation principle** — the sidecar hosts the creative + drafting + approval gate. The platform-specific publishing credentials live in vault, accessed via Katailyst MCP tool_execute (`tool:meta-ads.*`, `tool:google-ads.*` when built). No repo has both Meta and Google credentials in process memory at the same time.

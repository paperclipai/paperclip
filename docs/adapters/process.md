...
- Running a Python script that calls the AGNB API
...
1. AGNB spawns the configured command as a child process
2. Standard AGNB environment variables are injected (`PAPERCLIP_AGENT_ID`, `PAPERCLIP_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the AGNB API and perform work.

...
- Running a Python script that calls the Orchestrero API
...
1. Orchestrero spawns the configured command as a child process
2. Standard Orchestrero environment variables are injected (`PAPERCLIP_AGENT_ID`, `PAPERCLIP_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Orchestrero API and perform work.

...
| `fastMode` | boolean | No | Enables Codex Fast mode. Currently supported on `gpt-5.4`, `gpt-5.5`, and manual model IDs; burns credits faster |
...
Paperclip currently applies that when the selected model is `gpt-5.4`, `gpt-5.5`, or a manual model ID. On other known unsupported models, the toggle is preserved in config but ignored at execution time to avoid unsupported runs.

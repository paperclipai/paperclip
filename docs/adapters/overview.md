...
summary: What adapters are and how they connect agents to Orchestrero
---

Adapters are the bridge between Orchestrero's orchestration layer and agent runtimes. Each adapter knows how to invoke a specific type of AI agent and capture its results.

## How Adapters Work

When a heartbeat fires, Orchestrero:
...
| Hermes Local | `hermes_local` | Runs Hermes CLI locally (`hermes-paperclip-adapter`, with quiet mode forced for Paperclip-managed chat runs) |
...
You can build and distribute adapters as standalone packages — no changes to Orchestrero's source code required. External adapters are loaded at startup via the plugin system.
...
External adapters can ship a self-contained UI parser that tells the Orchestrero web UI how to render their stdout. Without it, the UI uses a generic shell parser. See the [UI Parser Contract](/adapters/adapter-ui-parser) for details.

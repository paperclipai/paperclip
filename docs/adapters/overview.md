...
You can build and distribute adapters as standalone packages — no changes to AiTeamCorp's source code required. External adapters are loaded at startup via the plugin system.
...
my-adapter/
  src/
    ...
    cli/
      format-event.ts   # Terminal output for `aiteamcorp run --watch`
```
...
External adapters can ship a self-contained UI parser that tells the AiTeamCorp web UI how to render their stdout. Without it, the UI uses a generic shell parser. See the [UI Parser Contract](/adapters/adapter-ui-parser) for details.

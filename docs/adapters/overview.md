...
| Hermes (external plugin) | `hermes_local` | Installed through Adapter Manager; its plugin supplies the Hermes runtime and UI schema |
...
### Hermes as an external adapter

Hermes is not bundled with Paperclip. Install its adapter package through
**Adapter Manager** (or configure a local `file:` package for development).
The external plugin supplies its own config schema and UI parser; once loaded,
the adapter type remains `hermes_local` for existing configurations.
...
- **Need a coding agent?** Use `claude_local`, `codex_local`, or `opencode_local`; install Hermes or Droid through Adapter Manager when needed

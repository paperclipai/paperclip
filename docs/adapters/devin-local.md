...
In Agent Chat conversations the adapter uses Paperclip's conversation prompt template (unless `promptTemplate` is configured) and includes the server's task-context markdown; fresh sessions also receive the task's initial communication guidance.
...
Desired Paperclip skills are symlinked into `<cwd>/.devin/skills/` only while
a run needs them: the links are staged before the Devin CLI spawns and
removed when the run exits (success, failure, or timeout). This is the
directory the Devin CLI scans for project skills. The operator's global
...
Concurrent runs sharing a working directory share skill discovery for the
overlap. Cleanup is reference-counted: each run writes a lease file under the
OS temp dir (keyed by the resolved cwd, never inside the workspace), and a
link is removed only when no live lease still holds it. Once a run's Devin
child has spawned, the lease follows the child's pid, so it stays valid for
the whole run even across a Paperclip server restart. Stale leases left by
crashed runs are detected via pid liveness plus a per-process token and
deleted, so leftover managed links from crashed runs or earlier builds of
this adapter are removed by the next run in that cwd. Foreign directories
and unrecognized links are always preserved, and a name occupied by an
external installation produces a warning instead of an overwrite. The
operational Paperclip skill remains selected when available.

`listSkills` / `syncSkills` report a configured-for-run view (`ephemeral`
mode): the desired set is stored in the agent config and materializes in the
workspace for the duration of each run. They never touch the filesystem.
...
settings from `$XDG_CONFIG_HOME/devin` (default `~/.config/devin`). Paperclip wake context travels in the run prompt; the adapter does not export
`PAPERCLIP_WAKE_PAYLOAD_JSON`. Scalar `PAPERCLIP_*` variables such as
`PAPERCLIP_TASK_ID`, `PAPERCLIP_WAKE_REASON`, and `PAPERCLIP_RUN_ID` stay in the
environment, and the agent can use plain `curl` with them to drive its own issue.

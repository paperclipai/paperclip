...
AGNB `external_reference` secrets without copying plaintext into
AGNB.
...
`ListSecrets` only and import stores a AGNB external reference plus
...
remote metadata, or copies secret plaintext into AGNB.
...
- `conflict`: the suggested AGNB `name` or `key` is already in use.
...
AGNB-managed AWS secret through a broad runtime role.
...
Each row may include an optional AGNB `description` entered during review;
...
the AGNB runtime role can list the AWS secret but lacks

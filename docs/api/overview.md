...
AiTeamCorp exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`AITEAMCORP_API_KEY`)
...
- Run audit trail: include `X-AiTeamCorp-Run-Id` header on all mutating requests during heartbeats

...
ValAdrien OS exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`VALADRIEN_OS_API_KEY`)
...
- Run audit trail: include `X-Valadrien-Os-Run-Id` header on all mutating requests during heartbeats

...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-AiTeamCorp-Signature` + `X-AiTeamCorp-Timestamp` header pair matching the trigger's signing mode.

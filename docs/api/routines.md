...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-AGNB-Signature` + `X-AGNB-Timestamp` header pair matching the trigger's signing mode.

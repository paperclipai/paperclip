...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Ironworks-Signature` + `X-Ironworks-Timestamp` header pair matching the trigger's signing mode.

...
Atomically claims the task and transitions to `in_progress`. Returns `409 Conflict` when a live run already holds it — that run is often **another run of your own agent**, so being the assignee is not an exception. **Never retry a 409**, with one exception: `details.conflictReason == "stale_lock_pending_reap"` means the holding run has already ended, and as the assignee you may repeat that same call exactly once. Read `details.conflictReason` / `holderRunIsLive` in the 409 body; see [Run-lock 409s](../../skills/paperclip/references/api-reference.md#run-lock-409s).

...
A question in an `ask_user_questions` payload can declare `intent: "decision"` when a person must own a consequential choice. That intent enforces a stricter contract: a single-select question with at least two distinct options, exactly one option marked `recommended: true`, a non-empty `recommendationRationale`, `required: true`, no free-form fallback, and `supersedeOnUserComment` not `true` — a plain comment must never stand in for a decision. Decision questions clamp to `human_only` (omitting `resolverPolicy` inherits it, while an explicit wider policy returns `422`), and the stored `effectiveResolverPolicySource` is `decision_question`. The card shows the recommendation without preselecting it; `respond` must select one of the declared options, and free text alone is supplementary rationale.

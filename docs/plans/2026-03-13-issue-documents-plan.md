...
Add first-class **documents** to AiTeamCorp as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by AiTeamCorp workflow/docs.
...
Update the AiTeamCorp agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/aiteamcorp/SKILL.md`

...
- [ ] Project detail and project workspace pages
...
- Current evidence:
  `bun scripts/check-i18n.ts` passed with `1747` translation keys across `6` locales.
  `git diff --check` passed.
  `origin/master` review found no additional `ui/` or `docs/` scope in the 2 remote-ahead commits.
  `pnpm --filter @paperclipai/ui typecheck` still hangs at `tsc -b` in this environment, so this batch is not claimed as freshly typecheck-clean.

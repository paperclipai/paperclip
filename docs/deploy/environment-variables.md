...
| `PAPERCLIP_ENABLE_PARKED_V1_LIVENESS_SUPPRESSION` | `true` | Suppresses the two parked-v1 issue-graph liveness findings described below. Only the exact value `false` disables it. |
...
### Parked-v1 liveness suppression

`PAPERCLIP_ENABLE_PARKED_V1_LIVENESS_SUPPRESSION` is default-on. Set it to the
exact, case-sensitive string `false` to roll back the suppression, then restart
the Paperclip server or redeploy every server instance. Changing an existing
process environment does not reconfigure an already-running server.

The policy applies only when the blocker is in `backlog` and has a
company-scoped label whose name is exactly `parked`. Label IDs are generated by
the normal company label lifecycle and are not required to match a global UUID.
It suppresses only these issue-graph liveness invariants:

- `blocked_by_unassigned_issue`
- `blocked_by_assigned_backlog_issue`

It does not change issue status, assignees, blocker relations, recovery data,
or any other liveness invariant, and it does not launch work or agents. With
the flag disabled, the original finding and reconciliation behavior is
restored.

Each suppression emits a structured log event with
`event=issue_graph_liveness_suppressed`, `reason=parked_v1`,
`policyVersion=parked-v1`, `issueId`, `companyId`, and `suppressedState`. Use
those fields to audit which invariant would otherwise have produced a finding.

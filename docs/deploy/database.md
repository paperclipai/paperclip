...
Ironworks uses PostgreSQL via Drizzle ORM. There are three ways to run the database.
...
1. Creates `~/.ironworks/instances/default/db/` for storage
2. Ensures the `ironworks` database exists
3. Runs migrations automatically
4. Starts serving requests

Data persists across restarts. To reset: `rm -rf ~/.ironworks/instances/default/db`.
...
```sh
cp .env.example .env
# DATABASE_URL=postgres://ironworks:ironworks@localhost:5432/ironworks
```

...
## Knowing *what* will run, not just *when* it restarts

The rules above assume a restart re-runs the code you meant to deploy. That's not guaranteed: dev-plane control-plane instances typically run TypeScript source directly out of a working tree via `tsx`/`pnpm paperclipai run` — there is no build/tag/release step that pins what "the deploy" is. Whatever branch/commit/dirty-state the checkout happens to be in at restart time is what runs.

**Near-miss (TWX-1314):** a hotfix was committed on `fix/TWX-1273-actorrunid-uuid-guard`, believed deployed. The checkout was then switched to `manual-sync/TWX-1312` for unrelated sync work — a branch that does **not** contain that commit. The next scheduled restart (`paperclip-restart.timer`) would have silently restarted the service *without* the fix everyone believed was live. Nothing would have said so; the mismatch was only caught by manual inspection.

This is a distinct failure mode from the restart-burst damage above: it's not about *when* you restart, it's about the checkout drifting out from under a running service between deploys, silently.

### Guard: `scripts/deploy/check-deploy-state.sh` + `scripts/deploy/mark-deploy.sh`

Two small scripts make drift loud instead of silent:

- **`./scripts/deploy/mark-deploy.sh [note]`** — run this every time you *intentionally* switch what a control-plane checkout should run (new branch, new commit, or a deliberate dirty-tree state for local iteration). It records branch, HEAD sha, dirty/clean, timestamp, and operator into `<repo>/.git/deploy-marker` (host-local, untracked, per-worktree — never committed).
- **`./scripts/deploy/check-deploy-state.sh [--enforce]`** — prints what the checkout will actually run next (branch, HEAD, dirty state) next to the last marked "intended" state, and flags drift: branch changed, HEAD doesn't contain the marked commit, or dirty/clean state changed. Without `--enforce` it only prints/warns (exit 0); with `--enforce` it exits non-zero on drift or on a missing marker.

One-liner status check before trusting a restart:

```bash
./scripts/deploy/check-deploy-state.sh
```

To wire this into a restart path without risking a blocked restart, use it as a non-blocking `ExecStartPre` (the leading `-` means a non-zero exit is logged but does not stop the unit from starting):

```ini
[Service]
ExecStartPre=-/home/vd/dev/paperclip-live/scripts/deploy/check-deploy-state.sh
```

This turns silent drift into a loud line in `journalctl --user -u paperclip-restart.service` (or `paperclip.service`) right before the restart that would otherwise have run the wrong tree. Treat wiring this into the live `paperclip-restart.service`/`paperclip.service` units on vd-fw as a host-config change requiring sign-off before applying, same as any other prod systemd edit — the scripts themselves are safe to run standalone at any time.

...
Agents in AiTeamCorp do not run continuously.  
...
AiTeamCorp stores session IDs for resumable adapters.
...
AiTeamCorp pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. AiTeamCorp surfaces this as a warning in environment tests, not a hard error.

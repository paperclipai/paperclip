...
Agents in Orchestrero do not run continuously.  
...
- `hermes_local`: runs your local `hermes` CLI (`hermes-paperclip-adapter`, with quiet mode forced for Paperclip-managed chat runs)
...
For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), Orchestrero assumes the CLI is already installed and authenticated on the host machine.
...
Orchestrero stores session IDs for resumable adapters.
...
Orchestrero pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Orchestrero surfaces this as a warning in environment tests, not a hard error.

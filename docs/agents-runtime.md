...
Audience: Operators setting up and running agents in Valadrien OS

## 1. What this system does

Agents in Valadrien OS do not run continuously.  
...
- `droid_local`: runs your local Factory Droid CLI (`@henkey/droid-valadrien-os-adapter`)

For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), Valadrien OS assumes the CLI is already installed and authenticated on the host machine.
...
Valadrien OS stores session IDs for resumable adapters.
...
Valadrien OS pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Valadrien OS surfaces this as a warning in environment tests, not a hard error.

...
AiTeamCorp is a monorepo with four main layers.
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with AiTeamCorp env vars and a prompt
4. **Agent work** — The agent calls AiTeamCorp's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between AiTeamCorp and agent runtimes. Each adapter is a package with three modules:
...
- **CLI module** — terminal formatter for `aiteamcorp run --watch`

Built-in adapters: `claude_local`, `codex_local`, `process`, `http`. You can create custom adapters for any runtime.

## Key Design Decisions

- **Control plane, not execution plane** — AiTeamCorp orchestrates agents; it doesn't run them

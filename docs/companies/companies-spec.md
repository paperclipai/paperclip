...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only AGNB.
...
- AGNB must not require extra top-level fields for skill validity
- AGNB-specific extensions must live under `metadata.paperclip` or `metadata.sources`
...
For AGNB, the preferred fidelity extension is:
...
- AGNB tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested AGNB shape:
...
Additional rules for AGNB exporters:
...
- omit empty and default-valued AGNB fields when possible
...
- AGNB exporters should emit `.paperclip.yaml` as a sidecar by default
...
AGNB can map this spec to its runtime model like this:
...
- AGNB extension:
  - `.paperclip.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other AGNB-specific fidelity

Inline AGNB-only metadata that must live inside a shared markdown file should use:

- `metadata.paperclip`

That keeps the base format broader than AGNB.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only AGNB.

## 20. Cutover

AGNB should cut over to this markdown-first package model as the primary portability format.

`paperclip.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For AGNB, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.

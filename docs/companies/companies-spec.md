...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Ironworks.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.ironworks.yaml`
...
- Ironworks must not require extra top-level fields for skill validity
- Ironworks-specific extensions must live under `metadata.ironworks` or `metadata.sources`
...
For Ironworks, the preferred fidelity extension is:
...
- issue/task Ironworks-only metadata
...
- Ironworks tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Ironworks shape:
...
Additional rules for Ironworks exporters:
...
- omit empty and default-valued Ironworks fields when possible
...
- Ironworks exporters should emit `.ironworks.yaml` as a sidecar by default
...
## 19. Ironworks Mapping

Ironworks can map this spec to its runtime model like this:
...
- Ironworks extension:
  - `.ironworks.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Ironworks-specific fidelity

Inline Ironworks-only metadata that must live inside a shared markdown file should use:

- `metadata.ironworks`

That keeps the base format broader than Ironworks.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Ironworks.

## 20. Cutover

Ironworks should cut over to this markdown-first package model as the primary portability format.

`ironworks.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Ironworks, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.

...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.odysseus.yaml`
...
- Paperclip-specific extensions must live under `metadata.odysseus` or `metadata.sources`
...
- Paperclip exporters should emit `.odysseus.yaml` as a sidecar by default
...
- Paperclip extension:
  - `.odysseus.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Paperclip-specific fidelity
...
`odysseus.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

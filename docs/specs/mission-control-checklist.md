...
- Run the next real-use validation slice from section 8, preferably an end-to-end `Main -> Ork` tracked handoff that exercises the same mission-control history expectations through the broader integration path instead of only the direct service seam
...
- Main remaining gap is broader real-use OpenClaw integration validation on top of the now-verified direct service seam, including end-to-end `Main -> Ork` flow coverage and at least one additional specialist/human-attention scenario

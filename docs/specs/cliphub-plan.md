# ClipHub: Marketplace for AiTeamCorp Team Configurations
...
> The "app store" for whole-company AI teams — pre-built AiTeamCorp configurations, agent blueprints, skills, and governance templates that ship real work from day one.
...
| Install target | AiTeamCorp company (agents, projects, governance) |
...
A complete AiTeamCorp company configuration:
...
Single-agent configurations designed to plug into a AiTeamCorp org:
...
Portable skill files that any AiTeamCorp agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with AiTeamCorp's skill loading system
...
interface Listing {
  ...
  aiteamcorpVersionMin: string;     // Minimum AiTeamCorp version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target AiTeamCorp company
...
| `POST` | `/api/listings/:id/install` | Install to AiTeamCorp company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target AiTeamCorp company (or create new)
  → ClipHub API calls AiTeamCorp API to:
  ...
  → Redirect to AiTeamCorp dashboard with new team running
```
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (AiTeamCorp brand blue/purple)
...
| Install flow | One-click deploy to AiTeamCorp company |
...
{
  "targetCompanyId": "uuid",         // Existing AiTeamCorp company
...
- **Option B**: A built-in section of the AiTeamCorp UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing AiTeamCorp UI and API.
...
- [ ] Install flow: blueprint → AiTeamCorp company

# ClipHub: Marketplace for Ironworks Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Ironworks configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Ironworks-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Ironworks company (agents, projects, governance) |
...
A complete Ironworks company configuration:
...
Single-agent configurations designed to plug into a Ironworks org:
...
Portable skill files that any Ironworks agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Ironworks's skill loading system
...
interface Listing {
  ...
  ironworksVersionMin: string;     // Minimum Ironworks version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Ironworks company
...
| `POST` | `/api/listings/:id/install` | Install to Ironworks company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Ironworks company (or create new)
  → ClipHub API calls Ironworks API to:
  ...
  → Redirect to Ironworks dashboard with new team running
```
...
### 5.3 Creator: Export from Ironworks → Publish

```
Running Ironworks company → "Export as Blueprint" (CLI or UI)
  → Ironworks exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Ironworks brand blue/purple)
...
| Install flow | One-click deploy to Ironworks company |
...
{
  "targetCompanyId": "uuid",         // Existing Ironworks company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `ironworks-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Ironworks
- **Backend**: Node.js API (or extend Ironworks server)
- **Database**: Postgres (can share Ironworks's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Ironworks auth (or OAuth2)

### 9.2 Integration with Ironworks

ClipHub can be:
- **Option A**: A separate app that calls Ironworks's API to install blueprints
- **Option B**: A built-in section of the Ironworks UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Ironworks UI and API.
...
- [ ] Install flow: blueprint → Ironworks company
...
- [ ] "Export from Ironworks" CLI command

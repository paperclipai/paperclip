# ClipHub: Marketplace for AGNB Team Configurations
...
> The "app store" for whole-company AI teams — pre-built AGNB configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for AGNB-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | AGNB company (agents, projects, governance) |
...
A complete AGNB company configuration:
...
Single-agent configurations designed to plug into a AGNB org:
...
Portable skill files that any AGNB agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with AGNB's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum AGNB version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target AGNB company
...
| `POST` | `/api/listings/:id/install` | Install to AGNB company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target AGNB company (or create new)
  → ClipHub API calls AGNB API to:
  ...
  → Redirect to AGNB dashboard with new team running
```
...
### 5.3 Creator: Export from AGNB → Publish

```
Running AGNB company → "Export as Blueprint" (CLI or UI)
  → AGNB exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (AGNB brand blue/purple)
...
| Install flow | One-click deploy to AGNB company |
...
{
  "targetCompanyId": "uuid",         // Existing AGNB company
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as AGNB
- **Backend**: Node.js API (or extend AGNB server)
- **Database**: Postgres (can share AGNB's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with AGNB auth (or OAuth2)

### 9.2 Integration with AGNB

ClipHub can be:
- **Option A**: A separate app that calls AGNB's API to install blueprints
- **Option B**: A built-in section of the AGNB UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing AGNB UI and API.
...
- [ ] Install flow: blueprint → AGNB company

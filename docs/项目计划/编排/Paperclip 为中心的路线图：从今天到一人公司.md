...
> **阶段 0（驯服平面，2026-05-20 已定）** — 在「第 1 周闭环」之前，先把 upstream 默认编排关掉、Board 可读、适配器跑稳。操作真源：[025-实践-默认编排关闭与按需启用](../最佳实践/025-实践-默认编排关闭与按需启用.md)；产品口径见 [项目需求说明 · 默认编排](项目需求说明.md#默认编排先关按需开已定)。

...
**与邻近文书分工：** 个案证据链、长表归因仍在 **[`../探查/`](../探查/README.md)**；可复用的 **API 顺序 / SQL 模板 / 命令** 在 **[002](002-实践-工单运行记录API取证路径.md)**、**[004](004-实践-工单心跳与僵尸run排障.md)**、**[010](010-实践-从Postgres核查公司与智能体.md)**；本文只钉 **「谁先谁后、脚本往哪登记、助手默认该怎么做」**。
...
- **中文 / 多行 JSON**：遵守 **`skills/paperclip/SKILL.md`**（UTF-8 文件、`--data-binary @file`、禁止 shell 内联大段中文 JSON）；脚本内由 Node 读写文件同理。
...
| `pnpm` 命令 | 典型用途 |
| --- | --- |
| **`issue:forensics`** | **`scripts/issue-run-forensics.mjs`** — 给定 **company + issue**，聚合 runs / activity / prompt 节选；详见 **[002](002-实践-工单运行记录API取证路径.md)** |
...
2. **禁止**在 Windows 上教人手写含中文 body 的一行 `curl`；应指向 **脚本、`paperclip` skill 安全写入** 或 **010 的 SQL**。  
3. 若结论稳定：**提议**（在人类许可下）新增 **`最佳实践/NNN`** + **`scripts/`** + **`package.json`** 一行，而不是每次都从零口述命令。

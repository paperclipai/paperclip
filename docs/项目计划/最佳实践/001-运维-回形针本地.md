...
**技能目录 `skills/` 整包快照与后悔恢复**：见同目录 [003-实践-技能默认包快照与恢复.md](003-实践-技能默认包快照与恢复.md)。
...
**Routic 实例（阶段 0）：** upstream 默认 recovery / 复盘 / timer 等编排开关的 **关断清单与按需启用顺序** 见 [025-实践-默认编排关闭与按需启用](025-实践-默认编排关闭与按需启用.md) — 与日常 `DATABASE_URL` 同属 `.env`，改后须 **重启 server**。
...
1. **日常开发**：一条龙 **`scripts/start-paperclip-dev-external.ps1`**，或仓库根 **`pnpm dev`**；收工按上文 **「停止与查看状态」** 整段流程（含 **`dev:list` / `dev:stop` / 3100 孤儿**）。  
...
- **阶梯收尾 / 监控（勿默认一棍子杀光 node）**：[006-运维-回形针-并发与CodeBuddy兜底.md](006-运维-回形针-并发与CodeBuddy兜底.md)

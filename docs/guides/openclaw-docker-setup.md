...
How to get OpenClaw running in a Docker container for local development and testing the Orchestrero OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Orchestrero includes an end-to-end join smoke harness:
...
- probes and prints a Orchestrero host URL that is reachable from inside OpenClaw Docker
...
If your Orchestrero deployment is `authenticated`, provide auth context:
...
Then restart Orchestrero and rerun the smoke script.

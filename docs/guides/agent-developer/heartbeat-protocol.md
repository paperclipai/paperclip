...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and AiTeamCorp.
...
If `AITEAMCORP_APPROVAL_ID` is set, handle the approval first:
...
- If `AITEAMCORP_TASK_ID` is set and assigned to you, prioritize it

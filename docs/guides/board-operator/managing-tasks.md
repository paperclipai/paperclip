...
From the issue detail page, you can also use **Wake Assignee** to manually nudge the currently assigned agent without changing the assignment. This calls the same agent wakeup path used elsewhere in the control plane, so paused agents or agents with wake-on-demand disabled may still skip the request.

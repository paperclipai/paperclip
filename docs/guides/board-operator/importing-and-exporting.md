...
├── tasks/
│   └── onboarding/TASK.md
└── .valadrien-os.yaml     # Adapter config, env inputs, routines
```
...
```sh
valadrien-os company export <company-id> --out ./my-export
```
...
valadrien-os company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
valadrien-os company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
valadrien-os company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.valadrien-os.yaml`
...
```sh
# From a local folder
valadrien-os company import ./my-export

# From a GitHub URL
valadrien-os company import https://github.com/org/repo

# From a GitHub subfolder
valadrien-os company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
valadrien-os company import org/repo
valadrien-os company import org/repo/companies/acme
```
...
```sh
valadrien-os company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
valadrien-os company import org/company-templates/engineering-team \
  --target new \
...
```sh
valadrien-os company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
valadrien-os company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
valadrien-os company import ./package \
  --target new \
  --yes \
  --json
```

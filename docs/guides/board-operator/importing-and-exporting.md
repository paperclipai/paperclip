...
├── tasks/
│   └── onboarding/TASK.md
└── .odysseus.yaml     # Adapter config, env inputs, routines
```
...
- **.odysseus.yaml** holds Paperclip-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
odysseus company export <company-id> --out ./my-export
```
...
odysseus company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
odysseus company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
odysseus company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.odysseus.yaml`
...
```sh
# From a local folder
odysseus company import ./my-export

# From a GitHub URL
odysseus company import https://github.com/org/repo

# From a GitHub subfolder
odysseus company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
odysseus company import org/repo
odysseus company import org/repo/companies/acme
```
...
```sh
odysseus company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
odysseus company import org/company-templates/engineering-team \
  --target new \
...
```sh
odysseus company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
odysseus company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
odysseus company import ./package \
  --target new \
  --yes \
  --json
```

...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in Valadrien OS.

Valadrien OS now presents this as a workspace-command model:
...
- Valadrien OS does not automatically start or stop these workspace services as part of issue execution.
- Valadrien OS also does not automatically restart workspace services on server boot.
...
2. Valadrien OS realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. Valadrien OS persists execution-workspace metadata such as paths, refs, and provisioning settings.

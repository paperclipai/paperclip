...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in Orchestrero.
...
- Orchestrero does not automatically start or stop these runtime services as part of issue execution.
- Orchestrero also does not automatically restart workspace runtime services on server boot.
...
2. Orchestrero realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. Orchestrero persists execution-workspace metadata such as paths, refs, and provisioning settings.

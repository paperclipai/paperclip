...
- AiTeamCorp does not automatically start or stop these workspace services as part of issue execution.
- AiTeamCorp also does not automatically restart workspace services on server boot.
...
2. AiTeamCorp realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. AiTeamCorp persists execution-workspace metadata such as paths, refs, and provisioning settings.

...
Telemetry is permanently disabled. Paperclip does not collect usage telemetry, and the `telemetry.enabled` config field and `PAPERCLIP_TELEMETRY_DISABLED` / `DO_NOT_TRACK` environment variables have no effect.

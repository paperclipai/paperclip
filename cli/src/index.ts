...
const DATA_DIR_OPTION_HELP =
  "Odysseus data directory root (isolates state from ~/.odysseus)";

program
  .name("odysseus")
  .description("Odysseus CLI — setup, diagnose, and configure your instance")
  .version(cliVersion);
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadOdysseusEnvFile(options.config);
...
program
  ...
  .option("--run", "Start Odysseus immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Odysseus setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "odysseus")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Odysseus")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Odysseus server API")

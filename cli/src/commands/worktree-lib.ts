...
export const DEFAULT_WORKTREE_HOME = "~/.valadrien-os-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".valadrien-os");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: ValadrienOsConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "valadrien-os",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    VALADRIEN_OS_HOME: paths.homeDir,
    VALADRIEN_OS_INSTANCE_ID: paths.instanceId,
    VALADRIEN_OS_CONFIG: paths.configPath,
    VALADRIEN_OS_CONTEXT: paths.contextPath,
    VALADRIEN_OS_IN_WORKTREE: "true",
    ...(branding?.name ? { VALADRIEN_OS_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { VALADRIEN_OS_WORKTREE_COLOR: branding.color } : {}),
  };
}

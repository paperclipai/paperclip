...
function buildAgentEnvExports(input: {
  ...
  return [
    `export ODYSSEUS_API_URL='${escaped(input.apiBase)}'`,
    `export ODYSSEUS_COMPANY_ID='${escaped(input.companyId)}'`,
    `export ODYSSEUS_AGENT_ID='${escaped(input.agentId)}'`,
    `export ODYSSEUS_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Odysseus skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Odysseus skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveOdysseusSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Odysseus skills directory. Expected ./skills in the repo checkout.",
              );
            }

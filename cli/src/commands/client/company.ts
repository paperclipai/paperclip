...
function shouldIncludePortableFile(filePath: string): boolean {
  ...
  const isOdysseusYaml = baseName === ".odysseus.yaml" || baseName === ".odysseus.yml";
  const contentType = binaryContentTypeByExtension[path.extname(baseName).toLowerCase()];
  return isMarkdown || isOdysseusYaml || Boolean(contentType);
}

function findPortableExtensionPath(files: Record<string, CompanyPortabilityFileEntry>): string | null {
  if (files[".odysseus.yaml"] !== undefined) return ".odysseus.yaml";
  if (files[".odysseus.yml"] !== undefined) return ".odysseus.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.odysseus.yaml") || entry.endsWith("/.odysseus.yml")) ?? null;
}
...
async function promptForImportSelection(preview: CompanyPortabilityPreviewResult): Promise<string[]> {
  ...
  while (true) {
    const choice = await p.select<ImportSelectableGroup | "company" | "confirm">({
      message: "Select what Odysseus should import",
      options: [
        {
          value: "company",
...
export function registerCompanyCommands(program: Command): void {
          ...
          printOutput(
            {
              ...
              odysseusExtensionPath: exported.odysseusExtensionPath,
  ...
  addCommonClientOptions(
    company
      ...
      .option("--odysseus-url <url>", "Alias for --api-base on this command")
      ...
      .action(async (fromPathOrUrl: string, opts: CompanyImportOptions) => {
        try {
          if (!opts.apiBase?.trim() && opts.odysseusUrl?.trim()) {
            opts.apiBase = opts.odysseusUrl.trim();
          }
          const ctx = resolveCommandContext(opts);

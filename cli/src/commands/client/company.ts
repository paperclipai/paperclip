...
function shouldIncludePortableFile(filePath: string): boolean {
  ...
  const isValadrienOsYaml = baseName === ".valadrien-os.yaml" || baseName === ".valadrien-os.yml";
  const contentType = binaryContentTypeByExtension[path.extname(baseName).toLowerCase()];
  return isMarkdown || isValadrienOsYaml || Boolean(contentType);
}

function findPortableExtensionPath(files: Record<string, CompanyPortabilityFileEntry>): string | null {
  if (files[".valadrien-os.yaml"] !== undefined) return ".valadrien-os.yaml";
  if (files[".valadrien-os.yml"] !== undefined) return ".valadrien-os.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.valadrien-os.yaml") || entry.endsWith("/.valadrien-os.yml")) ?? null;
}
...
async function promptForImportSelection(preview: CompanyPortabilityPreviewResult): Promise<string[]> {
  ...
  while (true) {
    const choice = await p.select<ImportSelectableGroup | "company" | "confirm">({
      message: "Select what ValadrienOs should import",
      options: [
        {
          value: "company",
...
export function registerCompanyCommands(program: Command): void {
  ...
  addCommonClientOptions(
    company
      ...
      .option("--valadrien-os-url <url>", "Alias for --api-base on this command")
      ...
      .action(async (fromPathOrUrl: string, opts: CompanyImportOptions) => {
        try {
          if (!opts.apiBase?.trim() && opts.valadrienOsUrl?.trim()) {
            opts.apiBase = opts.valadrienOsUrl.trim();
          }
          const ctx = resolveCommandContext(opts);

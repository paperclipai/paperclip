...
export function renderFeedbackReport(input: {
  ...
  for (const trace of input.traces) {
    ...
    lines.push(`  ${voteColor(voteIcon)} ${pc.bold(issueRef)} ${pc.dim(compactText(trace.issueTitle, 64))}`);
    lines.push(
...
export async function writeFeedbackExportBundle(input: {
  ...
  for (const trace of input.traces) {
    ...
    await writeFile(path.join(input.outputDir, "traces", traceFileName), `${JSON.stringify(trace, null, 2)}\n`, "utf8");
  ...
  await writeFile(path.join(input.outputDir, "index.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
...
async function collectJsonFilesForArchive(outputDir: string, relativePaths: string[]): Promise<Record<string, string>> {

...
import { ApiRequestError, OdysseusApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Odysseus config file")
    .option("-d, --data-dir <path>", "Odysseus data directory root (isolates state from ~/.odysseus)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Odysseus API")
...
export function resolveCommandContext(
  ...
  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.ODYSSEUS_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.ODYSSEUS_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set ODYSSEUS_COMPANY_ID, or set context profile companyId via `odysseus context set`.",
    );
  }

  const api = new OdysseusApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.ODYSSEUS_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.ODYSSEUS_SERVER_PORT || "");

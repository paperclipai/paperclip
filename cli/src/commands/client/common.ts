...
import { ApiRequestError, AiTeamCorpApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to AiTeamCorp config file")
    .option("-d, --data-dir <path>", "AiTeamCorp data directory root (isolates state from ~/.aiteamcorp)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the AiTeamCorp API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.AITEAMCORP_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.AITEAMCORP_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.AITEAMCORP_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set AITEAMCORP_COMPANY_ID, or set context profile companyId via `aiteamcorp context set`.",
    );
  }

  const api = new AiTeamCorpApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.AITEAMCORP_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.AITEAMCORP_SERVER_PORT || "");

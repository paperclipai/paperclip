...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@aiteamcorp/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://aiteamcorp:aiteamcorp@127.0.0.1:${port}/aiteamcorp`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" aiteamcorp db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveAiTeamCorpInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "aiteamcorp";

...
export function resolveWorktreeReseedSource(input: WorktreeReseedOptions): ResolvedWorktreeReseedSource {
  ...
  if (fromSelector && hasExplicitConfigSource) {
    throw new Error("Use either --from <worktree> or --from-config/--from-data-dir/--from-instance, not both.");
  }
...
export function resolveWorktreeReseedTargetPaths(input: { configPath: string; rootPath: string }): WorktreeLocalPaths {
...
async function ensureRepairTargetWorktree(input: {
  ...
  const targetPath = path.resolve(repoRoot, ".paperclip", "worktrees", resolveRepairWorktreeDirName(branchName));

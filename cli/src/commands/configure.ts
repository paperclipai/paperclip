...
import { printOdysseusCliBanner } from "../utils/banner.js";
...
function defaultConfig(): OdysseusConfig {
  const instanceId = resolveOdysseusInstanceId();
  return {
    $meta: {
      version: 1,
...
export async function configure(opts: {
  ...
  printOdysseusCliBanner();
  p.intro(pc.bgCyan(pc.black(" odysseus configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `odysseus onboard` first.");
    p.outro("");
    return;
  }

  let config: OdysseusConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because ODYSSEUS_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }

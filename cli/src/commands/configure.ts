...
import { printValadrienOsCliBanner } from "../utils/banner.js";
...
function defaultConfig(): ValadrienOsConfig {
  const instanceId = resolveValadrienOsInstanceId();
  return {
    $meta: {
      version: 1,
...
export async function configure(opts: {
  ...
  printValadrienOsCliBanner();
  p.intro(pc.bgCyan(pc.black(" valadrien-os configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `valadrien-os onboard` first.");
    p.outro("");
    return;
  }

  let config: ValadrienOsConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because VALADRIEN_OS_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }

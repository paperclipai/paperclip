...
function defaultConfig(): AiTeamCorpConfig {
  const instanceId = resolveAiTeamCorpInstanceId();
  return {
    $meta: {
      version: 1,
...
export async function configure(opts: {
  ...
  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `aiteamcorp onboard` first.");
    p.outro("");
    return;
  }

  let config: AiTeamCorpConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because AITEAMCORP_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }

...
type OnboardDefaults = Pick<OdysseusConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;
...
const ONBOARD_ENV_KEYS = [
  "ODYSSEUS_PUBLIC_URL",
  "DATABASE_URL",
  "ODYSSEUS_DB_BACKUP_ENABLED",
  "ODYSSEUS_DB_BACKUP_INTERVAL_MINUTES",
  "ODYSSEUS_DB_BACKUP_RETENTION_DAYS",
  "ODYSSEUS_DB_BACKUP_DIR",
  "ODYSSEUS_DEPLOYMENT_MODE",
  "ODYSSEUS_DEPLOYMENT_EXPOSURE",
  "ODYSSEUS_BIND",
  "ODYSSEUS_BIND_HOST",
  "ODYSSEUS_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "ODYSSEUS_ALLOWED_HOSTNAMES",
  "ODYSSEUS_AUTH_BASE_URL_MODE",
  "ODYSSEUS_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "ODYSSEUS_STORAGE_PROVIDER",
  "ODYSSEUS_STORAGE_LOCAL_DIR",
  "ODYSSEUS_STORAGE_S3_BUCKET",
  "ODYSSEUS_STORAGE_S3_REGION",
  "ODYSSEUS_STORAGE_S3_ENDPOINT",
  "ODYSSEUS_STORAGE_S3_PREFIX",
  "ODYSSEUS_STORAGE_S3_FORCE_PATH_STYLE",
  "ODYSSEUS_SECRETS_PROVIDER",
  "ODYSSEUS_SECRETS_STRICT_MODE",
  "ODYSSEUS_SECRETS_MASTER_KEY_FILE",
] as const;
...
function describeServerBinding(server: Pick<OdysseusConfig["server"], "bind" | "customBindHost" | "host" | "port">): string {
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const publicUrl = preferTrustedLocal
    ? undefined
    : (
      process.env.ODYSSEUS_PUBLIC_URL?.trim() ||
      process.env.ODYSSEUS_AUTH_PUBLIC_BASE_URL?.trim() ||
  ...
  const deploymentMode = preferTrustedLocal
    ? "local_trusted"
    : (parseEnumFromEnv<DeploymentMode>(process.env.ODYSSEUS_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted");
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.ODYSSEUS_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  const deploymentExposure =
    deploymentMode === "local_trusted" ? "private" : (deploymentExposureFromEnv ?? "private");
  const bindFromEnv = parseEnumFromEnv<BindMode>(process.env.ODYSSEUS_BIND, BIND_MODES);
  const customBindHostFromEnv = process.env.ODYSSEUS_BIND_HOST?.trim() || undefined;
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.ODYSSEUS_TAILNET_BIND_HOST?.trim(),
  });
  const authPublicBaseUrl = publicUrl;
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.ODYSSEUS_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.ODYSSEUS_ALLOWED_HOSTNAMES
    ? process.env.ODYSSEUS_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.ODYSSEUS_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.ODYSSEUS_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.ODYSSEUS_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.ODYSSEUS_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.ODYSSEUS_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.ODYSSEUS_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.ODYSSEUS_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.ODYSSEUS_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.ODYSSEUS_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.ODYSSEUS_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.ODYSSEUS_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.ODYSSEUS_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.ODYSSEUS_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.ODYSSEUS_SECRETS_MASTER_KEY_FILE) ??
  ...
  if (deploymentMode === "local_trusted" && process.env.ODYSSEUS_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "ODYSSEUS_DEPLOYMENT_EXPOSURE",
      reason: "Ignored because deployment mode local_trusted always forces private exposure",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.ODYSSEUS_BIND !== undefined) {
    ignoredEnvKeys.push({
      key: "ODYSSEUS_BIND",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.ODYSSEUS_BIND_HOST !== undefined) {
    ignoredEnvKeys.push({
      key: "ODYSSEUS_BIND_HOST",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
...
function canCreateBootstrapInviteImmediately(config: Pick<OdysseusConfig, "database" | "server">): boolean {
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" odysseus onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveOdysseusInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: OdysseusConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing Odysseus install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("odysseus configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("ODYSSEUS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.ODYSSEUS_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("ODYSSEUS_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("ODYSSEUS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: ODYSSEUS_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("odysseus run")}`,
        `Reconfigure later: ${pc.cyan("odysseus configure")}`,
        `Diagnose setup: ${pc.cyan("odysseus doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.ODYSSEUS_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing Odysseus setup is ready.");
    return;
  }
  ...
  let llm: OdysseusConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@odysseus/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `odysseus doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("odysseus run")}`,
        `Then: ${pc.cyan("odysseus auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }

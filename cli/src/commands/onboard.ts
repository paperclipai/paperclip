...
type OnboardDefaults = Pick<ValadrienOsConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;
...
const ONBOARD_ENV_KEYS = [
  "VALADRIEN_OS_PUBLIC_URL",
  "DATABASE_URL",
  "VALADRIEN_OS_DB_BACKUP_ENABLED",
  "VALADRIEN_OS_DB_BACKUP_INTERVAL_MINUTES",
  "VALADRIEN_OS_DB_BACKUP_RETENTION_DAYS",
  "VALADRIEN_OS_DB_BACKUP_DIR",
  "VALADRIEN_OS_DEPLOYMENT_MODE",
  "VALADRIEN_OS_DEPLOYMENT_EXPOSURE",
  "VALADRIEN_OS_BIND",
  "VALADRIEN_OS_BIND_HOST",
  "VALADRIEN_OS_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "VALADRIEN_OS_ALLOWED_HOSTNAMES",
  "VALADRIEN_OS_AUTH_BASE_URL_MODE",
  "VALADRIEN_OS_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "VALADRIEN_OS_STORAGE_PROVIDER",
  "VALADRIEN_OS_STORAGE_LOCAL_DIR",
  "VALADRIEN_OS_STORAGE_S3_BUCKET",
  "VALADRIEN_OS_STORAGE_S3_REGION",
  "VALADRIEN_OS_STORAGE_S3_ENDPOINT",
  "VALADRIEN_OS_STORAGE_S3_PREFIX",
  "VALADRIEN_OS_STORAGE_S3_FORCE_PATH_STYLE",
  "VALADRIEN_OS_SECRETS_PROVIDER",
  "VALADRIEN_OS_SECRETS_STRICT_MODE",
  "VALADRIEN_OS_SECRETS_MASTER_KEY_FILE",
] as const;
...
function describeServerBinding(server: Pick<ValadrienOsConfig["server"], "bind" | "customBindHost" | "host" | "port">): string {
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const publicUrl = preferTrustedLocal
    ...
    : (
      ...
      process.env.VALADRIEN_OS_AUTH_PUBLIC_BASE_URL?.trim() ||
  ...
  const deploymentMode = preferTrustedLocal
    ? "local_trusted"
    : (parseEnumFromEnv<DeploymentMode>(process.env.VALADRIEN_OS_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted");
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.VALADRIEN_OS_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  const deploymentExposure =
    deploymentMode === "local_trusted" ? "private" : (deploymentExposureFromEnv ?? "private");
  const bindFromEnv = parseEnumFromEnv<BindMode>(process.env.VALADRIEN_OS_BIND, BIND_MODES);
  const customBindHostFromEnv = process.env.VALADRIEN_OS_BIND_HOST?.trim() || undefined;
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.VALADRIEN_OS_TAILNET_BIND_HOST?.trim(),
  });
  const authPublicBaseUrl = publicUrl;
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.VALADRIEN_OS_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.VALADRIEN_OS_ALLOWED_HOSTNAMES
    ? process.env.VALADRIEN_OS_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.VALADRIEN_OS_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.VALADRIEN_OS_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.VALADRIEN_OS_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.VALADRIEN_OS_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.VALADRIEN_OS_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.VALADRIEN_OS_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.VALADRIEN_OS_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.VALADRIEN_OS_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.VALADRIEN_OS_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.VALADRIEN_OS_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.VALADRIEN_OS_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.VALADRIEN_OS_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.VALADRIEN_OS_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.VALADRIEN_OS_SECRETS_MASTER_KEY_FILE) ??
  ...
  if (deploymentMode === "local_trusted" && process.env.VALADRIEN_OS_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "VALADRIEN_OS_DEPLOYMENT_EXPOSURE",
      reason: "Ignored because deployment mode local_trusted always forces private exposure",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.VALADRIEN_OS_BIND !== undefined) {
    ignoredEnvKeys.push({
      key: "VALADRIEN_OS_BIND",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.VALADRIEN_OS_BIND_HOST !== undefined) {
    ignoredEnvKeys.push({
      key: "VALADRIEN_OS_BIND_HOST",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
...
function canCreateBootstrapInviteImmediately(config: Pick<ValadrienOsConfig, "database" | "server">): boolean {
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" valadrien-os onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveValadrienOsInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: ValadrienOsConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing ValadrienOs install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("valadrien-os configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("VALADRIEN_OS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.VALADRIEN_OS_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("VALADRIEN_OS_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("VALADRIEN_OS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: VALADRIEN_OS_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("valadrien-os run")}`,
        `Reconfigure later: ${pc.cyan("valadrien-os configure")}`,
        `Diagnose setup: ${pc.cyan("valadrien-os doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.VALADRIEN_OS_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing ValadrienOs setup is ready.");
    return;
  }
  ...
  let llm: ValadrienOsConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@valadrien-os/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `valadrien-os doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("valadrien-os run")}`,
        `Then: ${pc.cyan("valadrien-os auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }

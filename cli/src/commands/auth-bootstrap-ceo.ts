...
import { createDb, instanceUserRoles, invites } from "@valadrien-os/db";
...
export async function bootstrapCeoInvite(opts: {
  ...
  const config = readConfig(configPath);
  if (!config) {
    p.log.error(`No config found at ${configPath}. Run ${pc.cyan("valadrien-os onboard")} first.`);
    return;
  }
  ...
  try {
    ...
    p.log.info("If using embedded-postgres, start the ValadrienOs server and run this command again.");
  } finally {

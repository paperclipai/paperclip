...
function collectDeploymentEnvRows(config: IronworksConfig | null, configPath: string): EnvVarRow[] {
  ...
  const secretsProvider =
    process.env.IRONWORKS_SECRETS_PROVIDER ??
    config?.secrets?.provider ??
    DEFAULT_SECRETS_PROVIDER;
  const secretsStrictMode =
    process.env.IRONWORKS_SECRETS_STRICT_MODE ??
  ...
  const storageProvider =
    process.env.IRONWORKS_STORAGE_PROVIDER ??
  ...
  const storageS3Bucket =
    process.env.IRONWORKS_STORAGE_S3_BUCKET ??
    config?.storage?.s3?.bucket ??
    "ironworks";
  const storageS3Region =
    process.env.IRONWORKS_STORAGE_S3_REGION ??
    config?.storage?.s3?.region ??
    "us-east-1";
  const storageS3Endpoint =
    process.env.IRONWORKS_STORAGE_S3_ENDPOINT ??
    config?.storage?.s3?.endpoint ??
    "";
  const storageS3Prefix =
    process.env.IRONWORKS_STORAGE_S3_PREFIX ??
    config?.storage?.s3?.prefix ??
    "";
  const storageS3ForcePathStyle =
    process.env.IRONWORKS_STORAGE_S3_FORCE_PATH_STYLE ??
  ...
  const rows: EnvVarRow[] = [
    ...
    },
    {
      ...
      source: process.env.IRONWORKS_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    },
    {
      ...
      source: process.env.IRONWORKS_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    },
    {
      ...
      source: process.env.IRONWORKS_STORAGE_S3_BUCKET
        ? "env"
        : config?.storage?.s3?.bucket
          ? "config"
          : "default",
    ...
    },
    {
      ...
      source: process.env.IRONWORKS_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    },
    {
      ...
      source: process.env.IRONWORKS_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    },
    {
      ...
      source: process.env.IRONWORKS_STORAGE_S3_PREFIX
        ? "env"
        : config?.storage?.s3?.prefix
          ? "config"
          : "default",

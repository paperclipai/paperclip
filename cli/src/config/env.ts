...
const JWT_SECRET_ENV_KEY = "AITEAMCORP_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by AiTeamCorp CLI commands",
...
export function resolveAiTeamCorpEnvFile(configPath?: string): string {
...
export function loadAiTeamCorpEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeAiTeamCorpEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readAiTeamCorpEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeAiTeamCorpEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeAiTeamCorpEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readAiTeamCorpEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeAiTeamCorpEnvEntries(next, filePath);
  return next;
}

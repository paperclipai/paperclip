...
const JWT_SECRET_ENV_KEY = "VALADRIEN_OS_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by ValadrienOs CLI commands",
...
export function resolveValadrienOsEnvFile(configPath?: string): string {
...
export function loadValadrienOsEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeValadrienOsEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readValadrienOsEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeValadrienOsEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeValadrienOsEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readValadrienOsEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeValadrienOsEnvEntries(next, filePath);
  return next;
}

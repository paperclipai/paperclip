...
const JWT_SECRET_ENV_KEY = "ODYSSEUS_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Odysseus CLI commands",
...
export function resolveOdysseusEnvFile(configPath?: string): string {
...
export function loadOdysseusEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeOdysseusEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readOdysseusEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeOdysseusEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeOdysseusEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readOdysseusEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeOdysseusEnvEntries(next, filePath);
  return next;
}

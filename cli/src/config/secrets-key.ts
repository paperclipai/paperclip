...
import type { AiTeamCorpConfig } from "./schema.js";
...
export function ensureLocalSecretsKeyFile(
  config: Pick<AiTeamCorpConfig, "secrets">,
  configPath?: string,
  ...
  const envMasterKey = process.env.AITEAMCORP_SECRETS_MASTER_KEY;
  if (envMasterKey && envMasterKey.trim().length > 0) {
    return { status: "skipped_env", path: null };
  }

  const keyFileOverride = process.env.AITEAMCORP_SECRETS_MASTER_KEY_FILE;

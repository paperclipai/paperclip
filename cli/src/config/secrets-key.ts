...
import type { OdysseusConfig } from "./schema.js";
...
export function ensureLocalSecretsKeyFile(
  config: Pick<OdysseusConfig, "secrets">,
  configPath?: string,
  ...
  const envMasterKey = process.env.ODYSSEUS_SECRETS_MASTER_KEY;
  if (envMasterKey && envMasterKey.trim().length > 0) {
    return { status: "skipped_env", path: null };
  }

  const keyFileOverride = process.env.ODYSSEUS_SECRETS_MASTER_KEY_FILE;

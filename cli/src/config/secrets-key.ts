...
import type { ValadrienOsConfig } from "./schema.js";
...
export function ensureLocalSecretsKeyFile(
  config: Pick<ValadrienOsConfig, "secrets">,
  configPath?: string,
  ...
  const envMasterKey = process.env.VALADRIEN_OS_SECRETS_MASTER_KEY;
  if (envMasterKey && envMasterKey.trim().length > 0) {
    return { status: "skipped_env", path: null };
  }

  const keyFileOverride = process.env.VALADRIEN_OS_SECRETS_MASTER_KEY_FILE;

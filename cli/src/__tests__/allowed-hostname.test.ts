...
function writeBaseConfig(configPath: string) {
  const base: AiTeamCorpConfig = {
    ...
    database: {
      ...
      embeddedPostgresDataDir: "/tmp/aiteamcorp-db",
      embeddedPostgresPort: 54329,
      backup: {
        enabled: true,
        intervalMinutes: 60,
        retentionDays: 30,
        dir: "/tmp/aiteamcorp-backups",
      },
    },
    logging: {
      mode: "file",
      logDir: "/tmp/aiteamcorp-logs",
    },
    server: {
    ...
    storage: {
      provider: "local_disk",
      localDisk: { baseDir: "/tmp/aiteamcorp-storage" },
      s3: {
        bucket: "aiteamcorp",
        region: "us-east-1",
        prefix: "",
    ...
    secrets: {
      ...
      strictMode: false,
      localEncrypted: { keyFilePath: "/tmp/aiteamcorp-secrets/master.key" },
    },
  };
...
describe("allowed-hostname command", () => {
  it("adds and normalizes hostnames", async () => {
    ...
    const raw = JSON.parse(fs.readFileSync(configPath, "utf-8")) as AiTeamCorpConfig;

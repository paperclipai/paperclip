...
function createServerEnv(configPath: string, port: number, connectionString: string) {
  ...
  env.AITEAMCORP_CONFIG = configPath;
  env.DATABASE_URL = connectionString;
  env.HOST = "127.0.0.1";
  env.PORT = String(port);
  env.SERVE_UI = "false";
  env.AITEAMCORP_DB_BACKUP_ENABLED = "false";
  env.HEARTBEAT_SCHEDULER_ENABLED = "false";
  env.AITEAMCORP_MIGRATION_AUTO_APPLY = "true";
  env.AITEAMCORP_UI_DEV_MIDDLEWARE = "false";
...
function createCliEnv() {
  ...
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.AITEAMCORP_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.AITEAMCORP_MIGRATION_AUTO_APPLY;
  delete env.AITEAMCORP_UI_DEV_MIDDLEWARE;
  return env;
}
...
describeEmbeddedPostgres("aiteamcorp company import/export e2e", () => {
  ...
  it("exports a company package and imports it into new and existing companies", async () => {
    ...
    expect(readFileSync(path.join(exportDir, ".aiteamcorp.yaml"), "utf8")).toContain('schema: "aiteamcorp/v1"');
    ...
    writeFileSync(zipPath, createStoredZipArchive(portableFiles, "aiteamcorp-demo"));

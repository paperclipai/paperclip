...
function createBaseOdysseusEnv(options: TestOdysseusEnv) {
  ...
  env.ODYSSEUS_CONFIG = options.configPath;
  env.ODYSSEUS_HOME = options.odysseusHome;
  env.ODYSSEUS_INSTANCE_ID = options.instanceId;
  env.ODYSSEUS_CONTEXT = path.join(options.odysseusHome, "context.json");
  env.ODYSSEUS_AUTH_STORE = path.join(options.odysseusHome, "auth.json");
...
function createServerEnv(
  ...
  options: Omit<TestOdysseusEnv, "configPath">,
) {
  const env = createBaseOdysseusEnv({
    configPath,
    ...options,
  });
  ...
  env.SERVE_UI = "false";
  env.ODYSSEUS_DB_BACKUP_ENABLED = "false";
  env.HEARTBEAT_SCHEDULER_ENABLED = "false";
  env.ODYSSEUS_MIGRATION_AUTO_APPLY = "true";
  env.ODYSSEUS_UI_DEV_MIDDLEWARE = "false";

  return env;
}

function createCliEnv(options: TestOdysseusEnv) {
  const env = createBaseOdysseusEnv(options);
  delete env.DATABASE_URL;
  delete env.PORT;
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.ODYSSEUS_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.ODYSSEUS_MIGRATION_AUTO_APPLY;
  delete env.ODYSSEUS_UI_DEV_MIDDLEWARE;
  return env;
}
...
async function runCliJson<T>(
  args: string[],
  opts: TestOdysseusEnv & { apiBase?: string; includeConfigArg?: boolean },
) {
  const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../..");
  const cliArgs = ["--silent", "odysseus", ...args];
  if (opts.apiBase) {
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `odysseus run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describeEmbeddedPostgres("odysseus company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
  let odysseusHome = "";
  let cliShellHome = "";
  let odysseusInstanceId = "";
  let serverProcess: ServerProcess | null = null;
  let tempDb: Awaited<ReturnType<typeof startEmbeddedPostgresTestDatabase>> | null = null;

  beforeAll(async () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "odysseus-company-cli-e2e-"));
    configPath = path.join(tempRoot, "config", "config.json");
    exportDir = path.join(tempRoot, "exported-company");
    odysseusHome = path.join(tempRoot, "odysseus-home");
    cliShellHome = path.join(tempRoot, "shell-home");
    odysseusInstanceId = "company-cli-e2e";
    mkdirSync(odysseusHome, { recursive: true });
    mkdirSync(cliShellHome, { recursive: true });

    tempDb = await startEmbeddedPostgresTestDatabase("odysseus-company-cli-db-");
    ...
    const child = spawn(
      "pnpm",
      ["odysseus", "run", "--config", configPath],
      {
        cwd: repoRoot,
        env: createServerEnv(configPath, port, tempDb.connectionString, {
          odysseusHome,
          instanceId: odysseusInstanceId,
  ...
  it("exports a company package and imports it into new and existing companies", async () => {
    ...
    const expectedContextPath = path.join(odysseusHome, "context.json");
    const leakedContextPath = path.join(cliShellHome, ".odysseus", "context.json");
    ...
    expect(readFileSync(path.join(exportDir, ".odysseus.yaml"), "utf8")).toContain('schema: "odysseus/v1"');
    ...
    writeFileSync(zipPath, createStoredZipArchive(portableFiles, "odysseus-demo"));

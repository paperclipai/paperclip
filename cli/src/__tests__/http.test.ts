...
import { ApiConnectionError, ApiRequestError, ValadrienOsApiClient } from "../client/http.js";

describe("ValadrienOsApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new ValadrienOsApiClient({
    ...
    expect(headers["x-valadrien-os-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new ValadrienOsApiClient({ apiBase: "http://localhost:3100" });
  ...
  it("throws ApiConnectionError with recovery guidance when fetch fails", async () => {
    ...
    await expect(client.post("/api/companies/import/preview", {})).rejects.toThrow(
      /pnpm dev|pnpm valadrien-os run/,
    );
  });

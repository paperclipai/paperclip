...
import { ApiConnectionError, ApiRequestError, AiTeamCorpApiClient } from "../client/http.js";

describe("AiTeamCorpApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new AiTeamCorpApiClient({
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new AiTeamCorpApiClient({ apiBase: "http://localhost:3100" });
  ...
  it("throws ApiConnectionError with recovery guidance when fetch fails", async () => {
    ...
    await expect(client.post("/api/companies/import/preview", {})).rejects.toThrow(
      /Could not reach the AiTeamCorp API\./,
    );
    await expect(client.post("/api/companies/import/preview", {})).rejects.toThrow(
      /curl http:\/\/localhost:3100\/api\/health/,
    );
    await expect(client.post("/api/companies/import/preview", {})).rejects.toThrow(
      /pnpm dev|pnpm aiteamcorp run/,
    );
  });

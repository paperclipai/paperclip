...
import { ApiConnectionError, ApiRequestError, OdysseusApiClient } from "../client/http.js";

describe("OdysseusApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new OdysseusApiClient({
    ...
    expect(headers["x-odysseus-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new OdysseusApiClient({ apiBase: "http://localhost:3100" });

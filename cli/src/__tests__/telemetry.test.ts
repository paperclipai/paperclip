...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.VALADRIEN_OS_HOME = path.join(root, "home");
    process.env.VALADRIEN_OS_INSTANCE_ID = "telemetry-test";

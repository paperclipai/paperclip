...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "odysseus-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.ODYSSEUS_HOME = path.join(root, "home");
    process.env.ODYSSEUS_INSTANCE_ID = "telemetry-test";

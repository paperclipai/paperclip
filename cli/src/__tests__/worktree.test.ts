...
function buildSourceConfig(): ValadrienOsConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("valadrien-os-pr-432")).toBe(
      path.resolve(os.homedir(), "valadrien-os-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("valadrien-os/pr-432")).toThrow(
  ...
  it("rewrites auth URLs only when they already include a port", () => {
    ...
    expect(rewriteLocalUrlPort("https://valadrien-os.example", 3110)).toBe("https://valadrien-os.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/valadrien-os-feature",
      homeDir: "/tmp/valadrien-os-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/valadrien-os-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/valadrien-os-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.VALADRIEN_OS_HOME).toBe(path.resolve("/tmp/valadrien-os-worktrees"));
    expect(env.VALADRIEN_OS_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.VALADRIEN_OS_IN_WORKTREE).toBe("true");
    expect(env.VALADRIEN_OS_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.VALADRIEN_OS_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export VALADRIEN_OS_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  itEmbeddedPostgres("quarantines copied live execution state in seeded worktree databases", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-worktree-quarantine-");
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-secrets-"));
    const originalInlineMasterKey = process.env.VALADRIEN_OS_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.VALADRIEN_OS_SECRETS_MASTER_KEY_FILE;
    try {
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.VALADRIEN_OS_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.VALADRIEN_OS_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.VALADRIEN_OS_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          VALADRIEN_OS_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.VALADRIEN_OS_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.VALADRIEN_OS_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".valadrien-os-worktrees"),
      });

      const envPath = path.join(repoRoot, ".valadrien-os", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("VALADRIEN_OS_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("VALADRIEN_OS_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/VALADRIEN_OS_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.VALADRIEN_OS_AGENT_JWT_SECRET;
      } else {
        process.env.VALADRIEN_OS_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves repo-managed worktree checkouts when --force re-runs from the source repo", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-force-preserve-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      const repoConfigDir = path.join(repoRoot, ".valadrien-os");
  ...
  itEmbeddedPostgres(
    ...
    async () => {
      const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-auth-seed-"));
      ...
      const worktreeHome = path.join(tempRoot, ".valadrien-os-worktrees");
      const originalCwd = process.cwd();
      const sourceDb = await startEmbeddedPostgresTestDatabase("valadrien-os-worktree-auth-source-");

      try {
        const sourceDbClient = createDb(sourceDb.connectionString);
        await sourceDbClient.insert(authUsers).values({
          id: "user-existing",
          email: "existing@TODO_DOMAIN",
          name: "Existing User",
          emailVerified: true,
          createdAt: new Date(),
          updatedAt: new Date(),
        ...
        const targetConfig = JSON.parse(
          fs.readFileSync(path.join(worktreeRoot, ".valadrien-os", "config.json"), "utf8"),
        ) as ValadrienOsConfig;
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".valadrien-os-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".valadrien-os", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local ValadrienOs config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".valadrien-os", "config.json");
    const originalCwd = process.cwd();
    const originalValadrienOsConfig = process.env.VALADRIEN_OS_CONFIG;
    ...
    try {
      ...
      process.chdir(originalCwd);
      if (originalValadrienOsConfig === undefined) {
        delete process.env.VALADRIEN_OS_CONFIG;
      } else {
        process.env.VALADRIEN_OS_CONFIG = originalValadrienOsConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("derives worktree reseed target paths from the adjacent env file", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-reseed-target-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".valadrien-os", "config.json");
    const envPath = path.join(worktreeRoot, ".valadrien-os", ".env");
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          ...
          "VALADRIEN_OS_INSTANCE_ID=pap-1132-chat",
        ].join("\n"),
        "utf8",
      );
      expect(
  ...
  it("rejects reseed targets without worktree env metadata", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-reseed-target-missing-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".valadrien-os", "config.json");
    ...
    try {
      fs.mkdirSync(path.dirname(configPath), { recursive: true });
      fs.writeFileSync(configPath, JSON.stringify(buildSourceConfig()), "utf8");
      fs.writeFileSync(path.join(worktreeRoot, ".valadrien-os", ".env"), "", "utf8");

      expect(() =>
        resolveWorktreeReseedTargetPaths({
          configPath,
          rootPath: worktreeRoot,
        })).toThrow("does not look like a worktree-local ValadrienOs instance");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("reseed preserves the current worktree ports, instance id, and branding", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-reseed-"));
    const repoRoot = path.join(tempRoot, "repo");
    ...
    const sourcePaths = resolveWorktreeLocalPaths({
      cwd: sourceRoot,
      homeDir: path.join(tempRoot, ".valadrien-os-source"),
      instanceId: "default",
    });
    const originalCwd = process.cwd();
    const originalValadrienOsConfig = process.env.VALADRIEN_OS_CONFIG;
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          ...
          `VALADRIEN_OS_INSTANCE_ID=${currentInstanceId}`,
          "VALADRIEN_OS_WORKTREE_NAME=existing-name",
          "VALADRIEN_OS_WORKTREE_COLOR=\"#112233\"",
        ].join("\n"),
        "utf8",
      );
      ...
      expect(rewrittenEnv).toContain(`VALADRIEN_OS_INSTANCE_ID=${currentInstanceId}`);
      expect(rewrittenEnv).toContain("VALADRIEN_OS_WORKTREE_NAME=existing-name");
      expect(rewrittenEnv).toContain("VALADRIEN_OS_WORKTREE_COLOR=\"#112233\"");
    } finally {
      process.chdir(originalCwd);
      if (originalValadrienOsConfig === undefined) {
        delete process.env.VALADRIEN_OS_CONFIG;
  ...
  it("restores the current worktree config and instance data if reseed fails", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-reseed-rollback-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceRoot = path.join(tempRoot, "source");
    const homeDir = path.join(tempRoot, ".valadrien-os-worktrees");
    ...
    try {
      ...
      fs.writeFileSync(currentPaths.configPath, JSON.stringify(currentConfig, null, 2), "utf8");
      fs.writeFileSync(currentPaths.envPath, `VALADRIEN_OS_HOME=${homeDir}\nVALADRIEN_OS_INSTANCE_ID=${currentInstanceId}\n`, "utf8");
      ...
      expect(restoredEnv).toContain(`VALADRIEN_OS_INSTANCE_ID=${currentInstanceId}`);
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/valadrien-os",
        targetRepoRoot: "/Users/example/valadrien-os-pr-432",
        workspaceCwd: "/Users/example/valadrien-os",
      }),
    ).toBe("/Users/example/valadrien-os-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/valadrien-os",
        targetRepoRoot: "/Users/example/valadrien-os-pr-432",
        workspaceCwd: "/Users/example/valadrien-os/packages/db",
      }),
    ).toBe("/Users/example/valadrien-os-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "valadrien-os-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("valadrien-os-make-test", {
        seed: false,
        home: path.join(tempRoot, ".valadrien-os-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".valadrien-os", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".valadrien-os", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
  ...
  it("no-ops on the primary checkout unless --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-repair-primary-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      expect(fs.existsSync(path.join(repoRoot, ".valadrien-os", "config.json"))).toBe(false);
      expect(fs.existsSync(path.join(repoRoot, ".valadrien-os", "worktrees"))).toBe(false);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  ...
  it("repairs the current linked worktree when ValadrienOs metadata is missing", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-repair-current-"));
    const repoRoot = path.join(tempRoot, "repo");
    const worktreePath = path.join(repoRoot, ".valadrien-os", "worktrees", "repair-me");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".valadrien-os-worktrees");
  ...
  it("creates and repairs a missing branch worktree when --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-worktree-repair-branch-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".valadrien-os-worktrees");
    const originalCwd = process.cwd();
    const expectedWorktreePath = path.join(repoRoot, ".valadrien-os", "worktrees", "feature-repair-me");
    ...
    try {
      ...
      expect(fs.existsSync(path.join(expectedWorktreePath, ".valadrien-os", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(expectedWorktreePath, ".valadrien-os", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
...
describeEmbeddedPostgres("pauseSeededScheduledRoutines", () => {
  it("pauses only routines with enabled schedule triggers", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-worktree-routines-");
    const db = createDb(tempDb.connectionString);
    const companyId = randomUUID();

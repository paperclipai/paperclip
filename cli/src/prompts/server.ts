...
import { isLoopbackHost, type BindMode } from "@valadrien-os/shared";
...
const TAILNET_BIND_WARNING =
  "No Tailscale address was detected during setup. The saved config will stay on loopback until Tailscale is available or VALADRIEN_OS_TAILNET_BIND_HOST is set.";
...
export async function promptServer(opts?: {
  ...
  if (deploymentMode === "authenticated" && exposure === "public") {
    const urlInput = await p.text({
      ...
      placeholder: "https://valadrien-os.example.com",
      validate: (val) => {

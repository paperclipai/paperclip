...
import { resolveDefaultStorageDir, resolveAiTeamCorpInstanceId } from "../config/home.js";

function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveAiTeamCorpInstanceId());
}
...
export async function promptStorage(current?: StorageConfig): Promise<StorageConfig> {
  ...
  const bucket = await p.text({
    message: "S3 bucket",
    defaultValue: base.s3.bucket || "aiteamcorp",

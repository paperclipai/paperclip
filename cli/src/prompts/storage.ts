...
import { resolveDefaultStorageDir, resolveValadrienOsInstanceId } from "../config/home.js";

function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveValadrienOsInstanceId());
}
...
export async function promptStorage(current?: StorageConfig): Promise<StorageConfig> {
  ...
  const bucket = await p.text({
    message: "S3 bucket",
    defaultValue: base.s3.bucket || "valadrien-os",

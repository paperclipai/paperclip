...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Cortex API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Cortex server is not running, the configured URL is wrong, or the request is being blocked before it reaches Paperclip.",
    "",
    "Try:",
    "- Start Cortex with `pnpm dev` or `pnpm paperclipai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Cortex is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`PAPERCLIP_API_URL\`.`,
  );
  return lines.join("\n");
}

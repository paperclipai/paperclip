...
export class OdysseusApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-odysseus-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Odysseus API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Odysseus server is not running, the configured URL is wrong, or the request is being blocked before it reaches Odysseus.",
    "",
    "Try:",
    "- Start Odysseus with `pnpm dev` or `pnpm odysseus run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Odysseus is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`ODYSSEUS_API_URL\`.`,
  );
  return lines.join("\n");
}

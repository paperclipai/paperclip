...
export async function checkForUpdateNotice(options: { configPath?: string; now?: number; fetchImpl?: typeof fetch; cachePath?: string } = {}): Promise<string | null> {
  ...
  const manifestChannel = readInstallManifest(paths)?.channel;
  const tag = manifestChannel && manifestChannel !== "pinned" ? manifestChannel : "latest";
  try { const cache = JSON.parse(fs.readFileSync(cachePath, "utf8")) as { checkedAt?: number; latest?: string; tag?: string }; if ((cache.tag ?? "latest") === tag && cache.checkedAt && now - cache.checkedAt < NOTICE_INTERVAL_MS) return cache.latest && compareVersions(cache.latest, packageVersion) > 0 ? cache.latest : null; } catch {}

/**
 * esbuild configuration for building the odysseus CLI for npm.
 *
 * Bundles all workspace packages (@odysseus/*) into a single file.
...
// to npm and resolved at runtime (e.g. @odysseus/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@odysseus/")) {
      externals.add(name);
    }
  }

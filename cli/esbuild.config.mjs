/**
 * esbuild configuration for building the aiteamcorp CLI for npm.
 *
 * Bundles all workspace packages (@aiteamcorp/*) into a single file.
...
// to npm and resolved at runtime (e.g. @aiteamcorp/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@aiteamcorp/")) {
      externals.add(name);
    }
  }

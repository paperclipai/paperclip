const fs = require("fs");
const all = JSON.parse(fs.readFileSync("/work/.ckshots/emails.json", "utf8"));
// Inbound = not sent from our own identities (Tres Hermanos / Divino).
const rows = all
  .filter((e) => !/treshermanos\.|divinocigars/i.test(String(e.fromString || "")))
  .map((e) => ({ id: e.id, date: e.dateSent, from: e.fromString || "", subject: e.name || "", snippet: String(e.bodyPlain || "").replace(/\s+/g, " ").slice(0, 240) }));
...
fs.writeFileSync("/work/.ckshots/labeled.tsv", out.join("\n") + "\n");
console.log("rows: " + rows.length);
console.log("by relationship: " + JSON.stringify(counts.rel));
console.log("lead_relevant: " + JSON.stringify(counts.lead));

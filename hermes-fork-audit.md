...
| 13 | Docker runtime assumptions: binary path, Python venv, uv/uvx, Playwright path | `registry.ts:574` (default `hermesHome` = `/paperclip/hermes`), `hermes-test.ts:34` (default `hermesHome` = `/paperclip/hermes`) | No upstream equivalent. Upstream uses `os.homedir()` → `~/.hermes`; adapter is container-agnostic. | resolved | enhancement | Container paths belong in Dockerfile ENV (PR #2), not hardcoded in adapter. The adapter should remain path-agnostic; the Docker image provides the correct `HERMES_HOME` and `PATH` at build time. Fork's `/paperclip/hermes` default will be superseded by Dockerfile `ENV HERMES_HOME=/paperclip/hermes`. _PR #1 begins row 13 closure: the `fix-docker-deploy-and-verify-hermes/pr-1` adapter change introduces `packages/adapters/hermes/src/server/hermes-home.ts` (HERMES_HOME > HOME > USERPROFILE > os.homedir() resolver) and migrates `skills.ts`, `detect-model.ts`, and `test.ts` to use it. Rows 4 and 13 are marked `resolved` in PR #3 of the same change, with row 13 attributed to two sources: the prior upstream sync that already removed fork adapter-side `/paperclip/hermes` hardcoding by restoring upstream `HOME + .hermes` behavior, and the `hermes-home.ts` resolver that adds operator-level `HERMES_HOME` override support._ Resolved by `fix-docker-deploy-and-verify-hermes` change (PR #3) — date: 2026-07-01 — see https://github.com/MDominicRQ/paperclip/pull/<will-be-number>. Two-source attribution: (a) previous upstream `sync-upstream-internalize-hermes` sync restored upstream `HOME + .hermes` discovery, removing the fork's adapter-side `/paperclip/hermes` hardcoding; (b) `fix-docker-deploy-and-verify-hermes/pr-1`'s `hermes-home.ts` resolver adds operator-level `HERMES_HOME` override support that completes the row. |
...
| `follow-up-issue` | 1 | 6 (wake prompt enhancement — not blocking) |
| `resolved` (closed by `fix-docker-deploy-and-verify-hermes`) | 2 | 4, 13 |

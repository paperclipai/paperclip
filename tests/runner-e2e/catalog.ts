...
function warmTurnInstructions(turn: 1 | 2 | 3, nonce: string) {
  ...
  const nativeReviewRequest = {
    interactionKind: "confirmation",
    idempotencyKey: `daytona-warm-review-T${turn}-${nonce}`,
    title: `Warm continuity turn ${turn}`,
    prompt: `Is this warm continuity task ready to complete after turn ${turn}?`,
    ...
    payload: {
      ...
      target: { type: "custom", key: `daytona_warm_turn_${turn}`, revisionId: `${nonce}-T${turn}`, label: `Warm continuity turn ${turn}` },
    },
  };
  ...
  return [
    ...
    ...(!finalTurn ? [`In a native runner, after verification and before reporting needs_review, call request_human_input with ${JSON.stringify(nativeReviewRequest)}. Reuse its idempotencyKey on a retry; do not create another card. The pending card is the review gate; do not report done while it is waiting.`] : []),

...
describe("runner E2E catalog", () => {
  ...
  it("validates the core, local-integrity, breadth, and warm suites", () => {
    ...
    expect(validateRunnerCatalog()).toHaveLength(170);
    expect(new Set(runnerMatrix.map((entry) => entry.id)).size).toBe(170);
    expect(
...
describe("runner E2E selectors", () => {
  ...
  it("emits one independently schedulable job per scenario", () => {
    ...
    expect(jobs).toHaveLength(140);
    expect(jobs.filter((job) => job.needsDaytona)).toHaveLength(23);
    expect(jobs.filter((job) => !job.needsDaytona)).toHaveLength(117);
    expect(new Set(jobs.map((job) => job.executionId)).size).toBe(140);
    expect(
      jobs.find(
        (job) =>
          job.executionId ===

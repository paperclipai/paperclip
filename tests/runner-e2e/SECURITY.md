...
evidence, and declared runner PNG screenshots are expected public data. A
failed result may also publish the fixed `failure.png` capture. Screenshot
paths must be safe PNG basenames and must be tied to the exact normalized
execution ID and attempt. The CloudFront-backed S3 history also publishes one
...
raw/unredacted logs, and unallowlisted files are not public. Allowlisted `.log`
copies must pass the existing exact-value/key-shape scan and redaction boundary.
...
secret-scanned by bytes, so fixture authors must treat every captured page as
public and must never navigate screenshots to credentials, secrets, private
user data, or other non-public content. Videos remain access-controlled.
...
declared result PNGs, and failed-cell `failure.png` captures.
...
with the same declared screenshot allowlist. A leak detected by the existing
packager scan fails the cell and withholds the unsafe file.

...
test("captures planning mode UI for desktop and mobile", async ({ page }) => {
  ...
  await page.route("**/agent-hires", async (route) => {
    ...
    const real = await fetch(targetUrl.toString(), {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        // RES-1298: the route handler runs in Node, not the page, so the
        // browser-set Origin is lost. Re-issue with Origin = the request's
        // own origin so local_trusted promotes us to the board actor;
        // without it the POST bounces with 401/403 and the wizard never
        // reaches the "Review" heading (this spec times out at line 67).
        // Same pattern as conference-room-typing-intro.spec.ts (2fa9df2e).

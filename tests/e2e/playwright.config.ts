...
const PORT = Number(process.env.ODYSSEUS_E2E_PORT ?? 3199);
const BASE_URL = `http://127.0.0.1:${PORT}`;
const ODYSSEUS_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "odysseus-e2e-home-"));
...
export default defineConfig({
  ...
  // `onboard --yes --run` works in a non-interactive temp ODYSSEUS_HOME.
  webServer: {
    command: `pnpm odysseus onboard --yes --run`,
    url: `${BASE_URL}/api/health`,
    // Always boot a dedicated throwaway instance for e2e so browser tests
    // never attach to the developer's active Odysseus home/server.
    ...
    env: {
      ...
      PORT: String(PORT),
      ODYSSEUS_HOME,
      ODYSSEUS_INSTANCE_ID: "playwright-e2e",
      ODYSSEUS_BIND: "loopback",
      ODYSSEUS_DEPLOYMENT_MODE: "local_trusted",

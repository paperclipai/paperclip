...
- The `odysseus` skill now teaches agents to use those compact/incremental APIs first, while keeping full-thread fetches as a cold-start fallback.
...
- Runtime skill defaults now include `odysseus`, `para-memory-files`, and `odysseus-create-agent`. `create-agent-adapter` was moved to `.agents/skills/create-agent-adapter`.
...
- In practice, this means a worktree can contain newer `skills/odysseus/SKILL.md` guidance while Codex still follows an older checkout's skill content.

...
class CallbackHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if CallbackHandler.code:
            self.wfile.write(
                "<html><body><h2>인증이 완료되었습니다.</h2><p>터미널로 돌아가세요.</p></body></html>".encode(
                    "utf-8"
                )
            )
        else:
            self.wfile.write(
                "<html><body><h2>인증에 실패했습니다.</h2><p>터미널 출력을 확인하세요.</p></body></html>".encode(
                    "utf-8"
                )
            )
...
def main():
    ...
    print()
    print("refresh_token 값을 .env 의 GOOGLE_CALENDAR_REFRESH_TOKEN 에 저장하세요.")

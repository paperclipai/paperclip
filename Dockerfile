...
# reproducible across builds. The fresh install wipes node_modules and plugins
# no longer carry a postinstall (removed for supply-chain safety, #8255), so
# re-link the in-repo @paperclipai/plugin-sdk afterwards — the same
# install-then-link order scripts/build-standalone-public-packages.mjs uses.

...
| secret.existingSecret | string | `""` | Name of a pre-existing Secret to read credentials from. Leave empty to have the chart generate one from the inline values below. |
| secret.agentJwtSecret | string | `""` | JWT secret for the Paperclip agent. Required. Auto-generated on `helm install` if empty and no `existingSecret`. |
| secret.masterKey | string | `""` | Base64-encoded master key to seed `/paperclip/instances/<instanceId>/secrets/master.key` on first boot. Ignored if the file already exists on the PVC. Leave empty to let Paperclip generate its own on first run. |
...
| pdb | object | `{"enabled":true,"minAvailable":1}` | PodDisruptionBudget blocking voluntary disruptions when only one replica is available. |
| pdb.enabled | bool | `true` | Create a PodDisruptionBudget. |
| pdb.minAvailable | int | `1` | Minimum number of pods that must remain available during disruptions. |
...
| networkPolicy.enabled | bool | `true` | Create a NetworkPolicy. Defaults to allowing from `ingress-nginx` and `cloudflare` namespaces only. |
| networkPolicy.allowFromNamespaces | list | `ingress-nginx` and `cloudflare` namespaces. | Namespaces whose pods may reach the Paperclip service port. |

...
| probes | object | `{"liveness":{"failureThreshold":3,"initialDelaySeconds":60,"periodSeconds":30},"readiness":{"failureThreshold":3,"initialDelaySeconds":15,"periodSeconds":10},"startup":{"failureThreshold":30,"periodSeconds":10}}` | Health probe tuning. The chart templates each probe's `httpGet` (to `/healthz` on the `http` port) with a `Host` header set to `allowedHostnames[0]` — the loopback the private-hostname guard always accepts (see server/src/middleware/private-hostname-guard.ts), derived from the same source as `PAPERCLIP_ALLOWED_HOSTNAMES`. Only the timing fields below are tunable. |
| probes.liveness | object | `{"failureThreshold":3,"initialDelaySeconds":60,"periodSeconds":30}` | Liveness probe timings. Paperclip serves HTTP on the configured port once the embedded Postgres is up. |
| probes.readiness | object | `{"failureThreshold":3,"initialDelaySeconds":15,"periodSeconds":10}` | Readiness probe timings. Shorter cadence pulls the pod out of rotation quickly on transient failures. |
| probes.startup | object | `{"failureThreshold":30,"periodSeconds":10}` | Startup probe timings. Long timeout tolerates first-run DB init + 45 migrations. |

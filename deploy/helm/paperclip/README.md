...
| probes | object | `{"liveness":{"failureThreshold":3,"httpGet":{"httpHeaders":[{"name":"Host","value":"127.0.0.1"}],"path":"/healthz","port":"http"},"initialDelaySeconds":60,"periodSeconds":30},"readiness":{"failureThreshold":3,"httpGet":{"httpHeaders":[{"name":"Host","value":"127.0.0.1"}],"path":"/healthz","port":"http"},"initialDelaySeconds":15,"periodSeconds":10},"startup":{"failureThreshold":30,"httpGet":{"httpHeaders":[{"name":"Host","value":"127.0.0.1"}],"path":"/healthz","port":"http"},"periodSeconds":10}}` | Health probe configuration. `httpGet` to `/healthz` with an explicit `Host: 127.0.0.1` header — Paperclip's hostname allowlist always accepts loopback (see server/src/middleware/private-hostname-guard.ts), while the pod-IP Host header kubelet would otherwise send is rejected. The header is port-agnostic (the guard strips any port), so it tracks `env.port` overrides. |
| probes.liveness | object | `{"failureThreshold":3,"httpGet":{"httpHeaders":[{"name":"Host","value":"127.0.0.1"}],"path":"/healthz","port":"http"},"initialDelaySeconds":60,"periodSeconds":30}` | Liveness probe. Paperclip serves HTTP on the configured port once the embedded Postgres is up. |
| probes.readiness | object | `{"failureThreshold":3,"httpGet":{"httpHeaders":[{"name":"Host","value":"127.0.0.1"}],"path":"/healthz","port":"http"},"initialDelaySeconds":15,"periodSeconds":10}` | Readiness probe. Shorter cadence pulls the pod out of rotation quickly on transient failures. |
| probes.startup | object | `{"failureThreshold":30,"httpGet":{"httpHeaders":[{"name":"Host","value":"127.0.0.1"}],"path":"/healthz","port":"http"},"periodSeconds":10}` | Startup probe. Long timeout tolerates first-run DB init + 45 migrations. |

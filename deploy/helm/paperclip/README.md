...
| secret | object | `{"agentJwtSecret":"","existingSecret":"paperclip-credentials","masterKey":""}` | Secret containing `agentJwtSecret` and optional `masterKey` seed. |
...
| secret.existingSecret | string | `"paperclip-credentials"` | Name of a pre-existing Secret to read credentials from. Strongly recommended under GitOps: the templated secret uses `randAlphaNum` which re-rolls on every render, keeping the app perpetually OutOfSync (and thrashing pods under `selfHeal`). See `docs/operations.md` in the ops repo for the `kubectl create secret` recipe. |

...
def main():
    ...
    for branch in ["3793", "4056", "4331", "4821"]:
        ...
        if res and "value" in res:
            loans = res["value"]
            print(f"  Total loans returned: {len(loans)}")
            statuses = [l.get("status") for l in loans]
            milestones = [l.get("milestoneCurrentName") for l in loans]
            print(f"  Statuses: {Counter(statuses)}")
            print(f"  Milestones: {Counter(milestones).most_common(5)}")
        else:

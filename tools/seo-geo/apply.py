...
from changeset import EDITABLE_FIELDS
...
def apply_changeset(changeset, client, dry_run=False) -> ApplyLog:
    ...
    for c in changeset["changes"]:
        ...
        if c.get("field") not in EDITABLE_FIELDS:
            log.skipped.append({**c, "reason": f"feld nicht in whitelist: {c.get('field')}"})
            continue
        ...
        try:
            ...
            log.failed.append({**c, "error": str(e)})
            break  # bei erstem Fehler stoppen — keine weiteren Teil-Writes

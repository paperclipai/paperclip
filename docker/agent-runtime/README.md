...
A weekly schedule (Mondays) also runs in the same workflow, but it never builds or publishes anything: it queries the npm registry for the current `latest` version of each harness's CLI package, pulls the currently published `:latest` image and reads its baked-in CLI version from the installed package's `package.json` (the image is never started, and nothing is built, signed, or pushed), and opens/updates a tracking issue (labeled `agent-runtime-cli-check`) flagging any harness where the deployed `:latest` image's version differs from npm's current latest. A harness whose version couldn't be determined (registry outage, image not published yet) is reported as `UNKNOWN` rather than `STALE`. Actually publishing a refreshed image still always requires an explicit `push` or `workflow_dispatch` run.

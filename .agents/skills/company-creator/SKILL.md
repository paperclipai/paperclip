...
- Agent instructions go in the AGENTS.md body, not in .valadrien-os.yaml
...
In this example, only `release-engineer` appears because it needs `GH_TOKEN`. The other agents (ceo, cto, etc.) have no overrides, so they are omitted entirely from `.valadrien-os.yaml`.

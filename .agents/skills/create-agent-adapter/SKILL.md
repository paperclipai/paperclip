...
description: >
  Technical guide for creating a new AiTeamCorp agent adapter. Use when building
...
# Creating a AiTeamCorp Agent Adapter

An adapter bridges AiTeamCorp's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `aiteamcorp run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`@aiteamcorp/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@aiteamcorp/adapter-utils` (types) or `@aiteamcorp/adapter-utils/server-utils` (runtime helpers).
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell AiTeamCorp to forget the stored session
}
...
```json
{
  ...
  "dependencies": {
    "@aiteamcorp/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
The `agentConfigurationDoc` is read by LLM agents (including AiTeamCorp agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@aiteamcorp/adapter-utils/server-utils`)
2. **Build environment** — call `buildAiTeamCorpEnv(agent)` then layer in `AITEAMCORP_RUN_ID`, context vars (`AITEAMCORP_TASK_ID`, `AITEAMCORP_WAKE_REASON`, `AITEAMCORP_WAKE_COMMENT_ID`, `AITEAMCORP_APPROVAL_ID`, `AITEAMCORP_APPROVAL_STATUS`, `AITEAMCORP_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| `AITEAMCORP_API_URL` | Server's own URL |
| `AITEAMCORP_RUN_ID` | Current run id |
| `AITEAMCORP_TASK_ID` | `context.taskId` or `context.issueId` |
| `AITEAMCORP_WAKE_REASON` | `context.wakeReason` |
| `AITEAMCORP_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `AITEAMCORP_APPROVAL_ID` | `context.approvalId` |
| `AITEAMCORP_APPROVAL_STATUS` | `context.approvalStatus` |
| `AITEAMCORP_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `AITEAMCORP_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `aiteamcorp run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@aiteamcorp/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@aiteamcorp/adapter-my-agent";
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use AiTeamCorp API
...
import { parseMyAgentStdoutLine } from "@aiteamcorp/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@aiteamcorp/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@aiteamcorp/adapter-my-agent/cli";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so AiTeamCorp wipes the stale session
...
Import from `@aiteamcorp/adapter-utils/server-utils`:
...
| `buildAiTeamCorpEnv(agent)` | Standard `AITEAMCORP_*` env vars |
...
- Package name: `@aiteamcorp/adapter-<kebab-name>`
...
- Default prompt: `"You are agent {{agent.id}} ({{agent.name}}). Continue your AiTeamCorp work."`
...
### AiTeamCorp Skills Injection

AiTeamCorp ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `aiteamcorp` API skill and the `aiteamcorp-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with AiTeamCorp internals, break git status, and potentially leak into commits.

**The pattern:** create a clean, isolated location for skills and tell the agent runtime to look there.

**How claude-local does it:**

1. At execution time, create a fresh tmpdir: `mkdtemp("aiteamcorp-skills-")`
...
async function buildSkillsDir(): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "aiteamcorp-skills-"));
  const target = path.join(tmp, ".claude", "skills");
  await fs.mkdir(target, { recursive: true });
  const entries = await fs.readdir(AITEAMCORP_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(AITEAMCORP_SKILLS_DIR, entry.name),
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks AiTeamCorp skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
**Skills as loaded procedures, not prompt bloat.** The AiTeamCorp skills (like `aiteamcorp` and `aiteamcorp-create-agent`) are designed as on-demand procedures: the agent sees skill metadata (name + description) in its context, but only loads the full SKILL.md content when it decides to invoke a skill. This keeps the base prompt small. When writing `agentConfigurationDoc` or prompt templates for your adapter, do not inline skill content — let the agent runtime's skill discovery do the work. The descriptions in each SKILL.md frontmatter act as routing logic: they tell the agent when to load the full skill, not what the skill contains.

**Explicit vs. fuzzy skill invocation.** For production workflows where reliability matters (e.g. an agent that must always call the AiTeamCorp API to report status), use explicit instructions in the prompt template: "Use the aiteamcorp skill to report your progress." Fuzzy routing (letting the model decide based on description matching) is fine for exploratory tasks but unreliable for mandatory procedures.

---

## 8. Security Considerations

Adapters sit at the boundary between AiTeamCorp's orchestration layer and arbitrary agent execution. This is a high-risk surface.
...
- `AITEAMCORP_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the AiTeamCorp API and GitHub should not have access to arbitrary hosts.

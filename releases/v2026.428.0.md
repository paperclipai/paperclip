...
- **Pending confirmation issues self-normalize into review** — Creating or repairing a pending `request_confirmation` interaction now moves the issue to `in_review` when the confirmation is the active gate, while leaving real dependency-blocked and terminal issues unchanged. This removes the manual board corrections where confirmation waits were left in `todo` or `blocked`. Rollback is a direct revert of the normalization hook in `server/src/services/issue-thread-interactions.ts`.

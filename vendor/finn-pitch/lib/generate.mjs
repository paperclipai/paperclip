...
import { fetchClientLogo } from "./logo.mjs";
import {
...
export async function generate(answers) {
  ...
  if (answers.clientWebsite) {
    deck._clientLogo = await fetchClientLogo(answers.clientWebsite);
    if (deck._clientLogo) console.log(`🏷️   Client logo → ${deck._clientLogo}`);
  }
  writeFileSync(join(root, "slides.json"), JSON.stringify(deck, null, 2));

...
export function industryHero(industryText = "", matched = null) {
  ...
  for (const t of tries) {
    for (const ext of ["jpg", "jpeg", "png", "webp"]) {
      if (has(`assets/industry-hero/${t}.${ext}`)) return `/asset/industry-hero/${t}.${ext}`;
    }
  }
  return null;
}
...
export function logoUrls(names = []) {
  return names.map((n) => slugify(n))
    .map((n) => (has(`assets/logos/${n}.svg`) ? `/asset/logos/${n}.svg` : null))
    .filter(Boolean);
}
export function allIntegrationLogos(limit = 12) {
  if (!has("assets/logos")) return [];
...
export function customerLogo(testi) {
  ...
  if (!has("assets/customers")) return null;
  const f = readdirSync(join(root, "assets", "customers"))
    .find((x) => x.toLowerCase().startsWith(testi.id));
  return f ? `/asset/customers/${f}` : null;
}
export function brandAsset(name) {
  return has(`assets/brand/${name}`) ? `/asset/brand/${name}` : null;
}

...
This skill covers the day-to-day workflows for developing and operating a local Paperclip instance. It assumes you are working inside the Paperclip repo checkout with `origin` pointing to `git@github.com:PossibLaw/odysseus.git`.
...
| Task | Command |
|------|---------|
| Start server (first time or normal) | `npx odysseus run` |
...
| Back up database | `npx odysseus db:backup` |
| Health check | `npx odysseus doctor --repair` |
| Print env vars | `npx odysseus env` |
| Trigger agent heartbeat | `npx odysseus heartbeat run --agent-id <id>` |
| Install agent skills locally | `npx odysseus agent local-cli <agent> --company-id <id>` |
...
npx odysseus worktree:make my-feature --start-point origin/main

# 2. Move into the worktree (path printed by worktree:make) and source the environment
cd <worktree-path>
eval "$(npx odysseus worktree env)"
...
npx odysseus worktree:merge-history --from odysseus-my-feature --to current --apply

# 6. Clean up
npx odysseus worktree:cleanup my-feature
```

## Forks — Prefer Pushing to a User Fork

If the user has a personal fork of `PossibLaw/odysseus` configured as a git remote, push your feature branches to **that fork** instead of creating branches on the main repo. This keeps the upstream branch list clean and matches the standard open-source contribution flow.

### Detect a fork remote

Before pushing or creating a PR, list remotes and check for one that points at a non-`odysseus` GitHub fork:

```bash
git remote -v
```

Treat any remote whose URL points to `github.com:<user>/odysseus` (or `github.com/<user>/odysseus.git`) as the user's fork. Common names are `fork`, `<username>`, or `myfork`. The remote named `origin` or `upstream` that points at `PossibLaw/odysseus` is the canonical upstream — do not push feature branches there if a fork exists.
...
```bash
gh pr create --repo PossibLaw/odysseus --head <fork-owner>:<branch-name> ...
```
...
If `git remote -v` shows only `PossibLaw/odysseus` remotes (no user fork), fall back to pushing branches to `origin` as before. Do NOT create a fork on the user's behalf — ask first.

### Keeping the fork up to date

The canonical remote that points at `PossibLaw/odysseus` may be named `origin` **or** `upstream` depending on how the user set up the repo. Detect it the same way as in the "Detect a fork remote" step, then fetch and push from/with that remote so the sync works under either convention:

```bash
UPSTREAM_REMOTE=$(git remote -v | awk '/odysseus\/odysseus.*\(fetch\)/{print $1; exit}')
...
1. **CLI is the only interface to worktrees and databases.** All worktree and database operations MUST go through `npx odysseus` / `pnpm odysseus` commands. You MUST NOT:
   ...
   - Run `rm -rf` on any `.odysseus/`, `.odysseus-worktrees/`, or `db/` directory
   - Directly manipulate embedded postgres data directories
   - Kill postgres processes by PID

2. **If a CLI command fails, stop and report.** Do NOT attempt workarounds. If `worktree:make`, `worktree reseed`, `worktree init`, `worktree:cleanup`, or any other `odysseus` command fails:
   - Report the exact error message in your task comment
   - Set the task to `blocked`
   - Suggest running `npx odysseus doctor --repair` or recreating the worktree from scratch
...
4. **Starting a dev server in a worktree requires setup first.** The correct sequence is:
   ...
   eval "$(npx odysseus worktree env)"
   pnpm install && pnpm build
   npx odysseus run          # or pnpm dev

   # If the worktree needs a fresh database:
   npx odysseus worktree reseed --seed-mode full

   # If the worktree is broken beyond repair:
   npx odysseus worktree:cleanup <name>
   npx odysseus worktree:make <name> --seed-mode full
   ```
...
cd <worktree-path>
eval "$(npx odysseus worktree env)"   # skip if using the primary instance
...
| Mistake | Fix |
|---------|-----|
| Server won't start | Run `npx odysseus doctor --repair` to diagnose and auto-fix |
| Forgetting to source worktree env | Run `eval "$(npx odysseus worktree env)"` after cd-ing into the worktree |
...
| Running agents against wrong instance | Verify `ODYSSEUS_API_URL` points to the correct port |
...
| Pushed feature branch to `PossibLaw/odysseus` when a fork exists | Push to the user's fork remote instead — see "Forks" above |

...
- **Empty-shell hires (Step 10 skipped).** `POST /agent-hires` with `instructionsBundle.files` does **not** persist the file content; it only writes bundle config. If the post-hire `GET /instructions-bundle` is not run (or its result is not asserted), the agent ships with `files: []` and `resolvedEntryPath: null` and runs heartbeats with an empty prompt. Treat verification as part of the hire — never optional.

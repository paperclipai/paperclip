...
```sh
curl -sS -H "Authorization: Bearer $ODYSSEUS_API_KEY" \
  "$ODYSSEUS_API_URL/api/issues/$ODYSSEUS_TASK_ID/heartbeat-context"
```
...
curl -sS -X POST \
  ...
  "$ODYSSEUS_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/start" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-Paperclip-Run-Id: $ODYSSEUS_RUN_ID" \
  -H "Content-Type: application/json" \
  "$ODYSSEUS_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/restart" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-Paperclip-Run-Id: $ODYSSEUS_RUN_ID" \
  -H "Content-Type: application/json" \
  "$ODYSSEUS_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/stop" \
  -d '{}'
```
...
- `odysseusGetIssueWorkspaceRuntime` — reads `currentExecutionWorkspace` and service URLs for an issue.
- `odysseusControlIssueWorkspaceServices` — starts, stops, or restarts the current issue workspace services.
- `odysseusWaitForIssueWorkspaceService` — waits until a selected service is running and returns its URL when exposed.

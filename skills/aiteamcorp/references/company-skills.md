...
```sh
curl -sS -X POST "$AITEAMCORP_API_URL/api/companies/$AITEAMCORP_COMPANY_ID/skills/import" \
  -H "Authorization: Bearer $AITEAMCORP_API_KEY" \
...
```sh
curl -sS -X POST "$AITEAMCORP_API_URL/api/companies/$AITEAMCORP_COMPANY_ID/skills/scan-projects" \
  -H "Authorization: Bearer $AITEAMCORP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```sh
curl -sS "$AITEAMCORP_API_URL/api/companies/$AITEAMCORP_COMPANY_ID/skills" \
  -H "Authorization: Bearer $AITEAMCORP_API_KEY"
```
...
```sh
curl -sS "$AITEAMCORP_API_URL/api/companies/$AITEAMCORP_COMPANY_ID/skills/<skill-id>" \
  -H "Authorization: Bearer $AITEAMCORP_API_KEY"

curl -sS "$AITEAMCORP_API_URL/api/companies/$AITEAMCORP_COMPANY_ID/skills/<skill-id>/files?path=SKILL.md" \
  -H "Authorization: Bearer $AITEAMCORP_API_KEY"
```
...
```sh
curl -sS -X POST "$AITEAMCORP_API_URL/api/agents/<agent-id>/skills/sync" \
  -H "Authorization: Bearer $AITEAMCORP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```sh
curl -sS "$AITEAMCORP_API_URL/api/agents/<agent-id>/skills" \
  -H "Authorization: Bearer $AITEAMCORP_API_KEY"
```
...
```sh
curl -sS -X POST "$AITEAMCORP_API_URL/api/companies/$AITEAMCORP_COMPANY_ID/agent-hires" \
  -H "Authorization: Bearer $AITEAMCORP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```sh
curl -sS -X POST "$AITEAMCORP_API_URL/api/companies/$AITEAMCORP_COMPANY_ID/agents" \
  -H "Authorization: Bearer $AITEAMCORP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "QA Browser Agent",
...
- Built-in AiTeamCorp runtime skills are still added automatically when required by the adapter.

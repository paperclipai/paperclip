...
```sh
curl -sS "$ODYSSEUS_API_URL/api/agents/me" \
...
```sh
curl -sS "$ODYSSEUS_API_URL/llms/agent-configuration.txt" \
  -H "Authorization: Bearer $ODYSSEUS_API_KEY"

# Then the specific adapter you plan to use, e.g. claude_local:
curl -sS "$ODYSSEUS_API_URL/llms/agent-configuration/claude_local.txt" \
  -H "Authorization: Bearer $ODYSSEUS_API_KEY"
```
...
```sh
curl -sS "$ODYSSEUS_API_URL/api/companies/$ODYSSEUS_COMPANY_ID/agent-configurations" \
  -H "Authorization: Bearer $ODYSSEUS_API_KEY"
```
...
`skills/odysseus-create-agent/references/agent-instruction-templates.md`

Generic fallback for no-template hires:
`skills/odysseus-create-agent/references/baseline-role-guide.md`
...
```sh
curl -sS "$ODYSSEUS_API_URL/llms/agent-icons.txt" \
  -H "Authorization: Bearer $ODYSSEUS_API_KEY"
```
...
`skills/odysseus-create-agent/references/draft-review-checklist.md`
...
- when the board approves, you will be woken with `ODYSSEUS_APPROVAL_ID`; read linked issues and close/comment follow-up

```sh
curl -sS "$ODYSSEUS_API_URL/api/approvals/<approval-id>" \
  -H "Authorization: Bearer $ODYSSEUS_API_KEY"

curl -sS -X POST "$ODYSSEUS_API_URL/api/approvals/<approval-id>/comments" \
  -H "Authorization: Bearer $ODYSSEUS_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS -X POST "$ODYSSEUS_API_URL/api/issues/<issue-id>/approvals" \
  -H "Authorization: Bearer $ODYSSEUS_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS "$ODYSSEUS_API_URL/api/approvals/$ODYSSEUS_APPROVAL_ID" \
  -H "Authorization: Bearer $ODYSSEUS_API_KEY"

curl -sS "$ODYSSEUS_API_URL/api/approvals/$ODYSSEUS_APPROVAL_ID/issues" \
  -H "Authorization: Bearer $ODYSSEUS_API_KEY"
```
...
- Template index and how to apply a template: `skills/odysseus-create-agent/references/agent-instruction-templates.md`
- Individual role templates: `skills/odysseus-create-agent/references/agents/`
- Generic baseline role guide (no-template fallback): `skills/odysseus-create-agent/references/baseline-role-guide.md`
- Pre-submit draft-review checklist: `skills/odysseus-create-agent/references/draft-review-checklist.md`
- Endpoint payload shapes and full examples: `skills/odysseus-create-agent/references/api-reference.md`

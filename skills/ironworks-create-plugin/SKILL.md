---
name: ironworks-create-plugin
description: >
  Create new Ironworks plugins with the current alpha SDK/runtime. Use when
...
# Create a Ironworks Plugin

Use this skill when the task is to create, scaffold, or document a Ironworks plugin.
...
```bash
pnpm --filter @ironworksai/create-ironworks-plugin build
node packages/plugins/create-ironworks-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Ironworks repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.ironworks-sdk/`:

```bash
pnpm --filter @ironworksai/create-ironworks-plugin build
node packages/plugins/create-ironworks-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Ironworks from an absolute local path during development

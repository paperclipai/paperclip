...
| Task | Command |
|------|---------|
| Start server (first time or normal) | `npx valadrien-os run` |
...
| Back up database | `npx valadrien-os db:backup` |
| Health check | `npx valadrien-os doctor --repair` |
| Print env vars | `npx valadrien-os env` |
| Trigger agent heartbeat | `npx valadrien-os heartbeat run --agent-id <id>` |
| Install agent skills locally | `npx valadrien-os agent local-cli <agent> --company-id <id>` |
...
npx valadrien-os worktree:make my-feature --start-point origin/main
...
npx valadrien-os worktree:merge-history --from valadrien-os-my-feature --to current --apply

# 6. Clean up
npx valadrien-os worktree:cleanup my-feature
```

## Forks — Prefer Pushing to a User Fork

If the user has a personal fork of `ValDola-stack/valadrien-os` configured as a git remote, push your feature branches to **that fork** instead of creating branches on the main repo. This keeps the upstream branch list clean and matches the standard open-source contribution flow.

### Detect a fork remote

Before pushing or creating a PR, list remotes and check for one that points at a non-`valadrien-os` GitHub fork:

```bash
git remote -v
```

Treat any remote whose URL points to `github.com:<user>/valadrien-os` (or `github.com/<user>/valadrien-os.git`) as the user's fork. Common names are `fork`, `<username>`, or `myfork`. The remote named `origin` or `upstream` that points at `ValDola-stack/valadrien-os` is the canonical upstream — do not push feature branches there if a fork exists.
...
```bash
gh pr create --repo ValDola-stack/valadrien-os --head <fork-owner>:<branch-name> ...
```
...
If `git remote -v` shows only `ValDola-stack/valadrien-os` remotes (no user fork), fall back to pushing branches to `origin` as before. Do NOT create a fork on the user's behalf — ask first.

### Keeping the fork up to date

The canonical remote that points at `ValDola-stack/valadrien-os` may be named `origin` **or** `upstream` depending on how the user set up the repo. Detect it the same way as in the "Detect a fork remote" step, then fetch and push from/with that remote so the sync works under either convention:

```bash
UPSTREAM_REMOTE=$(git remote -v | awk '/valadrien-os\/valadrien-os.*\(fetch\)/{print $1; exit}')
...
1. **CLI is the only interface to worktrees and databases.** All worktree and database operations MUST go through `npx valadrien-os` / `pnpm valadrien-os` commands. You MUST NOT:
   ...
   - Run `rm -rf` on any `.valadrien-os/`, `.valadrien-os-worktrees/`, or `db/` directory
   - Directly manipulate embedded postgres data directories
   - Kill postgres processes by PID

2. **If a CLI command fails, stop and report.** Do NOT attempt workarounds. If `worktree:make`, `worktree reseed`, `worktree init`, `worktree:cleanup`, or any other `valadrien-os` command fails:
   - Report the exact error message in your task comment
   - Set the task to `blocked`
   - Suggest running `npx valadrien-os doctor --repair` or recreating the worktree from scratch
...
4. **Starting a dev server in a worktree requires setup first.** The correct sequence is:
   ...
   eval "$(npx valadrien-os worktree env)"
   pnpm install && pnpm build
   npx valadrien-os run          # or pnpm dev

   # If the worktree needs a fresh database:
   npx valadrien-os worktree reseed --seed-mode full

   # If the worktree is broken beyond repair:
   npx valadrien-os worktree:cleanup <name>
   npx valadrien-os worktree:make <name> --seed-mode full
   ```
...
cd <worktree-path>
eval "$(npx valadrien-os worktree env)"   # skip if using the primary instance
...
| Mistake | Fix |
|---------|-----|
| Server won't start | Run `npx valadrien-os doctor --repair` to diagnose and auto-fix |
| Forgetting to source worktree env | Run `eval "$(npx valadrien-os worktree env)"` after cd-ing into the worktree |
...
| Running agents against wrong instance | Verify `VALADRIEN_OS_API_URL` points to the correct port |
...
| Pushed feature branch to `ValDola-stack/valadrien-os` when a fork exists | Push to the user's fork remote instead — see "Forks" above |

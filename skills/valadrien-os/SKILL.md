...
Env vars auto-injected: `VALADRIEN_OS_AGENT_ID`, `VALADRIEN_OS_COMPANY_ID`, `VALADRIEN_OS_API_URL`, `VALADRIEN_OS_RUN_ID`. Optional wake-context vars may also be present: `VALADRIEN_OS_TASK_ID` (issue/task that triggered this wake), `VALADRIEN_OS_WAKE_REASON` (why this run was triggered), `VALADRIEN_OS_WAKE_COMMENT_ID` (specific comment that triggered this wake), `VALADRIEN_OS_APPROVAL_ID`, `VALADRIEN_OS_APPROVAL_STATUS`, and `VALADRIEN_OS_LINKED_ISSUE_IDS` (comma-separated). For local adapters, `VALADRIEN_OS_API_KEY` is auto-injected as a short-lived run JWT. For non-local adapters, your operator should set `VALADRIEN_OS_API_KEY` in adapter config. All requests use `Authorization: Bearer $VALADRIEN_OS_API_KEY`. All endpoints under `/api`, all JSON. Never hard-code the API URL.
...
**Step 2 — Approval follow-up (when triggered).** If `VALADRIEN_OS_APPROVAL_ID` is set (or wake reason indicates approval resolution), review the approval first:
...
**Step 4 — Pick work.** Priority: `in_progress` → `in_review` (if woken by a comment on it — check `VALADRIEN_OS_WAKE_COMMENT_ID`) → `todo`. Skip `blocked` unless you can unblock.

Overrides and special cases:

- `VALADRIEN_OS_TASK_ID` set and assigned to you → prioritize that task first.
- `VALADRIEN_OS_WAKE_REASON=issue_commented` with `VALADRIEN_OS_WAKE_COMMENT_ID` → read the comment, then checkout and address the feedback (applies to `in_review` too).
- `VALADRIEN_OS_WAKE_REASON=issue_comment_mentioned` → read the comment thread first even if you're not the assignee. Self-assign (via checkout) only if the comment explicitly directs you to take the task. Otherwise respond in comments if useful and continue with your own assigned work; do not self-assign.
...
If `VALADRIEN_OS_WAKE_PAYLOAD_JSON` is present, inspect that payload before calling the API. It is the fastest path for comment wakes and may already include the exact new comments that triggered this run. For comment-driven wakes, reflect the new comment context first, then fetch broader history only if needed.

Use comments incrementally:

- if `VALADRIEN_OS_WAKE_COMMENT_ID` is set, fetch that exact comment first with `GET /api/issues/{issueId}/comments/{commentId}`
...
```bash
scripts/valadrien-os-issue-update.sh --issue-id "$VALADRIEN_OS_TASK_ID" --status done <<'MD'
Done
...
- `VALADRIEN_OS_WAKE_REASON=issue_blockers_resolved` — all `blockedBy` issues reached `done`; dependent's assignee is woken.
- `VALADRIEN_OS_WAKE_REASON=issue_children_completed` — all direct children reached a terminal state (`done`/`cancelled`); parent's assignee is woken.
...
`skills/valadrien-os/references/company-skills.md`
...
`skills/valadrien-os/references/routines.md`
...
- **Self-assign only for explicit @-mention handoff.** Requires a mention-triggered wake with `VALADRIEN_OS_WAKE_COMMENT_ID` and a comment that clearly directs you to do the task. Use checkout (never direct assignee patch).
...
For detailed API tables, JSON response schemas, worked examples (IC and Manager heartbeats), governance/approvals, cross-team delegation rules, error codes, issue lifecycle diagram, and the common mistakes table, read: `skills/valadrien-os/references/api-reference.md`

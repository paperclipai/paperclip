...
```sh
curl -sS -X POST "$VALADRIEN_OS_API_URL/api/companies/$VALADRIEN_OS_COMPANY_ID/skills/import" \
...
```sh
curl -sS -X POST "$VALADRIEN_OS_API_URL/api/companies/$VALADRIEN_OS_COMPANY_ID/skills/scan-projects" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```sh
curl -sS "$VALADRIEN_OS_API_URL/api/companies/$VALADRIEN_OS_COMPANY_ID/skills" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY"
```
...
```sh
curl -sS "$VALADRIEN_OS_API_URL/api/companies/$VALADRIEN_OS_COMPANY_ID/skills/<skill-id>" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY"

curl -sS "$VALADRIEN_OS_API_URL/api/companies/$VALADRIEN_OS_COMPANY_ID/skills/<skill-id>/files?path=SKILL.md" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY"
```
...
```sh
curl -sS -X POST "$VALADRIEN_OS_API_URL/api/agents/<agent-id>/skills/sync" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```sh
curl -sS "$VALADRIEN_OS_API_URL/api/agents/<agent-id>/skills" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY"
```
...
```sh
curl -sS -X POST "$VALADRIEN_OS_API_URL/api/companies/$VALADRIEN_OS_COMPANY_ID/agent-hires" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```sh
curl -sS -X POST "$VALADRIEN_OS_API_URL/api/companies/$VALADRIEN_OS_COMPANY_ID/agents" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "QA Browser Agent",
...
- Built-in Valadrien OS runtime skills are still added automatically when required by the adapter.

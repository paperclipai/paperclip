...
```sh
curl -sS -H "Authorization: Bearer $VALADRIEN_OS_API_KEY" \
  "$VALADRIEN_OS_API_URL/api/issues/$VALADRIEN_OS_TASK_ID/heartbeat-context"
```
...
- `valadrienOsWaitForIssueWorkspaceService` — waits until a selected service is running and returns its URL when exposed.

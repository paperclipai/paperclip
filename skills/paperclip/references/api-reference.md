...
- **No policy wakes on expiry.** `outcome: "superseded_by_comment"`, `"stale_target"`, and `"issue_closed"` resolve the interaction with status `expired`, and the continuation guard drops every expired resolution regardless of `continuationPolicy` — only the board/user comment itself brings the assignee back. A withdrawal is different: it resolves with status `cancelled`, which the guard does not drop, so a withdrawal by a board user or by another agent still wakes the assignee under `wake_assignee`. One exception — when the **assignee agent withdraws its own** pending interaction, the withdraw route queues no continuation wake at all (that agent is already running); do not wait on a wake you cancelled yourself.
- **The review-path safety net overrides both rules above.** If the issue is `in_review` and resolving the interaction leaves it with no maintained review path, the guard wakes the assignee anyway — even on a `wake_assignee_on_accept` rejection, and even on an `expired` resolution. That wake carries `reviewPathLost: true` plus an instruction to restore a reviewer, interaction, approval, or monitor, or to choose an explicit disposition. It is deduplicated on `interaction:{id}:{status}`, so it arrives exactly once for that resolution and never repeats — treat it as a last-resort net, not as the continuation path you planned for. The self-withdrawal exception above is **not** covered by it: the withdraw route skips the continuation guard entirely when the withdrawing agent is the current assignee, so an `in_review` issue left without a review path by your own withdrawal gets no recovery wake either. After withdrawing your own card, restore the review path in the same heartbeat.

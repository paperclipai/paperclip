...
- Company skill mutations: board, a human/agent principal with an explicit `skill:create` grant, or an agent whose `canCreateSkills` permission is enabled. `canCreateSkills` defaults on for agents unless explicitly disabled.

...
If `PAPERCLIP_FOCUSED_TASK_MODE=true`: work ONLY on `PAPERCLIP_TASK_ID` — skip fetching other assignments entirely. After updating or completing that task, exit immediately. Do not continue to other `todo` or `in_progress` items.
If `PAPERCLIP_TASK_ID` is set but `PAPERCLIP_FOCUSED_TASK_MODE` is not set (agent opted out via config, or this is a comment-mention wake), prioritize `PAPERCLIP_TASK_ID` first, then continue with other assignments as usual. Note: `PAPERCLIP_FOCUSED_TASK_MODE` is never set for comment-mention wakes (`PAPERCLIP_WAKE_REASON=issue_comment_mentioned`) even when `PAPERCLIP_TASK_ID` is present — mention wakes may require responding to comments, potentially self-assigning, and continuing with other work.

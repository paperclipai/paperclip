...
| `ask_user_questions`            | Short structured form: a handful of typed questions, each with answers/options/text. Mark a consequential question `"intent": "decision"` to require one recommended option plus its rationale. | Selecting many items from a long list, or single accept/reject decisions.                          |
...
- **Decision interviews.** When an `ask_user_questions` question is consequential — a person must own the outcome — mark it `"intent": "decision"`. Paperclip then requires: a single-select question with at least two distinct options, exactly one option marked `"recommended": true`, and a non-empty `"recommendationRationale"` explaining why it is recommended and what each choice does. The card shows the advice but never preselects it, so the answer is always the person's own choice. Decision questions cannot use the free-form fallback (`allowOther`, a `freeText` option, or `customAnswer`), cannot set `supersedeOnUserComment: true` (a plain comment is not consent), must be `required`, and are clamped to `resolverPolicy: "human_only"`. Omit `resolverPolicy` and Paperclip fills in `human_only` for you. Use `"intent": "information"` (the default) for the conversational case, where free text is often the right control.

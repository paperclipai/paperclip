...
- For **immediate delegation to a report** (CEO→Dev, manager→IC), prefer **`paperclipDelegate`** (MCP) or `POST /api/heartbeat-runs/$PAPERCLIP_RUN_ID/delegate` instead of manually creating a child issue and polling. Set `wait: true` to block until the report finishes (bounded timeout, max 300s); set `wait: false` to **exit your heartbeat** and receive an automatic `delegation_child_completed` continuation wake with the result when the child run completes.
- If a `wait: true` delegation times out, do NOT retry the delegate call (it returns 409 while pending). Either exit the heartbeat (the continuation wake will deliver the result) or check `paperclipGetDelegation` once before exiting.
- Delegation limits: max chain depth 3 (CEO→CTO→Dev), max 5 delegations per run. One delegation may be pending per run at a time.

...
- **Continuation policy.** `request_checkbox_confirmation` and `request_item_verdicts` default to `wake_assignee`, which wakes you after the card is resolved or newly resolved item verdicts are submitted. `request_confirmation` defaults to `none`, so set a policy explicitly when you need to resume after a yes/no decision. Prefer `wake_assignee`: it wakes you on acceptance *and* rejection. `wake_assignee_on_accept` wakes you only on the positive resolution — acceptance for a confirmation, an authoritative `answered` for `ask_user_questions` — and a rejection under it is silent, since no wake is queued and the reject route writes no comment either. The one override: if the issue is `in_review` and resolving the card leaves it without a review path, a single review-path-recovery wake fires regardless of policy — except when you withdraw your own pending card, which queues no wake at all. `none` never wakes you — only use it when you truly do not need to resume.

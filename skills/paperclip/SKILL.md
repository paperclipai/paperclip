...
Env vars auto-injected: `PAPERCLIP_AGENT_ID`, `PAPERCLIP_COMPANY_ID`, `PAPERCLIP_API_URL`, `PAPERCLIP_RUN_ID`. Optional wake-context vars may also be present: `PAPERCLIP_TASK_ID` (issue/task that triggered this wake), `PAPERCLIP_WAKE_REASON` (why this run was triggered), `PAPERCLIP_WAKE_COMMENT_ID` (specific comment that triggered this wake), `PAPERCLIP_LATEST_COMMENT_ID` (the issue's current latest comment id at wake time — used for `If-Match` on comment writes; see "Comment freshness guard" below), `PAPERCLIP_APPROVAL_ID`, `PAPERCLIP_APPROVAL_STATUS`, and `PAPERCLIP_LINKED_ISSUE_IDS` (comma-separated). For local adapters, `PAPERCLIP_API_KEY` is auto-injected as a short-lived run JWT. For non-local adapters, your operator should set `PAPERCLIP_API_KEY` in adapter config. All requests use `Authorization: Bearer $PAPERCLIP_API_KEY`. All endpoints under `/api`, all JSON. Never hard-code the API URL.
...
**Comment freshness guard (`If-Match` + `Idempotency-Key`).** Every comment-writing endpoint enforces a server-side freshness check so an agent can't accidentally post over a comment it never saw. The plumbing is automatic — these are the rules:

- **Attach `If-Match: $PAPERCLIP_LATEST_COMMENT_ID`** on every `POST /api/issues/:id/comments`, `PATCH /api/issues/:id` that carries a `comment`, and `POST /api/issues/:id/interactions` (plus its `accept` / `reject` / `respond` sub-routes). Helper scripts under `scripts/` (e.g. `scripts/paperclip-issue-update.sh`) do this for you. If you're hand-crafting a `curl`, copy the value of `PAPERCLIP_LATEST_COMMENT_ID` straight into the `If-Match` header. On the first comment of an empty thread, `PAPERCLIP_LATEST_COMMENT_ID` is unset — omit the header.
- **Attach `Idempotency-Key: <unique-id>` on the same writes.** Reuse a deterministic key per logical write (e.g. `comment:{runId}:{intentSlug}`) so a retry returns the original response instead of double-posting. The server caches `(scope, actor, key)` → response for 5 minutes.
- **On `409 Conflict` with `error: "stale_comment_cursor"`:** the server attaches a `since[]` array of the comments you missed and `expected: <server-current>`. Read the missed comments, reconcile your draft, then retry **once** with the new `If-Match` (the server's `expected` becomes your next `If-Match`). On a **second** 409 stop retrying, post a surface comment via the system bypass header (`X-Paperclip-System-Comment: 1`, only honored when the harness/server itself is the caller — agents will get the header ignored), and exit the heartbeat.
- **Never invent a cursor.** If `PAPERCLIP_LATEST_COMMENT_ID` is missing and you're writing on a non-empty thread, fetch `GET /api/issues/{issueId}/comments?order=desc&limit=1` first, then use that comment id as your `If-Match`.

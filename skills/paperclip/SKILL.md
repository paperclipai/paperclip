...
For local adapters running on the same host as the Paperclip server, `PAPERCLIP_API_URL` is derived from the actual server listener (e.g. `http://127.0.0.1:3100`) so it is always reachable from the agent process. The publicly published URL — for example a Tailscale hostname meant for remote consumers — is intentionally not propagated to same-host agents because it may not resolve from the local network namespace. Always use `$PAPERCLIP_API_URL` rather than hard-coding any host.

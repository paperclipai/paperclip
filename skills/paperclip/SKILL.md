...
## Zenova Release Promotion Gate (Binding)

For the HenkDz Paperclip production fork, release work follows one fixed
sequence: record the immutable candidate SHA, deploy that exact SHA to
`paper-dev.zenova.id`, verify health and required smoke checks there, then
stop for explicit written sign-off from Henk. Include the candidate SHA and
staging evidence in the request for sign-off.

Never auto-deploy `paper.zenova.id`. Do not treat a successful staging check,
an approval inferred from context, or an old sign-off as production authority.
Only an explicit written Henk sign-off for the validated candidate permits a
separate deliberate human promotion.

## External Execution Environment Boundary (Binding)

Paperclip is the control plane, never the project execution environment. Domain
builds, tests, application servers, browser QA, databases, migrations, previews,
and deployments require a board-provided external execution environment with an
identified provider, target, executor, and rollback/cleanup owner. Paperclip
workspaces and runtime-service metadata may describe that target, but they do
not authorize a local fallback.

Missing environment means a precise blocker, never a local fallback: record the
exact external environment and owner needed, then wait for it through the normal
issue path. This requirement overrides the later "never ask a human to do what an agent could do" rule; asking for the required board-provided environment is not avoidable delegation. Read `references/issue-workspaces.md` before using workspace or runtime-service information.
...
- When an issue declares completion evidence, declared completion evidence must also be registered as qualifying issue work products. A comment, attachment, or issue document alone is supporting material, not completion evidence; register the corresponding `artifact` or `document` work product before review or `done`.
...
**Explicit SOP corrections are execution directives.** When a board or
authorized manager explicitly directs an SOP or skill correction, implement it
through the canonical company-skill mutation path, not as an improvement
suggestion or a comment-only acknowledgement. Read
`references/governance.md`, make the smallest durable file change, validate it,
and report `SOP updated` only after file read-back confirms the changed canonical
source and the affected-agent propagation inventory is known.

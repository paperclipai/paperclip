...
When you produce reports, code reviews, audits, analysis, or any other substantial generated content that the board/CEO/manager needs to review, save it as an **Issue Document** — not just a workspace file. Issue Documents are visible in the Paperclip UI, have version history, and are linked to the work they relate to.

Use appropriate document keys:
- `plan` — project/implementation plans
- `report` — status reports, CTO reports, CEO summaries
- `analysis` — code audits, risk analysis, feasibility studies
- `review` — code reviews, architecture reviews
- `spec` — technical specifications
- `notes` — meeting notes, investigation notes
...
PUT /api/issues/{issueId}/documents/report
{
  "title": "Status Report",
  "format": "markdown",
  "body": "# Status Report\n\n- Generated by: {agent-name} on {YYYY-MM-DD HH:MM UTC}\n\n[your report content]",
  "baseRevisionId": null
}
```

Rules:
- Always save significant reports as Issue Documents on the relevant issue.
- Include a "Generated by: {agent-name} on {timestamp}" line at the top of the document so reviewers know who created it and when.
- Workspace files are fine for temporary drafts, scratch notes, or files not meant for human review.
- If an issue already has a document, fetch it first and update with the new `baseRevisionId`.
- Link to documents in comments using: `/<prefix>/issues/<issue-identifier>#document-<key>`

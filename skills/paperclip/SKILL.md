...
- **Continuation policy.** `request_checkbox_confirmation` and `request_item_verdicts` default to `wake_assignee`, which wakes you after the card is resolved or newly resolved item verdicts are submitted. `request_confirmation` defaults to `none`, so set a policy explicitly when you need to resume after a yes/no decision. Prefer `wake_assignee`: it wakes you on acceptance *and* rejection. `wake_assignee_on_accept` wakes you only on acceptance — a rejection under it is silent, since no wake is queued and the reject route writes no comment either. `none` never wakes you — only use it when you truly do not need to resume.

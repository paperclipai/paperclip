...
- **A rejected `PATCH` is atomic — a bad `executionPolicy` (or any other field) destroys the `comment` in the same body, not just the field that failed.** If a `PATCH /api/issues/{id}` request that carried a `comment` returns a non-2xx status (e.g. `400 invalid_enum_value` from a wrong `monitor.kind`), the comment was **not** persisted — nothing partially applied. Re-post it immediately with `POST /api/issues/{id}/comments` (`{ "body": "..." }`) before treating the write as failed or moving on. Do not assume a `400` here is a permission gate (that is a `403`, see the checkout/write-gate guidance above) — read `details[].options` for the accepted values and fix the field, then re-send.

...
**Tailscale/MagicDNS fallback (required).** Before treating the control plane as unavailable, test the configured host with one read-only API request. If name resolution fails but `tailscale status` lists the hostname (or its short name) with a `100.x.y.z` address, retry the identical request with curl's `--resolve`, deriving both the hostname from `$PAPERCLIP_API_URL` and the address from the live Tailscale output. For example: `curl --resolve "${host}:443:${tailscale_ip}" ... "$PAPERCLIP_API_URL/api/..."`. Do not substitute a stale or hard-coded IP, disable TLS verification, or omit the configured hostname. If the resolved fallback succeeds, use it for every remaining control-plane request in the heartbeat and record the DNS recovery in the issue comment. Only mark a task blocked for control-plane connectivity after this fallback also fails.

...
If `PAPERCLIP_TASK_ID` is set but `PAPERCLIP_FOCUSED_TASK_MODE` is not set (agent opted out via config), prioritize `PAPERCLIP_TASK_ID` first, then continue with other assignments as usual.

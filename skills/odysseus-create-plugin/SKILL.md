...
description: >
  ...
  Paperclip instance, or updating plugin authoring docs. Covers `odysseus
  plugin init`, the local install loop via `odysseus plugin install <path>`,
...
- Scaffold the plugin into a directory outside the Paperclip checkout (e.g. `~/dev/odysseus-plugins/<name>`).
...
Use `odysseus plugin init`. Do not invoke the scaffold package node entrypoint by hand unless the CLI command is unavailable in the environment.

```bash
odysseus plugin init @acme/my-plugin --output ~/dev/odysseus-plugins
```
...
- `--sdk-path <path>` — snapshot the local SDK from a Paperclip checkout into `.odysseus-sdk/` (useful when developing against an unreleased SDK).

On success the command prints the exact next commands (`cd`, `pnpm install`, `pnpm dev`, `odysseus plugin install <abs-path>`). Run them in order.

If `odysseus` is not on PATH in your environment, fall back to:

```bash
pnpm --filter @odysseus/create-odysseus-plugin build
node packages/plugins/create-odysseus-plugin/dist/index.js @acme/my-plugin \
  --output /absolute/path \
  --sdk-path /absolute/path/to/odysseus/packages/plugins/sdk
```
...
odysseus plugin install /absolute/path/to/my-plugin
```

Notes:

- `odysseus plugin install` auto-detects local paths (absolute, `./`, `../`, `~`, or an existing relative folder) and forwards `isLocalPath: true` to the server. Pass `--local` to force local mode if the heuristic is ambiguous.
...
```bash
odysseus plugin list
odysseus plugin inspect <plugin-key>
```
...
- `package.json` — `odysseusPlugin` block points at `dist/manifest.js`, `dist/worker.js`, `dist/ui/`
...
- **Commands run** — the exact `odysseus plugin init`, `pnpm install`, `pnpm dev`, `odysseus plugin install <path>` invocations (and any verification commands).
- **Install status** — output of `odysseus plugin list` / `plugin inspect` (plugin key, version, status). Note if `status` is anything other than `ready` and include `lastError`.

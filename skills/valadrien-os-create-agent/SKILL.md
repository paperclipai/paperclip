...
```sh
curl -sS "$VALADRIEN_OS_API_URL/api/agents/me" \
...
`skills/valadrien-os-create-agent/references/agent-instruction-templates.md`

Generic fallback for no-template hires:
`skills/valadrien-os-create-agent/references/baseline-role-guide.md`
...
`skills/valadrien-os-create-agent/references/draft-review-checklist.md`
...
- when the board approves, you will be woken with `VALADRIEN_OS_APPROVAL_ID`; read linked issues and close/comment follow-up

```sh
curl -sS "$VALADRIEN_OS_API_URL/api/approvals/<approval-id>" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY"

curl -sS -X POST "$VALADRIEN_OS_API_URL/api/approvals/<approval-id>/comments" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS -X POST "$VALADRIEN_OS_API_URL/api/issues/<issue-id>/approvals" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS "$VALADRIEN_OS_API_URL/api/approvals/$VALADRIEN_OS_APPROVAL_ID" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY"

curl -sS "$VALADRIEN_OS_API_URL/api/approvals/$VALADRIEN_OS_APPROVAL_ID/issues" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY"
```
...
- Template index and how to apply a template: `skills/valadrien-os-create-agent/references/agent-instruction-templates.md`
- Individual role templates: `skills/valadrien-os-create-agent/references/agents/`
- Generic baseline role guide (no-template fallback): `skills/valadrien-os-create-agent/references/baseline-role-guide.md`
- Pre-submit draft-review checklist: `skills/valadrien-os-create-agent/references/draft-review-checklist.md`
- Endpoint payload shapes and full examples: `skills/valadrien-os-create-agent/references/api-reference.md`

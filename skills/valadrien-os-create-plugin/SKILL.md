...
Use `valadrien-os plugin init`. Do not invoke the scaffold package node entrypoint by hand unless the CLI command is unavailable in the environment.

```bash
valadrien-os plugin init @acme/my-plugin --output ~/dev/valadrien-os-plugins
```
...
If `valadrien-os` is not on PATH in your environment, fall back to:

```bash
pnpm --filter @valadrien-os/create-valadrien-os-plugin build
...
valadrien-os plugin install /absolute/path/to/my-plugin
```

Notes:

- `valadrien-os plugin install` auto-detects local paths (absolute, `./`, `../`, `~`, or an existing relative folder) and forwards `isLocalPath: true` to the server. Pass `--local` to force local mode if the heuristic is ambiguous.
...
```bash
valadrien-os plugin list
valadrien-os plugin inspect <plugin-key>
```
...
- `package.json` — `valadrienOsPlugin` block points at `dist/manifest.js`, `dist/worker.js`, `dist/ui/`
...
- **Commands run** — the exact `valadrien-os plugin init`, `pnpm install`, `pnpm dev`, `valadrien-os plugin install <path>` invocations (and any verification commands).
- **Install status** — output of `valadrien-os plugin list` / `plugin inspect` (plugin key, version, status). Note if `status` is anything other than `ready` and include `lastError`.

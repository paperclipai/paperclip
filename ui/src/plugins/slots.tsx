 ...
 *
 * Plugin bundles are built with `external: ["@valadrien-os/plugin-sdk/ui", "react", "react-dom"]`,
 * so their ESM output contains bare specifier imports like:
 *
 * ```js
 * import { usePluginData } from "@valadrien-os/plugin-sdk/ui";
 ...
 *    host's global bridge registry (`globalThis.__valadrienOsPluginBridge__`)
...
function createReactShimSource(reactModule: object): string {
  ...
  return `
        const R = globalThis.__valadrienOsPluginBridge__?.react;
        if (!R) {
          throw new Error("ValadrienOs plugin React runtime is not initialized.");
        }
        export default R;
${namedExports}
      `;
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__valadrienOsPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__valadrienOsPluginBridge__?.sdkUi ?? {};
        function missing(name) {
          return function MissingValadrienOsSdkUiComponent() {
            throw new Error('ValadrienOs plugin UI runtime is not initialized for "' + name + '". Ensure the host loaded the plugin bridge before rendering this UI module.');
          };
        }
 ...
 * - `import { ... } from "@valadrien-os/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@valadrien-os/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@valadrien-os/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@valadrien-os/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@valadrien-os/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__valadrienOsPluginBridge__) {
 ...
 * self-register via `window.valadrienOsPlugins.registerReactComponent()`.
 ...
 * of `@valadrien-os/plugin-sdk/ui`, `react`, and `react-dom` resolve to the

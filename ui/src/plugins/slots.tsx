 ...
 *
 * Plugin bundles are built with `external: ["@aiteamcorp/plugin-sdk/ui", "react", "react-dom"]`,
 * so their ESM output contains bare specifier imports like:
 *
 * ```js
 * import { usePluginData } from "@aiteamcorp/plugin-sdk/ui";
 ...
 *    host's global bridge registry (`globalThis.__aiteamcorpPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__aiteamcorpPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__aiteamcorpPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__aiteamcorpPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@aiteamcorp/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@aiteamcorp/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@aiteamcorp/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@aiteamcorp/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@aiteamcorp/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__aiteamcorpPluginBridge__) {
 ...
 * self-register via `window.aiteamcorpPlugins.registerReactComponent()`.
 ...
 * of `@aiteamcorp/plugin-sdk/ui`, `react`, and `react-dom` resolve to the

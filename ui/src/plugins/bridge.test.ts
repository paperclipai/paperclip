...
afterEach(() => {
  delete globalThis.__valadrienOsPluginBridge__;
});
...
describe("plugin SDK FileTree bridge", () => {
  ...
  it("throws a clear error when the host FileTree implementation is missing", () => {
    globalThis.__valadrienOsPluginBridge__ = {
      react: React,
      reactDom: ReactDOM,
      sdkUi: {},
    };
    ...
    ).toThrow('ValadrienOs plugin UI runtime is not initialized for "FileTree"');
  });
});
...
describe("plugin SDK markdown component bridge", () => {
  it("injects markdown display and editor components through the bridge runtime", () => {
    ...
    const registry = globalThis.__valadrienOsPluginBridge__?.sdkUi ?? {};

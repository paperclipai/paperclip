...
function normalizeError(error: unknown): FileViewerErrorShape {
  ...
  return {
    status: 0,
    code: "",
    message: t("components.fileViewerSheet.errorSomethingWentWrong", {
...
export function describeDenial(code: string, fallback: string): { title: string; body: string; icon: ReactNode } {
  ...
  if (lower.includes("policy") || lower.includes("denied") || lower.includes("sensitive")) {
    return {
      ...
      title: t("components.fileViewerSheet.denialPolicyTitle", {
        defaultValue: "Viewer blocked for this file",
      }),
      body: t("components.fileViewerSheet.denialPolicyBody", {
        defaultValue: "This file is not available through the viewer because it may contain sensitive data.",
      }),
    };
  }
  if (lower.includes("outside") || lower.includes("traversal")) {
    return {
      ...
      title: t("components.fileViewerSheet.denialOutsideTitle", {
        defaultValue: "Path is outside the workspace",
      }),
      body: t("components.fileViewerSheet.denialOutsideBody", {
        defaultValue: "The viewer can only open files that live under the issue's workspace.",
      }),
    };
  }
  if (lower.includes("archive") || lower.includes("cleaned")) {
    return {
      ...
      title: t("components.fileViewerSheet.denialArchiveTitle", {
        defaultValue: "Workspace is no longer available",
      }),
      body: t("components.fileViewerSheet.denialArchiveBody", {
        defaultValue: "The isolated worktree for this issue has been cleaned up, so files cannot be previewed.",
      }),
    };
  }
  if (lower.includes("remote")) {
    return {
      ...
      title: t("components.fileViewerSheet.denialRemoteTitle", {
        defaultValue: "Remote workspace preview not supported",
      }),
      body: t("components.fileViewerSheet.denialRemoteBody", {
        defaultValue: "This workspace is hosted remotely and is not available for inline preview yet.",
      }),
    };
  }
  if (lower.includes("too_large") || lower.includes("size")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: t("components.fileViewerSheet.denialTooLargeTitle", {
        defaultValue: "File is too large to preview",
      }),
      body: t("components.fileViewerSheet.denialTooLargeBody", {
        defaultValue: "This file exceeds the supported preview size.",
      }),
    };
  }
  if (lower.includes("binary") || lower.includes("unsupported")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: t("components.fileViewerSheet.denialUnsupportedTitle", {
        defaultValue: "Preview not supported for this file type",
      }),
      body: t("components.fileViewerSheet.denialUnsupportedBody", {
        defaultValue: "This file does not have a text, image, or video preview available.",
      }),
    };
  }
  return {
    icon: <Ban aria-hidden="true" className="h-6 w-6 text-red-500" />,
    title: t("components.fileViewerSheet.denialDefaultTitle", {
      defaultValue: "Can't preview this file",
    }),
    body:
      fallback ||
      t("components.fileViewerSheet.denialDefaultBody", {
        defaultValue: "The viewer was unable to load this file.",
      }),
  };
}
  ...
  return (
    <div className="flex min-h-[18px] flex-wrap items-center gap-x-2 gap-y-0.5 text-xs text-muted-foreground">
      {resolvedResource ? (
        <>
          ...
          {state?.line ? (
            <>
              <span aria-hidden="true" className="opacity-50">·</span>
              <span>
                {t("components.fileViewerSheet.lineLabel", {
                  line: state.line,
                  defaultValue: "Line {{line}}",
                })}
                {state.column
                  ? t("components.fileViewerSheet.columnLabel", {
                      column: state.column,
                      defaultValue: ", Col {{column}}",
                    })
                  : ""}
              </span>
            </>
        ...
        <span
          className="h-3 w-28 rounded bg-muted animate-pulse"
          aria-label={t("components.fileViewerSheet.loadingFileDetails", {
            defaultValue: "Loading file details",
          })}
        />
      ) : null}
    </div>
...
export function FileContentViewer({ content, highlightedLine, onLoaded }: FileContentViewerProps) {
  ...
  useEffect(() => {
    if (!lines) return;
    onLoaded?.(
      t("components.fileViewerSheet.fileLoadedLines", {
        count: lines.length,
        defaultValue: "File loaded, {{count}} line.",
        defaultValue_other: "File loaded, {{count}} lines.",
      }),
    );
  }, [lines, onLoaded, t]);
  ...
  if (resource.previewKind === "image") {
    ...
    if (!dataUrl) {
      return (
        <FileViewerStateView
          icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
          title={t("components.fileViewerSheet.imagePreviewUnavailable", {
  ...
  if (resource.previewKind === "video") {
    ...
    if (!dataUrl) {
      return (
        <FileViewerStateView
          icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
          title={t("components.fileViewerSheet.videoPreviewUnavailable", {
            defaultValue: "Video preview unavailable",
          })}
        />
      );
    }
    return (
      <div className="flex flex-1 items-center justify-center overflow-auto bg-black p-4">
        <video
          ...
          preload="metadata"
          playsInline
          aria-label={t("components.fileViewerSheet.videoPreviewLabel", {
            title: resource.title,
            defaultValue: "Video preview: {{title}}",
          })}
  ...
  if (resource.previewKind === "unsupported" || !lines) {
    return (
      <FileViewerStateView
        icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
        title={t("components.fileViewerSheet.unsupportedPreviewTitle", {
          defaultValue: "Preview not supported for this file type",
        })}
        body={
          resource.contentType
            ? t("components.fileViewerSheet.contentTypeLabel", {
                contentType: resource.contentType,
                defaultValue: "Content type: {{contentType}}",
              })
            : undefined
        }
      />
  ...
  const rawSourceView = (
    <div
      ref={codeScrollRef}
      role="region"
      aria-label={t("components.fileViewerSheet.sourceRegionLabel", {
        title: resource.title,
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col">
      <div className="absolute right-3 top-3 z-20">
        <div
          role="group"
          aria-label={t("components.fileViewerSheet.markdownPreviewMode", {
            defaultValue: "Markdown preview mode",
          })}
        ...
        >
          <Button
            ...
            size="icon-sm"
            aria-label={t("components.fileViewerSheet.showRenderedMarkdown", {
              defaultValue: "Show rendered Markdown",
            })}
            title={t("components.fileViewerSheet.renderedMarkdown", {
          ...
          <Button
            ...
            size="icon-sm"
            aria-label={t("components.fileViewerSheet.showRawMarkdown", {
              defaultValue: "Show raw Markdown",
            })}
            title={t("components.fileViewerSheet.rawMarkdown", {
              defaultValue: "Raw Markdown",
            })}
      ...
      {markdownMode === "raw" ? (
        ...
        <div
          role="region"
          aria-label={t("components.fileViewerSheet.renderedMarkdownRegionLabel", {
            title: resource.title,
            defaultValue: "{{title}} rendered Markdown",
          })}
          tabIndex={0}
    ...
    return (
      <div className="flex-1 space-y-2 p-6" aria-busy="true" aria-live="polite">
        <span className="sr-only">
          {t("components.fileViewerSheet.loadingFilePreview", {
            defaultValue: "Loading file preview",
          })}
        </span>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-muted-foreground">
        ...
        {t("components.fileViewerSheet.loadingFilePreviewEllipsis", {
          defaultValue: "Loading file preview...",
        })}
      </div>
    </div>
...
export function FileViewerSheet({
  ...
  useEffect(() => {
    if (resolveQuery.isError) {
      ...
      setAnnouncement(
        normalized.message ||
          t("components.fileViewerSheet.unableToLoadFile", {
            defaultValue: "Unable to load file.",
          }),
      );
    }
  ...
  useEffect(() => {
    if (contentQuery.isError) {
      ...
      setAnnouncement(
        normalized.message ||
          t("components.fileViewerSheet.unableToLoadFileContent", {
            defaultValue: "Unable to load file content.",
          }),
      );
    }
  ...
  const copyToClipboard = useCallback(async (value: string, field: "content" | "link", message: string) => {
    try {
      ...
      showCopyFeedback(
        null,
        t("components.fileViewerSheet.copyFailed", { defaultValue: "Copy failed" }),
      );
    } finally {
  ...
  const handleCopyContent = useCallback(() => {
    ...
    void (async () => {
      ...
      if (!content) {
        showCopyFeedback(
          null,
          t("components.fileViewerSheet.fileContentsUnavailable", {
            defaultValue: "File contents unavailable",
          }),
        );
        return;
      }
      const message =
        content.content.encoding === "base64"
          ? t("components.fileViewerSheet.copiedFileData", { defaultValue: "Copied file data" })
          : t("components.fileViewerSheet.copiedContents", { defaultValue: "Copied contents" });
  ...
  const handleCopyLink = useCallback(() => {
    ...
    void copyToClipboard(
      ...
      "link",
      t("components.fileViewerSheet.copiedLink", { defaultValue: "Copied link" }),
    );
  }, [copyToClipboard, t]);
  ...
  const title = state
    ? basename(state.path)
    : t("components.fileViewerSheet.browseWorkspace", { defaultValue: "Browse workspace" });
  const description = state
    ? middleTruncatePath(state.path)
    : t("components.fileViewerSheet.browseWorkspaceDescription", {
        defaultValue: "Search and preview files from this issue's workspace.",
      });
            ...
            <div className="flex shrink-0 items-center gap-1 self-start">
              ...
              {cameFromBrowse ? (
                <Button
                  ...
                  aria-label={t("components.fileViewerSheet.backToFiles", {
              ...
              {state ? (
                <Button
                  ...
                  aria-label={
                    copiedField === "content"
                      ? t("components.fileViewerSheet.copiedFileContents", {
                          defaultValue: "Copied file contents",
                        })
                      : t("components.fileViewerSheet.copyFileContents", {
                          defaultValue: "Copy file contents",
                        })
                  }
                  title={
                    copiedField === "content"
                      ? t("components.fileViewerSheet.copiedContents", {
                          defaultValue: "Copied contents",
                        })
                      : t("components.fileViewerSheet.copyFileContents", {
                          defaultValue: "Copy file contents",
                        })
              ...
              {state ? (
                <Button
                  ...
                  aria-label={
                    copiedField === "link"
                      ? t("components.fileViewerSheet.copiedFileViewLink", {
                          defaultValue: "Copied file view link",
                        })
                      : t("components.fileViewerSheet.copyLinkToFileView", {
                          defaultValue: "Copy link to this file view",
                        })
                  }
                  title={
                    copiedField === "link"
                      ? t("components.fileViewerSheet.copiedLink", { defaultValue: "Copied link" })
                      : t("components.fileViewerSheet.copyLink", { defaultValue: "Copy link" })
                  }
                  className="h-7 w-7"
                >
                  {copyingField === "link" ? (
              ...
              <Button
                ...
                className="h-7 w-7"
                aria-label={t("components.fileViewerSheet.closeFileViewer", {
          ...
          {state ? (
            <div className="flex min-h-0 flex-1 gap-3 bg-muted/30 p-3">
              ...
              <div
                ...
                aria-label={t("components.fileViewerSheet.resizeFileTree", {
                  defaultValue: "Resize file tree",
                })}
  ...
  if (resolveQuery.isError) {
    ...
    if (normalized.status === 404) {
      return (
        <FileViewerStateView
          ...
          title={t("components.fileViewerSheet.fileNotFoundTitle", {
            defaultValue: "File not found",
          })}
          body={t("components.fileViewerSheet.fileNotFoundBody", {
            defaultValue: "That file was not found in the active workspace.",
          })}
          actions={
            <>
              {onFallbackToProject ? (
                <Button type="button" variant="secondary" size="sm" onClick={onFallbackToProject}>
                  {t("components.fileViewerSheet.tryProjectWorkspace", {
                    defaultValue: "Try project workspace",
                  })}
                </Button>
              ) : null}
              <Button type="button" variant="ghost" size="sm" onClick={onRetry}>
                <RefreshCcw aria-hidden="true" className="mr-1 h-3 w-3" />{" "}
                {t("components.fileViewerSheet.retry", { defaultValue: "Retry" })}
              </Button>
            </>
          }
        />
    ...
    if (normalized.status === 422) {
      return (
        <FileViewerStateView
          icon={<FolderOpen aria-hidden="true" className="h-6 w-6 text-muted-foreground" />}
          title={t("components.fileViewerSheet.noWorkspaceAvailableTitle", {
            defaultValue: "No workspace available",
          })}
          body={t("components.fileViewerSheet.noWorkspaceAvailableBody", {
            defaultValue: "This issue does not have a workspace that supports preview yet.",
          })}
        />
      );
    }
  ...
  if (resource.kind === "remote_resource") {
    return (
      <FileViewerStateView
        ...
        title={t("components.fileViewerSheet.remoteWorkspaceComingSoonTitle", {
          defaultValue: "Remote workspace preview coming soon",
        })}
        body={t("components.fileViewerSheet.remoteWorkspaceComingSoonBody", {
          defaultValue: "This workspace is hosted remotely; inline previews are not supported yet.",
        })}
      />
    );
  }

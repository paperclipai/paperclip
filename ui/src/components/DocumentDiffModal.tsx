...
function getRevisionLabel(revision: DocumentRevision, t: (key: string, options?: any) => string) {
  const actorKey = revision.createdByUserId
    ? "documents.actor_board"
    : revision.createdByAgentId
      ? "documents.actor_agent"
      : "documents.actor_system";
  return `${t("documents.rev_label", { number: revision.revisionNumber })} — ${relativeTime(revision.createdAt)} • ${t(actorKey)}`;
}
      ...
      <DialogContent className="!max-w-[90%] w-full max-h-[85vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between gap-4">
          <DialogHeader className="shrink-0">
            <DialogTitle>
              {t("documents.diff_title")} — <span className="font-mono text-sm">{documentKey}</span>
            </DialogTitle>
          </DialogHeader>

          <div className="flex items-center gap-4 shrink-0">
            <div className="flex items-center gap-2">
              <span className="rounded-full border border-red-500/30 bg-red-500/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-red-400">{t("documents.old_label")}</span>
              <Select
              ...
              >
                <SelectTrigger className="h-7 w-60 text-xs border-border/60">
                  <SelectValue placeholder={t("documents.select_revision")} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="flex items-center gap-2">
              <span className="rounded-full border border-green-500/30 bg-green-500/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-green-400">{t("documents.new_label")}</span>
              <Select
        ...
        <div className="overflow-auto flex-1 rounded-md border border-border text-xs">
          {!revisions ? (
            ...
            <div className="p-6 text-center text-muted-foreground text-sm">{t("documents.select_two_revisions")}</div>
          ) : leftRevision.id === rightRevision.id ? (
            <div className="p-6 text-center text-muted-foreground text-sm">{t("documents.same_revision")}</div>
          ) : (
            <div className="font-mono text-[12px] leading-6">
              <div className="grid grid-cols-[56px_56px_24px_minmax(0,1fr)] border-b border-border/60 bg-muted/30 px-3 py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
                <span>{t("documents.old_label")}</span>
                <span>{t("documents.new_label")}</span>
                <span />
                <span>{t("documents.content_header")}</span>
              </div>

...
export function LegacyProjectRepository({ workspace, projectRef }: { workspace: ProjectWorkspace; projectRef: string }) {
  ...
  return <div className="flex min-w-0 flex-col gap-2">
    <span className="text-xs text-muted-foreground">{t("localizationProjectRepositories.existingUrl")}</span>
    {draft === null ? <div className="flex min-w-0 items-center gap-3 rounded-md border border-border px-3 py-2">
      ...
      <Button type="button" variant="ghost" size="sm" onClick={() => setDraft(workspace.repoUrl ?? "")}>{t("common.edit")}</Button>
    </div> : <form className="flex flex-col gap-2" onSubmit={(event) => { event.preventDefault(); if (!save.isPending) save.mutate(); }}>
      <Input aria-label={t("localizationProjectRepositories.existingUrl")} type="url" value={draft} disabled={save.isPending} onChange={(event) => setDraft(event.target.value)} />
      {save.isError && <p role="alert" className="text-sm text-destructive">{save.error.message}</p>}
      <div className="flex justify-end gap-2"><Button type="button" variant="ghost" disabled={save.isPending} onClick={() => setDraft(null)}>{t("localizationFilters.cancel")}</Button><Button type="submit" disabled={save.isPending}>{t("localizationProjectRepositories.saveUrl")}</Button></div>
    </form>}
  </div>;
}

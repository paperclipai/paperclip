  ...
  override render() {
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-4 px-4 py-10">
        <div>
          <h1 className="text-lg font-semibold">{t("app.routeErrorBoundary.thisPageHitAnError", { defaultValue: "This page hit an error" })}</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("app.routeErrorBoundary.somethingWentWrongWhileRenderingThisPageYouCanGoBackAndTryAgainOrReload", { defaultValue: "Something went wrong while rendering this page. You can go back and try again, or reload." })}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={this.props.onReset}>
            {t("app.routeErrorBoundary.goBack", { defaultValue: "Go back" })}
          </Button>
          <Button size="sm" onClick={() => window.location.reload()}>
            {t("app.routeErrorBoundary.reloadPage", { defaultValue: "Reload page" })}
          </Button>
        </div>
      </div>
    );

...
function useIssueColumnLabels(t: (key: string) => string) {
  return {
    status: t("status"),
    id: t("id"),
    assignee: t("assignee"),
...
function useIssueColumnDescriptions(t: (key: string) => string) {
  return {
    status: t("statusDesc"),
    id: t("idDesc"),
    assignee: t("assigneeDesc"),
    project: t("projectDesc"),
    workspace: t("workspaceDesc"),
    parent: t("parentDesc"),
    labels: t("labelsDesc"),
    updated: t("updatedDesc"),
  } as const;
}
...
export function IssueColumnPicker({
  ...
  const { t } = useTranslation("component.issueColumns");
  const columnLabels = useIssueColumnLabels(t);
  const columnDescriptions = useIssueColumnDescriptions(t);
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-[300px] rounded-xl border-border/70 p-1.5 shadow-xl shadow-black/10">
        ...
        >
          {t("resetDefaults")}
          <span className="ml-auto text-xs text-muted-foreground">{t("defaultColumns")}</span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
          ...
          return (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground" title={parentTitle ?? undefined}>
              {parentIdentifier ? (
                ...
                <span className="italic">{t("subIssue")}</span>
              )}
            </span>
          );
        }

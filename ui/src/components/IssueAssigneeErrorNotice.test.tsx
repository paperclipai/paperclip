...
describe("IssueAssigneeErrorNotice", () => {
  it("renders nothing without attention", () => {
    act(() => {
      root.render(<IssueAssigneeErrorNotice attention={null} />);
    });
    expect(container.querySelector('[data-testid="issue-assignee-error-notice"]')).toBeNull();
  });

  it("names the agent, links to it, shows the safe reason, and explains recovery", () => {
    act(() => {
      root.render(<IssueAssigneeErrorNotice attention={errorAttention} />);
    });
    ...
    const link = container.querySelector('[data-testid="issue-assignee-error-agent-link"]');

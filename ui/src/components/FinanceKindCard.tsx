  ...
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">{ t("app.financeKindCard.financialEventMix", { defaultValue: "Financial event mix" }) }</CardTitle>
        <CardDescription>{ t("app.financeKindCard.accountLevelChargesGroupedByEventKind", { defaultValue: "Account-level charges grouped by event kind." }) }</CardDescription>
      </CardHeader>
      <CardContent className="space-y-2 px-4 pb-4 pt-3">
        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">{ t("app.financeKindCard.noFinanceEventsInThisPeriod", { defaultValue: "No finance events in this period." }) }</p>
        ) : (
          rows.map((row) => (
            <div
              key={row.eventKind}

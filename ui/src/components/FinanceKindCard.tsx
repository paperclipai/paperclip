  ...
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">{i18n.t("components.FinanceKindCard.cardtitle")}</CardTitle>
        <CardDescription>{i18n.t("components.FinanceKindCard.carddescription")}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-2 px-4 pb-4 pt-3">
        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">{i18n.t("components.FinanceKindCard.p")}</p>
        ) : (
          rows.map((row) => (
            <div
              key={row.eventKind}

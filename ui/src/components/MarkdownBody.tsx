...
function MarkdownIssueLink({
  ...
  const issueLabel = title !== identifier
    ? t("markdownBody.issueLinkLabelWithTitle", {
        defaultValue: "Issue {{identifier}}: {{title}}",
        identifier,
        title,
      })
    : t("markdownBody.issueLinkLabel", {
...
function CodeBlock({
  ...
  const copyLabel = failed
    ? t("markdownBody.copyFailed", { defaultValue: "Copy failed" })
    : copied
      ? t("markdownBody.copied", { defaultValue: "Copied!" })
      : t("markdownBody.copy", { defaultValue: "Copy" });
    ...
    <div className="paperclip-markdown-codeblock" data-wrap-lines={wrapLines || undefined}>
      ...
      >
        ...
        <button
          type="button"
          onClick={handleCopy}
          aria-label={t("markdownBody.copyCodeAriaLabel", { defaultValue: "Copy code" })}
          title={copyLabel}
        ...
        <>
          <p className={cn("paperclip-mermaid-status", error && "paperclip-mermaid-status-error")}>
            {error
              ...
              : t("markdownBody.mermaidRendering", {
  ...
  const components: Components = {
    ...
    table: ({ node: _node, style: tableStyle, children: tableChildren, ...tableProps }) => (
      <div
        ...
        role="region"
        aria-label={t("markdownBody.scrollableTableAriaLabel", { defaultValue: "Scrollable table" })}
        tabIndex={0}
      >

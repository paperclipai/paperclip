  ...
  return (
    <SidebarSection
      ...
      menu={{
        ...
        actions: [
          { type: "item", label: t("localizationSidebar.browseAgents"), icon: Users, href: "/agents/all" },
          { type: "separator" },
        ],
        radioLabel: t("nav.sortAgents"),
        radioChoices: AGENT_SORT_CHOICES.map((choice) => ({ ...choice, label: t(`nav.sort.${choice.value}`) })),
        radioValue: sortMode,
      ...
      {showSeeAllLink && (() => {
        ...
        const seeAllLink = (
          <Link
            ...
            aria-label={rail ? t("localizationSidebar.seeAllAgents") : undefined}
            onClick={() => {
              if (isMobile) setSidebarOpen(false);
            }}
          ...
          >
            ...
            <span className={rail ? SIDEBAR_RAIL_HIDDEN_LABEL : undefined}>{t("localizationSidebar.seeAllAgents")}</span>
          </Link>
        );
        return rail ? (
          <Tooltip>
            <TooltipTrigger asChild>{seeAllLink}</TooltipTrigger>
            <TooltipContent side="right">{t("localizationSidebar.seeAllAgents")}</TooltipContent>
          </Tooltip>
        ) : (
          seeAllLink
        );

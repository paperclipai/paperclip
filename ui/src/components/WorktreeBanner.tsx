  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 overflow-hidden whitespace-nowrap">
        <span className="shrink-0 opacity-70">{t("app.worktreeBanner.worktree", { defaultValue: "Worktree" })}</span>
        <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-current opacity-70" aria-hidden="true" />
        <button
          type="button"
          onClick={handleCopyName}
          title={t("app.worktreeBanner.clickToCopyWorktreeName", { defaultValue: "Click to copy worktree name" })}
        ...
        >
          {copied ? t("app.worktreeBanner.copied", { defaultValue: "Copied!" }) : branding.name}
        </button>
      </div>
    </div>
  );

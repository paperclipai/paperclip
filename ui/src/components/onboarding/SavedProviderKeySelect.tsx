    ...
    <div className="space-y-2">
      {options.length > 0 && (
        <label className="block space-y-2 text-sm">
          <span>{kind === "api" ? t("localizationAgents.ui386_API_key") : t("localizationCommon.billingType.subscription_included")}</span>
          <select
            aria-label={kind === "api" ? t("agentSetup.savedApiKey") : t("agentSetup.savedSubscription")}
          ...
          >
            ...
            <option value="">
              {kind === "api"
                ? t("agentSetup.enterNewKey")
                : t("agentSetup.signInOther")}
            </option>
          </select>
        </label>
      )}
      {loading && (
        <p role="status" className="text-sm text-muted-foreground">
          {t("agentSetup.checkingSavedKeys")}
        </p>
      )}
      {error && (
        <p role="alert" className="text-sm text-destructive">
          {t("agentSetup.loadSavedKeysFailed")}
        </p>
      )}
      {value && (
        <p className="text-sm text-muted-foreground">
          {t(kind === "api" ? "agentSetup.reuseSavedKey" : "agentSetup.reuseSavedSubscription")}
        </p>
      )}
    </div>
  );

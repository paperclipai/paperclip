...
import { sidebarAgents } from "../lib/i18n";
import {
...
const AGENT_SORT_CHOICES: SidebarSectionRadioChoice[] = [
  { value: "top", label: sidebarAgents.sortTop },
  { value: "alphabetical", label: sidebarAgents.sortAlphabetical },
  { value: "recent", label: sidebarAgents.sortRecent },
];
...
function SidebarAgentItem({
  ...
  const pauseResumeLabel = isPaused ? sidebarAgents.resume : sidebarAgents.pause;
  const pauseResumeDisabled = disabled || agent.status === "pending_approval" || isBudgetPaused;
  const pauseResumeDisabledLabel = disabled
    ? sidebarAgents.updating
    : isBudgetPaused
      ? sidebarAgents.budgetPaused
      : pauseResumeLabel;
        ...
        {(agent.pauseReason === "budget" || runCount > 0) && (
          <span className="ml-auto flex items-center gap-1.5 shrink-0">
            {agent.pauseReason === "budget" ? (
              <BudgetSidebarMarker title={sidebarAgents.budgetMarker} />
            ) : null}
            {runCount > 0 ? (
            ...
            {runCount > 0 ? (
              <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400">
                {sidebarAgents.liveRuns(runCount)}
              </span>
            ) : null}
          </span>
        )}
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            aria-label={sidebarAgents.actionsAria(agent.name)}
          >
        ...
        <DropdownMenuContent align="end" className="w-44">
          ...
          <DropdownMenuItem
            ...
            title={isBudgetPaused ? sidebarAgents.budgetPausedTitle : undefined}
          >
...
export function SidebarAgents() {
  ...
  const pauseResumeAgent = useMutation({
    ...
    onSuccess: async (_agent, { agent, action }) => {
      ...
      pushToast({
        title: action === "pause" ? sidebarAgents.toastPaused : sidebarAgents.toastResumed,
        body: agent.name,
        tone: "success",
      });
    },
    onError: (error, { agent, action }) => {
      pushToast({
        title: action === "pause" ? sidebarAgents.toastPauseErr : sidebarAgents.toastResumeErr,
  ...
  return (
    <SidebarSection
      label={sidebarAgents.section}
      collapsible={{ open, onOpenChange: setOpen }}
      headerAction={{
        ariaLabel: sidebarAgents.newAgentAria,
        icon: Plus,
        onClick: openNewAgent,
      }}
      menu={{
        ariaLabel: sidebarAgents.menuAria,
        actions: [
          { type: "item", label: sidebarAgents.browse, icon: Users, href: "/agents/all" },
          { type: "separator" },
        ],
        radioLabel: sidebarAgents.sortLabel,

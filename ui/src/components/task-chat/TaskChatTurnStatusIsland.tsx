...
function fileCountLabel(files: number): string {
  return t("localizationTaskRuntime.changedFileCount", { count: files });
}
      ...
      {plan ? (
        <span className="flex shrink-0 items-center gap-2">
          ...
          <span className="font-mono text-sm tabular-nums" aria-live="polite" aria-atomic="true">
            {t("localizationTaskRuntime.ui_Step_jtn9kf")} {plan.currentStepIndex + 1} / {plan.steps.length}
...
function islandLabel(model: TaskChatTurnStatusModel): string {
  ...
  for (const segment of model.segments) {
    if (segment.kind === "plan") {
      ...
      parts.push(t("localizationTaskRuntime.islandStep", { step: segment.currentStepIndex + 1, total: segment.steps.length, label: step ? `: ${step.label}` : "" }));
    } else {
      parts.push(fileCountLabel(segment.files));
      if (segment.additions != null) parts.push(t("localizationTaskRuntime.additionsCount", { count: segment.additions }));
      if (segment.deletions != null) parts.push(t("localizationTaskRuntime.deletionsCount", { count: segment.deletions }));
    }
  }
  return parts.join(", ");
}
  ...
  return (
    ...
    >
      ...
      <PopoverContent
        ...
        aria-label={t("localizationTaskRuntime.ui_Turn_plan_ff08ut")}
        onPointerEnter={(event) => {
      ...
      >
        <ol className="flex flex-col gap-1" aria-label={t("localizationTaskRuntime.ui_Within_turn_checklist_17x2lu2")}>

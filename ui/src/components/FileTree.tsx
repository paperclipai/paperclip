...
export function FileTree({
  ...
  const effectiveAriaLabel = ariaLabel ?? t("component.fileTree.files");
  ...
  if (loading) {
    return (
      <div aria-busy="true" aria-label={effectiveAriaLabel} role="tree" className="py-1">
        {[0, 1, 2, 3].map((row) => (
  ...
  if (error) {
    return (
      <div aria-label={effectiveAriaLabel} role="tree" className="p-3">
        <div
          role="treeitem"
          aria-level={1}
    ...
    return (
      <div aria-label={effectiveAriaLabel} role="tree" className="p-3">
        <div className="rounded-md border border-dashed border-border px-4 py-8 text-center">
          <div className="text-sm font-medium">{empty?.title ?? t("component.fileTree.noFiles")}</div>
          <div className="mt-1 text-xs text-muted-foreground">
            {empty?.description ?? t("component.fileTree.emptyDescription")}
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div aria-label={effectiveAriaLabel} role="tree">
          ...
          >
            ...
            {node.kind === "dir" && (
              <button
                ...
                aria-label={expanded ? t("component.fileTree.collapse", { name: node.name }) : t("component.fileTree.expand", { name: node.name })}
              >
                {expanded ? (

...
const FILTER_OPTIONS: Array<{ value: FilterValue; label: string }> = [
  { value: "all", get label() { return t("localizationActivityChrome.all"); } },
  { value: "in-progress", get label() { return t("localizationActivityChrome.inProgress"); } },
  { value: "for-review", get label() { return t("localizationActivityChrome.inReview"); } },
  { value: "completed", get label() { return t("localizationActivityChrome.done"); } },
];
...
function CollapsedFeedGroup({
  ...
  const actorName = actor?.name
    ?? (group.latestEvent.actorType === "system" ? t("localizationActivityChrome.system")
      : group.latestEvent.actorType === "user" ? t("localizationActivityChrome.board")
      : t("localizationActivityChrome.unknown"));
    ...
    <div>
      ...
      >
        ...
        <span className="flex-1 min-w-0 truncate">
          <Trans t={t} i18nKey="localizationActivityChrome.collapsedUpdates" count={group.events.length}
            values={{ actorName, entityName: entityName ?? group.entityId }}
            components={{
              actor: <span data-fc="actor" className="font-medium text-(--hex-959596) group-hover:text-white" />,
              verb: <span data-fc="verb" className="ml-1 text-(--hex-959596)" />,
              entity: <span data-fc="title" className="ml-1 text-(--hex-959596) group-hover:text-white" />,
            }}
          />
        </span>
...
export function ActivityFeed({ className }: ActivityFeedProps) {
        ...
        separator = (
          <div className="flex items-center gap-2 px-4 py-1.5" key={`sep-${index}`}>
            ...
            <span className="text-(length:--text-nano) font-medium text-muted-foreground uppercase tracking-wider">
              {t("localizationActivityChrome.earlier")}
            </span>
            <div className="h-px flex-1 bg-border" />
          </div>
        );
  ...
  const renderGrouped = () => {
    ...
    return Array.from(groups.entries()).map(([groupKey, items]) => {
      ...
      const label = isOther
        ? t("localizationActivityChrome.otherActivity")
  ...
  const emptyMessage = useMemo(() => {
    ...
    if (agents.length === 0) {
      return {
        text: t("localizationActivityChrome.noAgents"),
        showPulse: false,
      };
    }
    ...
    if (allPaused) {
      return {
        text: t("localizationActivityChrome.allAgentsPaused"),
        showPulse: false,
      };
    }
    return {
      text: t("localizationActivityChrome.agentsWorking"),
      showPulse: true,
    };
  }, [agents, t]);
    ...
    <aside className={cn("flex min-h-0 min-w-0 flex-1 flex-col bg-background", className)}>
      ...
      <div className="relative flex shrink-0 items-start justify-between gap-2 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="text-sm font-semibold">{t("localizationActivityChrome.agentFeed")}</h3>
          <p className="text-xs text-muted-foreground">
            {t("localizationActivityChrome.agentFeedDescription")}
          </p>
        </div>
        <div className="flex items-center gap-1">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={t("localizationActivityChrome.groupByTaskAria")}
            ...
            <TooltipContent side="bottom">
              {groupMode === "flat" ? t("localizationActivityChrome.groupByTask") : t("localizationActivityChrome.showFlat")}
            </TooltipContent>
          </Tooltip>
          ...
          <DropdownMenu>
              ...
              <TooltipTrigger asChild>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    size="icon-sm"
                    className="shrink-0 text-muted-foreground"
                    aria-label={t("localizationActivityChrome.filterByAria")}
                  >
                    <ListFilter className="h-3.5 w-3.5" />
                  </Button>
                </DropdownMenuTrigger>
              </TooltipTrigger>
              <TooltipContent side="bottom">{t("localizationActivityChrome.filterBy")}</TooltipContent>
            </Tooltip>
            <DropdownMenuContent align="end" className="w-48">
              ...
              >
                {t("localizationActivityChrome.showAllActivity")}
              </DropdownMenuCheckboxItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
          ...
          <div className="flex min-h-0 flex-1 items-center justify-center p-6 h-full">
            <div className="flex flex-col items-center gap-2 max-w-(--sz-16rem)">
              ...
              <p className="text-center text-sm text-muted-foreground">
                {emptyMessage?.text ?? t("localizationActivityChrome.activityEmpty")}
              </p>
            </div>
          </div>
        ) : groupMode === "flat" ? (

...
export function SkillFolderRail({
  ...
  return (
    <div className="relative hidden h-full shrink-0 md:flex" style={{ width: `${width}px` }}>
      <nav
        aria-label={t("localizationFolders.skillFolders")}
      ...
      <div className="mb-2 flex items-center justify-between gap-2 pt-0.5">
        ...
        <Button variant="ghost" size="icon-sm" title={t("localizationFolders.newFolder")} onClick={() => onCreateFolder(null)}>
      ...
      {loading ? (
        ...
        <div className="space-y-0.5">
          <VirtualRow
            ...
            label={t("localizationFolders.allSkills")}
            count={allCount}
          ...
          {model.my ? (
            ...
            <VirtualRow
              active={false}
              label={t("localizationFolders.mySkills")}
              count={0}
          ...
          <RailHeading label={t("localizationFolders.organization")} onCreate={() => onCreateFolder(null)} />
          {model.company.length > 0 ? (
            ...
            <div className="px-2 py-1 text-xs text-muted-foreground">{t("localizationFolders.noOrganizationFolders")}</div>
          )}
          ...
          {model.projects ? (
            ...
            <VirtualRow
              active={false}
              label={t("localizationFolders.projects")}
              count={0}
      ...
      {tags.length > 0 ? (
        <div className="mt-4 border-t border-border pt-3">
          <div className="mb-1.5 flex items-center gap-1.5 px-2 text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
            <Hash className="h-3 w-3" />{t("localizationFolders.tags")}</div>
      ...
      <div
        role="separator"
        aria-label={t("localizationFolders.resize")}
  ...
  return (
    <div className="group/heading flex items-center justify-between px-2 pb-0.5 pt-3">
      ...
      <button
        type="button"
        onClick={onCreate}
        title={t("localizationFolders.newFolderIn", { section: label.toLowerCase() })}
  ...
  return (
    <div>
      ...
      >
        <button
          type="button"
          aria-label={isOpen ? t("localizationFolders.collapseFolder") : t("localizationFolders.expandFolder")}
          className={cn(
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                aria-label={t("localizationFolders.folderActions", { name: label })}
              >
            ...
            <DropdownMenuContent align="end">
              {canNest ? (
                <DropdownMenuItem onSelect={() => onCreateFolder(folder.id)}>
                  <FolderPlus className="h-3.5 w-3.5" />{t("localizationFolders.newSubfolder")}</DropdownMenuItem>
              ) : null}
              {editable ? (
                <>
                  <DropdownMenuItem onSelect={() => onStartRename(folder)}>{t("localizationFolders.rename")}</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onEditFolder(folder)}>{t("localizationFolders.editColor")}</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onMoveFolder(folder, isInMySkills ? "company" : "my")}>
                    <MoveRight className="h-3.5 w-3.5" />
                    {t(isInMySkills ? "localizationFolders.moveToOrganization" : "localizationFolders.moveToMySkills")}
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
...
export function FolderBreadcrumb({
  ...
  return (
    <nav aria-label={t("localizationFolders.folderPath")} className="flex flex-wrap items-center gap-1 text-sm">
      <button
        type="button"
      ...
      >
        <Home className="h-3.5 w-3.5" />{t("localizationFolders.allSkills")}</button>
      {selection === "unfiled" ? (
        <>
          <ChevronRight className="h-3.5 w-3.5 text-muted-foreground/60" />
          <span className="rounded px-1.5 py-0.5 font-medium text-foreground">{t("localizationFolders.unfiled")}</span>
        </>
      ) : null}
...
export function FolderTiles({
  ...
  return (
    <div className="mb-4">
      <div className="mb-2 text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">{t("localizationFolders.folders")}</div>
    ...
    return (
      <div key={folder.id}>
        ...
        >
          ...
          {isCurrent ? <span className="text-xs text-muted-foreground">{t("localizationFolders.current")}</span> : null}
          {bundled ? <span className="text-xs text-muted-foreground">{t("localizationFolders.readOnly")}</span> : null}
          {isChosen ? <Check className="h-3.5 w-3.5" /> : null}
          {nestable ? (
            <button
              type="button"
              title={t("localizationFolders.newInside")}
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="max-h-72 overflow-y-auto rounded-md border border-border p-1">
          ...
          >
            ...
            <span className="min-w-0 flex-1 truncate">{t("localizationFolders.unfiled")}</span>
            {currentFolderId == null ? <span className="text-xs text-muted-foreground">{t("localizationFolders.current")}</span> : null}
          ...
          <div className="mt-1 border-t border-border pt-1">
            {creatingParent === null ? (
              ...
              >
                <Plus className="h-3.5 w-3.5" />{t("localizationFolders.newTopLevel")}</button>
            )}
          </div>
        </div>
        ...
        <div className="min-h-5 text-xs text-muted-foreground">
          {previewPath ? (
            <span>
              <Trans i18nKey="localizationFolders.movingTo" values={{ path: previewPath }} components={{ path: <span className="font-mono text-foreground" /> }} />
            </span>
          ) : (
            <span>{t("localizationFolders.pickDestination")}</span>
          )}
        </div>
        ...
        <DialogFooter>
          ...
          >
            {pending ? t("localizationFolders.moving") : t("localizationFolders.moveHere")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  ...
  return (
    <div className="flex items-center gap-2 py-1" style={{ paddingLeft: `${0.5 + depth * 0.9}rem` }}>
      ...
      <Input
        ...
        placeholder={t("localizationFolders.folderName")}
        autoFocus
      ...
      />
      <Button size="sm" variant="ghost" onClick={onCancel} disabled={pending}>{t("localizationFolders.cancel")}</Button>
      <Button size="sm" onClick={onSubmit} disabled={pending || !value.trim()}>{t("localizationFolders.add")}</Button>
    </div>
  );
}

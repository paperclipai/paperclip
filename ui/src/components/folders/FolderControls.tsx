...
export function FolderSwatch({
  ...
  return (
    <span
      aria-hidden="true"
      className={cn("h-2.5 w-2.5 shrink-0 rounded-(--rad-3) border border-border/40", className)}
      style={{ backgroundColor: color || "var(--folder-swatch-6)" }}
    />
  );
}
          ...
          {folders.map((folder) => {
            ...
            return (
              <div
                key={folder.id}
                className={cn(
                  "group grid grid-cols-(--gtc-folder-action-row) items-center gap-2 rounded-md px-2 py-1 text-sm transition-colors hover:bg-accent/40",
                  active ? "bg-accent/60 text-foreground" : "text-muted-foreground",
                )}
              >
...
export function MobileFolderSheet({
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-(--sz-80dvh) rounded-t-lg pb-4">

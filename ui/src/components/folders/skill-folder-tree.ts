...
export function reservedRootLabel(folder: Pick<FolderListItem, "systemKey" | "name">): string {
  switch (folder.systemKey) {
    case "my":
      return t("localizationFolders.mySkills");
    case "projects":
      return t("localizationFolders.projects");
    case "bundled":
      return t("localizationFolders.bundled");
    default:
      return folder.name;
  }
}
...
export function skillFolderDisplayPath(
  ...
  if (!trail[0]?.systemKey) labels.unshift(t("localizationFolders.organization"));
  return labels.join(" / ");
}
...
export function skillFolderPathDisplayFallback(folderPath: string | null | undefined): string | null {
  ...
  if (root === "my") labels[0] = t("localizationFolders.mySkills");
  else if (root === "projects") labels[0] = t("localizationFolders.projects");
  else if (root === "bundled") labels[0] = t("localizationFolders.bundled");
  else labels.unshift(t("localizationFolders.organization"));
  return labels.join(" / ");
}

...
describe("IssuesList", () => {
  ...
  it("keeps rendering local issue batches while the user stays near the bottom", async () => {
    ...
    // Passive effects subscribe to window scroll after the first row batch has
    // mounted. Re-dispatch while the viewport remains at the bottom so this
    // assertion tests the scroll behavior rather than a scheduling race.
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      if (container.querySelectorAll('[data-testid="issue-row"]').length === 250) break;
      await flush();
    }

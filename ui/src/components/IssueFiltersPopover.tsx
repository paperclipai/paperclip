      ...
      >
        <div className="space-y-3 p-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium">{t("filtersPopover.title", { defaultValue: "Filters" })}</span>
            {activeFilterCount > 0 ? (
              <button
                type="button"
          ...
          <div className="space-y-1.5">
            <span className="text-xs text-muted-foreground">{t("filtersPopover.quickFilters", { defaultValue: "Quick filters" })}</span>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("filtersPopover.status", { defaultValue: "Status" })}</span>
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("filtersPopover.priority", { defaultValue: "Priority" })}</span>
                <div className="space-y-0.5">
            ...
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("filtersPopover.assignee", { defaultValue: "Assignee" })}</span>
                <div className="max-h-32 space-y-0.5 overflow-y-auto">
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("comments.noAssignee", { defaultValue: "No assignee" })}</span>
                  </label>
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
              ...
              {creatorOptions.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("filtersPopover.creator", { defaultValue: "Creator" })}</span>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder={t("filtersPopover.searchCreators", { defaultValue: "Search creators..." })}
                  ...
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {visibleCreatorOptions.length > 0 ? visibleCreatorOptions.map((creator) => {
                      ...
                      <div className="px-2 py-1 text-xs text-muted-foreground">{t("filtersPopover.noCreatorsMatch", { defaultValue: "No creators match." })}</div>
                    )}
                  </div>
                </div>
              ) : null}
              ...
              {projects && projects.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("filtersPopover.project", { defaultValue: "Project" })}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
            ...
            <div className="min-w-0 space-y-3">
              {labels && labels.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("filtersPopover.labels", { defaultValue: "Labels" })}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {labels.map((label) => (
              ...
              {workspaces && workspaces.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("filtersPopover.workspace", { defaultValue: "Workspace" })}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("filtersPopover.visibility", { defaultValue: "Visibility" })}</span>
                <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                  <Checkbox
                    checked={state.liveOnly}
                    onCheckedChange={(checked) => onChange({ liveOnly: checked === true })}
                  />
                  <span className="text-sm">{t("filtersPopover.liveRunsOnly", { defaultValue: "Live runs only" })}</span>
                </label>
                {enableRoutineVisibilityFilter ? (
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("filtersPopover.hideRoutineRuns", { defaultValue: "Hide routine runs" })}</span>
                  </label>
                ) : null}
              </div>
            </div>

...
export function IssueFiltersPopover({
  ...
  const quickFilterPresets = useMemo(() => getIssueQuickFilterPresets((key) => t(key)), [t]);
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={buttonVariant} size={iconOnly ? "icon" : "sm"} className={`text-xs ${iconOnly ? "relative h-8 w-8 shrink-0" : ""} ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`} title={iconOnly ? (activeFilterCount > 0 ? t("filters.activeCount", { count: activeFilterCount }) : t("filters.filter")) : undefined}>
          <Filter className={iconOnly ? "h-3.5 w-3.5" : "h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1"} />
          {!iconOnly && <span className="hidden sm:inline">{activeFilterCount > 0 ? t("filters.activeCount", { count: activeFilterCount }) : t("filters.filter")}</span>}
      ...
      >
        <div className="space-y-3 p-3">
          ...
          <div className="space-y-1.5">
            <span className="text-xs text-muted-foreground">{t("filters.quickFilters")}</span>
                ...
                <div className="space-y-0.5">
                  {issueStatusOrder.map((status) => (
                    <label key={status} className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      ...
                      <span className="text-sm">{issueFilterLabel(status, (key) => t(key))}</span>
                    </label>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-0.5">
                  {issuePriorityOrder.map((priority) => (
                    <label key={priority} className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      ...
                      <span className="text-sm">{issueFilterLabel(priority, (key) => t(key))}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="max-h-32 space-y-0.5 overflow-y-auto">
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("filters.noAssignee")}</span>
                  </label>
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
                ...
                <div className="space-y-1">
                      ...
                      {selectedCreatorOptions.map((creator) => (
                        <Badge key={creator.id} variant="secondary" className="gap-1 pr-1">
                          ...
                          <button
                            ...
                            aria-label={t("filters.removeCreator", { name: creator.label })}
                          >
                            <X className="h-3 w-3" />
                          </button>
                        </Badge>
                  ...
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {visibleCreatorOptions.length > 0 ? visibleCreatorOptions.map((creator) => {
                      ...
                      <div className="px-2 py-1 text-xs text-muted-foreground">{t("filters.noCreatorsMatch")}</div>
                    )}
                  </div>
                </div>
              ) : null}
            ...
            <div className="min-w-0 space-y-3">
              ...
              <div className="space-y-1">
                ...
                <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                  ...
                  />
                  <span className="text-sm">{t("filters.liveRunsOnly")}</span>
                </label>
                {enableRoutineVisibilityFilter ? (
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("filters.hideRoutineRuns")}</span>
                  </label>
                ) : null}
              </div>
            </div>

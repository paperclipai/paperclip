  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={buttonVariant} size={iconOnly ? "icon" : "sm"} className={`text-xs ${iconOnly ? "relative h-8 w-8 shrink-0" : ""} ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`} title={iconOnly ? (activeFilterCount > 0 ? `${t("component.issue.filters.filters")}: ${activeFilterCount}` : t("component.issue.filters.filter")) : undefined}>
          <Filter className={iconOnly ? "h-3.5 w-3.5" : "h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1"} />
          {!iconOnly && <span className="hidden sm:inline">{activeFilterCount > 0 ? `${t("component.issue.filters.filters")}: ${activeFilterCount}` : t("component.issue.filters.filter")}</span>}
      ...
      >
        <div className="space-y-3 p-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium">{t("component.issue.filters.title")}</span>
            {activeFilterCount > 0 ? (
              <button
                type="button"
              ...
              >
                {t("component.issue.filters.clear")}
              </button>
            ) : null}
          </div>

          <div className="space-y-1.5">
            <span className="text-xs text-muted-foreground">{t("component.issue.filters.quickFilters")}</span>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("component.issue.filters.status")}</span>
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("component.issue.filters.priority")}</span>
                <div className="space-y-0.5">

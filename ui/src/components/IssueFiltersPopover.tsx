...
export function IssueFiltersPopover({
  ...
  const filtersLabel = activeFilterCount > 0
    ? t("issue_filters.filters_with_count", { count: activeFilterCount })
    : t("common.filter");
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button
          ...
          title={iconOnly ? filtersLabel : undefined}
        >
          <Filter className={iconOnly ? "h-3.5 w-3.5" : "h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1"} />
          {!iconOnly && <span className="hidden sm:inline">{filtersLabel}</span>}
      ...
      <PopoverContent align="end" className="w-[min(480px,calc(100vw-2rem))] p-0">
        <div className="space-y-3 p-3">
          ...
          <div className="space-y-1.5">
            <span className="text-xs text-muted-foreground">{t("issue_filters.quick_filters")}</span>
          ...
          <div className="grid grid-cols-1 gap-x-4 gap-y-3 sm:grid-cols-2">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                ...
                <div className="max-h-32 space-y-0.5 overflow-y-auto">
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("issues.unassigned")}</span>
                  </label>
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
                      ...
                      <span className="text-sm">{t("common.me")}</span>
                    </label>
                  ) : null}
                  {(agents ?? []).map((agent) => (
              ...
              {workspaces && workspaces.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issues.workspace")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
              ...
              {enableRoutineVisibilityFilter ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("common.visibility")}</span>
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    <Checkbox
                      checked={state.showRoutineExecutions}
                      onCheckedChange={(checked) => onChange({ showRoutineExecutions: checked === true })}
                    />
                    <span className="text-sm">{t("issue_filters.show_routine_runs")}</span>
                  </label>
                </div>
              ) : null}
            </div>

        ...
        >
          {unknownManager ? (
            ...
            <>
              ...
              >
                {t("reportsToPicker.reportsTo", {
                  name: current.name,
                  suffix: terminatedManager ? t("reportsToPicker.terminatedSuffix") : "",
                })}
              </span>
            </>
          ) : (
            <>
              ...
              <span className="min-w-0 truncate">
                {disabled
                  ? (disabledEmptyLabel ?? t("reportsToPicker.disabledEmptyLabel"))
                  : (chooseLabel ?? t("reportsToPicker.chooseLabel"))}
              </span>
            </>
          )}
        </button>
      ...
      <PopoverContent className="w-48 p-1" align="start">
        ...
        {terminatedManager && (
          <div className="flex min-w-0 items-center gap-2 overflow-hidden px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            ...
            <span className="min-w-0 truncate">
              {t("reportsToPicker.currentManager", { name: current.name })}
            </span>
          </div>
        )}
        {unknownManager && (
        ...
        {rows.map((a) => (
          ...
          >
            ...
            <span className="text-muted-foreground ml-auto shrink-0">{tx(roleLabels[a.role] ?? a.role)}</span>
          </button>
        ))}
      </PopoverContent>
    </Popover>

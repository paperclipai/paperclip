...
import { agentConfigUi } from "../lib/i18n";
...
export function ReportsToPicker({
  ...
  disabled = false,
  excludeAgentIds = [],
  disabledEmptyLabel = agentConfigUi.reportsPickerCeoLocked,
  chooseLabel = agentConfigUi.reportsPickerChoosePlaceholder,
}: {
  agents: Agent[];
  value: string | null;
        ...
        >
          {unknownManager ? (
            <>
              ...
              <span className="min-w-0 truncate text-muted-foreground">{agentConfigUi.unknownManagerStaleId}</span>
            </>
          ) : current ? (
            <>
              ...
              >
                {agentConfigUi.reportingVerbWithName(
                  current.name,
                  terminatedManager ? agentConfigUi.reportingLineTerminatedSuffix : "",
                )}
              </span>
            </>
          ) : (
      ...
      <PopoverContent className="w-48 p-1" align="start">
        ...
        {terminatedManager && (
          <div className="flex min-w-0 items-center gap-2 overflow-hidden px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            ...
            <span className="min-w-0 truncate">
              {agentConfigUi.reportsCurrentTerminated(current.name)}
            </span>
          </div>
        )}
        {unknownManager && (
          <div className="px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            {agentConfigUi.reportsStaleExplanation}
          </div>
        )}
        {rows.map((a) => (
          <button

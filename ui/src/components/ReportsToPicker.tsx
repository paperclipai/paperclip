...
export function ReportsToPicker({
  ...
  const fallbackDisabledEmptyLabel = t("page.components.reportsToPicker.disabled_empty");
  const fallbackChooseLabel = t("page.components.reportsToPicker.choose");
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          {unknownManager ? (
            <>
              ...
              <span className="min-w-0 truncate text-muted-foreground">{t("page.components.reportsToPicker.unknown_manager")}</span>
            </>
          ) : current ? (
            <>
              ...
              >
                {t("page.components.reportsToPicker.reports_to_name", {
                  name: current.name,
                  suffix: terminatedManager ? ` (${t("page.components.reportsToPicker.terminated")})` : "",
                })}
              </span>
            </>
          ) : (
            <>
              ...
              <span className="min-w-0 truncate">
                {disabled ? (disabledEmptyLabel ?? fallbackDisabledEmptyLabel) : (chooseLabel ?? fallbackChooseLabel)}
              </span>
            </>
          )}
        </button>
      ...
      <PopoverContent className="w-48 p-1" align="start">
        ...
        >
          {t("page.components.reportsToPicker.no_manager")}
        </button>
        {terminatedManager && (
          <div className="flex min-w-0 items-center gap-2 overflow-hidden px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            ...
            <span className="min-w-0 truncate">
              {t("page.components.reportsToPicker.current_terminated", { name: current.name })}
            </span>
          </div>
        )}
        {unknownManager && (
          <div className="px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            {t("page.components.reportsToPicker.saved_missing")}
          </div>
        )}
        {rows.map((a) => (
          <button

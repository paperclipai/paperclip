...
export function SidebarAccountMenu({
  ...
  const displayName = session?.user.name?.trim() || t("account.boardName", { defaultValue: "Board" });
  const secondaryLabel =
    session?.user.email?.trim() ||
    (deploymentMode === "authenticated"
      ? t("account.signedIn", { defaultValue: "Signed in" })
      : t("account.localBoard", { defaultValue: "Local workspace board" }));
  const accountBadge =
    deploymentMode === "authenticated"
      ? t("account.badgeAccount", { defaultValue: "Account" })
      : t("account.badgeLocal", { defaultValue: "Local" });
        ...
        >
          ...
          <div className="-mt-8 px-4 pb-4">
              ...
              <div className="min-w-0 flex-1 pt-1">
                ...
                {version ? (
                  <p className="mt-1 text-xs text-muted-foreground">
                    {t("account.version", { defaultValue: "Paperclip v{{version}}", version })}
                  </p>
                ) : null}
              </div>
            </div>
            ...
            <div className="mt-4 space-y-1">
              ...
              />
              <MenuAction
                label={t("account.instanceSettings", { defaultValue: "Instance settings" })}
                description={t("account.instanceSettingsDescription", { defaultValue: "Jump back to the last settings page you opened." })}
                icon={Settings}
              ...
              <MenuAction
                label={theme === "dark"
                  ? t("account.switchToLight", { defaultValue: "Switch to light mode" })
                  : t("account.switchToDark", { defaultValue: "Switch to dark mode" })}
                description={t("account.toggleTheme", { defaultValue: "Toggle the app appearance." })}

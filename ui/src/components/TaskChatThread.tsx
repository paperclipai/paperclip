...
export function TaskChatThread(props: TaskChatThreadProps) {
  ...
  const liveRunActions = useMemo(() => {
    ...
    const isPausing = stoppingRunId === liveRun.id;
    return (
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button type="button" size="icon-xs" variant="ghost" title="Run actions" aria-label="Run actions">
            <MoreHorizontal />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end">
          ...
          {onStopRun ? (
            ...
            >
              ...
              {isPausing ? "Pausing..." : stopRunLabel}
            </DropdownMenuItem>
          ) : null}
          {runFinalizationActions.map((action) => (
            <DropdownMenuItem
              key={action.id}
              disabled={action.disabled || action.isPending || isPausing}
  ...
  }, [liveRun, onCancelRun, onStopRun, runFinalizationActions, stopRunLabel, stopRunVariant, stoppingRunId]);
            ...
            tail={tailRunId ? (
              <div data-testid="task-chat-live-transcript">
                <TaskChatLiveRunPill
                  ...
                  actions={tailStreaming ? liveRunActions : undefined}
                />
                <TaskChatLiveTail
                  items={tailItems}
                  emptyMessage={

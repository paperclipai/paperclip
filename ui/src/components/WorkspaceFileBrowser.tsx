...
export function describeUnavailable(reason: string): { title: string; body: string; icon: ReactNode } {
  ...
  if (lower.includes("remote")) {
    return {
      ...
      title: t("components.workspaceFileBrowser.remoteUnsupportedTitle", { defaultValue: "Remote workspace preview not supported" }),
      body: t("components.workspaceFileBrowser.remoteUnsupportedBody", { defaultValue: "This workspace is hosted remotely and is not available for inline preview yet." }),
    };
  }
  if (lower.includes("no_workspace") || lower.includes("no_local")) {
    return {
      ...
      title: t("components.workspaceFileBrowser.noWorkspaceTitle", { defaultValue: "No workspace yet" }),
      body: t("components.workspaceFileBrowser.noWorkspaceBody", { defaultValue: "This issue does not have a workspace to browse. Files appear here once a run creates one." }),
    };
  }
  if (lower.includes("archiv") || lower.includes("cleaned") || lower.includes("unavailable")) {
    return {
      icon: <FolderOpen aria-hidden="true" className="h-5 w-5 text-muted-foreground" />,
      title: t("components.workspaceFileBrowser.workspaceGoneTitle", { defaultValue: "Workspace is no longer available" }),
      body: t("components.workspaceFileBrowser.workspaceGoneBody", { defaultValue: "The isolated worktree for this issue has been cleaned up, so files cannot be previewed." }),
    };
  }
  return {
    icon: <AlertTriangle aria-hidden="true" className="h-5 w-5 text-amber-500" />,
    title: t("components.workspaceFileBrowser.workspaceUnavailableTitle", { defaultValue: "Workspace unavailable" }),
    body: t("components.workspaceFileBrowser.workspaceUnavailableBody", { defaultValue: "These workspace files can't be browsed right now." }),
  };
}
...
function WorkspaceFileBreadcrumbs({
  ...
  return (
    <nav aria-label={t("components.workspaceFileBrowser.currentFolderAria", { defaultValue: "Current folder" })} className="min-w-0 overflow-hidden text-[11px] text-muted-foreground">
...
function WorkspaceFileTree({
            ...
            <>
              ...
              {loading ? (
                ...
                >
                  ...
                  <span>{t("components.workspaceFileBrowser.loadingFolder", { defaultValue: "Loading folder…" })}</span>
                </div>
              ) : null}
              {truncated ? (
                <button
                ...
                >
                  ...
                  <span>{t("components.workspaceFileBrowser.loadMoreFromFolder", { defaultValue: "Load more from this folder" })}</span>
                </button>
              ) : null}
            </>
          ) : null}
  ...
  return (
    <div role="tree" id={listboxId} aria-label={t("components.workspaceFileBrowser.workspaceFilesAria", { defaultValue: "Workspace files" })} className="space-y-0.5 py-1">
...
export function WorkspaceFileBrowser({
  ...
  const announcement = useMemo(() => {
    if (listQuery.isFetching) return t("components.workspaceFileBrowser.announceLoading", { defaultValue: "Loading workspace files…" });
    if (listQuery.isError) return t("components.workspaceFileBrowser.announceError", { defaultValue: "Unable to load workspace files." });
    if (data?.state === "unavailable") return describeUnavailable(data.unavailableReason ?? "").title;
    if (items.length === 0) return t("components.workspaceFileBrowser.announceNoMatch", { defaultValue: "No matching files." });
    return t("components.workspaceFileBrowser.announceItemsFound", {
      count: items.length,
  ...
  if (source === "other" && !companyId) {
    body = (
      <StateMessage
        icon={<FolderOpen aria-hidden="true" className="h-5 w-5 text-muted-foreground" />}
        title={t("components.workspaceFileBrowser.noCompanyTitle", { defaultValue: "No company selected" })}
        body={t("components.workspaceFileBrowser.noCompanyBody", { defaultValue: "Choose a company before browsing another project workspace." })}
      />
    );
    ...
    body = (
      <StateMessage
        ...
        title={t("components.workspaceFileBrowser.loadingProjectWorkspacesTitle", { defaultValue: "Loading project workspaces" })}
        body={t("components.workspaceFileBrowser.loadingProjectWorkspacesBody", { defaultValue: "Registered workspaces will appear here." })}
      />
    );
    ...
    body = (
      <StateMessage
        icon={<FolderOpen aria-hidden="true" className="h-5 w-5 text-muted-foreground" />}
        title={t("components.workspaceFileBrowser.noProjectWorkspacesTitle", { defaultValue: "No project workspaces" })}
        body={t("components.workspaceFileBrowser.noProjectWorkspacesBody", { defaultValue: "No same-company project has a registered workspace to browse." })}
      />
    );
    ...
    body = (
      <StateMessage
        icon={<AlertTriangle aria-hidden="true" className="h-5 w-5 text-amber-500" />}
        title={t("components.workspaceFileBrowser.loadFailedTitle", { defaultValue: "Couldn't load files" })}
        body={
          status === 404
            ? t("components.workspaceFileBrowser.loadFailed404Body", { defaultValue: "Workspace browsing isn't available for this issue." })
            : t("components.workspaceFileBrowser.loadFailedGenericBody", { defaultValue: "Something went wrong loading workspace files." })
        }
      />
    );
    ...
    body = (
      <StateMessage
        ...
        title={isSearch
          ? t("components.workspaceFileBrowser.noFilesMatchQuery", { query: q, defaultValue: "No files match “{{query}}”" })
          : t("components.workspaceFileBrowser.noRecentlyChanged", { defaultValue: "No recently changed files yet" })}
        body={t("components.workspaceFileBrowser.emptyStateBody", { defaultValue: "Try searching by name or path." })}
      />
    );
  } else {
    body = (
    ...
    <div className={cn("flex min-h-0 min-w-0 flex-col gap-2", className)}>
      <div className="relative min-w-0 max-w-full overflow-hidden">
        ...
        <Input
          ...
          placeholder={t("components.workspaceFileBrowser.searchPlaceholder", { defaultValue: "Search files by name or path…" })}
          aria-label={t("components.workspaceFileBrowser.searchAria", { defaultValue: "Search workspace files" })}
          role="combobox"
      ...
      {lazyTruncatedFolders.has(currentFolderKey) || (data?.truncated && !isLazyBrowse) ? (
        <div className="border-t border-border pt-2 text-[11px] text-muted-foreground">
          {isLazyBrowse ? (
            ...
            >
              {t("components.workspaceFileBrowser.loadMoreFromFolder", { defaultValue: "Load more from this folder" })}
            </button>
          ) : (
            <>{t("components.workspaceFileBrowser.showingFirst", { count: items.length, defaultValue: "Showing first {{count}} — refine the search to narrow." })}</>
          )}
        </div>
      ) : null}
    </div>

        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(totalInputTokens)}</span> {t("costs.in")}
              {" · "}
              <span className="font-mono">{formatTokens(totalOutputTokens)}</span> {t("costs.out")}
              {(totalApiRuns > 0 || totalSubRuns > 0) && (
                <span className="ml-1.5">
                  ·{" "}
                  {totalApiRuns > 0 && `~${totalApiRuns} ${t("costs.api")}`}
                  {totalApiRuns > 0 && totalSubRuns > 0 && " / "}
                  {totalSubRuns > 0 && `~${totalSubRuns} ${t("costs.sub")}`}
                  {" "} {t("costs.runs")}
                </span>
              )}
            </CardDescription>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        {hasBudget && (
          <div className="space-y-3">
            ...
            />
            <QuotaBar
              label={t("costs.thisWeek")}
              percentUsed={weekPct}
              leftLabel={formatCents(weekSpendCents)}
              rightLabel={`~${formatCents(Math.round(weeklyBudgetShare))} / ${t("costs.wk")}`}
        ...
        {totalSubRuns > 0 && (
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("costs.quota.subscription")}
              </p>
              <p className="text-xs text-muted-foreground">
                <span className="font-mono text-foreground">{totalSubRuns}</span> {t("costs.runs")}
                {" · "}
                {totalSubTokens > 0 && (
                  <>
                    <span className="font-mono text-foreground">{formatTokens(totalSubTokens)}</span> {t("costs.quota.total")}
                    {" · "}
                  </>
                )}
                <span className="font-mono text-foreground">{formatTokens(totalSubInputTokens)}</span> {t("costs.in")}
                {" · "}
                <span className="font-mono text-foreground">{formatTokens(totalSubOutputTokens)}</span> {t("costs.out")}
              </p>
              {subSharePct > 0 && (
                <>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("costs.quota.usageViaSubscription", { percent: Math.round(subSharePct), defaultValue: `${Math.round(subSharePct)}% of token usage via subscription` })}
                  </p>
                </>
              )}
            </div>
                  ...
                  <div key={`${row.provider}:${row.model}`} className="space-y-1.5">
                    ...
                    <div className="relative h-2 w-full border border-border overflow-hidden">
                      <div
                        ...
                        title={t("costs.quota.ofProviderTokens", { percent: Math.round(tokenPct), defaultValue: `${Math.round(tokenPct)}% of provider tokens` })}
                      />
                      ...
                      <div
                        ...
                        style={{ width: `${costPct}%` }}
                        title={t("costs.quota.ofProviderCost", { percent: Math.round(costPct), defaultValue: `${Math.round(costPct)}% of provider cost` })}
                      />
                    </div>
                  </div>
                );
                        ...
                        <div key={qw.label} className="space-y-1">
                          <div className="flex items-center justify-between gap-2 text-xs">
                            ...
                            {qw.valueLabel != null ? (
                              ...
                              <span className="font-medium tabular-nums">{t("costs.quota.used", { percent: qw.usedPercent, defaultValue: `${qw.usedPercent}% used` })}</span>
                            ) : null}
                          </div>
                          ...
                          {qw.detail ? (
                            ...
                            <p className="text-xs text-muted-foreground">
                              {t("costs.quota.resets", { date: new Date(qw.resetsAt).toLocaleDateString(undefined, { month: "short", day: "numeric" }), defaultValue: `resets ${new Date(qw.resetsAt).toLocaleDateString(undefined, { month: "short", day: "numeric" })}` })}
                            </p>
                          ) : null}
                        </div>
                      );

        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(totalInputTokens)}</span> {t("localizationActivity.tokensIn")}
              {" · "}
              <span className="font-mono">{formatTokens(totalOutputTokens)}</span> {t("localizationActivity.tokensOut")}
              {(totalApiRuns > 0 || totalSubRuns > 0) && (
                <span className="ml-1.5">
                  ·{" "}
                  {totalApiRuns > 0 && `~${t("localizationActivity.apiRuns", { count: totalApiRuns })}`}
                  {totalApiRuns > 0 && totalSubRuns > 0 && " / "}
                  {totalSubRuns > 0 && `~${t("localizationActivity.subscriptionRuns", { count: totalSubRuns })}`}
                </span>
              )}
            </CardDescription>
          </div>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        {hasBudget && (
          <div className="space-y-3">
            <QuotaBar
              label={t("localizationActivity.periodSpend")}
              percentUsed={budgetPct}
              leftLabel={formatCents(totalCostCents)}
              rightLabel={t("localizationActivity.ofAllocation", { percent: Math.round(budgetPct) })}
              showDeficitNotch={showDeficitNotch}
            />
            <QuotaBar
              label={t("localizationActivity.thisWeek")}
              percentUsed={weekPct}
              leftLabel={formatCents(weekSpendCents)}
              rightLabel={t("localizationActivity.weeklyAllocation", { amount: formatCents(Math.round(weeklyBudgetShare)) })}
          ...
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("localizationActivity.rollingWindows")}
              </p>
                  ...
                  return (
                    <div key={w} className="space-y-1">
                      <div className="flex items-center justify-between gap-2 text-xs">
                        <span className="font-mono text-muted-foreground w-6 shrink-0">{t(`localizationActivity.window_${w}`)}</span>
                        <span className="text-muted-foreground font-mono flex-1">
                          {t("localizationActivity.tokensShort", { value: formatTokens(tokens) })}
                        </span>
        ...
        {totalSubRuns > 0 && (
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("localizationActivity.subscription")}
              </p>
              <p className="text-xs text-muted-foreground">
                <span className="font-mono text-foreground">{t("localizationActivity.runs", { count: totalSubRuns })}</span>
                {" · "}
                {totalSubTokens > 0 && (
                  <>
                    <span className="font-mono text-foreground">{t("localizationActivity.tokensTotal", { value: formatTokens(totalSubTokens) })}</span>
                    {" · "}
                  </>
                )}
                <span className="font-mono text-foreground">{formatTokens(totalSubInputTokens)}</span> {t("localizationActivity.tokensIn")}
                {" · "}
                <span className="font-mono text-foreground">{formatTokens(totalSubOutputTokens)}</span> {t("localizationActivity.tokensOut")}
              </p>
              {subSharePct > 0 && (
                <>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("localizationActivity.subscriptionTokenShare", { percent: Math.round(subSharePct) })}
                  </p>
                </>
              )}
            </div>
                  ...
                  <div key={`${row.provider}:${row.model}`} className="space-y-1.5">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-3 shrink-0 tabular-nums text-xs">
                        <span className="text-muted-foreground">
                          {t("localizationActivity.tokensShort", { value: formatTokens(rowTokens) })}
                        </span>
                    ...
                    <div className="relative h-2 w-full border border-border overflow-hidden">
                      <div
                        ...
                        title={t("localizationActivity.providerTokenShare", { percent: Math.round(tokenPct) })}
                      />
                      ...
                      <div
                        ...
                        style={{ width: `${costPct}%` }}
                        title={t("localizationActivity.providerCostShare", { percent: Math.round(costPct) })}
                      />
                    </div>
                  </div>
                );
          ...
          <>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-3">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                  {t("localizationActivity.subscriptionQuota")}
                </p>
                        ...
                        <div key={qw.label} className="space-y-1">
                          <div className="flex items-center justify-between gap-2 text-xs">
                            ...
                            {qw.valueLabel != null ? (
                              ...
                              <span className="font-medium tabular-nums">{t("localizationActivity.percentUsed", { percent: qw.usedPercent })}</span>
                            ) : null}
                          </div>
                          ...
                          {qw.detail ? (
                            ...
                            <p className="text-xs text-muted-foreground">
                              {t("localizationActivity.quotaReset", { date: new Date(qw.resetsAt).toLocaleDateString(i18n.resolvedLanguage ?? i18n.language, { month: "short", day: "numeric" }) })}
                            </p>
                          ) : null}
                        </div>
                      );

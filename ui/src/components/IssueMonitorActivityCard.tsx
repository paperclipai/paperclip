    ...
    <div className="mb-3 rounded-lg border border-border bg-muted/30 px-3 py-2">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <div className="text-sm font-medium text-foreground">{t("component.issue.monitor.scheduled")}</div>
          <div className="text-xs text-muted-foreground">
            {t("component.issue.monitor.nextCheck")} {formatDateTime(monitor.nextCheckAt)} ({formatMonitorOffset(monitor.nextCheckAt)})
          </div>
          {monitor.notes ? (
          ...
          {monitor.attemptCount > 0 ? (
            <div className="mt-1 text-xs text-muted-foreground">{t("component.issue.monitor.attempt")} {monitor.attemptCount}</div>
          ) : null}
        </div>
        {onCheckNow ? (
          <Button
          ...
          >
            {checkingNow ? t("component.issue.monitor.checking") : t("component.issue.monitor.checkNow")}
          </Button>
        ) : null}
      </div>
    </div>

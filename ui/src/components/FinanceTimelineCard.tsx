...
export function FinanceTimelineCard({
  ...
  const finalEmptyMessage = emptyMessage ?? t("finance.timelineEmpty", { defaultValue: "No financial events in this period." });
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">{t("finance.recentEvents", { defaultValue: "Recent financial events" })}</CardTitle>
        <CardDescription>{t("finance.recentEventsDescription", { defaultValue: "Top-ups, fees, credits, commitments, and other non-request charges." })}</CardDescription>
      </CardHeader>
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 space-y-2">
                  ...
                  {(row.description || row.externalInvoiceId || row.region || row.pricingTier) && (
                    <div className="space-y-1 text-xs text-muted-foreground">
                      ...
                      {row.externalInvoiceId ? <div>{t("finance.invoice", { defaultValue: "invoice" })} {row.externalInvoiceId}</div> : null}
                      {row.region ? <div>{t("finance.region", { defaultValue: "region" })} {row.region}</div> : null}
                      {row.pricingTier ? <div>{t("finance.tier", { defaultValue: "tier" })} {row.pricingTier}</div> : null}
                    </div>
                  )}
                </div>
                <div className="text-right tabular-nums">
                  ...
                  {row.estimated ? <div className="text-[11px] uppercase tracking-[0.12em] text-amber-600">{t("finance.estimated", { defaultValue: "estimated" })}</div> : null}
                </div>
              </div>
            </div>
          ))

...
export function FinanceTimelineCard({
  ...
  const resolvedEmptyMessage = emptyMessage ?? t("page.costs.finance_timeline.empty");

  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">{t("page.costs.finance_timeline.title")}</CardTitle>
        <CardDescription>{t("page.costs.finance_timeline.description")}</CardDescription>
      </CardHeader>
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 space-y-2">
                  ...
                  {(row.description || row.externalInvoiceId || row.region || row.pricingTier) && (
                    <div className="space-y-1 text-xs text-muted-foreground">
                      ...
                      {row.externalInvoiceId ? <div>{t("page.costs.finance_timeline.invoice", { value: row.externalInvoiceId })}</div> : null}
                      {row.region ? <div>{t("page.costs.finance_timeline.region", { value: row.region })}</div> : null}
                      {row.pricingTier ? <div>{t("page.costs.finance_timeline.tier", { value: row.pricingTier })}</div> : null}
                    </div>
                  )}
                </div>
                <div className="text-right tabular-nums">
                  ...
                  {row.estimated ? <div className="text-[11px] uppercase tracking-[0.12em] text-amber-600">{t("page.costs.finance_timeline.estimated")}</div> : null}
                </div>
              </div>
            </div>
          ))

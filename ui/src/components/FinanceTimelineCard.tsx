...
export function FinanceTimelineCard({
  ...
  const { t } = useI18n();
  const resolvedEmptyMessage = emptyMessage ?? t("finance_timeline.empty");
  return (
    <Card>
                ...
                <div className="min-w-0 space-y-2">
                  ...
                  {(row.description || row.externalInvoiceId || row.region || row.pricingTier) && (
                    <div className="space-y-1 text-xs text-muted-foreground">
                      ...
                      {row.externalInvoiceId ? <div>{t("finance_timeline.invoice", { value: row.externalInvoiceId })}</div> : null}
                      {row.region ? <div>{t("finance_timeline.region", { value: row.region })}</div> : null}
                      {row.pricingTier ? <div>{t("finance_timeline.tier", { value: row.pricingTier })}</div> : null}
                    </div>
                  )}
                </div>

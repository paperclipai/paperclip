    ...
    <CommandDialog open={open} onOpenChange={(v) => {
      ...
      <CommandList>
        <CommandEmpty>
          {showSearchAll ? (
            <span>
              {t("command.noQuickTaskMatches")}{" "}
              <kbd className="rounded border border-border bg-muted px-1 py-0.5 text-(length:--text-nano)">↵</kbd>{" "}
              {t("command.to")} <span className="font-medium">{t("command.searchAll")}</span>{" "}
              {t("command.orKeepTyping")}
            </span>
          ) : (
            t("command.noResults")
          )}
        ...
        {showSearchAll ? (
          <CommandGroup heading={t("command.search")}>
            <CommandItem
        ...
        <CommandGroup heading={t("command.quickFilters")}>

    ...
    >
      ...
      <CommandList>
        <CommandEmpty>
          {showSearchAll ? (
            <span>
              {t('commandPalette.noQuickMatches', { interpolation: { escapeValue: false } }).split('↵').map((part, i, arr) => (
                <Fragment key={i}>
                  {part}
                  {i < arr.length - 1 && <kbd className="rounded border border-border bg-muted px-1 py-0.5 text-[10px]">↵</kbd>}
                </Fragment>
              ))}
            </span>
          ) : (
          ...
          <CommandGroup heading={t('commandPalette.searchGroup')}>
            ...
            >
              ...
              <span className="flex-1 truncate">
                {t('commandPalette.searchAllFor', { query: searchQuery })}
              </span>

...
function getPipelineRevisionActor(
  ...
  return { kind: "system", name: t("localizationDocuments.system") };
}
...
export function PipelineItemBodyDocument({
  ...
  const restoreMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await invalidateAll();
      pushToast({ get ["title"]() { return t("localizationDocuments.ui_Revision_restored"); }, tone: "success" });
    },
    onError: () => pushToast({ get ["title"]() { return t("localizationDocuments.ui_Could_not_restore_the_revision"); }, tone: "error" }),
  });
  ...
  const handleSave = useCallback(async () => {
    try {
      ...
      if (error instanceof ApiError && error.status === 409) {
        ...
        pushToast({
          get ["title"]() { return t("localizationDocuments.ui_Body_changed_elsewhere"); },
          get ["body"]() { return t("localizationDocuments.ui_This_item_body_was_updated_by_someone_else_Reloaded_the_latest_re_apply_your_edit_"); },
          tone: "error",
        });
        return;
      }
      pushToast({ get ["title"]() { return t("localizationDocuments.ui_Could_not_save_the_body"); }, tone: "error" });
    }
  ...
  const handleStartConversationFromAnchor = useCallback(
    async (anchor: PendingAnchor) => {
      ...
      try {
        ...
        setPendingStartAnchor(null);
        pushToast({ get ["title"]() { return t("localizationDocuments.ui_Could_not_start_the_conversation"); }, tone: "error" });
      }
    },
  ...
  if (editing) {
      ...
      >
        <div className="rounded-md border border-border bg-background">
          <MarkdownEditor
            ...
            placeholder={t("localizationDocuments.ui_Write_the_item_body_in_Markdown_")}
            bordered={false}
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
          <span className="text-(length:--text-micro) text-muted-foreground">
            {t("localizationDocuments.savingCreatesRevision", { number: (doc?.latestRevisionNumber ?? 0) + 1 })}
          </span>
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" onClick={cancelEdit} disabled={saveMutation.isPending}>{t("localizationDocuments.ui_Cancel")}</Button>
            <Button size="sm" onClick={() => void handleSave()} disabled={saveMutation.isPending}>
              ...
              {saveMutation.isPending ? t("localizationDocuments.ui_Saving_") : t("localizationDocuments.ui_Save")}
            </Button>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-1">
              <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
                {t("localizationDocuments.viewingRevision", { number: selectedHistoricalRevision.revisionNumber })}
              </p>
              <p className="text-xs text-muted-foreground">{t("localizationDocuments.ui_Historical_preview_New_comments_are_disabled_while_previewing_a_historical_revision_Restoring_it")}</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => setSelectedRevisionId(null)}>{t("localizationTaskRuntime.ui_Return_to_latest_1qcx9un")}</Button>
              <Button
                size="sm"
                onClick={() => restoreMutation.mutate(selectedHistoricalRevision.id)}
                disabled={restoreMutation.isPending}
              >
                {restoreMutation.isPending ? t("localizationRoutines.restoring") : t("localizationRoutineHistory.restoreThis")}
              </Button>
            </div>
          </div>
        </div>
    ...
    // Truly empty (A).
    bodyContent = (
      <EmptyState icon={FileText} message={t("localizationDocuments.ui_No_body_yet_Capture_the_item_s_details_here_")} action={t("localizationDocuments.addItemBody")} onAction={beginEdit} />
    );
  ...
  return (
    <section
      aria-label={t("localizationOperations.ui_Item_body")}
    ...
    >
      <DocumentFrameHeader
        ...
        documentLabel={t("localizationDocuments.itemBodyDocument")}
        folded={folded}
        ...
        actionsSlot={editing ? (
          <span className="text-(length:--text-micro) font-medium text-amber-700 dark:text-amber-300">{t("localizationDocuments.ui__Editing_unsaved")}</span>
        ) : (
          <Button variant="ghost" size="sm" className="h-auto gap-1.5 px-2 py-1 text-xs" onClick={beginEdit}>
            <FilePenLine className="h-3.5 w-3.5" />{t("localizationDocuments.ui_Edit")}</Button>
        )}
      />

    ...
    <div className="rounded-xl border border-border/70 bg-card p-4 shadow-sm">
            ...
            <div className="min-w-0 flex-1 space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {requesterAgent && (
                  <div className="inline-flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground">
                    <span>{t("component.approval.requestedBy")}</span>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs leading-5 text-muted-foreground">
                  {t("component.approval.approvalRequestCreated", { timeAgo: timeAgo(approval.createdAt) })}
                </p>
              </div>
            </div>
          </div>
      ...
      {approval.decisionNote && (
        <div className="mt-4 rounded-lg border border-border/60 bg-muted/30 px-3.5 py-3 text-xs leading-5 text-muted-foreground">
          <span className="font-medium text-foreground">{t("component.approval.decisionNote")}.</span> {approval.decisionNote}
        </div>
      )}
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-border/60 pt-4">
            ...
            {showResolutionButtons && (
              <>
                <Button
                  ...
                  disabled={isPending}
                >
                  {pendingAction === "approve" ? t("component.approval.approving") : t("component.approval.approve")}
                </Button>
                <Button
                  variant="destructive"
                  size="sm"
                  ...
                  disabled={isPending}
                >
                  {pendingAction === "reject" ? t("component.approval.rejecting") : t("component.approval.reject")}
                </Button>
              </>
            )}
          </div>
          {(detailLink || onOpen) ? (
            detailLink ? (
              ...
              >
                {t("component.approval.viewDetails")}
              </Link>
            ) : (

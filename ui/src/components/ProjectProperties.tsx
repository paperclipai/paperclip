  ...
  if (state === "saving") {
    return (
      <span className="inline-flex items-center gap-1 text-(length:--text-micro) text-muted-foreground">
        ...
        {t("app.projectProperties.saving", { defaultValue: "Saving" })}
      </span>
    );
  }
  if (state === "saved") {
    return (
      <span className="inline-flex items-center gap-1 text-(length:--text-micro) text-green-600 dark:text-green-400">
        ...
        {t("app.projectProperties.saved", { defaultValue: "Saved" })}
      </span>
    );
  }
  if (state === "error") {
    return (
      <span className="inline-flex items-center gap-1 text-(length:--text-micro) text-destructive">
        ...
        {t("app.projectProperties.failed", { defaultValue: "Failed" })}
      </span>
    );
  }
  return null;
...
function ArchiveDangerZone({
  ...
  const action = isArchive ? t("common.archive", { defaultValue: "Archive" }) : t("app.projectProperties.unarchive", { defaultValue: "Unarchive" });

  return (
    <div className="space-y-3 rounded-md border border-destructive/40 bg-destructive/5 px-4 py-4">
      <p className="text-sm text-muted-foreground">
        {isArchive
          ? t("app.projectProperties.archiveThisProjectToHideItFromTheSidebarAndProjectSelectors", { defaultValue: "Archive this project to hide it from the sidebar and project selectors." })
          : t("app.projectProperties.unarchiveThisProjectToRestoreItInTheSidebarAndProjectSelectors", { defaultValue: "Unarchive this project to restore it in the sidebar and project selectors." })}
      </p>
      {archivePending ? (
        <Button size="sm" variant="destructive" disabled>
          <Loader2 className="h-3 w-3 animate-spin mr-1" />
          {isArchive ? t("app.projectProperties.archiving", { defaultValue: "Archiving..." }) : t("app.projectProperties.unarchiving", { defaultValue: "Unarchiving..." })}
        </Button>
      ) : confirming ? (
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const clearLocalWorkspace = () => {
    const confirmed = window.confirm(
      codebase.repoUrl
        ? t("app.projectProperties.clearLocalFolderFromThisWorkspace", { defaultValue: "Clear local folder from this workspace?" })
        : t("app.projectProperties.deleteThisWorkspaceLocalFolder", { defaultValue: "Delete this workspace local folder?" }),
    );
    if (!confirmed) return;
    persistCodebase({ cwd: null });
  };
  ...
  const clearRepoWorkspace = () => {
    ...
    const confirmed = window.confirm(
      hasLocalFolder
        ? t("app.projectProperties.clearRepoFromThisWorkspace", { defaultValue: "Clear repo from this workspace?" })
        : t("app.projectProperties.deleteThisWorkspaceRepo", { defaultValue: "Delete this workspace repo?" }),
    );
    if (!confirmed) return;
  ...
  return (
    <div>
      <div className="space-y-1 pb-4">
        <PropertyRow label={<FieldLabel label={t("app.projectProperties.name", { defaultValue: "Name" })} state={fieldState("name")} />}>
          {onUpdate || onFieldUpdate ? (
            <DraftInput
              ...
              placeholder={t("app.projectProperties.projectName", { defaultValue: "Project name" })}
            />
          ) : (
        ...
        <PropertyRow
          label={<FieldLabel label={t("app.projectProperties.description", { defaultValue: "Description" })} state={fieldState("description")} />}
          alignStart
        ...
        >
          {onUpdate || onFieldUpdate ? (
            <InlineEditor
              ...
              placeholder={t("app.projectProperties.addADescription", { defaultValue: "Add a description..." })}
              multiline
            />
          ) : (
            <p className="text-sm text-muted-foreground">
              {project.description?.trim() || t("app.projectProperties.noDescription", { defaultValue: "No description" })}
            </p>
          )}
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("app.projectProperties.status", { defaultValue: "Status" })} state={fieldState("status")} />}>
          {onUpdate || onFieldUpdate ? (
            <ProjectStatusPicker
        ...
        {project.leadAgentId && (
          <PropertyRow label={t("app.projectProperties.lead", { defaultValue: "Lead" })}>
            <span className="text-sm font-mono">{project.leadAgentId.slice(0, 8)}</span>
          </PropertyRow>
        )}
        <PropertyRow
          label={<FieldLabel label={t("app.projectProperties.goals", { defaultValue: "Goals" })} state={fieldState("goals")} />}
          alignStart
            ...
            <Popover open={goalOpen} onOpenChange={setGoalOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  ...
                  {t("app.projectProperties.goal", { defaultValue: "Goal" })}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-56 p-1" align="start">
                {availableGoals.length === 0 ? (
                  <div className="px-2 py-1.5 text-xs text-muted-foreground">
                    {t("app.projectProperties.allGoalsLinked", { defaultValue: "All goals linked." })}
                  </div>
                ) : (
        ...
        <PropertyRow
          label={<FieldLabel label={t("app.projectProperties.env", { defaultValue: "Env" })} state={fieldState("env")} />}
          alignStart
          valueClassName="space-y-2"
        >
          <div className="space-y-2">
            ...
            <p className="text-(length:--text-micro) text-muted-foreground">
              {t("app.projectProperties.appliedToAllRunsForTasksInThisProjectProjectValuesOverrideAgentEnvOnKeyConflicts", { defaultValue: "Applied to all runs for tasks in this project. Project values override agent env on key conflicts." })}
            </p>
          </div>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("app.projectProperties.created", { defaultValue: "Created" })} state="idle" />}>
          <span className="text-sm">{formatDate(project.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("app.projectProperties.updated", { defaultValue: "Updated" })} state="idle" />}>
          <span className="text-sm">{formatDate(project.updatedAt)}</span>
        </PropertyRow>
        {project.targetDate && (
          <PropertyRow label={<FieldLabel label={t("app.projectProperties.targetDate", { defaultValue: "Target Date" })} state="idle" />}>
      ...
      <div className="space-y-1 py-4">
        <div className="space-y-2">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <span>{t("app.projectProperties.codebase", { defaultValue: "Codebase" })}</span>
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  aria-label={t("app.projectProperties.codebaseHelp", { defaultValue: "Codebase help" })}
                >
                  ?
                </button>
              </TooltipTrigger>
              <TooltipContent side="top">
                {t("app.projectProperties.repoIdentifiesTheSourceOfTruthLocalFolderIsTheDefaultPlaceAgentsWriteCode", { defaultValue: "Repo identifies the source of truth. Local folder is the default place agents write code." })}
              </TooltipContent>
            </Tooltip>
          </div>
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            <div className="space-y-1">
              <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">{t("app.projectProperties.repo", { defaultValue: "Repo" })}</div>
              {codebase.repoUrl ? (
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    >
                      {t("app.projectProperties.changeRepo", { defaultValue: "Change repo" })}
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon-xs"
                      onClick={clearRepoWorkspace}
                      aria-label={t("app.projectProperties.clearRepo", { defaultValue: "Clear repo" })}
                    >
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs text-muted-foreground">{t("app.projectProperties.notSet", { defaultValue: "Not set." })}</div>
                  <Button
                    variant="outline"
                    size="xs"
                    className="h-6 px-2"
                  ...
                  >
                    {t("app.projectProperties.setRepo", { defaultValue: "Set repo" })}
                  </Button>
                </div>
              )}
            </div>
            ...
            <div className="space-y-1">
              <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">{t("app.projectProperties.localFolder", { defaultValue: "Local folder" })}</div>
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 space-y-1">
                  ...
                  {codebase.origin === "managed_checkout" && (
                    <div className="text-(length:--text-micro) text-muted-foreground">{t("app.projectProperties.paperclipManagedFolder", { defaultValue: "Paperclip-managed folder." })}</div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  <Button
                  ...
                  >
                    {codebase.localFolder ? t("app.projectProperties.changeLocalFolder", { defaultValue: "Change local folder" }) : t("app.projectProperties.setLocalFolder", { defaultValue: "Set local folder" })}
                  </Button>
                  {codebase.localFolder ? (
                    <Button
                      ...
                      aria-label={t("app.projectProperties.clearLocalFolder", { defaultValue: "Clear local folder" })}
                    >
                      <Trash2 className="h-3 w-3" />
                    </Button>
                  ) : null}
            ...
            {hasAdditionalLegacyWorkspaces && (
              <div className="text-(length:--text-micro) text-muted-foreground">
                {t("app.projectProperties.additionalLegacyWorkspaceRecordsExistOnThisProjectPaperclipIsUsingThePrimaryWorkspaceAsTheCodebaseView", { defaultValue: "Additional legacy workspace records exist on this project. Paperclip is using the primary workspace as the codebase view." })}
              </div>
            )}
                    ...
                    <div className="min-w-0 space-y-0.5">
                      ...
                      <div className="text-(length:--text-micro) text-muted-foreground">
                        {service.url ? (
                          ...
                          service.command ?? t("app.projectProperties.noUrl", { defaultValue: "No URL" })
                        )}
                      </div>
                      {service.exposure && service.exposure.state !== "removed" ? (
                        ...
                        >
                          {t("app.projectProperties.https", { defaultValue: "HTTPS" })} {service.exposure.state.replace("_", " ")}
                        </div>
                      ) : null}
                    </div>
          ...
          {createWorkspace.isError && (
            <p className="text-xs text-destructive">{t("app.projectProperties.failedToSaveWorkspace", { defaultValue: "Failed to save workspace." })}</p>
          )}
          {removeWorkspace.isError && (
            <p className="text-xs text-destructive">{t("app.projectProperties.failedToDeleteWorkspace", { defaultValue: "Failed to delete workspace." })}</p>
          )}
          {updateWorkspace.isError && (
            <p className="text-xs text-destructive">{t("app.projectProperties.failedToUpdateWorkspace", { defaultValue: "Failed to update workspace." })}</p>
          )}
        </div>
        ...
        {isolatedWorkspacesEnabled ? (
          <>
            ...
            <div className="py-1.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <span>{t("app.projectProperties.executionWorkspaces", { defaultValue: "Execution Workspaces" })}</span>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-(length:--text-nano) text-muted-foreground hover:text-foreground"
                      aria-label={t("app.projectProperties.executionWorkspacesHelp", { defaultValue: "Execution workspaces help" })}
                    >
                      ?
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="top">
                    {t("app.projectProperties.projectOwnedDefaultsForIsolatedTaskCheckoutsAndExecutionWorkspaceBehavior", { defaultValue: "Project-owned defaults for isolated task checkouts and execution workspace behavior." })}
                  </TooltipContent>
                </Tooltip>
              </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <span>{t("app.projectProperties.enableIsolatedTaskCheckouts", { defaultValue: "Enable isolated task checkouts" })}</span>
                      <SaveIndicator state={fieldState("execution_workspace_enabled")} />
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {t("app.projectProperties.letTasksChooseBetweenTheProjectSPrimaryCheckoutAndAnIsolatedExecutionWorkspace", { defaultValue: "Let tasks choose between the project's primary checkout and an isolated execution workspace." })}
                    </div>
                  </div>
                  {onUpdate || onFieldUpdate ? (
                    <ToggleSwitch
                    ...
                    <span className="text-xs text-muted-foreground">
                      {executionWorkspacesEnabled ? t("app.projectProperties.enabled", { defaultValue: "Enabled" }) : t("app.projectProperties.disabled", { defaultValue: "Disabled" })}
                    </span>
                  )}
                </div>
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2 text-sm">
                          <span>{t("app.projectProperties.newTasksDefaultToIsolatedCheckout", { defaultValue: "New tasks default to isolated checkout" })}</span>
                          <SaveIndicator state={fieldState("execution_workspace_default_mode")} />
                        </div>
                        <div className="text-(length:--text-micro) text-muted-foreground">
                          {t("app.projectProperties.ifDisabledNewTasksStayOnTheProjectSPrimaryCheckoutUnlessSomeoneOptsIn", { defaultValue: "If disabled, new tasks stay on the project's primary checkout unless someone opts in." })}
                        </div>
                      </div>
                      <ToggleSwitch
                    ...
                    <div className="space-y-0.5">
                      <div className="mb-1 flex items-center gap-1.5">
                        <label className="flex items-center gap-2 text-sm">
                          <span>{t("app.projectProperties.sharedWorkspaceConcurrency", { defaultValue: "Shared workspace concurrency" })}</span>
                      ...
                      {onUpdate || onFieldUpdate ? (
                        <select
                          className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs outline-none"
                          aria-label={t("app.projectProperties.sharedWorkspaceConcurrency", { defaultValue: "Shared workspace concurrency" })}
                    ...
                    <div className="border-t border-border/60 pt-2">
                      ...
                      >
                        {executionWorkspaceAdvancedOpen
                          ? t("app.projectProperties.hideAdvancedCheckoutSettings", { defaultValue: "Hide advanced checkout settings" })
                          : t("app.projectProperties.showAdvancedCheckoutSettings", { defaultValue: "Show advanced checkout settings" })}
                      </button>
                    </div>
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          {t("app.projectProperties.hostManagedImplementation", { defaultValue: "Host-managed implementation:" })} <span className="text-foreground">{t("app.projectProperties.gitWorktree", { defaultValue: "Git worktree" })}</span>
                        </div>
                        {showExecutionWorkspaceEnvironmentControl ? (
                          <div>
                            <div className="mb-1 flex items-center gap-1.5">
                              <label className="flex items-center gap-2 text-xs text-muted-foreground">
                                <span>{t("app.projectProperties.environment", { defaultValue: "Environment" })}</span>
                            ...
                            >
                              <option value="">{t("app.projectProperties.noEnvironment", { defaultValue: "No environment" })}</option>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("app.projectProperties.baseRef", { defaultValue: "Base ref" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("app.projectProperties.branchTemplate", { defaultValue: "Branch template" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("app.projectProperties.worktreeParentDir", { defaultValue: "Worktree parent dir" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("app.projectProperties.provisionCommand", { defaultValue: "Provision command" })}</span>
                          ...
                          <DraftInput
                            ...
                            immediate
                            className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                            placeholder={t("app.projectProperties.bashScriptsProvisionWorktreeSh", { defaultValue: "bash ./scripts/provision-worktree.sh" })}
                          />
                        </div>
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("app.projectProperties.runtimeProvisionCommand", { defaultValue: "Runtime provision command" })}</span>
                          ...
                          <DraftInput
                            ...
                            immediate
                            className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                            placeholder={t("app.projectProperties.bashScriptsProvisionWorktreeRuntimeSh", { defaultValue: "bash ./scripts/provision-worktree-runtime.sh" })}
                          />
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("app.projectProperties.teardownCommand", { defaultValue: "Teardown command" })}</span>
                          ...
                          <DraftInput
                            ...
                            immediate
                            className="w-full rounded border border-border bg-transparent px-2 py-1 text-xs font-mono outline-none"
                            placeholder={t("app.projectProperties.bashScriptsTeardownWorktreeSh", { defaultValue: "bash ./scripts/teardown-worktree.sh" })}
                          />
                        </div>
                        <p className="text-(length:--text-micro) text-muted-foreground">
                          {t("app.projectProperties.provisionRunsInsideTheDerivedWorktreeBeforeAgentExecutionTeardownIsStoredHereForFutureCleanupFlows", { defaultValue: "Provision runs inside the derived worktree before agent execution. Teardown is stored here for\n                          future cleanup flows." })}
                        </p>
                      </div>
                    ) : null}
                  </div>
        ...
        <>
          ...
          <div className="space-y-4 py-4">
            <div className="text-xs font-medium text-destructive uppercase tracking-wide">
              {t("app.projectProperties.dangerZone", { defaultValue: "Danger Zone" })}
            </div>
            <ArchiveDangerZone
              project={project}
              onArchive={onArchive}

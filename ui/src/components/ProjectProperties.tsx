...
function ArchiveDangerZone({
  ...
  const action = isArchive ? t("project_props.archive") : t("project_props.unarchive");

  return (
    <div className="space-y-3 rounded-md border border-destructive/40 bg-destructive/5 px-4 py-4">
      <p className="text-sm text-muted-foreground">
        {isArchive
          ? t("project_props.archive_hint")
          : t("project_props.unarchive_hint")}
      </p>
      {archivePending ? (
        <Button size="sm" variant="destructive" disabled>
          <Loader2 className="h-3 w-3 animate-spin mr-1" />
          {isArchive ? t("project_props.archiving") : t("project_props.unarchiving")}
        </Button>
      ) : confirming ? (
        ...
        >
          {isArchive ? (
            <><Archive className="h-3 w-3 mr-1" />{t("project_props.archive_project", { action })}</>
          ) : (
            <><ArchiveRestore className="h-3 w-3 mr-1" />{t("project_props.archive_project", { action })}</>
          )}
        </Button>
      )}
    </div>
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const clearLocalWorkspace = () => {
    const confirmed = window.confirm(
      codebase.repoUrl
        ? t("project_props.clear_local_folder")
        : t("project_props.delete_local_folder"),
    );
    if (!confirmed) return;
    persistCodebase({ cwd: null });
  };
  ...
  const clearRepoWorkspace = () => {
    ...
    const confirmed = window.confirm(
      hasLocalFolder
        ? t("project_props.clear_repo")
        : t("project_props.delete_repo"),
    );
    if (!confirmed) return;
  ...
  return (
    <div>
      <div className="space-y-1 pb-4">
        <PropertyRow label={<FieldLabel label={t("project_props.name")} state={fieldState("name")} />}>
          {onUpdate || onFieldUpdate ? (
            <DraftInput
              ...
              placeholder={t("project_props.name")}
            />
          ) : (
        ...
        <PropertyRow
          label={<FieldLabel label={t("project_props.description")} state={fieldState("description")} />}
          alignStart
        ...
        >
          {onUpdate || onFieldUpdate ? (
            <InlineEditor
              ...
              placeholder={t("project_props.add_description")}
              multiline
            />
          ) : (
            <p className="text-sm text-muted-foreground">
              {project.description?.trim() || t("project_props.no_description")}
            </p>
          )}
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("project_props.status")} state={fieldState("status")} />}>
          {onUpdate || onFieldUpdate ? (
            <ProjectStatusPicker
        ...
        {project.leadAgentId && (
          <PropertyRow label={t("project_props.lead")}>
            <span className="text-sm font-mono">{project.leadAgentId.slice(0, 8)}</span>
          </PropertyRow>
        )}
        <PropertyRow
          label={<FieldLabel label={t("project_props.goals")} state={fieldState("goals")} />}
          alignStart
              ...
              <PopoverContent className="w-56 p-1" align="start">
                {availableGoals.length === 0 ? (
                  <div className="px-2 py-1.5 text-xs text-muted-foreground">
                    {t("project_props.all_goals_linked")}
                  </div>
                ) : (
        ...
        <PropertyRow
          label={<FieldLabel label={t("project_props.env")} state={fieldState("env")} />}
          alignStart
          valueClassName="space-y-2"
        >
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-muted-foreground">
              {t("project_props.env_hint")}
            </p>
          </div>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("project_props.created")} state="idle" />}>
          <span className="text-sm">{formatDate(project.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("project_props.updated")} state="idle" />}>
          <span className="text-sm">{formatDate(project.updatedAt)}</span>
        </PropertyRow>
        {project.targetDate && (
          <PropertyRow label={<FieldLabel label={t("project_props.target_date")} state="idle" />}>
      ...
      <div className="space-y-1 py-4">
        <div className="space-y-2">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <span>{t("project_props.codebase")}</span>
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
                  className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-[10px] text-muted-foreground hover:text-foreground"
                  aria-label={t("project_props.codebase_help")}
                >
                  ?
                </button>
              </TooltipTrigger>
              <TooltipContent side="top">
                {t("project_props.codebase_tooltip")}
              </TooltipContent>
            </Tooltip>
          </div>
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("project_props.repo")}</div>
              {codebase.repoUrl ? (
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    >
                      {t("project_props.change_repo")}
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon-xs"
                      onClick={clearRepoWorkspace}
                      aria-label={t("project_props.clear_repo_aria")}
                    >
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs text-muted-foreground">{t("project_props.not_set")}</div>
                  <Button
                    variant="outline"
                    size="xs"
                    className="h-6 px-2"
            ...
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("project_props.local_folder")}</div>
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 space-y-1">
                  ...
                  {codebase.origin === "managed_checkout" && (
                    <div className="text-[11px] text-muted-foreground">{t("project_props.managed_folder")}</div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  <Button
                  ...
                  >
                    {codebase.localFolder ? t("project_props.change_local_folder") : t("project_props.set_local_folder")}
                  </Button>
                  {codebase.localFolder ? (
                    <Button
                      ...
                      aria-label={t("project_props.clear_local_folder_aria")}
                    >
                      <Trash2 className="h-3 w-3" />
                    </Button>
                  ) : null}
            ...
            {hasAdditionalLegacyWorkspaces && (
              <div className="text-[11px] text-muted-foreground">
                {t("project_props.legacy_workspaces_hint")}
              </div>
            )}
                    ...
                    <div className="min-w-0 space-y-0.5">
                      ...
                      <div className="text-[11px] text-muted-foreground">
                        {service.url ? (
                          ...
                          service.command ?? t("project_props.no_url")
                        )}
                      </div>
                    </div>
          ...
          {createWorkspace.isError && (
            <p className="text-xs text-destructive">{t("project_props.save_workspace_failed")}</p>
          )}
          {removeWorkspace.isError && (
            <p className="text-xs text-destructive">{t("project_props.delete_workspace_failed")}</p>
          )}
          {updateWorkspace.isError && (
            <p className="text-xs text-destructive">{t("project_props.update_workspace_failed")}</p>
          )}
        </div>
        ...
        {isolatedWorkspacesEnabled ? (
          <>
            ...
            <div className="py-1.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <span>{t("project_props.execution_workspaces")}</span>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-[10px] text-muted-foreground hover:text-foreground"
                      aria-label={t("project_props.execution_workspaces_help")}
                    >
                      ?
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="top">
                    {t("project_props.execution_workspaces_tooltip")}
                  </TooltipContent>
                </Tooltip>
              </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <span>{t("project_props.enable_isolated_checkouts")}</span>
                      <SaveIndicator state={fieldState("execution_workspace_enabled")} />
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {t("project_props.enable_isolated_checkouts_hint")}
                    </div>
                  </div>
                  {onUpdate || onFieldUpdate ? (
                    <ToggleSwitch
                    ...
                    <span className="text-xs text-muted-foreground">
                      {executionWorkspacesEnabled ? t("project_props.enabled") : t("project_props.disabled")}
                    </span>
                  )}
                </div>
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2 text-sm">
                          <span>{t("project_props.new_issues_default_isolated")}</span>
                          <SaveIndicator state={fieldState("execution_workspace_default_mode")} />
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          {t("project_props.new_issues_default_isolated_hint")}
                        </div>
                      </div>
                      <ToggleSwitch
                    ...
                    <div className="border-t border-border/60 pt-2">
                      ...
                      >
                        {executionWorkspaceAdvancedOpen
                          ? t("project_props.hide_advanced_checkout")
                          : t("project_props.show_advanced_checkout")}
                      </button>
                    </div>
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          {t("project_props.host_managed_impl")} <span className="text-foreground">{t("project_props.git_worktree")}</span>
                        </div>
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("project_props.base_ref")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("project_props.branch_template")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("project_props.worktree_parent_dir")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("project_props.provision_command")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("project_props.teardown_command")}</span>
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          {t("project_props.provision_teardown_hint")}
                        </p>
                      </div>
                    ) : null}
                  </div>

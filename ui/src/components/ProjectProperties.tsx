...
// PROJECT_STATUSES moved inside ProjectStatusPicker to support i18n t()
  ...
  if (state === "saved") {
    return (
      <span className="inline-flex items-center gap-1 text-[11px] text-green-600 dark:text-green-400">
        ...
        {t('agentDetail.status_succeeded')}
      </span>
    );
  }
  if (state === "error") {
...
function ProjectStatusPicker({ status, onChange }: { status: string; onChange: (status: string) => void }) {
  ...
  const PROJECT_STATUSES = [
    { value: "backlog", label: t("projects.statusValue.backlog") },
    { value: "planned", label: t("projects.statusValue.planned") },
    { value: "in_progress", label: t("projects.statusValue.in_progress") },
    { value: "completed", label: t("projects.statusValue.completed") },
    { value: "cancelled", label: t("projects.statusValue.cancelled") },
  ];
    ...
    <div className="space-y-3 rounded-md border border-destructive/40 bg-destructive/5 px-4 py-4">
      <p className="text-sm text-muted-foreground">
        {isArchive
          ...
          : t("projects.unarchiveDesc")}
      </p>
      {archivePending ? (
        <Button size="sm" variant="destructive" disabled>
          <Loader2 className="h-3 w-3 animate-spin mr-1" />
          {isArchive ? t("projects.archiving") : t("projects.unarchiving")}
        </Button>
      ) : confirming ? (
        <div className="flex items-center gap-2">
          <span className="text-sm text-destructive font-medium">
            {t("projects.confirmAction", { action, name: project.name })}
          </span>
          <Button
            size="sm"
            variant="destructive"
        ...
        >
          {isArchive ? (
            <><Archive className="h-3 w-3 mr-1" />{t("projects.archiveProject")}</>
          ) : (
            <><ArchiveRestore className="h-3 w-3 mr-1" />{t("projects.unarchiveProject")}</>
          )}
        </Button>
      )}
    </div>
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const submitLocalWorkspace = () => {
    ...
    if (!isAbsolutePath(cwd)) {
      setWorkspaceError(t("projects.localFolderAbsolute"));
      return;
    }
    setWorkspaceError(null);
    persistCodebase({ cwd });
  ...
  return (
    <div>
      <div className="space-y-1 pb-4">
        <PropertyRow label={<FieldLabel label={t("projects.name")} state={fieldState("name")} />}>
        ...
        <PropertyRow
          label={<FieldLabel label={t("projects.description")} state={fieldState("description")} />}
          alignStart
        ...
        >
          {onUpdate || onFieldUpdate ? (
            ...
            <p className="text-sm text-muted-foreground">
              {project.description?.trim() || t("common.noResults")}
            </p>
          )}
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("projects.status")} state={fieldState("status")} />}>
          {onUpdate || onFieldUpdate ? (
            <ProjectStatusPicker
        ...
        {project.leadAgentId && (
          <PropertyRow label={t('agentDetail.agent')}>
            <span className="text-sm font-mono">{project.leadAgentId.slice(0, 8)}</span>
          </PropertyRow>
        )}
        <PropertyRow
          label={<FieldLabel label={t("projects.goals")} state={fieldState("goals")} />}
          alignStart
        ...
        <PropertyRow
          label={<FieldLabel label={t("projects.env")} state={fieldState("env")} />}
          alignStart
          valueClassName="space-y-2"
        >
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-muted-foreground">
              {t("projects.envHelp2")} Project values override agent env on key conflicts.
            </p>
          </div>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("projects.created")} state="idle" />}>
          <span className="text-sm">{formatDate(project.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("projects.updated")} state="idle" />}>
          <span className="text-sm">{formatDate(project.updatedAt)}</span>
        </PropertyRow>
        {project.targetDate && (
          <PropertyRow label={<FieldLabel label={t("projects.targetDate")} state="idle" />}>
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t('projects.repo')}</div>
              {codebase.repoUrl ? (
            ...
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t('projects.localFolder')}</div>
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 space-y-1">
                  ...
                  {codebase.origin === "managed_checkout" && (
                    <div className="text-[11px] text-muted-foreground">{t('projects.paperclipManagedFolder')}</div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  <Button
                  ...
                  >
                    {codebase.localFolder ? t("projects.changeLocalFolder") : t("projects.setLocalFolder")}
                  </Button>

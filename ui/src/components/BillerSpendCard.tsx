    ...
    <Card>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(row.inputTokens + row.cachedInputTokens)}</span> {t("localizationActivity.tokensIn")}
              {" · "}
              <span className="font-mono">{formatTokens(row.outputTokens)}</span> {t("localizationActivity.tokensOut")}
              {" · "}
              {t("localizationActivity.providerCount", { count: row.providerCount })}
              {" · "}
              {t("localizationActivity.modelCount", { count: row.modelCount })}
            </CardDescription>
          </div>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        ...
        <div className="text-xs text-muted-foreground">
          {t("localizationActivity.meteredRuns", { count: row.apiRunCount })}
          {" · "}
          {t("localizationActivity.subscriptionRuns", { count: row.subscriptionRunCount })}
          {" · "}
          {t("localizationActivity.amountThisWeek", { amount: formatCents(weekSpendCents) })}
        </div>
          ...
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("localizationActivity.billingTypes")}
              </p>
        ...
        {providerBreakdown.length > 0 && (
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("localizationActivity.upstreamProviders")}
              </p>
              <div className="space-y-1.5">
                  ...
                  <div key={entry.provider} className="flex items-center justify-between gap-2 text-xs">
                    ...
                    <div className="text-right tabular-nums">
                      ...
                      <div className="text-muted-foreground">
                        {t("localizationActivity.tokensShort", { value: formatTokens(entry.inputTokens + entry.outputTokens) })}
                      </div>
                    </div>
                  </div>
                ))}

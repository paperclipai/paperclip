            ...
            <section key={run.id} className="px-4 py-4">
                ...
                <div className="flex items-center gap-2">
                  {isActive && (
                    ...
                    >
                      ...
                      {cancellingRunIds.has(run.id) ? t("loading") : t("agents:stopRun", { defaultValue: "Stop" })}
                    </button>
                  )}
                  <Link
                    to={`/agents/${run.agentId}/runs/${run.id}`}
                  ...
                  >
                    {t("agents:currentRun", { defaultValue: "Open run" })}
              ...
              <div className="max-h-[320px] overflow-y-auto pr-1">
                <RunTranscriptView
                  ...
                  streaming={isActive}
                  collapseStdout
                  emptyMessage={hasOutputForRun(run.id) ? t("agents:noTranscript", { defaultValue: "Waiting for transcript parsing..." }) : t("agents:noRunsDescription", { defaultValue: "Waiting for run output..." })}
                />
              </div>
            </section>
          );

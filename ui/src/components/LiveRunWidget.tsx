    ...
    <div className="overflow-hidden rounded-xl border border-cyan-500/25 bg-background/80 shadow-[0_18px_50px_rgba(6,182,212,0.08)]">
      <div className="border-b border-border/60 bg-cyan-500/[0.04] px-4 py-3">
        <div className="text-xs font-semibold uppercase tracking-[0.18em] text-cyan-700 dark:text-cyan-300">
          {t("dashboard.components.liveRunWidget.title")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("dashboard.components.liveRunWidget.description")}
        </div>
      </div>
            ...
            <section key={run.id} className="px-4 py-4">
                ...
                <div className="flex items-center gap-2">
                  {isActive && (
                    ...
                    >
                      ...
                      {cancellingRunIds.has(run.id) ? t("dashboard.components.liveRunWidget.stopping") : t("dashboard.components.liveRunWidget.stop")}
                    </button>
                  )}
                  <Link
                    to={`/agents/${run.agentId}/runs/${run.id}`}
                  ...
                  >
                    {t("dashboard.components.liveRunWidget.open_run")}
              ...
              <div className="max-h-[320px] overflow-y-auto pr-1">
                <RunTranscriptView
                  ...
                  streaming={isActive}
                  collapseStdout
                  emptyMessage={hasOutputForRun(run.id) ? t("dashboard.components.liveRunWidget.waiting_parsing") : t("dashboard.components.liveRunWidget.waiting_output")}
                />
              </div>
            </section>
          );

...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const uploadMarkdownImage = useMutation({
    mutationFn: async ({ file, namespace }: { file: File; namespace: string }) => {
      if (!selectedCompanyId) throw new Error("Selecione uma empresa para enviar imagens");
  ...
  const testEnvironment = useMutation({
    mutationFn: async () => {
      if (!selectedCompanyId) {
        throw new Error("Selecione uma empresa para testar o ambiente do adaptador");
      }
  ...
  return (
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            ...
            <Field label="Reporta para" hint={help.reportsTo}>
              <ReportsToPicker
                agents={companyAgents}
                value={eff("identity", "reportsTo", props.agent.reportsTo ?? null)}
                onChange={(id) => mark("identity", "reportsTo", id)}
                excludeAgentIds={[props.agent.id]}
                chooseLabel="Escolher gestor…"
              />
            </Field>
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                <Field label="Template de prompt" hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
                    ...
                    placeholder="Você é o agente {{ agent.name }}. Seu papel é {{ agent.role }}..."
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  O template de prompt é reaplicado a cada heartbeat. Mantenha-o compacto e dinâmico para evitar custo recorrente e perda de cache.
                </div>
              </>
            )}
          </div>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label="Diretório de trabalho (obsoleto)" hint={help.cwd}>
          ...
          {isLocal && isCreate && (
            <>
              ...
              <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                O template de prompt é reaplicado a cada heartbeat. Prefira enquadramento curto da tarefa e variáveis como <code>{"{{ context.* }}"}</code> ou <code>{"{{ run.* }}"}</code>; evite repetir instruções estáveis aqui.
              </div>
            </>
          )}
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">Permissões e configuração</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">Permissões e configuração</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              <ModelDropdown
                ...
                emptyDetectHint="Nenhum modelo detectado. Selecione ou informe um manualmente."
              />
              {fetchedModelsError && (
                <p className="text-xs text-destructive">
                  {fetchedModelsError instanceof Error
                    ? fetchedModelsError.message
                    : "Falha ao carregar os modelos do adaptador."}
                </p>
              )}
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        O Codex pode rejeitar pensamento `minimal` quando a busca está ativada.
                      </p>
                    )}
                </>
              )}
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label="Prompt de bootstrap (legado)" hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                        "bootstrapPromptTemplate",
                      ...
                      placeholder="Prompt inicial opcional para a primeira execução"
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    O prompt de bootstrap é legado e será removido em uma versão futura. Considere mover esse conteúdo para o template de prompt ou arquivo de instruções do agente.
                  </div>
                </>
              )}
              ...
              <Field label="Argumentos extras (separados por vírgula)" hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  immediate
                  className={inputClass}
                  placeholder="ex.: --verbose, --foo=bar"
                />
              </Field>
              ...
              {!isCreate && (
                <>
                  ...
                  <Field label="Período de tolerância para interrupção (seg)" hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      {isCreate && showCreateRunPolicySection ? (
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <CollapsibleSection
              title="Política avançada de execução"
              bordered={cards}
...
function EnvVarEditor({
  ...
  async function sealRow(i: number) {
    ...
    const suggested = defaultSecretName(key) || "segredo";
    ...
    try {
      ...
      setSealError(err instanceof Error ? err.message : "Falha ao criar o segredo");
    }
  }
  ...
  return (
    <div className="space-y-1.5">
            ...
            {row.source === "secret" ? (
              ...
              <>
                ...
                <button
                  ...
                  onClick={() => sealRow(i)}
                  disabled={!row.key.trim() || !row.plainValue}
                  title="Armazenar valor como segredo e substituir pela referência"
                >
                  Proteger
                </button>
              </>
      ...
      <p className="text-[11px] text-muted-foreground/60">
        As variáveis PAPERCLIP_* são injetadas automaticamente em tempo de execução.
      </p>
    </div>
  );
}
          ...
          {onDetectModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel
                ? "Detectando..."
                : detectedModel
                  ? "Redetectar pela configuração"
                  : (detectModelLabel ?? "Detectar pela configuração")}
            </button>
          )}
            ...
            {filteredModels.length === 0 && !canCreateManualModel && promotedModelIds.size === 0 && (
              <div className="px-2 py-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  {onDetectModel
                    ? (emptyDetectHint ?? "Nenhum modelo detectado ainda. Informe um provedor/modelo manualmente.")
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label="Intensidade de raciocínio" hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{selected?.label ?? "Automático"}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">

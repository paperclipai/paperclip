...
function getCodexThinkingEffortOptions() {
  return [
    { id: "", label: t("components.agentConfigForm.effortAuto", { defaultValue: "Auto" }) },
    { id: "minimal", label: t("components.agentConfigForm.effortMinimal", { defaultValue: "Minimal" }) },
    { id: "low", label: t("components.agentConfigForm.effortLow", { defaultValue: "Low" }) },
    { id: "medium", label: t("components.agentConfigForm.effortMedium", { defaultValue: "Medium" }) },
    { id: "high", label: t("components.agentConfigForm.effortHigh", { defaultValue: "High" }) },
    { id: "xhigh", label: t("components.agentConfigForm.effortXHigh", { defaultValue: "X-High" }) },
  ] as const;
}

function getOpenCodeThinkingEffortOptions() {
  return [
    { id: "", label: t("components.agentConfigForm.effortAuto", { defaultValue: "Auto" }) },
    { id: "minimal", label: t("components.agentConfigForm.effortMinimal", { defaultValue: "Minimal" }) },
    { id: "low", label: t("components.agentConfigForm.effortLow", { defaultValue: "Low" }) },
    ...
    { id: "high", label: t("components.agentConfigForm.effortHigh", { defaultValue: "High" }) },
    { id: "xhigh", label: t("components.agentConfigForm.effortXHigh", { defaultValue: "X-High" }) },
    { id: "max", label: t("components.agentConfigForm.effortMax", { defaultValue: "Max" }) },
  ] as const;
}

function getCursorModeOptions() {
  return [
    { id: "", label: t("components.agentConfigForm.effortAuto", { defaultValue: "Auto" }) },
    { id: "plan", label: t("components.agentConfigForm.cursorModePlan", { defaultValue: "Plan" }) },
    { id: "ask", label: t("components.agentConfigForm.cursorModeAsk", { defaultValue: "Ask" }) },
  ] as const;
}

function getClaudeThinkingEffortOptions() {
  return [
    { id: "", label: t("components.agentConfigForm.effortAuto", { defaultValue: "Auto" }) },
    { id: "low", label: t("components.agentConfigForm.effortLow", { defaultValue: "Low" }) },
    { id: "medium", label: t("components.agentConfigForm.effortMedium", { defaultValue: "Medium" }) },
...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const testActionLabel = t("components.agentConfigForm.test", { defaultValue: "Test" });
      ...
      errorMessage: testActionError
        ?? (testEnvironment.error instanceof Error
          ...
          : testEnvironment.error
            ? t("components.agentConfigForm.environmentTestFailed", { defaultValue: "Environment test failed" })
            : null),
  ...
  async function handleRefreshModels() {
    ...
    try {
      ...
      setRefreshModelsError(error instanceof Error ? error.message : t("components.agentConfigForm.failedToRefreshModels", { defaultValue: "Failed to refresh adapter models." }));
    } finally {
  ...
  const thinkingEffortOptions =
    adapterType === "codex_local"
      ? getCodexThinkingEffortOptions()
      : adapterType === "acpx_local" && acpxAgent === "codex"
        ? getCodexThinkingEffortOptions()
        : adapterType === "cursor"
          ? getCursorModeOptions()
          : adapterType === "opencode_local"
            ? getOpenCodeThinkingEffortOptions()
            : getClaudeThinkingEffortOptions();
    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground">{t("components.agentConfigForm.unsavedChanges", { defaultValue: "Unsaved changes" })}</span>
            <Button
              size="sm"
              onClick={handleSave}
            ...
            >
              {!isCreate && props.isSaving
                ? t("components.agentConfigForm.saving", { defaultValue: "Saving..." })
                : t("components.agentConfigForm.save", { defaultValue: "Save" })}
            </Button>
          </div>
        </div>
      )}
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("components.agentConfigForm.sectionIdentity", { defaultValue: "Identity" })}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("components.agentConfigForm.sectionIdentity", { defaultValue: "Identity" })}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("components.agentConfigForm.nameLabel", { defaultValue: "Name" })} hint={help.name}>
              <DraftInput
                value={eff("identity", "name", props.agent.name)}
                onCommit={(v) => mark("identity", "name", v)}
                immediate
                className={inputClass}
                placeholder={t("components.agentConfigForm.namePlaceholder", { defaultValue: "Agent name" })}
              />
            </Field>
            <Field label={t("components.agentConfigForm.titleLabel", { defaultValue: "Title" })} hint={help.title}>
              <DraftInput
                value={eff("identity", "title", props.agent.title ?? "")}
                onCommit={(v) => mark("identity", "title", v || null)}
                immediate
                className={inputClass}
                placeholder={t("components.agentConfigForm.titlePlaceholder", { defaultValue: "e.g. VP of Engineering" })}
              />
            </Field>
            <Field label={t("components.agentConfigForm.reportsToLabel", { defaultValue: "Reports to" })} hint={help.reportsTo}>
              <ReportsToPicker
                agents={companyAgents}
                value={eff("identity", "reportsTo", props.agent.reportsTo ?? null)}
                onChange={(id) => mark("identity", "reportsTo", id)}
                excludeAgentIds={[props.agent.id]}
                chooseLabel={t("components.agentConfigForm.chooseManager", { defaultValue: "Choose manager…" })}
              />
            </Field>
            <Field label={t("components.agentConfigForm.capabilitiesLabel", { defaultValue: "Capabilities" })} hint={help.capabilities}>
              <MarkdownEditor
                value={eff("identity", "capabilities", props.agent.capabilities ?? "") ?? ""}
                onChange={(v) => mark("identity", "capabilities", v || null)}
                placeholder={t("components.agentConfigForm.capabilitiesPlaceholder", { defaultValue: "Describe what this agent can do..." })}
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                <Field label={t("components.agentConfigForm.promptTemplateLabel", { defaultValue: "Prompt Template" })} hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  {t("components.agentConfigForm.promptTemplateNotice", { defaultValue: "Prompt template is replayed on every heartbeat. Keep it compact and dynamic to avoid recurring token cost and cache churn." })}
                </div>
              </>
            )}
          </div>
      ...
      {forcedKubernetes ? (
        ...
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("components.agentConfigForm.sectionEnvironment", { defaultValue: "Environment" })}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("components.agentConfigForm.sectionEnvironment", { defaultValue: "Environment" })}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
              label={t("components.agentConfigForm.defaultEnvironmentLabel", { defaultValue: "Default environment" })}
              hint={t("components.agentConfigForm.defaultEnvironmentKubernetesHint", { defaultValue: "This instance runs all agents in the Kubernetes sandbox. Local execution is disabled." })}
            >
              {kubernetesEnvironment ? (
                <div className={cn(inputClass, "flex items-center text-muted-foreground")}>
                  {t("components.agentConfigForm.kubernetesSandboxValue", { name: kubernetesEnvironment.name, defaultValue: "{{name}} · Kubernetes sandbox" })}
                </div>
              ) : (
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                  {t("components.agentConfigForm.kubernetesUnavailableNotice", { defaultValue: "This instance requires the Kubernetes sandbox, but no managed Kubernetes environment is available for this company yet. Configure one before creating agents; execution will not fall back to local." })}
                </div>
              )}
            </Field>
          </div>
        ...
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("components.agentConfigForm.environmentOverrideLabel", { defaultValue: "Environment override" })}>
              <div className="space-y-2">
                ...
                >
                  <option value="">{t("components.agentConfigForm.defaultEnvironmentOption", { defaultValue: "Default: {{label}}", label: inheritedEnvironmentLabel })}</option>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{t("components.agentConfigForm.sectionAdapter", { defaultValue: "Adapter" })}</h3>
            : <span className="text-xs font-medium text-muted-foreground">{t("components.agentConfigForm.sectionAdapter", { defaultValue: "Adapter" })}</span>
          }
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          {showAdapterTypeField && (
            <Field label={t("components.agentConfigForm.adapterTypeLabel", { defaultValue: "Adapter type" })} hint={help.adapterType}>
              <AdapterTypeDropdown
                value={adapterType}
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label={t("components.agentConfigForm.workingDirectoryDeprecatedLabel", { defaultValue: "Working directory (deprecated)" })} hint={help.cwd}>
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("components.agentConfigForm.sectionPermissionsConfiguration", { defaultValue: "Permissions & Configuration" })}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("components.agentConfigForm.sectionPermissionsConfiguration", { defaultValue: "Permissions & Configuration" })}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label={t("components.agentConfigForm.commandLabel", { defaultValue: "Command" })} hint={help.localCommand}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.command
              ...
              {supportsModelProfiles && (
                <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("components.agentConfigForm.primaryModel", { defaultValue: "Primary model" })}</div>
              )}
              <ModelDropdown
                models={models}
                ...
                detectModelLabel={t("components.agentConfigForm.detectModel", { defaultValue: "Detect model" })}
                emptyDetectHint={t("components.agentConfigForm.noModelDetectedManual", { defaultValue: "No model detected. Select or enter one manually." })}
              />
                ...
                <p className="text-xs text-destructive">
                  {refreshModelsError
                    ?? (fetchedModelsError instanceof Error
                      ...
                      : t("components.agentConfigForm.failedToLoadModels", { defaultValue: "Failed to load adapter models." }))}
                </p>
              )}
              {adapterType === "opencode_local"
                ...
                <p className="text-xs text-muted-foreground">
                  {t("components.agentConfigForm.openCodeModelDiscoveryNotice", { name: currentDefaultEnvironment.name, defaultValue: "Live OpenCode model discovery only runs for Local environments. Using the curated list and manual entry for {{name}}." })}
                </p>
              )}
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        {t("components.agentConfigForm.codexMinimalThinkingWarning", { defaultValue: "Codex may reject `minimal` thinking when search is enabled." })}
                      </p>
                    )}
                </>
              )}
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label={t("components.agentConfigForm.bootstrapPromptLegacyLabel", { defaultValue: "Bootstrap prompt (legacy)" })} hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                      ...
                      placeholder={t("components.agentConfigForm.bootstrapPromptPlaceholder", { defaultValue: "Optional initial setup prompt for the first run" })}
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    {t("components.agentConfigForm.bootstrapPromptLegacyNotice", { defaultValue: "Bootstrap prompt is legacy and will be removed in a future release. Consider moving this content into the agent's prompt template or instructions file instead." })}
                  </div>
                </>
              )}
              ...
              <Field label={t("components.agentConfigForm.extraArgsLabel", { defaultValue: "Extra args (comma-separated)" })} hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  className={inputClass}
                  placeholder={t("components.agentConfigForm.extraArgsPlaceholder", { defaultValue: "e.g. --verbose, --foo=bar" })}
                />
              </Field>

              <Field label={t("components.agentConfigForm.environmentVariablesLabel", { defaultValue: "Environment variables" })} hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
              ...
              {!isCreate && (
                <>
                  <Field label={t("components.agentConfigForm.timeoutSecLabel", { defaultValue: "Timeout (sec)" })} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={t("components.agentConfigForm.graceSecLabel", { defaultValue: "Interrupt grace period (sec)" })} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      {isCreate && showCreateRunPolicySection ? (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium flex items-center gap-2 mb-3"><Heart className="h-3 w-3" /> {t("components.agentConfigForm.sectionRunPolicy", { defaultValue: "Run Policy" })}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground flex items-center gap-2"><Heart className="h-3 w-3" /> {t("components.agentConfigForm.sectionRunPolicy", { defaultValue: "Run Policy" })}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              label={t("components.agentConfigForm.heartbeatOnInterval", { defaultValue: "Heartbeat on interval" })}
              hint={help.heartbeatInterval}
              checked={val!.heartbeatEnabled}
              onCheckedChange={(v) => set!({ heartbeatEnabled: v })}
              number={val!.intervalSec}
              onNumberChange={(v) => set!({ intervalSec: v })}
              numberLabel={t("components.agentConfigForm.secUnit", { defaultValue: "sec" })}
              numberPrefix={t("components.agentConfigForm.runHeartbeatEvery", { defaultValue: "Run heartbeat every" })}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <CollapsibleSection
              title={t("components.agentConfigForm.advancedRunPolicy", { defaultValue: "Advanced Run Policy" })}
              bordered={cards}
            ...
            <div className="space-y-3">
              <ToggleField
                label={t("components.agentConfigForm.wakeOnDemand", { defaultValue: "Wake on demand" })}
              ...
              />
              <Field label={t("components.agentConfigForm.cooldownSecLabel", { defaultValue: "Cooldown (sec)" })} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label={t("components.agentConfigForm.maxConcurrentRunsLabel", { defaultValue: "Max concurrent runs" })} hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
              ...
              <div className="rounded-md border border-border/70 px-3 py-2">
                <ToggleField
                  label={t("components.agentConfigForm.continueAfterMaxTurnStop", { defaultValue: "Continue after max-turn stop" })}
                ...
                {maxTurnContinuationEnabled ? (
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <Field label={t("components.agentConfigForm.continuationAttemptsLabel", { defaultValue: "Continuation attempts" })} hint={help.maxTurnContinuationMaxAttempts}>
                      <DraftNumberInput
                    ...
                    <Field label={t("components.agentConfigForm.continuationDelaySecLabel", { defaultValue: "Continuation delay (sec)" })} hint={help.maxTurnContinuationDelaySec}>
                      <DraftNumberInput
...
export function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  ...
  const statusLabel =
    result.status === "pass"
      ? t("components.agentConfigForm.resultStatusPassed", { defaultValue: "Passed" })
      : result.status === "warn"
        ? t("components.agentConfigForm.resultStatusWarnings", { defaultValue: "Warnings" })
        : t("components.agentConfigForm.resultStatusFailed", { defaultValue: "Failed" });
  const statusClass =
    result.status === "pass"
      ...
      <div className="mt-2 space-y-1.5">
        {result.checks.map((check, idx) => (
          <div key={`${check.code}-${idx}`} className="text-[11px] leading-relaxed break-words">
            ...
            {check.hint && <span className="block opacity-90 break-words">{t("components.agentConfigForm.checkHintPrefix", { hint: check.hint, defaultValue: "Hint: {{hint}}" })}</span>}
          </div>
        ))}
      </div>
    </div>
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
        {adapterList.map((item) => (
          ...
          >
            ...
            {item.comingSoon && (
              <span className="text-[10px] text-muted-foreground">{t("components.agentConfigForm.comingSoon", { defaultValue: "Coming soon" })}</span>
            )}
          </button>
        ))}
      </PopoverContent>
...
function ExperimentalBadge() {
  ...
  return (
    <span className="shrink-0 rounded border border-amber-500/30 bg-amber-500/10 px-1.5 py-0.5 text-[10px] font-medium leading-none text-amber-700 dark:text-amber-200">
      {t("components.agentConfigForm.experimentalBadge", { defaultValue: "Experimental" })}
    </span>
  );
}
...
function ModelDropdown({
  ...
  return (
    <Field label={t("components.agentConfigForm.modelLabel", { defaultValue: "Model" })} hint={help.model}>
      <Popover
        open={open}
          ...
          <button type="button" className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ...
                : value
                  || (allowDefault
                    ? (defaultLabel ?? t("components.agentConfigForm.modelDefault", { defaultValue: "Default" }))
                    : required
                      ? t("components.agentConfigForm.selectModelRequired", { defaultValue: "Select model (required)" })
                      : t("components.agentConfigForm.selectModel", { defaultValue: "Select model" }))}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          <div className="relative mb-1">
            <input
              ...
              placeholder={creatable
                ? t("components.agentConfigForm.searchModelsCreatable", { defaultValue: "Search models... (type to create)" })
                : t("components.agentConfigForm.searchModels", { defaultValue: "Search models..." })}
              value={modelSearch}
          ...
          {onDetectModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel
                ? t("components.agentConfigForm.detecting", { defaultValue: "Detecting..." })
                : detectedModel
                  ? (detectModelLabel
                      ? t("components.agentConfigForm.reDetectModel", { defaultValue: "Re-detect model" })
                      : t("components.agentConfigForm.reDetectFromConfig", { defaultValue: "Re-detect from config" }))
                  : (detectModelLabel ?? t("components.agentConfigForm.detectFromConfig", { defaultValue: "Detect from config" }))}
            </button>
          )}
          {onRefreshModels && !modelSearch.trim() && (
            ...
            >
              ...
              {refreshingModels
                ? t("components.agentConfigForm.refreshing", { defaultValue: "Refreshing..." })
                : t("components.agentConfigForm.refreshModels", { defaultValue: "Refresh models" })}
            </button>
          )}
          {value && (!models.some((m) => m.id === value) || promotedModelIds.has(value)) && (
            ...
            >
              ...
              <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-green-500/15 text-green-400 border border-green-500/20">
                {t("components.agentConfigForm.modelBadgeCurrent", { defaultValue: "current" })}
              </span>
            </button>
          )}
          {detectedModel && detectedModel !== value && (
            ...
            >
              ...
              <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-blue-500/15 text-blue-400 border border-blue-500/20">
                {t("components.agentConfigForm.modelBadgeDetected", { defaultValue: "detected" })}
              </span>
            </button>
          )}
          {detectedModelCandidates
              ...
              return (
                ...
                >
                  ...
                  <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-sky-500/15 text-sky-400 border border-sky-500/20">
                    {t("components.agentConfigForm.modelBadgeConfig", { defaultValue: "config" })}
                  </span>
                </button>
              );
            })}
          <div className="max-h-[240px] overflow-y-auto">
            {allowDefault && (
              ...
              >
                {t("components.agentConfigForm.modelDefault", { defaultValue: "Default" })}
              </button>
            )}
            {canCreateManualModel && (
              ...
              >
                <span>{t("components.agentConfigForm.useManualModel", { defaultValue: "Use manual model" })}</span>
              ...
              <div key={group.provider} className="mb-1 last:mb-0">
                {groupByProvider && (
                  <div className="px-2 py-1 text-[10px] uppercase tracking-wide text-muted-foreground">
                    {t("components.agentConfigForm.providerGroupHeader", { provider: group.provider, count: group.entries.length, defaultValue: "{{provider}} ({{count}})" })}
                  </div>
                )}
                {group.entries.map((m) => (
                  <button
            ...
            {filteredModels.length === 0 && !canCreateManualModel && promotedModelIds.size === 0 && (
              <div className="px-2 py-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  {onDetectModel
                    ? (emptyDetectHint ?? t("components.agentConfigForm.noModelDetectedYet", { defaultValue: "No model detected yet. Enter a provider/model manually." }))
                    : t("components.agentConfigForm.noModelsFound", { defaultValue: "No models found." })}
                </p>
              </div>
            )}
          </div>
...
function CheapModelSection({
  ...
  const placeholderHint = adapterDefaultModel
    ? t("components.agentConfigForm.cheapModelAdapterDefault", { model: adapterDefaultModel, defaultValue: "Adapter default · {{model}}" })
    : t("components.agentConfigForm.cheapModelNoAdapterDefault", { defaultValue: "No adapter default — choose a cheaper model" });
  return (
    <div className="rounded-md border border-border/70 bg-muted/20 p-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("components.agentConfigForm.cheapModelTitle", { defaultValue: "Cheap model" })}</div>
          <p className="text-xs text-muted-foreground">
            {t("components.agentConfigForm.cheapModelDescription", { defaultValue: "Used when a run requests the cheap profile (e.g. routine summaries). The primary model stays unchanged." })}
          </p>
        </div>
      ...
      {enabled && !model && adapterDefaultModel ? (
        <p className="text-[11px] text-muted-foreground">
          {t("components.agentConfigForm.cheapModelFallbackToDefault", { model: adapterDefaultModel, defaultValue: "No explicit cheap model selected — runtime falls back to {{model}}." })}
        </p>
      ) : null}
      {enabled && !model && !adapterDefaultModel ? (
        <p className="text-[11px] text-amber-500">
          {t("components.agentConfigForm.cheapModelNoDefaultWarning", { defaultValue: "No cheap model selected and the adapter has no default. Cheap-lane runs will continue on the primary model with a fallback note." })}
        </p>
      ) : null}
    </div>
  );
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label={t("components.agentConfigForm.thinkingEffortLabel", { defaultValue: "Thinking effort" })} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{selected?.label ?? t("components.agentConfigForm.effortAuto", { defaultValue: "Auto" })}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">

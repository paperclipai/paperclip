...
const codexThinkingEffortOptions: ReadonlyArray<ThinkingEffortOption> = [
  { id: "", labelKey: "config_form.options.auto" },
  { id: "minimal", labelKey: "config_form.options.minimal" },
  { id: "low", labelKey: "config_form.options.low" },
  { id: "medium", labelKey: "config_form.options.medium" },
  { id: "high", labelKey: "config_form.options.high" },
  { id: "xhigh", labelKey: "config_form.options.xhigh" },
] as const;
...
const openCodeThinkingEffortOptions: ReadonlyArray<ThinkingEffortOption> = [
  ...
  { id: "high", labelKey: "config_form.options.high" },
  { id: "xhigh", labelKey: "config_form.options.xhigh" },
  { id: "max", labelKey: "config_form.options.max" },
] as const;

const cursorModeOptions: ReadonlyArray<ThinkingEffortOption> = [
  { id: "", labelKey: "config_form.options.auto" },
  { id: "plan", labelKey: "config_form.options.plan" },
  { id: "ask", labelKey: "config_form.options.ask" },
] as const;
...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedCompanyId) throw new Error(t("config_form.errors.select_company_secrets"));
  ...
  const uploadMarkdownImage = useMutation({
    mutationFn: async ({ file, namespace }: { file: File; namespace: string }) => {
      if (!selectedCompanyId) throw new Error(t("config_form.errors.select_company_images"));
  ...
  const {
    ...
    queryFn: () => {
      if (!selectedCompanyId) {
        throw new Error(t("config_form.errors.select_company_detect_model"));
      }
  ...
  const testEnvironment = useMutation({
    mutationFn: async () => {
      if (!selectedCompanyId) {
        throw new Error(t("config_form.errors.select_company_test_environment"));
      }
    ...
    props.onTestFeedbackChange({
      errorMessage: testEnvironment.error instanceof Error
        ...
        : testEnvironment.error
          ? t("config_form.status.environment_test_failed")
          : null,
  ...
  async function handleRefreshModels() {
    ...
    try {
      ...
      setRefreshModelsError(error instanceof Error ? error.message : t("config_form.errors.failed_refresh_models"));
    } finally {
    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground">{t("config_form.unsaved_changes")}</span>
            <Button
              size="sm"
              onClick={handleSave}
              disabled={!isCreate && props.isSaving}
            >
              {!isCreate && props.isSaving ? t("config_form.actions.saving") : t("config_form.actions.save")}
            </Button>
          </div>
        </div>
      )}
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("config_form.section_identity")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("config_form.section_identity")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("config_form.fields.name")} hint={help.name}>
              <DraftInput
            ...
            <Field label={t("config_form.fields.title")} hint={help.title}>
              <DraftInput
            ...
            <Field label={t("config_form.fields.reports_to")} hint={help.reportsTo}>
              <ReportsToPicker
                agents={companyAgents}
                value={eff("identity", "reportsTo", props.agent.reportsTo ?? null)}
                onChange={(id) => mark("identity", "reportsTo", id)}
                excludeAgentIds={[props.agent.id]}
                chooseLabel={t("config_form.fields.reports_to_placeholder")}
              />
            </Field>
            <Field label={t("config_form.fields.capabilities")} hint={help.capabilities}>
              <MarkdownEditor
                value={eff("identity", "capabilities", props.agent.capabilities ?? "") ?? ""}
                onChange={(v) => mark("identity", "capabilities", v || null)}
                placeholder={t("config_form.fields.capabilities_placeholder")}
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                <Field label={t("config_form.fields.prompt_template")} hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
                    ...
                    placeholder={t("config_form.fields.prompt_template_placeholder")}
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  {t("config_form.prompt_template_warning")}
                </div>
              </>
            )}
          </div>
      ...
      {environmentsEnabled ? (
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("config_form.section_execution")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("config_form.section_execution")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
              label={t("config_form.fields.default_environment")}
              hint={t("config_form.fields.default_environment_hint")}
            >
              <select
                className={inputClass}
              ...
              >
                <option value="">{t("config_form.fields.default_environment_company_local")}</option>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{t("config_form.section_adapter")}</h3>
            : <span className="text-xs font-medium text-muted-foreground">{t("config_form.section_adapter")}</span>
          }
          {showInlineAdapterTestEnvironmentButton && (
            <Button
              ...
              disabled={testEnvironmentDisabled}
            >
              {testEnvironment.isPending ? t("config_form.actions.testing") : t("config_form.actions.test")}
            </Button>
          )}
        </div>
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          {showAdapterTypeField && (
            <Field label={t("config_form.fields.adapter_type")} hint={help.adapterType}>
              <AdapterTypeDropdown
                value={adapterType}
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label={t("config_form.fields.working_dir")} hint={help.cwd}>
              <div className="flex items-center gap-2 rounded-md border border-border px-2.5 py-1.5">
                ...
                <DraftInput
                  ...
                  placeholder={t("config_form.fields.working_dir_placeholder")}
                />
                <ChoosePathButton />
              </div>
            </Field>
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("config_form.section_permissions")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("config_form.section_permissions")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label={t("config_form.fields.command")} hint={help.localCommand}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.command
              ...
              {supportsModelProfiles && (
                <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("config_form.primary_model")}</div>
              )}
              <ModelDropdown
                models={models}
                ...
                detectModelLabel={t("config_form.actions.detect_model")}
                emptyDetectHint={t("config_form.status.no_model_detected")}
              />
                ...
                <p className="text-xs text-destructive">
                  {refreshModelsError
                    ?? (fetchedModelsError instanceof Error
                      ...
                      : t("config_form.status.failed_load_models"))}
                </p>
              )}
              {adapterType === "opencode_local"
                ...
                <p className="text-xs text-muted-foreground">
                  {t("config_form.status.live_opencode_local_only", { name: currentDefaultEnvironment.name })}
                </p>
              )}
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        {t("config_form.codex_minimal_warning")}
                      </p>
                    )}
                </>
              )}
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label={t("config_form.fields.bootstrap_prompt")} hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                      ...
                      placeholder={t("config_form.fields.bootstrap_prompt_placeholder")}
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    {t("config_form.bootstrap_prompt_warning")}
                  </div>
                </>
              )}
              ...
              <Field label={t("config_form.fields.extra_args")} hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  immediate
                  className={inputClass}
                  placeholder={t("config_form.fields.extra_args_placeholder")}
                />
              </Field>

              <Field label={t("config_form.fields.env_vars")} hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
              ...
              {!isCreate && (
                <>
                  <Field label={t("config_form.fields.timeout_sec")} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={t("config_form.fields.grace_sec")} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      {isCreate && showCreateRunPolicySection ? (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium flex items-center gap-2 mb-3"><Heart className="h-3 w-3" /> {t("config_form.section_run_policy")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground flex items-center gap-2"><Heart className="h-3 w-3" /> {t("config_form.section_run_policy")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              label={t("config_form.fields.heartbeat_on_interval")}
              hint={help.heartbeatInterval}
              checked={val!.heartbeatEnabled}
              onCheckedChange={(v) => set!({ heartbeatEnabled: v })}
              number={val!.intervalSec}
              onNumberChange={(v) => set!({ intervalSec: v })}
              numberLabel={t("config_form.fields.unit_sec")}
              numberPrefix={t("config_form.heartbeat_prefix")}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <CollapsibleSection
              title={t("config_form.fields.advanced_run_policy")}
              bordered={cards}
            ...
            <div className="space-y-3">
              <ToggleField
                label={t("config_form.fields.wake_on_demand")}
              ...
              />
              <Field label={t("config_form.fields.cooldown_sec")} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label={t("config_form.fields.max_concurrent_runs")} hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
              ...
              <div className="rounded-md border border-border/70 px-3 py-2">
                <ToggleField
                  label={t("config_form.fields.continue_after_max_turn")}
                ...
                {maxTurnContinuationEnabled ? (
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <Field label={t("config_form.fields.continuation_attempts")} hint={help.maxTurnContinuationMaxAttempts}>
                      <DraftNumberInput
                    ...
                    <Field label={t("config_form.fields.continuation_delay_sec")} hint={help.maxTurnContinuationDelaySec}>
                      <DraftNumberInput
...
export function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  ...
  const statusLabel =
    result.status === "pass"
      ? t("config_form.status.passed")
      : result.status === "warn"
        ? t("config_form.status.warnings")
        : t("config_form.status.failed");
  const statusClass =
    result.status === "pass"
      ...
      <div className="mt-2 space-y-1.5">
        {result.checks.map((check, idx) => (
          <div key={`${check.code}-${idx}`} className="text-[11px] leading-relaxed break-words">
            ...
            {check.hint && <span className="block opacity-90 break-words">{t("config_form.hint_prefix")}: {check.hint}</span>}
          </div>
        ))}
      </div>
    </div>
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
        {adapterList.map((item) => (
          ...
          >
            ...
            {item.comingSoon && (
              <span className="text-[10px] text-muted-foreground">{t("config_form.adapter_type.coming_soon")}</span>
            )}
          </button>
        ))}
      </PopoverContent>
...
function ExperimentalBadge() {
  ...
  return (
    <span className="shrink-0 rounded border border-amber-500/30 bg-amber-500/10 px-1.5 py-0.5 text-[10px] font-medium leading-none text-amber-700 dark:text-amber-200">
      {t("config_form.experimental_badge")}
    </span>
  );
}
...
function ModelDropdown({
  ...
  return (
    <Field label={t("config_form.fields.model")} hint={help.model}>
      <Popover
        open={open}
          ...
          <button type="button" className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ...
                : value
                  || (allowDefault
                    ? (defaultLabel ?? t("config_form.model_default"))
                    : required
                      ? t("config_form.model_required")
                      : t("config_form.model_select"))}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          <div className="relative mb-1">
            <input
              ...
              placeholder={creatable ? t("config_form.fields.model_search_creatable_placeholder") : t("config_form.fields.model_search_placeholder")}
              value={modelSearch}
          ...
          {onDetectModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel
                ? t("config_form.actions.detecting")
                : detectedModel
                  ? t("config_form.actions.re_detect_from_config")
                  : (detectModelLabel ?? t("config_form.actions.detect_from_config"))}
            </button>
          )}
          {onRefreshModels && !modelSearch.trim() && (
            ...
            >
              ...
              {refreshingModels ? t("config_form.actions.refreshing") : t("config_form.actions.refresh_models")}
            </button>
          )}
          {value && (!models.some((m) => m.id === value) || promotedModelIds.has(value)) && (
            ...
            >
              ...
              <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-green-500/15 text-green-400 border border-green-500/20">
                {t("config_form.status.model_badge_current")}
              </span>
            </button>
          )}
          {detectedModel && detectedModel !== value && (
            ...
            >
              ...
              <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-blue-500/15 text-blue-400 border border-blue-500/20">
                {t("config_form.status.model_badge_detected")}
              </span>
            </button>
          )}
          {detectedModelCandidates
              ...
              return (
                ...
                >
                  ...
                  <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-sky-500/15 text-sky-400 border border-sky-500/20">
                    {t("config_form.status.model_badge_config")}
                  </span>
                </button>
              );
            })}
          <div className="max-h-[240px] overflow-y-auto">
            ...
            {canCreateManualModel && (
              ...
              >
                <span>{t("config_form.model_use_manual")}</span>
            ...
            {filteredModels.length === 0 && !canCreateManualModel && promotedModelIds.size === 0 && (
              <div className="px-2 py-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  {onDetectModel
                    ? (emptyDetectHint ?? t("config_form.status.no_model_detected_hint"))
                    : t("config_form.status.no_models_found")}
                </p>
              </div>
            )}
          </div>
...
function CheapModelSection({
  ...
  const placeholderHint = adapterDefaultModel
    ? t("config_form.cheap_model.adapter_default_placeholder", { model: adapterDefaultModel })
    : t("config_form.cheap_model.no_default_placeholder");
  return (
    <div className="rounded-md border border-border/70 bg-muted/20 p-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("config_form.cheap_model.label")}</div>
          <p className="text-xs text-muted-foreground">
            {t("config_form.cheap_model.description")}
          </p>
        </div>
      ...
      {enabled && !model && adapterDefaultModel ? (
        <p className="text-[11px] text-muted-foreground">
          <Trans
            ns="agents"
            i18nKey="config_form.cheap_model.fallback_note"
      ...
      {enabled && !model && !adapterDefaultModel ? (
        <p className="text-[11px] text-amber-500">
          {t("config_form.cheap_model.no_default_warning")}
        </p>
      ) : null}
    </div>
  );
...
function ThinkingEffortDropdown({
  ...
  value: string;
  options: ReadonlyArray<ThinkingEffortOption>;
  onChange: (id: string) => void;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}) {
  ...
  return (
    <Field label={t("config_form.fields.thinking_effort")} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{selected ? t(selected.labelKey) : t("config_form.options.auto")}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">

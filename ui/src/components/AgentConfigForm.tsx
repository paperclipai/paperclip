      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            ...
            <Field label="직책" hint={help.title}>
              <DraftInput
            ...
            <Field label="역할 설명" hint={help.capabilities}>
              <MarkdownEditor
                value={eff("identity", "capabilities", props.agent.capabilities ?? "")}
                onChange={(v) => mark("identity", "capabilities", v || null)}
                placeholder="에이전트가 수행할 작업에 대한 상세 내용..."
              ...
              <>
                <Field label="프롬프트 템플릿" hint={help.promptTemplate}>
                  <MarkdownEditor
                    ...
                    placeholder="당신은 에이전트 {{ agent.name }}입니다. 당신의 역할은 {{ agent.role }}입니다..."
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  프롬프트 템플릿은 매 하트비트마다 재실행됩니다. 반복적인 토큰 비용과 캐시 낭비를 피하기 위해 간결하고 동적으로 유지하세요.
                </div>
              </>
            )}
          </div>
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          ...
          {isLocal && isCreate && (
            <>
              ...
              <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                Prompt template is replayed on every heartbeat. 작은 작업 프레이밍과 <code>{"{{ context.* }}"}</code> 또는 <code>{"{{ run.* }}"}</code> 같은 변수를 사용하고, 안정적인 지시사항은 여기에 반복하지 마세요.
              </div>
            </>
          )}
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        검색이 활성화된 경우 Codex가 `minimal` 사고 수준을 거부할 수 있습니다.
                      </p>
                    )}
                </>
              )}
                ...
                <>
                  <Field label="부트스트랩 프롬프트 (레거시)" hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      ...
                      placeholder="첫 실행 시 선택적 초기 설정 프롬프트"
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    부트스트랩 프롬프트는 레거시이며 향후 릴리스에서 제거될 예정입니다. 이 내용을 에이전트의 프롬프트 템플릿이나 지시사항 파일로 이동하는 것을 고려하세요.
                  </div>
                </>
              )}
              {adapterType === "claude_local" && (
              ...
              <Field label="추가 인자 (쉼표 구분)" hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <div className="space-y-3">
              ...
              />
              <Field label="대기 시간 (초)" hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
...
function EnvVarEditor({
  ...
  async function sealRow(i: number) {
    ...
    try {
      ...
      setSealError(err instanceof Error ? err.message : "시크릿 생성 실패");
    }
  }
        ...
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ? selected.label
                : value || (allowDefault ? "기본" : required ? "모델 선택 (필수)" : "모델 선택")}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>

...
const codexThinkingEffortOptions = [
  { id: "", label: t("agentConfigForm.labelsObj.auto") },
  { id: "minimal", label: t("agentConfigForm.labelsObj.minimal") },
  { id: "low", label: "Low" },
  { id: "medium", label: t("agentConfigForm.labelsObj.medium") },
  { id: "high", label: t("agentConfigForm.labelsObj.high") },
  { id: "xhigh", label: t("agentConfigForm.labelsObj.xHigh") },
] as const;
...
const cursorModeOptions = [
  { id: "", label: t("agentConfigForm.labelsObj.auto") },
  { id: "plan", label: t("agentConfigForm.labelsObj.plan") },
  { id: "ask", label: "Ask" },
] as const;
...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const testEnvironment = useMutation({
    mutationFn: async () => {
      ...
      const testResults: Array<{ label: string; model: string | null; result: AdapterEnvironmentTestResult }> = [
        {
          label: t("agentConfigForm.labelsObj.primaryModel"),
          model: primaryModel,
      ...
      if (cheapTestCase) {
        testResults.push({
          label: t("agentConfigForm.labelsObj.cheapModel"),
  ...
  const runEnvironmentTest = useCallback(async () => {
    ...
    try {
      ...
      setTestActionError(error instanceof Error ? error.message : t("agentConfigForm.errors.envTestFailed"));
      throw error;
    } finally {
  ...
  async function handleRefreshModels() {
    ...
    try {
      ...
      setRefreshModelsError(error instanceof Error ? error.message : t("agentConfigForm.errors.refreshAdapterModels"));
    } finally {
    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground">{t("agentConfigForm.text.unsavedChanges")}</span>
            <Button
              size="sm"
              onClick={handleSave}
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agentConfigForm.text.identity")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentConfigForm.text.identity")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("agentConfigForm.labelsJsx.name")} hint={help.name}>
              <DraftInput
            ...
            <Field label={t("agentConfigForm.labelsJsx.title")} hint={help.title}>
              <DraftInput
            ...
            <Field label={t("agentConfigForm.labelsJsx.reportsTo")} hint={help.reportsTo}>
              <ReportsToPicker
            ...
            <Field label={t("agentConfigForm.labelsJsx.capabilities")} hint={help.capabilities}>
              <MarkdownEditor
                value={eff("identity", "capabilities", props.agent.capabilities ?? "") ?? ""}
                onChange={(v) => mark("identity", "capabilities", v || null)}
                placeholder={t("agentConfigForm.placeholders.agentDesc")}
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                <Field label={t("agentConfigForm.labelsJsx.promptTemplate")} hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
      ...
      {forcedKubernetes ? (
        ...
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agentConfigForm.text.environment")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentConfigForm.text.environment")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
              label={t("agentConfigForm.labelsJsx.defaultEnvironment")}
        ...
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("agentConfigForm.labelsJsx.envOverride")}>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{t("agentConfigForm.text.adapter")}</h3>
            : <span className="text-xs font-medium text-muted-foreground">{t("agentConfigForm.text.adapter")}</span>
          }
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          {showAdapterTypeField && (
            <Field label={t("agentConfigForm.labelsJsx.adapterType")} hint={help.adapterType}>
              <AdapterTypeDropdown
                value={adapterType}
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label={t("agentConfigForm.labelsJsx.workingDir")} hint={help.cwd}>
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label={t("agentConfigForm.labelsJsx.command")} hint={help.localCommand}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.command
              ...
              {supportsModelProfiles && (
                <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("agentConfigForm.text.primaryModel")}</div>
              )}
              <ModelDropdown
                models={models}
              ...
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label={t("agentConfigForm.labelsJsx.bootstrapPrompt")} hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                      ...
                      placeholder={t("agentConfigForm.placeholders.initialPrompt")}
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
              ...
              <Field label={t("agentConfigForm.labelsJsx.extraArgs")} hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
              ...
              <Field label={t("agentConfigForm.labelsJsx.envVars")} hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
              ...
              {!isCreate && (
                <>
                  <Field label={t("agentConfigForm.labelsJsx.timeout")} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={t("agentConfigForm.labelsJsx.interruptGrace")} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      {isCreate && showCreateRunPolicySection ? (
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              label={t("agentConfigForm.labelsJsx.heartbeatInterval")}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <div className="space-y-3">
              <ToggleField
                label={t("agentConfigForm.labelsJsx.wakeOnDemand")}
              ...
              />
              <Field label={t("agentConfigForm.labelsJsx.cooldown")} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label={t("agentConfigForm.labelsJsx.maxConcurrent")} hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
              ...
              <div className="rounded-md border border-border/70 px-3 py-2">
                <ToggleField
                  label={t("agentConfigForm.labelsJsx.continueAfterMaxTurn")}
                ...
                {maxTurnContinuationEnabled ? (
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <Field label={t("agentConfigForm.labelsJsx.continuationAttempts")} hint={help.maxTurnContinuationMaxAttempts}>
                      <DraftNumberInput
                    ...
                    <Field label={t("agentConfigForm.labelsJsx.continuationDelay")} hint={help.maxTurnContinuationDelaySec}>
                      <DraftNumberInput
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
        {adapterList.map((item) => (
          ...
          >
            ...
            {item.comingSoon && (
              <span className="text-[10px] text-muted-foreground">{t("agentConfigForm.text.comingSoon")}</span>
            )}
          </button>
        ))}
      </PopoverContent>
...
function ModelDropdown({
  ...
  return (
    <Field label={t("agentConfigForm.labelsJsx.model")} hint={help.model}>
      <Popover
        open={open}
        ...
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          ...
          <div className="max-h-[240px] overflow-y-auto">
            ...
            {canCreateManualModel && (
              ...
              >
                <span>{t("agentConfigForm.text.useManualModel")}</span>
    ...
    <div className="rounded-md border border-border/70 bg-muted/20 p-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("agentConfigForm.text.cheapModel")}</div>
          <p className="text-xs text-muted-foreground">
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label={t("agentConfigForm.labelsJsx.thinkingEffort")} hint={help.thinkingEffort}>

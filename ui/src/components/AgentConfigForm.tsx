...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const {
    ...
    queryFn: () => {
      if (!selectedCompanyId) {
        throw new Error(t("agentConfig.selectCompanyToDetectModel"));
      }
  ...
  const translatedCodexThinkingEffortOptions = [
    { id: "", label: t("agentConfig.optionAuto") },
    { id: "minimal", label: t("agentConfig.optionMinimal") },
    { id: "low", label: t("agentConfig.optionLow") },
    { id: "medium", label: t("agentConfig.optionMedium") },
    { id: "high", label: t("agentConfig.optionHigh") },
    { id: "xhigh", label: t("agentConfig.optionXHigh") },
  ];
  const translatedOpenCodeThinkingEffortOptions = [
    { id: "", label: t("agentConfig.optionAuto") },
    { id: "minimal", label: t("agentConfig.optionMinimal") },
    { id: "low", label: t("agentConfig.optionLow") },
    { id: "medium", label: t("agentConfig.optionMedium") },
    { id: "high", label: t("agentConfig.optionHigh") },
    { id: "xhigh", label: t("agentConfig.optionXHigh") },
    { id: "max", label: t("agentConfig.optionMax") },
  ];
  const translatedCursorModeOptions = [
    { id: "", label: t("agentConfig.optionAuto") },
    { id: "plan", label: t("agentConfig.optionPlan") },
    { id: "ask", label: t("agentConfig.optionAsk") },
  ];
  const translatedClaudeThinkingEffortOptions = [
    { id: "", label: t("agentConfig.optionAuto") },
    { id: "low", label: t("agentConfig.optionLow") },
    { id: "medium", label: t("agentConfig.optionMedium") },
    { id: "high", label: t("agentConfig.optionHigh") },
  ...
  const thinkingEffortOptions =
    adapterType === "codex_local"
      ? translatedCodexThinkingEffortOptions
      : adapterType === "cursor"
        ? translatedCursorModeOptions
        : adapterType === "opencode_local"
          ? translatedOpenCodeThinkingEffortOptions
          : translatedClaudeThinkingEffortOptions;
    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            ...
            <Field label={t("common.title")} hint={help.title}>
              <DraftInput
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{t("common.adapters")}</h3>
            : <span className="text-xs font-medium text-muted-foreground">{t("common.adapters")}</span>
          }
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            ...
            <Field label={t("agentConfig.workingDirectoryDeprecated")} hint={help.cwd}>
              <div className="flex items-center gap-2 rounded-md border border-border px-2.5 py-1.5">
                ...
                <DraftInput
                  ...
                  placeholder={t("agentConfig.pathPlaceholder")}
                />
                <ChoosePathButton />
              </div>
            </Field>
          ...
          {isLocal && isCreate && (
            <>
              ...
              <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                {t("agentConfig.promptTemplateCreateWarning")} <code>{"{{ context.* }}"}</code> {t("agentConfig.promptTemplateWarningConnector")} <code>{"{{ run.* }}"}</code>.
              </div>
            </>
          )}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              <ModelDropdown
                ...
                redetectModelLabel={t("agentConfig.redetectModel")}
                emptyDetectHint={t("agentConfig.noModelDetectedSelectManually")}
              />
              {fetchedModelsError && (
              ...
              {!isCreate && (
                <>
                  ...
                  <Field label={t("agentConfig.interruptGracePeriodSec")} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              ...
              numberLabel={t("agentConfig.secondsShort")}
      ...
      <div className="mt-2 space-y-1.5">
        {result.checks.map((check, idx) => (
          <div key={`${check.code}-${idx}`} className="text-[11px] leading-relaxed break-words">
            ...
            {check.hint && <span className="block opacity-90 break-words">{t("agentConfig.hintPrefix", { hint: check.hint })}</span>}
          </div>
        ))}
      </div>
    </div>
        ...
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          <div className="relative mb-1">
            <input
              ...
              placeholder={creatable ? t("agentConfig.searchModelsCreate") : t("agentConfig.searchModels")}
              value={modelSearch}
          ...
          {onDetectModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel ? t("agentConfig.detecting") : detectedModel ? (redetectModelLabel ?? t("agentConfig.redetectModel")) : (detectModelLabel ?? t("agentConfig.detectModel"))}
            </button>
          )}
            ...
            {filteredModels.length === 0 && !canCreateManualModel && promotedModelIds.size === 0 && (
              <div className="px-2 py-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  {onDetectModel
                    ? (emptyDetectHint ?? t("agentConfig.noModelDetectedYet"))
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label={t("issue.thinkingEffort")} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{selected?.label ?? t("agentConfig.optionAuto")}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">

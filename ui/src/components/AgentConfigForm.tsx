...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const codexThinkingEffortOptions = useMemo(() => [
    { id: "", label: t('agentDetail.thinkingEffort_auto') },
    { id: "minimal", label: t('agentDetail.thinkingEffort_minimal') },
    { id: "low", label: t('agentDetail.thinkingEffort_low') },
    { id: "medium", label: t('agentDetail.thinkingEffort_medium') },
    { id: "high", label: t('agentDetail.thinkingEffort_high') },
    { id: "xhigh", label: t('agentDetail.thinkingEffort_xhigh') },
  ], [t]);

  const openCodeThinkingEffortOptions = useMemo(() => [
    { id: "", label: t('agentDetail.thinkingEffort_auto') },
    { id: "minimal", label: t('agentDetail.thinkingEffort_minimal') },
    { id: "low", label: t('agentDetail.thinkingEffort_low') },
    { id: "medium", label: t('agentDetail.thinkingEffort_medium') },
    { id: "high", label: t('agentDetail.thinkingEffort_high') },
    { id: "xhigh", label: t('agentDetail.thinkingEffort_xhigh') },
    { id: "max", label: t('agentDetail.thinkingEffort_max') },
  ], [t]);

  const cursorModeOptions = useMemo(() => [
    { id: "", label: t('agentDetail.thinkingEffort_auto') },
    { id: "plan", label: t('agentDetail.mode_plan') },
    { id: "ask", label: t('agentDetail.mode_ask') },
  ], [t]);

  const claudeThinkingEffortOptions = useMemo(() => [
    { id: "", label: t('agentDetail.thinkingEffort_auto') },
    { id: "low", label: t('agentDetail.thinkingEffort_low') },
    { id: "medium", label: t('agentDetail.thinkingEffort_medium') },
    { id: "high", label: t('agentDetail.thinkingEffort_high') },
    ...
    <div className={cn("relative", cards && "space-y-6")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("agentDetail.name")} hint={help.name}>
              <DraftInput
                ...
                placeholder={t("agentDetail.agentNamePlaceholder")}
              />
            </Field>
            <Field label={t("agentDetail.title")} hint={help.title}>
              <DraftInput
                ...
                immediate
                className={inputClass}
                placeholder={t("agentDetail.titlePlaceholder")}
              />
            </Field>
            <Field label={t("agentDetail.reportsTo")} hint={help.reportsTo}>
              <ReportsToPicker
                ...
                onChange={(v) => isCreate ? set!({ reportsTo: v }) : mark("identity", "reportsTo", v)}
                excludeAgentIds={isCreate ? [] : [props.agent.id]}
            ...
            <Field label={t("agentDetail.capabilities")} hint={help.capabilities}>
              <MarkdownEditor
                ...
                onChange={(v) => isCreate ? set!({ capabilities: v }) : mark("identity", "capabilities", v || null)}
      ...
      {environmentsEnabled ? (
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agentDetail.execution")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentDetail.execution")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
              label={t("agentDetail.defaultEnvironment")}
              hint={t("agentDetail.defaultEnvHint")}
            >
              <select
                className={inputClass}
              ...
              >
                <option value="">{t("agentDetail.companyDefaultLocal")}</option>
        ...
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          ...
          {showInlineAdapterTestEnvironmentButton && (
            <Button
              ...
              disabled={testEnvironmentDisabled}
            >
              {testEnvironment.isPending ? t("agentDetail.testing") : t("agentDetail.test")}
            </Button>
          )}
        </div>
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agentDetail.permissionsConfig")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentDetail.permissionsConfig")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              {supportsModelProfiles && (
                <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("agentDetail.primaryModel")}</div>
              )}
              <ModelDropdown
                models={models}
              ...
              {adapterType === "opencode_local"
                ...
                <p className="text-xs text-muted-foreground">
                  {t("agentDetail.openCodeDiscoveryHint", { name: currentDefaultEnvironment.name })}
                </p>
              )}
              ...
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label={t("agentDetail.bootstrapPromptLegacy")} hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                      ...
                      placeholder={t("agentDetail.bootstrapPromptPlaceholder")}
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    {t("agentDetail.bootstrapPromptLegacyWarning")}
                  </div>
                </>
              )}
              ...
              <Field label={t("agentDetail.extraArgs")} hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  immediate
                  className={inputClass}
                  placeholder={t("agentDetail.extraArgsPlaceholder")}
                />
              </Field>

              <Field label={t("agentDetail.environmentVariables")} hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
              ...
              {!isCreate && (
                <>
                  <Field label={t("agentDetail.timeoutSec")} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={t("agentDetail.interruptGracePeriod")} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      {isCreate && showCreateRunPolicySection ? (
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              label={t("agentDetail.heartbeatInterval")}
              hint={help.heartbeatInterval}
              checked={val!.heartbeatEnabled}
              onCheckedChange={(v) => set!({ heartbeatEnabled: v })}
              number={val!.intervalSec}
              onNumberChange={(v) => set!({ intervalSec: v })}
              numberLabel={t("agentDetail.sec")}
              numberPrefix={t("agentDetail.runHeartbeatEvery")}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <div className="space-y-3">
              <ToggleField
                label={t("agentDetail.wakeOnDemand")}
              ...
              />
              <Field label={t("agentDetail.cooldownSec")} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label={t("agentDetail.maxConcurrentRuns")} hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
              ...
              <div className="rounded-md border border-border/70 px-3 py-2">
                <ToggleField
                  label={t("agentDetail.continueAfterMaxTurnStop")}
                ...
                {maxTurnContinuationEnabled ? (
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <Field label={t("agentDetail.continuationAttempts")} hint={help.maxTurnContinuationMaxAttempts}>
                      <DraftNumberInput
                    ...
                    <Field label={t("agentDetail.continuationDelay")} hint={help.maxTurnContinuationDelaySec}>
                      <DraftNumberInput
...
export function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  const { t } = useTranslation();
  const statusLabel =
    result.status === "pass" ? t("agentDetail.passed") : result.status === "warn" ? t("agentDetail.warnings") : t("agentDetail.failed");
  const statusClass =
    result.status === "pass"
    ...
    <div className={`rounded-md border px-3 py-2 text-xs ${statusClass}`}>
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-[11px] opacity-80">
          {new Date(result.testedAt).toLocaleTimeString(t("common.timeLocale"))}
        </span>
      </div>
      <div className="mt-2 space-y-1.5">
        {result.checks.map((check, idx) => (
          <div key={`${check.code}-${idx}`} className="text-[11px] leading-relaxed break-words">
            ...
            {check.hint && <span className="block opacity-90 break-words">{t("agentDetail.hintPrefix")}{check.hint}</span>}
          </div>
        ))}
      </div>
    </div>
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
        {adapterList.map((item) => (
          ...
          >
            ...
            {item.comingSoon && (
              <span className="text-[10px] text-muted-foreground">{t("agentDetail.comingSoon")}</span>
            )}
          </button>
        ))}
      </PopoverContent>
...
function ModelDropdown({
  ...
  return (
    <Field label={t("agentDetail.model")} hint={help.model}>
      <Popover
        open={open}
          ...
          <button type="button" className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ? selected.label
                : value
                  || (allowDefault ? (defaultLabel ?? t("agentDetail.default")) : required ? t("agentDetail.selectModelRequired") : t("agentDetail.selectModel"))}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          <div className="relative mb-1">
            <input
              ...
              placeholder={creatable ? t("agentDetail.searchModels") + " " + t("agentDetail.typeToCreate") : t("agentDetail.searchModels")}
              value={modelSearch}
          ...
          {onDetectModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel ? t("agentDetail.detecting") : detectedModel ? (detectModelLabel?.replace(/^Detect\b/, t("agentDetail.reDetectLabel")) ?? t("agentDetail.reDetectFromConfig")) : (detectModelLabel ?? t("agentDetail.detectFromConfig"))}
            </button>
          )}
          {onRefreshModels && !modelSearch.trim() && (
            ...
            >
              ...
              {refreshingModels ? t("agentDetail.refreshing") : t("agentDetail.refreshModels")}
            </button>
          )}
...
function CheapModelSection({
  ...
  const placeholderHint = adapterDefaultModel
    ? `${t("agentDetail.adapterDefault")} · ${adapterDefaultModel}`
    : t("agentDetail.noAdapterDefaultCheaper");
  return (
    <div className="rounded-md border border-border/70 bg-muted/20 p-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("agentDetail.cheapModel")}</div>
          <p className="text-xs text-muted-foreground">
            {t("agentDetail.cheapModelHint")}
          </p>
        </div>
      ...
      {enabled && !model && adapterDefaultModel ? (
        <p className="text-[11px] text-muted-foreground">
          {t("agentDetail.noExplicitCheapModel")} <code>{adapterDefaultModel}</code>.
        </p>
      ) : null}
      {enabled && !model && !adapterDefaultModel ? (
        <p className="text-[11px] text-amber-500">
          {t("agentDetail.noCheapModelNoDefault")}
        </p>
      ) : null}
    </div>
  );
  ...
  return (
    <Field label={t("agentDetail.thinkingEffort")} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{selected?.label ?? t('agentDetail.thinkingEffort_auto')}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">

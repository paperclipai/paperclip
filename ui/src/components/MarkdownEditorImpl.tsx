...
export const MarkdownEditor = forwardRef<MarkdownEditorRef, MarkdownEditorProps>(function MarkdownEditor({
  ...
  return (
    <div
      ...
      onClick={() => {
        // MDXEditor's empty-state placeholder is rendered as a nested
        // paragraph. Treat the whole empty editor as the focus target so a
        // normal click on "Add description..." works without needing to hit
        // an invisible contenteditable edge.
        if (!readOnly && !editorValue.trim()) ref.current?.focus();
      }}
      onKeyDownCapture={(e) => {
        if (readOnly) return;

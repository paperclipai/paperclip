...
function BlockerRecoveryIndicator({
  ...
  return (
    <Badge variant="outline"
      ...
      title={detail
        ? t("localizationTaskRuntime.sourceTaskActionDetail", { label, detail })
: t("localizationTaskRuntime.sourceTaskAction", { label })}
...
? t("localizationTaskRuntime.scheduleDueNow")
: relative && dueAtIso
? t("localizationTaskRuntime.scheduledAt", { time: formatMonitorOffsetDisplay(dueAtIso) })
: t("localizationTaskRuntime.scheduled");
    ...
    <div className="mt-2 rounded-md border border-amber-300/70 bg-background/80 p-2 dark:border-amber-500/40 dark:bg-background/40">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0 text-xs leading-5 text-amber-900 dark:text-amber-100">
          {t("localizationTaskRuntime.retryNextStepScheduled", { schedule: scheduleLabel })}
        </div>
        <Button
          type="button"
          variant="outline"
        ...
        >
          {retryNow.isPending ? (
            <span className="inline-flex items-center gap-1.5">
              <Loader2 className="h-3.5 w-3.5 animate-spin" aria-hidden="true" />{t("onboarding.actions.retrying")}</span>
          ) : success ? (
            <span className="inline-flex items-center gap-1.5">
            ...
            <span className="inline-flex items-center gap-1.5">
              <RotateCcw className="h-3.5 w-3.5" aria-hidden="true" />{t("localizationIssueDetail.ui_Retry_now")}</span>
          )}
        </Button>
      </div>
      <RetryErrorBand
...
function waitingTaskStatusLabel(status: string): string {
  return t(`status.${status}`, { defaultValue: status.replace(/_/g, " ").replace(/\b\w/g, (character) => character.toUpperCase()) });
}
  ...
  return (
    ...
    >
      <StatusGlyph
        ...
        size="sm"
        title={t("localizationTaskRuntime.statusTitle", { status: waitingTaskStatusLabel(blocker.status) })}
      />
      ...
      {running ? (
        <span className="ml-0.5 rounded-full bg-blue-500/15 px-1.5 py-0.5 text-(length:--text-nano) font-medium uppercase tracking-wide text-blue-700 dark:bg-blue-400/20 dark:text-blue-200">{t("workspaces.runtime.status.running")}</span>
      ) : null}
    </IssueLinkQuicklook>
  );
}
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <p className="min-w-0 leading-5">
          {t("localizationTaskRuntime.ui_A_correction_run_is_in_progress_the_agent_is_working_This_alert_r_e087jh")}
          {shortRunId ? (
            <>
              {" "}
              {assigneeAgentId ? (
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="space-y-1">
            <p className="font-medium leading-5">{t("localizationTaskRuntime.ui_Waiting_on_live_work_dp4nby")}</p>
            <p className="leading-5">
              {t("localizationTaskRuntime.blockedQueue", { count: total })}
            </p>
          </div>

          <div className="space-y-1" data-testid="issue-blocked-notice-progress">
            <div className="text-xs font-medium text-blue-800 dark:text-blue-200">
              {t("localizationTaskRuntime.doneOfTotal", { done: doneCount, total })}
{runningCount > 0 ? ` · ${t("localizationTaskRuntime.runningCount", { count: runningCount })}` : null}
            </div>
            <div
              role="progressbar"
              aria-label={t("localizationTaskRuntime.ui_Blocker_chain_progress_ualgyv")}
              aria-valuemin={0}
            ...
            >
              {steps.map(({ blocker, status }) => (
                <span
                  ...
                  title={`${blocker.identifier ?? blocker.id.slice(0, 8)}: ${waitingTaskStatusLabel(status)}`}
                  aria-hidden
                />
              ))}
            </div>
            ...
            <div className="flex items-stretch gap-2">
              ...
              <div className="min-w-0 pb-0.5">
                <span className="inline-block rounded-md border border-dashed border-blue-300/70 px-2 py-1 text-xs text-blue-800 dark:border-blue-500/40 dark:text-blue-200">
                  {t("localizationTaskRuntime.ui_This_task_resumes_automatically_when_the_chain_is_done_15u6bfj")}
                </span>
              </div>
            </div>
          </div>
          ...
          {nowRunning.length > 0 ? (
            ...
            >
              <div className="text-xs font-medium text-blue-800 dark:text-blue-200">
                {t("localizationTaskRuntime.ui_Now_running_1ppzncs")}
              </div>
          ...
          {parkedBlockers.length > 0 ? (
            ...
            >
              <span className="inline-flex items-center gap-1 text-xs font-medium text-amber-800 dark:text-amber-200">
                <Flag className="h-3 w-3" aria-hidden />{t("localizationIssueLists.parkedBlocker")}</span>
...
export function IssueBlockedNotice({
  ...
  const blockerLabel = t("localizationTaskRuntime.linkedTask", { count: blockers.length });
  ...
  const responsibleName = agentName ?? t("localizationTaskRuntime.assignee");
  ...
  const reopenSuppressedLeafStatus = reopenSuppressedLeaf
    ? waitingTaskStatusLabel(reopenSuppressedLeaf.status).toLowerCase()
    : null;
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 space-y-1.5">
          {showSuccessfulRunHandoff ? (
            <>
              <p className="font-medium leading-5">{t("localizationTaskRuntime.ui_This_task_still_needs_a_next_step_1ds54rr")}</p>
              <p className="leading-5">
                {t("localizationTaskRuntime.ui_A_run_finished_successfully_but_the_task_is_still_open_Paperclip__15vck8g")}
              </p>
              <ul className="list-disc space-y-1 pl-5 text-xs leading-5 text-amber-900 dark:text-amber-100">
                <li>{t("localizationTaskRuntime.ui_Mark_it_done_or_cancelled_196ka53")}</li>
                <li>{t("localizationTaskRuntime.ui_Send_it_for_review_or_ask_for_input_1yvq2ii")}</li>
                <li>{t("localizationTaskRuntime.ui_Record_what_is_blocking_it_and_who_owns_that_blocker_n2lo5j")}</li>
                <li>{t("localizationTaskRuntime.ui_Delegate_follow_up_work_or_queue_a_continuation_1a6lr02")}</li>
              </ul>
              <div className="flex flex-wrap gap-1.5 text-xs">
                ...
                <span className="rounded-md border border-amber-300/70 bg-background/80 px-2 py-1 text-amber-900 dark:border-amber-500/40 dark:bg-background/40 dark:text-amber-100">
                  {t("localizationTaskRuntime.requestedNextStep", { name: responsibleName })}
                </span>
              </div>
              {successfulRunHandoff.detectedProgressSummary ? (
                <p className="text-xs leading-5 text-amber-800 dark:text-amber-200">
                  {t("localizationTaskRuntime.ui_Detected_progress_akfmm0")} {successfulRunHandoff.detectedProgressSummary}
                </p>
              ) : null}
          ...
          {blockers.length > 0 || issueStatus === "blocked" ? (
            <>
              <p className="leading-5">
                {blockers.length > 0
                  ? isStalled
                    ? t("localizationTaskRuntime.stalledReviews", { count: stalledLeafBlockers.length, blocker: blockerLabel })
                    : reopenSuppressed
                      ? t("localizationTaskRuntime.reopenSuppressed", { count: blockers.length, blocker: blockerLabel, responsible: responsibleName })
                      : t("localizationTaskRuntime.blockedWork", { count: blockers.length, blocker: blockerLabel })
                  : <>{t("localizationTaskRuntime.ui_Work_on_this_task_is_blocked_until_someone_moves_it_back_to_To_do_152cban")}</>}
              </p>
              {reopenSuppressed && reopenSuppressedLeafId ? (
                ...
                >
                  <Trans i18nKey="localizationTaskRuntime.stillBlocked" values={{
id: reopenSuppressedLeafId,
status: reopenSuppressedLeafStatus ? ` (${reopenSuppressedLeafStatus})` : "",
others: reopenSuppressedOtherCount > 0 ? t("localizationTaskRuntime.otherBlockers", { count: reopenSuppressedOtherCount }) : "",
}} components={{ id: <span className="font-mono" /> }} />
                </p>
              ) : null}
              {blockers.length > 0 ? (
              ...
              {showStalledRow ? (
                <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                  <span className="text-xs font-medium text-amber-800 dark:text-amber-200">
                    {t("localizationTaskRuntime.ui_Stalled_in_review_ldq2rt")}
                  </span>
                ...
                <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                  <span className="text-xs font-medium text-amber-800 dark:text-amber-200">
                    {t("localizationTaskRuntime.ui_Ultimately_waiting_on_qwqtbr")}
                  </span>

          ...
          <div className="min-w-0 space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="inline-flex items-center rounded-full border border-border/70 bg-background px-2.5 py-1 text-xs text-muted-foreground">
                {t("workspace.summary.updated_label", { time: timeAgo(summary.lastUpdatedAt) })}
              </span>
              {summary.serviceCount > 0 ? (
                ...
                >
                  ...
                  />
                  {t("workspace.summary.services_count", { running: summary.runningServiceCount, total: summary.serviceCount })}
                </span>
              ) : null}
          ...
          >
            {summary.hasRuntimeConfig ? (
              ...
              >
                ...
                {hasRunningServices ? t("workspace.summary.stop_services") : t("workspace.summary.start_services")}
              </Button>
            ) : null}
            {summary.kind === "execution_workspace" && summary.executionWorkspaceId && summary.executionWorkspaceStatus ? (
              ...
              >
                {summary.executionWorkspaceStatus === "cleanup_failed" ? t("workspace.summary.retry_close") : t("workspace.summary.close_workspace")}
              </Button>
            ) : null}
          </div>
        </div>
          ...
          <div className="space-y-2 text-sm">
            {summary.branchName ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("workspace.summary.branch")}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.branchName}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={t("workspace.summary.branch_copied")}
                    >
                      {summary.branchName}
                    </CopyText>
                    <CopyText
                      text={summary.branchName}
                      ariaLabel={t("workspace.summary.copy_branch_aria")}
            ...
            {summary.cwd ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("workspace.summary.path")}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.cwd}
                      title={summary.cwd}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={t("workspace.summary.path_copied")}
                    >
                      {truncatePath(summary.cwd)}
                    </CopyText>
                    <CopyText
                      text={summary.cwd}
                      ariaLabel={t("workspace.summary.copy_path_aria")}
                      className="mt-0.5 shrink-0 text-muted-foreground hover:text-foreground"
                      copiedLabel={t("workspace.summary.path_copied")}
                    >
                      <Copy className="h-3.5 w-3.5" />
            ...
            {summary.primaryServiceUrl ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("workspace.summary.service")}</div>
                  <a
        ...
        {summary.issues.length > 0 ? (
          <div className="space-y-2">
            <div className="text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground">
              {t("workspace.summary.linked_issues")}
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              {hiddenIssueCount > 0 ? (
                ...
                >
                  {t("workspace.summary.more_count", { count: hiddenIssueCount })}
                </Link>
              ) : null}
            </div>
          </div>

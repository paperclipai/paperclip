    ...
    <span className="relative inline-flex min-w-0 max-w-full">
      ...
      {copied ? (
        ...
        >
          ...
          {t("localizationActivityChrome.copied")}
        </span>
      ) : null}
    </span>
  );
    ...
    if (variant === "compact") {
      return (
        <CopyableCompactValue value={value} className="text-sm tabular-nums">
          {value.toLocaleString(i18n.resolvedLanguage ?? i18n.language, { maximumSignificantDigits: 21 })}
        </CopyableCompactValue>
      );
    }
    return <span className="text-sm tabular-nums">{value.toLocaleString(i18n.resolvedLanguage ?? i18n.language, { maximumSignificantDigits: 21 })}</span>;
  }

  if (typeof value === "boolean") {
    return value ? (
      <Check className="h-4 w-4 text-green-600 dark:text-green-400" aria-label={t("localizationActivityChrome.true")} />
    ) : (
      <EmptyValue />
    );
  }
  ...
  return (
    <section className="space-y-2">
      <div className="flex items-baseline gap-2">
        <h2 className="text-sm font-semibold">{t("localizationActivityChrome.fields")}</h2>
        <span className="text-xs text-muted-foreground">{t("localizationActivityChrome.fieldsDescription")}</span>
      </div>
      <Card className="gap-0 py-0">
        {entries.length === 0 ? (
          <div className="px-4 py-3 text-sm text-muted-foreground">{t("localizationActivityChrome.noFields")}</div>
        ) : (

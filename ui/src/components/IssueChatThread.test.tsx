...
describe("IssueChatThread", () => {
  ...
  it("does not apply a suggested assignee as an implicit reassignment", async () => {
    const root = createRoot(container);
      ...
      root.render(
        <MemoryRouter>
          <IssueChatThread
            ...
            timelineEvents={[]}
            liveRuns={[]}
            currentAssigneeValue="user:local-board"
            suggestedAssigneeValue="agent:agent-1"
            reassignOptions={[{ id: "agent:agent-1", label: "Agent One" }]}
            enableReassign
            onAdd={async () => {}}
            enableLiveTranscriptPolling={false}
          />
    ...
    expect(container.textContent).toContain("Handoff to Agent One");
    ...
    act(() => {
      ...
      valueSetter?.call(editor, "Plain reply");
      editor?.dispatchEvent(new Event("input", { bubbles: true }));
    });
  ...
  it("sends a reassignment when the suggested handoff action is chosen", async () => {
    const root = createRoot(container);
    ...
    const handoffButton = Array.from(container.querySelectorAll("button")).find(
      (element) => element.textContent === "Handoff to Agent One",
    ) as HTMLButtonElement | undefined;
    const editor = container.querySelector('textarea[aria-label="Task chat editor"]') as HTMLTextAreaElement | null;
    const submitButton = Array.from(container.querySelectorAll("button")).find(
      (element) => element.textContent === "Send",
    ...
    act(() => {
      ...
      valueSetter?.call(editor, "Please take this next");
      editor?.dispatchEvent(new Event("input", { bubbles: true }));
    });
    ...
    expect(appendMock).toHaveBeenCalledWith(
      expect.objectContaining({
        content: [{ type: "text", text: "Please take this next" }],
        runConfig: {
          custom: {
            reassignment: {

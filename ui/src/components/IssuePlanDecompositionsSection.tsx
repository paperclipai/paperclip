  ...
  if (status === "completed") {
    return (
      <span className="inline-flex items-center gap-1 rounded-sm border border-emerald-500/50 bg-emerald-500/10 px-2 py-0.5 text-(length:--text-micro) font-medium text-emerald-900 dark:text-emerald-100">
        ...
        {t("localizationIssueAux.ui_Completed_1tmo59u")}
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1 rounded-sm border border-amber-500/50 bg-amber-500/10 px-2 py-0.5 text-(length:--text-micro) font-medium text-amber-900 dark:text-amber-100">
      ...
      {t("localizationIssueAux.ui_In_flight_1p0n5rs")}
    </span>
  );
}
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-sm font-medium text-muted-foreground">{t("localizationIssueAux.ui_Plan_decomposition_1kzybsl")}</h3>
        <span className="text-(length:--text-micro) text-muted-foreground/80">
          {t("localizationIssueAux.acceptedPlans", { count: items.length })}
        </span>
      </div>
      ...
      <ul className="space-y-3">
        {items.map((record) => {
          ...
          const ownerName = record.ownerAgentId
            ? agentMap?.get(record.ownerAgentId)?.name ?? t("localizationIssueAux.agentLower")
            : null;
          const revisionLabel = t("localizationIssueAux.planRevision", {
            revision: record.acceptedPlanRevisionNumber ?? record.acceptedPlanRevisionId.slice(0, 8),
          });
          const completedAt =
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1 text-xs text-foreground">
                  ...
                  {t("localizationIssueAux.childTasksCreated", { created, count: requested })}
                </span>
                {record.status === "completed" && requested > 0 ? (
                  <span
                    ...
                    title={t("localizationIssueAux.ui_Repeat_attempts_with_this_fingerprint_reuse_this_record_instead_o_1ict6ao")}
                  >
                    <Repeat className="h-3 w-3" />
                    {t("localizationIssueAux.ui_Idempotent_claim_1bhb9ja")}
                  </span>
                ) : null}
              </div>

              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-(length:--text-micro) text-muted-foreground">
                {ownerName ? <span>{t("localizationIssueAux.owner", { name: ownerName })}</span> : null}
                {startedAt ? (
                  <span title={formatDateTime(startedAt)}>{t("localizationIssueAux.started", { time: relativeTime(startedAt) })}</span>
                ) : null}
                {completedAt ? (
                  <span title={formatDateTime(completedAt)}>{t("localizationIssueAux.completed", { time: relativeTime(completedAt) })}</span>
                ) : updatedAt ? (
                  <span title={formatDateTime(updatedAt)}>{t("localizationIssueAux.updated", { time: relativeTime(updatedAt) })}</span>
                ) : null}
                {issueIdentifier ? (
                  <Link
                  ...
                  >
                    {t("localizationIssueAux.ui_Plan_document_u2klt7")}
                  </Link>
                ) : null}
              </div>

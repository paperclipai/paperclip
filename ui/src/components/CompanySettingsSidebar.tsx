    ...
    <aside className="w-full h-full min-h-0 border-r border-border bg-background flex flex-col">
      <div className="flex flex-col gap-1 px-3 py-3 shrink-0">
        ...
        >
          ...
          <span className="truncate">{selectedCompany?.name ?? t("companySettingsNav.company")}</span>
        </Link>
        <div className="flex items-center gap-2 px-2 py-1">
          <Settings className="h-4 w-4 text-muted-foreground shrink-0" />
          <span className="flex-1 truncate text-sm font-bold text-foreground">
            {t("companySettingsNav.companySettingsTitle")}
          </span>
        </div>
      </div>
      ...
      <nav className="flex-1 min-h-0 overflow-y-auto scrollbar-auto-hide px-3 py-2">
        <div className="px-3 pb-1 text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
          {t("companySettingsNav.companySettings")}
        </div>
        <div className="flex flex-col gap-0.5">
          <SidebarNavItem to="/company/settings" label={t("companySettingsNav.general")} icon={SlidersHorizontal} end />
          <SidebarNavItem to="/company/export" label={t("companySettingsNav.export")} icon={Download} />
          <SidebarNavItem to="/company/import" label={t("companySettingsNav.import")} icon={Upload} end />
          <SidebarNavItem
            to="/company/settings/members"
            label={t("companySettingsNav.members")}
            icon={Users}
          ...
          <SidebarNavItem to="/company/settings/invites" label={t("companySettingsNav.invites")} icon={MailPlus} end />
          <SidebarNavItem to="/company/settings/secrets" label={t("companySettingsNav.secrets")} icon={KeyRound} end />
        </div>
        <div className="mt-5 px-3 pb-1 text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
          {t("companySettingsNav.instanceSettings")}
        </div>
        <div className="flex flex-col gap-0.5">
          <SidebarNavItem
            to={`${INSTANCE_SETTINGS_PATH_PREFIX}/profile`}
            label={t("companySettingsSidebar.profile")}
            icon={UserRoundPen}
            end
          />
          <SidebarNavItem
            to={`${INSTANCE_SETTINGS_PATH_PREFIX}/general`}
            label={t("companySettingsSidebar.general")}
          ...
          <SidebarNavItem
            ...
            label={t("companySettingsSidebar.heartbeats")}
            icon={Clock3}
            end
          />
          <SidebarNavItem
            to={`${INSTANCE_SETTINGS_PATH_PREFIX}/experimental`}
            label={t("companySettingsSidebar.experimental")}
            icon={FlaskConical}
          />
          <SidebarNavItem
            to={`${INSTANCE_SETTINGS_PATH_PREFIX}/plugins`}
            label={t("companySettingsSidebar.plugins")}
            icon={Puzzle}
          />
          ...
          <SidebarNavItem
            ...
            label={t("companySettingsSidebar.adapters")}
            icon={Cpu}
          />
        </div>
      </nav>

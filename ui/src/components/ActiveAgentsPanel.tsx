  ...
  return (
    <div>
      <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">
        {t("activeAgentsPanel.title")}
      </h3>
      {runs.length === 0 ? (
        <div className="rounded-xl border border-border p-4">
          <p className="text-sm text-muted-foreground">{t("activeAgentsPanel.empty")}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 sm:gap-4 xl:grid-cols-4">
          {runs.map((run) => (
            <AgentRunCard
              ...
              liveNowLabel={t("activeAgentsPanel.liveNow")}
              finishedLabel={(time) => t("activeAgentsPanel.finishedRelative", { time })}
              startedLabel={(time) => t("activeAgentsPanel.startedRelative", { time })}
            />
          ))}
        </div>
      )}

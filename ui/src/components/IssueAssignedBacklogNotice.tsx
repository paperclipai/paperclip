...
export function IssueAssignedBacklogNotice({
  ...
  const assigneeLabel = assigneeAgent?.name ?? t("component.issue.backlog.theAssignee");
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1 space-y-1.5">
          <p className="leading-5">
            <span className="font-medium">{t("component.issue.backlog.parked")}</span> —{" "}
            <span className="font-medium">{assigneeLabel}</span> {t("component.issue.backlog.willNotWake")}
            <code className="rounded bg-amber-100 px-1 py-0.5 text-[12px] dark:bg-amber-400/15">todo</code> {t("component.issue.backlog.or")}
          ...
          {onResume ? (
            <div className="pt-0.5">
              ...
              >
                {resuming ? t("component.issue.backlog.resuming") : t("component.issue.backlog.resumeNow")}
              </Button>
            </div>
          ) : null}
        </div>

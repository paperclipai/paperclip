  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger className="flex w-full items-center justify-between rounded-lg border border-border/70 px-3 py-2 text-left">
        <div>
          <p className="text-sm font-medium">{t("routine_variables.variables")}</p>
          <p className="text-xs text-muted-foreground">
            {t("routine_variables.detected_hint")}
          </p>
        </div>
          ...
          <div key={variable.name} className="rounded-lg border border-border/70 p-4">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                {t("routine_variables.prompt_hint")}
              </span>
            </div>

            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routine_variables.label")}</Label>
                <Input
              ...
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routine_variables.type")}</Label>
                <Select
                  value={variable.type}
              ...
              <div className="space-y-1.5 md:col-span-2">
                <div className="flex items-center justify-between gap-3">
                  <Label className="text-xs">{t("routine_variables.default_value")}</Label>
                ...
                {variable.type === "textarea" ? (
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="__unset__">{t("routine_variables.no_default")}</SelectItem>
                      <SelectItem value="true">{t("routine_variables.boolean_true")}</SelectItem>
                      <SelectItem value="false">{t("routine_variables.boolean_false")}</SelectItem>
                    </SelectContent>
                  </Select>
                ) : variable.type === "select" ? (
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("routine_variables.options")}</Label>
                      <Input
                        ...
                        placeholder={t("routine_variables.options_placeholder")}
                      />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("routine_variables.default_option")}</Label>
                      <Select
                      ...
                      >
                        <SelectTrigger>
                          <SelectValue placeholder={t("routine_variables.no_default")} />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="__unset__">{t("routine_variables.no_default")}</SelectItem>
                  ...
                  <Input
                    ...
                    placeholder={variable.type === "number" ? "42" : t("routine_variables.default_value")}
                  />
                )}
              </div>
            </div>

...
function getProjectStatuses(t: (key: string) => string) {
  return [
    { value: "backlog", label: t("newProjectDialog.statuses.backlog") },
    { value: "planned", label: t("newProjectDialog.statuses.planned") },
    { value: "in_progress", label: t("newProjectDialog.statuses.in_progress") },
...
export function NewProjectDialog() {
  ...
  const projectStatuses = useMemo(() => getProjectStatuses(t), [t]);
  ...
  const deriveWorkspaceNameFromPath = (value: string) => {
    ...
    return segments[segments.length - 1] ?? t("newProjectDialog.fallbackLocalFolder");
  };
  ...
  const deriveWorkspaceNameFromRepo = (value: string) => {
    try {
      ...
      return repo || t("newProjectDialog.fallbackRepo");
    } catch {
      return t("newProjectDialog.fallbackRepo");
    }
  };
  ...
  async function handleSubmit() {
    ...
    if (localPath && !isAbsolutePath(localPath)) {
      setWorkspaceError(t("newProjectDialog.errors.absolutePathRequired"));
      return;
    }
    if (repoUrl && !looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("newProjectDialog.errors.invalidRepoUrl"));
      return;
    }
        ...
        <div className="px-4 pt-3 pb-3 space-y-3 border-t border-border">
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("newProjectDialog.repoUrlLabel")}</label>
              ...
              <Tooltip delayDuration={300}>
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("newProjectDialog.repoUrlTooltip")}
                </TooltipContent>
              </Tooltip>
            </div>
            <input
          ...
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("newProjectDialog.localFolderLabel")}</label>
              <span className="text-xs text-muted-foreground/50">{t("newProjectDialog.optional")}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("newProjectDialog.localFolderTooltip")}
                </TooltipContent>
              </Tooltip>
            </div>
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap">
          ...
          <div className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs">
            ...
            <input
              ...
              placeholder={t("newProjectDialog.targetDatePlaceholder")}
            />
          </div>
        </div>

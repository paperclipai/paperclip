...
export function NewProjectDialog() {
  ...
  const projectStatuses = [
    { value: "backlog", label: t("projects:statuses.backlog") },
  ...
  async function handleSubmit() {
    ...
    if (localPath && !isAbsolutePath(localPath)) {
      setWorkspaceError(t("projects:newDialog.localFolderMustBeAbsolute"));
      return;
    }
    if (repoUrl && !isGitHubRepoUrl(repoUrl)) {
      setWorkspaceError(t("projects:newDialog.repoMustBeValidGithub"));
      return;
    }
    ...
    >
      ...
      >
        ...
        <div className="px-4 pt-3 pb-3 space-y-3 border-t border-border">
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("projects:newDialog.repoUrl")}</label>
              <span className="text-xs text-muted-foreground/50">{t("common:labels.optional", "optional")}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                </TooltipTrigger>
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("projects:newDialog.repoUrlHint")}
                </TooltipContent>
              </Tooltip>
            </div>
            <input
          ...
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("projects:newDialog.localFolder")}</label>
              <span className="text-xs text-muted-foreground/50">{t("common:labels.optional", "optional")}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("projects:newDialog.localFolderHint")}
                </TooltipContent>
              </Tooltip>
            </div>
            ...
            <PopoverContent className="w-56 p-1" align="start">
              ...
              {selectedGoals.length > 0 && availableGoals.length === 0 && (
                <div className="px-2 py-1.5 text-xs text-muted-foreground">
                  {t("projects:newDialog.allGoalsSelected", "All goals already selected.")}
                </div>
              )}
            </PopoverContent>
          </Popover>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          ...
          >
            {createProject.isPending ? t("common:creating") : t("projects:newDialog.createProject")}
          </Button>
        </div>
      </DialogContent>
    </Dialog>

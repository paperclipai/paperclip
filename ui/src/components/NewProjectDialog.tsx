...
const projectStatuses = [
  { value: "backlog", i18nKey: "backlog" },
  { value: "planned", i18nKey: "planned" },
  { value: "in_progress", i18nKey: "inProgress" },
  { value: "completed", i18nKey: "completed" },
...
export function NewProjectDialog() {
  ...
  async function handleSubmit() {
    ...
    if (localPath && !isAbsolutePath(localPath)) {
      setWorkspaceError(t("dialog.localFolderAbsoluteError"));
      return;
    }
    if (repoUrl && !looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("dialog.repoUrlError"));
      return;
    }
      ...
      >
        ...
        <div className="px-4 pt-3 pb-3 space-y-3 border-t border-border">
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("dialog.repoUrl")}</label>
              <span className="text-xs text-muted-foreground/50">{t("dialog.optional")}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                </TooltipTrigger>
                <TooltipContent side="top" className="max-w-(--sz-240px) text-xs">
                  {t("dialog.repoUrlTooltip")}
                </TooltipContent>
              </Tooltip>
            </div>
            <input
          ...
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("dialog.localFolder")}</label>
              <span className="text-xs text-muted-foreground/50">{t("dialog.optional")}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap">
          ...
          <Popover open={statusOpen} onOpenChange={setStatusOpen}>
            ...
            <PopoverContent className="w-40 p-1" align="start">
              {projectStatuses.map((s) => (
                ...
                >
                  {t(`dialog.projectStatus.${s.i18nKey}`)}
                </button>
              ))}
            </PopoverContent>
          </Popover>
          ...
          {selectedGoals.map((goal) => (
            ...
            >
              ...
              <button
                ...
                aria-label={t("dialog.removeGoal", { title: goal.title })}
                type="button"
              >
                <X className="h-3 w-3" />
              </button>
          ...
          <Popover open={goalOpen} onOpenChange={setGoalOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {selectedGoals.length > 0 ? t("dialog.addGoal") : t("dialog.goal")}
              </button>
            </PopoverTrigger>
          ...
          <div className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs">
            ...
            <input
              ...
              placeholder={t("dialog.targetDate")}
            />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{t("dialog.failedCreateProject")}</p>
          ) : (
            <span />
          )}
          <Button
          ...
          >
            {createProject.isPending ? t("dialog.creating") : t("dialog.createProject")}
          </Button>
        </div>
      </DialogContent>
    </Dialog>

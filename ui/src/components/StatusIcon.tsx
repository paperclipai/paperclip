...
function blockedAttentionLabel(blockerAttention: IssueBlockerAttention | null | undefined) {
  ...
  if (blockerAttention.reason === "active_child") {
    ...
    if (count === 1 && blockerAttention.sampleBlockerIdentifier) {
      return t("localizationIssuePanels.blockedChildNamed", { identifier: blockerAttention.sampleBlockerIdentifier });
    }
    return t("localizationIssuePanels.blockedChild", { count });
  }

  if (blockerAttention.reason === "active_dependency") {
    const count = blockerAttention.coveredBlockerCount;
    if (count === 1 && blockerAttention.sampleBlockerIdentifier) {
      return t("localizationIssuePanels.blockedDependencyNamed", { identifier: blockerAttention.sampleBlockerIdentifier });
    }
    return t("localizationIssuePanels.blockedDependency", { count });
  }
  ...
  if (blockerAttention.reason === "stalled_review") {
    ...
    if (count === 1 && leaf) return t("localizationIssuePanels.blockedReviewNamed", { identifier: leaf });
    return t(count === 1 ? "localizationIssuePanels.blockedReviewSingle" : "localizationIssuePanels.blockedReview", { count });
  }
  ...
  if (blockerAttention.reason === "attention_required") {
    ...
    return coveredCount > 0
      ? t("localizationIssuePanels.blockedAttentionCovered", { count, covered: coveredCount })
      : t("localizationIssuePanels.blockedAttention", { count });
  }
...
export function StatusIcon({ status, blockerAttention, onChange, className, showLabel, size = "md" }: StatusIconProps) {
  ...
  const trigger = showLabel ? (
    <button
      type="button"
      aria-label={t("localizationIssuePanels.changeStatus", { status: ariaLabel })}

    ...
    <>
      ...
      {collectingDownvoteReason ? (
        <div className="mt-2 rounded-md border border-border/60 bg-accent/20 p-3">
          <div className="mb-2 text-sm font-medium">{t("outputFeedback.whatCouldBeBetter")}</div>
          <Textarea
            value={downvoteReason}
            onChange={(event) => setDownvoteReason(event.target.value)}
            placeholder={t("outputFeedback.addShortNote")}
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("outputFeedback.savePreferenceTitle")}</DialogTitle>
            <DialogDescription>
              {t("outputFeedback.savePreferenceDescription")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm text-muted-foreground">
            <p>{t("outputFeedback.voteSavedLocally")}</p>
            <p>{t("outputFeedback.sharingExplanation")}</p>
            <p>{t("outputFeedback.changeLater")}</p>
            {termsUrl ? (
              <a
                href={termsUrl}
                target="_blank"
          ...
          <DialogFooter>
            ...
            >
              {isSaving ? t("outputFeedback.saving") : t("outputFeedback.allowSharing")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

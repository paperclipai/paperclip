    ...
    <>
      <div className={cn(
        ...
        >
          ...
          {t("pages.outputFeedback.helpful")}
        </Button>
        <Button
          type="button"
          size="sm"
        ...
        >
          ...
          {t("pages.outputFeedback.needsWork")}
        </Button>
        {rightSlot ? <div className="ml-auto">{rightSlot}</div> : null}
      </div>
      {collectingDownvoteReason ? (
        <div className="mt-2 rounded-md border border-border/60 bg-accent/20 p-3">
          <div className="mb-2 text-sm font-medium">{t("pages.outputFeedback.whatBetter")}</div>
          <Textarea
            value={downvoteReason}
            onChange={(event) => setDownvoteReason(event.target.value)}
            placeholder={t("pages.outputFeedback.addNote")}
          ...
          <div className="mt-3 flex items-center justify-end gap-2">
            ...
            >
              {t("pages.outputFeedback.dismiss")}
            </Button>
            <Button
              type="button"
              size="sm"
            ...
            >
              {isSaving ? t("pages.outputFeedback.saving") : t("pages.outputFeedback.saveNote")}
            </Button>
          </div>
        </div>
      ) : null}
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("pages.outputFeedback.feedbackSharing")}</DialogTitle>
            <DialogDescription>
              {t("pages.outputFeedback.feedbackSharingDesc")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm text-muted-foreground">
            <p>
              {t("pages.outputFeedback.voteSavedLocally")}
            </p>
            <p>
              {t("pages.outputFeedback.alwaysAllow")} <span className="font-medium text-foreground">{t("pages.outputFeedback.alwaysAllow")}</span>{" "}
              {t("pages.outputFeedback.alwaysAllow")}
            </p>
            <p>
              {t("pages.outputFeedback.changeLater")}
            </p>
            {termsUrl ? (
              <a
                href={termsUrl}
              ...
              >
                {t("pages.outputFeedback.readTerms")}
              </a>
            ) : null}
          </div>
          <DialogFooter>
            ...
            >
              {isSaving ? t("pages.outputFeedback.saving") : t("pages.outputFeedback.dontAllow")}
            </Button>
            <Button
              type="button"
              disabled={!pendingVote || isSaving}
            ...
            >
              {isSaving ? t("pages.outputFeedback.saving") : t("pages.outputFeedback.alwaysAllow")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

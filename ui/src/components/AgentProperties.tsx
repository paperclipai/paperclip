  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t('projects.status')}>
          <StatusBadge status={agent.status} />
        </PropertyRow>
        <PropertyRow label={t('agentDetail.role')}>
          <span className="text-sm">{roleLabels[agent.role] ?? agent.role}</span>
        </PropertyRow>
        {agent.title && (
          <PropertyRow label={t('agentDetail.title')}>
            <span className="text-sm">{agent.title}</span>
          </PropertyRow>
        )}
        <PropertyRow label={t('agentDetail.adapter')}>
      ...
      <div className="space-y-1">
        {(runtimeState?.sessionDisplayId ?? runtimeState?.sessionId) && (
          <PropertyRow label={t('agentDetail.session')}>
        ...
        <PropertyRow label={t('projects.created')}>

  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t("app.agentProperties.status", { defaultValue: "Status" })}>
          <AgentStatusBadge status={agent.status} />
        </PropertyRow>
        {lastErrorIsActive && agent.errorReason && (
          <PropertyRow label={t("app.agentProperties.errorReason", { defaultValue: "Error reason" })}>
            <span className="text-xs text-red-600 dark:text-red-400 break-words min-w-0">
              {agent.errorReason}
            </span>
          </PropertyRow>
        )}
        <PropertyRow label={t("app.agentProperties.role", { defaultValue: "Role" })}>
          <span className="text-sm">{roleLabels[agent.role] ?? agent.role}</span>
        </PropertyRow>
        {agent.title && (
          <PropertyRow label={t("app.agentProperties.title", { defaultValue: "Title" })}>
            <span className="text-sm">{agent.title}</span>
          </PropertyRow>
        )}
        <PropertyRow label={t("app.agentProperties.adapter", { defaultValue: "Adapter" })}>
      ...
      <div className="space-y-1">
        {(runtimeState?.sessionDisplayId ?? runtimeState?.sessionId) && (
          <PropertyRow label={t("app.agentProperties.session", { defaultValue: "Session" })}>
        ...
        {agent.lastHeartbeatAt && (
          <PropertyRow label={t("app.agentProperties.lastHeartbeat", { defaultValue: "Last Heartbeat" })}>
            <span className="text-sm">{formatDate(agent.lastHeartbeatAt)}</span>
          </PropertyRow>
        )}
        {agent.reportsTo && (
          <PropertyRow label={t("app.agentProperties.reportsTo", { defaultValue: "Reports To" })}>
            {reportsToAgent ? (
        ...
        <PropertyRow label={t("app.agentProperties.created", { defaultValue: "Created" })}>

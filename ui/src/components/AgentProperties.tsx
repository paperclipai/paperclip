    ...
    <div className="space-y-4">
      ...
      <div className="space-y-1">
        ...
        {runtimeState?.lastError && (
          <PropertyRow label={i18n.t("components.AgentProperties.label")}>
            <span className="text-xs text-red-600 dark:text-red-400 break-words min-w-0">{runtimeState.lastError}</span>
          </PropertyRow>
        )}
        {agent.lastHeartbeatAt && (
          <PropertyRow label={i18n.t("components.AgentProperties.label_1")}>
            <span className="text-sm">{formatDate(agent.lastHeartbeatAt)}</span>
          </PropertyRow>
        )}
        {agent.reportsTo && (
          <PropertyRow label={i18n.t("components.AgentProperties.label_2")}>
            {reportsToAgent ? (

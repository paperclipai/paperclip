...
const quickFilterPresets = [
  ...
  { key: "active", statuses: ["todo", "in_progress", "in_review", "blocked"] },
  ...
  return (
    <div className="relative w-48 sm:w-64 md:w-80">
      ...
      <Input
        ...
        placeholder={t("issues:filterBar.searchPlaceholder")}
        className="pl-7 text-xs sm:text-sm"
        aria-label={t("issues:filterBar.searchPlaceholder")}
      />
    </div>
  );
}
...
export function IssuesList({
  ...
  const groupedContent = useMemo(() => {
    const statusLabel = (status: string) => t(`issues:statuses.${status}`, statusLabelFallback(status));
    const priorityLabel = (priority: string) => t(`common:priorityValues.${priority}`, statusLabelFallback(priority));
    ...
    return Object.keys(groups).map((key) => ({
      ...
      label:
        key === "__unassigned"
          ? t("issues:list.unassigned")
          : key.startsWith("__user:")
            ? (formatAssigneeUserLabel(key.slice("__user:".length), currentUserId) ?? t("issues:list.user"))
  ...
  return (
    <div className="space-y-4">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                <span className="hidden sm:inline">{activeFilterCount > 0 ? t("issues:filterBar.filtersCount", { count: activeFilterCount }) : t("issues:filterBar.filter")}</span>
                {activeFilterCount > 0 && (
            ...
            <PopoverContent align="end" className="w-[min(480px,calc(100vw-2rem))] p-0">
              <div className="p-3 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">{t("issues:filterBar.filter")}</span>
                  {activeFilterCount > 0 && (
                    <button
                      className="text-xs text-muted-foreground hover:text-foreground"
                      onClick={() => updateView({ statuses: [], priorities: [], assignees: [], labels: [] })}
                    >
                      {t("issues:filterBar.clear")}
                    </button>
                  )}
                </div>
                ...
                <div className="space-y-1.5">
                  <span className="text-xs text-muted-foreground">{t("issues:filterBar.quickFiltersLabel")}</span>
                  <div className="flex flex-wrap gap-1.5">
                    {quickFilterPresets.map((preset) => {
                      ...
                      return (
                        ...
                        >
                          {t(`issues:quickFilters.${preset.key}`)}
                        </button>
                      );
                    })}
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                  ...
                  <div className="space-y-1">
                    <span className="text-xs text-muted-foreground">{t("issues:filterBar.status")}</span>
                    <div className="space-y-0.5">
                      {statusOrder.map((s) => (
                        <label key={s} className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                          ...
                          />
                          <StatusIcon status={s} />
                          <span className="text-sm">{t(`issues:statuses.${s}`, statusLabelFallback(s))}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    {/* Priority */}
                    <div className="space-y-1">
                      <span className="text-xs text-muted-foreground">{t("issues:filterBar.priority")}</span>
                      <div className="space-y-0.5">
                        {priorityOrder.map((p) => (
                          <label key={p} className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                            ...
                            <span className="text-sm">{t(`common:priorityValues.${p}`, statusLabelFallback(p))}</span>
                          </label>
                        ))}
                      </div>
                    </div>
                    ...
                    {/* Assignee */}
                    <div className="space-y-1">
                      <span className="text-xs text-muted-foreground">{t("issues:filterBar.assignee")}</span>
                      <div className="space-y-0.5 max-h-32 overflow-y-auto">
                        <label className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                          ...
                          />
                          <span className="text-sm">{t("issues:list.noAssignee")}</span>
                        </label>
                        {currentUserId && (
                          <label className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                            <Checkbox
                            ...
                            <span className="text-sm">{t("issues:list.me")}</span>
                          </label>
                        )}
                        {(agents ?? []).map((agent) => (
                    ...
                    {labels && labels.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">{t("issues:filterBar.labels")}</span>
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
                          {labels.map((label) => (
                    ...
                    {projects && projects.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">{t("issues:filterBar.project")}</span>
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="sm" className="text-xs">
                  ...
                  <span className="hidden sm:inline">{t("issues:filterBar.sort")}</span>
                </Button>
              </PopoverTrigger>
              <PopoverContent align="end" className="w-48 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["status", t("issues:filterBar.status")],
                    ["priority", t("issues:filterBar.priority")],
                    ["title", t("issues:filterBar.title")],
                    ["created", t("issues:filterBar.created")],
                    ["updated", t("issues:filterBar.updated")],
                  ] as ["status" | "priority" | "title" | "created" | "updated", string][]).map(([field, label]) => (
                    <button
                      key={field}
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="sm" className="text-xs">
                  ...
                  <span className="hidden sm:inline">{t("issues:filterBar.group")}</span>
                </Button>
              </PopoverTrigger>
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["status", t("issues:filterBar.status")],
                    ["priority", t("issues:filterBar.priority")],
                    ["assignee", t("issues:filterBar.assignee")],
                    ["none", t("issues:filterBar.none")],
                  ] as ["status" | "priority" | "assignee" | "none", string][]).map(([value, label]) => (
                    <button
                      key={value}
                      className={`flex items-center justify-between w-full px-2 py-1.5 text-sm rounded-sm ${
      ...
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("issues:list.noMatch")}
          action={t("issues:list.createIssue")}
                      ...
                      >
                          ...
                          >
                            {issue.assigneeAgentId && agentName(issue.assigneeAgentId) ? (
                              ...
                              <span className="inline-flex items-center gap-1.5 text-xs">
                                ...
                                {formatAssigneeUserLabel(issue.assigneeUserId, currentUserId) ?? t("issues:list.user")}
                              </span>
                            ) : (
                        ...
                        >
                          <input
                            ...
                            placeholder={t("issues:list.searchAssignees")}

...
function getIssueStatusLabels(): Record<IssueStatus, string> {
  return {
    backlog: t("components.issuesList.statusBacklog", { defaultValue: "Backlog" }),
    todo: t("components.issuesList.statusTodo", { defaultValue: "Todo" }),
    in_progress: t("components.issuesList.statusInProgress", { defaultValue: "In progress" }),
    in_review: t("components.issuesList.statusInReview", { defaultValue: "In review" }),
    done: t("components.issuesList.statusDone", { defaultValue: "Done" }),
    blocked: t("components.issuesList.statusBlocked", { defaultValue: "Blocked" }),
    cancelled: t("components.issuesList.statusCancelled", { defaultValue: "Cancelled" }),
  };
}
  ...
  return (
    <div className="relative w-48 sm:w-64 md:w-80">
      ...
      <Input
        ...
        placeholder={t("components.issuesList.searchTasksPlaceholder", { defaultValue: "Search tasks..." })}
        className="pl-7 text-xs sm:text-sm"
        aria-label={t("components.issuesList.searchTasksAriaLabel", { defaultValue: "Search tasks" })}
        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
            <span className="font-medium text-foreground">
              {t("components.issuesList.progressDoneCount", {
            ...
            <span className="text-muted-foreground">
              {t("components.issuesList.progressInProgressCount", {
                count: summary.inProgressCount,
                defaultValue: "{{count}} in progress",
              })}
            </span>
            <span className="text-muted-foreground">
              {t("components.issuesList.progressBlockedCount", {
            ...
            {showCostSummary && (
              <>
                <span
                  ...
                  title={t("components.issuesList.costSummaryRunsAcrossSubtasks", {
                    runs: t("components.issuesList.costSummaryRunCount", {
                      count: costSummary.runCount,
                      formattedCount: costSummary.runCount.toLocaleString(),
                      defaultValue: "{{formattedCount}} run",
                      defaultValue_other: "{{formattedCount}} runs",
                    }),
                    subtasks: t("components.issuesList.costSummarySubtaskCount", {
                      count: costSummary.issueCount,
                      defaultValue: "{{count}} sub-task",
                      defaultValue_other: "{{count}} sub-tasks",
                    }),
                    defaultValue: "{{runs}} across {{subtasks}}",
                  })}
                >
                  {t("components.issuesList.tokensLabel", {
                    tokens: formatTokens(totalTokens),
                    defaultValue: "{{tokens}} tokens",
                  })}
                </span>
                <span className="text-muted-foreground tabular-nums">
                  {t("components.issuesList.runtimeLabel", {
          ...
          >
            {statusEntries.map(({ status, count }) => (
              <span
                ...
                title={t("components.issuesList.statusSegmentTitle", {
                  status: getIssueStatusLabels()[status],
                  count,
          ...
          {target && targetIssue ? (
            <>
              <div className="text-xs font-medium text-muted-foreground">
                {target.kind === "next"
                  ? t("components.issuesList.nextUp", { defaultValue: "Next up" })
                  : t("components.issuesList.waitingOnBlockers", { defaultValue: "Waiting on blockers" })}
              </div>
              <Link
...
export function IssuesList({
    ...
    if (currentUserId) {
      options.set(`user:${currentUserId}`, {
        ...
        label: currentUserId === "local-board"
          ? t("components.issuesList.board", { defaultValue: "Board" })
          : t("components.issuesList.me", { defaultValue: "Me" }),
        kind: "user",
  ...
  const groupedContent = useMemo(() => {
    ...
    if (viewState.groupBy === "workspace") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_workspace" ? t("components.issuesList.noWorkspace", { defaultValue: "No Workspace" }) : (workspaceNameMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "project") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_project" ? t("components.issuesList.noProject", { defaultValue: "No Project" }) : (projectById.get(key)?.name ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "parent") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_parent" ? t("components.issuesList.noParent", { defaultValue: "No Parent" }) : (issueTitleMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    // assignee
    ...
    return Object.keys(groups).map((key) => ({
      ...
      label:
        key === "__unassigned"
          ? t("components.issuesList.unassigned", { defaultValue: "Unassigned" })
          : key.startsWith("__user:")
            ? (formatAssigneeUserLabel(key.slice("__user:".length), currentUserId, companyUserLabelMap) ?? t("components.issuesList.userLabel", { defaultValue: "User" }))
  ...
  const createActionLabel = createIssueLabel
    ? t("components.issuesList.createLabeled", { label: createIssueLabel, defaultValue: "Create {{label}}" })
    : t("components.issuesList.createTask", { defaultValue: "Create Task" });
  const createButtonLabel = createIssueLabel
    ? t("components.issuesList.newLabeled", { label: createIssueLabel, defaultValue: "New {{label}}" })
    : t("components.issuesList.newTask", { defaultValue: "New Task" });
    ...
    <div ref={rootRef} className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          ...
          {viewState.viewMode === "list" && (
            <Button
              ...
              title={viewState.nestingEnabled
                ? t("components.issuesList.disableNesting", { defaultValue: "Disable parent-child nesting" })
                : t("components.issuesList.enableNesting", { defaultValue: "Enable parent-child nesting" })}
            >
            ...
            <>
              <Button
                ...
                title={boardCompactCards
                  ? t("components.issuesList.useComfortableCards", { defaultValue: "Use comfortable cards" })
                  : t("components.issuesList.useCompactCards", { defaultValue: "Use compact cards" })}
              >
              ...
              <Button
                ...
                title={boardCollapsedStatuses.length > 0
                  ? t("components.issuesList.expandColdLanes", { defaultValue: "Expand cold lanes" })
                  : t("components.issuesList.collapseColdLanes", { defaultValue: "Collapse cold lanes" })}
              >
                ...
                <PopoverContent align="end" className="w-40 p-0">
                  <div className="p-2 space-y-0.5">
                    {KANBAN_COLUMN_PAGE_SIZE_OPTIONS.map((pageSize) => (
                      ...
                      >
                        <span>{t("components.issuesList.perColumnOption", { count: pageSize, defaultValue: "{{count}} per column" })}</span>
          ...
          <IssueColumnPicker
            ...
            title={t("components.issuesList.chooseVisibleColumns", { defaultValue: "Choose which task columns stay visible" })}
            iconOnly
          />
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              ...
              <PopoverContent align="end" className="w-48 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["workflow", t("components.issuesList.sortWorkflow", { defaultValue: "Workflow" })],
                    ["status", t("components.issuesList.sortStatus", { defaultValue: "Status" })],
                    ["priority", t("components.issuesList.sortPriority", { defaultValue: "Priority" })],
                    ["title", t("components.issuesList.sortTitle", { defaultValue: "Title" })],
                    ["created", t("components.issuesList.sortCreated", { defaultValue: "Created" })],
                    ["updated", t("components.issuesList.sortUpdated", { defaultValue: "Updated" })],
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              ...
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["status", t("components.issuesList.groupStatus", { defaultValue: "Status" })],
                    ["priority", t("components.issuesList.groupPriority", { defaultValue: "Priority" })],
                    ["assignee", t("components.issuesList.groupAssignee", { defaultValue: "Assignee" })],
                    ["project", t("components.issuesList.groupProject", { defaultValue: "Project" })],
                    ["workspace", t("components.issuesList.groupWorkspace", { defaultValue: "Workspace" })],
                    ["parent", t("components.issuesList.groupParentTask", { defaultValue: "Parent Task" })],
                    ["none", t("components.issuesList.groupNone", { defaultValue: "None" })],
      ...
      {!searchWithinLoadedIssues && normalizedIssueSearch.length > 0 && searchedIssues.length === ISSUE_SEARCH_RESULT_LIMIT && (
        <p className="text-xs text-muted-foreground">
          {t("components.issuesList.searchLimitNotice", {
            limit: ISSUE_SEARCH_RESULT_LIMIT,
      ...
      {boardColumnLimitReached && (
        <p className="text-xs text-muted-foreground">
          {t("components.issuesList.boardColumnLimitNotice", {
            limit: ISSUE_BOARD_COLUMN_RESULT_LIMIT,
      ...
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("components.issuesList.emptyStateMessage", { defaultValue: "No tasks match the current filters or search." })}
        ...
        <>
              ...
              <IssueGroupHeader
                ...
                trailing={(
                  <Button
                    ...
                    title={t("components.issuesList.newTaskInGroup", { group: group.label, defaultValue: "New task in {{group}}" })}
                    aria-label={t("components.issuesList.newTaskInGroup", { group: group.label, defaultValue: "New task in {{group}}" })}
              ...
              {(() => {
                ...
                const renderIssueRow = (issue: Issue, depth: number) => {
                  ...
                  const visibleBlockerChips = unresolvedVisibleBlockers
                    .map((blockerId) => {
                      ...
                      const blockerStepSuffix = blockerStep
                        ? t("components.issuesList.blockerStepSuffix", { step: blockerStep, defaultValue: " \u00b7 step {{step}}" })
                        : "";
                      return {
                        blockerId,
                        chipLabel: t("components.issuesList.blockedByChip", {
                          label,
                          stepSuffix: blockerStepSuffix,
                          defaultValue: "blocked by {{label}}{{stepSuffix}}",
                        }),
                      };
                    })
                  ...
                  const additionalVisibleBlockerLabel = additionalVisibleBlockerCount > 0
                    ? t("components.issuesList.andMoreBlockers", {
                      count: additionalVisibleBlockerCount,
                  ...
                  const firstVisibleBlockerTitle = additionalVisibleBlockerCount > 0
                    ? t("components.issuesList.blockerTitleWithHidden", {
                      summary: firstVisibleBlockerDisplayLabel,
                      hidden: hiddenVisibleBlockerLabels,
                          ...
                          <>
                            {hasChildren && !isExpanded ? (
                              <span className="ml-1.5 text-xs text-muted-foreground">
                                {t("components.issuesList.subtaskCountSuffix", {
                            ...
                            {isSuccessfulRunHandoffRequired(issue) ? (
                              <span
                                ...
                                title={t("components.issuesList.needsNextStepTitle", { defaultValue: "This task needs a next step" })}
                              >
                                ...
                                >
                                  <PopoverTrigger asChild>
                                    ...
                                    >
                                      {issue.assigneeAgentId && agentName(issue.assigneeAgentId) ? (
                                        ...
                                        <Identity
                                          name={assigneeUserLabel ?? t("components.issuesList.userLabel", { defaultValue: "User" })}
                                  ...
                                  >
                                    <input
                                      ...
                                      placeholder={t("components.issuesList.searchAssigneesPlaceholder", { defaultValue: "Search assignees..." })}
                                    ...
                                    <div className="max-h-48 overflow-y-auto overscroll-contain">
                                      ...
                                      >
                                        {t("components.issuesList.noAssignee", { defaultValue: "No assignee" })}
                                      </button>
                                      {currentUserId && (
                                        <button
                                          className={cn(
          ...
          {(remainingIssueRowCount > 0 || hasMoreIssues || isLoadingMoreIssues) && (
            <div className="py-2" data-testid="issues-load-more-sentinel">
              <p className="text-xs text-muted-foreground">
                {isLoadingMoreIssues
                  ? t("components.issuesList.loadingMoreTasks", { defaultValue: "Loading more tasks..." })
                  : remainingIssueRowCount > 0
                    ? t("components.issuesList.renderingTasksCount", {
                      rendered: Math.min(renderedIssueRowLimit, filtered.length),
                      total: filtered.length,
                      defaultValue: "Rendering {{rendered}} of {{total}} tasks",
                    })
                    : t("components.issuesList.scrollToLoadMore", { defaultValue: "Scroll to load more tasks" })}
              </p>
            </div>
          )}
        </>

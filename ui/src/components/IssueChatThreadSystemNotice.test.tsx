...
import type { Agent, SuccessfulRunHandoffState } from "@valadrien-os/shared";
...
describe("IssueChatThread system notice routing", () => {
  it("renders authorType=system comments as a SystemNotice rather than a user bubble", () => {
    ...
    expect(container.textContent).toContain("ValadrienOs needs a disposition");
  ...
  it("labels system notice source as ValadrienOs when no run agent can be resolved", () => {
    const comment: IssueChatComment = {
    ...
    const status = container.querySelector('[role="status"]');
    expect(status).not.toBeNull();
    expect(status?.textContent).toContain("ValadrienOs");
    expect(status?.textContent).not.toContain("You");
  });

  it("falls back to ValadrienOs in the system notice header when run agent is unknown to agentMap", () => {
    const comment: IssueChatComment = {
    ...
    expect(sourceLink?.textContent).toBe("ValadrienOs");
  });
  ...
  it("folds stale successful-run disposition warnings into the activity log disclosure style", () => {
    ...
    expect(row?.textContent).not.toContain("ValadrienOs needs a disposition before this issue can continue.");

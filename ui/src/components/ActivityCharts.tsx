...
export function RunActivityChart({ runs }: { runs: HeartbeatRun[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{translateActive("activity_charts.no_runs_yet")}</p>;
        ...
        {days.map(day => {
          ...
          return (
            <div
              ...
              title={translateActive("activity_charts.runs_for_day", { day, count: total })}
            >
              {total > 0 ? (
...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{translateActive("activity_charts.no_issues")}</p>;
  ...
  return (
    <div>
        ...
        {days.map(day => {
          ...
          return (
            <div
              key={day}
              className="flex-1 h-full flex flex-col justify-end"
              title={translateActive("activity_charts.issues_for_day", { day, count: total })}
            >
              {total > 0 ? (
                <div className="flex flex-col-reverse gap-px overflow-hidden" style={{ height: `${heightPct}%`, minHeight: 2 }}>
      ...
      <DateLabels days={days} />
      <ChartLegend items={priorityOrder.map((p) => ({ color: priorityColors[p], label: translateActive(`status.${p}`) }))} />
    </div>
  );
}
    ...
    <div>
      ...
      <ChartLegend
        items={statusOrder.map((s) => ({
          ...
          label: translateActive(`status.${s}`),
        }))}
      />
    </div>
  );
        ...
        {days.map(day => {
          ...
          return (
            <div
              key={day}
              className="flex-1 h-full flex flex-col justify-end"
              title={translateActive("activity_charts.success_for_day", {
                day,

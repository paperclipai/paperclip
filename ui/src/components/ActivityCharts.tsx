...
export function RunActivityChart(props: RunChartProps) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("chart.noRunsYet")}</p>;
...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("chart.noIssues")}</p>;
      ...
      <ChartLegend items={priorityOrder.map((p) => ({
        ...
        label:
          p === "critical"
            ...
            : p === "high"
              ? t("chart.priority.high")
              : p === "medium"
                ? t("chart.priority.medium")
                : t("chart.priority.low"),
      }))} />
    </div>
  );
}
      ...
      <ChartLegend items={statusOrder.map((s) => ({
        ...
        label:
          s === "todo"
            ? t("chart.status.todo")
            : s === "in_progress"
              ? t("chart.status.inProgress")
              : s === "in_review"
                ? t("chart.status.inReview")
                : s === "done"
                  ? t("chart.status.done")
                  : s === "blocked"
                    ? t("chart.status.blocked")
                    : s === "cancelled"
                      ? t("chart.status.cancelled")
                      : s === "backlog"
                        ? t("chart.status.backlog")
                        : (statusLabels[s] ?? s),
      }))} />
    </div>
  );

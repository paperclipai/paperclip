...
import { formatShortDate, issuePriorityDisplayName, issueStatusDisplayName } from "../lib/utils";
    ...
    <div>
      <div className="flex items-end gap-[3px] h-20">
        {days.map(day => {
          ...
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("activityCharts.runCountTooltip", { day, count: total })}>
              {total > 0 ? (
    ...
    <div>
      <div className="flex items-end gap-[3px] h-20">
        {days.map(day => {
          ...
          const heightPct = (total / maxValue) * 100;
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("activityCharts.issueCountTooltip", { day, count: total })}>
              {total > 0 ? (
                <div className="flex flex-col-reverse gap-px overflow-hidden" style={{ height: `${heightPct}%`, minHeight: 2 }}>
      ...
      <DateLabels days={days} />
      <ChartLegend items={priorityOrder.map((priority) => ({ color: priorityColors[priority], label: issuePriorityDisplayName(priority) }))} />
    </div>
  );
}
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={statusOrder.map((status) => ({ color: statusColors[status] ?? "#6b7280", label: issueStatusDisplayName(status) }))} />
    </div>
  );
}
        ...
        {days.map(day => {
          ...
          return (
            <div
              ...
              title={t("activityCharts.successRateTooltip", {
                day,

...
export function EnvVarEditor({
  ...
  async function sealRow(index: number) {
    ...
    const name = window.prompt(
      t("components.envVarEditor.secretNamePrompt", { defaultValue: "Secret name" }),
      suggested,
    )?.trim();
    if (!name) return;
    ...
    try {
      ...
      setSealError(
        error instanceof Error
          ? error.message
          : t("components.envVarEditor.createSecretFailed", { defaultValue: "Failed to create secret" }),
      );
    }
  }
  ...
  return (
    <div className="space-y-1.5">
          ...
          <div key={index} className="flex items-center gap-1.5">
            ...
            >
              <SelectTrigger
                ...
                aria-label={t("components.envVarEditor.bindingModeLabel", { defaultValue: "Binding mode" })}
              >
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="plain">
                  {t("components.envVarEditor.bindingModePlain", { defaultValue: "Plain" })}
                </SelectItem>
                <SelectItem value="secret">
                  {t("components.envVarEditor.bindingModeSecret", { defaultValue: "Secret" })}
                </SelectItem>
              </SelectContent>
            </Select>
            {row.source === "secret" ? (
              <>
                ...
                >
                  <SelectTrigger
                    aria-label={t("components.envVarEditor.secretLabel", { defaultValue: "Secret" })}
                    className={cn(
                      selectTriggerClass,
                      "flex-[3]",
                      row.secretId &&
                  ...
                  >
                    <SelectValue
                      placeholder={t("components.envVarEditor.selectSecretPlaceholder", {
                  ...
                  <SelectContent>
                    {row.secretId && !secrets.some((s) => s.id === row.secretId) ? (
                      <SelectItem value={row.secretId}>
                        {t("components.envVarEditor.missingSecret", {
                ...
                >
                  <SelectTrigger
                    className={cn(selectTriggerClass, "flex-[1]")}
                    aria-label={t("components.envVarEditor.versionLabel", { defaultValue: "Version" })}
                  >
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="latest">
                      {t("components.envVarEditor.versionLatest", { defaultValue: "latest" })}
                    </SelectItem>
                    {(() => {
                ...
                <button
                  ...
                  title={t("components.envVarEditor.newSecretTitle", {
                    defaultValue: "Create secret from current plain value",
                  })}
                >
                  {t("components.envVarEditor.newButton", { defaultValue: "New" })}
                </button>
              </>
            ) : (
              <>
                ...
                <button
                  ...
                  onClick={() => sealRow(index)}
                  disabled={!row.key.trim() || !row.plainValue}
                  title={t("components.envVarEditor.sealTitle", {
                    defaultValue: "Store value as secret and replace with reference",
                  })}
                >
                  {t("components.envVarEditor.sealButton", { defaultValue: "Seal" })}
                </button>
              </>
            )}
            {!isTrailing ? (
        ...
        return (
          <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
            <span className="inline-flex items-center gap-1 text-[11px] text-muted-foreground/70">
              ...
              {t("components.envVarEditor.recentlyUsed", { defaultValue: "Recently used:" })}
            </span>
            {quick.map((secret) => (
              <button
                key={secret.id}
                ...
                title={t("components.envVarEditor.bindSecretTitle", {
                  name: secret.name,
          ...
          <p className="text-[11px] text-amber-700 dark:text-amber-400 inline-flex items-start gap-1">
            ...
            <span>
              {t("components.envVarEditor.bindingsNeedAttention", {
                count: issues.length,
                defaultValue: "{{count}} secret binding need attention: ",
                defaultValue_other: "{{count}} secret bindings need attention: ",
              })}
              ...
              {t("components.envVarEditor.bindingsNeedAttentionSuffix", {
      ...
      <p className="text-[11px] text-muted-foreground/60">
        {t("components.envVarEditor.helpText", {
          defaultValue:

...
import { mobileNav, nav } from "../lib/i18n";
...
export function MobileBottomNav({ visible }: MobileBottomNavProps) {
  ...
  const items = useMemo<MobileNavItem[]>(
    () => [
      { type: "link", to: "/dashboard", label: mobileNav.home, icon: House },
      { type: "link", to: "/issues", label: nav.issues, icon: CircleDot },
      { type: "action", label: mobileNav.create, icon: SquarePen, onClick: () => openNewIssue() },
      { type: "link", to: "/agents/all", label: nav.agents, icon: Users },
      {
        type: "link",
        to: "/inbox",
        label: nav.inbox,

...
export function MobileBottomNav({ visible }: MobileBottomNavProps) {
  ...
  const items = useMemo<MobileNavItem[]>(
    () => [
      { type: "link", to: "/dashboard", label: t("mobileNav.home", { defaultValue: "Home" }), icon: House },
      { type: "link", to: "/issues", label: t("sidebar.issues", { defaultValue: "Issues" }), icon: CircleDot },
      { type: "action", label: t("mobileNav.create", { defaultValue: "Create" }), icon: SquarePen, onClick: () => openNewIssue() },
      { type: "link", to: "/agents/all", label: t("sidebar.agents", { defaultValue: "Agents" }), icon: Users },
      {
        type: "link",
        to: "/inbox",
  ...
  return (
    <nav
      ...
      aria-label={t("mobileNav.aria", { defaultValue: "Mobile navigation" })}
    >

    ...
    <aside className="w-60 h-full min-h-0 border-r border-border bg-background flex flex-col">
      <div className="flex items-center gap-2 px-3 h-12 shrink-0">
        ...
        <span className="flex-1 text-sm font-bold text-foreground truncate">
          {t("sidebar.instance_settings", { defaultValue: "Instance Settings" })}
        </span>
      </div>

      <nav className="flex-1 min-h-0 overflow-y-auto scrollbar-auto-hide flex flex-col gap-4 px-3 py-2">
        <div className="flex flex-col gap-0.5">
          <SidebarNavItem to="/instance/settings/profile" label={t("sidebar.profile", { defaultValue: "Profile" })} icon={UserRoundPen} end />
          <SidebarNavItem to="/instance/settings/general" label={t("sidebar.general", { defaultValue: "General" })} icon={SlidersHorizontal} end />
          <SidebarNavItem to="/instance/settings/access" label={t("sidebar.access", { defaultValue: "Access" })} icon={Shield} end />

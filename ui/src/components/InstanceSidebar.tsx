    ...
    <aside className="w-60 h-full min-h-0 border-r border-border bg-background flex flex-col">
      <div className="flex items-center gap-2 px-3 h-12 shrink-0">
        ...
        <span className="flex-1 text-sm font-bold text-foreground truncate">
          {t("instanceSidebar.title", "Instance Settings")}
        </span>
      </div>
      ...
      <nav className="flex-1 min-h-0 overflow-y-auto scrollbar-auto-hide flex flex-col gap-4 px-3 py-2">
        <div className="flex flex-col gap-0.5">
          <SidebarNavItem
            ...
            label={t("instanceSidebar.general", "General")}
            icon={SlidersHorizontal}
            end
          />
          <SidebarNavItem
            to="/instance/settings/heartbeats"
            label={t("instanceSidebar.heartbeats", "Heartbeats")}
            icon={Clock3}
            end
          />
          <SidebarNavItem
            to="/instance/settings/experimental"
            label={t("instanceSidebar.experimental", "Experimental")}
            icon={FlaskConical}
          />
          <SidebarNavItem
            to="/instance/settings/plugins"
            label={t("instanceSidebar.plugins", "Plugins")}
            icon={Puzzle}
          />
          <SidebarNavItem
            to="/instance/settings/adapters"
            label={t("instanceSidebar.adapters", "Adapters")}
            icon={Cpu}
          />
          {(plugins ?? []).length > 0 ? (

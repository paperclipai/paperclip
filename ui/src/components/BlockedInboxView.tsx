      ...
      >
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1 space-y-1">
            <p className="text-sm font-medium">{t("blockedInbox.loadFailed", { defaultValue: "Couldn't load the Blocked tab." })}</p>
            <p className="text-xs opacity-80">
              {t("blockedInbox.otherWork", { defaultValue: "Other Inbox tabs still work." })} {message}
            </p>
          </div>
          <Button
            type="button"
            ...
            disabled={isFetching}
          >
            {isFetching ? t("blockedInbox.trying", { defaultValue: "Trying…" }) : t("blockedInbox.tryAgain", { defaultValue: "Try again" })}
          </Button>
        </div>
      </div>
    );
    ...
    return (
      ...
      >
        ...
        <div className="space-y-1">
          <p className="text-sm font-medium text-foreground">{t("blockedInbox.empty", { defaultValue: "No work is stopped." })}</p>
          <p className="text-xs text-muted-foreground">
            {t("blockedInbox.emptyDescription", { defaultValue: "Issues that need a decision, recovery, or external action will appear here." })}
          </p>
        </div>
      </div>
    );

...
import { inboxUi, displayProductivityReviewIssueTitle } from "../lib/i18n";
...
export function IssueRow({
  ...
  const productivityReviewIndicator = productivityReview ? (
    <span
      ...
      title={inboxUi.productivityReviewTooltip(productivityReviewTriggerLabel(productivityReview.trigger))}
      aria-label={inboxUi.issueRowProductivityReviewAria}
    >
  ...
  const planningModeIndicator = issue.workMode === "planning" ? (
    <span
      ...
      title={inboxUi.issueRowPlanningTooltip}
    >
      {inboxUi.planningModeChip}
    </span>
  ) : null;
  const parkedBlockerIndicator = hasAssignedBacklogBlocker(issue.blockedBy) ? (
    <span
      ...
      title={inboxUi.issueRowParkedBlockerTooltip}
    >
      <Flag className="h-2.5 w-2.5" aria-hidden />
      {inboxUi.parkedWorkChip}
    </span>
  ) : null;
    ...
    >
      ...
      <span className="flex min-w-0 flex-1 flex-col gap-1 sm:contents">
        <span className={cn("line-clamp-2 text-sm sm:order-2 sm:min-w-0 sm:flex-1 sm:truncate sm:line-clamp-none", titleClassName)}>
          {displayProductivityReviewIssueTitle(issue)}{titleSuffix}
        </span>
        {checklistDependencyChips ? (
        ...
        <span className="inline-flex h-4 w-4 shrink-0 items-center justify-center self-center">
          {showUnreadDot ? (
            <button
              ...
              aria-label={inboxUi.markAsReadAria}
            >
              <span
                className={cn(
            ...
            <button
              ...
              aria-label={inboxUi.dismissFromInboxAria}
            >

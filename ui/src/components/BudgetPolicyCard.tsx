...
import {
  ...
  budgetWindowDisplayName,
  cn,
  formatCents,
  pauseReasonDisplayName,
} from "../lib/utils";
...
export function BudgetPolicyCard({
  ...
  const budgetStatusLabel = summary.paused
    ? t("budgetPolicy.statusPaused")
    : summary.status === "warning"
      ? t("budgetPolicy.statusWarning")
      : summary.status === "hard_stop"
        ? t("budgetPolicy.statusHardStop")
        : t("budgetPolicy.statusHealthy");
  const softAlertLabel = summary.paused && summary.pauseReason
    ? t("budgetPolicy.softAlertWithPause", {
        percent: summary.warnPercent,
        reason: pauseReasonDisplayName(summary.pauseReason),
      })
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("budgetPolicy.projectPausedNotice")
          : t("budgetPolicy.scopePausedNotice")}
      </div>
    </div>
  ) : null;
  ...
  if (isPlain) {
    return (
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          <div>
            <div className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground">
              {budgetScopeDisplayName(summary.scopeType)}
            </div>
            <div className="mt-2 text-xl font-semibold">{summary.scopeName}</div>
            <div className="mt-2 text-sm text-muted-foreground">{budgetWindowDisplayName(summary.windowKind)}</div>
          </div>
          <div
            className={cn(
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("budgetPolicy.invalidNonNegativeAmount")}</p>
        ) : null}
      </div>
    );
  }
      ...
      <CardHeader className={cn("gap-3", compact ? "px-4 pt-4 pb-2" : "px-5 pt-5 pb-3")}>
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <CardDescription className="mt-1">{budgetWindowDisplayName(summary.windowKind)}</CardDescription>
          </div>

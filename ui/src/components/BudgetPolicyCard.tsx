...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"], t: (key: string) => string) {
  return windowKind === "lifetime" ? t("budget.lifetime_budget") : t("budget.monthly_utc_budget");
}
...
export function BudgetPolicyCard({
    ...
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? t("budget.percent_of_limit", { percent: String(summary.utilizationPercent) }) : t("budget.no_cap_configured")}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("budget.budget_label")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("budget.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("budget.soft_alert_at", { percent: String(summary.warnPercent) })}{summary.paused && summary.pauseReason ? ` · ${t("budget.pause_suffix", { reason: summary.pauseReason })}` : ""}
        </div>
      </div>
    </div>
  ) : (
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("budget.execution_paused_project")
          : t("budget.heartbeats_paused_scope")}
      </div>
    </div>
  ) : null;
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? t("budget.saving") : summary.amount > 0 ? t("budget.update_budget") : t("budget.set_budget")}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? t("budget.paused") : summary.status === "warning" ? t("budget.warning") : summary.status === "hard_stop" ? t("budget.hard_stop") : t("budget.healthy")}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("budget.validation_error")}</p>
        ) : null}
      </div>
    );
  }

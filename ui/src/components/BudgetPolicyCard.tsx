...
export function BudgetPolicyCard({
  ...
  const windowLabel = summary.windowKind === "lifetime" ? t("budgets.lifetime_budget") : t("budgets.monthly_budget");
  ...
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      ...
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("budgets.budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("budgets.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("budgets.soft_alert_at", { percent: summary.warnPercent })}{summary.paused && summary.pauseReason ? ` · ${summary.pauseReason} pause` : ""}
        </div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        ...
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("budgets.unlimited")}</span>
      </div>
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? t("budgets.saving") : summary.amount > 0 ? t("budgets.update_budget") : t("budgets.set_budget")}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? t("budgets.paused") : summary.status === "warning" ? t("budgets.warning") : summary.status === "hard_stop" ? t("budgets.hard_stop") : t("budgets.healthy")}
          </div>
        </div>

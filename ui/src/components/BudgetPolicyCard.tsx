...
function windowLabelKey(windowKind: BudgetPolicySummary["windowKind"]) {
  return windowKind === "lifetime" ? "component.finance.lifetimeBudget" : "component.finance.monthlyUtcBudget";
}
...
export function BudgetPolicyCard({
  ...
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("component.finance.observed")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? `${summary.utilizationPercent}% ${t("component.finance.ofLimit")}` : t("component.finance.noCapConfigured")}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("component.finance.budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("component.finance.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("component.finance.softAlertAt", { percent: summary.warnPercent })}{summary.paused && summary.pauseReason ? ` · ${summary.pauseReason} ${t("component.finance.pause")}` : ""}
        </div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>{t("component.finance.remaining")}</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("component.finance.unlimited")}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("component.finance.executionPausedProject")
          : t("component.finance.heartbeatsPausedScope")}
      </div>
    </div>
  ) : null;
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? t("component.finance.saving") : summary.amount > 0 ? t("component.finance.updateBudget") : t("component.finance.setBudget")}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? t("component.finance.paused") : summary.status === "warning" ? t("component.finance.warning") : summary.status === "hard_stop" ? t("component.finance.hardStop") : t("component.finance.healthy")}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("component.finance.enterValidAmount")}</p>
        ) : null}
      </div>
    );
  }

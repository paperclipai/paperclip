...
export function getHelpText(key: string): string {
  return i18n.t(`adapters:help.${key}`, { defaultValue: key });
}

export const help: Record<string, string> = new Proxy({} as Record<string, string>, {
  get(_target, prop: string) {
    return i18n.t(`adapters:help.${prop}`, { defaultValue: prop });
  },
});

export function getAdapterLabel(key: string): string {
  return i18n.t(`adapters:label.${key}`, { defaultValue: key });
}

export const adapterLabels: Record<string, string> = new Proxy({} as Record<string, string>, {
  get(_target, prop: string) {
    return i18n.t(`adapters:label.${prop}`, { defaultValue: prop });
  },
});

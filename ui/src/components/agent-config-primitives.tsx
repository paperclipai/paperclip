...
export function ChoosePathButton() {
  ...
  return (
    <>
      ...
      >
        {t("pathPicker.choose", { defaultValue: "Choose" })}
      </button>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("pathPicker.title", { defaultValue: "Specify path manually" })}</DialogTitle>
            <DialogDescription>
              {t("pathPicker.description", { defaultValue: "Browser security blocks apps from reading full local paths via a file picker. Copy the absolute path and paste it into the input." })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 text-sm">
            <section className="space-y-1.5">
              <p className="font-medium">{t("pathPicker.macHeading", { defaultValue: "macOS (Finder)" })}</p>
              <ol className="list-decimal space-y-1 pl-5 text-muted-foreground">
                <li>{t("pathPicker.macStep1", { defaultValue: "Find the folder in Finder." })}</li>
                <li>{t("pathPicker.macStep2", { defaultValue: "Hold Option and right-click the folder." })}</li>
                <li>{t("pathPicker.macStep3", { defaultValue: "Click \"Copy <folder name> as Pathname\"." })}</li>
                <li>{t("pathPicker.macStep4", { defaultValue: "Paste the result into the path input." })}</li>
              </ol>
            ...
            <section className="space-y-1.5">
              <p className="font-medium">{t("pathPicker.winHeading", { defaultValue: "Windows (File Explorer)" })}</p>
              <ol className="list-decimal space-y-1 pl-5 text-muted-foreground">
                <li>{t("pathPicker.winStep1", { defaultValue: "Find the folder in File Explorer." })}</li>
                <li>{t("pathPicker.winStep2", { defaultValue: "Hold Shift and right-click the folder." })}</li>
                <li>{t("pathPicker.winStep3", { defaultValue: "Click \"Copy as path\"." })}</li>
                <li>{t("pathPicker.winStep4", { defaultValue: "Paste the result into the path input." })}</li>
              </ol>
              <p className="rounded-md bg-muted px-2 py-1 font-mono text-xs">
            ...
            <section className="space-y-1.5">
              <p className="font-medium">{t("pathPicker.terminalHeading", { defaultValue: "Terminal fallback (macOS/Linux)" })}</p>
              <ol className="list-decimal space-y-1 pl-5 text-muted-foreground">
                <li>{t("pathPicker.terminalStep1", { defaultValue: "Run cd /path/to/folder." })}</li>
                <li>{t("pathPicker.terminalStep2", { defaultValue: "Run pwd." })}</li>
                <li>{t("pathPicker.terminalStep3", { defaultValue: "Copy the output and paste it into the path input." })}</li>
              </ol>
            </section>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setOpen(false)}>
              {t("pathPicker.ok", { defaultValue: "OK" })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

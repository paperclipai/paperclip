  ...
  return (
    <div className="flex items-start gap-1.5 min-w-0 flex-1">
      ...
      <button
        ...
        onClick={handleCopy}
        title={copied ? i18n.t("common:copyText.copied", { defaultValue: "Copied!" }) : i18n.t("issues:properties.clickToCopy", { defaultValue: "Click to copy" })}
      >
        {value}
      </button>
...
export function IssueProperties({
  ...
  const projectName = (id: string | null) => {
    if (!id) return id?.slice(0, 8) ?? t("properties.none", { defaultValue: "None" });
  ...
  const executionParticipantLabel = (value: string) => {
    ...
    if (value.startsWith("user:")) {
      return userLabel(value.slice("user:".length)) ?? t("properties.user", { defaultValue: "User" });
    }
    return value;
  };
  ...
  const runExecutionButton = (stageType: "review" | "approval") => (
    <PropertyRow label="">
      ...
      >
        {stageType === "review" ? t("properties.runReviewNow", { defaultValue: "Run review now" }) : t("properties.runApprovalNow", { defaultValue: "Run approval now" })}
      </button>
    </PropertyRow>
  );
  const currentExecutionLabel = (() => {
    if (!issue.executionState?.currentStageType) return null;
    const stageLabel = issue.executionState.currentStageType === "review" ? t("properties.reviewStage", { defaultValue: "Review" }) : t("properties.approvalStage", { defaultValue: "Approval" });
  ...
  const labelsTrigger = (issue.labels ?? []).length > 0 ? (
    ...
    <>
      ...
      <span className="text-sm text-muted-foreground">{t("properties.noLabels", { defaultValue: "No labels" })}</span>
    </>
  );
  const labelsExtra = (issue.labelIds ?? []).length > 0 ? (
    <button
      ...
      aria-label={t("properties.addLabel", { defaultValue: "Add label" })}
      title={t("properties.addLabel", { defaultValue: "Add label" })}
    >
  ...
  const labelsContent = (
    <>
      <input
        ...
        placeholder={t("properties.searchLabels", { defaultValue: "Search labels..." })}
        value={labelSearch}
      ...
      <div className="mt-2 border-t border-border pt-2 space-y-1">
        <div className="flex items-center gap-1">
          ...
          <input
            ...
            placeholder={t("properties.newLabel", { defaultValue: "New label" })}
            value={newLabelName}
        ...
        >
          <Plus className="h-3 w-3" />
          {createLabel.isPending ? t("properties.creating", { defaultValue: "Creating…" }) : t("properties.createLabel", { defaultValue: "Create label" })}
        </button>
      </div>
    </>
  );
  ...
  const assigneeTrigger = assignee ? (
    ...
    <>
      <User className="h-3.5 w-3.5 text-muted-foreground" />
      <span className="text-sm text-muted-foreground">{t("properties.unassigned", { defaultValue: "Unassigned" })}</span>
    </>
  );
  ...
  const assigneeContent = (
    <>
      <input
        className="w-full px-2 py-1.5 text-xs bg-transparent outline-none border-b border-border mb-1 placeholder:text-muted-foreground/50"
        placeholder={t("properties.searchAssignees", { defaultValue: "Search assignees..." })}
  ...
  const executionParticipantsContent = (
    ...
    <>
      <input
        ...
        placeholder={
          stageType === "review"
            ? t("properties.searchReviewersPlaceholder", { defaultValue: "Search reviewers..." })
            : t("properties.searchApproversPlaceholder", { defaultValue: "Search approvers..." })
        }
        value={search}
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        ...
        >
          {stageType === "review"
            ? t("properties.noReviewers", { defaultValue: "No reviewers" })
            : t("properties.noApprovers", { defaultValue: "No approvers" })}
        </button>
        {currentUserId && (
          <button
            className={cn(
          ...
          >
            <User className="h-3 w-3 shrink-0 text-muted-foreground" />
            {t("properties.assignToMe", { defaultValue: "Assign to me" })}
          </button>
        )}
        {issue.createdByUserId && issue.createdByUserId !== currentUserId && (
          <button
          ...
          >
            <User className="h-3 w-3 shrink-0 text-muted-foreground" />
            {creatorUserLabel ? creatorUserLabel : t("properties.requester", { defaultValue: "Requester" })}
          </button>
        )}
        {otherUserOptions
          .filter((option) => {
  ...
  const projectTrigger = issue.projectId ? (
    ...
    <>
      ...
      <span className="text-sm text-muted-foreground">{t("properties.noProject", { defaultValue: "No project" })}</span>
    </>
  );
  ...
  const projectContent = (
    <>
      <input
        className="w-full px-2 py-1.5 text-xs bg-transparent outline-none border-b border-border mb-1 placeholder:text-muted-foreground/50"
        placeholder={t("properties.searchProjects", { defaultValue: "Search projects..." })}
        value={projectSearch}
  ...
  const parentTrigger = issue.parentId ? (
    ...
    <span className="text-sm text-muted-foreground">{t("properties.noParent", { defaultValue: "No parent" })}</span>
  );
  ...
  const blockedByTrigger = blockedByIds.length > 0 ? (
    ...
    <span className="text-sm text-muted-foreground">{t("properties.noBlockers", { defaultValue: "No blockers" })}</span>
  );
  ...
  const blockedByContent = (
    <>
      <input
        className="w-full px-2 py-1.5 text-xs bg-transparent outline-none border-b border-border mb-1 placeholder:text-muted-foreground/50"
        placeholder={t("properties.searchIssues", { defaultValue: "Search issues..." })}
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        ...
        >
          {t("properties.noBlockers", { defaultValue: "No blockers" })}
        </button>
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        ...
        <PropertyRow label={t("properties.priority", { defaultValue: "Priority" })}>
          <PriorityIcon
        ...
        <PropertyPicker
          inline={inline}
          label={t("properties.labels", { defaultValue: "Labels" })}
          open={labelsOpen}
        ...
        <PropertyPicker
          inline={inline}
          label={t("properties.assignee", { defaultValue: "Assignee" })}
          open={assigneeOpen}
        ...
        <PropertyPicker
          inline={inline}
          label={t("properties.project", { defaultValue: "Project" })}
          open={projectOpen}
        ...
        <PropertyPicker
          inline={inline}
          label={t("properties.parent", { defaultValue: "Parent" })}
          open={parentOpen}
          onOpenChange={(open) => {
            setParentOpen(open);
        ...
        <PropertyPicker
          inline={inline}
          label={t("properties.blockedBy", { defaultValue: "Blocked by" })}
          open={blockedByOpen}
          onOpenChange={(open) => {
        ...
        <PropertyRow label={t("properties.blocking", { defaultValue: "Blocking" })}>
        ...
        <PropertyRow label={t("properties.subIssues", { defaultValue: "Sub-issues" })}>
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {onAddSubIssue ? (
              ...
              >
                <Plus className="h-3 w-3" />
                {t("properties.addSubIssue", { defaultValue: "Add sub-issue" })}
              </button>
            ) : null}
          </div>
        </PropertyRow>
        ...
        <PropertyPicker
          inline={inline}
          label={t("properties.reviewers", { defaultValue: "Reviewers" })}
          open={reviewersOpen}
        ...
        <PropertyPicker
          inline={inline}
          label={t("properties.approvers", { defaultValue: "Approvers" })}
          open={approversOpen}
        ...
        {currentExecutionLabel && (
          <PropertyRow label={t("properties.execution", { defaultValue: "Execution" })}>
            <span className="text-sm">{currentExecutionLabel}</span>
          </PropertyRow>
        )}

        {issue.requestDepth > 0 && (
          <PropertyRow label={t("properties.depth", { defaultValue: "Depth" })}>
        ...
        <>
          ...
          <div className="space-y-1">
            ...
            {issue.executionWorkspaceId && (
              <PropertyRow label={t("properties.workspace", { defaultValue: "Workspace" })}>
                <Link
            ...
            {issue.currentExecutionWorkspace?.branchName && (
              <PropertyRow label={t("properties.branch", { defaultValue: "Branch" })}>
                <TruncatedCopyable
            ...
            {issue.currentExecutionWorkspace?.cwd && (
              <PropertyRow label={t("properties.folder", { defaultValue: "Folder" })}>
                <TruncatedCopyable
      ...
      <div className="space-y-1">
        {(issue.createdByAgentId || issue.createdByUserId) && (
          <PropertyRow label={t("properties.createdBy", { defaultValue: "Created by" })}>
            {issue.createdByAgentId ? (
              ...
              <>
                <User className="h-3.5 w-3.5 text-muted-foreground" />
                <span className="text-sm">{creatorUserLabel ?? t("properties.user", { defaultValue: "User" })}</span>
              </>
            )}
          </PropertyRow>
        )}
        {issue.startedAt && (
          <PropertyRow label={t("properties.started", { defaultValue: "Started" })}>
            <span className="text-sm">{formatDate(issue.startedAt)}</span>
          </PropertyRow>
        )}
        {issue.completedAt && (
          <PropertyRow label={t("properties.completed", { defaultValue: "Completed" })}>

...
export function BuiltInAgentGate({ agentKey, companyId, featureLabel, children }: BuiltInAgentGateProps) {
  ...
  if (state.status === "not_provisioned" || state.status === "needs_setup") {
    return (
      <>
        <EmptyState
          icon={Bot}
          title={t("localizationAgentChrome.setUpAgentTitle", { name: state.definition.displayName })}
          message={t("localizationAgentChrome.featureSetup", { feature: label })}
          action={t("localizationAgentChrome.setUpAgent", { name: state.definition.displayName })}
  ...
  if (state.status === "pending_approval") {
    return (
      <EmptyState
        icon={Clock3}
        title={t("localizationAgentChrome.agentPendingApproval", { name: state.definition.displayName })}
        message={t("localizationAgentChrome.featurePendingApproval", { feature: label })}
      />
    );
  }
  ...
  if (state.status === "paused" && state.agent) {
    ...
    return (
      <div className="space-y-4">
        <InlineBanner
          tone="warning"
          title={t("localizationAgentChrome.featurePaused", { feature: label })}
          actions={
            <>
              <Button variant="ghost" size="sm" asChild>
                <Link to={agentUrl(state.agent)}>{t("localizationAgentChrome.ui40_View_agent")}</Link>
              </Button>
              <Button
                size="sm"
              ...
              >
                {resume.isPending ? t("pages.agentDetail.resuming") : t("localizationTaskRuntime.ui_Resume_agent_1iqt3xn")}
              </Button>
            </>
          }
        >
          {pausedAt ? t("localizationAgentChrome.generationPausedAt", { when: pausedAt, feature: label.toLowerCase() }) : t("localizationAgentChrome.generationPaused", { feature: label.toLowerCase() })}
        </InlineBanner>

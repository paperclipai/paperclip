...
vi.mock("../adapters", () => ({
  getUIAdapter: (type: string) => ({
    type,
    label: type === "external_gateway" ? "External Gateway" : "Codex",
    ConfigFields: ({ adapterType }: { adapterType: string }) =>
      adapterType === "external_gateway"
        ? <div data-testid="external-gateway-config-fields">External Gateway fields</div>
        : null,
...
describe("AgentConfigForm environment selector", () => {
  ...
  it("renders non-local adapter config fields in the Adapter card", async () => {
    ...
    expect(result.container.querySelector('[data-testid="external-gateway-config-fields"]')).toBeTruthy();
    expect(result.container.textContent).toContain("External Gateway fields");
  });

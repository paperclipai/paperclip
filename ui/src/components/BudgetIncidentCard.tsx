...
function incidentStateLabel(incident: BudgetIncident, t: any) {
  if (incident.status === "resolved") return t("costs.incidents.resolved");
  if (incident.status === "dismissed") return t("costs.incidents.dismissed");
  if (incident.approvalStatus === "revision_requested") return t("costs.incidents.escalated");
  if (incident.approvalStatus === "pending") return t("costs.incidents.pendingApproval");
  return t("costs.incidents.open");
}
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <div className="text-[11px] uppercase tracking-[0.22em] text-red-200/80">
                {incident.scopeType} {t("costs.incidents.hardStop")}
              </div>
            ...
            <CardDescription className="mt-1 text-red-100/70">
              {t("costs.incidents.reachedLimit", { observed: formatCents(incident.amountObserved), limit: formatCents(incident.amountLimit), defaultValue: `Spending reached ${formatCents(incident.amountObserved)} against a limit of ${formatCents(incident.amountLimit)}.` })}
            </CardDescription>
          </div>
        ...
        <div className="flex items-start gap-2 rounded-xl border border-red-400/20 bg-red-500/10 px-3 py-2 text-sm text-red-50/90">
          ...
          <div>
            {incident.scopeType === "project"
              ? t("costs.incidents.projectPaused")
              : t("costs.incidents.scopePaused")}
          </div>
        </div>

        <div className="rounded-xl border border-border/60 bg-background/60 p-3">
          <label className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
            {t("costs.incidents.newBudgetUsd")}
          </label>
          <div className="mt-2 flex flex-col gap-3 sm:flex-row">
            ...
            >
              ...
              {isMutating ? t("costs.incidents.applying") : t("costs.incidents.raiseAndResume")}
            </Button>
          </div>
          {parsed !== null && parsed <= incident.amountObserved ? (
            <p className="mt-2 text-xs text-red-200/80">
              {t("costs.incidents.budgetMustExceed")}
            </p>
          ) : null}
        </div>

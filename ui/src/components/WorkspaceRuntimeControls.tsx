...
function buildJobItem(
  ...
  return {
    ...
    kind: "job",
    statusLabel: t
      ? t("workspaceRuntimeControls.statusLabel.runOnce", { defaultValue: "run once" })
      : "run once",
    lifecycle: null,
    healthStatus: null,
    ...
    disabledReason:
      command.disabledReason
        ?? (!command.command
          ? (t
              ? t("workspaceRuntimeControls.disabledReason.jobMissingCommand", {
                  defaultValue: "This job is missing a command.",
                })
...
export function buildWorkspaceRuntimeControlSections(input: {
  ...
  for (const command of commands) {
    if (command.kind === "job") {
      jobs.push(buildJobItem(command, input.canRunJobs ?? input.canStartServices, t));
      continue;
    }
  ...
  const otherServices = runtimeServices
    ...
    .map((runtimeService) => ({
      ...
      disabledReason: t
        ? t("workspaceRuntimeControls.disabledReason.runtimeUntracked", {
            defaultValue:
            ...
            <div key={item.key} className={cn("border border-border/80 bg-background px-3 py-3", square ? "rounded-none" : "rounded-xl")}>
              <div className="flex flex-col gap-3">
                ...
                <div className="space-y-1 text-xs text-muted-foreground">
                  ...
                  {item.port ? (
                    <div>
                      {t("workspaceRuntimeControls.itemDetails.port", {
...
export function WorkspaceRuntimeControls({
  ...
  const { t } = useTranslation();
  const stoppedLabel = t("workspaceRuntimeControls.statusLabel.stopped", { defaultValue: "stopped" });
  const resolvedSections = sections ?? {
    services: (items ?? []).map((item) => ({
      ...item,
      statusLabel: item.statusLabel ?? item.status ?? stoppedLabel,
    })),
    jobs: [],
    otherServices: [],
  };
  const resolvedServiceEmptyMessage =
    emptyMessage
      ?? serviceEmptyMessage
      ?? t("workspaceRuntimeControls.emptyMessages.services", {
        defaultValue: "No services are configured for this workspace.",
      });
  const resolvedJobEmptyMessage =
    jobEmptyMessage
      ?? t("workspaceRuntimeControls.emptyMessages.jobs", {
        defaultValue: "No one-shot jobs are configured for this workspace.",
      });
  ...
  return (
    <div className={cn("space-y-4", className)}>
      <div className={cn("border border-border/70 bg-background p-3", square ? "rounded-none" : "rounded-xl")}>
        <div className="space-y-1">
          <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
            {t("workspaceRuntimeControls.header.workspaceCommands", {
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {runningCount > 0
                ? t("workspaceRuntimeControls.header.servicesRunning", {
                    defaultValue: "{{count}} services running",
                    count: runningCount,
                  })
                : t("workspaceRuntimeControls.header.noServicesRunning", {
                    defaultValue: "No services running",
                  })}
            </span>
            <span className="text-xs text-muted-foreground">
              {resolvedSections.jobs.length > 0
                ? t("workspaceRuntimeControls.header.jobsAvailable", {
                    defaultValue: "{{count}} job available to run on demand.",
                    defaultValue_plural: "{{count}} jobs available to run on demand.",
                    count: resolvedSections.jobs.length,
                  })
                : t("workspaceRuntimeControls.header.independent", {
                    defaultValue: "Each command can be controlled independently.",
                  })}
            </span>
          </div>
      ...
      <CommandSection
        title={t("workspaceRuntimeControls.section.servicesTitle", { defaultValue: "Services" })}
        description={t("workspaceRuntimeControls.section.servicesDescription", {
          defaultValue: "Long-running commands that Paperclip can supervise for this workspace.",
        })}
      ...
      <CommandSection
        title={t("workspaceRuntimeControls.section.jobsTitle", { defaultValue: "Jobs" })}
        description={t("workspaceRuntimeControls.section.jobsDescription", {
          defaultValue: "One-shot commands that run now and exit when they finish.",
        })}
      ...
      {resolvedSections.otherServices.length > 0 ? (
        <CommandSection
          title={t("workspaceRuntimeControls.section.untrackedTitle", {
            defaultValue: "Untracked services",
          })}
          description={t("workspaceRuntimeControls.section.untrackedDescription", {
            defaultValue:
              "Running services that no longer match the current workspace command config.",
          })}

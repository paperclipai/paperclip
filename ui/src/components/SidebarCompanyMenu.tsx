...
import { sidebarCompany } from "@/lib/i18n";
    ...
    >
      ...
      {isEditing ? (
        <button
          ...
          aria-label={sidebarCompany.reorderTeam(company.name)}
    ...
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label={
            selectedCompany
              ? sidebarCompany.openSwitcherNamed(selectedCompany.name)
              : sidebarCompany.openSwitcherGeneric
          }
        >
          <span className="flex min-w-0 flex-1 items-center gap-2">
            {selectedCompany ? <WorkspaceIcon company={selectedCompany} /> : null}
            <span className="truncate text-sm font-bold text-foreground">
              {selectedCompany?.name ?? sidebarCompany.selectPrompt}
            </span>
          </span>
      ...
      <DropdownMenuContent align="start" sideOffset={8} className="w-64 p-1">
        <div className="flex items-center justify-between gap-2 px-2 py-1.5">
          <DropdownMenuLabel className="p-0 text-[11px] font-semibold uppercase text-muted-foreground">
            {sidebarCompany.switchWorkspaceTitle}
          </DropdownMenuLabel>
          <button
            type="button"
            onClick={(event) => {
          ...
          >
            {isEditingOrder ? sidebarCompany.done : sidebarCompany.edit}
          </button>
        </div>
        <div className="max-h-96 overflow-y-auto">
          ...
          {orderedCompanies.length === 0 ? (
            <DropdownMenuItem disabled>{sidebarCompany.noWorkspaces}</DropdownMenuItem>
          ) : null}
        </div>
        <DropdownMenuSeparator />
        <DropdownMenuItem
        ...
        >
          ...
          <span>{sidebarCompany.addCompany}</span>
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem asChild disabled={isEditingOrder}>
          ...
          >
            ...
            <span className="truncate">
              {selectedCompany ? sidebarCompany.inviteTo(selectedCompany.name) : sidebarCompany.inviteGeneric}
            </span>
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild disabled={isEditingOrder}>
          ...
          >
            ...
            <span>{sidebarCompany.companySettings}</span>
          </Link>
        </DropdownMenuItem>
        {session?.session ? (
          <>
            ...
            >
              ...
              <span>{signOutMutation.isPending ? sidebarCompany.signingOut : sidebarCompany.signOut}</span>
            </DropdownMenuItem>
          </>
        ) : null}
      </DropdownMenuContent>

    ...
    <Dialog open={open} onOpenChange={(next) => !isPending && onOpenChange(next)}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          ...
          <DialogTitle>{t("run_dialog.title", { defaultValue: "Run routine" })}</DialogTitle>
          <DialogDescription>
            {t("run_dialog.description", { defaultValue: "Choose the agent and optional project for this one run. Routine defaults are prefilled and won\u0027t be changed." })}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs">{t("run_dialog.agent_label", { defaultValue: "Agent" })} *</Label>
              <InlineEntitySelector
                value={selection.assigneeAgentId}
                options={assigneeOptions}
                recentOptionIds={recentAssigneeIds}
                placeholder={t("run_dialog.agent_placeholder", { defaultValue: "Agent" })}
                noneLabel={t("run_dialog.select_agent", { defaultValue: "Select an agent" })}
                searchPlaceholder={t("run_dialog.search_agents", { defaultValue: "Search agents..." })}
                emptyMessage={t("run_dialog.no_agents_found", { defaultValue: "No agents found." })}
                disablePortal
                openOnFocus={false}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("run_dialog.select_agent", { defaultValue: "Select an agent" })}</span>
                  )
                }
                renderOption={(option) => {
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">{t("run_dialog.project_label", { defaultValue: "Project" })}</Label>
              <InlineEntitySelector
                value={selection.projectId}
                options={projectOptions}
                recentOptionIds={recentProjectIds}
                placeholder={t("run_dialog.project_placeholder", { defaultValue: "Project" })}
                noneLabel={t("run_dialog.no_project", { defaultValue: "No project" })}
                searchPlaceholder={t("run_dialog.search_projects", { defaultValue: "Search projects..." })}
                emptyMessage={t("run_dialog.no_projects_found", { defaultValue: "No projects found." })}
                disablePortal
                openOnFocus={false}
                onChange={(projectId) => {
                ...
                renderTriggerValue={(option) =>
                  option && selectedProject ? (
                    ...
                    <span className="text-muted-foreground">{t("run_dialog.no_project", { defaultValue: "No project" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            <div key={variable.name} className="space-y-1.5">
              ...
              {variable.type === "textarea" ? (
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="__unset__">{t("run_dialog.no_value", { defaultValue: "No value" })}</SelectItem>
                    <SelectItem value="true">{t("run_dialog.true", { defaultValue: "True" })}</SelectItem>
                    <SelectItem value="false">{t("run_dialog.false", { defaultValue: "False" })}</SelectItem>
                  </SelectContent>
                </Select>
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t("run_dialog.choose_value", { defaultValue: "Choose a value" })} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__unset__">{t("run_dialog.no_value", { defaultValue: "No value" })}</SelectItem>
        ...
        <DialogFooter showCloseButton={false}>
          {!selection.assigneeAgentId ? (
            <p className="mr-auto text-xs text-amber-600">{t("run_dialog.agent_required", { defaultValue: "Default agent required for this run." })}</p>
          ) : missingRequired.length > 0 ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("run_dialog.missing", { defaultValue: "Missing: {{fields}}", fields: missingRequired.join(", ") })}
            </p>
          ) : workspaceSelectionEnabled && !workspaceConfigValid ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("run_dialog.choose_workspace", { defaultValue: "Choose an existing workspace before running." })}
            </p>
          ) : (
            <span className="mr-auto" />
          )}
          <Button variant="ghost" onClick={() => onOpenChange(false)} disabled={isPending}>
            {t("run_dialog.cancel", { defaultValue: "Cancel" })}
          </Button>
          <Button
            onClick={() => {
          ...
          >
            {isPending ? t("run_dialog.running", { defaultValue: "Running..." }) : t("run_dialog.run_routine", { defaultValue: "Run routine" })}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>

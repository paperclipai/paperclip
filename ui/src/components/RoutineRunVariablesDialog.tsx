    ...
    <Dialog open={open} onOpenChange={(next) => !isPending && onOpenChange(next)}>
      <DialogContent className="flex h-(--sz-calc-18) max-h-(--sz-calc-18) max-w-xl flex-col gap-0 overflow-hidden p-0 sm:h-auto sm:max-h-(--sz-calc-20)">
        <DialogHeader className="shrink-0 border-b border-border/60 px-6 pb-4 pr-12 pt-6">
          ...
          <DialogTitle>{t("app.routineRunVariablesDialog.runRoutine", { defaultValue: "Run routine" })}</DialogTitle>
          <DialogDescription>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs">{t("app.routineRunVariablesDialog.agent", { defaultValue: "Agent *" })}</Label>
              <InlineEntitySelector
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("app.routineRunVariablesDialog.selectAnAgent", { defaultValue: "Select an agent" })}</span>
                  )
                }
                renderOption={(option) => {
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">{t("app.routineRunVariablesDialog.project", { defaultValue: "Project" })}</Label>
              <InlineEntitySelector
                ...
                renderTriggerValue={(option) =>
                  option && selectedProject ? (
                    ...
                    <span className="text-muted-foreground">{t("app.routineRunVariablesDialog.noProject", { defaultValue: "No project" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            <div key={variable.name} className="space-y-1.5">
              ...
              {isAutoWorkspaceBranchVariable(variable) ? (
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="__unset__">{t("app.routineRunVariablesDialog.noValue", { defaultValue: "No value" })}</SelectItem>
                    <SelectItem value="true">{t("app.routineRunVariablesDialog.true", { defaultValue: "True" })}</SelectItem>
                    <SelectItem value="false">{t("app.routineRunVariablesDialog.false", { defaultValue: "False" })}</SelectItem>
                  </SelectContent>
                </Select>
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t("app.routineRunVariablesDialog.chooseAValue", { defaultValue: "Choose a value" })} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__unset__">{t("app.routineRunVariablesDialog.noValue", { defaultValue: "No value" })}</SelectItem>
        ...
        >
          {!selection.assigneeAgentId ? (
            <p className="mr-auto text-xs text-amber-600">{t("app.routineRunVariablesDialog.defaultAgentRequiredForThisRun", { defaultValue: "Default agent required for this run." })}</p>
          ) : missingRequired.length > 0 ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("app.routineRunVariablesDialog.missing", { defaultValue: "Missing:" })} {missingRequired.join(", ")}
            </p>
          ) : workspaceSelectionEnabled && !workspaceConfigValid ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("app.routineRunVariablesDialog.chooseAnExistingWorkspaceBeforeRunning", { defaultValue: "Choose an existing workspace before running." })}
            </p>
          ) : (
          ...
          >
            {isPending ? t("app.routineRunVariablesDialog.running", { defaultValue: "Running..." }) : t("app.routineRunVariablesDialog.runRoutine", { defaultValue: "Run routine" })}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>

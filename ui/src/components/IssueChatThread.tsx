...
import { AlertTriangle, ArrowRight, Brain, Check, ChevronDown, ChevronLeft, ChevronRight, ClipboardList, Copy, Download, ExternalLink, FileText, Hammer, ImageIcon, Loader2, Maximize2, Minimize2, MoreHorizontal, Paperclip, PauseCircle, Search, Square, ThumbsDown, ThumbsUp, Trash2 } from "lucide-react";
...
export function mergeIssueChatLiveRuns(
  ...
  for (const run of liveRuns) {
    if (issueStatus && !isLiveIssueRun(run, issueStatus)) continue;
    deduped.set(run.id, run);
  }
  if (activeRun && (!issueStatus || isLiveIssueRun(activeRun, issueStatus))) {
    ...
    deduped.set(activeRun.id, {
      ...
      // the issue live-runs endpoint. Preserve richer browser state so the
      // final frame stays visible once a focused action has settled.
  ...
  if (pages.length <= 1) {
    return (
      <WorkspaceFileMarkdownBody
        className={cn(className, onAccent && "paperclip-markdown-on-accent")}
        style={style}
...
const IssueChatTextPart = memo(function IssueChatTextPart({ text, recessed, onAccent }: { text: string; recessed?: boolean; onAccent?: boolean }) {
  const { onImageClick, resolveImageSrc, externalReferences, linkCaseReferences } = useContext(IssueChatCtx);
...
function IssueChatAssistantMessage({
  ...
  const toastActions = useOptionalToastActions();
  const copyText = deleted ? "" : getThreadMessageCopyText(displayMessage);
...
export function IssueChatThread({
  ...
  const displayLiveRuns = useMemo(
    () => mergeIssueChatLiveRuns(liveRuns, activeRun, issueStatus),

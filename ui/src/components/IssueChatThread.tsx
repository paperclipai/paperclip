...
const IssueChatComposer = forwardRef<IssueChatComposerHandle, IssueChatComposerProps>(function IssueChatComposer({
  ...
  return (
    <div
      ...
      className={cn(
        "relative rounded-md border border-border/70 bg-background/95 p-(--sz-15px) shadow-(--shadow-extract-4) backdrop-blur transition-(--tp-border-color-background-color-box-shadow) duration-150 supports-[backdrop-filter]:bg-background/85 focus-within:border-ts-accent/60 dark:shadow-(--shadow-extract-5)",
...
export function IssueChatThread({
  ...
  useEffect(() => {
    ...
    const isThreadHash = isThreadAnchorHash(hash);
    if (messages.length === 0) return;
    if (!isThreadHash) {
  ...
  // top (board rev-2 feedback for PAP-95). Long threads are read newest-first,
  // and the old top-default forced a manual scroll to the end on every open. We
  // reuse the settle loop (see scrollToLatestCommentWithSettle) so we walk to
  // the true bottom as the virtualizer measures tall rows instead of stranding
  // the user above the real end. An anchor hash owns the initial scroll (handled
  // by the effect above) and must win, so we bail when one is present. Keyed by
  // issueId because the route reuses this component across issues — React Router
  // does not remount on a param change, so a plain once-per-mount guard would
  // skip later issues.

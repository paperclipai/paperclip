...
export function ExecutionWorkspaceCloseDialog({
  ...
  const actionLabel = currentStatus === "cleanup_failed" ? t("workspaces.close.retry") : t("workspaces.close.action");
  ...
  const closeWorkspace = useMutation({
    ...
    onSuccess: (workspace) => {
      ...
      pushToast({
        title: currentStatus === "cleanup_failed" ? t("workspaces.close.retryStarted") : t("workspaces.close.closed"),
        tone: "success",
      });
      onOpenChange(false);
      onClosed?.(workspace);
    ...
    onError: (error) => {
      pushToast({
        title: t("workspaces.close.failed"),
        body: error instanceof Error ? error.message : t("workspaces.errors.unknown"),
        tone: "error",
      });
    },
  });
      ...
      <DialogContent className="max-h-(--sz-85vh) overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription className="break-words">
            {t("workspaces.close.descriptionPrefix")} <span className="font-medium text-foreground">{workspaceName}</span>{" "}
            {t("workspaces.close.descriptionSuffix")}
          </DialogDescription>
        </DialogHeader>
        ...
        {readinessQuery.isLoading ? (
          <div className="flex items-center gap-2 rounded-xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
            ...
            {t("workspaces.close.checking")}
          </div>
        ) : readinessQuery.error ? (
          <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">
            {readinessQuery.error instanceof Error ? readinessQuery.error.message : t("workspaces.close.readinessFailed")}
          </div>
        ) : readiness ? (
          <div className="space-y-4">
            <div className={`rounded-xl border px-4 py-3 text-sm ${readinessTone(readiness.state)}`}>
              <div className="font-medium">
                {readiness.state === "blocked"
                  ? t("workspaces.close.states.blocked")
                  : readiness.state === "ready_with_warnings"
                    ? t("workspaces.close.states.warnings")
                    : t("workspaces.close.states.ready")}
              </div>
              <div className="mt-1 text-xs opacity-80">
                {readiness.isSharedWorkspace
                  ? t("workspaces.close.stateDescriptions.shared")
                  : readiness.git?.workspacePath && readiness.git.repoRoot && readiness.git.workspacePath !== readiness.git.repoRoot
                    ? t("workspaces.close.stateDescriptions.independent")
                    : readiness.isProjectPrimaryWorkspace
                      ? t("workspaces.close.stateDescriptions.primary")
                      : t("workspaces.close.stateDescriptions.disposable")}
              </div>
            </div>

            {blockingIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaces.close.blockingTasks")}</h3>
                  ...
                  {blockingIssues.map((issue) => (
                    <div key={issue.id} className="rounded-xl border border-destructive/20 bg-destructive/5 px-4 py-3 text-sm">
                      <div className="flex min-w-0 flex-wrap items-center justify-between gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">{t(`status.${issue.status}`, { defaultValue: issue.status })}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {readiness.blockingReasons.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaces.close.blockingReasons")}</h3>
            ...
            {readiness.warnings.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaces.close.warnings")}</h3>
                <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            {readiness.git ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaces.close.gitStatus")}</h3>
                <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                  <div className="grid gap-2 sm:grid-cols-2">
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("workspaces.fields.branch")}</div>
                      <div className="font-mono text-xs">{readiness.git.branchName ?? t("workspaces.values.unknown")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("workspaces.fields.baseRef")}</div>
                      <div className="font-mono text-xs">{readiness.git.baseRef ?? t("workspaces.values.notSet")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("workspaces.close.mergedIntoBase")}</div>
                      <div>{readiness.git.isMergedIntoBase == null ? t("workspaces.values.unknown") : readiness.git.isMergedIntoBase ? t("workspaces.values.yes") : t("workspaces.values.no")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("workspaces.close.aheadBehind")}</div>
                      <div>
                        {(readiness.git.aheadCount ?? 0).toString()} / {(readiness.git.behindCount ?? 0).toString()}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("workspaces.close.dirtyFiles")}</div>
                      <div>{readiness.git.dirtyEntryCount}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("workspaces.close.untrackedFiles")}</div>
            ...
            {otherLinkedIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaces.close.otherTasks")}</h3>
                <div className="space-y-2">
            ...
            {readiness.runtimeServices.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaces.close.attachedServices")}</h3>
                <div className="space-y-2">
                    ...
                    <div key={service.id} className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                      <div className="flex min-w-0 flex-wrap items-center justify-between gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {t(`workspaces.runtime.status.${service.status}`, { defaultValue: service.status })} ·{" "}
                          {t(`workspaces.runtime.lifecycle.${service.lifecycle}`, { defaultValue: service.lifecycle })}
                        </span>
                      </div>
                      {/*
                      ...
                      */}
                      <div className="mt-1 break-words text-xs text-muted-foreground">
                        {service.url ?? service.command ?? t("workspaces.close.noDetails")}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section className="space-y-2">
              <h3 className="text-sm font-medium">{t("workspaces.close.cleanupActions")}</h3>
              <div className="space-y-2">
            ...
            {currentStatus === "cleanup_failed" ? (
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-muted-foreground">
                {t("workspaces.close.previousCleanupFailed")}
              </div>
            ) : null}

            {currentStatus === "archived" ? (
              <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
                {t("workspaces.close.alreadyArchived")}
              </div>
            ) : null}

            {readiness.git?.repoRoot ? (
              <div className="break-words text-xs text-muted-foreground">
                {t("workspaces.fields.repoRoot")}: <span className="font-mono break-all">{readiness.git.repoRoot}</span>
                {readiness.git.workspacePath ? (
                  <>
                    {" · "}{t("workspaces.fields.workspacePath")}: <span className="font-mono break-all">{readiness.git.workspacePath}</span>
                  </>
                ) : null}
              </div>
            ) : null}
            ...
            <div className="text-xs text-muted-foreground">
              {t("workspaces.close.lastChecked", { time: formatDateTime(new Date(readinessQuery.dataUpdatedAt)) })}
            </div>
          </div>
        ) : null}

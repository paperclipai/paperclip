...
export function ExecutionWorkspaceCloseDialog({
  ...
  const closeWorkspace = useMutation({
    ...
    onSuccess: (workspace) => {
      ...
      pushToast({
        title: currentStatus === "cleanup_failed" ? t("executionWorkspaceDetail.retryClose") : t("closeDialog.closeWorkspace"),
        tone: "success",
      });
      onOpenChange(false);
      onClosed?.(workspace);
    ...
    onError: (error) => {
      pushToast({
        title: t("executionWorkspaceDetail.failedToClose"),
        body: error instanceof Error ? error.message : t("emptyStates.error", { ns: "common" }),
        tone: "error",
      });
    },
  });
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription className="break-words">
            {t("closeDialog.archiveAndCleanup", { name: workspaceName })}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
              ...
              <section className="space-y-2">
                ...
                <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                  <div className="grid gap-2 sm:grid-cols-2">
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("closeDialog.branch")}</div>
                      <div className="font-mono text-xs">{readiness.git.branchName ?? t("closeDialog.unknown")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("closeDialog.baseRef")}</div>
                      <div className="font-mono text-xs">{readiness.git.baseRef ?? t("closeDialog.notSet")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("closeDialog.mergedIntoBase")}</div>
                      <div>{readiness.git.isMergedIntoBase == null ? t("closeDialog.unknown") : readiness.git.isMergedIntoBase ? t("closeDialog.yes") : t("closeDialog.no")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("closeDialog.aheadBehind")}</div>
                      <div>
                        {(readiness.git.aheadCount ?? 0).toString()} / {(readiness.git.behindCount ?? 0).toString()}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("closeDialog.dirtyTrackedFiles")}</div>
                      <div>{readiness.git.dirtyEntryCount}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("closeDialog.untrackedFiles")}</div>
            ...
            {currentStatus === "cleanup_failed" ? (
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-muted-foreground">
                {t("closeDialog.cleanupFailedHint")}
              </div>
            ) : null}
            ...
            <div className="text-xs text-muted-foreground">
              {t("closeDialog.lastChecked", { time: formatDateTime(new Date()) })}
            </div>
          </div>
        ) : null}

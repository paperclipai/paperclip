...
export function ExecutionWorkspaceCloseDialog({
  ...
  const actionLabel = currentStatus === "cleanup_failed" ? t("workspace_close.title_retry_close") : t("workspace_close.title_close_workspace");
  ...
  const closeWorkspace = useMutation({
    ...
    onSuccess: (workspace) => {
      ...
      pushToast({
        title: currentStatus === "cleanup_failed" ? t("workspace_close.toast_workspace_close_retried") : t("workspace_close.toast_workspace_closed"),
        tone: "success",
      });
      onOpenChange(false);
      onClosed?.(workspace);
    ...
    onError: (error) => {
      pushToast({
        title: t("workspace_close.toast_failed_to_close"),
        body: error instanceof Error ? error.message : t("workspace_close.unknown_error"),
        tone: "error",
      });
    },
  });
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription className="break-words">
            {t("workspace_close.archive_prefix")} <span className="font-medium text-foreground">{workspaceName}</span> {t("workspace_close.archive_suffix")}
          </DialogDescription>
        </DialogHeader>
        ...
        {readinessQuery.isLoading ? (
          <div className="flex items-center gap-2 rounded-xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
            ...
            {t("workspace_close.checking_safe_to_close")}
          </div>
        ) : readinessQuery.error ? (
          <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">
            {readinessQuery.error instanceof Error ? readinessQuery.error.message : t("workspace_close.failed_to_inspect")}
          </div>
        ) : readiness ? (
          <div className="space-y-4">
            <div className={`rounded-xl border px-4 py-3 text-sm ${readinessTone(readiness.state)}`}>
              <div className="font-medium">
                {readiness.state === "blocked"
                  ? t("workspace_close.state_blocked")
                  : readiness.state === "ready_with_warnings"
                    ? t("workspace_close.state_with_warnings")
                    : t("workspace_close.state_ready")}
              </div>
              <div className="mt-1 text-xs opacity-80">
                {readiness.isSharedWorkspace
                  ? t("workspace_close.shared_workspace_note")
                  : readiness.git?.workspacePath && readiness.git.repoRoot && readiness.git.workspacePath !== readiness.git.repoRoot
                    ? t("workspace_close.own_checkout_path_note")
                    : readiness.isProjectPrimaryWorkspace
                      ? t("workspace_close.primary_workspace_note")
                      : t("workspace_close.disposable_note")}
              </div>
            </div>

            {blockingIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspace_close.blocking_issues")}</h3>
            ...
            {readiness.blockingReasons.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspace_close.blocking_reasons")}</h3>
            ...
            {readiness.warnings.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspace_close.warnings")}</h3>
                <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            {readiness.git ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspace_close.git_status")}</h3>
                <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                  <div className="grid gap-2 sm:grid-cols-2">
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspace_close.branch")}</div>
                      <div className="font-mono text-xs">{readiness.git.branchName ?? t("workspace_close.unknown")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspace_close.base_ref")}</div>
                      <div className="font-mono text-xs">{readiness.git.baseRef ?? t("workspace_close.not_set")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspace_close.merged_into_base")}</div>
                      <div>{readiness.git.isMergedIntoBase == null ? t("workspace_close.unknown") : readiness.git.isMergedIntoBase ? t("workspace_close.yes_label") : t("workspace_close.no_label")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspace_close.ahead_behind")}</div>
                      <div>
                        {(readiness.git.aheadCount ?? 0).toString()} / {(readiness.git.behindCount ?? 0).toString()}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspace_close.dirty_tracked_files")}</div>
                      <div>{readiness.git.dirtyEntryCount}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspace_close.untracked_files")}</div>
            ...
            {otherLinkedIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspace_close.other_linked_issues")}</h3>
                <div className="space-y-2">
            ...
            {readiness.runtimeServices.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspace_close.attached_runtime_services")}</h3>
                <div className="space-y-2">
                  {readiness.runtimeServices.map((service) => (
                    <div key={service.id} className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                      ...
                      <div className="mt-1 break-words text-xs text-muted-foreground">
                        {service.url ?? service.command ?? service.cwd ?? t("workspace_close.no_additional_details")}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section className="space-y-2">
              <h3 className="text-sm font-medium">{t("workspace_close.cleanup_actions")}</h3>
              <div className="space-y-2">
            ...
            {currentStatus === "cleanup_failed" ? (
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-muted-foreground">
                {t("workspace_close.previously_failed_note")}
              </div>
            ) : null}

            {currentStatus === "archived" ? (
              <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
                {t("workspace_close.already_archived")}
              </div>
            ) : null}

            {readiness.git?.repoRoot ? (
              <div className="break-words text-xs text-muted-foreground">
                {t("workspace_close.repo_root")}: <span className="font-mono break-all">{readiness.git.repoRoot}</span>
                {readiness.git.workspacePath ? (
                  <>
                    {" · "}{t("workspace_close.workspace_path")}: <span className="font-mono break-all">{readiness.git.workspacePath}</span>
                  </>
                ) : null}
              </div>
            ) : null}
            ...
            <div className="text-xs text-muted-foreground">
              {t("workspace_close.last_checked", { time: formatDateTime(new Date()) })}
            </div>
          </div>
        ) : null}

...
const ISSUE_THINKING_EFFORT_VALUES = {
  claude_local: ["", "low", "medium", "high"] as const,
  codex_local: ["", "minimal", "low", "medium", "high", "xhigh"] as const,
  opencode_local: ["", "minimal", "low", "medium", "high", "xhigh", "max"] as const,
} as const;

function thinkingEffortLabelKey(value: string): string {
  switch (value) {
    case "":
      return "newIssueDialog.thinkingDefault";
    case "low":
      return "newIssueDialog.thinkingLow";
    case "medium":
      return "newIssueDialog.thinkingMedium";
    case "high":
      return "newIssueDialog.thinkingHigh";
    case "minimal":
      return "newIssueDialog.thinkingMinimal";
    case "xhigh":
      return "newIssueDialog.thinkingXHigh";
    case "max":
      return "newIssueDialog.thinkingMax";
    default:
      return "newIssueDialog.thinkingDefault";
  }
}
...
const STATUS_VALUES = [
  { value: "backlog", labelKey: "newIssueDialog.statusBacklog", color: issueStatusText.backlog ?? issueStatusTextDefault },
  { value: "todo", labelKey: "newIssueDialog.statusTodo", color: issueStatusText.todo ?? issueStatusTextDefault },
  { value: "in_progress", labelKey: "newIssueDialog.statusInProgress", color: issueStatusText.in_progress ?? issueStatusTextDefault },
  { value: "in_review", labelKey: "newIssueDialog.statusInReview", color: issueStatusText.in_review ?? issueStatusTextDefault },
  { value: "done", labelKey: "newIssueDialog.statusDone", color: issueStatusText.done ?? issueStatusTextDefault },
] as const;

const PRIORITY_VALUES = [
  { value: "critical", labelKey: "newIssueDialog.priorityCritical", icon: AlertTriangle, color: priorityColor.critical ?? priorityColorDefault },
  { value: "high", labelKey: "newIssueDialog.priorityHigh", icon: ArrowUp, color: priorityColor.high ?? priorityColorDefault },
  { value: "medium", labelKey: "newIssueDialog.priorityMedium", icon: Minus, color: priorityColor.medium ?? priorityColorDefault },
  { value: "low", labelKey: "newIssueDialog.priorityLow", icon: ArrowDown, color: priorityColor.low ?? priorityColorDefault },
] as const;

const EXECUTION_WORKSPACE_MODE_VALUES = [
  { value: "shared_workspace", labelKey: "newIssueDialog.workspaceModeShared" },
  { value: "isolated_workspace", labelKey: "newIssueDialog.workspaceModeIsolated" },
  { value: "reuse_existing", labelKey: "newIssueDialog.workspaceModeReuse" },
] as const;
...
export function NewIssueDialog() {
  ...
  const statuses = useMemo(
    () => STATUS_VALUES.map((entry) => ({ value: entry.value, label: t(entry.labelKey), color: entry.color })),
    [t],
  );
  const priorities = useMemo(
    () => PRIORITY_VALUES.map((entry) => ({ value: entry.value, label: t(entry.labelKey), icon: entry.icon, color: entry.color })),
    [t],
  );
  const executionWorkspaceModeOptions = useMemo(
    () => EXECUTION_WORKSPACE_MODE_VALUES.map((entry) => ({ value: entry.value, label: t(entry.labelKey) })),
    [t],
  );
  ...
  const createIssue = useMutation({
    ...
    onSuccess: ({ issue, companyId, failures }) => {
      ...
      if (failures.length > 0) {
        ...
        pushToast({
          title: t("newIssueDialog.toastCreatedWithWarnings", { ref: issueRef }),
          body: t(
            failures.length === 1
              ? "newIssueDialog.toastUploadFailureOne"
              : "newIssueDialog.toastUploadFailureOther",
            { count: failures.length },
          ),
          tone: "warn",
          action: prefix
            ? { label: t("newIssueDialog.toastOpen", { ref: issueRef }), href: `/${prefix}/issues/${issueRef}` }
            : undefined,
        });
      }
      clearDraft();
  ...
  const uploadDescriptionImage = useMutation({
    mutationFn: async (file: File) => {
      if (!effectiveCompanyId) throw new Error(t("newIssueDialog.errorNoCompany"));
  ...
  useEffect(() => {
    ...
    const validThinkingValues =
      assigneeAdapterType === "codex_local"
        ? ISSUE_THINKING_EFFORT_VALUES.codex_local
        : assigneeAdapterType === "opencode_local"
          ? ISSUE_THINKING_EFFORT_VALUES.opencode_local
          : ISSUE_THINKING_EFFORT_VALUES.claude_local;
    if (!validThinkingValues.includes(assigneeThinkingEffort as never)) {
      setAssigneeThinkingEffort("");
    }
  ...
  const thinkingEffortOptions = thinkingEffortValues.map((value) => ({
    value,
    label: t(thinkingEffortLabelKey(value)),
  }));
  ...
  const createIssueErrorMessage =
    createIssue.error instanceof Error ? createIssue.error.message : t("newIssueDialog.errorCreate");
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{isSubIssueMode ? t("newIssueDialog.breadcrumbSubIssue") : t("newIssueDialog.breadcrumb")}</span>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          ...
          <div className="px-4 pb-2">
            <div className="overflow-x-auto overscroll-x-contain">
              ...
              <span className="w-6 shrink-0 text-center">{t("newIssueDialog.forLabel")}</span>
              ...
              />
              <span>{t("newIssueDialog.inLabel")}</span>
              <InlineEntitySelector
                ...
                renderTriggerValue={(option) =>
                  option && currentProject ? (
                    ...
                    <span className="text-muted-foreground">{t("newIssueDialog.projectPlaceholder")}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
          ...
          {currentProject && currentProjectSupportsExecutionWorkspace && (
            ...
            <div className="space-y-1.5">
              ...
              <div className="text-[11px] text-muted-foreground">
                {t("newIssueDialog.executionWorkspaceHint")}
              </div>
              <select
              ...
              {executionWorkspaceMode === "reuse_existing" && selectedReusableExecutionWorkspace && (
                <div className="text-[11px] text-muted-foreground">
                  {t("newIssueDialog.reusingWorkspace", {
                    ...
                    source: selectedReusableExecutionWorkspace.branchName ?? selectedReusableExecutionWorkspace.cwd ?? t("newIssueDialog.existingWorkspaceFallback"),
                  })}
                </div>
              )}
          ...
          {supportsAssigneeOverrides && (
            ...
            {assigneeOptionsOpen && (
              <div className="mt-2 rounded-md border border-border p-3 bg-muted/20 space-y-3">
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">{t("newIssueDialog.modelLabel")}</div>
                  <InlineEntitySelector
                ...
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">{t("newIssueDialog.thinkingEffortLabel")}</div>

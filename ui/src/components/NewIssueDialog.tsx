...
const IssueTitleTextarea = memo(function IssueTitleTextarea({
  ...
  return (
    <textarea
      ...
      placeholder={t("app.newIssueDialog.taskTitle", { defaultValue: "Task title" })}
      rows={1}
      value={draftValue}
      onChange={(e) => {
...
const IssueDescriptionEditor = memo(function IssueDescriptionEditor({
  ...
  return (
    <MarkdownEditor
      ...
      placeholder={t("app.newIssueDialog.addDescription", { defaultValue: "Add description..." })}
      bordered={false}
      mentions={mentions}
...
export function NewIssueDialog() {
  ...
  const assigneeOptionsTitle =
    assigneeAdapterType === "claude_local"
      ? t("app.newIssueDialog.claudeOptions", { defaultValue: "Claude options" })
      : assigneeAdapterType === "codex_local"
        ? t("app.newIssueDialog.codexOptions", { defaultValue: "Codex options" })
        : assigneeAdapterType === "opencode_local"
          ? t("app.newIssueDialog.opencodeOptions", { defaultValue: "OpenCode options" })
        : t("app.newIssueDialog.agentOptions", { defaultValue: "Agent options" });
  ...
  const createIssueErrorMessage =
    createIssue.error instanceof Error ? createIssue.error.message : t("app.newIssueDialog.failedToCreateTaskTryAgain", { defaultValue: "Failed to create task. Try again." });
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{isSubIssueMode ? t("app.newIssueDialog.newSubTask", { defaultValue: "New sub-task" }) : t("app.newIssueDialog.newTask", { defaultValue: "New task" })}</span>
          </div>
        ...
        <div ref={dialogBodyRef} className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          ...
          <div className="px-4 pb-2">
            <div className="overflow-x-auto overscroll-x-contain">
              ...
              <span className="w-6 shrink-0 text-center">{t("app.newIssueDialog.for", { defaultValue: "For" })}</span>
              <InlineEntitySelector
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("app.newIssueDialog.assignee", { defaultValue: "Assignee" })}</span>
                  )
                }
                renderOption={(option) => {
                  ...
                  return (
                    <>
                      ...
                      {assignee && getTrustPreset(assignee.permissions) === "low_trust_review" ? (
                        <ShieldAlert className="ml-auto h-3.5 w-3.5 shrink-0 text-amber-600 dark:text-amber-300" aria-label={t("app.newIssueDialog.lowTrustReviewAgent", { defaultValue: "Low-trust review agent" })} />
                      ) : null}
                    </>
                  );
                }}
              ...
              <InlineEntitySelector
                ...
                renderTriggerValue={(option) =>
                  option && currentProject ? (
                    ...
                    <span className="text-muted-foreground">{t("app.newIssueDialog.project", { defaultValue: "Project" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
              ...
              <Popover open={participantMenuOpen} onOpenChange={setParticipantMenuOpen}>
                <PopoverTrigger asChild>
                  <button
                    ...
                    title={taskWatchdogsEnabled ? t("app.newIssueDialog.addReviewerApproverOrWatchdog", { defaultValue: "Add reviewer, approver, or watchdog" }) : t("app.newIssueDialog.addReviewerOrApprover", { defaultValue: "Add reviewer or approver" })}
                  >
                ...
                <PopoverContent className="w-44 p-1" align="start">
                  ...
                  >
                    ...
                    {t("app.newIssueDialog.reviewer", { defaultValue: "Reviewer" })}
                  </button>
                  <button
                    className={cn(
                      "flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50",
                  ...
                  >
                    ...
                    {t("app.newIssueDialog.approver", { defaultValue: "Approver" })}
                  </button>
                  {taskWatchdogsEnabled && (
                    ...
                    >
                      ...
                      {t("app.newIssueDialog.watchdog", { defaultValue: "Watchdog" })}
                    </button>
                  )}
                </PopoverContent>
              </Popover>
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("app.newIssueDialog.reviewer", { defaultValue: "Reviewer" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("app.newIssueDialog.approver", { defaultValue: "Approver" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
                ...
                <Popover open={watchdogEditorOpen} onOpenChange={setWatchdogEditorOpen}>
                  <PopoverTrigger asChild>
                    <button
                      ...
                      title={t("app.newIssueDialog.configureWatchdog", { defaultValue: "Configure watchdog" })}
                    >
                      {selectedWatchdogAgent ? (
                        ...
                        <span className="text-muted-foreground">{t("app.newIssueDialog.setWatchdog", { defaultValue: "Set watchdog" })}</span>
                      )}
                    </button>
                  </PopoverTrigger>
                  <PopoverContent className="w-80 p-3 space-y-3" align="start">
                    <div className="space-y-1.5">
                      <div className="text-xs font-medium text-foreground">{t("app.newIssueDialog.watchdogAgent", { defaultValue: "Watchdog agent" })}</div>
                      <InlineEntitySelector
                        ...
                        renderTriggerValue={(option) =>
                          option ? (
                            ...
                            <span className="text-muted-foreground">{t("app.newIssueDialog.selectAgent", { defaultValue: "Select agent" })}</span>
                          )
                        }
                        renderOption={(option) => {
                    ...
                    <div className="space-y-1.5">
                      <div className="text-xs font-medium text-foreground">{t("app.newIssueDialog.instructions", { defaultValue: "Instructions" })} <span className="font-normal text-muted-foreground">(optional)</span></div>
                      <Textarea
                        value={watchdogInstructions}
                        onChange={(event) => setWatchdogInstructions(event.target.value)}
                        placeholder={t("app.newIssueDialog.whatShouldTheWatchdogWatchForAndHowShouldItKeepWorkMoving", { defaultValue: "What should the watchdog watch for and how should it keep work moving?" })}
                        rows={4}
                        className="text-xs"
                      />
                    </div>
                    <div className="flex items-center justify-between">
                      ...
                      <Button type="button" size="sm" className="h-7 text-xs" onClick={() => setWatchdogEditorOpen(false)}>
                        {t("app.newIssueDialog.done", { defaultValue: "Done" })}
                      </Button>
                    </div>
                  </PopoverContent>
                </Popover>
          ...
          {isSubIssueMode ? (
            ...
            <div className="max-w-full rounded-md border border-border bg-muted/30 px-2.5 py-1.5 text-xs text-muted-foreground">
              <div className="flex items-center gap-1.5">
                ...
                <span className="shrink-0">{t("app.newIssueDialog.subTaskOf", { defaultValue: "Sub-task of" })}</span>
          ...
          {currentProject && currentProjectSupportsExecutionWorkspace && (
            ...
            <div className="space-y-1.5">
              <div className="text-xs font-medium">{t("app.newIssueDialog.executionWorkspace", { defaultValue: "Execution workspace" })}</div>
              <div className="text-(length:--text-micro) text-muted-foreground">
                {t("app.newIssueDialog.controlWhetherThisTaskRunsInTheSharedWorkspaceANewIsolatedWorkspaceOrAnExistingOne", { defaultValue: "Control whether this task runs in the shared workspace, a new isolated workspace, or an existing one." })}
              </div>
              <select
              ...
              {executionWorkspaceMode === "reuse_existing" && selectedReusableExecutionWorkspace && (
                <div className="text-(length:--text-micro) text-muted-foreground">
                  {t("app.newIssueDialog.reusing", { defaultValue: "Reusing" })} {selectedReusableExecutionWorkspace.name} from {selectedReusableExecutionWorkspace.branchName ?? selectedReusableExecutionWorkspace.cwd ?? "existing execution workspace"}.
                </div>
              )}
              {showParentWorkspaceWarning ? (
                <div className="rounded-md border border-amber-300/60 bg-amber-50 px-2 py-1.5 text-(length:--text-micro) text-amber-900 dark:border-amber-800/70 dark:bg-amber-950/30 dark:text-amber-100">
                  {t("app.newIssueDialog.warningThisSubTaskWillNoLongerUseTheParentTaskWorkspace", { defaultValue: "Warning: this sub-task will no longer use the parent task workspace" })}{parentExecutionWorkspaceLabel ? ` (${parentExecutionWorkspaceLabel})` : ""}.
                </div>
              ) : null}
            </div>
            </div>
          ...
          {supportsAssigneeOverrides && (
            ...
            {assigneeOptionsOpen && (
              <div className="mt-2 rounded-md border border-border p-3 bg-muted/20 space-y-3">
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">{t("app.newIssueDialog.modelLane", { defaultValue: "Model lane" })}</div>
                  <div
                    className="flex w-full overflow-hidden rounded-md border border-border"
                    role="radiogroup"
                    aria-label={t("app.newIssueDialog.modelLane", { defaultValue: "Model lane" })}
                  >
                    {(["primary", ...(assigneeSupportsCheapLane ? (["cheap"] as const) : ([] as const)), "custom"] as const).map((lane) => (
                      ...
                      >
                        {lane === "primary"
                          ? t("app.newIssueDialog.primary", { defaultValue: "Primary" })
                          : lane === "cheap"
                            ? t("app.newIssueDialog.cheap", { defaultValue: "Cheap" })
                            : t("app.newIssueDialog.custom", { defaultValue: "Custom" })}
                      </button>
                    ))}
                  </div>
                  {assigneeModelLane === "cheap" && (
                    <p className="text-(length:--text-micro) text-muted-foreground">
                      {t("app.newIssueDialog.sends", { defaultValue: "Sends" })} <code>modelProfile: "cheap"</code>{" "}
                      {assigneeCheapProfile?.adapterConfig && typeof (assigneeCheapProfile.adapterConfig as Record<string, unknown>).model === "string"
                        ? <>{t("app.newIssueDialog.adapterDefault", { defaultValue: "· adapter default" })} <code>{String((assigneeCheapProfile.adapterConfig as Record<string, unknown>).model)}</code></>
                        : assigneeCheapProfile
                          ? <>{t("app.newIssueDialog.usesCheapProfile", { defaultValue: "· uses the agent's configured cheap profile" })}</>
                          : <>{t("app.newIssueDialog.fallsBackToPrimary", { defaultValue: "· falls back to the primary model if no cheap profile is configured" })}</>}
                    </p>
                  )}
                  {assigneeModelLane === "primary" && (
                    <p className="text-(length:--text-micro) text-muted-foreground">{t("app.newIssueDialog.runsOnTheAgentSPrimaryModel", { defaultValue: "Runs on the agent's primary model." })}</p>
                  )}
                  {assigneeModelLane === "custom" && (
                    <p className="text-(length:--text-micro) text-muted-foreground">{t("app.newIssueDialog.overrideTheModelAndEffortForThisTaskOnly", { defaultValue: "Override the model and effort for this task only." })}</p>
                  )}
                </div>
                {assigneeModelLane === "custom" && (
                  <div className="space-y-1.5">
                    <div className="text-xs text-muted-foreground">{t("app.newIssueDialog.model", { defaultValue: "Model" })}</div>
                    <InlineEntitySelector
                ...
                {assigneeModelLane === "custom" && (
                  <div className="space-y-1.5">
                    <div className="text-xs text-muted-foreground">{t("app.newIssueDialog.thinkingEffort", { defaultValue: "Thinking effort" })}</div>
                ...
                {assigneeAdapterType === "claude_local" && assigneeModelLane === "custom" && (
                  <div className="flex items-center justify-between rounded-md border border-border px-2 py-1.5">
                    <div className="text-xs text-muted-foreground">{t("app.newIssueDialog.enableChrome", { defaultValue: "Enable Chrome (--chrome)" })}</div>
                    <ToggleSwitch
            ...
            {stagedFiles.length > 0 ? (
              ...
              {stagedDocuments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">{t("app.newIssueDialog.documents", { defaultValue: "Documents" })}</div>
                  <div className="space-y-2">
                    {stagedDocuments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        ...
                        <Button
                          ...
                          disabled={createIssue.isPending}
                          title={t("app.newIssueDialog.removeDocument", { defaultValue: "Remove document" })}
                        >
              ...
              {stagedAttachments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">{t("app.newIssueDialog.attachments", { defaultValue: "Attachments" })}</div>
                  <div className="space-y-2">
                    {stagedAttachments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        ...
                        <Button
                          ...
                          onClick={() => removeStagedFile(file.id)}
                          disabled={createIssue.isPending}
                          title={t("app.newIssueDialog.removeAttachment", { defaultValue: "Remove attachment" })}
                        >
                          <X className="h-3.5 w-3.5" />
                        </Button>
                      </div>
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap shrink-0">
              ...
              >
                {currentPriority ? (
                  ...
                  <>
                    ...
                    {t("app.newIssueDialog.priority", { defaultValue: "Priority" })}
                  </>
                )}
              </button>
            </PopoverTrigger>
          ...
          >
            ...
            {t("app.newIssueDialog.upload", { defaultValue: "Upload" })}
          </button>
          ...
          <Popover open={moreOpen} onOpenChange={setMoreOpen}>
            ...
            <PopoverContent className="w-44 p-1" align="start" data-testid="new-issue-more-menu">
              ...
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                ...
                {t("app.newIssueDialog.startDate", { defaultValue: "Start date" })}
              </button>
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                <Calendar className="h-3 w-3" />
                {t("app.newIssueDialog.dueDate", { defaultValue: "Due date" })}
              </button>
            </PopoverContent>
          </Popover>
        </div>
        ...
        {assigneeValue && status === "backlog" ? (
          ...
          >
            ...
            <span className="leading-snug">
              {t("app.newIssueDialog.assigningImpliesExecutableIntentLeaveStatusAs", { defaultValue: "Assigning implies executable intent - leave status as" })} <span className="font-medium">{t("app.newIssueDialog.backlog", { defaultValue: "Backlog" })}</span> {t("app.newIssueDialog.onlyToDeliberatelyParkThisTheAssigneeWillNotBeWokenUntilStatusMovesTo", { defaultValue: "only to deliberately park this. The assignee will not be woken until status moves to" })} <span className="font-medium">{t("app.newIssueDialog.todo", { defaultValue: "Todo" })}</span> or <span className="font-medium">{t("app.newIssueDialog.inProgress", { defaultValue: "In Progress" })}</span>.
            </span>
          </div>
        ) : null}
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border shrink-0">
          ...
          >
            {t("app.newIssueDialog.discardDraft", { defaultValue: "Discard Draft" })}
          </Button>
          <div className="flex items-center gap-3">
            ...
            >
              <span className="inline-flex items-center justify-center gap-1.5">
                ...
                <span>{createIssue.isPending ? t("app.newIssueDialog.creating", { defaultValue: "Creating..." }) : isSubIssueMode ? t("app.newIssueDialog.createSubTask", { defaultValue: "Create Sub-Task" }) : t("app.newIssueDialog.createTask", { defaultValue: "Create Task" })}</span>
              </span>
            </Button>
          </div>
        </div>

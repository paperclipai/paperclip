...
// on arrival and a resumed transfer re-uploads only the parts it lost.
...
export interface ImportTransferDeclaredPart {
  index: number;
  byteSize: number;
  sha256: string;
}

/** Declaration body for POST /companies/import/transfers. */
export interface ImportTransferManifest {
  totalBytes: number;
  zipSha256: string;
  partSizeBytes: number;
  parts: ImportTransferDeclaredPart[];
}
 ...
 */
export async function buildImportTransferManifest(buffer: ArrayBuffer): Promise<ImportTransferManifest> {
  const zipSha256 = await sha256Hex(buffer);
  const parts: ImportTransferDeclaredPart[] = [];

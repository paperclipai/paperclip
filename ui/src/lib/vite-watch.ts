const TEST_DIRECTORY_NAMES = new Set(["__tests__", "_tests", "test", "tests"]);
...
export function shouldIgnoreUiDevWatchPath(watchedPath: string): boolean {
  ...
  return segments.some((segment) => TEST_DIRECTORY_NAMES.has(segment)) || TEST_FILE_BASENAME_RE.test(basename);
}
...
export function createUiDevWatchOptions(currentWorkingDirectory: string) {
  return {
    ...
    ...(currentWorkingDirectory.startsWith("/mnt/") ? { usePolling: true, interval: 1000 } : {}),
  };
}

...
import type { Agent, IssueComment } from "@valadrien-os/shared";
...
function authorNameForComment(
  ...
  if (authorAgentId) {
    return agentMap?.get(authorAgentId)?.name ?? (options?.isSystemNotice ? "ValadrienOs" : authorAgentId.slice(0, 8));
  }

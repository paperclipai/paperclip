...
describe("buildIssuePropertiesPanelKey", () => {
  it("ignores plain updatedAt churn", () => {
    ...
    const second = buildIssuePropertiesPanelKey(createIssue({ updatedAt: new Date("2026-04-12T12:05:00.000Z") }), []);
  ...
  it("changes when a displayed property changes", () => {
    const first = buildIssuePropertiesPanelKey(createIssue(), []);
    const second = buildIssuePropertiesPanelKey(createIssue({ assigneeAgentId: "agent-2" }), []);

...
export function relativeTime(date: Date | string): string {
  ...
  if (diffSec < 60) return translateUiLiteral("just now");
  const diffMin = Math.round(diffSec / 60);
  if (diffMin < 60) return translateUiLiteral("{{value1}}m ago".replace("{{value1}}", String(diffMin)));
  const diffHr = Math.round(diffMin / 60);
  if (diffHr < 24) return translateUiLiteral("{{value1}}h ago".replace("{{value1}}", String(diffHr)));
  const diffDay = Math.round(diffHr / 24);
  if (diffDay < 30) return translateUiLiteral("{{value1}}d ago".replace("{{value1}}", String(diffDay)));

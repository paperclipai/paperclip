...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("odysseus-worktree-enabled") !== "true") return null;

  const name = readMetaContent("odysseus-worktree-name");
  const color = normalizeHexColor(readMetaContent("odysseus-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("odysseus-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}

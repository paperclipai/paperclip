...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("aiteamcorp-worktree-enabled") !== "true") return null;

  const name = readMetaContent("aiteamcorp-worktree-name");
  const color = normalizeHexColor(readMetaContent("aiteamcorp-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("aiteamcorp-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}

...
export function mentionChipInlineStyle(mention: ParsedMentionChip): CSSProperties | undefined {
  ...
  if (mention.kind === "project" && mention.color) {
    ...
    style["--odysseus-mention-project-color"] = mention.color;
  }

  if (mention.kind === "agent") {
    const iconMask = buildAgentIconMask(mention.icon);
    if (iconMask) {
      style["--odysseus-mention-icon-mask"] = iconMask;
    }
  }
...
export function applyMentionChipDecoration(element: HTMLElement, mention: ParsedMentionChip) {
  ...
  element.classList.add("odysseus-mention-chip", `odysseus-mention-chip--${mention.kind}`);
  if (mention.kind === "project") {
    element.classList.add("odysseus-project-mention-chip");
  }
...
export function clearMentionChipDecoration(element: HTMLElement) {
  ...
  element.classList.remove(
    "odysseus-mention-chip",
    "odysseus-mention-chip--agent",
    "odysseus-mention-chip--issue",
    "odysseus-mention-chip--project",
    "odysseus-mention-chip--user",
    "odysseus-mention-chip--skill",
    "odysseus-project-mention-chip",
  );
  element.removeAttribute("contenteditable");
  element.style.removeProperty("border-color");
  element.style.removeProperty("background-color");
  element.style.removeProperty("color");
  element.style.removeProperty("--odysseus-mention-project-color");
  element.style.removeProperty("--odysseus-mention-icon-mask");
}

...
/** 例行任务历史标签页 */
export const routineHistoryTab = {
  // 加载错误状态
  couldNotLoadRevisions: "无法加载修订版本",
  unknownError: "未知错误",
  unknownErrorRestore: "Paperclip 无法还原该修订版本。",
  retry: "重试",
  restoreFailed: "无法还原修订版本",
  // 空状态
  noEditsYet: "尚无编辑记录",
  firstRevisionHint: "修订版本 1 是该例行任务的唯一历史。保存编辑后将创建首个额外修订。",
  // 历史版本列表
  revisions: "修订版本",
  totalRevisions: (count: number) => `共 ${count} 个`,
  current: "当前",
  restored: "已还原",
  showOlder: (count: number) => `显示 ${count} 个更早版本…`,
  // 历史版本预览横幅
  viewingHistorical: (revisionNumber: number) => `正在查看修订版本 ${revisionNumber}（只读）`,
  restoreCreatesNew: (nextRevisionNumber: number) => `还原此版本将创建新的修订版本 ${nextRevisionNumber}，内容保持一致。历史记录保持仅追加。`,
  returnToCurrent: "返回当前版本",
  restoreAsNew: "还原为新修订版本",
  // 未保存编辑冲突横幅
  unsavedEdits: "未保存的例行任务编辑",
  unsavedEditsHint: (fieldsText: string) => `你已修改${fieldsText}但尚未保存。在预览或还原旧版本前，请先保存或放弃更改。`,
  theRoutine: "例行任务",
  discardChanges: "放弃更改",
  saveAndContinue: "保存并继续",
  // 修订版本预览
  savedBy: (actorLabel: string) => `由 ${actorLabel} 保存`,
  compareWithCurrent: "与当前版本比较",
  restoreThisVersion: "还原此版本",
  structuredFields: "结构化字段",
  differsFromCurrent: "与当前版本不同",
  description: "描述",
  noDescription: "无描述",
  triggers: (count: number) => `触发器（${count}）`,
  noTriggers: "此版本中无触发器。",
  enabled: "已启用",
  disabled: "已禁用",
  webhookSecretsHint: "Webhook 密钥不存储在修订版本中。若还原的 Webhook 触发器需要重建，Paperclip 会在还原时生成新的密钥材料。",
  variables: (count: number) => `变量（${count}）`,
  // 字段标签
  fieldTitle: "标题",
  fieldPriority: "优先级",
  fieldStatus: "状态",
  fieldDefaultAgent: "默认智能体",
  fieldProject: "项目",
  fieldConcurrency: "并发策略",
  fieldCatchUp: "补跑策略",
  // 还原确认对话框
  restoreConfirmTitle: (revisionNumber: number) => `确认还原修订版本 ${revisionNumber}？`,
  restoreConfirmDescription: (targetNumber: number, currentNumber: number, newNumber: number) => `这将创建新的修订版本 ${newNumber}，内容与修订版本 ${targetNumber} 一致。修订版本 ${targetNumber}–${currentNumber} 保留在历史中且不会被修改。`,
  confirmRestoreFieldValuesRevert: "例行任务字段值、变量和调度 Cron 将恢复。",
  confirmRestoreHistoryPreserved: "历史运行记录保持不变。",
  confirmRestoreWebhookRecreated: (label: string) => `Webhook 触发器「${label}」将以新的 URL 和密钥重建。Paperclip 会在还原后一次性显示密钥——请在关闭前复制。`,
  changeSummary: "变更摘要",
  changeSummaryHint: "可选——说明为何还原此修订版本",
  confirmRestore: "确认还原",
  restoring: "还原中…",
  restoreAsRevision: (revisionNumber: number) => `还原为修订版本 ${revisionNumber}`,
  webhookDifferences: "Webhook 触发器差异",
  recreatedWebhookHint: "下列 Webhook 触发器在还原时将重建密钥：",
  noWebhookDifferences: "Webhook 触发器配置无变化。",
  cancel: "取消",
  // 比较对话框
  compareRevisionsTitle: "比较例行任务修订版本",
  diffOldLabel: "旧",
  diffNewLabel: "新",
  fieldChangesHeader: "字段变更",
  noFieldChanges: "无结构性字段变更。",
  diffTableField: "字段",
  diffTableOldValue: "旧值",
  diffTableNewValue: "新值",
  descriptionDiffHeader: "描述差异",
  noDescriptionEitherRevision: "两个版本均无描述。",
  descriptionsIdentical: "描述内容完全相同。",
  diffContentHeader: "内容",
  close: "关闭",
  restoreRevAsNew: (revisionNumber: number) => `还原修订版本 ${revisionNumber} 为新修订版本`,
  // 辅助函数
  unassigned: "未分配",
  noProject: "无项目",
  noneVariables: "（无）",
  triggerRemoved: (label: string) => `触发器已移除（${label}）`,
  triggerAdded: (label: string) => `触发器已添加（${label}）`,
  triggerChanged: (label: string) => `触发器变更（${label}）`,
  triggerEnabled: "已启用",
  triggerDisabled: "已禁用",
  actorBoard: "面板",
  actorAgent: "智能体",
...
/** 例行任务变量编辑器 */
export const routineVariablesEditor = {
  title: "变量",
  hint: "从标题和说明中的 `{{name}}` 占位符自动检测。",
  promptBeforeRun: "每次手动运行前会提示用户输入此值。",
  label: "标签",
  type: "类型",
  defaultValue: "默认值",
  required: "必填",
  ...
  falseLabel: "否",
  options: "选项",
  optionsPlaceholder: "高, 中, 低",
  defaultOption: "默认选项",
  textPlaceholder: "默认值",
  numberPlaceholder: "42",
  // 变量帮助对话框
  helpTitle: "例行任务变量",
  helpDescription: "如何提示输入以及 Paperclip 自动填充的变量。",
  customVariables: "自定义变量",
  customVariablesHint: "在标题或说明中的任何位置输入",
  customVariablesList: [
    "名称必须以字母开头，可使用字母、数字和下划线。",
    "选择类型（文本、多行文本、数字、布尔值、选择）、默认值及是否必填。",
    "同一名称在标题和说明中重复出现时，视为同一个变量。",
  ],
  builtInVariables: "内置变量",
  builtInVariablesHint: "这些变量会自动填充——无需设置，也不会出现在变量列表中。",
  placeholder: "占位符",
  example: "示例",
  variableDescription: "说明",
  dateDescription: "例行任务运行时的当前日期，格式为 YYYY-MM-DD（UTC）。",
  timestampDescription: "例行任务运行时的当前日期和时间（UTC），人类可读格式。",
} as const;

/** 例行任务运行对话框 */
export const routineRunDialog = {
  title: "运行例行任务",
  subtitle: "为本次运行选择智能体和可选项目。例行任务默认值已预填，不会被修改。",
  agentLabel: "智能体 *",
  ...
  running: "运行中…",
  runRoutine: "运行例行任务",
  defaultAgentRequired: "本次运行需要默认智能体。",
  missingRequired: (fields: string) => `缺少：${fields}`,
  chooseExistingWorkspace: "运行前请选择现有工作区。",
} as const;

/** 托管例行任务列表 */
export const managedRoutinesList = {
  noManagedRoutines: "无托管例行任务。",
  configure: "配置",
  managedBy: (displayName: string) => `由 ${displayName} 托管`,
  schedule: "调度",
  missingRefs: (refs: string) => `缺少 ${refs}`,
  canRepair: "可修复例行任务默认值。",
  reconciling: "协调中…",
  reconcile: "协调",
  resetting: "重置中…",
  reset: "重置",

...
describe("issue-reference", () => {
  ...
  it("does not treat GitHub issue URLs as internal Odysseus issue links", () => {
    expect(parseIssuePathIdFromPath("https://github.com/PossibLaw/odysseus/issues/1778")).toBeNull();
    expect(parseIssueReferenceFromHref("https://github.com/PossibLaw/odysseus/issues/1778")).toBeNull();
  });
  ...
  it("preserves absolute Odysseus issue URLs so origin, port, and hash are not lost", () => {

...
function formatIssueUpdatedVerb(details: ActivityDetails, options: ActivityFormatOptions = {}): string | null {
  if (!details || !options.t) return null;
  const previous = asRecord(details._previous) ?? {};
  if (details.status !== undefined) {
    const from = previous.status;
    return from
      ? options.t('activity.verbs.changedStatusFromTo', { from: humanizeValue(from), to: humanizeValue(details.status) })
      : options.t('activity.verbs.changedStatusTo', { to: humanizeValue(details.status) });
  }
  if (details.priority !== undefined) {
    const from = previous.priority;
    return from
      ? options.t('activity.verbs.changedPriorityFromTo', { from: humanizeValue(from), to: humanizeValue(details.priority) })
      : options.t('activity.verbs.changedPriorityTo', { to: humanizeValue(details.priority) });
  }
  return null;
}
...
function formatIssueUpdatedAction(details: ActivityDetails, options: ActivityFormatOptions = {}): string | null {
  ...
  if (details.assigneeAgentId !== undefined || details.assigneeUserId !== undefined) {
    ...
    parts.push(assigneeName ? options.t('activity.verbs.assignedTo', { name: assigneeName }) : options.t('activity.verbs.unassigned'));
  }
  if (details.title !== undefined) parts.push(options.t('activity.verbs.updatedTitle'));
  if (details.description !== undefined) parts.push(options.t('activity.verbs.updatedDescription'));

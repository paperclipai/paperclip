  ...
  return (
    <div className="inline-flex flex-wrap gap-1 rounded-md border border-border p-0.5">
      ...
      {!options.some((option) => option.value === value) && (
        <span role="status" className="break-all px-2.5 py-1 text-sm text-muted-foreground">
          Current value: {value} (not listed)
        </span>
      )}
    </div>
  );
...
export function DevinLocalConfigFields({
  ...
  models,
  fusionView,
}: AdapterConfigFieldsProps & { fusionView?: boolean }) {
  ...
  const modelId = isCreate
    ? String(values!.model ?? '')
    : String(eff('adapterConfig', 'model', String((config as Record<string, unknown>).model ?? '')));
  const fusionActive =
    Boolean(fusionView) ||
    isFusionModelId(modelId) ||
    models.some((m) => m.id === modelId && m.fusion != null);
  ...
  return configFieldsForSection(section, (
    <>
      ...
      <Field label="Working directory" hint="Absolute path. Paperclip resolves the run workspace when omitted.">

...
describe("fusion-safe tiers", () => {
  it("dedupes tiers and excludes empty and auto ids", () => {
    expect(buildDevinEffortOptions(["low", "low", "", "auto", "high"], "")).toEqual([
      { id: "", label: "Auto" },
      { id: "low", label: "Low" },
      { id: "high", label: "High" },
    ]);
  ...
  it("does not flag a stored auto as unavailable", () => {
    expect(buildDevinEffortOptions(["low"], "auto")).toEqual([
      { id: "", label: "Auto" },
      { id: "low", label: "Low" },
    ]);
  });

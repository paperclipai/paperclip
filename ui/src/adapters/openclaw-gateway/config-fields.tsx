...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="AiTeamCorp API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "aiteamcorpApiUrl",
                  String(config.aiteamcorpApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "aiteamcorpApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://aiteamcorp.example"
            />
          </Field>
          ...
          <Field label="Claimed API key path">
            <DraftInput
              ...
              immediate
              className={inputClass}
              placeholder="~/.openclaw/workspace/aiteamcorp-claimed-api-key.json"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "aiteamcorp"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. AiTeamCorp persists a device key during onboarding so pairing approvals

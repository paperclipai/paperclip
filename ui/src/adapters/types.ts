...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@valadrien-os/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@valadrien-os/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@valadrien-os/adapter-utils").TranscriptEntry[];

...
function CaseRelationshipsSection({
  ...
  return (
    <section className="space-y-3" aria-label={t("case-detail.case-relationships-98s")}>
      {parent ? (
        <div className="space-y-1">
          <h2 className="text-xs font-medium text-muted-foreground">{t("case-detail.parent-1hn")}</h2>
          <CaseChildrenTree children={[parent]} />
        </div>
      ) : null}
      {children.length > 0 ? (
        <div className="space-y-1">
          <h2 className="text-xs font-medium text-muted-foreground">{t("case-detail.children-349")} {children.length}</h2>
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          aria-label={t("case-detail.change-case-status-1dg")}
        >
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="sm" className="h-6 gap-1 px-2 text-xs text-muted-foreground">
          <Plus className="h-3.5 w-3.5" /> {t("case-detail.labels-1t3")}
        </Button>
      </PopoverTrigger>
      <PopoverContent align="start" className="w-64 p-2">
        <Input
          ...
          placeholder={t("case-detail.search-labels-176")}
        ...
        <div className="max-h-52 space-y-0.5 overflow-y-auto">
          ...
          {filtered.length === 0 && !search.trim() && (
            <p className="px-2 py-1 text-xs text-muted-foreground">{t("case-detail.no-labels-yet-1hp")}</p>
          )}
        </div>
        {search.trim() && !all.some((l) => l.name.toLowerCase() === search.trim().toLowerCase()) && (
          <div className="mt-2 flex items-center gap-2 border-t border-border pt-2">
            <input
              ...
              aria-label={t("case-detail.new-label-color-1kq")}
            />
            <Button
              size="sm"
              variant="secondary"
            ...
            >
              {t("case-detail.create-4ge")}{search.trim()}”
            </Button>
          </div>
        )}
      </PopoverContent>
...
function CasePropertiesContent({
  ...
  return (
    <div className={cn("space-y-4", isFull && "space-y-6")}>
      <PropertySection title={t("case-detail.case-7kv")} first>
        <CasePropertyRow label={t("case-detail.type-1m2")} mode={mode}>
          <PropertyChip>{caseData.caseType}</PropertyChip>
        </CasePropertyRow>
        {caseData.key ? (
          <CasePropertyRow label={t("case-detail.key-1kw")} mode={mode}>
            <CaseCopyableToken
              value={caseData.key}
              label={t("case-detail.case-key-149")}
              className="font-mono text-xs text-muted-foreground"
              truncate={!isFull}
            />
          </CasePropertyRow>
        ) : null}
        <CasePropertyRow label={t("case-detail.labels-1t3")} wrap mode={mode}>
          {caseData.labels.length > 0 ? (
            ...
            <span className="text-xs text-muted-foreground">{t("case-detail.none-dek")}</span>
          )}
          {companyId ? (
            <CaseLabelsPicker
              companyId={companyId}
          ...
          {labelsPending ? <span className="text-xs text-muted-foreground">{t("case-detail.saving-8kf")}</span> : null}
        </CasePropertyRow>
      </PropertySection>

      {propertyRows.length > 0 ? (
        <PropertySection title={t("case-detail.fields-e5r")}>
      ...
      <PropertySection title={t("case-detail.linked-tasks-joa")}>
        {caseData.issueLinks.length === 0 ? (
          <CasePropertyRow label={t("case-detail.tasks-fzj")} mode={mode}>
            <span className="text-xs text-muted-foreground">{t("case-detail.none-yet-i37")}</span>
          </CasePropertyRow>
        ) : (
          <CasePropertyRow label={t("case-detail.tasks-fzj")} wrap mode={mode}>
      ...
      {caseData.attachments.length > 0 ? (
        <PropertySection title={t("case-detail.attachments-892")}>
          <CasePropertyRow label={t("case-detail.files-1s4")} mode={mode}>
  ...
  if (caseQuery.isError || !caseData) {
    return (
      <div className="mx-auto max-w-md py-16 text-center">
        <p className="text-sm text-muted-foreground">{t("case-detail.case-not-found-l6v")}</p>
        <Link to={caseHref()} className="mt-2 inline-block text-sm text-primary hover:underline">
          {t("case-detail.back-to-cases-1i1")}
        </Link>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <header className="space-y-3">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="icon-xs" aria-label={t("case-detail.more-case-actions-1a3")} title={t("case-detail.more-case-actions-1a3")}>
              ...
              <PopoverContent align="end" className="w-48 p-1">
                ...
                >
                  ...
                  {t("case-detail.copy-as-markdown-13f")}
                </button>
                <button
                  type="button"
                  className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-xs hover:bg-accent/50"
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        <TabsList variant="line" className="w-full justify-start gap-1">
          <TabsTrigger value="overview">{t("case-detail.overview-thn")}</TabsTrigger>
          <TabsTrigger value="properties">{t("case-detail.properties-100")}</TabsTrigger>
          <TabsTrigger value="activity">
            {t("case-detail.activity-17y")}{events.length > 0 && <span className="ml-1 text-muted-foreground">{events.length}</span>}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          {description ? (
            <section className="space-y-2">
              <h2 className="text-sm font-semibold">{t("case-detail.description-sjj")}</h2>
          ...
          {caseData.attachments.length > 0 && (
            <section className="space-y-2">
              <h2 className="text-sm font-semibold">{t("case-detail.attachments-pbl")}{caseData.attachments.length})</h2>

...
const STATUS_LABEL: Record<CaseStatus, string> = {
  draft: t("caseDetail.status.draft"),
  in_progress: t("caseDetail.status.inProgress"),
  in_review: t("caseDetail.status.inReview"),
  approved: t("caseDetail.status.approved"),
  done: t("caseDetail.status.done"),
  cancelled: t("caseDetail.status.cancelled"),
};
...
function CaseRelationshipsSection({
  ...
  return (
    <section className="space-y-3" aria-label={t("caseDetail.relationshipsAria")}>
      {parent ? (
      ...
      {children.length > 0 ? (
        <div className="space-y-1">
          <h2 className="text-xs font-medium text-muted-foreground">{t("caseDetail.childrenCount", { count: children.length })}</h2>
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="start" className="w-64 p-2">
        <Input
          ...
          placeholder={t("caseDetail.searchLabels")}
        ...
        {search.trim() && !all.some((l) => l.name.toLowerCase() === search.trim().toLowerCase()) && (
          <div className="mt-2 flex items-center gap-2 border-t border-border pt-2">
            ...
            >
              {t("caseDetail.createLabelNamed", { name: search.trim() })}
            </Button>
          </div>
        )}
      </PopoverContent>
...
function CasePropertiesContent({
  ...
  return (
    <div className={cn("space-y-4", isFull && "space-y-6")}>
      <PropertySection title={t("caseDetail.sectionCase")} first>
        <CasePropertyRow label={t("caseDetail.type")} mode={mode}>
          <PropertyChip>{caseData.caseType}</PropertyChip>
        </CasePropertyRow>
        {caseData.key ? (
          <CasePropertyRow label={t("caseDetail.key")} mode={mode}>
            <CaseCopyableToken
              value={caseData.key}
              label={t("caseDetail.caseKeyLabel")}
        ...
        <CasePropertyRow label={t("caseDetail.labels")} wrap mode={mode}>
          {caseData.labels.length > 0 ? (
            ...
            <span className="text-xs text-muted-foreground">{t("caseDetail.none")}</span>
          )}
          {companyId ? (
            <CaseLabelsPicker
              companyId={companyId}
      ...
      <PropertySection title={childCases.length > 0 ? t("caseDetail.childrenTitleCount", { count: childCases.length }) : t("caseDetail.childrenTitle")}>
          ...
          <CasePropertyRow label={t("caseDetail.files")} mode={mode}>
            <span className="text-xs text-muted-foreground">
              {caseData.attachments.length === 1
                ? t("caseDetail.fileCountOne", { count: caseData.attachments.length })
                : t("caseDetail.fileCountMany", { count: caseData.attachments.length })}
            </span>
          </CasePropertyRow>
        </PropertySection>
      ) : null}
...
export function CaseDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("caseDetail.casesTitle"), href: caseHref() },
      { label: caseData ? `${caseData.identifier} — ${caseData.title}` : (caseIdentifier ?? t("caseDetail.caseFallback")) },
    ]);
  ...
  function copyCaseToClipboard(currentCase: CaseDetailData) {
    const markdown = [
      ...
      "",
      `- ${t("caseDetail.copyKey")}: ${currentCase.key ?? t("caseDetail.copyNone")}`,
      `- ${t("caseDetail.copyType")}: ${currentCase.caseType}`,
      `- ${t("caseDetail.copyStatus")}: ${STATUS_LABEL[currentCase.status]}`,
      currentCase.labels.length > 0 ? `- ${t("caseDetail.copyLabels")}: ${currentCase.labels.map((label) => label.name).join(", ")}` : `- ${t("caseDetail.copyLabels")}: ${t("caseDetail.copyNone")}`,
    ].join("\n");
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <header className="space-y-3">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="icon-xs" aria-label={t("caseDetail.moreActions")} title={t("caseDetail.moreActions")}>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        <TabsList variant="line" className="w-full justify-start gap-1">
          ...
          <TabsTrigger value="properties">{t("caseDetail.tabProperties")}</TabsTrigger>
          <TabsTrigger value="activity">
            {t("caseDetail.tabActivity")}{events.length > 0 && <span className="ml-1 text-muted-foreground">{events.length}</span>}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          {caseData.attachments.length > 0 && (
            <section className="space-y-2">
              <h2 className="text-sm font-semibold">{t("caseDetail.attachmentsCount", { count: caseData.attachments.length })}</h2>

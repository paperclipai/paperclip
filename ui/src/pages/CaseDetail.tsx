    ...
    <section className="space-y-3" aria-label={t("pages.caseDetail.relationshipsAriaLabel", { defaultValue: "Case relationships" })}>
      {parent ? (
        <div className="space-y-1">
          <h2 className="text-xs font-medium text-muted-foreground">{t("pages.caseDetail.parent", { defaultValue: "Parent" })}</h2>
      ...
      {children.length > 0 ? (
        <div className="space-y-1">
          <h2 className="text-xs font-medium text-muted-foreground">
            {t("pages.caseDetail.childrenCount", { defaultValue_one: "{{count}} child", defaultValue_other: "Children {{count}}", count: children.length })}
          </h2>
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          aria-label={t("pages.caseDetail.changeStatus", { defaultValue: "Change case status" })}
        >
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="sm" className="h-6 gap-1 px-2 text-xs text-muted-foreground">
          <Plus className="h-3.5 w-3.5" /> {t("pages.caseDetail.labels", { defaultValue: "Labels" })}
        </Button>
      </PopoverTrigger>
        ...
        {search.trim() && !all.some((l) => l.name.toLowerCase() === search.trim().toLowerCase()) && (
          <div className="mt-2 flex items-center gap-2 border-t border-border pt-2">
            <input
              ...
              aria-label={t("pages.caseDetail.newLabelColor", { defaultValue: "New label color" })}
            />
            <Button
              size="sm"
              variant="secondary"
            ...
            >
              {t("pages.caseDetail.createLabel", { defaultValue: "Create “{{name}}”", name: search.trim() })}
            </Button>
          </div>
        )}
      </PopoverContent>
...
function CasePropertiesContent({
  ...
  return (
    <div className={cn("space-y-4", isFull && "space-y-6")}>
      <PropertySection title={t("pages.caseDetail.caseSection", { defaultValue: "Case" })} first>
        <CasePropertyRow label={t("pages.caseDetail.type", { defaultValue: "Type" })} mode={mode}>
        ...
        {caseData.key ? (
          <CasePropertyRow label={t("pages.caseDetail.key", { defaultValue: "Key" })} mode={mode}>
            <CaseCopyableToken
              value={caseData.key}
              label={t("pages.caseDetail.copyCaseKeyLabel", { defaultValue: "case key" })}
              className="font-mono text-xs text-muted-foreground"
              truncate={!isFull}
            />
          </CasePropertyRow>
        ) : null}
        <CasePropertyRow label={t("pages.caseDetail.labels", { defaultValue: "Labels" })} wrap mode={mode}>
          {caseData.labels.length > 0 ? (
            ...
            <span className="text-xs text-muted-foreground">{t("pages.caseDetail.none", { defaultValue: "None" })}</span>
          )}
          {companyId ? (
            <CaseLabelsPicker
              companyId={companyId}
      ...
      {propertyRows.length > 0 ? (
        <PropertySection title={t("pages.caseDetail.fieldsSection", { defaultValue: "Fields" })}>
      ...
      <PropertySection title={t("pages.caseDetail.linkedTasks", { defaultValue: "Linked tasks" })}>
        {caseData.issueLinks.length === 0 ? (
          <CasePropertyRow label={t("pages.caseDetail.tasks", { defaultValue: "Tasks" })} mode={mode}>
            <span className="text-xs text-muted-foreground">{t("pages.caseDetail.noneYet", { defaultValue: "None yet" })}</span>
          </CasePropertyRow>
        ) : (
          <CasePropertyRow label={t("pages.caseDetail.tasks", { defaultValue: "Tasks" })} wrap mode={mode}>
      ...
      <PropertySection title={childCases.length > 0
        ? t("pages.caseDetail.childrenCount", { defaultValue_one: "{{count}} child", defaultValue_other: "Children {{count}}", count: childCases.length })
        : t("pages.caseDetail.children", { defaultValue: "Children" })}>
        <CaseChildrenTree children={childCases} />
      </PropertySection>

      {caseData.attachments.length > 0 ? (
        <PropertySection title={t("pages.caseDetail.attachmentsSection", { defaultValue: "Attachments" })}>
          <CasePropertyRow label={t("pages.caseDetail.files", { defaultValue: "Files" })} mode={mode}>
            <span className="text-xs text-muted-foreground">
              {t("pages.caseDetail.fileCount", { defaultValue_one: "{{count}} file", defaultValue_other: "{{count}} files", count: caseData.attachments.length })}
            </span>
          </CasePropertyRow>
        </PropertySection>
      ) : null}
...
export function CaseDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("pages.caseDetail.casesCrumb", { defaultValue: "Cases" }), href: caseHref() },
      { label: caseData ? `${caseData.identifier} — ${caseData.title}` : (caseIdentifier ?? t("pages.caseDetail.caseFallback", { defaultValue: "Case" })) },
    ]);
  ...
  function statusLabel(status: CaseStatus): string {
    switch (status) {
      case "draft": return t("pages.caseDetail.statusDraft", { defaultValue: "Draft" });
      case "in_progress": return t("pages.caseDetail.statusInProgress", { defaultValue: "In progress" });
      case "in_review": return t("pages.caseDetail.statusInReview", { defaultValue: "In review" });
      case "approved": return t("pages.caseDetail.statusApproved", { defaultValue: "Approved" });
      case "done": return t("pages.caseDetail.statusDone", { defaultValue: "Done" });
      case "cancelled": return t("pages.caseDetail.statusCancelled", { defaultValue: "Cancelled" });
    }
  }

  function copyCaseToClipboard(currentCase: CaseDetailData) {
    const noneValue = t("pages.caseDetail.markdownNone", { defaultValue: "none" });
    const labelsValue = currentCase.labels.length > 0
      ? currentCase.labels.map((label) => label.name).join(", ")
      : noneValue;
    const markdown = [
      t("pages.caseDetail.markdownTitle", { defaultValue: "# {{identifier}} {{title}}", identifier: currentCase.identifier, title: currentCase.title }),
      "",
      t("pages.caseDetail.markdownKey", { defaultValue: "- Key: {{value}}", value: currentCase.key ?? noneValue }),
      t("pages.caseDetail.markdownType", { defaultValue: "- Type: {{value}}", value: currentCase.caseType }),
      t("pages.caseDetail.markdownStatus", { defaultValue: "- Status: {{value}}", value: statusLabel(currentCase.status) }),
      t("pages.caseDetail.markdownLabels", { defaultValue: "- Labels: {{value}}", value: labelsValue }),
    ].join("\n");
    ...
    <div className="mx-auto max-w-3xl space-y-6">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon-xs"
                  aria-label={t("pages.caseDetail.moreActions", { defaultValue: "More case actions" })}
                  title={t("pages.caseDetail.moreActions", { defaultValue: "More case actions" })}
                >
              ...
              <PopoverContent align="end" className="w-48 p-1">
                ...
                >
                  ...
                  {t("pages.caseDetail.properties", { defaultValue: "Properties" })}
                </button>
              </PopoverContent>
            </Popover>
          </div>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        <TabsList variant="line" className="w-full justify-start gap-1">
          <TabsTrigger value="overview">{t("pages.caseDetail.overviewTab", { defaultValue: "Overview" })}</TabsTrigger>
          <TabsTrigger value="properties">{t("pages.caseDetail.properties", { defaultValue: "Properties" })}</TabsTrigger>
          <TabsTrigger value="activity">
            {t("pages.caseDetail.activityTab", { defaultValue: "Activity" })}{events.length > 0 && <span className="ml-1 text-muted-foreground">{events.length}</span>}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          {description ? (
            <section className="space-y-2">
              <h2 className="text-sm font-semibold">{t("pages.caseDetail.description", { defaultValue: "Description" })}</h2>
          ...
          {caseData.attachments.length > 0 && (
            <section className="space-y-2">
              <h2 className="text-sm font-semibold">
                {t("pages.caseDetail.attachmentsCount", { defaultValue: "Attachments ({{count}})", count: caseData.attachments.length })}
              </h2>

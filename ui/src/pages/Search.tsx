        ...
        <div className="mt-2 flex min-h-6 flex-wrap items-center gap-1.5 text-(length:--text-micro) text-muted-foreground">
          ...
          {operatorSuggestions.length > 0 ? (
            <div className="flex flex-wrap items-center gap-1.5" data-testid="search-operator-suggestions">
              {operatorSuggestions.map((suggestion) => (
                <button
                  ...
                  type="button"
                  aria-label={t("search.insertOperator", { operator: suggestion.token })}
            ...
            <span className="truncate">
              {t("search.tryHintPrefix")} <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">status:todo</code>,{" "}
              <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">assignee:me</code>,{" "}
              {t("search.tryHintOr")} <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">updated:&gt;7d</code>.
            </span>
          )}
        </div>
      </div>
  ...
  if (showInitialState) {
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        <div>
          <h2 className="text-lg font-semibold">{t("search.initialTitle")}</h2>
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            <span className="font-medium text-foreground">{t("search.identifierLookupLabel")}</span> type{" "}
            <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">PAP-123</code> {t("search.identifierLookupSuffix")}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("search.quotedPhrasesLabel")}</span> {t("search.quotedPhrasesHint")}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("search.cmdKLabel")}</span> {t("search.cmdKHint")}
          </li>
        </ul>
      </div>
    );
  ...
  if (hasError) {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="text-base font-semibold">{t("search.couldNotRun")}</div>
        <p className="text-sm text-muted-foreground">
          {status ? t("search.serverReturned", { status }) : t("search.requestFailed")} {t("search.retryHint")}
        </p>
  ...
  if (isEmpty) {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {t("search.noMatchHint", { scope: describeScope(scope).toLowerCase() })}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
        ...
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          ...
          <li>{t("search.tipQuotedPhrases")}</li>
        </ul>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between py-2 text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
        <span>
          {allMatchTotal > totalResults
            ? t("search.resultsOfTotal", { shown: totalResults, total: allMatchTotal })
            : totalResults === 1
              ? t("search.oneResult")
              : t("search.resultsCount", { count: totalResults })}
          {` · ${t("search.sortedBy", { sort: sortLabel })}`}
          {activeFilterCount > 0

...
const SCOPE_LABELS: Record<CompanySearchScope, string> = {
  ...
  issues: "search.scopes.issues",
  comments: "search.scopes.comments",
  documents: "search.scopes.documents",
  agents: "search.scopes.agents",
  projects: "search.scopes.projects",
};
...
function describeScope(scope: CompanySearchScope, t: any) {
      ...
      return {
        ...
        label: (
          <span className="flex items-center">
            {t(SCOPE_LABELS[value as CompanySearchScope])}
  ...
  if (showInitialState) {
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        <div>
          <h2 className="text-lg font-semibold">{t("search.initial.title")}</h2>
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            <span className="font-medium text-foreground">{t("search.initial.identifierLabel")}</span>{" "}
            {t("search.initial.identifierDesc")}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("search.initial.quotesLabel")}</span>{" "}
            {t("search.initial.quotesDesc")}
          </li>
          <li>
            <span className="font-medium text-foreground">⌘K:</span> {t("search.initial.shortcutDesc")}
          </li>
        </ul>
      </div>
    );
  ...
  if (hasError) {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="text-base font-semibold">{t("search.error.title")}</div>
        <p className="text-sm text-muted-foreground">
          {status ? t("search.error.serverError", { status }) : t("search.error.requestFailed")} {t("search.error.retryHint")}
        </p>
    ...
    return (
      <div className="flex flex-col gap-2 px-2 py-3 sm:px-4">
        <div className="px-3 text-xs text-muted-foreground" data-testid="search-loading">
          {t("search.loading", { query: trimmedQuery })}
        </div>
  ...
  if (isEmpty) {
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="text-base font-semibold">{t("search.empty.title", { query: trimmedQuery })}</div>
        <p className="text-sm text-muted-foreground">
          {t("search.empty.subtitle", { scope: describeScope(scope, t).toLowerCase() })}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
            <Button onClick={showAllScope} size="sm" variant="outline">
              {t("search.empty.searchAll")}
            </Button>
          ) : null}
          <Button onClick={openNewIssue} size="sm" variant="default">
            <Plus className="mr-1.5 h-4 w-4" />
            {t("search.empty.createIssue")}
          </Button>
        ...
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          <li>{t("search.empty.tip1")}</li>
          <li>{t("search.empty.tip2")}</li>
          <li>{t("search.empty.tip3")}</li>
        </ul>
      </div>
    );
  }
  ...
  return (
    <div className="flex w-full max-w-[960px] flex-col px-2 sm:px-4" data-testid="search-results">
      <div className="flex items-center justify-between py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
        <span>
          {t("search.resultsCount", { count: totalResults })} · {t("search.sortedByRelevance")}
        </span>
      ...
      <div className="flex flex-col pb-10">
        {scope === "all" ? (
          subgroups.map((group, groupIndex) => (
            ...
            >
              <IssueGroupHeader
                label={t(SUBGROUP_LABELS[group.key])}
                trailing={

  ...
  return (
    <div className="flex h-full min-h-0 flex-col" data-page="search">
      <div className="border-b border-border px-4 py-3 sm:px-6">
        <h1 className="sr-only">{t("search.search-1j0")}</h1>
        <div className="relative">
          ...
          <Input
            ...
            placeholder={t("search.search-tasks-comments-documents-arti-qm1")}
            aria-label={t("search.search-query-c6n")}
          ...
          {draftQuery.length > 0 ? (
            <button
              type="button"
              onClick={handleClear}
              aria-label={t("search.clear-search-ozr")}
        ...
        <div className="mt-2 flex min-h-6 flex-wrap items-center gap-1.5 text-(length:--text-micro) text-muted-foreground">
          ...
          {operatorSuggestions.length > 0 ? (
            ...
            <span className="truncate">
              {t("search.try-42e")} <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">status:todo</code>,{" "}
  ...
  if (showInitialState) {
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        <div>
          <h2 className="text-lg font-semibold">{t("search.type-to-search-organization-memory-1nt")}</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("search.tasks-comments-plan-documents-artifa-2vc")}
          </p>
        </div>
        {recentSearches.length > 0 ? (
          <div>
            <div className="mb-2 text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
              {t("search.recent-searches-1sl")}
            </div>
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            <span className="font-medium text-foreground">{t("search.identifier-lookup-1y6")}</span> type{" "}
            <code className="rounded bg-muted px-1 py-0.5 text-(length:--text-micro)">PAP-123</code> {t("search.to-jump-straight-to-a-task-1cu")}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("search.quoted-phrases-109")}</span> {t("search.wrap-a-phrase-in-quotes-to-match-the-3hn")}
          </li>
          <li>
            <span className="font-medium text-foreground">⌘K:</span> {t("search.reopens-the-command-palette-pre-seed-7ap")}
          </li>
        </ul>
      </div>
    );
  ...
  if (hasError) {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="text-base font-semibold">{t("search.couldn-t-run-that-search-sex")}</div>
        <p className="text-sm text-muted-foreground">
          {status ? `The server returned ${status}.` : "The request failed."} {t("search.your-input-and-filters-are-still-her-18w")}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          ...
          <Button onClick={navigateIssuesFallback} variant="outline" size="sm">
            {t("search.open-tasks-filter-view-163")}
          </Button>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex flex-col gap-2 px-2 py-3 sm:px-4">
        <div className="px-3 text-xs text-muted-foreground" data-testid="search-loading">
          {t("search.searching-for-mla")}{trimmedQuery}&rdquo;…
        </div>
  ...
  if (isEmpty) {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="text-base font-semibold">{t("search.no-results-for-ujh")}{trimmedQuery}&rdquo;</div>
        <p className="text-sm text-muted-foreground">
          {t("search.we-couldn-t-find-a-match-in-1j4")} {describeScope(scope).toLowerCase()}{t("search.try-widening-the-scope-or-rephrasing-6sc")}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
            <Button onClick={showAllScope} size="sm" variant="outline">
              {t("search.search-all-scopes-kt7")}
            </Button>
          ) : null}
          <Button onClick={openNewIssue} size="sm" variant="default">
            <Plus className="mr-1.5 h-4 w-4" />
            {t("search.create-task-from-this-query-xvm")}
          </Button>
        ...
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          <li>{t("search.try-fewer-tokens-or-a-single-distinc-wud")}</li>
          <li>
            {t("search.use-an-identifier-shortcut-like-ljb")} <code className="rounded bg-muted px-1 py-0.5">PAP-123</code>.
          </li>
          <li>{t("search.wrap-multi-word-phrases-in-quotes-1ed")}</li>
        </ul>
      </div>
    );
  }
  ...
  return (
    <div className="flex w-full max-w-(--sz-960px) flex-col px-2 sm:px-4" data-testid="search-results">
      <div className="flex items-center justify-between py-2 text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
        ...
        {isFetching ? <span aria-live="polite" className="normal-case tracking-normal">{t("search.updating-ly6")}</span> : null}
      </div>

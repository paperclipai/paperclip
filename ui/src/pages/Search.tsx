...
function scopeLabels(t: TFn): Record<CompanySearchScope, string> {
  return {
    all: t("search.scope.all", { defaultValue: "All" }),
    issues: t("search.scope.issues", { defaultValue: "Issues" }),
    comments: t("search.scope.comments", { defaultValue: "Comments" }),
    documents: t("search.scope.documents", { defaultValue: "Documents" }),
    agents: t("search.scope.agents", { defaultValue: "Agents" }),
    projects: t("search.scope.projects", { defaultValue: "Projects" }),
  };
}
...
function subgroupLabels(t: TFn): Record<SubGroupKey, string> {
  return {
    issues: t("search.subgroup.issues", { defaultValue: "Issues" }),
    comments: t("search.subgroup.comments", { defaultValue: "Comments" }),
    documents: t("search.subgroup.documents", { defaultValue: "Documents" }),
    agents: t("search.subgroup.agents", { defaultValue: "Agents" }),
    projects: t("search.subgroup.projects", { defaultValue: "Projects" }),
  };
}
...
function describeScope(scope: CompanySearchScope, t: TFn) {
  const labels = scopeLabels(t);
  if (scope === "all") return t("search.allScopes", { defaultValue: "All scopes" });
  return labels[scope];
}
...
function shapeError(error: unknown, t: TFn): { message: string; status?: number } {
  if (!error) return { message: t("search.unknownError", { defaultValue: "Unknown error" }) };
      ...
      return {
        ...
        label: (
          <span className="flex items-center">
            {labels[value as CompanySearchScope]}
      ...
      <div className="border-b border-border px-4 py-3 sm:px-6">
        ...
        <div className="relative">
          ...
          <Input
            ...
            placeholder={t("search.placeholder", { defaultValue: "Search issues, comments, documents, agents, projects…" })}
            aria-label={t("search.queryLabel", { defaultValue: "Search query" })}
...
function SearchTabContent({
  ...
  const { t } = useTranslation();
  const subLabels = subgroupLabels(t);
  if (showInitialState) {
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        <div>
          <h2 className="text-lg font-semibold">{t("search.initial.heading", { defaultValue: "Type to search company memory." })}</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("search.initial.subheading", { defaultValue: "Issues, comments, plan documents, agents, projects — same surface, ranked by relevance." })}
          </p>
        </div>
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            <span className="font-medium text-foreground">{t("search.tip.identifierLabel", { defaultValue: "Identifier lookup:" })}</span> {t("search.tip.identifierBefore", { defaultValue: "type" })}{" "}
            <code className="rounded bg-muted px-1 py-0.5 text-[11px]">PAP-123</code> {t("search.tip.identifierAfter", { defaultValue: "to jump straight to an issue." })}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("search.tip.quotedLabel", { defaultValue: "Quoted phrases:" })}</span> {t("search.tip.quotedDesc", { defaultValue: "wrap a phrase in quotes to match the exact sequence." })}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("search.tip.cmdKLabel", { defaultValue: "⌘K:" })}</span> {t("search.tip.cmdKDesc", { defaultValue: "reopens the command palette pre-seeded with your current query." })}
          </li>
        </ul>
      </div>
    );
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {status ? t("search.error.serverReturned", { defaultValue: "The server returned {{status}}.", status }) : t("search.error.requestFailed", { defaultValue: "The request failed." })} {t("search.error.retryHint", { defaultValue: "Your input and filters are still here, so you can retry or fall back to the Issues filter." })}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          ...
          <Button onClick={navigateIssuesFallback} variant="outline" size="sm">
            {t("search.openIssuesFilter", { defaultValue: "Open Issues filter view" })}
          </Button>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex flex-col gap-2 px-2 py-3 sm:px-4">
        <div className="px-3 text-xs text-muted-foreground" data-testid="search-loading">
          {t("search.searchingFor", { defaultValue: "Searching for “{{query}}”…", query: trimmedQuery })}
        </div>
  ...
  if (isEmpty) {
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {t("search.empty.body", { defaultValue: "We couldn’t find a match in {{scope}}. Try widening the scope or rephrasing your query.", scope: describeScope(scope, t).toLowerCase() })}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
            <Button onClick={showAllScope} size="sm" variant="outline">
              {t("search.searchAllScopes", { defaultValue: "Search all scopes" })}
            </Button>
          ) : null}
          <Button onClick={openNewIssue} size="sm" variant="default">
            <Plus className="mr-1.5 h-4 w-4" />
            {t("search.createIssueFromQuery", { defaultValue: "Create issue from this query" })}
          </Button>
        ...
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          <li>{t("search.empty.hint1", { defaultValue: "Try fewer tokens or a single distinctive term." })}</li>
          <li>
            {t("search.empty.hint2Before", { defaultValue: "Use an identifier shortcut like" })} <code className="rounded bg-muted px-1 py-0.5">PAP-123</code>.
          </li>
          <li>{t("search.empty.hint3", { defaultValue: "Wrap multi-word phrases in quotes." })}</li>
        </ul>
      </div>
    );
  }
  ...
  return (
    <div className="flex w-full max-w-[960px] flex-col px-2 sm:px-4" data-testid="search-results">
      <div className="flex items-center justify-between py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
        <span>
          {totalResults === 1
            ? t("search.resultsSummaryOne", { defaultValue: "1 result · sorted by relevance" })
            : t("search.resultsSummary", { defaultValue: "{{count}} results · sorted by relevance", count: totalResults })}
        </span>
        {isFetching ? <span aria-live="polite" className="normal-case tracking-normal">{t("search.updating", { defaultValue: "Updating…" })}</span> : null}
      </div>
      <div className="flex flex-col pb-10">
        {scope === "all" ? (
          subgroups.map((group, groupIndex) => (
            <section
              key={group.key}
              aria-label={subLabels[group.key]}
              className={cn("flex flex-col", groupIndex > 0 && "mt-6")}
            >
              <IssueGroupHeader
                label={subLabels[group.key]}
                trailing={

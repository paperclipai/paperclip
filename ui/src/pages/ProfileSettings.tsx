...
import { profileSettingsPage, sidebarNav } from "../lib/i18n";
...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: sidebarNav.instanceProfile },
    ]);
  }, [setBreadcrumbs]);
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : profileSettingsPage.failedToUpdateProfile);
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(profileSettingsPage.selectCompanyBeforeUpload);
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : profileSettingsPage.failedToUploadAvatar);
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : profileSettingsPage.failedToRemoveAvatar);
    },
  });

  if (sessionQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{profileSettingsPage.loadingProfile}</div>;
  }

  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : profileSettingsPage.failedToLoadProfile}
      </div>
    );
  }
  ...
  const uploadHint = selectedCompany
    ? profileSettingsPage.storedInStorage(selectedCompany.name)
    : profileSettingsPage.selectCompanyToUpload;
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{profileSettingsPage.title}</h1>
        </div>
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-[24px] border border-border/70 bg-background/92 p-5 shadow-[0_18px_44px_-28px_rgba(0,0,0,0.45)] backdrop-blur-sm">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? profileSettingsPage.changePhoto : profileSettingsPage.uploadPhoto}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">{sessionQuery.data.user.email ?? profileSettingsPage.noEmail}</p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {profileSettingsPage.subtitle} {uploadHint}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">{profileSettingsPage.displayName}</Label>
            <Input
              id="profile-name"
              value={name}
            ...
            <p className="text-xs text-muted-foreground">
              {profileSettingsPage.displayNameHint}
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="profile-email">{profileSettingsPage.emailLabel}</Label>
            <Input
              id="profile-email"
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? profileSettingsPage.saving : profileSettingsPage.saveProfile}
            </Button>
          </div>
        </form>
      </section>

...
export function ProfileSettings() {
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.profile.updateFailed"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("instanceSettings.profile.selectCompanyAvatar"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.profile.uploadAvatarFailed"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.profile.removeAvatarFailed"));
    },
  });

  if (sessionQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("instanceSettings.profile.loading")}</div>;
  }

  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("instanceSettings.profile.loadFailed")}
      </div>
    );
  }
  ...
  const uploadHint = selectedCompany
    ? `${t("instanceSettings.profile.uploadHintStored", "Stored in Paperclip file storage for {{name}}.", { name: selectedCompany.name })}`
    : t("instanceSettings.profile.uploadHintSelect");
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {t("instanceSettings.profile.description")}
        </p>
      </div>
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-[24px] border border-border/70 bg-background/92 p-5 shadow-[0_18px_44px_-28px_rgba(0,0,0,0.45)] backdrop-blur-sm">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? t("instanceSettings.profile.changePhoto") : t("instanceSettings.profile.uploadPhoto")}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">{sessionQuery.data.user.email ?? t("instanceSettings.profile.noEmail")}</p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("instanceSettings.profile.clickAvatar")} {uploadHint}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">{t("instanceSettings.profile.displayName")}</Label>
            <Input
              id="profile-name"
              value={name}
              onChange={(event) => setName(event.target.value)}
              maxLength={120}
              placeholder={t("instanceSettings.profile.boardPlaceholder")}
            />
            <p className="text-xs text-muted-foreground">
              {t("instanceSettings.profile.displayNameHint")}
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              {t("instanceSettings.profile.emailHint")}
            </p>
          </div>
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("common.saving") : t("instanceSettings.profile.saveProfile")}
            </Button>
          </div>
        </form>
      </section>

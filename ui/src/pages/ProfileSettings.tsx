...
export function ProfileSettings() {
  ...
  function resolveProfileName() {
    return name.trim() || sessionQuery.data?.user.name || t("account.board");
  }
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.profilePage.updateFailed"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("settings.profilePage.selectCompanyBeforeUpload"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.profilePage.uploadFailed"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.profilePage.removeFailed"));
    },
  });

  if (sessionQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("settings.profilePage.loading")}</div>;
  }

  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("settings.profilePage.loadFailed")}
      </div>
    );
  }

  const currentName = name.trim() || sessionQuery.data.user.name || t("account.board");
  ...
  const uploadHint = selectedCompany
    ? t("settings.profilePage.storedForCompany", { name: selectedCompany.name })
    : t("settings.profilePage.selectCompanyForAvatar");
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-(--rad-24) border border-border/70 bg-background/92 p-5 shadow-(--shadow-extract-18) backdrop-blur-sm">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? t("settings.profilePage.changePhoto") : t("settings.profilePage.uploadPhoto")}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">{sessionQuery.data.user.email ?? t("settings.profilePage.noEmail")}</p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("settings.profilePage.clickAvatar")} {uploadHint}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">{t("settings.profilePage.displayName")}</Label>
            <Input
              id="profile-name"
              value={name}
              onChange={(event) => setName(event.target.value)}
              maxLength={120}
              placeholder={t("account.board")}
            />
            <p className="text-xs text-muted-foreground">
              {t("settings.profilePage.displayNameHint")}
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              {t("settings.profilePage.emailHint")}
            </p>
          </div>
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("common.saving") : t("settings.profilePage.saveProfile")}
            </Button>
          </div>
        </form>

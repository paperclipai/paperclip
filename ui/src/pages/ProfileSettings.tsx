...
export function ProfileSettings() {
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("pages.profile.failedToUpdate"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("pages.profile.selectCompanyBeforeAvatar"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("pages.profile.failedToUploadAvatar"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("pages.profile.failedToRemoveAvatar"));
    },
  });

  if (sessionQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("pages.profile.loading")}</div>;
  }

  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("pages.profile.failedToLoad")}
      </div>
    );
  }
  ...
  const uploadHint = selectedCompany
    ? t("pages.profile.storedInFileStorage", { company: selectedCompany.name })
    : t("pages.profile.selectCompanyToUpload");
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("pages.profile.title")}</h1>
        </div>
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-(--rad-24) border border-border/70 bg-background/92 p-5 shadow-(--shadow-extract-18) backdrop-blur-sm">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? t("pages.profile.changePhoto") : t("pages.profile.uploadPhoto")}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">
                    {sessionQuery.data.user.email ?? t("pages.profile.noEmail")}
                  </p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("pages.profile.clickAvatarToUpload")} {uploadHint}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">{t("pages.profile.displayName")}</Label>
            <Input
              id="profile-name"
              value={name}
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("pages.profile.saving") : t("pages.profile.saveProfile")}
            </Button>
          </div>
        </form>

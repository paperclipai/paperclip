...
export function ProfileSettings() {
  ...
  function resolveProfileName() {
    return name.trim() || sessionQuery.data?.user.name || t("profileSettings.defaultName");
  }
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("profileSettings.uploadRequiresCompany"));
      }
  ...
  const currentName = name.trim() || sessionQuery.data.user.name || t("profileSettings.defaultName");
  ...
  const uploadHint = selectedCompany
    ? t("profileSettings.uploadStoredHint", { name: selectedCompany.name })
    : t("profileSettings.uploadSelectCompanyHint");
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-[24px] border border-border/70 bg-background/92 p-5 shadow-[0_18px_44px_-28px_rgba(0,0,0,0.45)] backdrop-blur-sm">
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                ...
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("profileSettings.avatarHint", { hint: uploadHint })}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            ...
            <Input
              ...
              maxLength={120}
              placeholder={t("profileSettings.defaultName")}
            />
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("common.saving") : t("profileSettings.saveProfile")}
            </Button>
          </div>
        </form>
      </section>

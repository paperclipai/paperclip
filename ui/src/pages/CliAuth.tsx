    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <Card className="block p-6">
          <h1 className="text-lg font-semibold">{t("pages.cliAuth.unavailable")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {challengeQuery.error instanceof Error ? challengeQuery.error.message : t("pages.cliAuth.invalidOrExpired")}
          </p>
        </Card>
      </div>
    );
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <Card className="block p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {t("pages.cliAuth.restartDescription")}
          </p>
        </Card>
      </div>
    );
      ...
      <div className="mx-auto max-w-xl py-10">
        <Card className="block p-6">
          ...
          <Button asChild className="mt-4">
            <Link to={`/auth?next=${encodeURIComponent(currentPath)}`}>{t("pages.cliAuth.signInOrCreate")}</Link>
          </Button>
        </Card>
      </div>
    );
    ...
    <div className="mx-auto max-w-xl py-10">
      <Card className="block p-6">
        ...
        <div className="mt-5 space-y-3 text-sm">
          ...
          <div>
            ...
            <div className="text-foreground">
              {challenge.requestedAccess === "instance_admin_required" ? t("pages.cliAuth.instanceAdmin") : t("pages.cliAuth.board")}
            </div>
          </div>
          {challenge.requestedCompanyName && (
            <div>
              <div className="text-muted-foreground">{t("pages.cliAuth.requestedOrganization")}</div>
        ...
        {(approveMutation.error || cancelMutation.error) && (
          <p className="mt-4 text-sm text-destructive">
            {(approveMutation.error ?? cancelMutation.error) instanceof Error
              ...
              : t("pages.cliAuth.updateFailed")}
          </p>
        )}

        {!challenge.canApprove && (
          <p className="mt-4 text-sm text-destructive">
            {t("pages.cliAuth.adminRequired")}
          </p>
        )}
        ...
        <div className="mt-5 flex gap-3">
          ...
          >
            {approveMutation.isPending ? t("pages.cliAuth.approving") : t("pages.cliAuth.approve")}
          </Button>
          <Button
            type="button"
            variant="outline"
          ...
          >
            {cancelMutation.isPending ? t("pages.cliAuth.cancelling") : t("pages.cliAuth.cancel")}
          </Button>
        </div>
      </Card>
    </div>

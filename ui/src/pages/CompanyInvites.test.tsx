...
describe("CompanyInvites", () => {
  ...
  const inviteHistory = Array.from({ length: 25 }, (_, index) => {
    ...
    return {
      ...
      invitedByUser: {
        ...
        email: `board${inviteNumber}@aiteamcorp.local`,
        image: null,
      },
  ...
  beforeEach(() => {
    ...
    createCompanyInviteMock.mockResolvedValue({
      inviteUrl: "https://aiteamcorp.local/invite/new-token",
  ...
  it("renders a human-only invite flow and keeps invite history in a table", async () => {
    ...
    expect(clipboardWriteTextMock).toHaveBeenCalledWith("https://aiteamcorp.local/invite/new-token");
    expect(container.textContent).toContain("Latest invite link");
    expect(container.textContent).toContain("This URL includes the current AiTeamCorp domain returned by the server.");
    expect(container.textContent).toContain("https://aiteamcorp.local/invite/new-token");
    ...
    const inviteFieldButton = Array.from(container.querySelectorAll("button")).find(
      (button) => button.textContent?.includes("https://aiteamcorp.local/invite/new-token"),
    );

...
const ACTION_DOMAINS: { value: string; label: string }[] = [
  { value: ALL, label: t("app.auditFeed.allActions", { defaultValue: "All actions" }) },
  { value: "issue.", label: t("app.auditFeed.tasks", { defaultValue: "Tasks" }) },
  { value: "agent.", label: t("app.auditFeed.agents", { defaultValue: "Agents" }) },
  { value: "heartbeat.", label: t("app.auditFeed.runs", { defaultValue: "Runs" }) },
  { value: "approval.", label: t("app.auditFeed.approvals", { defaultValue: "Approvals" }) },
  { value: "project.", label: t("app.auditFeed.projects", { defaultValue: "Projects" }) },
  { value: "goal.", label: t("app.auditFeed.goals", { defaultValue: "Goals" }) },
  { value: "tool_gateway.", label: t("app.auditFeed.tools", { defaultValue: "Tools" }) },
  { value: "cost.", label: t("app.auditFeed.costs", { defaultValue: "Costs" }) },
  { value: "company.", label: t("app.auditFeed.company", { defaultValue: "Company" }) },
];

/** Entity types offered in the filter (server does an exact match). */
const ENTITY_TYPES: { value: string; label: string }[] = [
  { value: ALL, label: t("app.auditFeed.allEntities", { defaultValue: "All entities" }) },
  { value: "issue", label: t("app.auditFeed.task", { defaultValue: "Task" }) },
  { value: "agent", label: t("app.auditFeed.agent", { defaultValue: "Agent" }) },
  { value: "project", label: t("app.auditFeed.project", { defaultValue: "Project" }) },
  { value: "goal", label: t("app.auditFeed.goal", { defaultValue: "Goal" }) },
  { value: "company", label: t("app.auditFeed.company", { defaultValue: "Company" }) },
];
...
function AuditActor({
  ...
  if (record.actorType === "user" && record.actorId) {
    ...
    return (
      <Identity
        name={profile?.label ?? t("app.auditFeed.user", { defaultValue: "User" })}
  ...
  const label =
    record.actorType === "plugin"
      ? t("app.auditFeed.plugin", { defaultValue: "Plugin" })
      : record.actorType === "agent"
        ? t("app.auditFeed.agent", { defaultValue: "Agent" })
        : record.actorType === "user"
          ? t("app.auditFeed.user", { defaultValue: "User" })
          : t("app.auditFeed.system", { defaultValue: "System" });
  ...
  if (issueRef) {
    return (
      <Link to={`/issues/${issueRef}`} className="font-medium text-primary hover:underline">
        {issue?.identifier ? `${issue.identifier}${issue.title ? ` · ${issue.title}` : ""}` : t("app.auditFeed.theTask", { defaultValue: "the task" })}
      </Link>
    );
  }
  if (document) {
...
function AuditRow({
  ...
  const responsibleLabel = responsible?.label ?? (record.responsibleUserId ? t("app.auditFeed.aUser", { defaultValue: "a user" }) : null);
        ...
        <div className="min-w-0 space-y-1">
          ...
          {documentKey ? (
            <p className="text-xs text-muted-foreground">
              {t("app.auditFeed.document", { defaultValue: "Document " })}<span className="font-mono text-(length:--text-micro)">{documentKey}</span>
            </p>
          ) : null}
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            {showOnBehalf && responsibleLabel ? (
              <span className="inline-flex items-center rounded-full bg-muted px-2 py-0.5">
                {t("app.auditFeed.onBehalfOf", { defaultValue: "on behalf of " })}{responsibleLabel}
              </span>
            ) : null}
            {record.runId && record.agentId ? (
              ...
              >
                {t("app.auditFeed.viewRun", { defaultValue: "View run" })}</Link>
            ) : null}
  ...
  return (
    <Card>
      <CardContent className="flex flex-col items-center gap-3 py-14 text-center">
        ...
        <div>
          <p className="text-sm font-medium text-foreground">{t("app.auditFeed.agentAuditIsAPaperclipEnterpriseView", { defaultValue: "Agent audit is a Paperclip Enterprise view" })}</p>
          <p className="mx-auto mt-1 max-w-md text-sm text-muted-foreground">
            {t("app.auditFeed.theAgentAuditLogGivesYouASearchableExportableRecordOfEverythingYourAgentsDidEveryCommentTaskChangeApprovalAndRunWithTheResponsiblePersonForEachActionAskAnAdministratorToGrantYouThe", { defaultValue: "The agent audit log gives you a searchable, exportable record of everything your agents did — every comment, task change, approval, and run — with the responsible person for each action. Ask an administrator to grant you the" })}{" "}
            <span className="font-mono text-(length:--text-micro)">audit:view_agent_actions</span>{" "}
            {t("app.auditFeed.permissionToViewIt", { defaultValue: "permission to view it." })}</p>
        </div>
      </CardContent>
    </Card>
  );
...
export function AuditFeed({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      pushToast({ title: t("app.auditFeed.auditExported", { defaultValue: "Audit exported" }), body: "Your CSV download has started.", tone: "success" });
    } catch (error) {
      pushToast({
        title: t("app.auditFeed.exportFailed", { defaultValue: "Export failed" }),
        body: error instanceof Error ? error.message : t("app.auditFeed.couldNotExportTheAuditLog", { defaultValue: "Could not export the audit log." }),
        tone: "error",
      });
    } finally {
      setExporting(false);
    ...
    <div className="space-y-4">
      {!hideHeader ? (
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-lg font-semibold text-foreground">{t("app.auditFeed.activity", { defaultValue: "Activity" })}</h1>
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
              {resolvedMode === "agents"
                ? t("app.auditFeed.everyRecordedAgentActionNewestFirstWithTheResponsiblePersonAndRunBehindEachOne", { defaultValue: "Every recorded agent action, newest first — with the responsible person and run behind each one." })
                : t("app.auditFeed.everythingHappeningInYourCompanyNewestFirstPeopleAgentsAndTheSystemEachLineIsOneRecordedAction", { defaultValue: "Everything happening in your company, newest first — people, agents, and the system. Each line is one recorded action." })}
            </p>
          </div>
        </div>
      ) : null}
      ...
      {showModeToggle ? (
        <Tabs value={resolvedMode} onValueChange={(value) => onModeChange?.(value as AuditFeedMode)}>
          <TabsList aria-label={t("app.auditFeed.activityScope", { defaultValue: "Activity scope" })}>
            <TabsTrigger value="all">{t("app.auditFeed.allActivity", { defaultValue: "All activity" })}</TabsTrigger>
            <TabsTrigger value="agents">{t("app.auditFeed.agentActions", { defaultValue: "Agent actions" })}</TabsTrigger>
          </TabsList>
        </Tabs>
      ) : null}
        ...
        <div className="flex flex-wrap items-center gap-2">
          {!lockedAgentId ? (
            <Select value={agent} onValueChange={setAgent}>
              <SelectTrigger className="w-40">
                <SelectValue placeholder={t("app.auditFeed.agent", { defaultValue: "Agent" })} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value={ALL}>{t("app.auditFeed.allAgents", { defaultValue: "All agents" })}</SelectItem>
                {(agents.data ?? []).map((a) => (
          ...
          <Select value={responsibleUser} onValueChange={setResponsibleUser}>
            ...
            <SelectTrigger className="w-52">
              <SelectValue placeholder={t("app.auditFeed.responsibleUser", { defaultValue: "Responsible user" })} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value={ALL}>{t("app.auditFeed.allResponsibleUsers", { defaultValue: "All responsible users" })}</SelectItem>
          ...
          <Select value={actionDomain} onValueChange={setActionDomain}>
            <SelectTrigger className="w-36">
              <SelectValue placeholder={t("app.auditFeed.action", { defaultValue: "Action" })} />
            </SelectTrigger>
            <SelectContent>
          ...
          <Select value={entityType} onValueChange={setEntityType}>
            <SelectTrigger className="w-36">
              <SelectValue placeholder={t("app.auditFeed.entity", { defaultValue: "Entity" })} />
            </SelectTrigger>
            <SelectContent>
              {ENTITY_TYPES.map((e) => (
          ...
          <Input
            type="date"
            aria-label={t("app.auditFeed.fromDate", { defaultValue: "From date" })}
            value={dateFrom}
          ...
          <Input
            type="date"
            aria-label={t("app.auditFeed.toDate", { defaultValue: "To date" })}
            value={dateTo}
          ...
          {hasActiveFilters ? (
            <Button variant="ghost" size="sm" onClick={clearFilters}>
              {t("app.auditFeed.clearFilters", { defaultValue: "Clear filters" })}</Button>
          ) : null}
          <Button
            variant="outline"
            size="sm"
          ...
          >
            ...
            {exporting ? "Exporting…" : t("app.auditFeed.exportCsv", { defaultValue: "Export CSV" })}
          </Button>
        </div>
      ) : null}
      ...
      {recoveringFromAccessDowngrade || fallingBackToAllActivity ? (
        <Card>
          <CardContent className="py-14 text-center text-sm text-muted-foreground">
            {t("app.auditFeed.refreshingAuditAccess", { defaultValue: "Refreshing audit access…" })}</CardContent>
        </Card>
      ) : feed.isLoading ? (
        <Card>
          <CardContent className="py-14 text-center text-sm text-muted-foreground">{t("app.auditFeed.loading", { defaultValue: "Loading…" })}</CardContent>
        </Card>
      ) : feed.error ? (
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-14 text-center">
            <p className="text-sm text-muted-foreground">
              {feed.error instanceof Error ? feed.error.message : t("app.auditFeed.failedToLoadTheAuditLog", { defaultValue: "Failed to load the audit log." })}
            </p>
            <Button variant="outline" size="sm" onClick={() => feed.refetch()}>
              {t("app.auditFeed.tryAgain", { defaultValue: "Try again" })}</Button>
          </CardContent>
        </Card>
      ) : items.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-14 text-center">
            ...
            <div>
              <p className="text-sm font-medium text-foreground">
                {hasActiveFilters ? t("app.auditFeed.noActionsMatchTheseFilters", { defaultValue: "No actions match these filters" }) : t("app.auditFeed.nothingHereYet", { defaultValue: "Nothing here yet" })}
              </p>
              <p className="mt-1 max-w-md text-sm text-muted-foreground">
                {hasActiveFilters
                  ? t("app.auditFeed.tryAWiderDateRangeOrDifferentFilters", { defaultValue: "Try a wider date range or different filters." })
                  : resolvedMode === "agents"
                    ? t("app.auditFeed.asSoonAsYourAgentsStartDoingThingsTheirActionsShowUpHere", { defaultValue: "As soon as your agents start doing things, their actions show up here." })
                    : t("app.auditFeed.asSoonAsAnyoneInYourCompanyDoesSomethingItShowsUpHere", { defaultValue: "As soon as anyone in your company does something, it shows up here." })}
              </p>
            </div>
            {hasActiveFilters ? (
      ...
      {feed.hasNextPage ? (
        <div className="flex justify-center">
          ...
          >
            {feed.isFetchingNextPage ? t("app.auditFeed.loading", { defaultValue: "Loading…" }) : t("app.auditFeed.loadMore", { defaultValue: "Load more" })}
          </Button>
        </div>
      ) : null}

      <p className="text-xs text-muted-foreground">
        {t("app.auditFeed.recordedByPaperclipEntriesCanTBeEditedSensitiveValuesAreNeverStored", { defaultValue: "Recorded by Paperclip — entries can't be edited. Sensitive values are never stored." })}</p>
    </div>
  );
}

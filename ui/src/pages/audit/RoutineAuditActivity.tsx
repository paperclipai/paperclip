...
export function RoutineAuditActivity({
  ...
  if (activity.isLoading) {
    return (
      <div className="border-y border-border py-14 text-center text-sm text-muted-foreground">
        {t("localizationActivity.loadingRoutineActivity")}
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center gap-3 border-y border-border py-14 text-center">
        <p className="text-sm text-muted-foreground">
          {activity.error instanceof Error ? activity.error.message : t("localizationActivity.routineActivityError")}
        </p>
  ...
  if (events.length === 0) {
    return <EmptyState icon={Activity} message={t("localizationActivity.noRoutineActivity")} />;
  }

  return (
    <div className="border-y border-border" aria-label={t("localizationActivity.routineActivity")}>
      {events.map((event) => (

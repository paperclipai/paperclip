      ...
      <div className="border-b border-border/60 bg-background/90 px-5 py-4">
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant="outline" className="uppercase tracking-[0.18em] text-[10px]">
            {t("runTranscriptUxLab.runDetail.badge", "Run Detail")}
          </Badge>
        ...
        <div className="mt-2 text-sm font-medium">
          {t("runTranscriptUxLab.runDetail.transcriptCount", "Transcript ({{count}})", {
    ...
    <div className="overflow-hidden rounded-xl border border-cyan-500/25 bg-background/85 shadow-[0_20px_50px_rgba(6,182,212,0.10)]">
      <div className="border-b border-border/60 bg-cyan-500/[0.05] px-5 py-4">
        <div className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-700 dark:text-cyan-300">
          {t("runTranscriptUxLab.liveWidget.title", "Live Runs")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t(
            "runTranscriptUxLab.liveWidget.description",
      ...
      <div className="px-5 py-4">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <span className="inline-flex items-center gap-1 rounded-full border border-border/70 bg-background/70 px-2.5 py-1 text-[11px] text-muted-foreground">
            {t("runTranscriptUxLab.liveWidget.openRun", "Open run")}
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="mt-2 text-[11px] text-muted-foreground">
                {streaming
                  ? t("runTranscriptUxLab.dashboard.liveNow", "Live now")
                  : t("runTranscriptUxLab.dashboard.finishedAgo", "Finished 2m ago")}
              </div>
            </div>
...
export function RunTranscriptUxLab() {
  ...
  const surfaceOptions: SurfaceOption[] = [
    {
      id: "detail",
      label: t("runTranscriptUxLab.surfaces.detail.label", "Run Detail"),
      eyebrow: t("runTranscriptUxLab.surfaces.detail.eyebrow", "Full transcript"),
      description: t(
        "runTranscriptUxLab.surfaces.detail.description",
    ...
    {
      id: "live",
      label: t("runTranscriptUxLab.surfaces.live.label", "Issue Widget"),
      eyebrow: t("runTranscriptUxLab.surfaces.live.eyebrow", "Live stream"),
      description: t(
        "runTranscriptUxLab.surfaces.live.description",
    ...
    {
      id: "dashboard",
      label: t("runTranscriptUxLab.surfaces.dashboard.label", "Dashboard Card"),
      eyebrow: t("runTranscriptUxLab.surfaces.dashboard.eyebrow", "Dense card"),
      description: t(
        "runTranscriptUxLab.surfaces.dashboard.description",
          ...
          <aside className="border-b border-border/60 bg-background/75 p-5 lg:border-b-0 lg:border-r">
            <div className="mb-5">
              <div className="inline-flex items-center gap-2 rounded-full border border-cyan-500/25 bg-cyan-500/[0.08] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.24em] text-cyan-700 dark:text-cyan-300">
                ...
                {t("runTranscriptUxLab.badge", "UX Lab")}
              </div>
              <h1 className="mt-4 text-2xl font-semibold tracking-tight">
                {t("runTranscriptUxLab.title", "Run Transcript Fixtures")}
              </h1>
              <p className="mt-2 text-sm text-muted-foreground">
                {t(
                  "runTranscriptUxLab.description",
          ...
          <main className="min-w-0 p-5">
            <div className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Badge variant="outline" className="rounded-full px-3 py-1 text-[10px] uppercase tracking-[0.18em]">
                  {t("runTranscriptUxLab.sourceRun", "Source run {{id}}", {
            ...
            <div className="mb-5 flex flex-wrap items-center gap-2">
              <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                {t("runTranscriptUxLab.controls", "Controls")}
              </span>
              <div className="inline-flex rounded-full border border-border/70 bg-background/80 p-1">
                {(["nice", "raw"] as const).map((mode) => (
                  ...
                  >
                    {mode === "nice"
                      ? t("runTranscriptUxLab.modes.nice", "Nice")
                      : t("runTranscriptUxLab.modes.raw", "Raw")}
                  </button>
                ))}
              </div>
              <div className="inline-flex rounded-full border border-border/70 bg-background/80 p-1">
                {(["comfortable", "compact"] as const).map((nextDensity) => (
                  ...
                  >
                    {nextDensity === "comfortable"
                      ? t("runTranscriptUxLab.density.comfortable", "Comfortable")
                      : t("runTranscriptUxLab.density.compact", "Compact")}
                  </button>
                ))}
              </div>
              <Button
              ...
              >
                {streaming
                  ? t("runTranscriptUxLab.actions.showSettled", "Show settled state")
                  : t("runTranscriptUxLab.actions.showStreaming", "Show streaming state")}
              </Button>
            </div>

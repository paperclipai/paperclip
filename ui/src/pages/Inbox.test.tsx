...
describe("Inbox toolbar", () => {
  ...
  it("requests live descendant summaries for issue rows", async () => {
    ...
    await vi.waitFor(() => {
      expect(
        apiMocks.issuesList.mock.calls.filter(
          (call) => call[1]?.includeLiveDescendantSummary === true,
        ),
      ).toHaveLength(3);
    });

    const liveSummaryCalls = apiMocks.issuesList.mock.calls.filter(
      (call) => call[1]?.includeLiveDescendantSummary === true,
    );
    expect(liveSummaryCalls.map((call) => call[1]?.limit)).toEqual([
      500,
      500,
      500,
    ]);

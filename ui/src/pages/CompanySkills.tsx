...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null, t: (key: string, opts?: { defaultValue?: string }) => string) {
  ...
  switch (sourceBadge) {
    case "skills_sh":
      return { icon: VercelMark, label: sourceLabel ?? "skills.sh", managedLabel: t("companySkills.managed.skillsSh", { defaultValue: "skills.sh managed" }) };
    case "github":
      return isSkillsShManaged
        ? { icon: VercelMark, label: sourceLabel ?? "skills.sh", managedLabel: t("companySkills.managed.skillsSh", { defaultValue: "skills.sh managed" }) }
        : { icon: Github, label: sourceLabel ?? "GitHub", managedLabel: t("companySkills.managed.github", { defaultValue: "GitHub managed" }) };
    case "url":
      return { icon: Link2, label: sourceLabel ?? "URL", managedLabel: t("companySkills.managed.url", { defaultValue: "URL managed" }) };
    case "local":
      return { icon: Folder, label: sourceLabel ?? t("companySkills.source.folder", { defaultValue: "Folder" }), managedLabel: t("companySkills.managed.folder", { defaultValue: "Folder managed" }) };
    case "paperclip":
      return { icon: Paperclip, label: sourceLabel ?? "Paperclip", managedLabel: t("companySkills.managed.paperclip", { defaultValue: "Paperclip managed" }) };
    default:
      return { icon: Boxes, label: sourceLabel ?? t("companySkills.source.catalog", { defaultValue: "Catalog" }), managedLabel: t("companySkills.managed.catalog", { defaultValue: "Catalog managed" }) };
  }
}
...
function formatProjectScanSummary(result: CompanySkillProjectScanResult, t: (key: string, opts?: Record<string, unknown>) => string) {
  const parts = [
    t("companySkills.scan.found", { defaultValue: "{{count}} found", count: result.discovered }),
    t("companySkills.scan.imported", { defaultValue: "{{count}} imported", count: result.imported.length }),
    t("companySkills.scan.updated", { defaultValue: "{{count}} updated", count: result.updated.length }),
  ];
  if (result.conflicts.length > 0) parts.push(t("companySkills.scan.conflicts", { defaultValue: "{{count}} conflicts", count: result.conflicts.length }));
  if (result.skipped.length > 0) parts.push(t("companySkills.scan.skipped", { defaultValue: "{{count}} skipped", count: result.skipped.length }));
  return t("companySkills.scan.summary", {
    defaultValue: "{{parts}} across {{count}} workspace{{plural}}.",
    parts: parts.join(", "),
    count: result.scannedWorkspaces,
    plural: result.scannedWorkspaces === 1 ? "" : "s",
  });
}
    ...
    <div className="border-b border-border px-4 py-4">
      <div className="space-y-3">
        <Input
          ...
          placeholder={t("companySkills.form.namePlaceholder", { defaultValue: "Skill name" })}
          className="h-9 rounded-none border-0 border-b border-border px-0 shadow-none focus-visible:ring-0"
        />
        <Input
          value={slug}
          onChange={(event) => setSlug(event.target.value)}
          placeholder={t("companySkills.form.slugPlaceholder", { defaultValue: "optional-shortname" })}
          className="h-9 rounded-none border-0 border-b border-border px-0 shadow-none focus-visible:ring-0"
        />
        <Textarea
          value={description}
          onChange={(event) => setDescription(event.target.value)}
          placeholder={t("companySkills.form.descriptionPlaceholder", { defaultValue: "Short description" })}
        ...
        <div className="flex items-center justify-end gap-2">
          <Button variant="ghost" size="sm" onClick={onCancel} disabled={isPending}>
            {t("companies.cancel", { defaultValue: "Cancel" })}
          </Button>
          <Button
            size="sm"
          ...
          >
            {isPending ? t("companySkills.form.creating", { defaultValue: "Creating..." }) : t("companySkills.form.create", { defaultValue: "Create skill" })}
          </Button>
        </div>
      </div>
    </div>
  ...
  if (filteredSkills.length === 0) {
    return (
      <div className="px-4 py-6 text-sm text-muted-foreground">
        {t("companySkills.list.noMatches", { defaultValue: "No skills match this filter." })}
      </div>
    );
  }
          ...
          <div key={skill.id} className="border-b border-border">
            ...
            >
              ...
              <button
                ...
                aria-label={expanded
                  ? t("companySkills.list.collapse", { defaultValue: "Collapse {{name}}", name: skill.name })
                  : t("companySkills.list.expand", { defaultValue: "Expand {{name}}", name: skill.name })}
              >
                {expanded ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
              </button>
            </div>
...
function SkillPane({
  ...
  if (!detail) {
    ...
    return (
      <EmptyState
        icon={Boxes}
        message={t("companySkills.pane.empty", { defaultValue: "Select a skill to inspect its files." })}
      />
    );
  }
  ...
  const removeDisabledReason = removeBlocked
    ? t("companySkills.pane.removeBlocked", { defaultValue: "Detach this skill from all agents before removing it." })
    : null;
    ...
    <div className="min-w-0">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            >
              ...
              {deletePending ? t("companySkills.pane.removing", { defaultValue: "Removing..." }) : t("companySkills.pane.remove", { defaultValue: "Remove" })}
            </Button>
            {detail.editable ? (
              <button
              ...
              >
                ...
                {editMode ? t("companySkills.pane.stopEditing", { defaultValue: "Stop editing" }) : t("companySkills.pane.edit", { defaultValue: "Edit" })}
              </button>
            ) : (
        ...
        <div className="mt-4 space-y-3 border-t border-border pt-4 text-sm">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            <div className="flex min-w-0 items-center gap-2">
              <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("companySkills.pane.source", { defaultValue: "Source" })}</span>
                ...
                {detail.sourcePath && displaySourcePath ? (
                  <>
                    ...
                    <CopyText
                      ...
                      copiedLabel={t("companySkills.pane.copiedPath", { defaultValue: "Copied path" })}
                      ariaLabel={t("companySkills.pane.copySourcePath", { defaultValue: "Copy source path" })}
                      title={t("companySkills.pane.copySourcePath", { defaultValue: "Copy source path" })}
            ...
            {detail.sourceType === "github" && (
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("companySkills.pane.pin", { defaultValue: "Pin" })}</span>
                <span className="font-mono text-xs">{currentPin ?? t("companySkills.pane.untracked", { defaultValue: "untracked" })}</span>
                {updateStatus?.trackingRef && (
                  <span className="text-xs text-muted-foreground">{t("companySkills.pane.tracking", { defaultValue: "tracking {{ref}}", ref: updateStatus.trackingRef })}</span>
                )}
                <Button
                  variant="ghost"
                  size="sm"
                ...
                >
                  ...
                  {t("companySkills.pane.checkUpdates", { defaultValue: "Check for updates" })}
                </Button>
                {updateStatus?.supported && updateStatus.hasUpdate && (
                  ...
                  >
                    ...
                    {latestPin
                      ? t("companySkills.pane.installUpdateRef", { defaultValue: "Install update {{ref}}", ref: latestPin })
                      : t("companySkills.pane.installUpdate", { defaultValue: "Install update" })}
                  </Button>
                )}
                {updateStatus?.supported && !updateStatus.hasUpdate && !updateStatusLoading && (
                  <span className="text-xs text-muted-foreground">{t("companySkills.pane.upToDate", { defaultValue: "Up to date" })}</span>
                )}
            ...
            <div className="flex items-center gap-2">
              <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("companySkills.pane.key", { defaultValue: "Key" })}</span>
              <span className="font-mono text-xs">{detail.key}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("companySkills.pane.mode", { defaultValue: "Mode" })}</span>
              <span>{detail.editable ? t("companySkills.pane.editable", { defaultValue: "Editable" }) : t("companySkills.pane.readOnly", { defaultValue: "Read only" })}</span>
            </div>
          </div>
          <div className="flex flex-wrap items-start gap-x-3 gap-y-1">
            <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("companySkills.pane.usedBy", { defaultValue: "Used by" })}</span>
            {usedBy.length === 0 ? (
              <span className="text-muted-foreground">{t("companySkills.pane.noAgentsAttached", { defaultValue: "No agents attached" })}</span>
            ) : (
              ...
              <div className="flex items-center border border-border">
                ...
                >
                  <span className="flex items-center gap-1.5">
                    ...
                    {t("companySkills.pane.view", { defaultValue: "View" })}
                  </span>
                </button>
                <button
                ...
                >
                  <span className="flex items-center gap-1.5">
                    ...
                    {t("companySkills.pane.code", { defaultValue: "Code" })}
                  </span>
                </button>
              </div>
            )}
            {editMode && file?.editable && (
              <>
                ...
                <Button size="sm" onClick={onSave} disabled={savePending}>
                  ...
                  {savePending ? t("companySkills.pane.saving", { defaultValue: "Saving..." }) : t("companies.save", { defaultValue: "Save" })}
                </Button>
              </>
            )}
          </div>
      ...
      <div className="min-h-[560px] px-5 py-5">
        {fileLoading ? (
          ...
          <div className="text-sm text-muted-foreground">{t("companySkills.pane.selectFile", { defaultValue: "Select a file to inspect." })}</div>
        ) : editMode && file.editable ? (
          file.markdown ? (
            <MarkdownEditor
              value={draft}
...
export function CompanySkills() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("sidebar.skills", { defaultValue: "Skills" }), href: "/skills" },
      ...(routeSkillId ? [{ label: t("companySkills.detail", { defaultValue: "Detail" }) }] : []),
    ]);
  ...
  const importSkill = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.toast.imported.title", { defaultValue: "Skills imported" }),
        body: t("companySkills.toast.imported.body", {
          defaultValue: "{{count}} skill{{plural}} added.",
          count: result.imported.length,
          plural: result.imported.length === 1 ? "" : "s",
        }),
      });
      if (result.warnings[0]) {
        pushToast({ tone: "warn", title: t("companySkills.toast.importWarnings", { defaultValue: "Import warnings" }), body: result.warnings[0] });
      }
      setSource("");
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.toast.importFailed.title", { defaultValue: "Skill import failed" }),
        body: error instanceof Error ? error.message : t("companySkills.toast.importFailed.body", { defaultValue: "Failed to import skill source." }),
      });
    },
  });
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.toast.created.title", { defaultValue: "Skill created" }),
        body: t("companySkills.toast.created.body", {
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.toast.createFailed.title", { defaultValue: "Skill creation failed" }),
        body: error instanceof Error ? error.message : t("companySkills.toast.createFailed.body", { defaultValue: "Failed to create skill." }),
      });
    },
  });
  ...
  const scanProjects = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.toast.scanComplete.title", { defaultValue: "Project skill scan complete" }),
        body: summary,
      });
      if (result.conflicts[0]) {
        pushToast({
          tone: "warn",
          title: t("companySkills.toast.scanConflicts", { defaultValue: "Skill conflicts found" }),
    ...
    onError: (error) => {
      ...
      pushToast({
        tone: "error",
        title: t("companySkills.toast.scanFailed.title", { defaultValue: "Project skill scan failed" }),
        body: error instanceof Error ? error.message : t("companySkills.toast.scanFailed.body", { defaultValue: "Failed to scan project workspaces." }),
      });
    },
  });
  ...
  const saveFile = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.toast.saved.title", { defaultValue: "Skill saved" }),
        body: result.path,
      });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.toast.saveFailed.title", { defaultValue: "Save failed" }),
        body: error instanceof Error ? error.message : t("companySkills.toast.saveFailed.body", { defaultValue: "Failed to save skill file." }),
      });
    },
  });
  ...
  const installUpdate = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.toast.updated.title", { defaultValue: "Skill updated" }),
        body: skill.sourceRef
          ? t("companySkills.toast.updated.body", { defaultValue: "Pinned to {{ref}}", ref: shortRef(skill.sourceRef) ?? "" })
          : skill.name,
      });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.toast.updateFailed.title", { defaultValue: "Update failed" }),
        body: error instanceof Error ? error.message : t("companySkills.toast.updateFailed.body", { defaultValue: "Failed to install skill update." }),
      });
    },
  });
  ...
  const deleteSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("companySkills.toast.removed.title", { defaultValue: "Skill removed" }),
        body: t("companySkills.toast.removed.body", {
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("companySkills.toast.removeFailed.title", { defaultValue: "Remove failed" }),
        body: error instanceof Error ? error.message : t("companySkills.toast.removeFailed.body", { defaultValue: "Failed to remove skill." }),
      });
    },
  });
  ...
  return (
    <>
      <Dialog open={deleteOpen} onOpenChange={closeDeleteDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("companySkills.deleteDialog.description", { defaultValue: "Remove this skill from the company library. If any agents still use it, removal will be blocked until it is detached." })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <p>
              {deleteTargetDetail
                ? t("companySkills.deleteDialog.confirmNamed", { defaultValue: "You are about to remove {{name}}.", name: deleteTargetDetail.name })
                : t("companySkills.deleteDialog.confirm", { defaultValue: "You are about to remove this skill." })}
            </p>
            {deleteTargetDetail?.usedByAgents?.length ? (
              <div className="rounded-md border border-border px-3 py-3 text-muted-foreground">
                {t("companySkills.deleteDialog.usedBy", {
                  defaultValue: "Currently used by {{names}}.",
                  names: deleteTargetDetail.usedByAgents.map((agent) => agent.name).join(", "),
                })}
              </div>
            ) : null}
            {(deleteTargetDetail?.usedByAgents.length ?? 0) > 0 ? (
              <p className="text-muted-foreground">
                {t("companySkills.deleteDialog.detachHint", { defaultValue: "Detach this skill from all agents to enable removal." })}
              </p>
            ) : null}
          </div>
          <DialogFooter>
            {(deleteTargetDetail?.usedByAgents.length ?? 0) > 0 ? (
              <Button variant="ghost" onClick={() => closeDeleteDialog(false)}>
                {t("companySkills.deleteDialog.close", { defaultValue: "Close" })}
              </Button>
            ) : (
              <>
                ...
                >
                  {deleteSkill.isPending ? t("companySkills.pane.removing", { defaultValue: "Removing..." }) : t("companySkills.deleteDialog.removeSkill", { defaultValue: "Remove skill" })}
                </Button>
              </>
            )}
          </DialogFooter>
      ...
      <Dialog open={emptySourceHelpOpen} onOpenChange={setEmptySourceHelpOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("companySkills.sourceHelp.title", { defaultValue: "Add a skill source" })}</DialogTitle>
            <DialogDescription>
              {t("companySkills.sourceHelp.description", { defaultValue: "Paste a local path, GitHub URL, or `skills.sh` command into the field first." })}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <a
            ...
            >
              <span>
                <span className="block font-medium">{t("companySkills.sourceHelp.browseSkillsSh", { defaultValue: "Browse skills.sh" })}</span>
                <span className="mt-1 block text-muted-foreground">
                  {t("companySkills.sourceHelp.browseSkillsShBody", { defaultValue: "Find install commands and paste one here." })}
                </span>
              </span>
            ...
            >
              <span>
                <span className="block font-medium">{t("companySkills.sourceHelp.searchGithub", { defaultValue: "Search GitHub" })}</span>
                <span className="mt-1 block text-muted-foreground">
                  {t("companySkills.sourceHelp.searchGithubBody", { defaultValue: "Look for repositories with `SKILL.md`, then paste the repo URL here." })}
                </span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
        ...
        <aside className="border-r border-border">
          <div className="border-b border-border px-4 py-3">
            <div className="flex items-center justify-between gap-2">
              <div>
                <h1 className="text-base font-semibold">{t("companySkills.title", { defaultValue: "Skills" })}</h1>
                <p className="text-xs text-muted-foreground">
                  {t("companySkills.available", { defaultValue: "{{count}} available", count: skillsQuery.data?.length ?? 0 })}
                </p>
              </div>
              <div className="flex items-center gap-1">
                <Button
                  ...
                  title={t("companySkills.scanTitle", { defaultValue: "Scan project workspaces for skills" })}
                >
            ...
            <div className="mt-3 flex items-center gap-2 border-b border-border pb-2">
              ...
              <input
                ...
                placeholder={t("companySkills.filterPlaceholder", { defaultValue: "Filter skills" })}
            ...
            <div className="mt-3 flex items-center gap-2 border-b border-border pb-2">
              <input
                ...
                placeholder={t("companySkills.sourcePlaceholder", { defaultValue: "Paste path, GitHub URL, or skills.sh command" })}
                className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground"
              />
              <Button
                size="sm"
              ...
              >
                {importSkill.isPending ? <RefreshCw className="h-4 w-4 animate-spin" /> : t("companySkills.add", { defaultValue: "Add" })}
              </Button>
            </div>
            {scanStatusMessage && (

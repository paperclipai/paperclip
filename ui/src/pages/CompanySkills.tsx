      ...
      <div className="mt-auto pt-3">
        ...
        <div className="flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
          <span>
            {card.installed
              ? card.agentCount > 0
                ? t("localizationSkills.enabledForCount", { count: card.agentCount })
                : t("localizationSkills.notEnabledForAnyAgents71")
              : t("localizationSkills.availableToInstall72")}
          </span>
          {card.starCount > 0 ? (
            <>
        ...
        <div className="mt-2 flex flex-wrap items-center gap-1">
          {card.installed ? (
            <Badge variant="secondary" className="text-(length:--text-nano)">{t("localizationSkills.installed73")}</Badge>
          ) : null}
          ...
          {card.required ? (
            ...
            <Badge variant="outline" className="ml-auto text-(length:--text-nano) text-muted-foreground">{t("pages.secrets.common.optionalPlain")}</Badge>
          ) : null}
        </div>
      </div>
    </div>
...
export function DiscoveryGrid({
  ...
  const viewTitle = installedView ? t("localizationSkills.installedSkills74") : t("localizationSkills.discoverSkills75");
  const viewDescription = installedView
    ? t("localizationSkills.skillsAvailableToThisOrganization76")
    : t("localizationSkills.browseSkillsFromEveryAvailableSource77");
  const searchLabel = installedView ? t("localizationSkills.searchInstalledSkills78") : t("localizationSkills.searchDiscoverableSkills79");
  ...
  const availableSources = useMemo(() => {
    ...
    for (const card of cards) {
      if (card.sourceKind) {
        facets.set(`kind:${card.sourceKind}`, card.sourceKind === "bundled" ? t("localizationSkills.bundled22") : t("localizationSkills.optional23"));
      }
      if (card.sourceBadge) {
      ...
      {showBrowseRails ? (
        <aside className={cn("hidden w-60 shrink-0 flex-col overflow-hidden border-r border-border md:flex", showFolderRail && "md:hidden")}>
          <div className="border-b border-border px-4 py-4">
            <h2 className="text-sm font-semibold text-foreground">{t("localizationSkills.browseByCategory81")}</h2>
            <p className="text-xs text-muted-foreground">{t(installedView ? "localizationSkills.filterInstalledSkills" : "localizationSkills.filterDiscoverableSkills")}
            </p>
          </div>
              ...
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm">
                  ...
                  <span className="ml-1.5">
                    {sourceBadgeFilter === "all"
                      ? t("localizationSkills.all20")
                      : availableSources.find((source) => source.value === sourceBadgeFilter)?.label ?? t("localizationSkills.all20")}
                  </span>
                  <ChevronDown className="ml-1 h-3.5 w-3.5" />
                </Button>
              </DropdownMenuTrigger>
            ...
            <div className="py-12">
              <EmptyState
                ...
                message={
                  totalCount === 0
                    ? installedView
                      ? t("localizationSkills.noInstalledSkillsYetDiscoverASkill97")
                      : t("localizationSkills.noSkillsAreAvailableToDiscoverYet98")
                    : search || activeCategory || sourceFilterActive
                      ? t("localizationSkills.noSkillsMatchYourFilters99")
                      : t("localizationSkills.noSkillsInThisViewYet100")
                }
              />
              {totalCount === 0 ? (
        ...
        {!streamlinedUiEnabled ? (
          <div className="px-4 pt-4">
            <Tabs value={legacyDiscoveryTab} onValueChange={(value) => setLegacyDiscoveryTab(value as "all" | "installed" | "catalog" | "bundled")}>
              <TabsList variant="line" aria-label={t("localizationSkills.skillsView399")}>
                <TabsTrigger value="all">{t("localizationSkills.all20")}</TabsTrigger>
                <TabsTrigger value="installed">{t("localizationSkills.installed73")}</TabsTrigger>
                <TabsTrigger value="catalog">{t("localizationSkills.catalog11")}</TabsTrigger>
                <TabsTrigger value="bundled">{t("localizationSkills.bundled22")}</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
        ) : null}

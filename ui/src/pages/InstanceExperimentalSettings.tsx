  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">Enable Issue Dependencies</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
              Allow issues to declare blocker dependencies on other issues. When enabled, the &ldquo;Blocked By&rdquo;
              section appears in issue detail, agents can set <code>blockedByIssueIds</code> on issues, and resolved
              blockers automatically wake dependent agents.
            </p>
          </div>
          <ToggleSwitch
            checked={enableDependencies}
            onCheckedChange={() => toggleMutation.mutate({ enableDependencies: !enableDependencies })}
            disabled={toggleMutation.isPending}
            aria-label="Toggle issue dependencies experimental setting"
          />
        </div>
      </section>
    </div>

    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {preview
              ? `${count} ${t("instanceSettings.experimental.recoveryTasks")} ${t("instanceSettings.experimental.matchLastHours")} ${preview.lookbackHours} ${t("instanceSettings.experimental.hours")}`
...
export function InstanceExperimentalSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.experimental.updateFailed"));
    },
  });
  ...
  const previewMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.experimental.previewFailed"));
    },
  });
  ...
  const runRecoveryMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.experimental.runFailed"));
    },
  });
    ...
    return (
      <div className="text-sm text-destructive">
        {experimentalQuery.error instanceof Error
          ...
          : t("instanceSettings.experimental.loadFailed")}
      </div>
    );
  }
  ...
  function previewForEnable() {
    if (!lookbackHoursIsValid) {
      setActionError(`${t("instanceSettings.experimental.lookbackHours")} must be a whole number from 1 to 720.`);
      return;
    }
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("instanceSettings.experimental.autoRestartDev")}</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
              {t("instanceSettings.experimental.autoRestartDevDesc")}
            </p>
          </div>
          <ToggleSwitch
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex flex-col gap-5">
          ...
          <p className="text-xs text-muted-foreground">
            {t("instanceSettings.experimental.currentWindowLast")} {lookbackHours} {lookbackHours === 1 ? "hour" : "hours"}.
          </p>
        </div>
      </section>

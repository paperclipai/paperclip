...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("app.pluginSettings.company", { defaultValue: "Company" }), href: "/dashboard" },
      { label: t("app.pluginSettings.settings", { defaultValue: "Settings" }), href: "/company/settings" },
      { label: t("app.pluginSettings.plugins", { defaultValue: "Plugins" }), href: "/company/settings/instance/plugins" },
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? t("app.pluginSettings.pluginDetails", { defaultValue: "Plugin Details" }) },
    ]);
  ...
  if (pluginLoading) {
    return <div className="p-4 text-sm text-muted-foreground">{t("app.pluginSettings.loadingPluginDetails", { defaultValue: "Loading plugin details..." })}</div>;
  }
  ...
  const statusVariant =
    plugin.status === "ready"
      ...
      : plugin.status === "error"
        ? "destructive"
        : "secondary";
  const pluginDescription = plugin.manifestJson.description || t("app.pluginSettings.noDescriptionProvided", { defaultValue: "No description provided." });
    ...
    <div className="max-w-6xl space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "configuration" | "status")} className="space-y-6">
        <PageTabBar
          align="start"
          items={[
            { value: "configuration", label: t("app.pluginSettings.configuration", { defaultValue: "Configuration" }) },
            { value: "status", label: t("app.pluginSettings.status", { defaultValue: "Status" }) },
          ]}
          value={activeTab}
        ...
        <TabsContent value="configuration" className="space-y-6">
          <div className="space-y-8">
            <section className="space-y-5">
              <h2 className="text-base font-semibold">{t("app.pluginSettings.about", { defaultValue: "About" })}</h2>
              <div className="grid gap-8 lg:grid-cols-(--gtc-52)">
                <div className="space-y-2">
                  <h3 className="text-sm font-medium text-muted-foreground">{t("app.pluginSettings.description", { defaultValue: "Description" })}</h3>
                  <p className="text-sm leading-6 text-foreground/90">{pluginDescription}</p>
                </div>
                <div className="space-y-4 text-sm">
                  <div className="space-y-1.5">
                    <h3 className="font-medium text-muted-foreground">{t("app.pluginSettings.author", { defaultValue: "Author" })}</h3>
                    <p className="text-foreground">{plugin.manifestJson.author}</p>
                  </div>
                  <div className="space-y-2">
                    <h3 className="font-medium text-muted-foreground">{t("app.pluginSettings.categories", { defaultValue: "Categories" })}</h3>
                    <div className="flex flex-wrap gap-2">
                      {plugin.categories.length > 0 ? (
                        ...
                        <span className="text-foreground">{t("app.pluginSettings.none", { defaultValue: "None" })}</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <section className="space-y-4">
              <div className="space-y-1">
                <h2 className="text-base font-semibold">{t("app.pluginSettings.settings", { defaultValue: "Settings" })}</h2>
              </div>
              {hasLocalFolders ? (
              ...
              {hasCustomSettingsPage ? (
                ...
                <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm">
                  <p className="font-medium text-foreground">{t("app.pluginSettings.configureThisPluginFromSettingsEnvironments", { defaultValue: "Configure this plugin from Settings → Environments." })}</p>
                  <p className="mt-1 text-muted-foreground">
                    {driverLabel || t("app.pluginSettings.thisPlugin", { defaultValue: "This plugin" })} {t("app.pluginSettings.registersEnvironmentRuntimeSettingsThereSoTheExecutionTargetStaysInstanceScopedWhileSecretBindingsStillResolveThroughTheSelectedCompanyContext", { defaultValue: "registers environment runtime settings there so the execution target stays instance-scoped while secret bindings still resolve through the selected company context." })}</p>
                  <div className="mt-3">
                    <Link to="/company/settings/instance/environments">
                      <Button variant="outline" size="sm">{t("app.pluginSettings.openEnvironments", { defaultValue: "Open Environments" })}</Button>
                    </Link>
                  </div>
                </div>
              ) : !hasLocalFolders ? (
                <p className="text-sm text-muted-foreground">
                  {t("app.pluginSettings.thisPluginDoesNotRequireAnySettings", { defaultValue: "This plugin does not require any settings." })}</p>
              ) : null}
            </section>
          </div>
        </TabsContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {t("app.pluginSettings.runtimeDashboard", { defaultValue: "Runtime Dashboard" })}</CardTitle>
                  <CardDescription>
                    {t("app.pluginSettings.workerProcessScheduledJobsAndWebhookDeliveries", { defaultValue: "Worker process, scheduled jobs, and webhook deliveries" })}</CardDescription>
                </CardHeader>
                    ...
                    <>
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("app.pluginSettings.workerProcess", { defaultValue: "Worker Process" })}</h3>
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("app.pluginSettings.status", { defaultValue: "Status" })}</span>
                              <Badge variant={dashboardData.worker.status === "running" ? "default" : "secondary"}>
                                {dashboardData.worker.status}
                              </Badge>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("app.pluginSettings.pid", { defaultValue: "PID" })}</span>
                              <span className="font-mono text-xs">{dashboardData.worker.pid ?? "—"}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("app.pluginSettings.uptime", { defaultValue: "Uptime" })}</span>
                              <span className="text-xs">{formatUptime(dashboardData.worker.uptime)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("app.pluginSettings.pendingRpcs", { defaultValue: "Pending RPCs" })}</span>
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  <span className="text-muted-foreground flex items-center gap-1">
                                    ...
                                    {t("app.pluginSettings.crashes", { defaultValue: "Crashes" })}</span>
                                  <span className="text-xs">
                                    {dashboardData.worker.consecutiveCrashes} {t("app.pluginSettings.consecutive", { defaultValue: "consecutive / " })}{dashboardData.worker.totalCrashes} total
                                  </span>
                                </div>
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    <span className="text-muted-foreground">{t("app.pluginSettings.lastCrash", { defaultValue: "Last Crash" })}</span>
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("app.pluginSettings.noWorkerProcessRegistered", { defaultValue: "No worker process registered." })}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("app.pluginSettings.recentJobRuns", { defaultValue: "Recent Job Runs" })}</h3>
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("app.pluginSettings.noJobRunsRecordedYet", { defaultValue: "No job runs recorded yet." })}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("app.pluginSettings.recentWebhookDeliveries", { defaultValue: "Recent Webhook Deliveries" })}</h3>
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("app.pluginSettings.noWebhookDeliveriesRecordedYet", { defaultValue: "No webhook deliveries recorded yet." })}</p>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        <Clock className="h-3 w-3" />
                        {t("app.pluginSettings.lastChecked", { defaultValue: "Last checked: " })}{new Date(dashboardData.checkedAt).toLocaleTimeString()}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      {t("app.pluginSettings.runtimeDiagnosticsAreUnavailableRightNow", { defaultValue: "Runtime diagnostics are unavailable right now." })}</p>
                  )}
                </CardContent>
              </Card>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base flex items-center gap-1.5">
                      ...
                      {t("app.pluginSettings.recentLogs", { defaultValue: "Recent Logs" })}</CardTitle>
                    <CardDescription>{t("app.pluginSettings.last", { defaultValue: "Last " })}{recentLogs.length} {t("app.pluginSettings.logEntries", { defaultValue: "log entries" })}</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    <ActivitySquare className="h-4 w-4" />
                    {t("app.pluginSettings.healthStatus", { defaultValue: "Health Status" })}</CardTitle>
                </CardHeader>
                <CardContent>
                  {healthLoading ? (
                    <p className="text-sm text-muted-foreground">{t("app.pluginSettings.checkingHealth", { defaultValue: "Checking health..." })}</p>
                  ) : healthData ? (
                    <div className="space-y-4 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">{t("app.pluginSettings.overall", { defaultValue: "Overall" })}</span>
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      <div className="flex items-center justify-between">
                        <span>{t("app.pluginSettings.lifecycle", { defaultValue: "Lifecycle" })}</span>
                        <Badge variant={statusVariant}>{displayStatus}</Badge>
                      </div>
                      <p>{t("app.pluginSettings.healthChecksRunOnceThePluginIsReady", { defaultValue: "Health checks run once the plugin is ready." })}</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">{t("app.pluginSettings.details", { defaultValue: "Details" })}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3 text-sm text-muted-foreground">
                  <div className="flex justify-between gap-3">
                    <span>{t("app.pluginSettings.pluginId", { defaultValue: "Plugin ID" })}</span>
                    <span className="font-mono text-xs text-right">{plugin.id}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("app.pluginSettings.pluginKey", { defaultValue: "Plugin Key" })}</span>
                    <span className="font-mono text-xs text-right">{plugin.pluginKey}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("app.pluginSettings.npmPackage", { defaultValue: "NPM Package" })}</span>
                    <span className="max-w-(--sz-170px) truncate text-right text-xs" title={plugin.packageName}>
                      {plugin.packageName}
                    </span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("app.pluginSettings.version", { defaultValue: "Version" })}</span>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {t("app.pluginSettings.permissions", { defaultValue: "Permissions" })}</CardTitle>
                </CardHeader>
                <CardContent>
                  {pluginCapabilities.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">{t("app.pluginSettings.noSpecialPermissionsRequested", { defaultValue: "No special permissions requested." })}</p>
                  )}
                </CardContent>
              </Card>
            </div>
  ...
  if (!companyId) {
    return (
      <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
        {t("app.pluginSettings.selectACompanyToConfigureThisPluginSLocalFolders", { defaultValue: "Select a company to configure this plugin's local folders." })}</div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-medium">{t("app.pluginSettings.localFolders", { defaultValue: "Local folders" })}</h3>
      </div>
      {error ? (
        <div className="rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          {(error as Error).message || t("app.pluginSettings.failedToLoadLocalFolderSettings", { defaultValue: "Failed to load local folder settings." })}
        </div>
      ) : null}
      {isLoading ? (
        <div className="flex items-center gap-2 py-3 text-sm text-muted-foreground">
          <Loader2 className="h-4 w-4 animate-spin" />
          {t("app.pluginSettings.loadingLocalFolders", { defaultValue: "Loading local folders..." })}</div>
      ) : (
        <div className="space-y-3">
    ...
    onSuccess: (nextStatus) => {
      setMessage({
        ...
        text: nextStatus.healthy
          ? t("app.pluginSettings.localFolderSaved", { defaultValue: "Local folder saved." })
          : t("app.pluginSettings.localFolderSavedButValidationStillNeedsAttention", { defaultValue: "Local folder saved, but validation still needs attention." }),
      });
      queryClient.invalidateQueries({ queryKey: queryKeys.plugins.localFolders(pluginId, companyId) });
    },
    onError: (err: Error) => {
      setMessage({ type: "error", text: err.message || t("app.pluginSettings.failedToSaveLocalFolder", { defaultValue: "Failed to save local folder." }) });
    },
  });
    ...
    <div className="space-y-4 rounded-md border border-border/70 bg-background px-4 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant={status?.healthy ? "default" : "secondary"}>
              {status?.healthy ? t("app.pluginSettings.healthy", { defaultValue: "Healthy" }) : t("app.pluginSettings.needsAttention", { defaultValue: "Needs attention" })}
            </Badge>
          </div>
        ...
        <Badge variant={access === "readWrite" ? "default" : "outline"}>
          {access === "readWrite" ? "Read/write" : t("app.pluginSettings.readOnly", { defaultValue: "Read only" })}
        </Badge>
      </div>

      <div className="grid gap-3 text-sm sm:grid-cols-3">
        <FolderStatusMetric label={t("app.pluginSettings.configured", { defaultValue: "Configured" })} value={status?.configured ? t("app.pluginSettings.yes", { defaultValue: "Yes" }) : t("app.pluginSettings.no", { defaultValue: "No" })} ok={!!status?.configured} />
        <FolderStatusMetric label={t("app.pluginSettings.readable", { defaultValue: "Readable" })} value={status?.readable ? t("app.pluginSettings.yes", { defaultValue: "Yes" }) : t("app.pluginSettings.no", { defaultValue: "No" })} ok={!!status?.readable} />
        <FolderStatusMetric
          label={t("app.pluginSettings.writable", { defaultValue: "Writable" })}
          value={access === "read" ? t("app.pluginSettings.notRequested", { defaultValue: "Not requested" }) : status?.writable ? t("app.pluginSettings.yes", { defaultValue: "Yes" }) : t("app.pluginSettings.no", { defaultValue: "No" })}
      ...
      {status?.path ? (
        <div className="space-y-1 text-sm">
          <div className="text-xs font-medium text-muted-foreground">{t("app.pluginSettings.configuredPath", { defaultValue: "Configured path" })}</div>
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-medium text-muted-foreground" htmlFor={`local-folder-${declaration.folderKey}`}>
          {t("app.pluginSettings.localFolderPath", { defaultValue: "Local folder path" })}</label>
        <div className="flex items-center gap-2">
          <input
      ...
      {status?.problems?.length ? (
        <div className="space-y-2 rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          <div className="font-medium">{t("app.pluginSettings.validationProblems", { defaultValue: "Validation problems" })}</div>
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {inspectionUnavailable ? (
          <Badge variant="secondary" className="text-(length:--text-nano)">
            {t("app.pluginSettings.notInspected", { defaultValue: "Not inspected" })}</Badge>
        ) : missingItems.length > 0 ? (
          <Badge variant="destructive" className="text-(length:--text-nano)">
            {missingItems.length} missing
          </Badge>
        ) : (
          <Badge variant="outline" className="text-(length:--text-nano)">{t("app.pluginSettings.present", { defaultValue: "Present" })}</Badge>
        )}
      </div>
      {items.length > 0 ? (
        ...
        <p className="text-xs text-muted-foreground">{t("app.pluginSettings.noneDeclared", { defaultValue: "None declared." })}</p>
      )}
      {inspectionUnavailable ? (
        <p className="text-xs text-amber-700 dark:text-amber-300">{t("app.pluginSettings.configuredRootWasNotInspected", { defaultValue: "Configured root was not inspected." })}</p>
      ) : missingItems.length > 0 ? (
        <p className="text-xs text-destructive">{missingLabel}{t("app.pluginSettings.text", { defaultValue: ": " })}{missingItems.join(", ")}</p>
      ) : null}
    </div>
  );
}
...
function PluginConfigForm({ pluginId, companyId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("app.pluginSettings.failedToSaveConfiguration", { defaultValue: "Failed to save configuration." }) });
    },
  });
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.valid) {
        ...
        setTestResult({ type: "error", text: result.message || t("app.pluginSettings.configurationTestFailed", { defaultValue: "Configuration test failed." }) });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || t("app.pluginSettings.configurationTestFailed", { defaultValue: "Configuration test failed." }) });
    },
  });
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 text-sm text-muted-foreground py-4">
        <Loader2 className="h-4 w-4 animate-spin" />
        {t("app.pluginSettings.loadingConfiguration", { defaultValue: "Loading configuration..." })}</div>
    );
  }
      ...
      <div className="flex items-center gap-2 pt-2">
        ...
        >
          {saveMutation.isPending ? (
            <>
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
              {t("app.pluginSettings.saving", { defaultValue: "Saving..." })}</>
          ) : (
            "Save Configuration"
          )}
        </Button>
        {pluginStatus === "ready" && supportsConfigTest && (
          ...
          >
            {testMutation.isPending ? (
              <>
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                {t("app.pluginSettings.testing", { defaultValue: "Testing..." })}</>
            ) : (
              "Test Configuration"
            )}
          </Button>

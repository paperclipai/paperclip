...
import { pluginSettingsPage } from "@/lib/i18n";
...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? pluginSettingsPage.breadcrumbPluginDetails },
    ]);
  ...
  if (pluginLoading) {
    return <div className="p-4 text-sm text-muted-foreground">{pluginSettingsPage.loadingPlugin}</div>;
  }
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "configuration" | "status")} className="space-y-6">
        <PageTabBar
          align="start"
          items={[
            { value: "configuration", label: pluginSettingsPage.tabConfiguration },
            { value: "status", label: pluginSettingsPage.tabStatus },
          ]}
          value={activeTab}
        ...
        <TabsContent value="configuration" className="space-y-6">
          <div className="space-y-8">
            <section className="space-y-5">
              <h2 className="text-base font-semibold">{pluginSettingsPage.about}</h2>
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.8fr)]">
                <div className="space-y-2">
                  <h3 className="text-sm font-medium text-muted-foreground">{pluginSettingsPage.description}</h3>
                  <p className="text-sm leading-6 text-foreground/90">{pluginDescription}</p>
                </div>
                <div className="space-y-4 text-sm">
                  <div className="space-y-1.5">
                    <h3 className="font-medium text-muted-foreground">{pluginSettingsPage.author}</h3>
                    <p className="text-foreground">{plugin.manifestJson.author}</p>
                  </div>
                  <div className="space-y-2">
                    <h3 className="font-medium text-muted-foreground">{pluginSettingsPage.categories}</h3>
                    <div className="flex flex-wrap gap-2">
                      {plugin.categories.length > 0 ? (
                        ...
                        <span className="text-foreground">{pluginSettingsPage.none}</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <section className="space-y-4">
              <div className="space-y-1">
                <h2 className="text-base font-semibold">{pluginSettingsPage.settings}</h2>
              </div>
              {hasLocalFolders ? (
              ...
              {hasCustomSettingsPage ? (
                ...
                <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm">
                  <p className="font-medium text-foreground">{pluginSettingsPage.configureFromEnvironments}</p>
                  <p className="mt-1 text-muted-foreground">
                    {pluginSettingsPage.configureFromEnvironmentsDesc(driverLabel)}
                  </p>
                  <div className="mt-3">
                    <Link to="/company/settings/environments">
                      <Button variant="outline" size="sm">{pluginSettingsPage.openCompanyEnvironments}</Button>
                    </Link>
                  </div>
                </div>
              ) : !hasLocalFolders ? (
                <p className="text-sm text-muted-foreground">
                  {pluginSettingsPage.noSettingsRequired}
                </p>
              ) : null}
            </section>
          </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {pluginSettingsPage.runtimeDashboard}
                  </CardTitle>
                  <CardDescription>
                    {pluginSettingsPage.runtimeDashboardDesc}
                  </CardDescription>
                </CardHeader>
                    ...
                    <>
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {pluginSettingsPage.workerProcess}
                        </h3>
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{pluginSettingsPage.status}</span>
                              <Badge variant={dashboardData.worker.status === "running" ? "default" : "secondary"}>
                                {dashboardData.worker.status}
                              </Badge>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{pluginSettingsPage.pid}</span>
                              <span className="font-mono text-xs">{dashboardData.worker.pid ?? "—"}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{pluginSettingsPage.uptime}</span>
                              <span className="text-xs">{formatUptime(dashboardData.worker.uptime)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{pluginSettingsPage.pendingRPCs}</span>
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  <span className="text-muted-foreground flex items-center gap-1">
                                    ...
                                    {pluginSettingsPage.crashes}
                                  </span>
                                  <span className="text-xs">
                                    {pluginSettingsPage.consecutiveTotal(dashboardData.worker.consecutiveCrashes, dashboardData.worker.totalCrashes)}
                                  </span>
                                </div>
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    <span className="text-muted-foreground">{pluginSettingsPage.lastCrash}</span>
                          ...
                          <p className="text-sm text-muted-foreground italic">{pluginSettingsPage.noWorker}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {pluginSettingsPage.recentJobRuns}
                        </h3>
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{pluginSettingsPage.noJobRuns}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {pluginSettingsPage.recentWebhookDeliveries}
                        </h3>
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{pluginSettingsPage.noWebhookDeliveries}</p>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        <Clock className="h-3 w-3" />
                        {pluginSettingsPage.lastChecked(new Date(dashboardData.checkedAt).toLocaleTimeString())}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      {pluginSettingsPage.diagnosticsUnavailable}
                    </p>
                  )}
                </CardContent>
              </Card>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base flex items-center gap-1.5">
                      ...
                      {pluginSettingsPage.recentLogs}
                    </CardTitle>
                    <CardDescription>{pluginSettingsPage.recentLogEntries(recentLogs.length)}</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    <ActivitySquare className="h-4 w-4" />
                    {pluginSettingsPage.healthStatus}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {healthLoading ? (
                    <p className="text-sm text-muted-foreground">{pluginSettingsPage.checkingHealth}</p>
                  ) : healthData ? (
                    <div className="space-y-4 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">{pluginSettingsPage.overall}</span>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">{pluginSettingsPage.detailsCard}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3 text-sm text-muted-foreground">
                  <div className="flex justify-between gap-3">
                    <span>{pluginSettingsPage.pluginIdLabel}</span>
                    <span className="font-mono text-xs text-right">{plugin.id}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{pluginSettingsPage.pluginKeyLabel}</span>
                    <span className="font-mono text-xs text-right">{plugin.pluginKey}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{pluginSettingsPage.npmPackageLabel}</span>
                    <span className="max-w-[170px] truncate text-right text-xs" title={plugin.packageName}>
                      {plugin.packageName}
                    </span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{pluginSettingsPage.versionLabel}</span>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {pluginSettingsPage.permissions}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {pluginCapabilities.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">{pluginSettingsPage.noPermissionsRequested}</p>
                  )}
                </CardContent>
              </Card>
            </div>
  ...
  if (!companyId) {
    return (
      <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
        {pluginSettingsPage.selectCompanyForFolders}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-medium">{pluginSettingsPage.localFolders}</h3>
      </div>
      {error ? (
      ...
      {isLoading ? (
        <div className="flex items-center gap-2 py-3 text-sm text-muted-foreground">
          ...
          {pluginSettingsPage.loadingFolders}
        </div>
      ) : (
        <div className="space-y-3">
...
function PluginLocalFolderRow({ pluginId, companyId, declaration, status }: PluginLocalFolderRowProps) {
  ...
  const handleSave = useCallback(() => {
    if (!trimmedPath) {
      setMessage({ type: "error", text: pluginSettingsPage.localFolderRequired });
      return;
    }
    if (!isLikelyAbsolutePath(trimmedPath)) {
      setMessage({ type: "error", text: pluginSettingsPage.localFolderAbsolute });
      return;
    }
    setMessage(null);
    ...
    <div className="space-y-4 rounded-md border border-border/70 bg-background px-4 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant={status?.healthy ? "default" : "secondary"}>
              {status?.healthy ? pluginSettingsPage.healthy : pluginSettingsPage.needsAttention}
            </Badge>
          </div>
        ...
        <Badge variant={access === "readWrite" ? "default" : "outline"}>
          {access === "readWrite" ? pluginSettingsPage.readWrite : pluginSettingsPage.readOnly}
        </Badge>
      </div>

      <div className="grid gap-3 text-sm sm:grid-cols-3">
        <FolderStatusMetric label={pluginSettingsPage.configured} value={status?.configured ? pluginSettingsPage.yes : pluginSettingsPage.no} ok={!!status?.configured} />
        <FolderStatusMetric label={pluginSettingsPage.readable} value={status?.readable ? pluginSettingsPage.yes : pluginSettingsPage.no} ok={!!status?.readable} />
        <FolderStatusMetric
          label={pluginSettingsPage.writable}
          value={access === "read" ? pluginSettingsPage.notRequested : status?.writable ? pluginSettingsPage.yes : pluginSettingsPage.no}
      ...
      {status?.path ? (
        <div className="space-y-1 text-sm">
          <div className="text-xs font-medium text-muted-foreground">{pluginSettingsPage.configuredPath}</div>
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-medium text-muted-foreground" htmlFor={`local-folder-${declaration.folderKey}`}>
          {pluginSettingsPage.localFolderRequired}
        </label>
        <div className="flex items-center gap-2">
          <input
...
function FolderRequirements({
  ...
  return (
    <div className="grid gap-3 text-sm md:grid-cols-2">
      <RequirementList
        title={pluginSettingsPage.requiredDirectories}
        items={requiredDirectories}
        missingItems={missingDirectories}
        missingLabel={missingDirectories.length > 0 ? `缺少目录：${missingDirectories.join(", ")}` : ""}
        inspectionUnavailable={rootNotInspected}
      />
      <RequirementList
        title={pluginSettingsPage.requiredFiles}
        items={requiredFiles}
        missingItems={missingFiles}
        missingLabel={missingFiles.length > 0 ? `缺少文件：${missingFiles.join(", ")}` : ""}
        inspectionUnavailable={rootNotInspected}
      />
    </div>
  );

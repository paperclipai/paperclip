...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("pages.pluginSettings.breadcrumbCompany", { defaultValue: "Company" }), href: "/dashboard" },
      { label: t("pages.pluginSettings.breadcrumbSettings", { defaultValue: "Settings" }), href: "/company/settings" },
      { label: t("pages.pluginSettings.breadcrumbInstanceSettings", { defaultValue: "Instance settings" }), href: "/company/settings/instance/general" },
      { label: t("pages.pluginSettings.breadcrumbPlugins", { defaultValue: "Plugins" }), href: "/company/settings/instance/plugins" },
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? t("pages.pluginSettings.breadcrumbPluginDetails", { defaultValue: "Plugin Details" }) },
    ]);
  ...
  if (pluginLoading) {
    return <div className="p-4 text-sm text-muted-foreground">{t("pages.pluginSettings.loadingPluginDetails", { defaultValue: "Loading plugin details..." })}</div>;
  }
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "configuration" | "status")} className="space-y-6">
        <PageTabBar
          align="start"
          items={[
            { value: "configuration", label: t("pages.pluginSettings.tabConfiguration", { defaultValue: "Configuration" }) },
            { value: "status", label: t("pages.pluginSettings.tabStatus", { defaultValue: "Status" }) },
          ]}
          value={activeTab}
        ...
        <TabsContent value="configuration" className="space-y-6">
          <div className="space-y-8">
            <section className="space-y-5">
              <h2 className="text-base font-semibold">{t("pages.pluginSettings.aboutHeading", { defaultValue: "About" })}</h2>
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.8fr)]">
                <div className="space-y-2">
                  <h3 className="text-sm font-medium text-muted-foreground">{t("pages.pluginSettings.descriptionLabel", { defaultValue: "Description" })}</h3>
                  <p className="text-sm leading-6 text-foreground/90">{pluginDescription}</p>
                </div>
                <div className="space-y-4 text-sm">
                  <div className="space-y-1.5">
                    <h3 className="font-medium text-muted-foreground">{t("pages.pluginSettings.authorLabel", { defaultValue: "Author" })}</h3>
                    <p className="text-foreground">{plugin.manifestJson.author}</p>
                  </div>
                  <div className="space-y-2">
                    <h3 className="font-medium text-muted-foreground">{t("pages.pluginSettings.categoriesLabel", { defaultValue: "Categories" })}</h3>
                    <div className="flex flex-wrap gap-2">
                      {plugin.categories.length > 0 ? (
                        ...
                        <span className="text-foreground">{t("pages.pluginSettings.categoriesNone", { defaultValue: "None" })}</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <section className="space-y-4">
              <div className="space-y-1">
                <h2 className="text-base font-semibold">{t("pages.pluginSettings.settingsHeading", { defaultValue: "Settings" })}</h2>
              </div>
              {hasLocalFolders ? (
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {t("pages.pluginSettings.runtimeDashboardTitle", { defaultValue: "Runtime Dashboard" })}
                  </CardTitle>
                  <CardDescription>
                    {t("pages.pluginSettings.runtimeDashboardDescription", { defaultValue: "Worker process, scheduled jobs, and webhook deliveries" })}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  {dashboardData ? (
                    <>
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("pages.pluginSettings.workerProcessHeading", { defaultValue: "Worker Process" })}
                        </h3>
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("pages.pluginSettings.workerStatusLabel", { defaultValue: "Status" })}</span>
                              <Badge variant={dashboardData.worker.status === "running" ? "default" : "secondary"}>
                                {dashboardData.worker.status}
                              </Badge>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("pages.pluginSettings.workerPidLabel", { defaultValue: "PID" })}</span>
                              <span className="font-mono text-xs">{dashboardData.worker.pid ?? "—"}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("pages.pluginSettings.workerUptimeLabel", { defaultValue: "Uptime" })}</span>
                              <span className="text-xs">{formatUptime(dashboardData.worker.uptime)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("pages.pluginSettings.workerPendingRpcsLabel", { defaultValue: "Pending RPCs" })}</span>
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  <span className="text-muted-foreground flex items-center gap-1">
                                    ...
                                    {t("pages.pluginSettings.crashesLabel", { defaultValue: "Crashes" })}
                                  </span>
                                  <span className="text-xs">
                                    {t("pages.pluginSettings.crashesSummary", {
                                ...
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    <span className="text-muted-foreground">{t("pages.pluginSettings.lastCrashLabel", { defaultValue: "Last Crash" })}</span>
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("pages.pluginSettings.noWorkerProcess", { defaultValue: "No worker process registered." })}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("pages.pluginSettings.recentJobRunsHeading", { defaultValue: "Recent Job Runs" })}
                        </h3>
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("pages.pluginSettings.noJobRuns", { defaultValue: "No job runs recorded yet." })}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("pages.pluginSettings.recentWebhookDeliveriesHeading", { defaultValue: "Recent Webhook Deliveries" })}
                        </h3>
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("pages.pluginSettings.noWebhookDeliveries", { defaultValue: "No webhook deliveries recorded yet." })}</p>
                        )}
                      </div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {t("pages.pluginSettings.runtimeDiagnosticsUnavailable", { defaultValue: "Runtime diagnostics are unavailable right now." })}
                    </p>
                  )}
                </CardContent>
              </Card>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base flex items-center gap-1.5">
                      ...
                      {t("pages.pluginSettings.recentLogsTitle", { defaultValue: "Recent Logs" })}
                    </CardTitle>
                    <CardDescription>{t("pages.pluginSettings.recentLogsDescription", { count: recentLogs.length, defaultValue: "Last {{count}} log entries" })}</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    <ActivitySquare className="h-4 w-4" />
                    {t("pages.pluginSettings.healthStatusTitle", { defaultValue: "Health Status" })}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {healthLoading ? (
                    ...
                    <div className="space-y-4 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">{t("pages.pluginSettings.healthOverallLabel", { defaultValue: "Overall" })}</span>
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      <div className="flex items-center justify-between">
                        <span>{t("pages.pluginSettings.lifecycleLabel", { defaultValue: "Lifecycle" })}</span>
                        <Badge variant={statusVariant}>{displayStatus}</Badge>
                      </div>
                      <p>{t("pages.pluginSettings.healthChecksWhenReady", { defaultValue: "Health checks run once the plugin is ready." })}</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">{t("pages.pluginSettings.detailsTitle", { defaultValue: "Details" })}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3 text-sm text-muted-foreground">
                  <div className="flex justify-between gap-3">
                    <span>{t("pages.pluginSettings.pluginIdLabel", { defaultValue: "Plugin ID" })}</span>
                    <span className="font-mono text-xs text-right">{plugin.id}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("pages.pluginSettings.pluginKeyLabel", { defaultValue: "Plugin Key" })}</span>
                    <span className="font-mono text-xs text-right">{plugin.pluginKey}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("pages.pluginSettings.npmPackageLabel", { defaultValue: "NPM Package" })}</span>
                    <span className="max-w-[170px] truncate text-right text-xs" title={plugin.packageName}>
                      {plugin.packageName}
                    </span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("pages.pluginSettings.versionLabel", { defaultValue: "Version" })}</span>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {t("pages.pluginSettings.permissionsTitle", { defaultValue: "Permissions" })}
                  </CardTitle>
                </CardHeader>
                <CardContent>
  ...
  if (!companyId) {
    return (
      <div className="rounded-md border border-border/60 bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
        {t("pages.pluginSettings.selectCompanyForLocalFolders", { defaultValue: "Select a company to configure this plugin's local folders." })}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-medium">{t("pages.pluginSettings.localFoldersHeading", { defaultValue: "Local folders" })}</h3>
      </div>
      {error ? (
        <div className="rounded-md border border-destructive/20 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          {(error as Error).message || t("pages.pluginSettings.failedToLoadLocalFolders", { defaultValue: "Failed to load local folder settings." })}
        </div>
      ) : null}
      {isLoading ? (
        <div className="flex items-center gap-2 py-3 text-sm text-muted-foreground">
          <Loader2 className="h-4 w-4 animate-spin" />
          {t("pages.pluginSettings.loadingLocalFolders", { defaultValue: "Loading local folders..." })}
        </div>
      ) : (
        <div className="space-y-3">
...
function PluginLocalFolderRow({ pluginId, companyId, declaration, status }: PluginLocalFolderRowProps) {
    ...
    onSuccess: (nextStatus) => {
      setMessage({
        ...
        text: nextStatus.healthy
          ? t("pages.pluginSettings.localFolderSaved", { defaultValue: "Local folder saved." })
          : t("pages.pluginSettings.localFolderSavedNeedsAttention", { defaultValue: "Local folder saved, but validation still needs attention." }),
      });
      queryClient.invalidateQueries({ queryKey: queryKeys.plugins.localFolders(pluginId, companyId) });
    },
    onError: (err: Error) => {
      setMessage({ type: "error", text: err.message || t("pages.pluginSettings.failedToSaveLocalFolder", { defaultValue: "Failed to save local folder." }) });
    },
  });
  ...
  const handleSave = useCallback(() => {
    if (!trimmedPath) {
      setMessage({ type: "error", text: t("pages.pluginSettings.localFolderPathRequired", { defaultValue: "Local folder path is required." }) });
      return;
    }
    if (!isLikelyAbsolutePath(trimmedPath)) {
      setMessage({ type: "error", text: t("pages.pluginSettings.localFolderMustBeAbsolute", { defaultValue: "Local folder must be a full absolute path." }) });
      return;
    }
    setMessage(null);
    ...
    <div className="space-y-4 rounded-md border border-border/70 bg-background px-4 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant={status?.healthy ? "default" : "secondary"}>
              {status?.healthy
                ? t("pages.pluginSettings.folderHealthy", { defaultValue: "Healthy" })
                : t("pages.pluginSettings.folderNeedsAttention", { defaultValue: "Needs attention" })}
            </Badge>
          </div>
        ...
        <Badge variant={access === "readWrite" ? "default" : "outline"}>
          {access === "readWrite"
            ? t("pages.pluginSettings.accessReadWrite", { defaultValue: "Read/write" })
            : t("pages.pluginSettings.accessReadOnly", { defaultValue: "Read only" })}
        </Badge>
      </div>

      <div className="grid gap-3 text-sm sm:grid-cols-3">
        <FolderStatusMetric
          label={t("pages.pluginSettings.metricConfigured", { defaultValue: "Configured" })}
          value={status?.configured
            ? t("pages.pluginSettings.metricYes", { defaultValue: "Yes" })
            : t("pages.pluginSettings.metricNo", { defaultValue: "No" })}
          ok={!!status?.configured}
        />
        <FolderStatusMetric
          label={t("pages.pluginSettings.metricReadable", { defaultValue: "Readable" })}
          value={status?.readable
            ? t("pages.pluginSettings.metricYes", { defaultValue: "Yes" })
            : t("pages.pluginSettings.metricNo", { defaultValue: "No" })}
          ok={!!status?.readable}
        />
        <FolderStatusMetric
          label={t("pages.pluginSettings.metricWritable", { defaultValue: "Writable" })}
          value={access === "read"
            ? t("pages.pluginSettings.metricNotRequested", { defaultValue: "Not requested" })
            : status?.writable
              ? t("pages.pluginSettings.metricYes", { defaultValue: "Yes" })
              : t("pages.pluginSettings.metricNo", { defaultValue: "No" })}
      ...
      {status?.path ? (
        <div className="space-y-1 text-sm">
          <div className="text-xs font-medium text-muted-foreground">{t("pages.pluginSettings.configuredPathLabel", { defaultValue: "Configured path" })}</div>
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-medium text-muted-foreground" htmlFor={`local-folder-${declaration.folderKey}`}>
          {t("pages.pluginSettings.localFolderPathLabel", { defaultValue: "Local folder path" })}
        </label>
        <div className="flex items-center gap-2">
          <input
          ...
          >
            ...
            {t("pages.pluginSettings.saveButton", { defaultValue: "Save" })}
          </Button>
        </div>
      </div>
...
function FolderRequirements({
  ...
  return (
    <div className="grid gap-3 text-sm md:grid-cols-2">
      <RequirementList
        title={t("pages.pluginSettings.requiredDirectoriesTitle", { defaultValue: "Required directories" })}
        items={requiredDirectories}
        missingItems={missingDirectories}
        missingLabel={t("pages.pluginSettings.missingDirectoriesLabel", { defaultValue: "Missing directories" })}
        inspectionUnavailable={rootNotInspected}
      />
      <RequirementList
        title={t("pages.pluginSettings.requiredFilesTitle", { defaultValue: "Required files" })}
        items={requiredFiles}
        missingItems={missingFiles}
        missingLabel={t("pages.pluginSettings.missingFilesLabel", { defaultValue: "Missing files" })}
        inspectionUnavailable={rootNotInspected}
      />
    </div>
  );
...
function RequirementList({
  ...
  return (
    <div className="space-y-2 rounded-md border border-border/60 px-3 py-2">
      <div className="flex items-center justify-between gap-2">
        ...
        {inspectionUnavailable ? (
          <Badge variant="secondary" className="text-[10px]">
            {t("pages.pluginSettings.requirementNotInspected", { defaultValue: "Not inspected" })}
          </Badge>
        ) : missingItems.length > 0 ? (
          <Badge variant="destructive" className="text-[10px]">
            {t("pages.pluginSettings.requirementMissingCount", { count: missingItems.length, defaultValue: "{{count}} missing" })}
          </Badge>
        ) : (
          <Badge variant="outline" className="text-[10px]">{t("pages.pluginSettings.requirementPresent", { defaultValue: "Present" })}</Badge>
        )}
      </div>
      {items.length > 0 ? (
      ...
      {inspectionUnavailable ? (
        <p className="text-xs text-amber-700 dark:text-amber-300">{t("pages.pluginSettings.configuredRootNotInspected", { defaultValue: "Configured root was not inspected." })}</p>
      ) : missingItems.length > 0 ? (
        <p className="text-xs text-destructive">{t("pages.pluginSettings.missingItemsSummary", { missingLabel, items: missingItems.join(", "), defaultValue: "{{missingLabel}}: {{items}}" })}</p>
      ) : null}
    </div>
  );
}
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      setSaveMessage({ type: "success", text: t("pages.pluginSettings.configurationSaved", { defaultValue: "Configuration saved." }) });
      setTestResult(null);
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("pages.pluginSettings.failedToSaveConfiguration", { defaultValue: "Failed to save configuration." }) });
    },
  });
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({ type: "success", text: t("pages.pluginSettings.configurationTestPassed", { defaultValue: "Configuration test passed." }) });
      } else {
        setTestResult({ type: "error", text: result.message || t("pages.pluginSettings.configurationTestFailed", { defaultValue: "Configuration test failed." }) });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || t("pages.pluginSettings.configurationTestFailed", { defaultValue: "Configuration test failed." }) });
    },
  });
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 text-sm text-muted-foreground py-4">
        <Loader2 className="h-4 w-4 animate-spin" />
        {t("pages.pluginSettings.loadingConfiguration", { defaultValue: "Loading configuration..." })}
      </div>
    );
  }
      ...
      <div className="flex items-center gap-2 pt-2">
        ...
        >
          {saveMutation.isPending ? (
            <>
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
              {t("pages.pluginSettings.savingButton", { defaultValue: "Saving..." })}
            </>
          ) : (
            t("pages.pluginSettings.saveConfigurationButton", { defaultValue: "Save Configuration" })
          )}
        </Button>
        {pluginStatus === "ready" && supportsConfigTest && (
          ...
          >
            {testMutation.isPending ? (
              <>
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                {t("pages.pluginSettings.testingButton", { defaultValue: "Testing..." })}
              </>
            ) : (
              t("pages.pluginSettings.testConfigurationButton", { defaultValue: "Test Configuration" })
            )}
          </Button>
        )}
      </div>

                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {plugin.categories.length > 0 ? (
                        ...
                        <span className="text-foreground">{t("settings.none", { defaultValue: "None" })}</span>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_320px]">
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {t("settings.runtimeDashboardDesc", { defaultValue: "Worker process, scheduled jobs, and webhook deliveries" })}
                  </CardDescription>
                </CardHeader>
                    ...
                    <>
                        ...
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("settings.statusLabel", { defaultValue: "Status" })}</span>
                              <Badge variant={dashboardData.worker.status === "running" ? "default" : "secondary"}>
                                {dashboardData.worker.status}
                              </Badge>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("settings.pid", { defaultValue: "PID" })}</span>
                              <span className="font-mono text-xs">{dashboardData.worker.pid ?? "—"}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("settings.uptime", { defaultValue: "Uptime" })}</span>
                              <span className="text-xs">{formatUptime(dashboardData.worker.uptime)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("settings.pendingRpcs", { defaultValue: "Pending RPCs" })}</span>
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  ...
                                  <span className="text-xs">
                                    {t("settings.crashCount", { defaultValue: "{{consecutive}} consecutive / {{total}} total", consecutive: dashboardData.worker.consecutiveCrashes, total: dashboardData.worker.totalCrashes })}
                                  </span>
                                </div>
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    <span className="text-muted-foreground">{t("settings.lastCrash", { defaultValue: "Last Crash" })}</span>
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("settings.noWorkerProcess", { defaultValue: "No worker process registered." })}</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("settings.noJobRuns", { defaultValue: "No job runs recorded yet." })}</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("settings.noWebhookDeliveries", { defaultValue: "No webhook deliveries recorded yet." })}</p>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        <Clock className="h-3 w-3" />
                        {t("settings.lastChecked", { defaultValue: "Last checked:" })} {new Date(dashboardData.checkedAt).toLocaleTimeString()}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
              ...
              {recentLogs && recentLogs.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>{t("settings.lastLogEntries", { defaultValue: "Last {{count}} log entries", count: recentLogs.length })}</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="space-y-6">
              <Card>
                ...
                <CardContent>
                  {healthLoading ? (
                    <p className="text-sm text-muted-foreground">{t("settings.checkingHealth", { defaultValue: "Checking health..." })}</p>
                  ) : healthData ? (
                    <div className="space-y-4 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">{t("settings.overall", { defaultValue: "Overall" })}</span>
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      <div className="flex items-center justify-between">
                        <span>{t("settings.lifecycle", { defaultValue: "Lifecycle" })}</span>
                        <Badge variant={statusVariant}>{displayStatus}</Badge>
                      </div>
                      <p>{t("settings.healthChecksRunOnce", { defaultValue: "Health checks run once the plugin is ready." })}</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">{t("settings.details", { defaultValue: "Details" })}</CardTitle>
                </CardHeader>
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      setSaveMessage({ type: "success", text: t("settings.configSaved", { defaultValue: "Configuration saved." }) });
      setTestResult(null);
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("settings.configSaveFailed", { defaultValue: "Failed to save configuration." }) });
    },
  });
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({ type: "success", text: t("settings.configTestPassed", { defaultValue: "Configuration test passed." }) });
      } else {
        setTestResult({ type: "error", text: result.message || t("settings.configTestFailed", { defaultValue: "Configuration test failed." }) });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || t("settings.configTestFailed", { defaultValue: "Configuration test failed." }) });
    },
  });
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 text-sm text-muted-foreground py-4">
        ...
        {t("settings.loadingConfig", { defaultValue: "Loading configuration..." })}
      </div>
    );
  }
      ...
      <div className="flex items-center gap-2 pt-2">
        ...
        >
          {saveMutation.isPending ? (
            ...
            t("settings.saveConfiguration", { defaultValue: "Save Configuration" })
          )}
        </Button>
        {pluginStatus === "ready" && supportsConfigTest && (
          ...
          >
            {testMutation.isPending ? (
              <>
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                {t("settings.testing", { defaultValue: "Testing..." })}
              </>
            ) : (
              t("settings.testConfiguration", { defaultValue: "Test Configuration" })
            )}
          </Button>
        )}
      </div>

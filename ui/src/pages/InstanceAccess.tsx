...
export function InstanceAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("instanceAccess.breadcrumbInstance", { defaultValue: "Instance Settings" }), href: "/instance/settings/general" },
      { label: t("instanceAccess.breadcrumbAccess", { defaultValue: "Access" }) },
    ]);
  }, [setBreadcrumbs]);
  ...
  const updateCompanyAccessMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: i18n.t("settings:instanceAccess.toastCompanyAccessUpdated", { defaultValue: "Company access updated" }), tone: "success" });
    },
  });
  ...
  const setAdminMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: i18n.t("settings:instanceAccess.toastInstanceRoleUpdated", { defaultValue: "Instance role updated" }), tone: "success" });
    },
  });

  if (usersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{i18n.t("settings:instanceAccess.loading", { defaultValue: "Loading instance users\u2026" })}</div>;
  }

  if (usersQuery.error) {
    const message =
      usersQuery.error instanceof ApiError && usersQuery.error.status === 403
        ? i18n.t("settings:instanceAccess.noPermission", { defaultValue: "Instance admin access is required to manage users." })
        : usersQuery.error instanceof Error
          ? usersQuery.error.message
          : i18n.t("settings:instanceAccess.loadFailed", { defaultValue: "Failed to load users." });
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("instanceAccess.description", { defaultValue: "Search users, manage instance-admin status, and control which companies they can access." })}
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
        <section className="space-y-4 rounded-xl border border-border bg-card p-4">
          <label className="block space-y-2 text-sm">
            <span className="font-medium">{t("instanceAccess.searchUsersLabel", { defaultValue: "Search users" })}</span>
            <input
            ...
            {(usersQuery.data ?? []).map((user) => (
              ...
              >
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  {t("instanceAccess.activeCompanyMemberships", { defaultValue: "{{count}} active company memberships", count: user.activeCompanyMembershipCount })}
                </div>
              </button>
            ))}
          </div>
        ...
        <section className="space-y-4 rounded-xl border border-border bg-card p-5">
          {!selectedUserId ? (
            ...
            <div className="text-sm text-destructive">
              {userAccessQuery.error instanceof Error ? userAccessQuery.error.message : t("instanceAccess.loadUserAccessFailed", { defaultValue: "Failed to load user access." })}
            </div>
          ) : (
            <>
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                >
                  {selectedUser?.isInstanceAdmin
                    ? t("instanceAccess.removeInstanceAdmin", { defaultValue: "Remove instance admin" })
                    : t("instanceAccess.promoteToInstanceAdmin", { defaultValue: "Promote to instance admin" })}
                </Button>
              </div>

              <div className="space-y-3">
                <div>
                  <h2 className="text-sm font-semibold">{t("instanceAccess.companyAccessHeading", { defaultValue: "Company access" })}</h2>
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {updateCompanyAccessMutation.isPending
                      ? t("instanceAccess.savingCompanyAccess", { defaultValue: "Saving\u2026" })
              ...
              <div className="space-y-2">
                <h2 className="text-sm font-semibold">{t("instanceAccess.currentMembershipsHeading", { defaultValue: "Current memberships" })}</h2>
                <div className="space-y-2">
                    ...
                    >
                      <div>
                        ...
                        <div className="text-muted-foreground">
                          {membership.membershipRole || t("instanceAccess.roleUnset", { defaultValue: "unset" })} {"\u2022"} {membership.status}
                        </div>
                      </div>

...
export function InstanceAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: i18n.t("pages.InstanceAccess.label"), href: "/instance/settings/general" },
      { label: "Access" },
    ]);
  }, [setBreadcrumbs]);
  ...
  const updateCompanyAccessMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: i18n.t("pages.InstanceAccess.title"), tone: "success" });
    },
  });
  ...
  const setAdminMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: i18n.t("pages.InstanceAccess.title_1"), tone: "success" });
    },
  });

  if (usersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{i18n.t("pages.InstanceAccess.div")}</div>;
  }

  if (usersQuery.error) {
    const message =
      usersQuery.error instanceof ApiError && usersQuery.error.status === 403
        ? i18n.t("pages.InstanceAccess.conditional")
        : usersQuery.error instanceof Error
          ? usersQuery.error.message
          : i18n.t("pages.InstanceAccess.conditional_1");
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{i18n.t("pages.InstanceAccess.h1")}</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          {i18n.t("pages.InstanceAccess.p")}</p>
      </div>

      <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
        <section className="space-y-4 rounded-xl border border-border bg-card p-4">
          <label className="block space-y-2 text-sm">
            <span className="font-medium">{i18n.t("pages.InstanceAccess.span")}</span>
            <input
              className="w-full rounded-md border border-border bg-background px-3 py-2"
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder={i18n.t("pages.InstanceAccess.placeholder")}
            />
          </label>
        ...
        <section className="space-y-4 rounded-xl border border-border bg-card p-5">
          {!selectedUserId ? (
            <div className="text-sm text-muted-foreground">{i18n.t("pages.InstanceAccess.div_1")}</div>
          ) : userAccessQuery.isLoading ? (
            <div className="text-sm text-muted-foreground">{i18n.t("pages.InstanceAccess.div_2")}</div>
          ) : userAccessQuery.error ? (
            <div className="text-sm text-destructive">
              {userAccessQuery.error instanceof Error ? userAccessQuery.error.message : i18n.t("pages.InstanceAccess.conditional_2")}
            </div>
          ) : (
            <>
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                >
                  {selectedUser?.isInstanceAdmin ? i18n.t("pages.InstanceAccess.conditional_3") : i18n.t("pages.InstanceAccess.conditional_4")}
                </Button>
              </div>

              <div className="space-y-3">
                <div>
                  <h2 className="text-sm font-semibold">{i18n.t("pages.InstanceAccess.h2")}</h2>
                  <p className="text-sm text-muted-foreground">
                    {i18n.t("pages.InstanceAccess.p_1")}</p>
                </div>
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {updateCompanyAccessMutation.isPending ? i18n.t("pages.InstanceAccess.conditional_5") : i18n.t("pages.InstanceAccess.conditional_6")}
                  </Button>
                </div>
              </div>

              <div className="space-y-2">
                <h2 className="text-sm font-semibold">{i18n.t("pages.InstanceAccess.h2_1")}</h2>
                <div className="space-y-2">

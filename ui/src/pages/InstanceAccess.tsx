...
import { instanceAccessPage, sidebarNav } from "../lib/i18n";
...
export function InstanceAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: sidebarNav.instanceSettings, href: "/instance/settings/general" },
      { label: sidebarNav.instanceAccess },
    ]);
  }, [setBreadcrumbs]);
  ...
  const updateCompanyAccessMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: instanceAccessPage.toastCompanyAccessUpdated, tone: "success" });
    },
  });
  ...
  const setAdminMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: instanceAccessPage.toastInstanceRoleUpdated, tone: "success" });
    },
  });

  if (usersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{instanceAccessPage.loadingUsers}</div>;
  }

  if (usersQuery.error) {
    const message =
      usersQuery.error instanceof ApiError && usersQuery.error.status === 403
        ? instanceAccessPage.adminAccessRequired
        : usersQuery.error instanceof Error
          ? usersQuery.error.message
          : instanceAccessPage.failedToLoadUsers;
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{instanceAccessPage.title}</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          {instanceAccessPage.subtitle}
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
        <section className="space-y-4 rounded-xl border border-border bg-card p-4">
          <label className="block space-y-2 text-sm">
            <span className="font-medium">{instanceAccessPage.searchUsers}</span>
            <input
              className="w-full rounded-md border border-border bg-background px-3 py-2"
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder={instanceAccessPage.searchPlaceholder}
            />
          </label>
            ...
            {(usersQuery.data ?? []).map((user) => (
              ...
              >
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  {instanceAccessPage.activeMemberships(user.activeCompanyMembershipCount)}
                </div>
              </button>
            ))}
          </div>
        ...
        <section className="space-y-4 rounded-xl border border-border bg-card p-5">
          {!selectedUserId ? (
            <div className="text-sm text-muted-foreground">{instanceAccessPage.selectUserToInspect}</div>
          ) : userAccessQuery.isLoading ? (
            <div className="text-sm text-muted-foreground">{instanceAccessPage.loadingUserAccess}</div>
          ) : userAccessQuery.error ? (
            <div className="text-sm text-destructive">
              {userAccessQuery.error instanceof Error ? userAccessQuery.error.message : instanceAccessPage.failedToLoadUserAccess}
            </div>
          ) : (
            <>
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                >
                  {selectedUser?.isInstanceAdmin ? instanceAccessPage.removeInstanceAdmin : instanceAccessPage.promoteToInstanceAdmin}
                </Button>
              </div>

              <div className="space-y-3">
                <div>
                  <h2 className="text-sm font-semibold">{instanceAccessPage.companyAccess}</h2>
                  <p className="text-sm text-muted-foreground">
                    {instanceAccessPage.companyAccessHint}
                  </p>
                </div>
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {updateCompanyAccessMutation.isPending ? instanceAccessPage.saving : instanceAccessPage.saveCompanyAccess}
                  </Button>
                </div>
              </div>

              <div className="space-y-2">
                <h2 className="text-sm font-semibold">{instanceAccessPage.currentMemberships}</h2>
                <div className="space-y-2">
                    ...
                    >
                      <div>
                        ...
                        <div className="text-muted-foreground">
                          {membership.membershipRole || instanceAccessPage.membershipRoleUnset} • {membership.status}
                        </div>
                      </div>

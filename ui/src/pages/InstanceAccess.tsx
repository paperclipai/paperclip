...
export function InstanceAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("instanceAccess.tabs.settings"), href: "/company/settings" },
      { label: t("instanceAccess.tabs.instanceSettings"), href: "/company/settings/instance/general" },
      { label: t("instanceAccess.tabs.access") },
    ]);
  }, [setBreadcrumbs]);
  ...
  const updateCompanyAccessMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("instanceAccess.toasts.companyAccessUpdated"), tone: "success" });
    },
  });
  ...
  const setAdminMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("instanceAccess.toasts.instanceRoleUpdated"), tone: "success" });
    },
  });
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instanceAccess.text.instanceAccess")}</h1>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
        <section className="space-y-4 rounded-xl border border-border bg-card p-4">
          <label className="block space-y-2 text-sm">
            <span className="font-medium">{t("instanceAccess.text.searchUsers")}</span>
            <input
              className="w-full rounded-md border border-border bg-background px-3 py-2"
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder={t("instanceAccess.placeholders.searchByName")}
            />
          </label>
        ...
        <section className="space-y-4 rounded-xl border border-border bg-card p-5">
          {!selectedUserId ? (
            <div className="text-sm text-muted-foreground">{t("instanceAccess.text.selectUserToInspect")}</div>
          ) : userAccessQuery.isLoading ? (
            <div className="text-sm text-muted-foreground">Loading user access…</div>
          ) : userAccessQuery.error ? (
            <div className="text-sm text-destructive">
              {userAccessQuery.error instanceof Error ? userAccessQuery.error.message : t("instanceAccess.errors.loadUserAccess")}
            </div>
          ) : (
            <>
              ...
              <div className="space-y-3">
                <div>
                  <h2 className="text-sm font-semibold">{t("instanceAccess.text.companyAccess")}</h2>
              ...
              <div className="space-y-2">
                <h2 className="text-sm font-semibold">{t("instanceAccess.text.currentMemberships")}</h2>
                <div className="space-y-2">

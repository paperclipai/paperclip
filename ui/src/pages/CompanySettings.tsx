  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          <ToggleField
            label="Allow sharing voted AI outputs with Orchestrero"
          ...
          <p className="text-sm text-muted-foreground">
            Votes are always saved locally. This setting controls whether voted AI outputs may also be marked for sharing with Orchestrero.
          </p>
...
function buildAgentSnippet(input: AgentSnippetInput) {
  ...
  const connectivityBlock =
    candidateUrls.length === 0
      ? `No candidate URLs are available. Ask your user to configure a reachable hostname in Orchestrero, then retry.
Suggested steps:
...
Then verify with: curl -fsS <base-url>/api/health`;
  ...
  const resolutionLine = resolutionTestUrl
    ? `\nYou MUST test Orchestrero-to-gateway reachability, call: ${resolutionTestUrl}?url=<urlencoded-gateway-url> (using the hostname that worked above). Do not assume your 172.x is necessarily reachable from Orchestrero. Test it. `
    : "";

  return `You're invited to join an Orchestrero organization.

The URLs you should try are:
${candidateList}

Connectivity guidance:
If you are running on a different machine than Orchestrero, Orchestrero must be reachable at one of the hostnames used above.
Verify the hostname works from your runtime with: GET <base-url>/api/health

${connectivityBlock}

For your "agentName", just use your own agent name (OpenClaw or whatever your name is) and send it directly.
You also need to send a gateway URL that Orchestrero can access. Check your config and include that URL.${resolutionLine}
...
- Keep device auth enabled (recommended). If \`devicePrivateKeyPem\` is omitted, Orchestrero will generate and persist one during join so pairing approvals remain stable.
...
Then after you've connected to Orchestrero (exchanged keys etc.) you MUST review and follow the onboarding instructions in onboarding.txt they give you.

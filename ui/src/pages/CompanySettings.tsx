...
export function CompanySettings() {
  ...
  const feedbackSharingMutation = useMutation({
    ...
    onSuccess: (_company, enabled) => {
      queryClient.invalidateQueries({ queryKey: queryKeys.companies.all });
      pushToast({
        title: enabled ? t("companySettings.feedbackEnabled") : t("companySettings.feedbackDisabledToast"),
        tone: "success",
      });
    },
    onError: (err) => {
      pushToast({
        title: t("companySettings.feedbackUpdateFailed"),
                    ...
                    <span className="text-xs text-destructive">
                      {logoUploadError ??
                        (logoUploadMutation.error instanceof Error
                          ...
                          : t("companySettings.uploadFailed"))}
                    </span>
                  )}
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          <div className="flex items-center gap-1.5">
            <span className="text-xs text-muted-foreground">
              {t("companySettings.generateHint")}
            </span>
            <HintIcon text={t("companySettings.generateHintTooltip")} />
          </div>

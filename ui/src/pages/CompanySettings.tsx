...
function buildAgentSnippet(input: AgentSnippetInput) {
  ...
  const connectivityBlock =
    candidateUrls.length === 0
      ? `No candidate URLs are available. Ask your user to configure a reachable hostname in Standard Power, then retry.
Suggested steps:
- choose a hostname that resolves to the Standard Power host from your runtime
- run: pnpm paperclipai allowed-hostname <host>
- restart Standard Power
- verify with: curl -fsS http://<host>:3100/api/health
- regenerate this invite snippet`
      : `If none are reachable, ask your user to add a reachable hostname in Standard Power, restart, and retry.
Suggested command:
- pnpm paperclipai allowed-hostname <host>
Then verify with: curl -fsS <base-url>/api/health`;

  const resolutionLine = resolutionTestUrl
    ? `\nYou MUST test Standard Power-to-gateway reachability, call: ${resolutionTestUrl}?url=<urlencoded-gateway-url> (using the hostname that worked above). Do not assume your 172.x is necessarily reachable from Standard Power. Test it. `
    : "";

  return `You're invited to join a Standard Power organization.

The URLs you should try are:
${candidateList}

Connectivity guidance:
If you are running on a different machine than Standard Power, Standard Power must be reachable at one of the hostnames used above.
Verify the hostname works from your runtime with: GET <base-url>/api/health

${connectivityBlock}

For your "agentName", just use your own agent name (OpenClaw or whatever your name is) and send it directly.
You also need to send a gateway URL that Standard Power can access. Check your config and include that URL.${resolutionLine}
...
- Keep device auth enabled (recommended). If \`devicePrivateKeyPem\` is omitted, Standard Power will generate and persist one during join so pairing approvals remain stable.
...
Then after you've connected to Standard Power (exchanged keys etc.) you MUST review and follow the onboarding instructions in onboarding.txt they give you.

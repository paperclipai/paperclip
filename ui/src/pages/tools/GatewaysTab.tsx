...
function formatAllowedTools(profile: ToolProfileWithDetails | undefined) {
  ...
  return allowed === 0 ? t("app.gatewaysTab.noToolsAllowed", { defaultValue: "No tools allowed" }) : `${pluralize(allowed, "tool")} allowed`;
}
...
export function GatewaysTab({ companyId }: { companyId: string }) {
  ...
  const createGatewayMutation = useMutation({
    ...
    onSuccess: async (gateway) => {
      ...
      setCreating(false);
      pushToast({ title: t("app.gatewaysTab.gatewayCreated", { defaultValue: "Gateway created" }), body: gateway.name, tone: "success" });
      await invalidateGateways();
    },
    onError: (error) => {
      pushToast({ title: t("app.gatewaysTab.gatewayWasNotCreated", { defaultValue: "Gateway was not created" }), body: error instanceof Error ? error.message : String(error), tone: "error" });
    },
  });
  ...
  const createTokenMutation = useMutation({
    ...
    onSuccess: async (token) => {
      ...
      pushToast({ title: t("app.gatewaysTab.tokenIssued", { defaultValue: "Token issued" }), body: `${token.name} was created. Copy it now; it will not be shown again.`, tone: "success" });
      await invalidateGateways();
    },
    onError: (error) => {
      pushToast({ title: t("app.gatewaysTab.tokenWasNotIssued", { defaultValue: "Token was not issued" }), body: error instanceof Error ? error.message : String(error), tone: "error" });
    },
  });
  ...
  const revokeTokenMutation = useMutation({
    ...
    onSuccess: async (token) => {
      ...
      pushToast({ title: t("app.gatewaysTab.tokenRevoked", { defaultValue: "Token revoked" }), body: token.name, tone: "success" });
      await invalidateGateways();
    },
    onError: (error) => {
      pushToast({ title: t("app.gatewaysTab.tokenWasNotRevoked", { defaultValue: "Token was not revoked" }), body: error instanceof Error ? error.message : String(error), tone: "error" });
    },
  });
  ...
  async function copyText(value: string, label: string) {
    try {
      ...
      pushToast({ title: t("app.gatewaysTab.copiedToClipboard", { defaultValue: "Copied to clipboard" }), body: label, tone: "success" });
    } catch (error) {
      pushToast({ title: t("app.gatewaysTab.copyFailed", { defaultValue: "Copy failed" }), body: error instanceof Error ? error.message : t("app.gatewaysTab.clipboardAccessIsUnavailable", { defaultValue: "Clipboard access is unavailable." }), tone: "error" });
    }
  }
  ...
  function submitCreateGateway(event: FormEvent<HTMLFormElement>) {
    ...
    if (!createDraft.profileId) {
      pushToast({ title: t("app.gatewaysTab.pickAProfile", { defaultValue: "Pick a profile" }), body: "A gateway needs an access profile before it can be created.", tone: "warn" });
      return;
    }
  ...
  function submitCreateToken(event: FormEvent<HTMLFormElement>, gatewayId: string) {
    ...
    if (draft.allowedActions.length === 0) {
      pushToast({ title: t("app.gatewaysTab.pickTokenActions", { defaultValue: "Pick token actions" }), body: "Gateway tokens need at least one allowed MCP action.", tone: "warn" });
      return;
    }
    createTokenMutation.mutate(gatewayId);
  }

  if (gatewaysQuery.isLoading) return <LoadingState label={t("app.gatewaysTab.loadingGateways", { defaultValue: "Loading gateways..." })} />;
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <ToolsPageHeader
          title={t("app.gatewaysTab.namedMcpGateways", { defaultValue: "Named MCP gateways" })}
          description={t("app.gatewaysTab.stableEndpointsForExternalClientsThatUseTheSameProfilesRulesAndAuditTrailAsAgentToolAccess", { defaultValue: "Stable endpoints for external clients that use the same profiles, rules, and audit trail as agent tool access." })}
        />
        <Button
          type="button"
          size="sm"
        ...
        >
          ...
          {t("app.gatewaysTab.createGateway", { defaultValue: "Create gateway" })}</Button>
      </div>
        ...
        <form className="space-y-3 rounded-md border border-border p-4" onSubmit={submitCreateGateway}>
          <div className="grid gap-3 md:grid-cols-(--gtc-60)">
            <label className="space-y-1.5 text-sm">
              <span className="text-xs font-medium text-muted-foreground">{t("app.gatewaysTab.gatewayName", { defaultValue: "Gateway name" })}</span>
              <input
                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                value={createDraft.name}
                onChange={(event) => setCreateDraft((current) => ({ ...current, name: event.target.value }))}
                placeholder={t("app.gatewaysTab.engineeringLaptops", { defaultValue: "Engineering laptops" })}
                required
              />
            </label>
            <label className="space-y-1.5 text-sm">
              <span className="text-xs font-medium text-muted-foreground">{t("app.gatewaysTab.accessProfile", { defaultValue: "Access profile" })}</span>
              <select
                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
              ...
              >
                <option value="" disabled>
                  {profileLoading ? t("app.gatewaysTab.loadingProfiles", { defaultValue: "Loading profiles..." }) : t("app.gatewaysTab.chooseAProfile", { defaultValue: "Choose a profile" })}
                </option>
                {activeProfiles.map((profile) => (
                  <option key={profile.id} value={profile.id}>
                    {profile.name} {t("app.gatewaysTab.text", { defaultValue: "- " })}{formatAllowedTools(profile)}
                  </option>
                ))}
              </select>
            </label>
          ...
          <label className="space-y-1.5 text-sm">
            <span className="text-xs font-medium text-muted-foreground">{t("app.gatewaysTab.description", { defaultValue: "Description" })}</span>
            <textarea
              className="min-h-20 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
              value={createDraft.description}
              onChange={(event) => setCreateDraft((current) => ({ ...current, description: event.target.value }))}
              placeholder={t("app.gatewaysTab.whoThisEndpointIsForAndWhenItShouldBeRotated", { defaultValue: "Who this endpoint is for and when it should be rotated." })}
            />
          </label>
          {activeProfiles.length === 0 && !profileLoading ? (
            <p className="text-xs text-muted-foreground">{t("app.gatewaysTab.createAnAccessProfileBeforeAddingAGateway", { defaultValue: "Create an access profile before adding a gateway." })}</p>
          ) : null}
          <div className="flex flex-wrap justify-end gap-2">
            ...
            <Button type="submit" size="sm" disabled={createDisabled || !createDraft.name.trim() || !createDraft.profileId}>
              {createGatewayMutation.isPending ? t("app.gatewaysTab.creating", { defaultValue: "Creating..." }) : t("app.gatewaysTab.createGateway", { defaultValue: "Create gateway" })}
            </Button>
          </div>
        </form>
      ) : null}
      ...
      {gateways.length === 0 ? (
        <div className="rounded-md border border-dashed border-border p-5 text-sm text-muted-foreground">
          {t("app.gatewaysTab.noNamedGatewaysYetCreateOneHereThenIssueATokenForTheClientThatWillConnectToIt", { defaultValue: "No named gateways yet. Create one here, then issue a token for the client that will connect to it." })}</div>
      ) : (
            ...
            return (
              <section key={gateway.id} className="space-y-4 p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button type="button" variant="outline" size="sm" onClick={() => void copyText(endpoint, "Gateway endpoint")}>
                      ...
                      {t("app.gatewaysTab.copyEndpoint", { defaultValue: "Copy endpoint" })}</Button>
                    <Button type="button" variant="outline" size="sm" onClick={() => startIssuing(gateway.id)}>
                      <KeyRound className="mr-1.5 h-3.5 w-3.5" />
                      {t("app.gatewaysTab.issueToken", { defaultValue: "Issue token" })}</Button>
                  </div>
                </div>
                ...
                <dl className="grid gap-x-4 gap-y-2 text-sm sm:grid-cols-2 lg:grid-cols-4">
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("app.gatewaysTab.owner", { defaultValue: "Owner" })}</dt>
                    <dd className="mt-0.5 text-foreground">{formatOwner(gateway, agentNames)}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("app.gatewaysTab.scope", { defaultValue: "Scope" })}</dt>
                    <dd className="mt-0.5 text-foreground">{formatScope(gateway, projectNames, agentNames)}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("app.gatewaysTab.allowedTools", { defaultValue: "Allowed tools" })}</dt>
                    <dd className="mt-0.5 text-foreground">
                      {profile ? `${formatAllowedTools(profile)} via ${profile.name}` : `Profile ${shortId(gateway.profileId)}`}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("app.gatewaysTab.lastActivity", { defaultValue: "Last activity" })}</dt>
                    <dd className="mt-0.5 text-foreground">
                      {lastActivity ? <RelativeTime value={lastActivity} /> : t("app.gatewaysTab.neverUsed", { defaultValue: "Never used" })}
                    </dd>
                  </div>
                </dl>
                ...
                {issuingGatewayId === gateway.id ? (
                  <form className="space-y-3 rounded-md border border-border p-3" onSubmit={(event) => submitCreateToken(event, gateway.id)}>
                    <div className="grid gap-3 md:grid-cols-2">
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("app.gatewaysTab.tokenName", { defaultValue: "Token name" })}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          ...
                          placeholder={t("app.gatewaysTab.dottaSMacbook", { defaultValue: "Dotta's MacBook" })}
                          required
                        />
                      </label>
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("app.gatewaysTab.clientLabel", { defaultValue: "Client label" })}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          ...
                          placeholder={t("app.gatewaysTab.cursorOnWorkLaptop", { defaultValue: "Cursor on work laptop" })}
                          required
                        />
                      </label>
                    </div>
                    <div className="grid gap-3 md:grid-cols-[1fr_auto]">
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("app.gatewaysTab.ownerNote", { defaultValue: "Owner note" })}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          ...
                          placeholder={t("app.gatewaysTab.whoOwnsThisTokenAndWhyItExists", { defaultValue: "Who owns this token and why it exists" })}
                          required
                        />
                      </label>
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("app.gatewaysTab.expires", { defaultValue: "Expires" })}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          type="date"
                    ...
                    <div className="flex flex-wrap justify-end gap-2">
                      ...
                      >
                        {createTokenMutation.isPending ? t("app.gatewaysTab.issuing", { defaultValue: "Issuing..." }) : t("app.gatewaysTab.issueToken", { defaultValue: "Issue token" })}
                      </Button>
                    </div>
                  </form>
                ) : null}
                ...
                {createdToken ? (
                  <div className="space-y-2 rounded-md border border-border bg-muted/40 p-3 text-sm">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="font-medium text-foreground">{t("app.gatewaysTab.newTokenFor", { defaultValue: "New token for " })}{createdToken.name}</div>
                      <Button type="button" variant="outline" size="sm" onClick={() => void copyText(createdToken.token, "Gateway bearer token")}>
                        <Copy className="mr-1.5 h-3.5 w-3.5" />
                        {t("app.gatewaysTab.copyToken", { defaultValue: "Copy token" })}</Button>
                    </div>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div>
                    <div className="mb-1.5 flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
                      ...
                      {t("app.gatewaysTab.tokens", { defaultValue: "Tokens" })}</div>
                    <div className="space-y-1 text-sm">
                      {gateway.tokens.length === 0 ? (
                        <p className="text-muted-foreground">{t("app.gatewaysTab.noTokensIssued", { defaultValue: "No tokens issued." })}</p>
                      ) : (
                              ...
                              <div className="flex items-center justify-between gap-3">
                                <div className="min-w-0">
                                  ...
                                  <div className="text-xs text-muted-foreground">
                                    {token.clientLabel || token.tokenPrefix} {t("app.gatewaysTab.text2", { defaultValue: "· " })}{token.allowedActions.join(", ")}
                                  </div>
                                </div>
                                <div className="flex shrink-0 items-center gap-2">
                                  <span className="text-xs text-muted-foreground">
                                    {token.revokedAt ? (
                                      <>
                                        {t("app.gatewaysTab.revoked", { defaultValue: "revoked " })}<RelativeTime value={token.revokedAt} />
                                      </>
                                    ) : token.expiresAt ? (
                                      <>
                                        {t("app.gatewaysTab.expires2", { defaultValue: "expires " })}<RelativeTime value={token.expiresAt} />
                                      </>
                                    ) : (
                                      "no expiry"
                                    )}
                                  ...
                                  {!revoked ? (
                                    ...
                                    >
                                      ...
                                      {t("app.gatewaysTab.revoke", { defaultValue: "Revoke" })}</Button>
                                  ) : null}
                                </div>
                              </div>
                              {confirming ? (
                                <div className="flex flex-wrap items-center justify-end gap-2 text-xs text-muted-foreground">
                                  <span>{t("app.gatewaysTab.revokeThisTokenNow", { defaultValue: "Revoke this token now?" })}</span>
                                  <Button
                                    type="button"
                                    variant="ghost"
                                    size="sm"
                  ...
                  <div>
                    <div className="mb-1.5 text-xs font-medium text-muted-foreground">{t("app.gatewaysTab.clientSnippets", { defaultValue: "Client snippets" })}</div>
                    <div className="space-y-1 text-sm">
                      {snippets.length === 0 ? (
                        <p className="text-muted-foreground">{t("app.gatewaysTab.noSnippetsAvailable", { defaultValue: "No snippets available." })}</p>
                      ) : (

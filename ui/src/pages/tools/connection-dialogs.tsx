  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t("connection-dialogs.tool-catalog-1ld")} {connection.name}</DialogTitle>
        </DialogHeader>
        {catalog.isLoading ? (
          <LoadingState />
        ) : catalog.error ? (
          ...
          <p className="py-6 text-sm text-muted-foreground">
            {t("connection-dialogs.no-tools-discovered-yet-use-refresh-icw")}
          </p>
        ) : (
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>{t("connection-dialogs.add-application-1uq")}</DialogTitle>
          <DialogDescription>
            {t("connection-dialogs.choose-an-existing-application-or-cr-pnz")}
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-3">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <span className={step === 1 ? "font-medium text-foreground" : ""}>{t("connection-dialogs.1-application-1uw")}</span>
            <span>/</span>
            <span className={step === 2 ? "font-medium text-foreground" : ""}>{t("connection-dialogs.2-connection-1ge")}</span>
          </div>

          {step === 1 && !locked ? (
            <>
              <div className="space-y-1.5">
                <Label>{t("connection-dialogs.application-9mg")}</Label>
                <Select value={applicationMode} onValueChange={(v) => setApplicationMode(v as "existing" | "new")}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="existing">{t("connection-dialogs.use-existing-application-1mj")}</SelectItem>
                    <SelectItem value="new">{t("connection-dialogs.create-new-application-5g0")}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              {applicationMode === "existing" ? (
                <div className="space-y-1.5">
                  <Label>{t("connection-dialogs.existing-application-119")}</Label>
                  <Select value={applicationId} onValueChange={setApplicationId}>
                    <SelectTrigger>
                      <SelectValue placeholder={t("connection-dialogs.select-an-application-1qu")} />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="app-name">{t("connection-dialogs.new-application-name-179")}</Label>
                  <Input
                    id="app-name"
                    value={applicationName}
                    onChange={(e) => setApplicationName(e.target.value)}
                    placeholder={t("connection-dialogs.e-g-git-hub-triage-1ox")}
                  />
                  <p className="text-xs text-muted-foreground">
                    {t("connection-dialogs.application-type-is-inferred-from-th-dng")}
                  </p>
                </div>
              )}
            </>
            ...
            <>
              {applicationMode === "new" ? (
                <div className="rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
                  <span className="font-medium text-foreground">{applicationName.trim()}</span> {t("connection-dialogs.will-be-created-as-t6k")}{" "}
                  {inferredType}.
                </div>
              ) : null}

              <div className="space-y-1.5">
                <Label htmlFor="conn-name">{t("connection-dialogs.connection-name-cau")}</Label>
                <Input
                  id="conn-name"
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder={t("connection-dialogs.e-g-production-git-hub-15s")}
                  disabled={locked}
                />
              </div>

              <div className="space-y-1.5">
                <Label>{t("connection-dialogs.transport-113")}</Label>
                <Select
                  value={transport}
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="mcp_remote">{t("connection-dialogs.remote-http-no-local-process-1sc")}</SelectItem>
                    <SelectItem value="local_stdio">{t("connection-dialogs.local-stdio-approved-template-27z")}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              {transport === "mcp_remote" ? (
                <div className="space-y-1.5">
                  <Label htmlFor="conn-url">{t("connection-dialogs.endpoint-url-15v")}</Label>
                  <Input
                    id="conn-url"
                    value={endpointUrl}
                ...
                <div className="space-y-1.5">
                  <Label>{t("connection-dialogs.command-template-pgx")}</Label>
                  <Select value={templateId} onValueChange={setTemplateId} disabled={locked}>
                    <SelectTrigger>
                      <SelectValue placeholder={t("connection-dialogs.select-an-approved-template-19a")} />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("connection-dialogs.only-board-approved-command-template-t0x")}
                  </p>
                </div>
              )}
              ...
              <div className="space-y-1.5">
                <Label>{t("connection-dialogs.credential-references-l4h")}</Label>
                {creds.length > 0 ? (
                ...
                {!locked ? (
                  <>
                    <div className="flex items-end gap-2">
                      <div className="flex-1 space-y-1">
                        <Select value={pendingSecretId} onValueChange={setPendingSecretId}>
                          <SelectTrigger>
                            <SelectValue placeholder={t("connection-dialogs.select-a-vault-secret-8yg")} />
                          </SelectTrigger>
                          <SelectContent>
                            {(secrets.data ?? []).map((s) => (
                      ...
                      <Input
                        ...
                        placeholder={t("connection-dialogs.header-4sw")}
                        className="w-32"
                        aria-label={t("connection-dialogs.header-name-8vz")}
                      />
                      <Button type="button" size="sm" variant="outline" onClick={addCred} disabled={!pendingSecretId}>
                        {t("connection-dialogs.add-17r")}
                      </Button>
                    </div>
                    {pendingSecretId ? (
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("connection-dialogs.free-text-secrets-are-not-accepted-p-1i2")}
                      <span className="font-mono"> vault://</span> {t("connection-dialogs.reference-and-resolves-it-at-gateway-1cy")}
                    </p>
                  </>
                ) : null}
              </div>
          ...
          {locked ? (
            probe.isPending ? (
              <div className="rounded-md border border-border bg-muted/40 p-3">
                <LoadingState label={t("connection-dialogs.probing-connection-uia")} />
              </div>
            ) : probe.isError ? (
              ...
              <div className="rounded-md border border-border bg-muted/40 p-3">
                <div className="flex items-center gap-2 text-sm">
                  <span className="font-medium text-foreground">{t("connection-dialogs.probe-result-y2m")}</span>
                ...
                <div className="mt-2 grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{t("connection-dialogs.tools-discovered-x84")}</p>
                  </div>
                  <div>
                    <p className="text-lg font-semibold tabular-nums text-foreground">
                      {probeResult.latencyMs != null ? `${probeResult.latencyMs}ms` : "—"}
                    </p>
                    <p className="text-xs text-muted-foreground">{t("connection-dialogs.probe-latency-14l")}</p>
                  </div>
                  <div>
                    <p className="text-lg font-semibold tabular-nums text-foreground">
                ...
                <p className="mt-2 text-(length:--text-micro) text-muted-foreground">
                  {t("connection-dialogs.probe-latency-is-a-single-round-trip-1ie")}
                </p>
              </div>
            ) : null
          ) : null}
        ...
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>
            {t("connection-dialogs.cancel-ew9")}
          </Button>
          {step === 1 && !locked ? (
            <Button disabled={!appChoiceValid} onClick={() => setStep(2)}>
              {t("connection-dialogs.continue-1bi")}
            </Button>
          ) : !locked ? (
            <>
              <Button variant="outline" onClick={() => setStep(1)}>
                {t("connection-dialogs.back-1hz")}
              </Button>

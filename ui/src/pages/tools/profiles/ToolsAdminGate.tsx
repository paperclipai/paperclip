...
export function ToolsAdminGate({ children }: { children: ReactNode }) {
  ...
  if (boardAccess.isLoading) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">{t("tools-admin-gate.loading-cwe")}</div>;
  }
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="flex flex-col gap-3 rounded-lg border border-border bg-card p-6">
          <div className="flex items-center gap-2 text-foreground">
            ...
            <h1 className="text-lg font-semibold">{t("tools-admin-gate.access-profiles-are-for-administrato-1ym")}</h1>
          </div>
          <p className="text-sm text-muted-foreground">
            {t("tools-admin-gate.access-profiles-decide-which-tools-y-w07")}{" "}
            <Link to="/apps" className="font-medium text-primary hover:underline">
              {t("tools-admin-gate.your-apps-vw4")}
            </Link>
            .
          </p>
        </div>

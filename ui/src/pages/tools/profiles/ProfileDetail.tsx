...
export function ProfileDetail({
  ...
  const updateProfile = useMutation({
    ...
    onSuccess: () => {
      invalidate();
      pushToast({ title: t("app.profileDetail.profileUpdated", { defaultValue: "Profile updated" }), tone: "success" });
    },
    onError: (error: unknown) => pushToast({ title: t("app.profileDetail.couldNotUpdateProfile", { defaultValue: "Could not update profile" }), body: errorBody(error), tone: "error" }),
  });
  ...
  const duplicateProfile = useMutation({
    ...
    onSuccess: (copy) => {
      invalidate();
      pushToast({ title: t("app.profileDetail.profileDuplicated", { defaultValue: "Profile duplicated" }), body: "The copy is not assigned to anyone yet.", tone: "success" });
      navigate(`/apps/advanced/profiles/${copy.id}?created=1`);
    },
    onError: (error: unknown) => pushToast({ title: t("app.profileDetail.couldNotDuplicate", { defaultValue: "Could not duplicate" }), body: errorBody(error), tone: "error" }),
  });
  ...
  const deleteProfile = useMutation({
    ...
    onSuccess: () => {
      invalidate();
      pushToast({ title: t("app.profileDetail.profileDeleted", { defaultValue: "Profile deleted" }), tone: "success" });
      navigate("/apps/advanced/profiles");
    },
    onError: (error: unknown) => pushToast({ title: t("app.profileDetail.couldNotDelete", { defaultValue: "Could not delete" }), body: errorBody(error), tone: "error" }),
  });
  ...
  const removeAssignment = useMutation({
    ...
    onSuccess: () => {
      ...
      invalidate();
      pushToast({ title: t("app.profileDetail.assignmentRemoved", { defaultValue: "Assignment removed" }), tone: "success" });
    },
    onError: (error: unknown) => pushToast({ title: t("app.profileDetail.couldNotRemoveAssignment", { defaultValue: "Could not remove assignment" }), body: errorBody(error), tone: "error" }),
  });
  ...
  const reviewNewTools = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({ title: t("app.profileDetail.newToolsReviewed", { defaultValue: "New tools reviewed" }), tone: "success" });
    },
    onError: (error: unknown) => pushToast({ title: t("app.profileDetail.couldNotSubmitReview", { defaultValue: "Could not submit review" }), body: errorBody(error), tone: "error" }),
  });

  if (data.profiles.isLoading) return <LoadingState label={t("app.profileDetail.loadingProfile", { defaultValue: "Loading profile..." })} />;
  if (data.profiles.isError) return <ErrorState error={data.profiles.error} onRetry={() => data.profiles.refetch()} />;
  if (!profile) {
    return (
      <div className="space-y-4">
        <ToolsPageHeader title={t("app.profileDetail.profileNotFound", { defaultValue: "Profile not found" })} description={t("app.profileDetail.thisAccessProfileMayHaveBeenDeleted", { defaultValue: "This access profile may have been deleted." })} />
        <Button variant="outline" onClick={() => navigate("/apps/advanced/profiles")}>{t("app.profileDetail.backToProfiles", { defaultValue: "Back to profiles" })}</Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <ToolsPageHeader
        title={profile.name}
        description={profile.description ?? t("app.profileDetail.noDescriptionYet", { defaultValue: "No description yet." })}
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" disabled={archived} onClick={() => setDialog("duplicate")}>
              ...
              {t("app.profileDetail.duplicate", { defaultValue: "Duplicate" })}</Button>
            {archived ? (
              ...
              <Button variant="outline" onClick={() => setDialog("archive")}>{t("common.archive", { defaultValue: "Archive" })}</Button>
            )}
      ...
      <div className="flex flex-wrap items-center gap-3 text-sm">
        ...
        <span className="text-muted-foreground">{t("app.profileDetail.updated", { defaultValue: "Updated " })}<RelativeTime value={profile.updatedAt} /></span>
      ...
      {created ? (
        <div className="flex items-center justify-between gap-3 rounded-lg border border-primary/30 bg-primary/5 px-4 py-3">
          <div>
            <p className="text-sm font-medium text-foreground">{t("app.profileDetail.profileSaved", { defaultValue: "Profile saved" })}</p>
            <p className="text-sm text-muted-foreground">
              {unassigned ? t("app.profileDetail.assignItToAgentsBeforeItChangesTheirAccess", { defaultValue: "Assign it to agents before it changes their access." }) : t("app.profileDetail.assignmentsAreActiveNow", { defaultValue: "Assignments are active now." })}
            </p>
          </div>
          <div className="flex gap-2">
            <Button size="sm" onClick={() => navigate(`/apps/advanced/profiles/${profile.id}/edit?step=3`)}>
              {t("app.profileDetail.assign", { defaultValue: "Assign" })}</Button>
            <Button size="sm" variant="ghost" onClick={() => setSearchParams({})}>
              {t("app.profileDetail.dismiss", { defaultValue: "Dismiss" })}</Button>
          </div>
        </div>
      ) : null}
      ...
      {archived ? (
        <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-950">
          {t("app.profileDetail.thisProfileIsArchivedItDoesNotApplyToAgentsUntilItIsRestored", { defaultValue: "This profile is archived. It does not apply to agents until it is restored." })}</div>
      ) : null}
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-base font-semibold text-foreground">{t("app.profileDetail.whatItAllows", { defaultValue: "What it allows" })}</h2>
          <Button variant="outline" size="sm" disabled={archived} onClick={() => navigate(`/apps/advanced/profiles/${profile.id}/edit?step=2`)}>
            {t("app.profileDetail.editTools", { defaultValue: "Edit tools" })}</Button>
        </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-base font-semibold text-foreground">{t("app.profileDetail.whoHasIt", { defaultValue: "Who has it" })}</h2>
      ...
      <section className="space-y-3">
        <h2 className="text-base font-semibold text-foreground">{t("app.profileDetail.newToolsThatAppearLater", { defaultValue: "New tools that appear later" })}</h2>
        <NewToolsSetting
      ...
      <Button variant="link" className="h-auto px-0" onClick={() => navigate("/apps/advanced/profiles?check=1")}>
        ...
        {t("app.profileDetail.checkWhatAnAgentCanActuallyDo", { defaultValue: "Check what an agent can actually do" })}</Button>
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-950">
      <div>
        <p className="font-medium">
          {loading ? t("app.profileDetail.newToolsNeedReview", { defaultValue: "New tools need review" }) : `${appLabel} added ${count} new ${count === 1 ? "tool" : "tools"} since your last review`}
        </p>
        <p className="text-amber-900/80">{t("app.profileDetail.chooseWhichOnesThisProfileShouldAllow", { defaultValue: "Choose which ones this profile should allow." })}</p>
      </div>
      <Button size="sm" onClick={onReview}>{t("app.profileDetail.review", { defaultValue: "Review" })}</Button>
    </div>
  );
}
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t("app.profileDetail.reviewNewTools", { defaultValue: "Review new tools" })}</DialogTitle>
          <DialogDescription>
            {t("app.profileDetail.allowTheToolsThisProfileShouldUseKeepTheRestBlocked", { defaultValue: "Allow the tools this profile should use. Keep the rest blocked." })}</DialogDescription>
        </DialogHeader>
        {loading ? (
          <LoadingState label={t("app.profileDetail.loadingNewTools", { defaultValue: "Loading new tools..." })} />
        ) : error ? (
          <ErrorState error={error} onRetry={onRetry} />
        ) : tools.length === 0 ? (
          <div className="rounded-lg border border-dashed border-border px-4 py-6 text-sm text-muted-foreground">
            {t("app.profileDetail.thereAreNoNewToolsWaitingForReview", { defaultValue: "There are no new tools waiting for review." })}</div>
        ) : (
              ...
              <div key={tool.catalogEntryId} className="grid gap-3 px-3 py-3 sm:grid-cols-[1fr_auto]">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {tool.applicationName ?? tool.connectionName ?? t("app.profileDetail.appTool", { defaultValue: "App tool" })} {t("app.profileDetail.added", { defaultValue: "· added " })}{formatShortDate(tool.addedAt)}
                  </p>
                </div>
                <div className="flex items-center gap-2 sm:justify-end">
                  <label className="inline-flex items-center gap-1.5 text-sm">
                    ...
                    />
                    {t("app.profileDetail.allow", { defaultValue: "Allow" })}</label>
                  <label className="inline-flex items-center gap-1.5 text-sm">
                    <input
                      type="radio"
                      name={`review-${tool.catalogEntryId}`}
                    ...
                    />
                    {t("app.profileDetail.keepBlocked", { defaultValue: "Keep blocked" })}</label>
                </div>
              </div>
            ))}
          </div>
        ...
        <DialogFooter>
          ...
          <Button disabled={pending || loading || tools.length === 0} onClick={onSubmit}>
            {t("app.profileDetail.submitReview", { defaultValue: "Submit review" })}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function AllowList({ rows, total }: { rows: AllowRow[]; total: number }) {
  if (rows.length === 0) {
    return (
      <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-950">
        {t("app.profileDetail.thisProfileAllows0ToolsAgentsWithOnlyThisProfileWillNotBeAbleToUseAppTools", { defaultValue: "This profile allows 0 tools. Agents with only this profile will not be able to use app tools." })}</div>
    );
  }
  return (
    <div className="overflow-hidden rounded-lg border border-border">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border bg-muted/40 text-left text-xs font-medium text-muted-foreground">
            <th className="px-3 py-2 font-medium">{t("app.profileDetail.tool", { defaultValue: "Tool" })}</th>
            <th className="px-3 py-2 font-medium">{t("app.profileDetail.app", { defaultValue: "App" })}</th>
            <th className="px-3 py-2 font-medium">{t("app.profileDetail.capabilities", { defaultValue: "Capabilities" })}</th>
            <th className="px-3 py-2 font-medium">{t("app.profileDetail.source", { defaultValue: "Source" })}</th>
          </tr>
        </thead>
        <tbody>
                ...
                <div className="flex flex-col">
                  ...
                  {row.degraded ? (
                    <a className="mt-0.5 inline-flex items-center gap-1 text-xs font-medium text-primary hover:underline" href={`/apps/${row.connectionId}`}>
                      ...
                      {t("app.profileDetail.reconnect", { defaultValue: "Reconnect" })}</a>
                  ) : null}
                </div>
              </td>
              <td className="px-3 py-2">
                <span>{row.app}</span>
                {row.degraded ? <span className="ml-2 text-xs text-muted-foreground">{row.app} {t("app.profileDetail.isDisconnected", { defaultValue: "is disconnected" })}</span> : null}
              </td>
              ...
              <td className="px-3 py-2">
                ...
                {row.autoAddedAt ? (
                  <div className="mt-0.5 text-xs text-muted-foreground">
                    {t("app.profileDetail.addedAutomatically", { defaultValue: "added automatically · " })}{formatShortDate(row.autoAddedAt)}
                  </div>
                ) : null}
              </td>
            </tr>
      ...
      {rows.length > 80 ? (
        <p className="border-t border-border px-3 py-2 text-xs text-muted-foreground">
          {t("app.profileDetail.showing80Of", { defaultValue: "Showing 80 of " })}{rows.length} {t("app.profileDetail.allowedTools", { defaultValue: "allowed tools." })}</p>
      ) : (
        <p className="border-t border-border px-3 py-2 text-xs text-muted-foreground">
          {t("app.profileDetail.allows", { defaultValue: "Allows " })}{rows.length} {t("app.profileDetail.of", { defaultValue: "of " })}{total} {t("app.profileDetail.knownTools", { defaultValue: "known tools." })}</p>
      )}
    </div>
  );
}
  ...
  if (profile.bindings.length === 0) {
    return (
      <div className="rounded-lg border border-dashed border-border px-4 py-5">
        <p className="text-sm font-medium text-foreground">{t("app.profileDetail.notAssignedYet", { defaultValue: "Not assigned yet" })}</p>
        <p className="text-sm text-muted-foreground">{t("app.profileDetail.assignThisProfileBeforeItChangesAccess", { defaultValue: "Assign this profile before it changes access." })}</p>
      </div>
    );
  }
  return (
        ...
        <div key={binding.id} className="flex items-center justify-between gap-3 px-3 py-2.5">
          <div className="flex min-w-0 items-center gap-3">
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-muted text-xs font-semibold text-muted-foreground">
              {binding.targetType === "company" ? t("app.profileDetail.co", { defaultValue: "Co" }) : binding.targetType.slice(0, 2).toUpperCase()}
            </span>
            <div className="min-w-0">
...
function NewToolsSetting({
  ...
  const options: Array<{ value: ToolProfileDefaultAction; title: string; body: string }> = [
    { value: "deny", title: t("app.profileDetail.stayBlockedUntilReviewed", { defaultValue: "Stay blocked until reviewed" }), body: "New tools do not become available automatically." },
    { value: "allow", title: t("app.profileDetail.allowedAutomatically", { defaultValue: "Allowed automatically" }), body: "New tools from selected apps become available right away." },
  ];
  return (
  ...
  if (kind === "edit") {
    return (
      <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("app.profileDetail.editProfile", { defaultValue: "Edit profile" })}</DialogTitle>
            <DialogDescription>{t("app.profileDetail.updateTheProfileNameAndDescription", { defaultValue: "Update the profile name and description." })}</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1.5">
              <Label htmlFor="edit-profile-name">{t("app.profileDetail.name", { defaultValue: "Name" })}</Label>
              <Input id="edit-profile-name" value={name} onChange={(e) => setName(e.target.value)} />
              {duplicateName ? <p className="text-xs text-destructive">{t("app.profileDetail.anotherProfileAlreadyUsesThisName", { defaultValue: "Another profile already uses this name." })}</p> : null}
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="edit-profile-description">{t("app.profileDetail.description", { defaultValue: "Description" })}</Label>
            ...
            <button type="button" className="text-sm font-medium text-muted-foreground hover:text-foreground" onClick={() => setAdvancedOpen((v) => !v)}>
              {t("app.profileDetail.advanced", { defaultValue: "Advanced" })}</button>
            {advancedOpen ? (
              <div className="space-y-1.5">
                <Label htmlFor="edit-profile-key">{t("app.profileDetail.identifier", { defaultValue: "Identifier" })}</Label>
  ...
  if (kind === "duplicate") {
    return (
      <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("app.profileDetail.duplicateProfile", { defaultValue: "Duplicate profile" })}</DialogTitle>
            <DialogDescription>{t("app.profileDetail.theCopyStartsUnassignedUnlessYouChooseToCopyAssignmentsToo", { defaultValue: "The copy starts unassigned unless you choose to copy assignments too." })}</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1.5">
              <Label htmlFor="copy-profile-name">{t("app.profileDetail.name", { defaultValue: "Name" })}</Label>
              <Input id="copy-profile-name" value={copyName} onChange={(e) => setCopyName(e.target.value)} />
              {duplicateCopyName ? <p className="text-xs text-destructive">{t("app.profileDetail.anotherProfileAlreadyUsesThisName", { defaultValue: "Another profile already uses this name." })}</p> : null}
            </div>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={copyAssignments} onChange={(e) => setCopyAssignments(e.target.checked)} />
              {t("app.profileDetail.alsoCopyAssignments", { defaultValue: "Also copy assignments?" })}</label>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={onClose}>{t("common.cancel", { defaultValue: "Cancel" })}</Button>
...
function RemoveAssignmentDialog({
  ...
  return (
    <Dialog open={Boolean(binding)} onOpenChange={(next) => !next && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t("app.profileDetail.removeAssignment", { defaultValue: "Remove assignment" })}</DialogTitle>
          <DialogDescription>
            {binding?.targetType === "company"
              ? t("app.profileDetail.removingTheCompanyDefaultChangesAccessForEveryAgentThatReliesOnIt", { defaultValue: "Removing the company default changes access for every agent that relies on it." })
              : `Remove this profile from ${label}.`}
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
          <Button variant="ghost" onClick={onClose}>{t("common.cancel", { defaultValue: "Cancel" })}</Button>
          <Button disabled={pending} onClick={onConfirm}>{t("common.remove", { defaultValue: "Remove" })}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function buildAllowRows(
  ...
  return catalog
    ...
    .map((tool) => {
      ...
      const app = appNames.get(tool.applicationId ?? "") ?? connectionNames.get(tool.connectionId) ?? t("app.profileDetail.unknownApp", { defaultValue: "Unknown app" });
...
function newToolsAppLabel(tools: ToolProfileNewToolReviewItem[]): string {
  ...
  if (names.length === 1) return names[0] ?? t("app.profileDetail.anApp", { defaultValue: "An app" });
...
function assignmentLabel(
  ...
  if (binding.targetType === "agent") return maps.agentsById.get(binding.targetId) ?? t("app.profileDetail.unknownAgent", { defaultValue: "Unknown agent" });
  if (binding.targetType === "project") return maps.projectsById.get(binding.targetId) ?? t("app.profileDetail.unknownProject", { defaultValue: "Unknown project" });
  if (binding.targetType === "routine") return maps.routinesById.get(binding.targetId) ?? t("app.profileDetail.unknownRoutine", { defaultValue: "Unknown routine" });

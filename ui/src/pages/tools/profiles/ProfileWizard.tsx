...
const STEP_LABELS: Array<{ step: WizardStep; label: string }> = [
  { step: 1, label: t("app.profileWizard.name", { defaultValue: "Name" }) },
  { step: 2, label: t("app.profileWizard.chooseTools", { defaultValue: "Choose tools" }) },
  { step: 3, label: t("app.profileWizard.assign", { defaultValue: "Assign" }) },
];
...
export function ProfileWizard({
  ...
  const saveDraft = useMutation({
    mutationFn: async (input: {
      ...
      if (!draftId) {
        const created = await toolsApi.createProfile(companyId, {
          ...
          name: name.trim() || t("app.profileWizard.untitledProfile", { defaultValue: "Untitled profile" }),
    ...
    onError: (error: unknown) =>
      pushToast({ title: t("app.profileWizard.couldNotSave", { defaultValue: "Could not save" }), body: String((error as Error)?.message ?? error), tone: "error" }),
  });
  ...
  const finish = useMutation({
    ...
    onSuccess: (profile) => {
      pushToast({ title: t("app.profileWizard.profileSaved", { defaultValue: "Profile saved" }), tone: "success" });
      invalidate();
      navigate(`/apps/advanced/profiles/${profile.id}${selectedAgentIds.size === 0 && !companyDefault ? "?created=1" : ""}`);
    },
    onError: (error: unknown) =>
      pushToast({ title: t("app.profileWizard.couldNotSaveProfile", { defaultValue: "Could not save profile" }), body: String((error as Error)?.message ?? error), tone: "error" }),
  });
  ...
  const saveAndExit = () => {
    ...
    saveDraft.mutate(
      ...
      {
        onSuccess: () => {
          pushToast({ title: t("app.profileWizard.draftSaved", { defaultValue: "Draft saved" }), body: "Pick it back up from the profiles list.", tone: "success" });
  ...
  if (profileId && profiles.isLoading) return <LoadingState label={t("app.profileWizard.loadingDraft", { defaultValue: "Loading draft…" })} />;
      ...
      <div className="fixed inset-x-0 bottom-0 z-10 border-t border-border bg-background/95 backdrop-blur">
        <div className="mx-auto flex w-full max-w-3xl items-center justify-between gap-3 px-4 py-3">
          <div className="flex items-center gap-3 text-sm text-muted-foreground">
            {step >= 2 ? (
              <span>
                {t("app.profileWizard.allows", { defaultValue: "Allows " })}<span className="font-medium text-foreground">{live.allowed}</span> {t("app.profileWizard.of", { defaultValue: "of " })}{live.total}{" "}
                tools
              </span>
            ) : null}
            {draftId ? (
              ...
              >
                {t("app.profileWizard.saveFinishLater", { defaultValue: "Save & finish later" })}</button>
            ) : null}
          </div>
          ...
          <div className="flex items-center gap-2">
            {step > 1 ? (
              <Button variant="outline" disabled={busy} onClick={() => setStep((s) => (s - 1) as WizardStep)}>
                {t("app.profileWizard.back", { defaultValue: "Back" })}</Button>
            ) : (
            ...
            {step === 3 ? (
              <Button disabled={busy} onClick={() => finish.mutate()}>
                {busy ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : null}
                {t("app.profileWizard.saveProfile", { defaultValue: "Save profile" })}</Button>
            ) : null}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        <h3 className="text-sm font-medium text-foreground">{t("app.profileWizard.startFrom", { defaultValue: "Start from" })}</h3>
      ...
      {template === "copy" ? (
        <div className="space-y-2">
          <h3 className="text-sm font-medium text-foreground">{t("app.profileWizard.whichProfile", { defaultValue: "Which profile?" })}</h3>
          {copyOptions.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("app.profileWizard.youDonTHaveAnotherProfileToCopyYet", { defaultValue: "You don't have another profile to copy yet." })}</p>
          ) : (
      ...
      <div className="space-y-3">
        <div className="space-y-1.5">
          <Label htmlFor="profile-name">{t("app.profileWizard.name", { defaultValue: "Name" })}</Label>
          <Input
            id="profile-name"
            value={name}
            onChange={(e) => onName(e.target.value)}
            placeholder={t("app.profileWizard.eGEverydayWork", { defaultValue: "e.g. Everyday work" })}
          />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="profile-description">{t("app.profileWizard.descriptionOptional", { defaultValue: "Description (optional)" })}</Label>
          <Textarea
            id="profile-description"
            value={description}
            onChange={(e) => onDescription(e.target.value)}
            placeholder={t("app.profileWizard.whatIsThisProfileFor", { defaultValue: "What is this profile for?" })}
            rows={2}
          />
        </div>
      </div>
      ...
      <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
        <CollapsibleTrigger className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
          ...
          {t("app.profileWizard.advanced", { defaultValue: "Advanced" })}</CollapsibleTrigger>
        <CollapsibleContent className="pt-2">
          <div className="space-y-1.5">
            <Label htmlFor="profile-key">{t("app.profileWizard.identifier", { defaultValue: "Identifier" })}</Label>
            <Input
              id="profile-key"
              value={profileKey}
            ...
            <p className="text-xs text-muted-foreground">
              {t("app.profileWizard.usedInExportsAndTheApiAutoFilledFromTheName", { defaultValue: "Used in exports and the API. Auto-filled from the name." })}</p>
          </div>
        </CollapsibleContent>
      </Collapsible>
    </div>
    ...
    <div className="space-y-5">
      <label className="flex items-start gap-3 rounded-lg border border-border p-4">
        ...
        <span className="flex flex-col gap-0.5">
          <span className="text-sm font-medium text-foreground">{t("app.profileWizard.makeThisTheCompanyDefault", { defaultValue: "Make this the company default" })}</span>
          <span className="text-xs text-muted-foreground">
            {t("app.profileWizard.everyAgentWithoutItsOwnProfileUsesThisOne", { defaultValue: "Every agent without its own profile uses this one." })}{defaultProfileName ? ` Replaces “${defaultProfileName}”.` : ""}
          </span>
        </span>
      </label>

      <div className="space-y-2">
        <h3 className="text-sm font-medium text-foreground">{t("app.profileWizard.assignToAgents", { defaultValue: "Assign to agents" })}</h3>
        <AgentMultiSelect
          agents={agents}
          selectedAgentIds={selectedAgentIds}
          ...
          getDescription={(agent) => {
            ...
            return bits.length > 0 ? `already has: ${bits.join(" · ")}` : t("app.profileWizard.noProfilesYet", { defaultValue: "no profiles yet" });
          }}
        />
        <p className="text-xs text-muted-foreground">
          {t("app.profileWizard.ifAnAgentHasSeveralProfilesItCanUseAnythingAnyOfThemAllows", { defaultValue: "If an agent has several profiles, it can use anything any of them allows." })}</p>
      </div>
      ...
      {(projects.length > 0 || routines.length > 0) && onToggleProject && onToggleRoutine ? (
        <Collapsible open={moreOpen} onOpenChange={setMoreOpen} className="rounded-lg border border-border">
          <CollapsibleTrigger className="flex w-full items-center justify-between px-4 py-3 text-left">
            <span className="text-sm font-medium text-foreground">{t("app.profileWizard.moreTargets", { defaultValue: "More targets" })}</span>
          ...
          <CollapsibleContent className="space-y-4 border-t border-border px-4 py-3">
            <p className="text-xs text-muted-foreground">
              {t("app.profileWizard.assignThisProfileToAWholeProjectOrAScheduledRoutineInsteadOfOrAsWellAsIndividualAgents", { defaultValue: "Assign this profile to a whole project or a scheduled routine instead of (or as well as) individual agents." })}</p>
            <TargetChecklist
              label={t("app.profileWizard.projects", { defaultValue: "Projects" })}
              options={projects}
              selected={selectedProjectIds ?? new Set()}
              onToggle={onToggleProject}
            />
            <TargetChecklist
              label={t("app.profileWizard.routines", { defaultValue: "Routines" })}
              options={routines}

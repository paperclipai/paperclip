...
export function WizardToolsStep(props: WizardToolsStepProps) {
  ...
  if (catalogLoading) return <LoadingState label={t("app.wizardToolsStep.loadingTools", { defaultValue: "Loading tools…" })} />;
  ...
  if (appGroups.length === 0) {
    return (
      <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-border py-12 text-center">
        ...
        <div>
          <p className="text-sm font-medium text-foreground">{t("app.wizardToolsStep.appConnectionsAreComingSoon", { defaultValue: "App connections are coming soon" })}</p>
          <p className="mx-auto max-w-sm text-sm text-muted-foreground">
            {t("app.wizardToolsStep.profilesWillBeAvailableOnceAppConnectionsAreReadyBrowseThePlannedIntegrationsInTheMeantime", { defaultValue: "Profiles will be available once app connections are ready. Browse the planned integrations in the meantime." })}</p>
        </div>
        <Button asChild variant="outline">
          <Link to="/apps">{t("app.wizardToolsStep.browseAppConnections", { defaultValue: "Browse app connections" })}</Link>
        </Button>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative min-w-(--sz-220px) flex-1">
          ...
          <Input
            ...
            placeholder={t("app.wizardToolsStep.searchTools", { defaultValue: "Search tools…" })}
            className="pl-8"
          />
        </div>
      ...
      {filteredGroups.length === 0 ? (
        ...
        <div className="flex flex-col items-center gap-2 rounded-lg border border-dashed border-border py-10 text-center">
          <p className="text-sm font-medium text-foreground">{t("app.wizardToolsStep.noToolsMatch", { defaultValue: "No tools match “" })}{search}”.</p>
          <button
            type="button"
            onClick={() => {
              setSearch("");
          ...
          >
            {t("app.wizardToolsStep.clearSearch", { defaultValue: "Clear search" })}</button>
        </div>
      ) : (
        ...
        >
          ...
          <span className="flex flex-col">
            <span className="text-sm font-medium text-foreground">
              {t("app.wizardToolsStep.all", { defaultValue: "All " })}{group.name} {t("app.wizardToolsStep.tools", { defaultValue: "tools (" })}{group.tools.length})
            </span>
...
function NewToolsRadio({
  ...
  const options: Array<{ value: NewToolsAction; label: string; hint: string; recommended?: boolean }> = [
    {
      value: "deny",
      label: t("app.wizardToolsStep.stayBlockedUntilSomeoneAllowsThem", { defaultValue: "Stay blocked until someone allows them" }),
      hint: t("app.wizardToolsStep.newToolsAnAppAddsLaterWonTBeUsableUntilYouReviewThem", { defaultValue: "New tools an app adds later won't be usable until you review them." }),
      recommended: true,
    },
    {
      value: "allow",
      label: t("app.wizardToolsStep.allowedAutomatically", { defaultValue: "Allowed automatically" }),
      hint: t("app.wizardToolsStep.anyToolAnAppAddsLaterBecomesUsableRightAway", { defaultValue: "Any tool an app adds later becomes usable right away." }),
    },
  ];
  return (
    <fieldset className="space-y-2 rounded-lg border border-border p-4">
      <legend className="px-1 text-sm font-medium text-foreground">{t("app.wizardToolsStep.newToolsThatAppearLater", { defaultValue: "New tools that appear later" })}</legend>
              ...
              <span className="flex items-center gap-2 text-sm font-medium text-foreground">
                ...
                {opt.recommended ? (
                  <Badge variant="outline" className="text-(length:--text-nano)">
                    {t("app.wizardToolsStep.recommended", { defaultValue: "Recommended" })}</Badge>
                ) : (
...
const RULE_KIND_OPTIONS: Array<{ value: AdvancedRuleKind; label: string }> = [
  { value: "tool_name", label: t("app.wizardToolsStep.toolNamePattern", { defaultValue: "Tool name pattern" }) },
  { value: "risk_level", label: t("app.wizardToolsStep.riskLevel", { defaultValue: "Risk level" }) },
  { value: "catalog_entry", label: t("app.wizardToolsStep.byToolId", { defaultValue: "By tool ID" }) },
];
...
function ruleSummary(rule: AdvancedRule): string {
  const verb = rule.effect === "include" ? t("app.wizardToolsStep.allow", { defaultValue: "Allow" }) : t("app.wizardToolsStep.block", { defaultValue: "Block" });
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="rounded-lg border border-border">
      <CollapsibleTrigger className="flex w-full items-center justify-between px-4 py-3 text-left">
        <span className="text-sm font-medium text-foreground">{t("app.wizardToolsStep.advancedRules", { defaultValue: "Advanced rules" })}</span>
        <ChevronDown className={cn("h-4 w-4 text-muted-foreground transition-transform", open && "rotate-180")} />
      </CollapsibleTrigger>
      <CollapsibleContent className="space-y-3 border-t border-border px-4 py-3">
        <p className="text-xs text-muted-foreground">
          {t("app.wizardToolsStep.matchToolsByANamePatternARiskLevelOrASpecificToolIdTheseRunOnTopOfTheChoicesAbove", { defaultValue: "Match tools by a name pattern, a risk level, or a specific tool ID. These run on top of the choices above." })}</p>
          ...
          <ul className="space-y-1.5">
            {rules.map((rule) => (
              ...
              >
                ...
                <button
                  type="button"
                  aria-label={t("app.wizardToolsStep.removeRule", { defaultValue: "Remove rule" })}
        ...
        <div className="flex flex-wrap items-end gap-2">
          <Select value={effect} onValueChange={(v) => setEffect(v as "include" | "exclude")}>
            ...
            <SelectContent>
              <SelectItem value="include">{t("app.wizardToolsStep.allow", { defaultValue: "Allow" })}</SelectItem>
              <SelectItem value="exclude">{t("app.wizardToolsStep.block", { defaultValue: "Block" })}</SelectItem>
            </SelectContent>
          </Select>
          ...
          {kind === "risk_level" ? (
            <Select value={value || "destructive"} onValueChange={setValue}>
              ...
              <SelectContent>
                <SelectItem value="read">{t("app.wizardToolsStep.readOnly", { defaultValue: "Read-only" })}</SelectItem>
                <SelectItem value="write">{t("app.wizardToolsStep.makesChanges", { defaultValue: "Makes changes" })}</SelectItem>
                <SelectItem value="destructive">{t("app.wizardToolsStep.destructive", { defaultValue: "Destructive" })}</SelectItem>
              </SelectContent>
            </Select>
          ) : (
            <Input
              ...
              placeholder={kind === "tool_name" ? t("app.wizardToolsStep.eGGmailSend", { defaultValue: "e.g. gmail.send*" }) : t("app.wizardToolsStep.toolId", { defaultValue: "tool ID" })}
              className="w-44"
            />
          )}
          <Button type="button" variant="outline" size="sm" onClick={addRule}>
            <Plus className="mr-1 h-3.5 w-3.5" />
            {t("app.wizardToolsStep.addRule", { defaultValue: "Add rule" })}</Button>
        </div>
      </CollapsibleContent>
    </Collapsible>
  );

...
export function GoalPropertiesToggleButton({
  ...
  return (
    <Button
      ...
      title={t("goalDetail.actions.showProperties", "Show properties")}
    >
...
export function GoalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("goals.breadcrumb", "Goals"), href: "/goals" },
      { label: goal?.title ?? goalId ?? t("goalDetail.breadcrumb.fallback", "Goal") }
    ]);
    ...
    <div className="space-y-6">
      <div className="space-y-3">
        ...
        <InlineEditor
          ...
          placeholder={t("goalDetail.editor.placeholder", "Add a description...")}
          multiline
      ...
      <Tabs defaultValue="children">
        <TabsList>
          <TabsTrigger value="children">
            {t("goalDetail.tabs.subGoals", "Sub-Goals")} ({childGoals.length})
          </TabsTrigger>
          <TabsTrigger value="projects">
            {t("goalDetail.tabs.projects", "Projects")} ({linkedProjects.length})
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="children" className="mt-4 space-y-3">
          <div className="flex items-center justify-start">
            ...
            >
              ...
              {t("goalDetail.actions.subGoal", "Sub Goal")}
            </Button>
          </div>
          {childGoals.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("goalDetail.empty.subGoals", "No sub-goals.")}</p>
          ) : (
        ...
        <TabsContent value="projects" className="mt-4">
          {linkedProjects.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("goalDetail.empty.linkedProjects", "No linked projects.")}</p>
          ) : (

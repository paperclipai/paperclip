...
export function GoalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("goalDetail.breadcrumb"), href: "/goals" },
      { label: goal?.title ?? goalId ?? t("goalDetail.fallbackTitle") }
    ]);
    ...
    <div className="space-y-6">
      <div className="space-y-3">
        ...
        <InlineEditor
          ...
          placeholder={t("goalDetail.descriptionPlaceholder")}
          multiline
      ...
      <Tabs defaultValue="children">
        <TabsList>
          <TabsTrigger value="children">
            {t("goalDetail.tabs.subGoals", { count: childGoals.length })}
          </TabsTrigger>
          <TabsTrigger value="projects">
            {t("goalDetail.tabs.projects", { count: linkedProjects.length })}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="children" className="mt-4 space-y-3">
          <div className="flex items-center justify-start">
            ...
            >
              ...
              {t("goalDetail.actions.subGoal")}
            </Button>
          </div>
          {childGoals.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("goalDetail.empty.subGoals")}</p>
          ) : (
        ...
        <TabsContent value="projects" className="mt-4">
          {linkedProjects.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("goalDetail.empty.projects")}</p>
          ) : (

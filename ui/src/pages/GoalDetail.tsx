...
export function GoalPropertiesToggleButton({
  ...
  return (
    <Button
      ...
      title={t("showProperties", { defaultValue: "Show properties" })}
    >
...
export function GoalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("breadcrumb", { defaultValue: "Goals" }), href: "/goals" },
      { label: goal?.title ?? goalId ?? t("breadcrumb_fallback", { defaultValue: "Goal" }) }
    ]);
      ...
      <Tabs defaultValue="children">
        <TabsList>
          <TabsTrigger value="children">
            {t("tabs.sub_goals", { defaultValue: "Sub-Goals" })} ({childGoals.length})
          </TabsTrigger>
          <TabsTrigger value="projects">
            {t("tabs.projects", { defaultValue: "Projects" })} ({linkedProjects.length})
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="children" className="mt-4 space-y-3">
          <div className="flex items-center justify-start">
            ...
            >
              ...
              {t("button.sub_goal", { defaultValue: "Sub Goal" })}
            </Button>
          </div>
          {childGoals.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("empty.sub_goals", { defaultValue: "No sub-goals." })}</p>
          ) : (
        ...
        <TabsContent value="projects" className="mt-4">
          {linkedProjects.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("empty.linked_projects", { defaultValue: "No linked projects." })}</p>
          ) : (

        ...
        <div className="overflow-x-auto rounded-lg border border-border">
          <table className="w-full text-sm">
            <thead className="border-b border-border bg-muted/40 text-left text-xs text-muted-foreground">
              <tr><th className="p-2">Domain</th><th className="p-2 text-right">Rank</th><th className="p-2">Via</th><th className="p-2">Competitor</th><th className="p-2">Status</th></tr>
            </thead>
            <tbody>
              {rows.map((p) => (
                <tr key={p.id} className="border-b border-border/60">
                  <td className="p-2">{p.source_url ? <a href={p.source_url} target="_blank" rel="noreferrer" className="hover:underline">{p.source_domain}</a> : p.source_domain}</td>
                  ...
                  <td className="p-2 text-xs">{p.competitor_name ?? "—"}</td>

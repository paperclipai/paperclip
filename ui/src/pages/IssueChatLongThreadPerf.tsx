    ...
    <div data-testid="issue-chat-long-thread-perf" className="space-y-5">
      <div className="flex flex-col gap-3 border-b border-border pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
            {t("issue-chat-long-thread-perf.deterministic-local-fixture-for-meas-2i0")}
          </p>
        </div>
        <div className="grid min-w-(--sz-280px) grid-cols-2 gap-2">
          <MetricTile label={t("issue-chat-long-thread-perf.fixture-rows-l49")} value={String(rowTarget)} testId="perf-fixture-row-target" />
          <MetricTile label={t("issue-chat-long-thread-perf.markdown-rows-1fm")} value={String(LONG_THREAD_MARKDOWN_COMMENT_COUNT)} testId="perf-fixture-markdown-rows" />
        </div>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-(--gtc-40)">
        <main className="min-w-0 space-y-4">
          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("issue-chat-long-thread-perf.issue-documents-vp8")}</CardTitle>
            </CardHeader>
          ...
          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("issue-chat-long-thread-perf.sub-issues-z47")}</CardTitle>
            </CardHeader>
        ...
        <aside className="space-y-4 xl:sticky xl:top-4 xl:self-start">
          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("issue-chat-long-thread-perf.baseline-metrics-mx9")}</CardTitle>
            </CardHeader>
            <CardContent className="grid gap-2">
              <MetricTile label={t("issue-chat-long-thread-perf.profiler-commits-1ou")} value={String(metrics.commitCount)} testId="perf-commit-count" />
              <MetricTile label={t("issue-chat-long-thread-perf.mount-duration-ry3")} value={formatMs(metrics.mountActualDuration)} testId="perf-mount-duration" />
              <MetricTile label={t("issue-chat-long-thread-perf.latest-duration-xxp")} value={formatMs(metrics.latestActualDuration)} testId="perf-latest-duration" />
              <MetricTile label={t("issue-chat-long-thread-perf.max-duration-1dq")} value={formatMs(metrics.maxActualDuration)} testId="perf-max-duration" />
              <MetricTile label={t("issue-chat-long-thread-perf.total-duration-joz")} value={formatMs(metrics.totalActualDuration)} testId="perf-total-duration" />
            </CardContent>
          </Card>

          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("issue-chat-long-thread-perf.fixture-shape-1g1")}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">

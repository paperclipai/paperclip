...
export function CompanyImport() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message="请选择要导入的目标公司。" />;
  }
    ...
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        ...
        {sourceMode === "local" ? (
          ...
          <Field
            label="GitHub URL"
            hint="仓库树路径或指向 COMPANY.md 的 blob URL（如 github.com/owner/repo/tree/main/company）。"
          >
            <input
        ...
        <Field label="目标" hint="导入到当前公司或创建新公司。">
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {

...
function renderImportFileExtra(node: FileTreeNode, checked: boolean, renameMap: Map<string, string>, t: (key: string, options?: Record<string, unknown>) => string) {
  ...
  const actionBadge = node.action ? (
    <span className={cn(
      ...
      {checked ? node.action : t("companyImport.renames.skip")}
    </span>
  ) : null;
      ...
      <div className="min-h-[560px] px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
            {t("companyImport.preview.binaryUnavailable")}
          </div>
        )}
      </div>
    </div>
      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">
            {t("companyImport.renames.title")}
          </h3>
          <span className="text-xs text-muted-foreground">
            {t("companyImport.renames.items", { count: conflicts.length })}
          </span>
        </div>
          ...
          {conflicts.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isSkipped ? t("companyImport.renames.skipped") : t("companyImport.renames.skip")}
                </button>
                ...
                <span className={cn(
                  ...
                  {t(`companyImport.kind.${item.kind}`)}
                </span>
                  ...
                  >
                    {isConfirmed ? (
                      <>
                        ...
                        {t("companyImport.renames.confirmed")}
                      </>
                    ) : (
                      t("companyImport.renames.confirmRename")
                    )}
                  </button>
                )}
              </div>
    ...
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {t("companyImport.adapters.items", { count: agents.length })}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
              ...
              <div key={agent.slug}>
                <div className="flex items-center gap-3 px-4 py-2.5 text-sm">
                  <span className={cn(
                    ...
                    {t("companyImport.adapters.agent")}
                  </span>
                  ...
                  >
                    ...
                    {t("companyImport.adapters.configureAdapter")}
                  </button>
                </div>
                {isExpanded && (
...
const ZIP_ERROR_NOT_ZIP = "Select a .zip company package.";
const ZIP_ERROR_EMPTY = "No package files were found in the selected zip archive.";
...
async function readLocalPackageZip(file: File): Promise<{
  ...
  if (!/\.zip$/i.test(file.name)) {
    throw new Error(ZIP_ERROR_NOT_ZIP);
  }
  const archive = await readZipArchive(await file.arrayBuffer());
  if (Object.keys(archive.files).length === 0) {
    throw new Error(ZIP_ERROR_EMPTY);
  }
  return {
    name: file.name,
...
export function CompanyImport() {
  ...
  const previewMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyImport.toast.previewFailedTitle"),
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companyImport.toast.importCompleteTitle"),
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyImport.toast.importFailedTitle"),
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const body = message === ZIP_ERROR_NOT_ZIP
        ? t("companyImport.error.notZip")
        : message === ZIP_ERROR_EMPTY
        ? t("companyImport.error.emptyZip")
        : message || t("companyImport.toast.packageReadFailedBody");
      pushToast({
        tone: "error",
        title: t("companyImport.toast.packageReadFailedTitle"),
        body,
      });
    }
  }
    ...
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {(
            [
              { key: "github", icon: Github, label: t("companyImport.source.github") },
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {localPackage.name} {t("companyImport.filesInPackage", { count: Object.keys(localPackage.files).length })}
                </span>
              )}
            </div>
            {!localPackage && (
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              <span className="text-muted-foreground">
                {t("companyImport.filesSelected", { selected: selectedCount, total: totalFiles, count: totalFiles })}
              </span>
              {conflicts.length > 0 && (
                <span className="text-amber-500">
                  {t("companyImport.conflicts", { count: conflicts.length })}
                </span>
              )}
              {importPreview.errors.length > 0 && (
                <span className="text-destructive">
                  {t("companyImport.errors", { count: importPreview.errors.length })}
                </span>
              )}
            </div>
          </div>

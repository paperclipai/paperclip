...
export function CompanyEnvironments() {
  ...
  const environmentMutation = useMutation({
    ...
    onSuccess: async (environment) => {
      ...
      pushToast({
        title: editingEnvironmentId ? t("settings.environmentUpdated") : t("settings.environmentCreated"),
        body: t("settings.environmentReady", { name: environment.name }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("settings.failedToSaveEnvironment"),
        body: error instanceof Error ? error.message : t("settings.environmentSaveFailed"),
        tone: "error",
      });
    },
  });
  ...
  const environmentProbeMutation = useMutation({
    ...
    onSuccess: (probe, environmentId) => {
      ...
      pushToast({
        title: probe.ok ? t("settings.probePassed") : t("settings.probeFailed"),
        body: probe.summary,
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("settings.selectCompanyToManageEnvironments")}</div>;
  }
    ...
    return (
      <div className="max-w-3xl space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("settings.companyEnvironments")}</h1>
        </div>
        <div className="rounded-md border border-border px-4 py-4 text-sm text-muted-foreground">
          {t("settings.enableEnvironmentsNotice")}
        </div>
      </div>
    );
  }
      ...
      <div className="space-y-4 rounded-md border border-border px-4 py-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[34rem] text-left text-xs">
            ...
            <thead className="border-b border-border text-muted-foreground">
              <tr>
                <th className="py-2 pr-3 font-medium">{t("settings.adapter")}</th>
                <th className="px-3 py-2 font-medium">{t("settings.local")}</th>
                <th className="px-3 py-2 font-medium">SSH</th>
                {sandboxSupportVisible ? (
                  <th className="px-3 py-2 font-medium">{t("settings.sandboxViaPlugin")}</th>
                ) : null}
              </tr>
            </thead>
        ...
        <div className="space-y-3">
          {(environments ?? []).length === 0 ? (
            <div className="text-sm text-muted-foreground">{t("settings.noEnvironmentsYet")}</div>
          ) : (
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="space-y-1">
                      ...
                      {environment.driver === "ssh" ? (
                        ...
                        <div className="text-xs text-muted-foreground">{t("settings.runsOnHost")}</div>
                      )}
                    </div>
        ...
        <div className="border-t border-border/60 pt-4">
          <div className="mb-3 text-sm font-medium">
            {editingEnvironmentId ? t("settings.editEnvironment") : t("settings.addEnvironment")}
          </div>
          <div className="space-y-3">
            <Field label={t("common.name")} hint={t("settings.environmentNameHint")}>
              <input
            ...
            <Field label={t("settings.description")} hint={t("settings.environmentDescriptionHint")}>
              <input
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                type="text"
            ...
            <Field label={t("settings.driver")} hint={t("settings.driverHint")}>
              <select
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            ...
            {environmentForm.driver === "ssh" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label={t("settings.host")} hint={t("settings.hostHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("settings.remoteWorkspacePath")} hint={t("settings.remoteWorkspacePathHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("settings.privateKey")} hint={t("settings.privateKeyHint")}>
                  <div className="space-y-2">
                    <select
                      className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    ...
                    >
                      <option value="">{t("settings.noSavedSecret")}</option>
                      {(secrets ?? []).map((secret) => (
                ...
                <Field label={t("settings.knownHosts")} hint={t("settings.knownHostsHint")}>
                  <textarea
                    className="h-32 w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-xs font-mono outline-none"
                ...
                <div className="md:col-span-2">
                  <ToggleField
                    label={t("settings.strictHostKeyChecking")}
                    hint={t("settings.strictHostKeyCheckingHint")}
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {environmentMutation.isPending
                  ? editingEnvironmentId
                    ...
                    : t("common.creating")
                  : editingEnvironmentId
                    ? t("settings.saveEnvironment")
                    : t("settings.createEnvironment")}
              </Button>
              {editingEnvironmentId ? (
                <Button
                  size="sm"
              ...
              {environmentForm.driver !== "local" ? (
                ...
                >
                  {draftEnvironmentProbeMutation.isPending ? t("common.testing") : t("settings.testDraft")}
                </Button>
              ) : null}

...
function SupportMark({ supported }: { supported: boolean }) {
  ...
  return supported ? (
    ...
    <span className="text-muted-foreground">{t("environments.no")}</span>
  );
}
...
export function CompanyEnvironments() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("environments.breadcrumb.company"), href: "/dashboard" },
      { label: t("environments.breadcrumb.settings"), href: "/company/settings" },
      { label: t("environments.breadcrumb.environments") },
    ]);
  ...
  const environmentMutation = useMutation({
    ...
    onSuccess: async (environment) => {
      ...
      pushToast({
        title: editingEnvironmentId ? t("environments.toast.updated") : t("environments.toast.created"),
        body: `${environment.name} ${t("environments.toast.readySuffix")}`,
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("environments.toast.saveFailed"),
        body: error instanceof Error ? error.message : t("environments.failedToSave"),
        tone: "error",
      });
    },
  });
  ...
  const environmentProbeMutation = useMutation({
    ...
    onSuccess: (probe, environmentId) => {
      ...
      pushToast({
        title: probe.ok ? t("environments.toast.probePassed") : t("environments.toast.probeFailed"),
        body: probe.summary,
    ...
    onError: (error, environmentId) => {
      ...
      setProbeResults((current) => ({
        ...
        [environmentId]: {
          ...
          summary: error instanceof Error ? error.message : t("environments.toast.probeFailed"),
          details: null,
        },
      }));
      pushToast({
        title: t("environments.toast.probeFailed"),
        body: error instanceof Error ? error.message : t("environments.toast.probeFailed"),
        tone: "error",
      });
    },
  });
  ...
  const draftEnvironmentProbeMutation = useMutation({
    ...
    onSuccess: (probe) => {
      pushToast({
        title: probe.ok ? t("environments.toast.draftProbePassed") : t("environments.toast.draftProbeFailed"),
        body: probe.summary,
        tone: probe.ok ? "success" : "error",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("environments.toast.draftProbeFailed"),
        body: error instanceof Error ? error.message : t("environments.toast.draftProbeFailed"),
        tone: "error",
      });
    },
  });
      ...
      <div className="space-y-4 rounded-md border border-border px-4 py-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[34rem] text-left text-xs">
            <caption className="sr-only">{t("environments.tableCaption")}</caption>
            <thead className="border-b border-border text-muted-foreground">
              <tr>
                <th className="py-2 pr-3 font-medium">{t("environments.adapter")}</th>
                <th className="px-3 py-2 font-medium">{t("environments.local")}</th>
                <th className="px-3 py-2 font-medium">{t("environments.ssh")}</th>
                {sandboxSupportVisible ? (
                  <th className="px-3 py-2 font-medium">{t("environments.sandbox")}</th>
                ) : null}
              </tr>
            </thead>
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="space-y-1">
                      ...
                      {environment.driver === "ssh" ? (
                        ...
                        <div className="text-xs text-muted-foreground">
                          {(() => {
                            const provider =
                              typeof environment.config.provider === "string" ? environment.config.provider : t("environments.sandboxProvider");
                            const displayName =
                            ...
                            return t("environments.sandboxProviderDisplay", { provider: displayName }) + (summary ? ` · ${summary}` : "");
                          })()}
                        </div>
                      ) : (
                        <div className="text-xs text-muted-foreground">{t("environments.localDescription")}</div>
                      )}
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      >
                        {isEditing ? t("environments.editing") : t("environments.edit")}
                      </Button>
                    </div>
                  </div>
                  {probe ? (

...
function getConcurrencyPolicyDescriptions(t: Translate): Record<string, string> {
  return {
    coalesce_if_active: t("routines.concurrencyCoalesceDescription"),
    always_enqueue: t("routines.concurrencyAlwaysEnqueueDescription"),
    skip_if_active: t("routines.concurrencySkipDescription"),
  };
}
...
function formatRoutineRunStatus(value: string | null | undefined, t: Translate) {
  ...
  switch (value) {
    case "queued":
      return t("routines.runStatusQueued");
    case "running":
      return t("routines.runStatusRunning");
    case "completed":
      return t("routines.runStatusCompleted");
    case "failed":
      return t("routines.runStatusFailed");
    case "cancelled":
      return t("routines.runStatusCancelled");
    default:
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(isArchived || routine.status === "paused" || isDraft) ? (
            <span className="text-xs text-muted-foreground">
              {isArchived ? t("routines.rowArchived") : isDraft ? t("routines.rowDraft") : t("routines.rowPaused")}
            </span>
          ) : null}
        </div>
      ...
      <div className="flex items-center gap-3" onClick={(event) => { event.preventDefault(); event.stopPropagation(); }}>
        <div className="flex items-center gap-3">
          <ToggleSwitch
            ...
            aria-label={enabled ? t("routines.disableRoutine", { title: routine.title }) : t("routines.enableRoutine", { title: routine.title })}
          />
          <span className="w-12 text-xs text-muted-foreground">
            {isArchived ? t("routines.stateArchived") : isDraft ? t("routines.stateDraft") : enabled ? t("routines.stateOn") : t("routines.stateOff")}
          </span>
        </div>
...
export function Routines() {
  ...
  const groupingLabels = useMemo(
    () => ({
      ...
      unknownProject: t("routines.unknownProject"),
      unassigned: t("routines.unassigned"),
      unknownAgent: t("routines.unknownAgent"),
    }),
    [t],
  );
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("routines.createdTitle"),
        body: routine.assigneeAgentId
          ? t("routines.createdBodyLiveWorkflow")
          : t("routines.createdBodyDraft"),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.updateFailedTitle"),
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.runFailedTitle"),
  ...
  const routineGroups = useMemo(
    () => buildRoutineGroups(routines ?? [], routineViewState.groupBy, projectById, agentById, groupingLabels),
    [agentById, groupingLabels, projectById, routineViewState.groupBy, routines],
  );
  ...
  function handleToggleEnabled(routine: RoutineListItem, enabled: boolean) {
    if (!enabled && !routine.assigneeAgentId) {
      pushToast({
        title: t("routines.defaultAgentRequiredTitle"),
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        ...
        <TabsContent value="routines" className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm text-muted-foreground">
              {t("routines.count", { count: (routines ?? []).length, suffix: (routines ?? []).length === 1 ? "" : "s" })}
            </p>
            <Popover>
            ...
            <div className="px-5 pb-3">
              <div className="overflow-x-auto overscroll-x-contain">
                <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                  <span>{t("routines.assignmentFor")}</span>
                  ...
                  />
                  <span>{t("routines.assignmentIn")}</span>
                  <InlineEntitySelector

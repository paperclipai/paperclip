  ...
  return (
    <div className="space-y-4">
      ...
      {open && (
        <AgnbFormModal
          title="New persona"
          fields={[{ key: "name", label: "Name", required: true }, { key: "title", label: "Title" }, { key: "description", label: "Description / brief", type: "textarea" }]}
          onClose={() => setOpen(false)}
          onSubmit={async (v) => { await campaignsApi.createPersona({ name: v.name, title: v.title || undefined, description: v.description || undefined }); qc.invalidateQueries({ queryKey: queryKeys.agnb.personas }); }}
        />
      )}

...
export function BoardClaimPage() {
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("boardClaim.unavailable")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {statusQuery.error instanceof Error ? statusQuery.error.message : t("cliAuth.invalidOrExpired")}
          </p>
        </div>
      </div>
    );
  ...
  if (!status) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">{t("boardClaim.unavailable")}</div>;
  }
  ...
  if (status.status === "claimed") {
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("boardClaim.claimed.title")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("boardClaim.claimed.description")}
          </p>
          <Button asChild className="mt-4">
            <Link to="/">{t("boardClaim.claimed.openBoard")}</Link>
          </Button>
        </div>
      </div>
    );

...
export function AgentSkillRow({
  ...
  const trailing = readOnly ? (
    <Lock className="h-4 w-4 shrink-0 text-muted-foreground/60" aria-label={t("localizationSkills.readOnly459")} />
  ) : (
    (() => {
      const toggle = (
        <ToggleSwitch
          ...
          aria-label={checked ? t("localizationAgentManagement.disableSkill", { name: data.name }) : t("localizationAgentManagement.enableSkill", { name: data.name })}
        />
      );

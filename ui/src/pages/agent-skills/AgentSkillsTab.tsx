        ...
        badge={
          showReleasePicker && pinnedRelease ? (
            <Badge variant="secondary" className="text-(length:--text-nano)">
              {t("app.agentSkillsTab.beta", { defaultValue: "Beta · " })}{releaseShortLabel(pinnedRelease)}
            </Badge>
          ) : undefined
        }
        accessory={
    ...
    <div className="max-w-4xl space-y-4">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-sm font-medium text-foreground">
            {enabledRows.length} {t("app.agentSkillsTab.of", { defaultValue: "of " })}{libraryRows.length} enabled
          </span>
          {applicationLabel ? (
            <Tooltip>
          ...
          <div className="ml-auto flex w-full flex-col items-stretch gap-2 sm:w-auto sm:flex-row sm:items-center">
            <div className="relative w-full sm:w-auto">
              ...
              <Input
                ...
                placeholder={t("app.agentSkillsTab.searchSkills", { defaultValue: "Search skills" })}
                className="h-8 w-full pl-8 sm:w-56"
                aria-label={t("app.agentSkillsTab.searchSkills", { defaultValue: "Search skills" })}
              />
            </div>
            <Button asChild variant="outline" size="sm" className="shrink-0">
              <Link to="/skills" className="no-underline">
                <Store className="h-3.5 w-3.5" />
                {t("app.agentSkillsTab.browseSkillsStore", { defaultValue: "Browse skills store" })}</Link>
            </Button>
          </div>
        </div>
        ...
        {syncSkills.isError ? (
          <p className="text-xs text-destructive">
            {syncSkills.error instanceof Error ? syncSkills.error.message : t("app.agentSkillsTab.failedToUpdateSkills", { defaultValue: "Failed to update skills" })}
          </p>
        ) : null}
      </div>
          ...
          {staleDesiredKeys.map((key) => (
            ...
            >
              <span className="min-w-0 truncate">
                <span className="font-medium">{key}</span> {t("app.agentSkillsTab.isEnabledButMissingFromTheCompanyLibrary", { defaultValue: "is enabled but missing from the company library." })}</span>
              <button
                type="button"
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          <SkillSection title={t("app.agentSkillsTab.enabledOnThisAgent", { defaultValue: "Enabled on this agent" })} count={filteredEnabled.length}>
            {filteredEnabled.length > 0 ? (
              filteredEnabled.map((row) => renderRow(row, "enabled"))
            ) : (
              <SectionEmpty>
                {search ? t("app.agentSkillsTab.noEnabledSkillsMatchYourSearch", { defaultValue: "No enabled skills match your search." }) : t("app.agentSkillsTab.noSkillsEnabledOnThisAgentYet", { defaultValue: "No skills enabled on this agent yet." })}
              </SectionEmpty>
            )}
          </SkillSection>

          <SkillSection title={t("app.agentSkillsTab.availableFromTheLibrary", { defaultValue: "Available from the library" })} count={filteredAvailable.length}>
            {filteredAvailable.length > 0 ? (
              filteredAvailable.map((row) => renderRow(row, "available"))
            ) : (
              <SectionEmpty>
                {search
                  ? t("app.agentSkillsTab.noAvailableSkillsMatchYourSearch", { defaultValue: "No available skills match your search." })
                  : libraryEmpty
                    ? t("app.agentSkillsTab.importSkillsIntoTheCompanyLibraryToEnableThemHere", { defaultValue: "Import skills into the company library to enable them here." })
                    : t("app.agentSkillsTab.everyLibrarySkillIsEnabledOnThisAgent", { defaultValue: "Every library skill is enabled on this agent." })}
              </SectionEmpty>
            )}
          </SkillSection>
              ...
              <div className="overflow-hidden rounded-lg border border-border">
                <CollapsibleTrigger className="flex w-full items-center gap-2 bg-muted/50 px-3 py-2 text-left outline-none focus-visible:ring-2 focus-visible:ring-ring">
                  ...
                  <span className="text-xs font-medium text-muted-foreground">
                    {t("app.agentSkillsTab.detectedOnAdapterReadOnly", { defaultValue: "Detected on adapter (read-only)" })}</span>
                ...
                <CollapsibleContent>
                  {filteredDetected.length > 0 ? (
                    ...
                    <SectionEmpty>{t("app.agentSkillsTab.noDetectedSkillsMatchYourSearch", { defaultValue: "No detected skills match your search." })}</SectionEmpty>
                  )}
                </CollapsibleContent>
              </div>
            </Collapsible>
          ...
          <div className="text-xs text-muted-foreground">
            {t("app.agentSkillsTab.adapter", { defaultValue: "Adapter: " })}{adapterLabels[agent.adapterType] ?? agent.adapterType}
          </div>
        </div>
      )}
    </div>
  ...
  if (error) {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs text-destructive">
        ...
        {t("app.agentSkillsTab.couldnTSave", { defaultValue: "Couldn’t save" })}</span>
    );
  }
  if (unsaved) {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
        <Loader2 className="h-3.5 w-3.5 animate-spin" />
        {t("app.agentSkillsTab.savingSoon", { defaultValue: "Saving soon…" })}</span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1.5 text-xs text-(--status-task-done)">
      <CheckCircle2 className="h-3.5 w-3.5" />
      {t("app.agentSkillsTab.saved", { defaultValue: "Saved" })}</span>
  );
}
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-border px-6 py-10 text-center">
      ...
      <div className="space-y-1">
        <p className="text-sm font-medium text-foreground">{t("app.agentSkillsTab.noSkillsInTheCompanyLibrary", { defaultValue: "No skills in the company library" })}</p>
        <p className="text-xs text-muted-foreground">
          {t("app.agentSkillsTab.installSkillsToTheCompanyThenEnableThemOnThisAgent", { defaultValue: "Install skills to the company, then enable them on this agent." })}</p>
      </div>

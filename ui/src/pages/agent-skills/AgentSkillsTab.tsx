        ...
        badge={
          showReleasePicker && pinnedRelease ? (
            <Badge variant="secondary" className="text-(length:--text-nano)">
              {t("agent-skills-tab.beta-1m4")} {releaseShortLabel(pinnedRelease)}
            </Badge>
          ) : undefined
        }
        accessory={
    ...
    <div className="max-w-4xl space-y-4">
          ...
          <div className="ml-auto flex w-full flex-col items-stretch gap-2 sm:w-auto sm:flex-row sm:items-center">
            <div className="relative w-full sm:w-auto">
              ...
              <Input
                ...
                placeholder={t("agent-skills-tab.search-skills-1s0")}
                className="h-8 w-full pl-8 sm:w-56"
                aria-label={t("agent-skills-tab.search-skills-1s0")}
              />
            </div>
            <Button asChild variant="outline" size="sm" className="shrink-0">
              <Link to="/skills" className="no-underline">
                <Store className="h-3.5 w-3.5" />
                {t("agent-skills-tab.browse-skills-store-rvu")}
              </Link>
            </Button>
          </div>
        </div>
          ...
          {staleDesiredKeys.map((key) => (
            ...
            >
              <span className="min-w-0 truncate">
                <span className="font-medium">{key}</span> {t("agent-skills-tab.is-enabled-but-missing-from-the-orga-6f4")}
              </span>
              <button
                type="button"
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          <SkillSection title={t("agent-skills-tab.enabled-on-this-agent-1d9")} count={filteredEnabled.length}>
          ...
          <SkillSection title={t("agent-skills-tab.available-from-the-library-o57")} count={filteredAvailable.length}>
              ...
              <div className="overflow-hidden rounded-lg border border-border">
                <CollapsibleTrigger className="flex w-full items-center gap-2 bg-muted/50 px-3 py-2 text-left outline-none focus-visible:ring-2 focus-visible:ring-ring">
                  ...
                  <span className="text-xs font-medium text-muted-foreground">
                    {t("agent-skills-tab.automatic-and-detected-skills-read-o-18v")}
                  </span>
                ...
                <CollapsibleContent>
                  {filteredDetected.length > 0 ? (
                    ...
                    <SectionEmpty>{t("agent-skills-tab.no-detected-skills-match-your-search-1vs")}</SectionEmpty>
                  )}
                </CollapsibleContent>
              </div>
            </Collapsible>
          ...
          <div className="text-xs text-muted-foreground">
            {t("agent-skills-tab.adapter-wkt")} {adapterLabels[agent.adapterType] ?? agent.adapterType}
          </div>
        </div>
      )}
    </div>
  ...
  if (error) {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs text-destructive">
        ...
        {t("agent-skills-tab.couldn-t-save-sp6")}
      </span>
    );
  }
  if (unsaved) {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
        <Loader2 className="h-3.5 w-3.5 animate-spin" />
        {t("agent-skills-tab.saving-soon-1na")}
      </span>
    );
  }
  return (
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-border px-6 py-10 text-center">
      ...
      <div className="space-y-1">
        <p className="text-sm font-medium text-foreground">{t("agent-skills-tab.no-skills-in-the-organization-librar-1bn")}</p>
        <p className="text-xs text-muted-foreground">
          {t("agent-skills-tab.install-skills-to-the-organization-t-6l4")}
        </p>
      </div>

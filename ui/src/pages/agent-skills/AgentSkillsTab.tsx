...
const MATERIALIZATION_NOTE =
  t("agentSkill.tab.materializationNote");
...
export function AgentSkillsTab({ agent, companyId }: { agent: Agent; companyId?: string }) {
  ...
  const applicationLabel = useMemo(() => {
    switch (skillSnapshot?.mode) {
      case "persistent":
        return t("agentSkill.tab.application.persistent");
      case "ephemeral":
        return t("agentSkill.tab.application.ephemeral");
      case "unsupported":
        return t("agentSkill.tab.application.unsupported");
      default:
        return null;
    }
  ...
  const unsupportedMessage = useMemo(() => {
    ...
    if (
      ...
      return t("agentSkill.tab.unsupported.customAcp");
    }
    if (agent.adapterType === "openclaw_gateway") {
      return t("agentSkill.tab.unsupported.openclaw");
    }
    return t("agentSkill.tab.unsupported.generic");
    ...
    <div className="max-w-4xl space-y-4">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-sm font-medium text-foreground">
            {t("agentSkill.tab.enabledOfTotal", { enabled: enabledRows.length, total: libraryRows.length })}
          </span>
          {applicationLabel ? (
            <Tooltip>
          ...
          <div className="ml-auto flex w-full flex-col items-stretch gap-2 sm:w-auto sm:flex-row sm:items-center">
            <div className="relative w-full sm:w-auto">
              ...
              <Input
                ...
                placeholder={t("agentSkill.tab.searchPlaceholder")}
                className="h-8 w-full pl-8 sm:w-56"
                aria-label={t("agentSkill.tab.searchPlaceholder")}
              />
            </div>
            <Button asChild variant="outline" size="sm" className="shrink-0">
              <Link to="/skills" className="no-underline">
                <Store className="h-3.5 w-3.5" />
                {t("agentSkill.tab.browseStore")}
              </Link>
            </Button>
          </div>
        </div>
        ...
        {syncSkills.isError ? (
          <p className="text-xs text-destructive">
            {syncSkills.error instanceof Error ? syncSkills.error.message : t("agentSkill.tab.failedToUpdate")}
          </p>
        ) : null}
      </div>
          ...
          {staleDesiredKeys.map((key) => (
            ...
            >
              <span className="min-w-0 truncate">
                {t("agentSkill.tab.staleSkill", { key })}
              </span>
              <button
                type="button"
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          <SkillSection title={t("agentSkill.tab.enabledSection")} count={filteredEnabled.length}>
            {filteredEnabled.length > 0 ? (
              filteredEnabled.map((row) => renderRow(row, "enabled"))
            ) : (
              <SectionEmpty>
                {search ? t("agentSkill.tab.noEnabledMatch") : t("agentSkill.tab.noEnabledYet")}
              </SectionEmpty>
            )}
          </SkillSection>

          <SkillSection title={t("agentSkill.tab.availableSection")} count={filteredAvailable.length}>
            {filteredAvailable.length > 0 ? (
              filteredAvailable.map((row) => renderRow(row, "available"))
            ) : (
              <SectionEmpty>
                {search
                  ? t("agentSkill.tab.noAvailableMatch")
                  : libraryEmpty
                    ? t("agentSkill.tab.importToEnable")
                    : t("agentSkill.tab.allEnabled")}
              </SectionEmpty>
            )}
          </SkillSection>
              ...
              <div className="overflow-hidden rounded-lg border border-border">
                <CollapsibleTrigger className="flex w-full items-center gap-2 bg-muted/50 px-3 py-2 text-left outline-none focus-visible:ring-2 focus-visible:ring-ring">
                  ...
                  <span className="text-xs font-medium text-muted-foreground">
                    {t("agentSkill.tab.detectedSection")}
                  </span>
                ...
                <CollapsibleContent>
                  {filteredDetected.length > 0 ? (
                    ...
                    <SectionEmpty>{t("agentSkill.tab.noDetectedMatch")}</SectionEmpty>
                  )}
                </CollapsibleContent>
              </div>
            </Collapsible>
          ...
          <div className="text-xs text-muted-foreground">
            {t("agentSkill.tab.adapter", { label: adapterLabels[agent.adapterType] ?? agent.adapterType })}
          </div>
        </div>
      )}
    </div>
  ...
  if (pending) {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
        ...
        {t("agentSkill.tab.status.saving")}
      </span>
    );
  }
  if (error) {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs text-destructive">
        ...
        {t("agentSkill.tab.status.couldntSave")}
      </span>
    );
  }
  if (unsaved) {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
        <Loader2 className="h-3.5 w-3.5 animate-spin" />
        {t("agentSkill.tab.status.savingSoon")}
      </span>
    );
  }
  return (
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-border px-6 py-10 text-center">
      ...
      <div className="space-y-1">
        <p className="text-sm font-medium text-foreground">{t("agentSkill.tab.emptyLibraryTitle")}</p>
        <p className="text-xs text-muted-foreground">
          {t("agentSkill.tab.emptyLibraryBody")}
        </p>
      </div>

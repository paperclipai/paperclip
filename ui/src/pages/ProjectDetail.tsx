  ...
  return (
    <div className="space-y-6">
      <InlineEditor
        ...
        placeholder={t('projects.addDescriptionPlaceholder')}
        multiline
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        aria-label={t('projects.changeProjectColor')}
      />
      {open && (
          ...
          <div className="grid grid-cols-5 gap-1.5">
            {PROJECT_COLORS.map((color) => (
              <button
                ...
                aria-label={t('projects.selectColor', { color })}
              />
            ))}
          </div>
        </div>
...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onSuccess: (updatedProject, archived) => {
      ...
      if (archived) {
        pushToast({ title: t('projects.projectArchived', { name }), tone: "success" });
        navigate("/dashboard");
      } else {
        pushToast({ title: t('projects.projectUnarchived', { name }), tone: "success" });
      }
    },
    onError: (_, archived) => {
      pushToast({
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: project?.name ?? routeProjectRef ?? t('sidebar.projects') },
    ]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="min-w-0 space-y-2">
          ...
          {project.managedByPlugin ? (
            <div className="inline-flex items-center gap-2 rounded-full border border-border bg-muted px-3 py-1 text-[11px] font-medium text-muted-foreground">
              ...
              {t('projects.managedByPlugin', { pluginName: project.managedByPlugin.pluginDisplayName })}
            </div>
          ) : null}
        </div>
      </div>
      ...
      <Tabs value={activeTab ?? "list"} onValueChange={(value) => handleTabChange(value as ProjectTab)}>
        <PageTabBar
          items={[
            { value: "list", label: t('sidebar.issues') },
            { value: "overview", label: t('projects.overview') },
            ...(project.managedByPlugin ? [{ value: "plugin-operations", label: t('projects.pluginOperations') }] : []),
            ...(showWorkspacesTab ? [{ value: "workspaces", label: t('sidebar.workspaces') }] : []),

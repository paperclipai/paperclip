    ...
    >
      <PopoverTrigger asChild>
        <button
          ...
          aria-label={t("projectDetail.changeIconColor")}
        >
      ...
      <PopoverContent className="w-72 p-3" align="start">
        {/* Icon search + grid */}
        <p className="text-xs font-medium text-muted-foreground mb-2">{t("projectDetail.icon")}</p>
        <Input
          placeholder={t("projectDetail.searchIcons")}
          value={search}
        ...
        <div className="grid grid-cols-7 gap-1 max-h-40 overflow-y-auto">
          ...
          {filteredIcons.length === 0 && (
            <p className="col-span-7 text-xs text-muted-foreground text-center py-2">{t("projectDetail.noIconsMatch")}</p>
          )}
        </div>

        {/* Color swatches */}
        <div className="mt-3 border-t border-border pt-3">
          <p className="text-xs font-medium text-muted-foreground mb-2">{t("projectDetail.color")}</p>
          <div className="grid grid-cols-5 gap-1.5">
            ...
            <button
              ...
              aria-label={t("projectDetail.resetColor")}
              title={t("projectDetail.neutralColor")}
            >
            ...
            {PROJECT_COLORS.map((swatch) => (
              <button
                ...
                aria-label={t("projectDetail.selectColor", { color: swatch })}
              />
            ))}
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
      {showLeftProjectNotice ? (
        <div className="flex items-center gap-3 border border-yellow-300/35 bg-yellow-300/10 px-3 py-2 text-sm text-yellow-900 dark:text-yellow-100">
          ...
          <button
            ...
            aria-label={t("projectDetail.dismissMembershipNotice")}
        ...
        <div className="min-w-0 space-y-2">
          ...
          {project.pauseReason === "budget" ? (
            <div className="inline-flex items-center gap-2 rounded-full border border-red-500/30 bg-red-500/10 px-3 py-1 text-(length:--text-micro) font-medium uppercase tracking-(--tracking-caps) text-red-800 dark:text-red-200">
              ...
              {t("projectDetail.pausedBudgetHardStop")}
            </div>
          ) : null}
          {project.managedByPlugin ? (
            <div className="inline-flex items-center gap-2 rounded-full border border-border bg-muted px-3 py-1 text-(length:--text-micro) font-medium text-muted-foreground">
              ...
              {t("projectDetail.managedByPlugin", { name: project.managedByPlugin.pluginDisplayName })}
            </div>
          ) : null}
        </div>
      ...
      <SummarySlotCard
        ...
        scopeKind="project"
        scopeId={project.id}
        title={t("projectDetail.summaryTitle")}
        description={t("projectDetail.summaryDescription")}
      />

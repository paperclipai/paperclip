    ...
    >
      <PopoverTrigger asChild>
        <button
          ...
          aria-label={t("project-detail.change-project-icon-and-color-182")}
        >
      ...
      <PopoverContent className="w-72 p-3" align="start">
        {/* Icon search + grid */}
        <p className="text-xs font-medium text-muted-foreground mb-2">{t("project-detail.icon-t3t")}</p>
        <Input
          placeholder={t("project-detail.search-icons-1a2")}
          value={search}
        ...
        <div className="grid grid-cols-7 gap-1 max-h-40 overflow-y-auto">
          ...
          {filteredIcons.length === 0 && (
            <p className="col-span-7 text-xs text-muted-foreground text-center py-2">{t("project-detail.no-icons-match-rxo")}</p>
          )}
        </div>

        {/* Color swatches */}
        <div className="mt-3 border-t border-border pt-3">
          <p className="text-xs font-medium text-muted-foreground mb-2">{t("project-detail.color-1rq")}</p>
          <div className="grid grid-cols-5 gap-1.5">
            ...
            <button
              ...
              aria-label={t("project-detail.reset-to-neutral-gray-1gq")}
              title={t("project-detail.neutral-default-e08")}
            >
  ...
  return (
    <div className="space-y-6">
      {showLeftProjectNotice ? (
        <div className="flex items-center gap-3 border border-yellow-300/35 bg-yellow-300/10 px-3 py-2 text-sm text-yellow-900 dark:text-yellow-100">
          <p className="min-w-0 flex-1">
            {t("project-detail.you-left-this-project-it-no-longer-a-1q7")}
          </p>
          <MembershipAction
            compact
            state="left"
          ...
          <button
            ...
            aria-label={t("project-detail.dismiss-project-membership-notice-1o3")}
        ...
        <div className="min-w-0 space-y-2">
          ...
          {project.pauseReason === "budget" ? (
            <div className="inline-flex items-center gap-2 rounded-full border border-red-500/30 bg-red-500/10 px-3 py-1 text-(length:--text-micro) font-medium uppercase tracking-(--tracking-caps) text-red-800 dark:text-red-200">
              ...
              {t("project-detail.paused-by-budget-hard-stop-137")}
            </div>
          ) : null}
          {project.managedByPlugin ? (
            <div className="inline-flex items-center gap-2 rounded-full border border-border bg-muted px-3 py-1 text-(length:--text-micro) font-medium text-muted-foreground">
              ...
              {t("project-detail.managed-by-1l4")} {project.managedByPlugin.pluginDisplayName}
            </div>
          ) : null}
        </div>
      ...
      <SummarySlotCard
        ...
        scopeKind="project"
        scopeId={project.id}
        title={t("project-detail.project-summary-14d")}
        description={t("project-detail.summarizer-keeps-the-latest-project-16i")}
      />
      ...
      {activeTab === "workspaces" ? (
        workspaceTabDecisionLoaded ? (
          ...
          <p className="text-sm text-muted-foreground">{t("project-detail.loading-workspaces-go2")}</p>
        )
      ) : null}

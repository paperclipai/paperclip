...
export function CompanyAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("companyAccess.members", { defaultValue: "Members" }) },
    ]);
  ...
  const updateMemberMutation = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("companyAccess.memberUpdateFailed", { defaultValue: "Failed to update member" }),
  ...
  const approveJoinRequestMutation = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("companyAccess.joinRequestApproveFailed", { defaultValue: "Failed to approve join request" }),
        body: error instanceof Error ? error.message : t("companyAccess.unknownError", { defaultValue: "Unknown error" }),
        tone: "error",
      });
    },
  ...
  const rejectJoinRequestMutation = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("companyAccess.joinRequestRejectFailed", { defaultValue: "Failed to reject join request" }),
        body: error instanceof Error ? error.message : t("companyAccess.unknownError", { defaultValue: "Unknown error" }),
        tone: "error",
      });
    },
  ...
  const archiveMemberMutation = useMutation({
      ...
      pushToast({
        ...
        body:
          result.reassignedIssueCount > 0
            ? t("companyAccess.assignedIssuesCleaned", {
                defaultValue: "{{count}} assigned issues cleaned up.",
    ...
    onError: (error) => {
      pushToast({
        title: t("companyAccess.memberRemoveFailed", { defaultValue: "Failed to remove member" }),
        body: error instanceof Error ? error.message : t("companyAccess.unknownError", { defaultValue: "Unknown error" }),
        tone: "error",
      });
    },
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("companyAccess.selectCompany", { defaultValue: "Select a company to manage access." })}</div>;
  }

  if (membersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("companyAccess.loadingAccess", { defaultValue: "Loading company access…" })}</div>;
  }
    ...
    <div className="max-w-6xl space-y-8">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("companyAccess.companyMembers", { defaultValue: "Company Members" })}</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("companyAccess.manageMembersIntro", {
        ...
        <div className="rounded-lg border border-border bg-muted/30 px-4 py-3 text-sm text-muted-foreground">
          {t("companyAccess.coreFocusNotice", { defaultValue: "Core keeps this page focused on membership, invite approvals, and safe member removal." })}
        </div>
      </div>

      {access && !access.currentUserRole && (
        <div className="rounded-xl border border-amber-500/40 px-4 py-3 text-sm text-amber-200">
          {t("companyAccess.instanceAdminWithoutMembership", { defaultValue: "This account can manage access here through instance-admin privileges, but it does not currently hold an active company membership." })}
        </div>
      )}
      ...
      <section className="space-y-4">
        <div className="space-y-1">
          ...
          <p className="max-w-3xl text-sm text-muted-foreground">
            {t("companyAccess.humansIntro", { defaultValue: "Manage human company memberships and status here." })}
          </p>
        </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {t("companyAccess.pendingHumanJoinsDescription", { defaultValue: "Review human join requests before they become active company members." })}
                </p>
              </div>
            ...
            <div className="space-y-3">
              {pendingHumanJoinRequests.map((request) => (
                <PendingJoinRequestCard
                  ...
                  context={
                    request.invite
                      ? t("companyAccess.inviteContext", {
                          defaultValue: "{{types}} join invite{{role}}",
                          types: request.invite.allowedJoinTypes,
                          role: request.invite.humanRole ? ` • default role ${request.invite.humanRole}` : "",
                        })
                      : t("companyAccess.inviteMetadataUnavailable", { defaultValue: "Invite metadata unavailable" })
                  }
                  detail={t("companyAccess.submittedAt", { defaultValue: "Submitted {{date}}", date: new Date(request.createdAt).toLocaleString() })}
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          <div className="grid grid-cols-[minmax(0,1.5fr)_120px_120px_180px] gap-3 border-b border-border px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted-foreground">
            <div>{t("companyAccess.userAccount", { defaultValue: "User account" })}</div>
            <div>{t("companyAccess.role", { defaultValue: "Role" })}</div>
            <div>{t("companyAccess.status", { defaultValue: "Status" })}</div>
            <div className="text-right">{t("companyAccess.action", { defaultValue: "Action" })}</div>
          </div>
          {members.length === 0 ? (
            <div className="px-4 py-8 text-sm text-muted-foreground">{t("companyAccess.noMemberships", { defaultValue: "No user memberships found for this company yet." })}</div>
          ) : (
            members.map((member) => {
              ...
              <div className="rounded-lg border border-border px-3 py-3">
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  {assignedIssuesQuery.isLoading
                    ...
                    : t("companyAccess.openAssignedIssues", {
                        defaultValue: "{{count}} open assigned issues",
          ...
          <DialogFooter>
            ...
            >
              {archiveMemberMutation.isPending
                ...
                : t("companyAccess.removeMemberButton", { defaultValue: "Remove member" })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
export function CompanyAccessLegacyRoute() {
  ...
  if (isLoading) {
    return <div className="text-sm text-muted-foreground">{t("companyAccess.checkingExtensions", { defaultValue: "Checking for advanced permission extensions..." })}</div>;
  }
  ...
  return (
    <div className="max-w-2xl space-y-5">
      <div className="space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          {t("companyAccess.advancedPermissionsIntro", { defaultValue: "Advanced access, scoped assignment, and explicit grant controls are provided by installed company settings extensions." })}
        </p>
      </div>
      ...
      <div className="space-y-4 rounded-xl border border-border px-5 py-5">
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            {t("companyAccess.advancedPermissionsUnavailableBody", { defaultValue: "Core Paperclip keeps enforcing company boundaries and any existing restrictive policy data, but editing advanced permissions requires an installed extension." })}
          </p>
          {errorMessage ? (
            <p className="text-sm text-destructive">{t("companyAccess.pluginExtensionsUnavailable", { defaultValue: "Plugin extensions unavailable: {{message}}", message: errorMessage })}</p>
          ) : null}
        </div>
...
function memberDisplayName(
  ...
  if (!member) {
    return t ? t("companyAccess.thisMember", { defaultValue: "this member" }) : "this member";
  }

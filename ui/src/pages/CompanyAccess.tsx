...
import { companyAccessPage } from "../lib/i18n";

const permissionLabels: Record<PermissionKey, string> = {
  "agents:create": companyAccessPage.createAgents,
  "users:invite": companyAccessPage.inviteHumansAndAgents,
  "users:manage_permissions": companyAccessPage.manageMembersAndGrants,
  "tasks:assign": companyAccessPage.assignTasks,
  "tasks:assign_scope": companyAccessPage.assignScopedTasks,
  "tasks:manage_active_checkouts": companyAccessPage.manageActiveCheckouts,
  "joins:approve": companyAccessPage.approveJoinRequests,
  "environments:manage": companyAccessPage.manageEnvironments,
};

function formatGrantSummary(member: CompanyMember) {
  if (member.grants.length === 0) return companyAccessPage.noExplicitGrants;
...
export function CompanyAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? companyAccessPage.title, href: "/dashboard" },
      { label: companyAccessPage.humans, href: "/company/settings" },
      { label: companyAccessPage.title },
    ]);
  ...
  const updateMemberMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({
        title: companyAccessPage.memberUpdated,
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: companyAccessPage.failedToUpdate,
        body: error instanceof Error ? error.message : companyAccessPage.unknownError,
        tone: "error",
      });
    },
  });
  ...
  const approveJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: companyAccessPage.joinRequestApproved,
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: companyAccessPage.failedToApprove,
        body: error instanceof Error ? error.message : companyAccessPage.unknownError,
        tone: "error",
      });
    },
  ...
  const rejectJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: companyAccessPage.joinRequestRejected,
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: companyAccessPage.failedToReject,
        body: error instanceof Error ? error.message : companyAccessPage.unknownError,
        tone: "error",
      });
    },
  ...
  const archiveMemberMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        title: companyAccessPage.memberRemoved,
        body:
          result.reassignedIssueCount > 0
            ? companyAccessPage.assignedCleanedUp(result.reassignedIssueCount)
            : undefined,
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: companyAccessPage.failedToRemove,
        body: error instanceof Error ? error.message : companyAccessPage.unknownError,
        tone: "error",
      });
    },
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{companyAccessPage.noCompanySelected}</div>;
  }

  if (membersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{companyAccessPage.loading}</div>;
  }

  if (membersQuery.error) {
    const message =
      membersQuery.error instanceof ApiError && membersQuery.error.status === 403
        ? companyAccessPage.noPermission
        : membersQuery.error instanceof Error
          ? membersQuery.error.message
          : companyAccessPage.failedToLoad;
    ...
    <div className="max-w-6xl space-y-8">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{companyAccessPage.title}</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          {companyAccessPage.description(selectedCompany?.name ?? "")}
        </p>
      </div>

      {access && !access.currentUserRole && (
        <div className="rounded-xl border border-amber-500/40 px-4 py-3 text-sm text-amber-200">
          {companyAccessPage.instanceAdminWarning}
        </div>
      )}
      ...
      <section className="space-y-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{companyAccessPage.humans}</h2>
          </div>
          <p className="max-w-3xl text-sm text-muted-foreground">
            {companyAccessPage.humansHint}
          </p>
        </div>
        ...
        {access?.canApproveJoinRequests && pendingHumanJoinRequests.length > 0 ? (
          <div className="space-y-3 rounded-xl border border-border px-4 py-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <h3 className="text-sm font-semibold">{companyAccessPage.pendingHumanJoins}</h3>
                <p className="text-sm text-muted-foreground">
                  {companyAccessPage.pendingHumanJoinsHint}
                </p>
              </div>
              <Badge variant="outline">{pendingHumanJoinRequests.length} {companyAccessPage.pending}</Badge>
            </div>
            <div className="space-y-3">
              {pendingHumanJoinRequests.map((request) => (
                <PendingJoinRequestCard
                  ...
                  title={
                    ...
                    companyAccessPage.unknownRequester
                  }
                  subtitle={
                    request.requesterUser?.email ||
                    request.requestEmailSnapshot ||
                    request.requestingUserId ||
                    companyAccessPage.noEmail
                  }
                  context={
                    request.invite
                      ? `${request.invite.allowedJoinTypes}${companyAccessPage.joinInviteSuffix}${request.invite.humanRole ? `${companyAccessPage.defaultRoleSuffix}${request.invite.humanRole}` : ""}`
                      : companyAccessPage.inviteMetadataUnavailable
                  }
                  detail={`${companyAccessPage.submitted}${new Date(request.createdAt).toLocaleString()}`}
                  approveLabel={companyAccessPage.approveHuman}
                  rejectLabel={companyAccessPage.rejectHuman}
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          <div className="grid grid-cols-[minmax(0,1.5fr)_120px_120px_minmax(0,1.2fr)_180px] gap-3 border-b border-border px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted-foreground">
            <div>{companyAccessPage.userAccount}</div>
            <div>{companyAccessPage.role}</div>
            <div>{companyAccessPage.status}</div>
            <div>{companyAccessPage.grants}</div>
            <div className="text-right">{companyAccessPage.action}</div>
          </div>
          {members.length === 0 ? (
            <div className="px-4 py-8 text-sm text-muted-foreground">{companyAccessPage.noMemberships}</div>
          ) : (
            members.map((member) => {
      ...
      <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMemberId(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{companyAccessPage.editMember}</DialogTitle>
            <DialogDescription>
              {companyAccessPage.editMemberDesc(editingMember?.user?.name || editingMember?.user?.email || editingMember?.principalId || "")}
            </DialogDescription>
          </DialogHeader>
          {editingMember && (
            <div className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{companyAccessPage.companyRole}</span>
                  <select
                  ...
                  >
                    <option value="">{companyAccessPage.unset}</option>
                ...
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{companyAccessPage.membershipStatus}</span>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2"
                    value={draftStatus}
                    onChange={(event) =>
                  ...
                  >
                    <option value="active">{companyAccessPage.active}</option>
                    <option value="pending">{companyAccessPage.pending}</option>
                    <option value="suspended">{companyAccessPage.suspended}</option>
                  </select>
                </label>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  <h3 className="text-sm font-medium">{companyAccessPage.grantsSection}</h3>
                  <p className="text-sm text-muted-foreground">
                    {companyAccessPage.grantsHint}
                  </p>
                </div>
                <div className="rounded-lg border border-border px-3 py-3">
                  <div className="text-sm font-medium">{companyAccessPage.implicitGrants}</div>
                  <p className="mt-1 text-sm text-muted-foreground">
                    {draftRole
                      ? companyAccessPage.implicitGrantsIncluded(HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[draftRole])
                      : companyAccessPage.noImplicitGrants}
                  </p>
                      ...
                      <span className="space-y-1">
                        ...
                        {implicitGrantSet.has(permissionKey) ? (
                          <span className="block text-xs text-muted-foreground">
                            {companyAccessPage.includedImplicitly(draftRole ? HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[draftRole] : companyAccessPage.unset)}
                          </span>
                        ) : null}
                        {draftGrants.has(permissionKey) ? (
                          <span className="block text-xs text-muted-foreground">
                            {companyAccessPage.storedExplicitly}
                          </span>
                        ) : null}
                      </span>
                    </label>
          ...
          <DialogFooter>
            ...
            >
              {updateMemberMutation.isPending ? companyAccessPage.saving : companyAccessPage.saveAccess}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!removingMember} onOpenChange={(open) => !open && setRemovingMemberId(null)}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>{companyAccessPage.removeMember}</DialogTitle>
            <DialogDescription>
              {companyAccessPage.removeMemberDesc(memberDisplayName(removingMember))}
            </DialogDescription>
          </DialogHeader>
          {removingMember && (
            <div className="space-y-5">
              <div className="rounded-lg border border-border px-3 py-3">
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  {assignedIssuesQuery.isLoading
                    ? companyAccessPage.checkingAssigned
                    : companyAccessPage.openAssigned(assignedIssues.length)}
                </div>
              </div>

              {assignedIssues.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-sm font-medium">{companyAccessPage.issueReassignment}</div>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                    value={reassignmentTarget}
                    onChange={(event) => setReassignmentTarget(event.target.value)}
                  >
                    <option value="__unassigned">{companyAccessPage.leaveUnassigned}</option>
                    {activeReassignmentUsers.length > 0 ? (
                      <optgroup label={companyAccessPage.humansOptgroup}>
                    ...
                    {activeReassignmentAgents.length > 0 ? (
                      <optgroup label={companyAccessPage.agentsOptgroup}>
                  ...
                  <div className="max-h-36 overflow-auto rounded-lg border border-border">
                    ...
                    {assignedIssues.length > 6 ? (
                      <div className="px-3 py-2 text-sm text-muted-foreground">
                        {companyAccessPage.moreIssues(assignedIssues.length - 6)}
                      </div>
                    ) : null}
                  </div>
                </div>
          ...
          <DialogFooter>
            ...
            >
              {archiveMemberMutation.isPending ? companyAccessPage.saving : companyAccessPage.removeMember}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
function memberDisplayName(member: CompanyMember | null) {
  if (!member) return companyAccessPage.unset;

...
function createIssueComment(
  indexOrOverrides: number | Partial<IssueComment> = 1,
  maybeOverrides: Partial<IssueComment> = {},
): IssueComment {
  const index = typeof indexOrOverrides === "number" ? indexOrOverrides : 1;
  const overrides = typeof indexOrOverrides === "number" ? maybeOverrides : indexOrOverrides;
...
describe("IssueDetail", () => {
  ...
  it("requests and renders issue comments in 50-comment pages", async () => {
    const pagedComments = Array.from({ length: 52 }, (_value, index) => createIssueComment(index + 1));
    mockIssuesApi.get.mockResolvedValue(createIssue());
    mockIssuesApi.listComments.mockResolvedValue([...pagedComments].reverse());
    ...
    expect(latestChatProps?.comments?.map((comment: IssueComment) => comment.id)).toEqual(
      Array.from({ length: 50 }, (_value, index) => `comment-${index + 3}`),
    );

...
export function PendingActions() {
  ...
  const refresh = () => qc.invalidateQueries({ queryKey: queryKeys.agnb.pending });
  const decide = async (id: string, decision: "approved" | "rejected") => {
    if (decision === "approved" && !confirm("Approve + execute this action?")) return;
    await opsApi.pendingDecision([id], decision).catch((e) => alert(e instanceof Error ? e.message : "Failed"));
    refresh();
  };

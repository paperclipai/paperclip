...
export function InviteLandingPage() {
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {t("inviteLanding.bootstrapCompleteDescription")}
          </p>
  ...
  if (result?.kind === "join") {
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("inviteLanding.joinSubmittedTitle")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("inviteLanding.joinSubmittedDescription")}
          </p>
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
            {t("inviteLanding.requestIdLabel")}: <span className="font-mono">{payload.id}</span>
          </div>
          {claimSecret && claimApiKeyPath && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("inviteLanding.claimSecretLabel")}</p>
          ...
          {(onboardingSkillUrl || onboardingSkillPath || onboardingInstallPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("inviteLanding.skillBootstrapLabel")}</p>
              ...
              {onboardingInstallPath && <p className="font-mono break-all">{t("inviteLanding.installToPrefix")} {onboardingInstallPath}</p>}
            </div>
          )}
          {(onboardingTextUrl || onboardingTextPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("inviteLanding.onboardingTextLabel")}</p>
          ...
          {diagnostics.length > 0 && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("inviteLanding.connectivityDiagnosticsLabel")}</p>
    ...
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">
          {invite.inviteType === "bootstrap_ceo" ? t("inviteLanding.bootstrapInstanceTitle") : t("inviteLanding.joinCompanyTitle")}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">{t("inviteLanding.inviteExpires", { date: dateTime(invite.expiresAt) })}</p>
        ...
        {invite.inviteType !== "bootstrap_ceo" && (
          <div className="mt-5 flex gap-2">
            {availableJoinTypes.map((type) => (
              ...
              >
                {type === "human" ? t("inviteLanding.joinAsHuman") : t("inviteLanding.joinAsAgent")}
              </button>
            ))}
          </div>
        )}
        ...
        {joinType === "agent" && invite.inviteType !== "bootstrap_ceo" && (
          <div className="mt-4 space-y-3">
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">{t("inviteLanding.agentNameLabel")}</span>
              <input
            ...
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">{t("inviteLanding.adapterTypeLabel")}</span>
              <select
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                value={adapterType}
              ...
              >
                {joinAdapterOptions.map((type) => (
                  <option key={type} value={type} disabled={!ENABLED_INVITE_ADAPTERS.has(type)}>
                    {adapterLabels[type]}{!ENABLED_INVITE_ADAPTERS.has(type) ? ` (${t("inviteLanding.comingSoon")})` : ""}
                  </option>
                ))}
              </select>
            </label>
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">{t("inviteLanding.capabilitiesLabel")}</span>
              <textarea
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                rows={4}
                value={capabilities}
        ...
        {requiresAuthForHuman && (
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-sm">
            {t("inviteLanding.signInBeforeHumanJoin")}
            <div className="mt-2">
              <Button asChild size="sm" variant="outline">
                <Link to={`/auth?next=${encodeURIComponent(`/invite/${token}`)}`}>{t("inviteLanding.signInOrCreateAccount")}</Link>
              </Button>
            </div>
          </div>
        )}
        ...
        >
          {acceptMutation.isPending
            ? t("inviteLanding.submitting")
            : invite.inviteType === "bootstrap_ceo"
              ? t("inviteLanding.acceptBootstrapButton")
              : t("inviteLanding.submitJoinButton")}
        </Button>
      </div>
    </div>
  );

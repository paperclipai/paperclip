...
export function InviteLandingPage() {
  ...
  const joinTypeLabels = useMemo(
    () => ({
      human: t("invite.join_type_human"),
      agent: t("invite.join_type_agent"),
    }),
    [t],
  );
  ...
  const acceptMutation = useMutation({
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : t("invite.accept_failed"));
    },
  });
  ...
  if (result?.kind === "bootstrap") {
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("invite.bootstrap_complete_title")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("invite.bootstrap_complete_body")}
          </p>
  ...
  if (result?.kind === "join") {
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("invite.join_request_submitted_title")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("invite.join_request_submitted_body")}
          </p>
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
            {t("invite.request_id")}: <span className="font-mono">{payload.id}</span>
          </div>
          {claimSecret && claimApiKeyPath && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("invite.claim_secret_title")}</p>
          ...
          {(onboardingSkillUrl || onboardingSkillPath || onboardingInstallPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("invite.skill_bootstrap_title")}</p>
              ...
              {onboardingInstallPath && (
                <p className="font-mono break-all">
                  {t("invite.install_to", { path: onboardingInstallPath })}
                </p>
              )}
            </div>
          )}
          {(onboardingTextUrl || onboardingTextPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("invite.onboarding_text_title")}</p>
          ...
          {diagnostics.length > 0 && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("invite.diagnostics_title")}</p>
              {diagnostics.map((diag, idx) => (
                <div key={`${diag.code}:${idx}`} className="space-y-0.5">
                  <p className={diag.level === "warn" ? "text-amber-600 dark:text-amber-400" : undefined}>
                    [{diag.level === "warn" ? t("invite.diagnostic_level_warn") : t("invite.diagnostic_level_info")}]{" "}
                    {diag.message}
                  </p>
      ...
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">
          {invite.inviteType === "bootstrap_ceo"
            ? t("invite.bootstrap_title")
            : companyName
              ? t("invite.join_company_title", { company: companyName })
              : t("invite.join_company_fallback_title")}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {invite.inviteType !== "bootstrap_ceo" && companyName
            ? `${t("invite.invited_to_company", { company: companyName })} `
            : null}
          {t("invite.expires", { value: formatDateTime(invite.expiresAt) })}
        </p>
        ...
        {invite.inviteType !== "bootstrap_ceo" && (
          <div className="mt-5 flex gap-2">
            {availableJoinTypes.map((type) => (
              ...
              >
                {t("invite.join_as_type", { type: joinTypeLabels[type] })}
              </button>
            ))}
          </div>
        )}
        ...
        {joinType === "agent" && invite.inviteType !== "bootstrap_ceo" && (
          <div className="mt-4 space-y-3">
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">{t("invite.agent_name")}</span>
              <input
            ...
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">{t("invite.adapter_type")}</span>
              <select
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                value={adapterType}
              ...
              >
                {joinAdapterOptions.map((type) => (
                  <option key={type} value={type} disabled={!ENABLED_INVITE_ADAPTERS.has(type)}>
                    ...
                    {!ENABLED_INVITE_ADAPTERS.has(type) ? ` (${t("invite.coming_soon")})` : ""}
                  </option>
                ))}
              </select>
            </label>
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">{t("invite.capabilities_optional")}</span>
              <textarea
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                rows={4}
                value={capabilities}
        ...
        {requiresAuthForHuman && (
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-sm">
            {t("invite.sign_in_required")}
            <div className="mt-2">
              <Button asChild size="sm" variant="outline">
                <Link to={`/auth?next=${encodeURIComponent(`/invite/${token}`)}`}>
                  {t("invite.sign_in_or_create")}
                </Link>
              </Button>
            </div>
          </div>
        ...
        >
          {acceptMutation.isPending
            ? t("invite.submitting")
            : invite.inviteType === "bootstrap_ceo"
              ? t("invite.accept_bootstrap_invite")
              : t("invite.submit_join_request")}
        </Button>
      </div>
    </div>
  );

...
function TimelineSummaryStats({
  ...
  const stats: { label: string; value: string; icon: LucideIcon }[] = [
    { label: t("app.timeline.runs", { defaultValue: "Runs" }), value: formatInteger(summary.runs), icon: GanttChartSquare },
    { label: t("app.timeline.agents", { defaultValue: "Agents" }), value: formatInteger(summary.agents), icon: Bot },
    { label: t("app.timeline.runTime", { defaultValue: "Run time" }), value: formatDuration(0, summary.activeMs), icon: Clock3 },
    {
      label: t("app.timeline.tokensUsed", { defaultValue: "Tokens used" }),
      value: summary.totalTokens > 0 ? formatCompactInteger(summary.totalTokens) : t("app.timeline.notTracked", { defaultValue: "Not tracked" }),
      icon: Coins,
    },
  ];
...
export function Timeline() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("app.pages.timeline", { defaultValue: "Timeline" }) }]);
  }, [setBreadcrumbs]);
  ...
  if (!selectedCompanyId) {
    return (
      <>
        ...
        <EmptyState icon={GanttChartSquare} message={t("app.timeline.selectACompanyToViewItsWorkTimeline", { defaultValue: "Select a company to view its work timeline." })} />
      </>
    );
  }
  ...
  const header = (
    <div className="flex items-center gap-2">
      ...
      <h1 className="text-3xl font-semibold tracking-tight">{t("app.timeline.workTimeline", { defaultValue: "Work Timeline" })}</h1>
    </div>
  );
  ...
  const rangeControls = (
    <label className="flex min-w-0 flex-wrap items-center gap-2 text-xs text-muted-foreground">
      {t("app.timeline.range", { defaultValue: "Range" })}<Segmented
        value={rangePreset}
        onChange={(preset) => {
        ...
        options={[
          { value: "today", label: t("app.timeline.today", { defaultValue: "Today" }) },
          { value: "7d", label: t("app.timeline.7Days", { defaultValue: "7 days" }) },
          { value: "30d", label: t("app.timeline.30Days", { defaultValue: "30 days" }) },
        ]}
      />
      <Input
        type="date"
        ...
        aria-label={t("app.timeline.timelineStartDate", { defaultValue: "Timeline start date" })}
      />
      <span>to</span>
      <Input
        type="date"
        ...
        className="h-8 w-(--sz-150px) text-xs"
        aria-label={t("app.timeline.timelineEndDate", { defaultValue: "Timeline end date" })}
      />
    </label>
  );
  ...
  const toolbar = (
    <div className="flex flex-wrap items-start gap-3">
      ...
      <div className="ml-auto flex items-center gap-1 pt-3" aria-label={t("app.timeline.timelineZoomControls", { defaultValue: "Timeline zoom controls" })}>
        <Button
          type="button"
          variant="outline"
          size="icon-xs"
          onClick={() => adjustZoom(0.8)}
          aria-label={t("app.timeline.zoomOut", { defaultValue: "Zoom out" })}
          title={t("app.timeline.zoomOut", { defaultValue: "Zoom out" })}
        >
        ...
        <Button
          ...
          aria-label={t("app.timeline.zoomIn", { defaultValue: "Zoom in" })}
          title={t("app.timeline.zoomIn", { defaultValue: "Zoom in" })}
        >
        ...
        <Button
          ...
          size="icon-xs"
          onClick={resetZoom}
          aria-label={t("app.timeline.resetZoom", { defaultValue: "Reset zoom" })}
          title={t("app.timeline.resetZoom", { defaultValue: "Reset zoom" })}
        >
  ...
  return (
    <div className="space-y-6">
      ...
      {error && (
        <EmptyState
          icon={GanttChartSquare}
          message={t("app.timeline.couldnTLoadTheTimelineTheAggregationEndpointMayBeUnavailable", { defaultValue: "Couldn't load the timeline. The aggregation endpoint may be unavailable." })}
        />
      )}
      ...
      {data && !isLoading && !dateRangeError && (
        data.spans.length === 0 ? (
          <div className="space-y-3">
            <EmptyState icon={GanttChartSquare} message={t("app.timeline.noActivityInThisWindow", { defaultValue: "No activity in this window." })} />
            <div className="flex flex-wrap items-center justify-end gap-3">
              {rangeControls}
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-xs text-muted-foreground">
                ...
                {new Date(data.window.from).toLocaleString()} {t("app.timeline.to", { defaultValue: "to " })}{new Date(data.window.to).toLocaleString()}
                {data.window.capped ? t("app.timeline.windowCapped", { defaultValue: " · window capped" }) : ""}
              </p>
              {rangeControls}
            </div>
          </div>

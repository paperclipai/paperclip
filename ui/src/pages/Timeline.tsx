...
function rangeError(range: DateRangeState): string | null {
  if (!range.fromDate || !range.toDate) return t("timeline.chooseDateRange");
  if (!rangeWindow(range)) return t("timeline.startBeforeEnd");
  return null;
}
...
export function Timeline() {
  ...
  const header = (
    <div className="flex items-center gap-2">
      ...
      <h1 className="text-3xl font-semibold tracking-tight">{t("timeline.pageTitle")}</h1>
    </div>
  );
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-xs text-muted-foreground">
                {t("timeline.runsInWindow", { count: data.spans.length })} ·{" "}
                {new Date(data.window.from).toLocaleString()} {t("timeline.to")} {new Date(data.window.to).toLocaleString()}

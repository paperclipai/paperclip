...
/**
 * Filter .odysseus.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterOdysseusYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://odysseus.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm odysseus company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Odysseus](https://odysseus.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Odysseus](https://odysseus.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .odysseus.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .odysseus.yaml
    const yamlPath = exportData.odysseusExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterOdysseusYaml(exportData.files[yamlPath], checkedFiles);
    }

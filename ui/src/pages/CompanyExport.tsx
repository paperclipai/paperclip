...
function ExportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("companyExport.preview.selectFile")} />
    );
  }
      ...
      <div className="min-h-[560px] px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
            {t("companyExport.preview.binaryUnavailable")}
          </div>
        )}
      </div>
    </div>
...
export function CompanyExport() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("companyExport.breadcrumb.orgChart"), href: "/org" },
      { label: t("companyExport.breadcrumb.export") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const exportPreviewMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyExport.toast.exportFailedTitle"),
        body: err instanceof Error ? err.message : t("companyExport.toast.exportFailedBody"),
      });
    },
  });
  ...
  const downloadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("companyExport.toast.downloadedTitle"),
        body: t("companyExport.toast.downloadedBody", { count: resultCheckedFiles.size, filename: `${result.rootPath}.zip` }),
      });
    },
    onError: (err) => {
      pushToast({
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-4 text-sm">
            <span className="font-medium">
              {t("companyExport.title", { name: selectedCompany?.name ?? "Company" })}
            </span>
            <span className="text-muted-foreground">
              {t("companyExport.filesSelected", { selected: selectedCount, total: totalFiles, count: totalFiles })}
            </span>
            {warnings.length > 0 && (
              <span className="text-amber-500">
                {t("companyExport.warnings", { count: warnings.length })}
              </span>
            )}
          </div>
          <Button
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {totalTaskChildren > visibleTaskChildren && !treeSearch && (
              <div className="px-4 py-2">
                ...
                >
                  {t("companyExport.showMoreIssues", { visible: visibleTaskChildren, total: totalTaskChildren })}
                </button>
              </div>
            )}
          </div>

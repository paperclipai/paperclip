...
const PROJECT_SORT_OPTIONS: Array<{ field: ProjectSortField; labelKey: string }> = [
  { field: "name", labelKey: "pages.projects.sort.name" },
  { field: "updated", labelKey: "pages.projects.sort.updated" },
  { field: "created", labelKey: "pages.projects.sort.created" },
  { field: "targetDate", labelKey: "pages.projects.sort.targetDate" },
];
...
export function Projects() {
  ...
  const sortLabel = t(
    PROJECT_SORT_OPTIONS.find((option) => option.field === sortField)?.labelKey
      ?? "pages.projects.sort.name",
  );
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="ghost" size="sm" className="w-fit text-xs" title={t("pages.projects.sortTitle")}>
              <ArrowUpDown className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
              <span>{t("pages.projects.sortLabel", { label: sortLabel })}</span>
            </Button>
          </PopoverTrigger>
                ...
                >
                  ...
                  {sortField === option.field ? (
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                      ...
                      {sortDir === "asc"
                        ? t("pages.projects.sort.asc")
                        : t("pages.projects.sort.desc")}
                    </span>
                  ) : null}
                </button>
              ))}

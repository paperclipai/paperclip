...
export function NotFoundPage({ scope = "global", requestedPrefix }: NotFoundPageProps) {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: "No encontrado" }]);
  }, [setBreadcrumbs]);
  ...
  const title = scope === "invalid_company_prefix" ? "Empresa no encontrada" : "Página no encontrada";
  const description =
    scope === "invalid_company_prefix"
      ? `No existe una empresa con el prefijo "${normalizedPrefix ?? "desconocido"}".`
      : "Esta ruta no existe.";
    ...
    <div className="mx-auto max-w-2xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <div className="mt-4 rounded-md border border-border bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          Ruta solicitada: <code className="font-mono">{currentPath}</code>
        </div>

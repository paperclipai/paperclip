...
export interface NotFoundPageProps {
  scope?: "global" | "invalid_company_prefix";
...
export function NotFoundPage({ scope = "global", requestedPrefix }: NotFoundPageProps) {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("page.not_found.title_page") }]);
  }, [setBreadcrumbs, t]);
  ...
  const title = scope === "invalid_company_prefix" ? t("page.not_found.title_company") : t("page.not_found.title_page");
  const description =
    scope === "invalid_company_prefix"
      ? t("page.not_found.desc_company", { prefix: normalizedPrefix ?? "unknown" })
      : t("page.not_found.desc_page");
    ...
    <div className="mx-auto max-w-2xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <div className="mt-4 rounded-md border border-border bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("page.not_found.requested_path")} <code className="font-mono">{currentPath}</code>
        </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <Button asChild>
            <Link to={dashboardHref}>
              ...
              {t("page.not_found.open_dashboard")}
            </Link>
          </Button>
          <Button variant="outline" asChild>
            <Link to="/">{t("page.not_found.go_home")}</Link>
          </Button>
        </div>
      </div>
    </div>

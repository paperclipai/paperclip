  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("pages.costs.financeLedgerTitle", { defaultValue: "Finance ledger" })}</CardTitle>
        <CardDescription>
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("pages.costs.debitsLabel", { defaultValue: "Debits" })}
          value={formatCents(debitCents)}
          subtitle={t("pages.costs.totalEventsInRange", { count: eventCount, defaultValue: "{{count}} total events in range" })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("pages.costs.creditsLabel", { defaultValue: "Credits" })}
          value={formatCents(creditCents)}
          subtitle={t("pages.costs.creditsSubtitle", { defaultValue: "Refunds, offsets, and credit returns" })}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("pages.costs.netLabel", { defaultValue: "Net" })}
          value={formatCents(netCents)}
          subtitle={t("pages.costs.netSubtitle", { defaultValue: "Debit minus credit for the selected period" })}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("pages.costs.estimatedLabel", { defaultValue: "Estimated" })}
...
export function Costs() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("pages.costs.breadcrumbCosts", { defaultValue: "Costs" }) }]);
  }, [setBreadcrumbs, t]);
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{t("pages.costs.pageTitle", { defaultValue: "Costs" })}</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("pages.costs.pageSubtitle", { defaultValue: "Inference spend, platform fees, credits, and live quota windows." })}
                </p>
            </div>
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{t("pages.costs.dateRangeTo", { defaultValue: "to" })}</span>
              <input
                type="date"
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("pages.costs.inferenceSpendLabel", { defaultValue: "Inference spend" })}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={t("pages.costs.inferenceSpendSubtitle", { tokens: formatTokens(inferenceTokenTotal), defaultValue: "{{tokens}} tokens across request-scoped events" })}
              icon={DollarSign}
            />
            <MetricTile
              label={t("pages.costs.budgetLabel", { defaultValue: "Budget" })}
              value={activeBudgetIncidents.length > 0 ? String(activeBudgetIncidents.length) : (
                spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                  ? `${spendData.summary.utilizationPercent}%`
                  : t("pages.costs.budgetOpen", { defaultValue: "Open" })
              )}
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("pages.costs.budgetPausedSummary", { agents: budgetData?.pausedAgentCount ?? 0, projects: budgetData?.pausedProjectCount ?? 0, defaultValue: "{{agents}} agents paused · {{projects}} projects paused" })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("pages.costs.budgetSpendOf", { spend: formatCents(spendData.summary.spendCents), budget: formatCents(spendData.summary.budgetCents), defaultValue: "{{spend}} of {{budget}}" })
                    : t("pages.costs.noMonthlyCap", { defaultValue: "No monthly cap configured" })
              }
              icon={Coins}
            />
            <MetricTile
              label={t("pages.costs.financeNetLabel", { defaultValue: "Finance net" })}
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={t("pages.costs.financeNetSubtitle", { debits: formatCents(financeData?.summary.debitCents ?? 0), credits: formatCents(financeData?.summary.creditCents ?? 0), defaultValue: "{{debits}} debits · {{credits}} credits" })}
              icon={ReceiptText}
            />
            <MetricTile
              label={t("pages.costs.financeEventsLabel", { defaultValue: "Finance events" })}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("pages.costs.financeEventsSubtitle", { estimated: formatCents(financeData?.summary.estimatedDebitCents ?? 0), defaultValue: "{{estimated}} estimated in range" })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
          ...
          {showCustomPrompt ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("pages.costs.inferenceLedgerTitle", { defaultValue: "Inference ledger" })}</CardTitle>
                    <CardDescription>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("pages.costs.budgetAmount", { budget: formatCents(spendData.summary.budgetCents), defaultValue: "Budget {{budget}}" })
                            : t("pages.costs.unlimitedBudget", { defaultValue: "Unlimited budget" })}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("pages.costs.usageLabel", { defaultValue: "usage" })}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("pages.costs.utilizationConsumed", { percent: spendData.summary.utilizationPercent, defaultValue: "{{percent}}% of monthly budget consumed in this range." })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("pages.costs.byAgentTitle", { defaultValue: "By agent" })}</CardTitle>
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("pages.costs.agentTokenInOut", { in: formatTokens(row.inputTokens + row.cachedInputTokens), out: formatTokens(row.outputTokens), defaultValue: "in {{in}} · out {{out}}" })}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {t("pages.costs.agentApiCount", { count: row.apiRunCount, defaultValue: "{{count}} api" })}
                                    {" · "}
                                    {t("pages.costs.agentSubscriptionCount", { count: row.subscriptionRunCount, defaultValue: "{{count}} subscription" })}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                                    ...
                                    >
                                      ...
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {t("pages.costs.modelTokens", { tokens: formatTokens(modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens), defaultValue: "{{tokens}} tok" })}
                                        </div>
                                      </div>
                                    </div>
                                  );
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("pages.costs.byProjectTitle", { defaultValue: "By project" })}</CardTitle>
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("pages.costs.budgetControlPlaneTitle", { defaultValue: "Budget control plane" })}</CardTitle>
                  <CardDescription>
                ...
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("pages.costs.activeIncidentsLabel", { defaultValue: "Active incidents" })}
                  ...
                  />
                  <MetricTile
                    label={t("pages.costs.pendingApprovalsLabel", { defaultValue: "Pending approvals" })}
                  ...
                  />
                  <MetricTile
                    label={t("pages.costs.pausedAgentsLabel", { defaultValue: "Paused agents" })}
                  ...
                  />
                  <MetricTile
                    label={t("pages.costs.pausedProjectsLabel", { defaultValue: "Paused projects" })}
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("pages.costs.activeIncidentsHeading", { defaultValue: "Active incidents" })}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("pages.costs.activeIncidentsHeadingDescription", { defaultValue: "Resolve hard stops here by raising the budget or explicitly keeping the scope paused." })}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                    ...
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">
                          {scopeType === "company"
                            ? t("pages.costs.companyBudgetsHeading", { defaultValue: "company budgets" })
                            : scopeType === "agent"
                              ? t("pages.costs.agentBudgetsHeading", { defaultValue: "agent budgets" })
                              : t("pages.costs.projectBudgetsHeading", { defaultValue: "project budgets" })}
                        </h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("pages.costs.companyBudgetsDescription", { defaultValue: "Company-wide monthly policy." })
                            : scopeType === "agent"
                              ? t("pages.costs.agentBudgetsDescription", { defaultValue: "Recurring monthly spend policies for individual agents." })
                              : t("pages.costs.projectBudgetsDescription", { defaultValue: "Lifetime spend policies for execution-bound projects." })}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("pages.costs.noBillableEventsInPeriod", { defaultValue: "No billable events in this period." })}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("pages.costs.byBillerTitle", { defaultValue: "By biller" })}</CardTitle>

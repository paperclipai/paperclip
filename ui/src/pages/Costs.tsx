...
function AllProvidersTabLabel({ tokenCount, costCents, hasProviders }: { tokenCount: number; costCents: number; hasProviders: boolean }) {
  const { t } = useTranslation();
  return (
    <span className="flex items-center gap-1.5">
      <span>{t("costs.allProviders", { defaultValue: "All providers" })}</span>
      {hasProviders ? (
        <>
          <span className="font-mono text-xs text-muted-foreground">{formatTokens(tokenCount)}</span>
          <span className="text-xs text-muted-foreground">{formatCents(costCents)}</span>
        </>
      ) : null}
    </span>
  );
...
function AllBillersTabLabel({ tokenCount, costCents, hasBillers }: { tokenCount: number; costCents: number; hasBillers: boolean }) {
  const { t } = useTranslation();
  return (
    <span className="flex items-center gap-1.5">
      <span>{t("costs.allBillers", { defaultValue: "All billers" })}</span>
      {hasBillers ? (
        <>
          <span className="font-mono text-xs text-muted-foreground">{formatTokens(tokenCount)}</span>
          <span className="text-xs text-muted-foreground">{formatCents(costCents)}</span>
  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("costs.financeLedger", { defaultValue: "Finance ledger" })}</CardTitle>
        <CardDescription>
          {t("costs.financeLedgerDescription", { defaultValue: "Account-level charges that do not map to a single inference request." })}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("costs.metrics.debits", { defaultValue: "Debits" })}
          value={formatCents(debitCents)}
          subtitle={t("costs.metrics.debitsSubtitle", { count: eventCount, defaultValue: `${eventCount} total event${eventCount === 1 ? "" : "s"} in range` })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("costs.metrics.credits", { defaultValue: "Credits" })}
          value={formatCents(creditCents)}
          subtitle={t("costs.metrics.creditsSubtitle", { defaultValue: "Refunds, offsets, and credit returns" })}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("costs.metrics.net", { defaultValue: "Net" })}
          value={formatCents(netCents)}
          subtitle={t("costs.metrics.netSubtitle", { defaultValue: "Debit minus credit for the selected period" })}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("costs.metrics.estimated", { defaultValue: "Estimated" })}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("costs.metrics.estimatedSubtitle", { defaultValue: "Estimated debits that are not yet invoice-authoritative" })}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("sidebar.costs", { defaultValue: "Costs" }) }]);
  }, [setBreadcrumbs, t]);
  ...
  const providerTabItems = useMemo(() => {
    ...
    return [
      {
        value: "all",
        label: <AllProvidersTabLabel tokenCount={allTokens} costCents={allCents} hasProviders={providerKeys.length > 0} />,
      },
  ...
  const billerTabItems = useMemo(() => {
    ...
    return [
      {
        value: "all",
        label: <AllBillersTabLabel tokenCount={allTokens} costCents={allCents} hasBillers={billerKeys.length > 0} />,
      },
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={t("costs.selectCompany", { defaultValue: "Select a company to view costs." })} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{t("sidebar.costs", { defaultValue: "Costs" })}</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("costs.subtitle", { defaultValue: "Inference spend, platform fees, credits, and live quota windows." })}
                </p>
            </div>
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{t("costs.dateRangeTo", { defaultValue: "to" })}</span>
              <input
                type="date"
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              ...
              subtitle={t("costs.metrics.inferenceSpendSubtitle", { tokens: formatTokens(inferenceTokenTotal), defaultValue: `${formatTokens(inferenceTokenTotal)} tokens across request-scoped events` })}
              icon={DollarSign}
            />
            <MetricTile
              label={t("costs.metrics.budget", { defaultValue: "Budget" })}
              value={activeBudgetIncidents.length > 0 ? String(activeBudgetIncidents.length) : (
                spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                  ? `${spendData.summary.utilizationPercent}%`
                  : t("costs.metrics.budgetOpen", { defaultValue: "Open" })
              )}
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("costs.metrics.budgetPaused", {
                  ...
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("costs.metrics.budgetOf", {
                        spend: formatCents(spendData.summary.spendCents),
                        cap: formatCents(spendData.summary.budgetCents),
                        defaultValue: `${formatCents(spendData.summary.spendCents)} of ${formatCents(spendData.summary.budgetCents)}`,
                      })
                    : t("costs.metrics.noMonthlyCap", { defaultValue: "No monthly cap configured" })
              }
              icon={Coins}
            />
            <MetricTile
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          <TabsTrigger value="overview">{t("costs.tabs.overview", { defaultValue: "Overview" })}</TabsTrigger>
          <TabsTrigger value="budgets">{t("costs.tabs.budgets", { defaultValue: "Budgets" })}</TabsTrigger>
          <TabsTrigger value="providers">{t("costs.tabs.providers", { defaultValue: "Providers" })}</TabsTrigger>
          <TabsTrigger value="billers">{t("costs.tabs.billers", { defaultValue: "Billers" })}</TabsTrigger>
          <TabsTrigger value="finance">{t("costs.tabs.finance", { defaultValue: "Finance" })}</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costs.selectDateRange", { defaultValue: "Select a start and end date to load data." })}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs.inferenceLedger", { defaultValue: "Inference ledger" })}</CardTitle>
                    <CardDescription>
                      {t("costs.inferenceLedgerDescription", { defaultValue: "Request-scoped inference spend for the selected period." })}
                    </CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("costs.budgetAmount", { amount: formatCents(spendData.summary.budgetCents), defaultValue: `Budget ${formatCents(spendData.summary.budgetCents)}` })
                            : t("costs.unlimitedBudget", { defaultValue: "Unlimited budget" })}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("costs.usageLabel", { defaultValue: "usage" })}</div>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs.byAgent", { defaultValue: "By agent" })}</CardTitle>
                    <CardDescription>{t("costs.byAgentDescription", { defaultValue: "What each agent consumed in the selected period." })}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("costs.noCostEvents", { defaultValue: "No cost events yet." })}</p>
                    ) : (
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("costs.inOutTokens", {
                                    ...
                                    defaultValue: `in ${formatTokens(row.inputTokens + row.cachedInputTokens)} · out ${formatTokens(row.outputTokens)}`,
                                  })}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {t("costs.apiCount", { count: row.apiRunCount, defaultValue: `${row.apiRunCount} api` })}
                                    {" · "}
                                    {t("costs.subscriptionCount", { count: row.subscriptionRunCount, defaultValue: `${row.subscriptionRunCount} subscription` })}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs.byProject", { defaultValue: "By project" })}</CardTitle>
                      <CardDescription>{t("costs.byProjectDescription", { defaultValue: "Run costs attributed through project-linked issues." })}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.noProjectRunCosts", { defaultValue: "No project-attributed run costs yet." })}</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? t("costs.unattributed", { defaultValue: "Unattributed" })}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costs.noFinanceEventsEmpty", { defaultValue: "No finance events yet. Add account-level charges once biller invoices or credits land." })} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("costs.budgetControlPlane", { defaultValue: "Budget control plane" })}</CardTitle>
                  <CardDescription>
                    {t("costs.budgetControlPlaneDescription", { defaultValue: "Hard-stop spend limits for agents and projects. Provider subscription quota stays separate and appears under Providers." })}
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("costs.metrics.activeIncidents", { defaultValue: "Active incidents" })}
                    value={String(activeBudgetIncidents.length)}
                    subtitle={t("costs.metrics.activeIncidentsSubtitle", { defaultValue: "Open soft or hard threshold crossings" })}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("costs.metrics.pendingApprovals", { defaultValue: "Pending approvals" })}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("costs.metrics.pendingApprovalsSubtitle", { defaultValue: "Budget override approvals awaiting board action" })}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("costs.metrics.pausedAgents", { defaultValue: "Paused agents" })}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("costs.metrics.pausedAgentsSubtitle", { defaultValue: "Agent heartbeats blocked by budget" })}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("costs.metrics.pausedProjects", { defaultValue: "Paused projects" })}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("costs.metrics.pausedProjectsSubtitle", { defaultValue: "Project execution blocked by budget" })}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("costs.activeIncidentsHeading", { defaultValue: "Active incidents" })}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("costs.activeIncidentsBody", { defaultValue: "Resolve hard stops here by raising the budget or explicitly keeping the scope paused." })}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                    ...
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">
                          {scopeType === "company"
                            ? t("costs.companyBudgets", { defaultValue: "Company budgets" })
                            : scopeType === "agent"
                              ? t("costs.agentBudgets", { defaultValue: "Agent budgets" })
                              : t("costs.projectBudgets", { defaultValue: "Project budgets" })}
                        </h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costs.companyBudgetDescription", { defaultValue: "Company-wide monthly policy." })
                            : scopeType === "agent"
                              ? t("costs.agentBudgetDescription", { defaultValue: "Recurring monthly spend policies for individual agents." })
                              : t("costs.projectBudgetDescription", { defaultValue: "Lifetime spend policies for execution-bound projects." })}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      {t("costs.noBudgetPolicies", { defaultValue: "No budget policies yet. Set agent and project budgets from their detail pages, or use the existing company monthly budget control." })}
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs.noCostEventsPeriod", { defaultValue: "No cost events in this period." })}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs.noBillableEvents", { defaultValue: "No billable events in this period." })}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs.byBiller", { defaultValue: "By biller" })}</CardTitle>
                      <CardDescription>{t("costs.byBillerDescription", { defaultValue: "Account-level financial events grouped by who charged or credited them." })}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.noFinanceEvents", { defaultValue: "No finance events yet." })}</p>
                      ) : (

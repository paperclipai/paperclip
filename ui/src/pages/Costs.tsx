    ...
    <Card>
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          ...
          subtitle={t("costs.totalEvents", { count: eventCount, defaultValue: `${eventCount} total events in range` })}
          icon={ArrowUpRight}
        />
        <MetricTile
        ...
        <MetricTile
          ...
          subtitle={t("costs.estimatedInRangeSubtitle", { defaultValue: `Estimated debits that are not yet invoice-authoritative` })}
          icon={Coins}
        />
      </CardContent>
    </Card>
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {PRESET_KEYS.map((key) => (
                ...
                >
                  {t(`costs.datePresets.${key}`, PRESET_LABELS[key])}
                </Button>
              ))}
            </div>
          </div>
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              ...
              subtitle={t("costs.tokensAcross", { count: inferenceTokenTotal, defaultValue: `${formatTokens(inferenceTokenTotal)} tokens across request-scoped events` })}
              icon={DollarSign}
            />
            <MetricTile
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("costs.pausedStatus", { agents: budgetData?.pausedAgentCount ?? 0, projects: budgetData?.pausedProjectCount ?? 0, defaultValue: `${budgetData?.pausedAgentCount ?? 0} agents paused · ${budgetData?.pausedProjectCount ?? 0} projects paused` })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("costs.budgetUsage", { spend: formatCents(spendData.summary.spendCents), budget: formatCents(spendData.summary.budgetCents), defaultValue: `${formatCents(spendData.summary.spendCents)} of ${formatCents(spendData.summary.budgetCents)}` })
            ...
            <MetricTile
              ...
              subtitle={t("costs.financeSummary", { debits: formatCents(financeData?.summary.debitCents ?? 0), credits: formatCents(financeData?.summary.creditCents ?? 0), defaultValue: `${formatCents(financeData?.summary.debitCents ?? 0)} debits · ${formatCents(financeData?.summary.creditCents ?? 0)} credits` })}
              icon={ReceiptText}
            />
            <MetricTile
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costs.customRangePrompt")}</p>
          ) : showOverviewLoading ? (
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("costs.budgetConsumed", { percent: spendData.summary.utilizationPercent, defaultValue: `${spendData.summary.utilizationPercent}% of monthly budget consumed in this range.` })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("costs.noUsage")}</p>
                    ) : (
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("costs.budgetPlane")}</CardTitle>
                  <CardDescription>
                    {t("costs.budgetPlaneDesc")}
                  </CardDescription>
                </CardHeader>
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{t(`costs.${scopeType}Budgets`)}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costs.companyPolicyDesc")
                            : scopeType === "agent"
                              ? t("costs.agentPolicyDesc")
                              : t("costs.projectPolicyDesc")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                  ...
                  <Card>
                    ...
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.noFinanceEventsBrief")}</p>
                      ) : (

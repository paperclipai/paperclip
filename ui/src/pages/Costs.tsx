  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("page.costs.finance_ledger.title")}</CardTitle>
        <CardDescription>
          {t("page.costs.finance_ledger.description")}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("page.costs.finance_ledger.debits")}
          value={formatCents(debitCents)}
          subtitle={t("page.costs.finance_ledger.events_in_range", { count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("page.costs.finance_ledger.credits")}
          value={formatCents(creditCents)}
          subtitle={t("page.costs.finance_ledger.credits_description")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("page.costs.finance_ledger.net")}
          value={formatCents(netCents)}
          subtitle={t("page.costs.finance_ledger.net_description")}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("page.costs.finance_ledger.estimated")}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("page.costs.finance_ledger.estimated_description")}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("page.costs.providers.all")}</span>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("page.costs.billers.all")}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={t("page.costs.select_company")} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {PRESET_KEYS.map((key) => (
                ...
                >
                  {t(`page.costs.presets.${key}`, { defaultValue: PRESET_LABELS[key] })}
                </Button>
              ))}
            </div>
          </div>
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{t("page.costs.to")}</span>
              <input
                type="date"
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("page.costs.metrics.inference_spend")}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={t("page.costs.metrics.inference_spend_subtitle", { tokens: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              label={t("page.costs.metrics.budget")}
              value={activeBudgetIncidents.length > 0 ? String(activeBudgetIncidents.length) : (
                spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                  ? `${spendData.summary.utilizationPercent}%`
                  : t("page.costs.metrics.open")
              )}
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("page.costs.metrics.paused_summary", {
                      agents: budgetData?.pausedAgentCount ?? 0,
                      projects: budgetData?.pausedProjectCount ?? 0,
                    })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("page.costs.metrics.of_budget", {
                        spend: formatCents(spendData.summary.spendCents),
                        budget: formatCents(spendData.summary.budgetCents),
                      })
                    : t("page.costs.metrics.no_monthly_cap")
              }
              icon={Coins}
            />
            <MetricTile
              label={t("page.costs.metrics.finance_net")}
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={t("page.costs.metrics.finance_net_subtitle", {
            ...
            />
            <MetricTile
              label={t("page.costs.metrics.finance_events")}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("page.costs.metrics.estimated_in_range", {
                value: formatCents(financeData?.summary.estimatedDebitCents ?? 0),
              })}
              icon={ArrowUpRight}
            />
          </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          <TabsTrigger value="overview">{t("page.costs.tabs.overview")}</TabsTrigger>
          <TabsTrigger value="budgets">{t("page.costs.tabs.budgets")}</TabsTrigger>
          <TabsTrigger value="providers">{t("page.costs.tabs.providers")}</TabsTrigger>
          <TabsTrigger value="billers">{t("page.costs.tabs.billers")}</TabsTrigger>
          <TabsTrigger value="finance">{t("page.costs.tabs.finance")}</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("page.costs.select_range")}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("page.costs.inference_ledger.title")}</CardTitle>
                    <CardDescription>
                      {t("page.costs.inference_ledger.description")}
                    </CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("page.costs.inference_ledger.budget_value", { value: formatCents(spendData.summary.budgetCents) })
                            : t("page.costs.inference_ledger.unlimited_budget")}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("page.costs.inference_ledger.usage")}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("page.costs.inference_ledger.utilization", { percent: spendData.summary.utilizationPercent })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("page.costs.by_agent.title")}</CardTitle>
                    <CardDescription>{t("page.costs.by_agent.description")}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("page.costs.by_agent.empty")}</p>
                    ) : (
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("page.costs.by_project.title")}</CardTitle>
                      <CardDescription>{t("page.costs.by_project.description")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("page.costs.by_project.empty")}</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? t("page.costs.by_project.unattributed")}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("page.costs.finance_timeline.empty")} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("page.costs.budgets_section.title")}</CardTitle>
                  <CardDescription>
                    {t("page.costs.budgets_section.description")}
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("page.costs.budgets_section.metrics.active_incidents.label")}
                    value={String(activeBudgetIncidents.length)}
                    subtitle={t("page.costs.budgets_section.metrics.active_incidents.subtitle")}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("page.costs.budgets_section.metrics.pending_approvals.label")}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("page.costs.budgets_section.metrics.pending_approvals.subtitle")}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("page.costs.budgets_section.metrics.paused_agents.label")}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("page.costs.budgets_section.metrics.paused_agents.subtitle")}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("page.costs.budgets_section.metrics.paused_projects.label")}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("page.costs.budgets_section.metrics.paused_projects.subtitle")}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("page.costs.budgets_section.active_incidents_title")}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("page.costs.budgets_section.active_incidents_description")}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                    ...
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">
                          {t(`page.costs.budgets_section.scope_title.${scopeType}`)}
                        </h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("page.costs.budgets_section.scope_description.company")
                            : scopeType === "agent"
                              ? t("page.costs.budgets_section.scope_description.agent")
                              : t("page.costs.budgets_section.scope_description.project")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      {t("page.costs.budgets_section.empty")}
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("page.costs.providers.empty")}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("page.costs.billers.empty")}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("page.costs.by_biller.title")}</CardTitle>
                      <CardDescription>{t("page.costs.by_biller.description")}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("page.costs.by_biller.empty")}</p>
                      ) : (

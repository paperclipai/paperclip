    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costs.all-providers-rrx")}</span>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costs.all-billers-1go")}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
      ...
      {showSummaryChrome ? (
        <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              {embedded ? (
                <h2 className="text-lg font-semibold text-foreground">{t("costs.costs-1fk")}</h2>
              ) : (
                <h1 className="text-3xl font-semibold tracking-tight">{t("costs.costs-1fk")}</h1>
              )}
              <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                {t("costs.inference-spend-platform-fees-credit-13p")}
              </p>
            </div>
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("costs.inference-spend-wdx")}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={`${formatTokens(inferenceTokenTotal)} tokens across request-scoped events`}
              icon={DollarSign}
            />
            <MetricTile
              label={t("costs.budget-aue")}
            ...
            />
            <MetricTile
              label={t("costs.finance-events-656")}
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        {!lockTab ? (
          <TabsList variant="line" className="justify-start">
            <TabsTrigger value="overview">{t("costs.overview-thn")}</TabsTrigger>
            {!hideBudgetsTab ? <TabsTrigger value="budgets">{t("costs.budgets-rb4")}</TabsTrigger> : null}
            <TabsTrigger value="providers">{t("costs.providers-awn")}</TabsTrigger>
            <TabsTrigger value="billers">{t("costs.billers-8x6")}</TabsTrigger>
            <TabsTrigger value="finance">{t("costs.finance-1iw")}</TabsTrigger>
          </TabsList>
        ) : null}

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costs.select-a-start-and-end-date-to-load-1k7")}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-31)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs.inference-ledger-1ay")}</CardTitle>
                    <CardDescription>
                      {t("costs.request-scoped-inference-spend-for-t-1qj")}
                    </CardDescription>
                  </CardHeader>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {spendData.summary.utilizationPercent}{t("costs.of-monthly-budget-consumed-in-this-r-12v")}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-32)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs.by-agent-1qi")}</CardTitle>
                    <CardDescription>{t("costs.what-each-agent-consumed-in-the-sele-pu1")}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("costs.no-cost-events-yet-18x")}</p>
                    ) : (
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  in {formatTokens(row.inputTokens + row.cachedInputTokens)} {t("costs.out-41i")} {formatTokens(row.outputTokens)}
                                </div>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs.by-project-1ce")}</CardTitle>
                      <CardDescription>{t("costs.run-costs-attributed-through-project-12v")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.no-project-attributed-run-costs-yet-1q7")}</p>
                      ) : (
            ...
            <>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("costs.active-incidents-rjs")}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("costs.resolve-hard-stops-here-by-raising-t-xeg")}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      {t("costs.no-budget-policies-yet-set-agent-and-g95")}
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs.no-cost-events-in-this-period-36w")}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs.no-billable-events-in-this-period-1tx")}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-33)">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs.by-biller-ri9")}</CardTitle>
                      <CardDescription>{t("costs.account-level-financial-events-group-l34")}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.no-finance-events-yet-15b")}</p>
                      ) : (

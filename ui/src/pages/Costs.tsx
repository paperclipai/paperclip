    ...
    <Card>
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricTile
          ...
          subtitle={t("costs.creditsSubtitle")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("costs.net")}
          value={formatCents(netCents)}
          subtitle={t("costs.netSubtitle")}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("costs.estimated")}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("costs.estimatedSubtitle")}
          icon={Coins}
        />
      </CardContent>
    </Card>
      ...
      <div className="space-y-5">
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              ...
              subtitle={t("costs.tokensAcrossRequests", { tokens: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("costs.agentsPausedProjectsPaused", {
            ...
            <MetricTile
              ...
              subtitle={t("costs.debitsCredits", {
            ...
            <MetricTile
              ...
              subtitle={t("costs.estimatedInRange", {
            ...
            <>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("costs.budgetLabel", { budget: formatCents(spendData.summary.budgetCents) })
                            : t("costs.unlimitedBudget")}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("costs.usageLabel")}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("costs.percentMonthlyBudget", { percent: spendData.summary.utilizationPercent })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {t("costs.apiCount", { count: row.apiRunCount })}
                                    {" · "}
                                    {t("costs.subscriptionCount", { count: row.subscriptionRunCount })}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                                    ...
                                    >
                                      ...
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {t("costs.tokensAbbr", { tokens: formatTokens(modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens) })}
                                        </div>
                                      </div>
                                    </div>
                                  );
                ...
                <div className="space-y-4">
                  <Card>
                    ...
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.noProjectAttributed")}</p>
                      ) : (
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costs.noFinanceEventsTimeline")} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("costs.activeIncidentsHeading")}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("costs.activeIncidentsInstruction")}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">

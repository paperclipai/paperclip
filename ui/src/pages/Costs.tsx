  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        ...
        <CardDescription>
          {t("finance.ledgerDescription")}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("finance.debits")}
          value={formatCents(debitCents)}
          subtitle={t("finance.debitsSubtitle", { count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("finance.credits")}
          value={formatCents(creditCents)}
          subtitle={t("finance.creditsSubtitle")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("finance.net")}
          value={formatCents(netCents)}
          subtitle={t("finance.netSubtitle")}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("finance.estimated")}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("finance.estimatedSubtitle")}
          icon={Coins}
        />
      </CardContent>
    </Card>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("providers.allProviders")}</span>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("billers.allBillers")}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("metrics.inferenceSpend")}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={t("metrics.tokensAcrossEvents", { tokens: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("metrics.agentsPaused", { agents: budgetData?.pausedAgentCount ?? 0, projects: budgetData?.pausedProjectCount ?? 0 })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("metrics.spendOfBudget", { spent: formatCents(spendData.summary.spendCents), budget: formatCents(spendData.summary.budgetCents) })
                    : t("metrics.noMonthlyCap")
              }
              icon={Coins}
            />
            <MetricTile
              label={t("metrics.financeNet")}
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={t("metrics.debitsCredits", { debits: formatCents(financeData?.summary.debitCents ?? 0), credits: formatCents(financeData?.summary.creditCents ?? 0) })}
              icon={ReceiptText}
            />
            <MetricTile
              label={t("metrics.financeEvents")}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("metrics.estimatedInRange", { estimated: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
          ...
          {showCustomPrompt ? (
            ...
            <>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("inference.budget", { budget: formatCents(spendData.summary.budgetCents) })
                            : t("inference.unlimitedBudget")}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("inference.usage")}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("inference.percentConsumed", { percent: spendData.summary.utilizationPercent })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("byAgent.title")}</CardTitle>
                    <CardDescription>{t("byAgent.description")}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("byAgent.noEvents")}</p>
                    ) : (
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("byAgent.in")} {formatTokens(row.inputTokens + row.cachedInputTokens)} · {t("byAgent.out")} {formatTokens(row.outputTokens)}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {t("byAgent.apiRuns", { count: row.apiRunCount })}
                                    {" · "}
                                    {t("byAgent.subscriptionRuns", { count: row.subscriptionRunCount })}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                                    ...
                                    >
                                      ...
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {formatTokens(modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens)} {t("byAgent.tokensAbbreviation")}
                                        </div>
                                      </div>
                                    </div>
                                  );
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("byProject.title")}</CardTitle>
                      <CardDescription>{t("byProject.description")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("byProject.noCosts")}</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? t("byProject.unattributed")}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("finance.noEvents")} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("budget.controlPlane")}</CardTitle>
                  <CardDescription>
                    {t("budget.controlPlaneDescription")}
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("budget.activeIncidents")}
                    value={String(activeBudgetIncidents.length)}
                    subtitle={t("budget.activeIncidentsSubtitle")}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("budget.pendingApprovals")}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("budget.pendingApprovalsSubtitle")}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("budget.pausedAgents")}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("budget.pausedAgentsSubtitle")}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("budget.pausedProjects")}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("budget.pausedProjectsSubtitle")}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("budget.activeIncidentsTitle")}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("budget.activeIncidentsDescription")}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold">{t(`budget.${scopeType}Budgets`)}</h2>
                        <p className="text-sm text-muted-foreground">
                          {t(`budget.${scopeType}BudgetsDescription`)}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("providers.noEvents")}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("providers.noBillableEvents")}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("finance.byBiller")}</CardTitle>
                      <CardDescription>{t("finance.byBillerDescription")}</CardDescription>
                    </CardHeader>

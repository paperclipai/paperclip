...
function ProviderTabLabel({
  ...
  const totalTokens = rows.reduce(
    (sum, row) =>
      sum + row.inputTokens + row.cachedInputTokens + row.outputTokens,
    0,
  );
...
export function Costs() {
  ...
  const { data: providerData } = useQuery({
    queryKey: queryKeys.usageByProvider(
      companyId,
      from || undefined,
      to || undefined,
    ),
    ...
    enabled:
      !!selectedCompanyId &&
      customReady &&
      (mainTab === "providers" || mainTab === "billers"),
    refetchInterval: 30_000,
    staleTime: 10_000,
  });

  const { data: billerData } = useQuery({
    queryKey: queryKeys.usageByBiller(
      companyId,
      from || undefined,
      to || undefined,
    ),
    queryFn: () =>
      costsApi.byBiller(companyId, from || undefined, to || undefined),
  ...
  const { data: weekData } = useQuery({
    ...
    enabled:
      !!selectedCompanyId && (mainTab === "providers" || mainTab === "billers"),
    refetchInterval: 30_000,
    staleTime: 10_000,
  });
  ...
  const deficitNotchByProvider = useMemo(() => {
    ...
    for (const [providerKey, rows] of byProvider) {
      const providerCostCents = rows.reduce(
      ...
      map.set(
        providerKey,
        providerCostCents + burnRate * (daysInMonth - daysElapsed) >
          providerBudget,
      );
    }
    return map;
  ...
  const effectiveBiller =
    activeBiller === "all" || billers.includes(activeBiller)
      ? activeBiller
      : "all";
  useEffect(() => {
  ...
  const providerTabItems = useMemo(() => {
    ...
    const allTokens = providerKeys.reduce(
      (sum, provider) =>
        ...
        (byProvider
          ...
          ?.reduce(
            (acc, row) =>
              acc + row.inputTokens + row.cachedInputTokens + row.outputTokens,
            0,
          ) ?? 0),
      0,
    );
    const allCents = providerKeys.reduce(
      (sum, provider) =>
        ...
        (byProvider
          .get(provider)
          ?.reduce((acc, row) => acc + row.costCents, 0) ?? 0),
      0,
    );
    return [
      {
  ...
  const billerTabItems = useMemo(() => {
    ...
    const allCents = billerKeys.reduce(
      (sum, biller) =>
        sum +
        (byBiller.get(biller)?.reduce((acc, row) => acc + row.costCents, 0) ??
          0),
      0,
    );
    return [
      ...
      ...billerKeys.map((biller) => ({
        value: biller,
        label: (
          <BillerTabLabel biller={biller} rows={byBiller.get(biller) ?? []} />
        ),
      })),
    ];
  }, [byBiller]);
  ...
  const budgetPoliciesByScope = useMemo(
    () => ({
      company: budgetPolicies.filter(
        (policy) => policy.scopeType === "company",
      ),
      agent: budgetPolicies.filter((policy) => policy.scopeType === "agent"),
      project: budgetPolicies.filter(
        (policy) => policy.scopeType === "project",
      ),
    }),
    [budgetPolicies],
  );
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                      ...
                      <div className="space-y-2">
                        <div className="h-2 overflow-hidden rounded-full bg-muted">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, spendData.summary.utilizationPercent)}%`,
                            }}
                          />
                        </div>
                        <div className="text-xs text-muted-foreground">
                          {spendData.summary.utilizationPercent}% of monthly
                          budget consumed in this range.
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
                ...
                <FinanceSummaryCard
                  ...
                  estimatedDebitCents={
                    financeData?.summary.estimatedDebitCents ?? 0
                  }
                              ...
                              <div className="mt-3 space-y-2 border-l border-border pl-4">
                                {modelRows.map((modelRow) => {
                                  const sharePct =
                                    row.costCents > 0
                                      ? Math.round(
                                          (modelRow.costCents / row.costCents) *
                                            100,
                                        )
                                      : 0;
                                  return (
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  ...
                  <CardDescription>
                    Hard-stop spend limits for agents and projects. Provider
                    subscription quota stays separate and appears under
                    Providers.
                  </CardDescription>
                </CardHeader>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Resolve hard stops here by raising the budget or
                      explicitly keeping the scope paused.
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      No budget policies yet. Set agent and project budgets from
                      their detail pages, or use the existing company monthly
                      budget control.
                    </CardContent>
                  </Card>
                ) : null}
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      {providers.map((provider) => (
                        <ProviderQuotaCard
                          ...
                          weekSpendCents={
                            weekSpendByProvider.get(provider) ?? 0
                          }
                          windowRows={windowSpendByProvider.get(provider) ?? []}
                          showDeficitNotch={
                            deficitNotchByProvider.get(provider) ?? false
                          }
                          quotaWindows={
                            quotaWindowsByProvider.get(provider) ?? []
                          }
                          quotaError={
                            quotaErrorsByProvider.get(provider) ?? null
                          }
                          quotaSource={
                  ...
                  {billers.length === 0 ? (
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
                        ...
                        if (!row) return null;
                        const providerRows = (providerData ?? []).filter(
                          (entry) => entry.biller === biller,
                        );
                        return (
                          <BillerSpendCard
                            key={biller}
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>
                        Account-level financial events grouped by who charged or
                        credited them.
                      </CardDescription>
                    </CardHeader>

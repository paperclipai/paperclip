...
export function Costs() {
  ...
  const presetLabels = useMemo(
    (): Record<DatePreset, string> => ({
      mtd: t("costs.presetMonthToDate"),
      "7d": t("costs.presetLast7Days"),
      "30d": t("costs.presetLast30Days"),
      ytd: t("costs.presetYearToDate"),
      all: t("costs.presetAllTime"),
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("costs.metricInferenceSpend")}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={t("costs.metricInferenceSpendSubtitle", {
                tokens: formatTokens(inferenceTokenTotal),
              })}
              icon={DollarSign}
            />
            <MetricTile
              label={t("costs.metricBudget")}
              value={activeBudgetIncidents.length > 0 ? String(activeBudgetIncidents.length) : (
                spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                  ? `${spendData.summary.utilizationPercent}%`
                  : t("costs.metricBudgetOpen")
              )}
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("costs.metricBudgetPaused", {
                    agents: budgetData?.pausedAgentCount ?? 0,
                    projects: budgetData?.pausedProjectCount ?? 0,
                  })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("costs.metricBudgetConfigured", {
                      spend: formatCents(spendData.summary.spendCents),
                      budget: formatCents(spendData.summary.budgetCents),
                    })
                    : t("costs.metricBudgetNoCap")
              }
              icon={Coins}
            />
            <MetricTile
              label={t("costs.metricFinanceNet")}
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={t("costs.metricFinanceNetSubtitle", {
            ...
            />
            <MetricTile
              label={t("costs.metricFinanceEvents")}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("costs.metricFinanceEventsSubtitle", {
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costs.selectDateRangePrompt")}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs.inferenceLedgerTitle")}</CardTitle>
                    <CardDescription>{t("costs.inferenceLedgerDescription")}</CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("costs.budgetAmount", { budget: formatCents(spendData.summary.budgetCents) })
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("costs.budgetUtilization", { percent: spendData.summary.utilizationPercent })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
                ...
                <FinanceSummaryCard
                  ...
                  title={t("costs.financeLedgerTitle")}
                  description={t("costs.financeLedgerDescription")}
                  debitLabel={t("costs.financeDebits")}
                  debitSubtitle={t("costs.financeDebitsSubtitle", {
                    count: financeData?.summary.eventCount ?? 0,
                    suffix: (financeData?.summary.eventCount ?? 0) === 1 ? "" : "s",
                  })}
                  creditLabel={t("costs.financeCredits")}
                  creditSubtitle={t("costs.financeCreditsSubtitle")}
                  netLabel={t("costs.financeNet")}
                  netSubtitle={t("costs.financeNetSubtitle")}
                  estimatedLabel={t("costs.financeEstimated")}
                  estimatedSubtitle={t("costs.financeEstimatedSubtitle")}
                />
              </div>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs.byAgentTitle")}</CardTitle>
                    <CardDescription>{t("costs.byAgentDescription")}</CardDescription>
                  </CardHeader>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs.byProjectTitle")}</CardTitle>
                      <CardDescription>{t("costs.byProjectDescription")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.noProjectCostsYet")}</p>
                      ) : (
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costs.noFinanceEventsWithHint")} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("costs.budgetControlPlaneTitle")}</CardTitle>
                  <CardDescription>{t("costs.budgetControlPlaneDescription")}</CardDescription>
                </CardHeader>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {t("costs.activeIncidentsDescription")}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                    ...
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold">
                          {scopeType === "company"
                            ? t("costs.companyBudgetsTitle")
                            : scopeType === "agent"
                              ? t("costs.agentBudgetsTitle")
                              : t("costs.projectBudgetsTitle")}
                        </h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costs.companyBudgetsDescription")
                            : scopeType === "agent"
                              ? t("costs.agentBudgetsDescription")
                              : t("costs.projectBudgetsDescription")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs.byBillerTitle")}</CardTitle>
                      <CardDescription>{t("costs.byBillerDescription")}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.noFinanceEventsYet")}</p>
                      ) : (

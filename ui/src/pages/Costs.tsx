...
export function Costs({
  ...
  useEffect(() => {
    if (!embedded) setBreadcrumbs([{ label: t("nav.costs") }]);
      ...
      {showSummaryChrome ? (
        <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              {embedded ? (
                <h2 className="text-lg font-semibold text-foreground">{t("nav.costs")}</h2>
              ) : (
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        {!lockTab ? (
          <TabsList variant="line" className="justify-start">
            ...
            {!hideBudgetsTab ? <TabsTrigger value="budgets">{t("pages.costs.tabBudgets")}</TabsTrigger> : null}
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{t(`localizationCosts.budgets_${scopeType}`)}</h2>

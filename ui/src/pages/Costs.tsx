  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("costs.finance.ledger", { defaultValue: "Finance ledger" })}</CardTitle>
        <CardDescription>
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("costs.finance.debits", { defaultValue: "Debits" })}
          value={formatCents(debitCents)}
          subtitle={`${eventCount} total event${eventCount === 1 ? "" : "s"} in range`}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("costs.finance.credits", { defaultValue: "Credits" })}
          value={formatCents(creditCents)}
          subtitle={t("costs.finance.creditsDesc", { defaultValue: "Refunds, offsets, and credit returns" })}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("costs.finance.net", { defaultValue: "Net" })}
          value={formatCents(netCents)}
          subtitle={t("costs.finance.netDesc", { defaultValue: "Debit minus credit for the selected period" })}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("costs.finance.estimated", { defaultValue: "Estimated" })}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("costs.finance.estimatedDesc", { defaultValue: "Estimated debits that are not yet invoice-authoritative" })}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("costs.pageTitle", { defaultValue: "Costs" }) }]);
  }, [setBreadcrumbs]);
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costs.overview.allProviders", { defaultValue: "All providers" })}</span>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("costs.overview.allBillers", { defaultValue: "All billers" })}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={t("costs.noCompanySelected", { defaultValue: "Select a company to view costs." })} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{t("costs.pageTitle", { defaultValue: "Costs" })}</h1>
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("costs.overview.inferenceSpend", { defaultValue: "Inference spend" })}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={`${formatTokens(inferenceTokenTotal)} tokens across request-scoped events`}
              icon={DollarSign}
            />
            <MetricTile
              label={t("costs.overview.budget", { defaultValue: "Budget" })}
            ...
            />
            <MetricTile
              label={t("costs.overview.financeNet", { defaultValue: "Finance net" })}
            ...
            />
            <MetricTile
              label={t("costs.overview.financeEvents", { defaultValue: "Finance events" })}
          ...
          {showCustomPrompt ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs.overview.inferenceLedger", { defaultValue: "Inference ledger" })}</CardTitle>
                    <CardDescription>
                      ...
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ...
                            : t("costs.overview.unlimitedBudget", { defaultValue: "Unlimited budget" })}
                        </div>
                      </div>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs.overview.byAgent", { defaultValue: "By agent" })}</CardTitle>
                    <CardDescription>{t("costs.overview.byAgentDescription", { defaultValue: "What each agent consumed in the selected period." })}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("costs.overview.noCostEvents", { defaultValue: "No cost events yet." })}</p>
                    ) : (
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs.overview.byProject", { defaultValue: "By project" })}</CardTitle>
                      <CardDescription>{t("costs.overview.byProjectDescription", { defaultValue: "Run costs attributed through project-linked issues." })}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.overview.noProjectCosts", { defaultValue: "No project-attributed run costs yet." })}</p>
                      ) : (
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costs.finance.noEvents", { defaultValue: "No finance events yet. Add account-level charges once biller invoices or credits land." })} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("costs.budgets.controlPlane", { defaultValue: "Budget control plane" })}</CardTitle>
                  <CardDescription>
                ...
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("costs.budgets.activeIncidents", { defaultValue: "Active incidents" })}
                    value={String(activeBudgetIncidents.length)}
                    subtitle={t("costs.budgets.openIncidentsDesc", { defaultValue: "Open soft or hard threshold crossings" })}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("costs.budgets.pendingApprovals", { defaultValue: "Pending approvals" })}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("costs.budgets.pendingApprovalsDesc", { defaultValue: "Budget override approvals awaiting board action" })}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("costs.budgets.pausedAgents", { defaultValue: "Paused agents" })}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("costs.budgets.pausedAgentsDesc", { defaultValue: "Agent heartbeats blocked by budget" })}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("costs.budgets.pausedProjects", { defaultValue: "Paused projects" })}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("costs.budgets.pausedProjectsDesc", { defaultValue: "Project execution blocked by budget" })}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("costs.budgets.activeIncidents", { defaultValue: "Active incidents" })}</h2>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs.finance.noCostPeriod", { defaultValue: "No cost events in this period." })}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs.finance.noBillableEvents", { defaultValue: "No billable events in this period." })}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {

...
function getExecutionWorkspaceBaseTabItems(): OrderedExecutionWorkspaceTabItem[] {
  return [
    { value: "issues", label: t("pages.executionWorkspaceDetail.tabTasks", { defaultValue: "Tasks" }), order: 10 },
    { value: "services", label: t("pages.executionWorkspaceDetail.tabServices", { defaultValue: "Services" }), order: 20 },
    { value: "configuration", label: t("pages.executionWorkspaceDetail.tabConfiguration", { defaultValue: "Configuration" }), order: 30 },
    { value: "runtime_logs", label: t("pages.executionWorkspaceDetail.tabRuntimeLogs", { defaultValue: "Runtime logs" }), order: 40 },
    { value: "routines", label: t("pages.executionWorkspaceDetail.tabRoutines", { defaultValue: "Routines" }), order: 60 },
  ];
}
...
function formatOptionalDateTime(value: Date | string | null | undefined) {
  return value ? formatDateTime(value) : t("pages.executionWorkspaceDetail.never", { defaultValue: "Never" });
}
...
function parseWorkspaceRuntimeJson(value: string) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: t("pages.executionWorkspaceDetail.runtimeJsonMustBeObject", { defaultValue: "Workspace commands JSON must be a JSON object." }),
      };
    }
    ...
    return {
      ok: false as const,
      error: error instanceof Error ? error.message : t("pages.executionWorkspaceDetail.invalidJson", { defaultValue: "Invalid JSON." }),
    };
  }
}
...
function validateForm(form: WorkspaceFormState) {
  ...
  if (repoUrl) {
    try {
      ...
      return t("pages.executionWorkspaceDetail.repoUrlInvalid", { defaultValue: "Repo URL must be a valid URL." });
    }
  }
...
function MonoValue({ value, copy }: { value: string; copy?: boolean }) {
  ...
  return (
    <div className="inline-flex max-w-full items-start gap-2">
      ...
      {copy ? (
        <CopyText text={value} className="shrink-0 text-muted-foreground hover:text-foreground" copiedLabel={t("pages.executionWorkspaceDetail.copied", { defaultValue: "Copied" })}>
    ...
    <div className="flex flex-col gap-3 border-b border-border px-3 py-3 last:border-b-0 sm:flex-row sm:items-center">
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span>{routine.assigneeAgentId ? t("pages.executionWorkspaceDetail.defaultAgentSet", { defaultValue: "Default agent set" }) : t("pages.executionWorkspaceDetail.chooseAgentWhenRunning", { defaultValue: "Choose agent when running" })}</span>
          <span>{t("pages.executionWorkspaceDetail.lastRun", { value: formatOptionalDateTime(routine.lastRun?.triggeredAt ?? routine.lastTriggeredAt), defaultValue: "Last run {{value}}" })}</span>
      ...
      >
        ...
        {isRunning ? t("pages.executionWorkspaceDetail.running", { defaultValue: "Running..." }) : t("pages.executionWorkspaceDetail.runNow", { defaultValue: "Run now" })}
      </Button>
    </div>
  );
}
...
function ExecutionWorkspaceRoutinesList({
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      pushToast({
        title: t("pages.executionWorkspaceDetail.routineStartedTitle", { defaultValue: "Routine started" }),
        body: t("pages.executionWorkspaceDetail.routineStartedBody", { defaultValue: "Paperclip created a run using this execution workspace." }),
        tone: "success",
      });
    },
    onSettled: () => {
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("pages.executionWorkspaceDetail.routineRunFailedTitle", { defaultValue: "Routine run failed" }),
        body: mutationError instanceof Error ? mutationError.message : t("pages.executionWorkspaceDetail.routineRunFailedBody", { defaultValue: "Paperclip could not start the routine run." }),
        tone: "error",
      });
    },
  });
  ...
  return (
    <>
      <Card className="rounded-none">
        <CardHeader>
          <CardTitle>{t("pages.executionWorkspaceDetail.workspaceRoutinesTitle", { defaultValue: "Workspace routines" })}</CardTitle>
          <CardDescription>
            {t("pages.executionWorkspaceDetail.workspaceRoutinesDescription", { defaultValue: "Routines that use workspace-specific variables can be run against this execution workspace." })}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <p className="text-sm text-muted-foreground">{t("pages.executionWorkspaceDetail.loadingRoutines", { defaultValue: "Loading routines..." })}</p>
          ) : error ? (
            <p className="text-sm text-destructive">
              {error instanceof Error ? error.message : t("pages.executionWorkspaceDetail.failedToLoadRoutines", { defaultValue: "Failed to load routines." })}
            </p>
          ) : workspaceRoutines.length === 0 ? (
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              <Repeat className="h-5 w-5 text-muted-foreground" />
              <p className="text-sm text-muted-foreground">
                {t("pages.executionWorkspaceDetail.noWorkspaceRoutines", { defaultValue: "No routines use workspace-specific variables yet." })}
              </p>
            </div>
          ) : (
...
export function ExecutionWorkspaceDetail() {
  ...
  const workspaceTabItems = useMemo(
    () => orderExecutionWorkspaceTabItems([...getExecutionWorkspaceBaseTabItems(), ...workspacePluginTabItems]),
  ...
  useEffect(() => {
    if (!workspace) return;
    const crumbs = [
      { label: t("pages.executionWorkspaceDetail.breadcrumbProjects", { defaultValue: "Projects" }), href: "/projects" },
      ...(project ? [{ label: project.name, href: `/projects/${projectRef}` }] : []),
      ...(project ? [{ label: t("pages.executionWorkspaceDetail.breadcrumbWorkspaces", { defaultValue: "Workspaces" }), href: `/projects/${projectRef}/workspaces` }] : []),
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("pages.executionWorkspaceDetail.failedToSaveWorkspace", { defaultValue: "Failed to save execution workspace." }));
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? t("pages.executionWorkspaceDetail.workspaceJobCompleted", { defaultValue: "Workspace job completed." })
          : request.action === "stop"
            ? t("pages.executionWorkspaceDetail.workspaceServiceStopped", { defaultValue: "Workspace service stopped." })
            : request.action === "restart"
              ? t("pages.executionWorkspaceDetail.workspaceServiceRestarted", { defaultValue: "Workspace service restarted." })
              : t("pages.executionWorkspaceDetail.workspaceServiceStarted", { defaultValue: "Workspace service started." }),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setRuntimeActionErrorMessage(error instanceof Error ? error.message : t("pages.executionWorkspaceDetail.failedToControlCommands", { defaultValue: "Failed to control workspace commands." }));
    },
  });

  if (workspaceQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("pages.executionWorkspaceDetail.loadingWorkspace", { defaultValue: "Loading workspace…" })}</p>;
  if (workspaceQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {workspaceQuery.error instanceof Error ? workspaceQuery.error.message : t("pages.executionWorkspaceDetail.failedToLoadWorkspace", { defaultValue: "Failed to load workspace" })}
      </p>
    );
  }
  ...
  const saveChanges = () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : t("pages.executionWorkspaceDetail.failedToBuildUpdate", { defaultValue: "Failed to build workspace update." }));
      return;
    }
      ...
      <div className="space-y-4 overflow-hidden sm:space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 space-y-2">
            <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
              {t("pages.executionWorkspaceDetail.eyebrowExecutionWorkspace", { defaultValue: "Execution workspace" })}
            </div>
        ...
        {activeTab === "services" ? (
          <WorkspaceRuntimeControls
            ...
            serviceEmptyMessage={
              effectiveRuntimeConfig
                ? t("pages.executionWorkspaceDetail.noServicesStarted", { defaultValue: "No services have been started for this execution workspace yet." })
                : t("pages.executionWorkspaceDetail.noCommandConfig", { defaultValue: "No workspace command config is defined for this execution workspace yet." })
            }
            jobEmptyMessage={t("pages.executionWorkspaceDetail.noJobsConfigured", { defaultValue: "No one-shot jobs are configured for this execution workspace yet." })}
            disabledHint={
              canStartRuntimeServices
                ? null
                : t("pages.executionWorkspaceDetail.commandsDisabledHint", { defaultValue: "Execution workspaces need a working directory before local commands can run, and services also need runtime config." })
            }
            onAction={(request) => controlRuntimeServices.mutate(request)}
          />
          ...
          <div className="space-y-4 sm:space-y-6">
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("pages.executionWorkspaceDetail.workspaceSettingsTitle", { defaultValue: "Workspace settings" })}</CardTitle>
                <CardDescription>
                  {t("pages.executionWorkspaceDetail.workspaceSettingsDescription", { defaultValue: "Edit the concrete path, repo, branch, provisioning, teardown, and runtime overrides attached to this execution workspace." })}
                </CardDescription>
                <CardAction>
                  <Button
                    variant="destructive"
                  ...
                  >
                    {workspace.status === "cleanup_failed" ? t("pages.executionWorkspaceDetail.retryClose", { defaultValue: "Retry close" }) : t("pages.executionWorkspaceDetail.closeWorkspace", { defaultValue: "Close workspace" })}
                  </Button>
                </CardAction>
              </CardHeader>
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("pages.executionWorkspaceDetail.sectionGeneral", { defaultValue: "General" })}</div>
                  <Field label={t("pages.executionWorkspaceDetail.fieldWorkspaceName", { defaultValue: "Workspace name" })}>
                    <Input
                      value={form.name}
                      onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                      placeholder={t("pages.executionWorkspaceDetail.placeholderWorkspaceName", { defaultValue: "Execution workspace name" })}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("pages.executionWorkspaceDetail.sectionSourceControl", { defaultValue: "Source control" })}</div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label={t("pages.executionWorkspaceDetail.fieldBranchName", { defaultValue: "Branch name" })} hint={t("pages.executionWorkspaceDetail.hintBranchName", { defaultValue: "Useful for isolated worktrees" })}>
                      <Input
                        className="font-mono"
                    ...
                    <Field label={t("pages.executionWorkspaceDetail.fieldBaseRef", { defaultValue: "Base ref" })}>
                      <Input
                        className="font-mono"
                        value={form.baseRef}
                  ...
                  <Field label={t("pages.executionWorkspaceDetail.fieldRepoUrl", { defaultValue: "Repo URL" })}>
                    <Input
                      value={form.repoUrl}
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("pages.executionWorkspaceDetail.sectionPaths", { defaultValue: "Paths" })}</div>
                  <Field label={t("pages.executionWorkspaceDetail.fieldWorkingDirectory", { defaultValue: "Working directory" })}>
                    <Input
                      className="font-mono"
                      value={form.cwd}
                  ...
                  <Field label={t("pages.executionWorkspaceDetail.fieldProviderPathRef", { defaultValue: "Provider path / ref" })}>
                    <Input
                      className="font-mono"
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("pages.executionWorkspaceDetail.sectionLifecycleCommands", { defaultValue: "Lifecycle commands" })}</div>
                  <Field label={t("pages.executionWorkspaceDetail.fieldProvisionCommand", { defaultValue: "Provision command" })} hint={t("pages.executionWorkspaceDetail.hintProvisionCommand", { defaultValue: "Runs when Paperclip prepares this execution workspace" })}>
                    <Textarea
                  ...
                  <Field label={t("pages.executionWorkspaceDetail.fieldTeardownCommand", { defaultValue: "Teardown command" })} hint={t("pages.executionWorkspaceDetail.hintTeardownCommand", { defaultValue: "Runs when the execution workspace is archived or cleaned up" })}>
                    <Textarea
                      className="min-h-20 font-mono"
                  ...
                  <Field label={t("pages.executionWorkspaceDetail.fieldCleanupCommand", { defaultValue: "Cleanup command" })} hint={t("pages.executionWorkspaceDetail.hintCleanupCommand", { defaultValue: "Workspace-specific cleanup before teardown" })}>
                    <Textarea
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("pages.executionWorkspaceDetail.sectionRuntimeConfig", { defaultValue: "Runtime config" })}</div>
                  <div className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                      <div className="space-y-1">
                        <div className="text-sm font-medium text-foreground">
                          {t("pages.executionWorkspaceDetail.runtimeConfigSource", { defaultValue: "Runtime config source" })}
                        </div>
                        <p className="text-sm text-muted-foreground">
                          {runtimeConfigSource === "execution_workspace"
                            ? t("pages.executionWorkspaceDetail.runtimeConfigOverrides", { defaultValue: "This execution workspace currently overrides the project workspace runtime config." })
                            : runtimeConfigSource === "project_workspace"
                              ? t("pages.executionWorkspaceDetail.runtimeConfigInheriting", { defaultValue: "This execution workspace is inheriting the project workspace runtime config." })
                              : t("pages.executionWorkspaceDetail.runtimeConfigNone", { defaultValue: "No runtime config is currently defined on this execution workspace or its project workspace." })}
                        </p>
                      </div>
                      <Button
                        variant="outline"
                      ...
                      >
                        {t("pages.executionWorkspaceDetail.resetToInherit", { defaultValue: "Reset to inherit" })}
                      </Button>
                    </div>
                  </div>

                  <details className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <summary className="cursor-pointer text-sm font-medium">{t("pages.executionWorkspaceDetail.advancedRuntimeJson", { defaultValue: "Advanced runtime JSON" })}</summary>
                    <p className="mt-2 text-sm text-muted-foreground">
                      {t("pages.executionWorkspaceDetail.advancedRuntimeJsonHelp", { defaultValue: "Override the inherited workspace command model only when this execution workspace truly needs different service or job behavior." })}
                    </p>
                    <div className="mt-3">
                      <Field label={t("pages.executionWorkspaceDetail.fieldWorkspaceCommandsJson", { defaultValue: "Workspace commands JSON" })} hint={t("pages.executionWorkspaceDetail.hintWorkspaceCommandsJson", { defaultValue: "Legacy `services` arrays still work, but `commands` supports both services and jobs." })}>
                        <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          />
                          <label htmlFor="inherit-runtime-config">{t("pages.executionWorkspaceDetail.inheritRuntimeConfigLabel", { defaultValue: "Inherit project workspace runtime config" })}</label>
                        </div>
                        <Textarea
              ...
              <div className="mt-6 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                <Button className="w-full sm:w-auto" disabled={!isDirty || updateWorkspace.isPending} onClick={saveChanges}>
                  ...
                  {t("pages.executionWorkspaceDetail.saveChanges", { defaultValue: "Save changes" })}
                </Button>
                <Button
                  variant="outline"
                  className="w-full sm:w-auto"
                ...
                >
                  {t("pages.executionWorkspaceDetail.reset", { defaultValue: "Reset" })}
                </Button>
                {errorMessage ? <p className="text-sm text-destructive">{errorMessage}</p> : null}
                {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">{t("pages.executionWorkspaceDetail.noUnsavedChanges", { defaultValue: "No unsaved changes." })}</p> : null}
              </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("pages.executionWorkspaceDetail.workspaceContextTitle", { defaultValue: "Workspace context" })}</CardTitle>
                <CardDescription>{t("pages.executionWorkspaceDetail.workspaceContextDescription", { defaultValue: "Linked objects and relationships" })}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label={t("pages.executionWorkspaceDetail.detailProject", { defaultValue: "Project" })}>
                {project ? <Link to={`/projects/${projectRef}`} className="hover:underline">{project.name}</Link> : <MonoValue value={workspace.projectId} />}
              </DetailRow>
              <DetailRow label={t("pages.executionWorkspaceDetail.detailProjectWorkspace", { defaultValue: "Project workspace" })}>
                {project && linkedProjectWorkspace ? (
                  ...
                  t("pages.executionWorkspaceDetail.none", { defaultValue: "None" })
                )}
              </DetailRow>
              <DetailRow label={t("pages.executionWorkspaceDetail.detailSourceTask", { defaultValue: "Source task" })}>
                {sourceIssue ? (
              ...
              <DetailRow label={t("pages.executionWorkspaceDetail.detailDerivedFrom", { defaultValue: "Derived from" })}>
                {derivedWorkspace ? (
              ...
              <DetailRow label={t("pages.executionWorkspaceDetail.detailWorkspaceId", { defaultValue: "Workspace ID" })}>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("pages.executionWorkspaceDetail.concreteLocationTitle", { defaultValue: "Concrete location" })}</CardTitle>
                <CardDescription>{t("pages.executionWorkspaceDetail.concreteLocationDescription", { defaultValue: "Paths and refs" })}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label={t("pages.executionWorkspaceDetail.detailWorkingDir", { defaultValue: "Working dir" })}>
                {workspace.cwd ? <MonoValue value={workspace.cwd} copy /> : t("pages.executionWorkspaceDetail.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("pages.executionWorkspaceDetail.detailProviderRef", { defaultValue: "Provider ref" })}>
                {workspace.providerRef ? <MonoValue value={workspace.providerRef} copy /> : t("pages.executionWorkspaceDetail.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("pages.executionWorkspaceDetail.detailRepoUrl", { defaultValue: "Repo URL" })}>
                {workspace.repoUrl && isSafeExternalUrl(workspace.repoUrl) ? (
                  <div className="inline-flex max-w-full items-start gap-2">
                    ...
                    <CopyText text={workspace.repoUrl} className="shrink-0 text-muted-foreground hover:text-foreground" copiedLabel={t("pages.executionWorkspaceDetail.copied", { defaultValue: "Copied" })}>
                      <Copy className="h-3.5 w-3.5" />
                    </CopyText>
                  </div>
                ) : workspace.repoUrl ? (
              ...
              <DetailRow label={t("pages.executionWorkspaceDetail.detailBaseRef", { defaultValue: "Base ref" })}>
                {workspace.baseRef ? <MonoValue value={workspace.baseRef} copy /> : t("pages.executionWorkspaceDetail.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("pages.executionWorkspaceDetail.detailBranch", { defaultValue: "Branch" })}>
                {workspace.branchName ? <MonoValue value={workspace.branchName} copy /> : t("pages.executionWorkspaceDetail.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("pages.executionWorkspaceDetail.detailOpened", { defaultValue: "Opened" })}>{formatDateTime(workspace.openedAt)}</DetailRow>
              <DetailRow label={t("pages.executionWorkspaceDetail.detailLastUsed", { defaultValue: "Last used" })}>{formatDateTime(workspace.lastUsedAt)}</DetailRow>
              <DetailRow label={t("pages.executionWorkspaceDetail.detailCleanup", { defaultValue: "Cleanup" })}>
                {workspace.cleanupEligibleAt
                  ? `${formatDateTime(workspace.cleanupEligibleAt)}${workspace.cleanupReason ? ` · ${workspace.cleanupReason}` : ""}`
                  : t("pages.executionWorkspaceDetail.notScheduled", { defaultValue: "Not scheduled" })}
              </DetailRow>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="rounded-none">
            <CardHeader>
              <CardTitle>{t("pages.executionWorkspaceDetail.runtimeLogsTitle", { defaultValue: "Runtime and cleanup logs" })}</CardTitle>
              <CardDescription>{t("pages.executionWorkspaceDetail.runtimeLogsDescription", { defaultValue: "Recent operations" })}</CardDescription>
            </CardHeader>
            <CardContent>
            {workspaceOperationsQuery.isLoading ? (
              <p className="text-sm text-muted-foreground">{t("pages.executionWorkspaceDetail.loadingOperations", { defaultValue: "Loading workspace operations…" })}</p>
            ) : workspaceOperationsQuery.error ? (
              <p className="text-sm text-destructive">
                {workspaceOperationsQuery.error instanceof Error
                  ? workspaceOperationsQuery.error.message
                  : t("pages.executionWorkspaceDetail.failedToLoadOperations", { defaultValue: "Failed to load workspace operations." })}
              </p>
              ...
              <p className="text-sm text-muted-foreground">{t("pages.executionWorkspaceDetail.noOperationsRecorded", { defaultValue: "No workspace operations have been recorded yet." })}</p>
            )}
            </CardContent>
          </Card>
        ) : activeTab === "issues" ? (
          ...
          <Card>
            <CardContent className="py-6 text-sm text-muted-foreground">{t("pages.executionWorkspaceDetail.loadingPlugin", { defaultValue: "Loading workspace plugin..." })}</CardContent>
          </Card>
          ...
          <MissingPluginTabPlaceholder
            ...
            defaultTabLabel={t("pages.executionWorkspaceDetail.backToTasks", { defaultValue: "Back to tasks" })}
          />
        ) : activeTab === "routines" ? (

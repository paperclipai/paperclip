...
function formatOptionalDateTime(value: Date | string | null | undefined) {
  return value ? formatDateTime(value) : i18n.t("status.never", { ns: "common", defaultValue: "Never" });
}
...
function parseWorkspaceRuntimeJson(value: string) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: i18n.t("workspace.error_runtime_must_be_object", { ns: "common", defaultValue: "Workspace commands JSON must be a JSON object." }),
      };
    }
    ...
    return {
      ok: false as const,
      error: error instanceof Error ? error.message : i18n.t("workspace.error_invalid_json", { ns: "common", defaultValue: "Invalid JSON." }),
    };
  }
}
...
function validateForm(form: WorkspaceFormState) {
  ...
  if (repoUrl) {
    try {
      ...
      return i18n.t("workspace.error_repo_url_invalid", { ns: "common", defaultValue: "Repo URL must be a valid URL." });
    }
  }
...
function MonoValue({ value, copy }: { value: string; copy?: boolean }) {
  ...
  return (
    <div className="inline-flex max-w-full items-start gap-2">
      ...
      {copy ? (
        <CopyText text={value} className="shrink-0 text-muted-foreground hover:text-foreground" copiedLabel={t("actions.copied")}>
    ...
    <div className="flex flex-col gap-3 border-b border-border px-3 py-3 last:border-b-0 sm:flex-row sm:items-center">
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          ...
          <span>{t("workspace_runtime.last_run", { time: formatOptionalDateTime(routine.lastRun?.triggeredAt ?? routine.lastTriggeredAt) })}</span>
      ...
      >
        ...
        {isRunning ? i18n.t("actions.running", { ns: "routines", defaultValue: "Running..." }) : i18n.t("actions.run_now", { ns: "routines", defaultValue: "Run now" })}
      </Button>
    </div>
  );
}
...
function ExecutionWorkspaceRoutinesList({
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      pushToast({
        title: i18n.t("messages.routine_started", { ns: "common", defaultValue: "Routine started" }),
        body: i18n.t("messages.routine_started_body", { ns: "common", defaultValue: "Paperclip created a run using this execution workspace." }),
        tone: "success",
      });
    },
    onSettled: () => {
    ...
    onError: (mutationError) => {
      pushToast({
        title: i18n.t("messages.routine_run_failed", { ns: "common", defaultValue: "Routine run failed" }),
        body: mutationError instanceof Error ? mutationError.message : i18n.t("messages.routine_run_failed_body", { ns: "common", defaultValue: "Paperclip could not start the routine run." }),
        tone: "error",
      });
    },
  });
        ...
        <CardContent>
          {isLoading ? (
            ...
            <p className="text-sm text-destructive">
              {error instanceof Error ? error.message : t("messages.failed_load_routines", { defaultValue: "Failed to load routines." })}
            </p>
...
export function ExecutionWorkspaceDetail() {
  ...
  useEffect(() => {
    if (!workspace) return;
    const crumbs = [
      { label: i18n.t("nav.projects", { ns: "common", defaultValue: "Projects" }), href: "/projects" },
      ...(project ? [{ label: project.name, href: `/projects/${projectRef}` }] : []),
      ...(project ? [{ label: i18n.t("nav.workspaces", { ns: "common", defaultValue: "Workspaces" }), href: `/projects/${projectRef}/workspaces` }] : []),
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : i18n.t("workspace.error_failed_to_save", { ns: "common", defaultValue: "Failed to save execution workspace." }));
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? i18n.t("workspace_runtime.job_completed", { ns: "common", defaultValue: "Workspace job completed." })
          : request.action === "stop"
            ? i18n.t("workspace_runtime.service_stopped", { ns: "common", defaultValue: "Workspace service stopped." })
            : request.action === "restart"
              ? i18n.t("workspace_runtime.service_restarted", { ns: "common", defaultValue: "Workspace service restarted." })
              : i18n.t("workspace_runtime.service_started", { ns: "common", defaultValue: "Workspace service started." }),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setRuntimeActionErrorMessage(error instanceof Error ? error.message : i18n.t("workspace.error_control_workspace_commands", { ns: "common", defaultValue: "Failed to control workspace commands." }));
    },
  });
  ...
  const saveChanges = () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : i18n.t("workspace.error_build_update", { ns: "common", defaultValue: "Failed to build workspace update." }));
      return;
    }
        ...
        <Tabs value={activeTab ?? "issues"} onValueChange={(value) => handleTabChange(value as ExecutionWorkspaceTab)}>
          <PageTabBar
            items={[
              ...
              { value: "services", label: t("workspace_runtime.services") },
            ...
            {workspaceOperationsQuery.isLoading ? (
              ...
              <p className="text-sm text-destructive">
                {workspaceOperationsQuery.error instanceof Error
                  ...
                  : t("messages.failed_load_operations", { defaultValue: "Failed to load workspace operations." })}
              </p>

...
const EXECUTION_WORKSPACE_BASE_TAB_ITEMS: OrderedExecutionWorkspaceTabItem[] = [
  { value: "issues", label: t("app.executionWorkspaceDetail.tasks", { defaultValue: "Tasks" }), order: 10 },
  { value: "services", label: t("app.executionWorkspaceDetail.services", { defaultValue: "Services" }), order: 20 },
  { value: "configuration", label: t("app.executionWorkspaceDetail.configuration", { defaultValue: "Configuration" }), order: 30 },
  { value: "runtime_logs", label: t("app.executionWorkspaceDetail.runtimeLogs", { defaultValue: "Runtime logs" }), order: 40 },
  { value: "routines", label: t("app.executionWorkspaceDetail.routines", { defaultValue: "Routines" }), order: 60 },
];
...
function formatOptionalDateTime(value: Date | string | null | undefined) {
  return value ? formatDateTime(value) : t("app.executionWorkspaceDetail.never", { defaultValue: "Never" });
}
...
function parseWorkspaceRuntimeJson(value: string) {
  ...
  try {
    ...
    return {
      ok: false as const,
      error: error instanceof Error ? error.message : t("app.executionWorkspaceDetail.invalidJson", { defaultValue: "Invalid JSON." }),
    };
  }
}
...
export function RuntimeProvisionStatusValue({
  ...
  if (status.kind === "eager") {
    return (
      <span className="text-sm text-muted-foreground">{t("app.executionWorkspaceDetail.eagerProvisionedDuringWorkspaceSetup", { defaultValue: "Eager · provisioned during workspace setup" })}</span>
    );
  }
  if (status.kind === "deferred") {
    return (
      <div className="flex flex-col gap-1">
        <StatusPill className="border-amber-500/40 text-amber-600 dark:text-amber-400">{t("app.executionWorkspaceDetail.deferred", { defaultValue: "Deferred" })}</StatusPill>
        <span className="text-xs text-muted-foreground">
          {t("app.executionWorkspaceDetail.runsOnceBeforeTheFirstRuntimeServiceStart", { defaultValue: "Runs once before the first runtime-service start." })}</span>
      </div>
    );
  }
  ...
  if (status.kind === "provisioned") {
    return (
      <StatusPill className="border-emerald-500/40 text-emerald-600 dark:text-emerald-400">
        {t("app.executionWorkspaceDetail.provisioned", { defaultValue: "Provisioned" })}{status.at ? ` · ${formatDateTime(status.at)}` : ""}
      </StatusPill>
    );
  }
  return (
    <div className="flex flex-col gap-1">
      <StatusPill className="border-destructive/50 text-destructive">
        {t("app.executionWorkspaceDetail.provisioningFailed", { defaultValue: "Provisioning failed" })}{status.at ? ` · ${formatDateTime(status.at)}` : ""}
      </StatusPill>
      <button type="button" onClick={onViewLogs} className="self-start text-xs text-muted-foreground underline underline-offset-2 hover:text-foreground">
        {t("app.executionWorkspaceDetail.viewRuntimeLogs", { defaultValue: "View runtime logs" })}</button>
    </div>
  );
}
    ...
    <div className="flex flex-col gap-3 border-b border-border px-3 py-3 last:border-b-0 sm:flex-row sm:items-center">
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span>{routine.assigneeAgentId ? t("app.executionWorkspaceDetail.defaultAgentSet", { defaultValue: "Default agent set" }) : t("app.executionWorkspaceDetail.chooseAgentWhenRunning", { defaultValue: "Choose agent when running" })}</span>
          <span>{t("app.executionWorkspaceDetail.lastRun", { defaultValue: "Last run " })}{formatOptionalDateTime(routine.lastRun?.triggeredAt ?? routine.lastTriggeredAt)}</span>
      ...
      >
        ...
        {isRunning ? t("app.executionWorkspaceDetail.running", { defaultValue: "Running..." }) : t("app.executionWorkspaceDetail.runNow", { defaultValue: "Run now" })}
      </Button>
    </div>
  );
}
...
function ExecutionWorkspaceRoutinesList({
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      pushToast({
        title: t("app.executionWorkspaceDetail.routineStarted", { defaultValue: "Routine started" }),
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("app.executionWorkspaceDetail.routineRunFailed", { defaultValue: "Routine run failed" }),
        body: mutationError instanceof Error ? mutationError.message : t("app.executionWorkspaceDetail.paperclipCouldNotStartTheRoutineRun", { defaultValue: "Paperclip could not start the routine run." }),
        tone: "error",
      });
    },
  });
  ...
  return (
    <>
      <Card className="rounded-none">
        <CardHeader>
          <CardTitle>{t("app.executionWorkspaceDetail.workspaceRoutines", { defaultValue: "Workspace routines" })}</CardTitle>
          <CardDescription>
            {t("app.executionWorkspaceDetail.routinesThatUseWorkspaceSpecificVariablesCanBeRunAgainstThisExecutionWorkspace", { defaultValue: "Routines that use workspace-specific variables can be run against this execution workspace." })}</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <p className="text-sm text-muted-foreground">{t("app.executionWorkspaceDetail.loadingRoutines", { defaultValue: "Loading routines..." })}</p>
          ) : error ? (
            <p className="text-sm text-destructive">
              {error instanceof Error ? error.message : t("app.executionWorkspaceDetail.failedToLoadRoutines", { defaultValue: "Failed to load routines." })}
            </p>
          ) : workspaceRoutines.length === 0 ? (
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              <Repeat className="h-5 w-5 text-muted-foreground" />
              <p className="text-sm text-muted-foreground">
                {t("app.executionWorkspaceDetail.noRoutinesUseWorkspaceSpecificVariablesYet", { defaultValue: "No routines use workspace-specific variables yet." })}</p>
            </div>
          ) : (
...
export function ExecutionWorkspaceDetail() {
  ...
  useEffect(() => {
    if (!workspace) return;
    const crumbs = [
      { label: t("app.pages.projects", { defaultValue: "Projects" }), href: "/projects" },
      ...(project ? [{ label: project.name, href: `/projects/${projectRef}` }] : []),
      ...(project ? [{ label: t("app.pages.workspaces", { defaultValue: "Workspaces" }), href: `/projects/${projectRef}/workspaces` }] : []),
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("app.executionWorkspaceDetail.failedToSaveExecutionWorkspace", { defaultValue: "Failed to save execution workspace." }));
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? t("app.executionWorkspaceDetail.workspaceJobCompleted", { defaultValue: "Workspace job completed." })
          : request.action === "stop"
            ? t("app.executionWorkspaceDetail.workspaceServiceStopped", { defaultValue: "Workspace service stopped." })
            : request.action === "restart"
              ? t("app.executionWorkspaceDetail.workspaceServiceRestarted", { defaultValue: "Workspace service restarted." })
              : t("app.executionWorkspaceDetail.workspaceServiceStarted", { defaultValue: "Workspace service started." }),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setRuntimeActionErrorMessage(error instanceof Error ? error.message : t("app.executionWorkspaceDetail.failedToControlWorkspaceCommands", { defaultValue: "Failed to control workspace commands." }));
    },
  ...
  const openWorkspace = useMutation({
    ...
    onError: async (error) => {
      ...
      setHandoffErrorMessage(
        failure ? null : error instanceof Error ? error.message : t("app.executionWorkspaceDetail.failedToOpenTheWorkspace", { defaultValue: "Failed to open the workspace." }),
      );
  ...
  const repairWorkspace = useMutation({
    ...
    onError: (error) => {
      setRuntimeActionMessage(null);
      setRuntimeActionErrorMessage(error instanceof Error ? error.message : t("app.executionWorkspaceDetail.failedToRepairTheWorkspace", { defaultValue: "Failed to repair the workspace." }));
    },
  });

  if (workspaceQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("app.executionWorkspaceDetail.loadingWorkspace", { defaultValue: "Loading workspace…" })}</p>;
  if (workspaceQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {workspaceQuery.error instanceof Error ? workspaceQuery.error.message : t("app.executionWorkspaceDetail.failedToLoadWorkspace", { defaultValue: "Failed to load workspace" })}
      </p>
    );
  }
  ...
  const saveChanges = () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : t("app.executionWorkspaceDetail.failedToBuildWorkspaceUpdate", { defaultValue: "Failed to build workspace update." }));
      return;
    }
      ...
      <div className="space-y-4 overflow-hidden sm:space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 space-y-2">
            <div className="text-xs font-medium uppercase tracking-(--tracking-eyebrow) text-muted-foreground">
              {t("app.executionWorkspaceDetail.executionWorkspace", { defaultValue: "Execution workspace" })}</div>
        ...
        {activeTab === "services" ? (
          <WorkspaceRuntimeControls
            ...
            disabledHint={
              canStartRuntimeServices
                ? null
                : t("app.executionWorkspaceDetail.executionWorkspacesNeedAWorkingDirectoryBeforeLocalCommandsCanRunAndServicesAlsoNeedRuntimeConfig", { defaultValue: "Execution workspaces need a working directory before local commands can run, and services also need runtime config." })
            }
          ...
          <div className="space-y-4 sm:space-y-6">
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("app.executionWorkspaceDetail.workspaceSettings", { defaultValue: "Workspace settings" })}</CardTitle>
                <CardDescription>
                  {t("app.executionWorkspaceDetail.editTheConcretePathRepoBranchProvisioningTeardownAndRuntimeOverridesAttachedToThisExecutionWorkspaceSavedChangesAffectFutureRunsPaperclipMayRefreshOrReplaceAReusedWorkspaceWhenConfigChanges", { defaultValue: "Edit the concrete path, repo, branch, provisioning, teardown, and runtime overrides attached to this execution workspace. Saved changes affect future runs; Paperclip may refresh or replace a reused workspace when config changes." })}</CardDescription>
                <CardAction>
                  <Button
                    variant="destructive"
                    size="sm"
                  ...
                  >
                    {workspace.status === "cleanup_failed" ? t("app.executionWorkspaceDetail.retryClose", { defaultValue: "Retry close" }) : t("app.executionWorkspaceDetail.closeWorkspace", { defaultValue: "Close workspace" })}
                  </Button>
                </CardAction>
              </CardHeader>
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("app.executionWorkspaceDetail.general", { defaultValue: "General" })}</div>
                  <Field label={t("app.executionWorkspaceDetail.workspaceName", { defaultValue: "Workspace name" })}>
                    <Input
                      value={form.name}
                      onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                      placeholder={t("app.executionWorkspaceDetail.executionWorkspaceName", { defaultValue: "Execution workspace name" })}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("app.executionWorkspaceDetail.sourceControl", { defaultValue: "Source control" })}</div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label={t("app.executionWorkspaceDetail.branchName", { defaultValue: "Branch name" })} hint={t("app.executionWorkspaceDetail.usefulForIsolatedWorktrees", { defaultValue: "Useful for isolated worktrees" })}>
                      <Input
                        className="font-mono"
                        value={form.branchName}
                        onChange={(event) => setForm((current) => current ? { ...current, branchName: event.target.value } : current)}
                        placeholder={t("app.executionWorkspaceDetail.pap946Workspace", { defaultValue: "PAP-946-workspace" })}
                      />
                    </Field>

                    <Field label={t("app.executionWorkspaceDetail.baseRef", { defaultValue: "Base ref" })}>
                      <Input
                        className="font-mono"
                        value={form.baseRef}
                  ...
                  <Field label={t("app.executionWorkspaceDetail.repoUrl", { defaultValue: "Repo URL" })}>
                    <Input
                      value={form.repoUrl}
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("app.executionWorkspaceDetail.paths", { defaultValue: "Paths" })}</div>
                  <Field label={t("app.executionWorkspaceDetail.workingDirectory", { defaultValue: "Working directory" })}>
                    <Input
                      className="font-mono"
                      value={form.cwd}
                  ...
                  <Field label={t("app.executionWorkspaceDetail.providerPathRef", { defaultValue: "Provider path / ref" })}>
                    <Input
                      className="font-mono"
                      value={form.providerRef}
                      onChange={(event) => setForm((current) => current ? { ...current, providerRef: event.target.value } : current)}
                      placeholder={t("app.executionWorkspaceDetail.pathToWorktreeOrProviderRef", { defaultValue: "/path/to/worktree or provider ref" })}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("app.executionWorkspaceDetail.lifecycleCommands", { defaultValue: "Lifecycle commands" })}</div>
                  <Field label={t("app.executionWorkspaceDetail.provisionCommand", { defaultValue: "Provision command" })} hint={t("app.executionWorkspaceDetail.runsWhenPaperclipPreparesThisExecutionWorkspace", { defaultValue: "Runs when Paperclip prepares this execution workspace" })}>
                    <Textarea
                      className="min-h-20 font-mono"
                      value={form.provisionCommand}
                      onChange={(event) => setForm((current) => current ? { ...current, provisionCommand: event.target.value } : current)}
                      placeholder={t("app.executionWorkspaceDetail.bashScriptsProvisionWorktreeSh", { defaultValue: "bash ./scripts/provision-worktree.sh" })}
                    />
                  </Field>

                  <Field
                    label={t("app.executionWorkspaceDetail.runtimeProvisionCommand", { defaultValue: "Runtime provision command" })}
                    hint={t("app.executionWorkspaceDetail.runsOnceBeforeTheFirstRuntimeServiceStartLeaveEmptyToKeepEagerProvisioning", { defaultValue: "Runs once before the first runtime-service start. Leave empty to keep eager provisioning." })}
                  >
                    <Textarea
                      className="min-h-20 font-mono"
                      ...
                      placeholder={t("app.executionWorkspaceDetail.bashScriptsProvisionWorktreeRuntimeSh", { defaultValue: "bash ./scripts/provision-worktree-runtime.sh" })}
                    />
                  </Field>

                  <Field label={t("app.executionWorkspaceDetail.teardownCommand", { defaultValue: "Teardown command" })} hint={t("app.executionWorkspaceDetail.runsWhenTheExecutionWorkspaceIsArchivedOrCleanedUp", { defaultValue: "Runs when the execution workspace is archived or cleaned up" })}>
                    <Textarea
                      className="min-h-20 font-mono"
                      value={form.teardownCommand}
                      onChange={(event) => setForm((current) => current ? { ...current, teardownCommand: event.target.value } : current)}
                      placeholder={t("app.executionWorkspaceDetail.bashScriptsTeardownWorktreeSh", { defaultValue: "bash ./scripts/teardown-worktree.sh" })}
                    />
                  </Field>

                  <Field label={t("app.executionWorkspaceDetail.cleanupCommand", { defaultValue: "Cleanup command" })} hint={t("app.executionWorkspaceDetail.workspaceSpecificCleanupBeforeTeardown", { defaultValue: "Workspace-specific cleanup before teardown" })}>
                    <Textarea
                      className="min-h-16 font-mono"
                      value={form.cleanupCommand}
                      onChange={(event) => setForm((current) => current ? { ...current, cleanupCommand: event.target.value } : current)}
                      placeholder={t("app.executionWorkspaceDetail.pkillFViteTrue", { defaultValue: "pkill -f vite || true" })}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("app.executionWorkspaceDetail.runtimeConfig", { defaultValue: "Runtime config" })}</div>
                  <div className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                      <div className="space-y-1">
                        <div className="text-sm font-medium text-foreground">
                          {t("app.executionWorkspaceDetail.runtimeConfigSource", { defaultValue: "Runtime config source" })}</div>
                        <p className="text-sm text-muted-foreground">
                          {runtimeConfigSource === "execution_workspace"
                            ? t("app.executionWorkspaceDetail.thisExecutionWorkspaceCurrentlyOverridesTheProjectWorkspaceRuntimeConfig", { defaultValue: "This execution workspace currently overrides the project workspace runtime config." })
                            : runtimeConfigSource === "project_workspace"
                              ? t("app.executionWorkspaceDetail.thisExecutionWorkspaceIsInheritingTheProjectWorkspaceRuntimeConfig", { defaultValue: "This execution workspace is inheriting the project workspace runtime config." })
                              : t("app.executionWorkspaceDetail.noRuntimeConfigIsCurrentlyDefinedOnThisExecutionWorkspaceOrItsProjectWorkspace", { defaultValue: "No runtime config is currently defined on this execution workspace or its project workspace." })}
                        </p>
                      </div>
                      <Button
                        variant="outline"
                      ...
                      >
                        {t("app.executionWorkspaceDetail.resetToInherit", { defaultValue: "Reset to inherit" })}</Button>
                    </div>
                  </div>

                  <details className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <summary className="cursor-pointer text-sm font-medium">{t("app.executionWorkspaceDetail.advancedRuntimeJson", { defaultValue: "Advanced runtime JSON" })}</summary>
                    <p className="mt-2 text-sm text-muted-foreground">
                      {t("app.executionWorkspaceDetail.overrideTheInheritedWorkspaceCommandModelOnlyWhenThisExecutionWorkspaceTrulyNeedsDifferentServiceOrJobBehavior", { defaultValue: "Override the inherited workspace command model only when this execution workspace truly needs different service or job behavior." })}</p>
                    <div className="mt-3">
                      <Field label={t("app.executionWorkspaceDetail.workspaceCommandsJson", { defaultValue: "Workspace commands JSON" })} hint={t("app.executionWorkspaceDetail.legacyServicesArraysStillWorkButCommandsSupportsBothServicesAndJobs", { defaultValue: "Legacy `services` arrays still work, but `commands` supports both services and jobs." })}>
                        <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          />
                          <label htmlFor="inherit-runtime-config">{t("app.executionWorkspaceDetail.inheritProjectWorkspaceRuntimeConfig", { defaultValue: "Inherit project workspace runtime config" })}</label>
                        </div>
                        <Textarea
                  ...
                  {configuredRuntimeServicePorts.length > 0 ? (
                    <div className="space-y-3 rounded-md border border-border bg-muted/20 p-4">
                      <div>
                        <div className="text-sm font-medium">{t("app.executionWorkspaceDetail.servicePorts", { defaultValue: "Service ports" })}</div>
                        <p className="mt-1 text-sm text-muted-foreground">
                          {t("app.executionWorkspaceDetail.setAFixedPortForAServiceOrLeaveItBlankToUseItsConfiguredAutomaticBehaviorEditingAnInheritedServiceCreatesAnExecutionWorkspaceRuntimeOverride", { defaultValue: "Set a fixed port for a service or leave it blank to use its configured automatic behavior. Editing an inherited service creates an execution-workspace runtime override." })}</p>
                      </div>
                      <div className="grid gap-3 sm:grid-cols-2">
                        {configuredRuntimeServicePorts.map((service) => (
                          <Field key={`${service.collection}-${service.index}`} label={service.name} hint={t("app.executionWorkspaceDetail.fixedPort", { defaultValue: "Fixed port" })}>
                            <Input
                              type="number"
                              min="1"
                              max="65535"
                      ...
                      <p className="text-sm text-muted-foreground">
                        {t("app.executionWorkspaceDetail.paperclipChecksFixedPortsAgainWhenAServiceStartsAndRejectsCrossWorkspaceConflicts", { defaultValue: "Paperclip checks fixed ports again when a service starts and rejects cross-workspace conflicts." })}</p>
                    </div>
                  ) : null}
                </div>
              </div>
              ...
              <div className="mt-6 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                <Button className="w-full sm:w-auto" disabled={!isDirty || updateWorkspace.isPending} onClick={saveChanges}>
                  ...
                  {t("app.executionWorkspaceDetail.saveChanges", { defaultValue: "Save changes" })}</Button>
                <Button
                  variant="outline"
                  className="w-full sm:w-auto"
                ...
                >
                  {t("app.executionWorkspaceDetail.reset", { defaultValue: "Reset" })}</Button>
                {errorMessage ? <p className="text-sm text-destructive">{errorMessage}</p> : null}
                {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">{t("app.executionWorkspaceDetail.noUnsavedChanges", { defaultValue: "No unsaved changes." })}</p> : null}
              </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("app.executionWorkspaceDetail.workspaceContext", { defaultValue: "Workspace context" })}</CardTitle>
                <CardDescription>{t("app.executionWorkspaceDetail.linkedObjectsAndRelationships", { defaultValue: "Linked objects and relationships" })}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label={t("app.executionWorkspaceDetail.project", { defaultValue: "Project" })}>
                {project ? <Link to={`/projects/${projectRef}`} className="hover:underline">{project.name}</Link> : <MonoValue value={workspace.projectId} />}
              </DetailRow>
              <DetailRow label={t("app.executionWorkspaceDetail.projectWorkspace", { defaultValue: "Project workspace" })}>
              ...
              <DetailRow label={t("app.executionWorkspaceDetail.sourceTask", { defaultValue: "Source task" })}>
                {sourceIssue ? (
                  <Link to={issueUrl(sourceIssue)} className="hover:underline">
                    {sourceIssue.identifier ?? sourceIssue.id} {t("app.executionWorkspaceDetail.text", { defaultValue: "· " })}{sourceIssue.title}
                  </Link>
              ...
              <DetailRow label={t("app.executionWorkspaceDetail.derivedFrom", { defaultValue: "Derived from" })}>
                {derivedWorkspace ? (
              ...
              <DetailRow label={t("app.executionWorkspaceDetail.runtimeProvisioning", { defaultValue: "Runtime provisioning" })}>
                <RuntimeProvisionStatusValue
                  status={runtimeProvisionStatus}
                  onViewLogs={() => handleTabChange("runtime_logs")}
                />
              </DetailRow>
              <DetailRow label={t("app.executionWorkspaceDetail.workspaceId", { defaultValue: "Workspace ID" })}>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("app.executionWorkspaceDetail.concreteLocation", { defaultValue: "Concrete location" })}</CardTitle>
                <CardDescription>{t("app.executionWorkspaceDetail.pathsAndRefs", { defaultValue: "Paths and refs" })}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label={t("app.executionWorkspaceDetail.workingDir", { defaultValue: "Working dir" })}>
                {workspace.cwd ? <MonoValue value={workspace.cwd} copy /> : t("app.executionWorkspaceDetail.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("app.executionWorkspaceDetail.providerRef", { defaultValue: "Provider ref" })}>
                {workspace.providerRef ? <MonoValue value={workspace.providerRef} copy /> : t("app.executionWorkspaceDetail.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("app.executionWorkspaceDetail.repoUrl", { defaultValue: "Repo URL" })}>
              ...
              <DetailRow label={t("app.executionWorkspaceDetail.baseRef", { defaultValue: "Base ref" })}>
                {workspace.baseRef ? <MonoValue value={workspace.baseRef} copy /> : t("app.executionWorkspaceDetail.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("app.executionWorkspaceDetail.branch", { defaultValue: "Branch" })}>
                {workspace.branchName ? <MonoValue value={workspace.branchName} copy /> : t("app.executionWorkspaceDetail.none", { defaultValue: "None" })}
              </DetailRow>
              <DetailRow label={t("app.executionWorkspaceDetail.opened", { defaultValue: "Opened" })}>{formatDateTime(workspace.openedAt)}</DetailRow>
              <DetailRow label={t("app.executionWorkspaceDetail.lastUsed", { defaultValue: "Last used" })}>{formatDateTime(workspace.lastUsedAt)}</DetailRow>
              <DetailRow label={t("app.executionWorkspaceDetail.cleanup", { defaultValue: "Cleanup" })}>
                {workspace.cleanupEligibleAt
                  ? `${formatDateTime(workspace.cleanupEligibleAt)}${workspace.cleanupReason ? ` · ${workspace.cleanupReason}` : ""}`
                  : t("app.executionWorkspaceDetail.notScheduled", { defaultValue: "Not scheduled" })}
              </DetailRow>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="rounded-none">
            <CardHeader>
              <CardTitle>{t("app.executionWorkspaceDetail.runtimeAndCleanupLogs", { defaultValue: "Runtime and cleanup logs" })}</CardTitle>
              <CardDescription>{t("app.executionWorkspaceDetail.recentOperations", { defaultValue: "Recent operations" })}</CardDescription>
            </CardHeader>
            <CardContent>
            {workspaceOperationsQuery.isLoading ? (
              <p className="text-sm text-muted-foreground">{t("app.executionWorkspaceDetail.loadingWorkspaceOperations", { defaultValue: "Loading workspace operations…" })}</p>
            ) : workspaceOperationsQuery.error ? (
              <p className="text-sm text-destructive">
                {workspaceOperationsQuery.error instanceof Error
                  ? workspaceOperationsQuery.error.message
                  : t("app.executionWorkspaceDetail.failedToLoadWorkspaceOperations", { defaultValue: "Failed to load workspace operations." })}
              </p>
              ...
              <p className="text-sm text-muted-foreground">{t("app.executionWorkspaceDetail.noWorkspaceOperationsHaveBeenRecordedYet", { defaultValue: "No workspace operations have been recorded yet." })}</p>
            )}
            </CardContent>
          </Card>
        ) : activeTab === "issues" ? (
          <div className="space-y-6">
            <SummarySlotCard
              ...
              title={t("app.executionWorkspaceDetail.workspaceSummary", { defaultValue: "Workspace summary" })}
              description={t("app.executionWorkspaceDetail.summarizerKeepsTheLatestWorkspaceStatusNextStepAndOperatorNeededItemsHere", { defaultValue: "Summarizer keeps the latest workspace status, next step, and operator-needed items here." })}
            />
          ...
          <Card>
            <CardContent className="py-6 text-sm text-muted-foreground">{t("app.executionWorkspaceDetail.loadingWorkspacePlugin", { defaultValue: "Loading workspace plugin..." })}</CardContent>
          </Card>

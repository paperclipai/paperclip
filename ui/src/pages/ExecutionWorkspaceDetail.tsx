...
import type { ExecutionWorkspace, Issue, Project, ProjectWorkspace, RoutineListItem } from "@valadrien-os/shared";
...
function LegacyWorkspaceTabRedirect({ workspaceId }: { workspaceId: string }) {
  useEffect(() => {
    try {
      localStorage.removeItem(`valadrien-os:execution-workspace-tab:${workspaceId}`);
    } catch {}
  }, [workspaceId]);
...
function ExecutionWorkspaceIssuesList({
  ...
  return (
    <IssuesList
      ...
      viewStateKey="valadrien-os:execution-workspace-issues-view"
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      pushToast({
        ...
        body: "ValadrienOs created a run using this execution workspace.",
        tone: "success",
      });
    },
    onSettled: () => {
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : "ValadrienOs could not start the routine run.",
        tone: "error",
      });
    },
  });

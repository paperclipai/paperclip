...
export function RuntimeProvisionStatusValue({
  ...
  if (status.kind === "eager") {
    return (
      <span className="text-sm text-muted-foreground">{t("execution-workspace-detail.eager-provisioned-during-workspace-s-hfn")}</span>
    );
  }
  if (status.kind === "deferred") {
    return (
      <div className="flex flex-col gap-1">
        <StatusPill className="border-amber-500/40 text-amber-600 dark:text-amber-400">{t("execution-workspace-detail.deferred-ova")}</StatusPill>
        <span className="text-xs text-muted-foreground">
          {t("execution-workspace-detail.runs-once-before-the-first-runtime-s-16p")}
        </span>
      </div>
    );
  }
  if (status.kind === "provisioning") {
    return (
      <StatusPill className="border-border text-muted-foreground">
        ...
        {t("execution-workspace-detail.provisioning-se3")}
      </StatusPill>
    );
  }
  if (status.kind === "provisioned") {
    return (
      <StatusPill className="border-emerald-500/40 text-emerald-600 dark:text-emerald-400">
        {t("execution-workspace-detail.provisioned-1hb")}{status.at ? ` · ${formatDateTime(status.at)}` : ""}
      </StatusPill>
    );
  }
  return (
    <div className="flex flex-col gap-1">
      <StatusPill className="border-destructive/50 text-destructive">
        {t("execution-workspace-detail.provisioning-failed-123")}{status.at ? ` · ${formatDateTime(status.at)}` : ""}
      </StatusPill>
      <button type="button" onClick={onViewLogs} className="self-start text-xs text-muted-foreground underline underline-offset-2 hover:text-foreground">
        {t("execution-workspace-detail.view-runtime-logs-1mr")}
      </button>
    </div>
  );
}
    ...
    <div className="flex flex-col gap-3 border-b border-border px-3 py-3 last:border-b-0 sm:flex-row sm:items-center">
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          ...
          <span>{t("execution-workspace-detail.last-run-k0w")} {formatOptionalDateTime(routine.lastRun?.triggeredAt ?? routine.lastTriggeredAt)}</span>
  ...
  return (
    <>
      <Card className="rounded-none">
        <CardHeader>
          <CardTitle>{t("execution-workspace-detail.workspace-routines-liq")}</CardTitle>
          <CardDescription>
            {t("execution-workspace-detail.routines-that-use-workspace-specific-ccy")}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <p className="text-sm text-muted-foreground">{t("execution-workspace-detail.loading-routines-uj6")}</p>
          ) : error ? (
            ...
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {t("execution-workspace-detail.no-routines-use-workspace-specific-v-n47")}
              </p>
            </div>
          ) : (
...
export function ExecutionWorkspaceDetail() {
  ...
  if (workspaceQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("execution-workspace-detail.loading-workspace-wfr")}</p>;
      ...
      <div className="space-y-4 overflow-hidden sm:space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 space-y-2">
            <div className="text-xs font-medium uppercase tracking-(--tracking-eyebrow) text-muted-foreground">
              {t("execution-workspace-detail.execution-workspace-1he")}
            </div>
        ...
        {activeTab === "services" ? (
          ...
          <div className="space-y-4 sm:space-y-6">
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("execution-workspace-detail.workspace-settings-95b")}</CardTitle>
                <CardDescription>
                  {t("execution-workspace-detail.edit-the-concrete-path-repo-branch-p-j9w")}
                </CardDescription>
                <CardAction>
                  <Button
                    variant="destructive"
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("execution-workspace-detail.general-nov")}</div>
                  <Field label={t("execution-workspace-detail.workspace-name-4w5")}>
                    <Input
                      value={form.name}
                      onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                      placeholder={t("execution-workspace-detail.execution-workspace-name-p0j")}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("execution-workspace-detail.source-control-n55")}</div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label={t("execution-workspace-detail.branch-name-rl7")} hint="Useful for isolated worktrees">
                      <Input
                        className="font-mono"
                        value={form.branchName}
                        onChange={(event) => setForm((current) => current ? { ...current, branchName: event.target.value } : current)}
                        placeholder={t("execution-workspace-detail.pap-946-workspace-dny")}
                      />
                    </Field>

                    <Field label={t("execution-workspace-detail.base-ref-1ha")}>
                      <Input
                        className="font-mono"
                        value={form.baseRef}
                        onChange={(event) => setForm((current) => current ? { ...current, baseRef: event.target.value } : current)}
                        placeholder={t("execution-workspace-detail.origin-main-1gk")}
                      />
                    </Field>
                  </div>

                  <Field label={t("execution-workspace-detail.repo-url-1jn")}>
                    <Input
                      value={form.repoUrl}
                ...
                */}
                {!hideHostPaths && (
                  <>
                    <div className="space-y-4">
                      <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("execution-workspace-detail.paths-1f1")}</div>
                      <Field label={t("execution-workspace-detail.working-directory-cpj")}>
                        <Input
                          className="font-mono"
                          value={form.cwd}
                      ...
                      <Field label={t("execution-workspace-detail.provider-path-ref-12k")}>
                        <Input
                          className="font-mono"
                ...
                */}
                {!hideHostPaths && (
                  <>
                    <div className="space-y-4">
                      <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("execution-workspace-detail.lifecycle-commands-1nb")}</div>
                      <Field label={t("execution-workspace-detail.provision-command-yc2")} hint="Runs when Paperclip prepares this execution workspace">
                        <Textarea
                          className="min-h-20 font-mono"
                          value={form.provisionCommand}
                          onChange={(event) => setForm((current) => current ? { ...current, provisionCommand: event.target.value } : current)}
                          placeholder={t("execution-workspace-detail.bash-scripts-provision-worktree-sh-9ob")}
                        />
                      </Field>

                      <Field
                        label={t("execution-workspace-detail.runtime-provision-command-19u")}
                      ...
                      <Field label={t("execution-workspace-detail.teardown-command-zlz")} hint="Runs when the execution workspace is archived or cleaned up">
                        <Textarea
                          className="min-h-20 font-mono"
                          value={form.teardownCommand}
                          onChange={(event) => setForm((current) => current ? { ...current, teardownCommand: event.target.value } : current)}
                          placeholder={t("execution-workspace-detail.bash-scripts-teardown-worktree-sh-h95")}
                        />
                      </Field>

                      <Field label={t("execution-workspace-detail.cleanup-command-oud")} hint="Workspace-specific cleanup before teardown">
                        <Textarea
                          className="min-h-16 font-mono"
                          value={form.cleanupCommand}
                          onChange={(event) => setForm((current) => current ? { ...current, cleanupCommand: event.target.value } : current)}
                          placeholder={t("execution-workspace-detail.pkill-f-vite-true-i8p")}
                        />
                      </Field>
                    </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("execution-workspace-detail.runtime-config-1q7")}</div>
                  <div className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                      <div className="space-y-1">
                        <div className="text-sm font-medium text-foreground">
                          {t("execution-workspace-detail.runtime-config-source-1yh")}
                        </div>
                        <p className="text-sm text-muted-foreground">
                      ...
                      >
                        {t("execution-workspace-detail.reset-to-inherit-1hi")}
                      </Button>
                    </div>
                  </div>

                  <details className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <summary className="cursor-pointer text-sm font-medium">{t("execution-workspace-detail.advanced-runtime-json-ktr")}</summary>
                    <p className="mt-2 text-sm text-muted-foreground">
                      {t("execution-workspace-detail.override-the-inherited-workspace-com-17t")}
                    </p>
                    <div className="mt-3">
                      <Field label={t("execution-workspace-detail.workspace-commands-json-e4p")} hint="Legacy `services` arrays still work, but `commands` supports both services and jobs.">
                        <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          />
                          <label htmlFor="inherit-runtime-config">{t("execution-workspace-detail.inherit-project-workspace-runtime-co-181")}</label>
                        </div>
                        <Textarea
                  ...
                  {configuredRuntimeServicePorts.length > 0 ? (
                    <div className="space-y-3 rounded-md border border-border bg-muted/20 p-4">
                      <div>
                        <div className="text-sm font-medium">{t("execution-workspace-detail.service-ports-qx9")}</div>
                        <p className="mt-1 text-sm text-muted-foreground">
                          {t("execution-workspace-detail.set-a-fixed-port-for-a-service-or-le-owx")}
                        </p>
                      </div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {t("execution-workspace-detail.paperclip-checks-fixed-ports-again-w-1kd")}
                      </p>
                    </div>
                  ) : null}
                </div>
              ...
              <div className="mt-6 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                <Button className="w-full sm:w-auto" disabled={!isDirty || updateWorkspace.isPending} onClick={saveChanges}>
                  ...
                  {t("execution-workspace-detail.save-changes-r69")}
                </Button>
                <Button
                  variant="outline"
                  className="w-full sm:w-auto"
                ...
                >
                  {t("execution-workspace-detail.reset-2zp")}
                </Button>
                {errorMessage ? <p className="text-sm text-destructive">{errorMessage}</p> : null}
                {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">{t("execution-workspace-detail.no-unsaved-changes-11u")}</p> : null}
              </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("execution-workspace-detail.workspace-context-1ju")}</CardTitle>
                <CardDescription>{t("execution-workspace-detail.linked-objects-and-relationships-1y0")}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label={t("execution-workspace-detail.project-y8c")}>
                {project ? <Link to={`/projects/${projectRef}`} className="hover:underline">{project.name}</Link> : <MonoValue value={workspace.projectId} />}
              </DetailRow>
              <DetailRow label={t("execution-workspace-detail.project-workspace-1yk")}>
              ...
              <DetailRow label={t("execution-workspace-detail.source-task-19x")}>
                {sourceIssue ? (
              ...
              <DetailRow label={t("execution-workspace-detail.derived-from-1xr")}>
                {derivedWorkspace ? (
              ...
              <DetailRow label={t("execution-workspace-detail.runtime-provisioning-17y")}>
                <RuntimeProvisionStatusValue
                  status={runtimeProvisionStatus}
                  onViewLogs={() => handleTabChange("runtime_logs")}
                />
              </DetailRow>
              <DetailRow label={t("execution-workspace-detail.workspace-id-1mv")}>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("execution-workspace-detail.concrete-location-1l0")}</CardTitle>
                <CardDescription>{t("execution-workspace-detail.paths-and-refs-161")}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label={t("execution-workspace-detail.working-dir-kiq")}>
                {workspace.cwd ? <MonoValue value={workspace.cwd} copy /> : "None"}
              </DetailRow>
              <DetailRow label={t("execution-workspace-detail.provider-ref-1pj")}>
                {workspace.providerRef ? <MonoValue value={workspace.providerRef} copy /> : "None"}
              </DetailRow>
              <DetailRow label={t("execution-workspace-detail.repo-url-1jn")}>
              ...
              <DetailRow label={t("execution-workspace-detail.base-ref-1ha")}>
                {workspace.baseRef ? <MonoValue value={workspace.baseRef} copy /> : "None"}
              </DetailRow>
              <DetailRow label={t("execution-workspace-detail.branch-19g")}>
                {workspace.branchName ? <MonoValue value={workspace.branchName} copy /> : "None"}
              </DetailRow>
              <DetailRow label={t("execution-workspace-detail.opened-45z")}>{formatDateTime(workspace.openedAt)}</DetailRow>
              <DetailRow label={t("execution-workspace-detail.last-used-7s3")}>{formatDateTime(workspace.lastUsedAt)}</DetailRow>
              <DetailRow label={t("execution-workspace-detail.cleanup-81e")}>
          ...
          <Card className="rounded-none">
            <CardHeader>
              <CardTitle>{t("execution-workspace-detail.runtime-and-cleanup-logs-1gm")}</CardTitle>
              <CardDescription>{t("execution-workspace-detail.recent-operations-5br")}</CardDescription>
            </CardHeader>
            <CardContent>
            {workspaceOperationsQuery.isLoading ? (
              <p className="text-sm text-muted-foreground">{t("execution-workspace-detail.loading-workspace-operations-9wb")}</p>
              ...
              <p className="text-sm text-muted-foreground">{t("execution-workspace-detail.no-workspace-operations-have-been-re-5rj")}</p>
            )}
            </CardContent>
          </Card>
        ) : activeTab === "issues" ? (
          <div className="space-y-6">
            <SummarySlotCard
              ...
              title={t("execution-workspace-detail.workspace-summary-ifh")}
              description={t("execution-workspace-detail.summarizer-keeps-the-latest-workspac-gu7")}
            />
          ...
          <Card>
            <CardContent className="py-6 text-sm text-muted-foreground">{t("execution-workspace-detail.loading-workspace-plugin-gzf")}</CardContent>
          </Card>

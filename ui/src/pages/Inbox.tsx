...
function runFailureMessage(t: (key: string) => string, run: HeartbeatRun): string {
  return firstNonEmptyLine(run.error) ?? firstNonEmptyLine(run.stderrExcerpt) ?? t("inbox.run_exited_with_error");
}

function approvalStatusLabel(t: (key: string, params?: Record<string, string | number>) => string, status: Approval["status"]): string {
  const key = `status.${status}`;
  const translated = t(key);
  return translated === key ? status.replaceAll("_", " ") : translated;
}

function inboxGroupLabel(
  ...
  switch (key) {
    case "issue":
      return t("nav.issues");
    case "approval":
      return t("inbox.category_approvals");
    case "failed_run":
      return t("inbox.category_failed_runs");
    case "join_request":
      return t("inbox.category_join_requests");
    default:
      return fallback;
  }
}
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              ...
              {requesterName ? <span>{t("approval_detail.requested_by")} {requesterName}</span> : null}
              <span>{t("inbox.updated_relative", { value: formatRelativeTime(approval.updatedAt) })}</span>
            </span>
          </span>
        </Link>
  ...
  const label =
    joinRequest.requestType === "human"
      ...
      : joinRequest.agentName
        ? t("inbox.agent_join_request", { agentName: joinRequest.agentName })
        : t("inbox.agent_join_request_no_name");
  const showUnreadSlot = unreadState !== null;
  const showUnreadDot = unreadState === "visible" || unreadState === "fading";
      ...
      <div className="flex items-start gap-2 sm:items-center">
        ...
        <div className="flex min-w-0 flex-1 items-start gap-2">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              <span>{t("inbox.requested_from_ip", { value: formatRelativeTime(joinRequest.createdAt), ip: joinRequest.requestIp })}</span>
              {joinRequest.adapterType && <span>{t("inbox.adapter_type", { value: joinRequest.adapterType })}</span>}
            </span>
          </span>
        </div>
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
...
export function Inbox() {
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("approval_detail.approve_failed"));
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("approval_detail.reject_failed"));
    },
  });
  ...
  const approveJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.approve_join_request_failed"));
    },
  });
  ...
  const rejectJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.reject_join_request_failed"));
    },
  });
  ...
  const archiveIssueMutation = useMutation({
    ...
    onError: (err, id, context) => {
      setActionError(err instanceof Error ? err.message : t("issueDetail.archive_failed"));
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            title={nestingEnabled ? t("inbox.disable_nesting") : t("inbox.enable_nesting")}
          >
            ...
            <>
              ...
              <Dialog open={showMarkAllReadConfirm} onOpenChange={setShowMarkAllReadConfirm}>
                <DialogContent className="sm:max-w-md">
                  <DialogHeader>
                    <DialogTitle>{t("inbox.mark_all_as_read_title")}</DialogTitle>
                    <DialogDescription>
                      {t("inbox.mark_all_as_read_desc", { count: unreadIssueIds.length, itemOrItems: unreadIssueIds.length === 1 ? t("inbox.item") : t("inbox.items") })}
                    </DialogDescription>
                  </DialogHeader>
                  <DialogFooter>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="issues_i_touched">{t("inbox.category_issues_i_touched")}</SelectItem>
          ...
          {showApprovalsCategory && (
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="all">{t("inbox.approval_all")}</SelectItem>
                <SelectItem value="actionable">{t("inbox.approval_actionable")}</SelectItem>
                <SelectItem value="resolved">{t("inbox.approval_resolved")}</SelectItem>
              </SelectContent>
            </Select>
          )}
        </div>
                ...
                return groupedSections.flatMap((group, groupIndex) => {
                  ...
                  if (group.label) {
                    elements.push(
                      ...
                      >
                        {inboxGroupLabel(t, group.key, group.label)}
                      </div>,
                    );
                  }

...
export function formatJoinRequestInboxLabel(
  ...
  if (joinRequest.requestType !== "human") {
    return joinRequest.agentName
      ? t("pages.inbox.agentJoinRequestNamed", {
        name: joinRequest.agentName,
        defaultValue: "Agent join request: {{name}}",
      })
      : t("pages.inbox.agentJoinRequest", { defaultValue: "Agent join request" });
  }
          ...
          <span className="min-w-0 flex-1">
            <span className="line-clamp-2 text-sm font-medium sm:truncate sm:line-clamp-none">
              {issue ? (
                ...
                <>
                  {linkedAgentName
                    ? t("pages.inbox.failedRunNamed", {
                      name: linkedAgentName,
                      defaultValue: "Failed run — {{name}}",
                    })
                    : t("pages.inbox.failedRun", { defaultValue: "Failed run" })}
                </>
              )}
            </span>
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              ...
              <span>
                {t("pages.inbox.updatedAt", {
                  time: timeAgo(approval.updatedAt),
...
export function Inbox() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("pages.inbox.breadcrumbInbox", { defaultValue: "Inbox" }) }]);
  }, [setBreadcrumbs]);
    ...
    onError: (err) => {
      setActionError(
        err instanceof Error
          ? err.message
          : t("pages.inbox.failedToApprove", { defaultValue: "Failed to approve" }),
      );
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(
        err instanceof Error
          ? err.message
          : t("pages.inbox.failedToReject", { defaultValue: "Failed to reject" }),
      );
    },
  });
  ...
  const approveJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(
        err instanceof Error
          ? err.message
          : t("pages.inbox.failedToApproveJoinRequest", { defaultValue: "Failed to approve join request" }),
      );
    },
  });
  ...
  const rejectJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(
        err instanceof Error
          ? err.message
          : t("pages.inbox.failedToRejectJoinRequest", { defaultValue: "Failed to reject join request" }),
      );
    },
  });
  ...
  const archiveIssueMutation = useMutation({
    ...
    onError: (err, id, context) => {
      setActionError(
        err instanceof Error
          ? err.message
          : t("pages.inbox.failedToArchiveTask", { defaultValue: "Failed to archive task" }),
      );
  ...
  const unarchiveIssueMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(
        err instanceof Error
          ? err.message
          : t("pages.inbox.failedToUndoArchive", { defaultValue: "Failed to undo inbox archive" }),
      );
    },
    onSuccess: (_data, id) => {
  ...
  if (!selectedCompanyId) {
    return (
      <EmptyState
        icon={InboxIcon}
        message={t("pages.inbox.selectCompany", { defaultValue: "Select a company to view inbox." })}
      />
    );
  }
    ...
    <div className="space-y-6">
          ...
          {tab === "blocked" ? (
            <>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    title={t("pages.inbox.groupTitle", { defaultValue: "Group" })}
                  >
              ...
              <IssueColumnPicker
                ...
                title={t("pages.inbox.columnPickerTitle", { defaultValue: "Choose which inbox columns stay visible" })}
                iconOnly
              />
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    title={t("pages.inbox.sortTitle", { defaultValue: "Sort" })}
                  >
              ...
              <Popover>
                ...
                <PopoverContent align="end" className="w-40 p-2">
                  <div className="space-y-0.5">
                    {([
                      ["none", t("pages.inbox.groupByNone", { defaultValue: "None" })],
                      ["type", t("pages.inbox.groupByType", { defaultValue: "Type" })],
                      ["assignee", t("pages.inbox.groupByAssignee", { defaultValue: "Assignee" })],
                ...
                <>
                  ...
                  <Dialog open={showMarkAllReadConfirm} onOpenChange={setShowMarkAllReadConfirm}>
                    <DialogContent className="sm:max-w-md">
                      <DialogHeader>
                        <DialogTitle>
                          {t("pages.inbox.markAllConfirmTitle", { defaultValue: "Mark all as read?" })}
                        </DialogTitle>
                        <DialogDescription>
                          {t("pages.inbox.markAllConfirmDescription", {
                            count: unreadIssueIds.length,
                            defaultValue: "This will mark {{count}} unread items as read.",
                          })}
                        </DialogDescription>
                      </DialogHeader>
                      <DialogFooter>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <SelectTrigger className="h-8 w-[170px] text-xs">
              <SelectValue placeholder={t("pages.inbox.categoryPlaceholder", { defaultValue: "Category" })} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="everything">{t("pages.inbox.categoryAll", { defaultValue: "All categories" })}</SelectItem>
              <SelectItem value="issues_i_touched">{t("pages.inbox.categoryMyRecentTasks", { defaultValue: "My recent tasks" })}</SelectItem>
              <SelectItem value="join_requests">{t("pages.inbox.categoryJoinRequests", { defaultValue: "Join requests" })}</SelectItem>
              <SelectItem value="approvals">{t("pages.inbox.categoryApprovals", { defaultValue: "Approvals" })}</SelectItem>
              <SelectItem value="failed_runs">{t("pages.inbox.categoryFailedRuns", { defaultValue: "Failed runs" })}</SelectItem>
              <SelectItem value="alerts">{t("pages.inbox.categoryAlerts", { defaultValue: "Alerts" })}</SelectItem>
            </SelectContent>
          </Select>
          ...
          {showApprovalsCategory && (
            ...
            >
              <SelectTrigger className="h-8 w-[170px] text-xs">
                <SelectValue placeholder={t("pages.inbox.approvalStatusPlaceholder", { defaultValue: "Approval status" })} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t("pages.inbox.approvalStatusAll", { defaultValue: "All approval statuses" })}</SelectItem>
                <SelectItem value="actionable">{t("pages.inbox.approvalStatusActionable", { defaultValue: "Needs action" })}</SelectItem>
                <SelectItem value="resolved">{t("pages.inbox.approvalStatusResolved", { defaultValue: "Resolved" })}</SelectItem>
              </SelectContent>
            </Select>
          )}
        </div>
      ...
      {tab !== "blocked" && allLoaded && visibleSections.length === 0 && (
        <EmptyState
          ...
          message={
            searchQuery.trim()
              ? t("pages.inbox.emptyNoSearchMatch", { defaultValue: "No inbox items match your search." })
              : tab === "mine"
              ? t("pages.inbox.emptyInboxZero", { defaultValue: "Inbox zero." })
              : tab === "unread"
              ? t("pages.inbox.emptyNoNew", { defaultValue: "No new inbox items." })
              : tab === "recent"
                ? t("pages.inbox.emptyNoRecent", { defaultValue: "No recent inbox items." })
                : t("pages.inbox.emptyNoFilterMatch", { defaultValue: "No inbox items match these filters." })
          }
        />
      )}
                      ...
                      >
                        ...
                        <span className="shrink-0 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                          {group.searchSection === "archived"
                            ? t("pages.inbox.searchSectionArchived", { defaultValue: "Archived" })
                            : t("pages.inbox.searchSectionOther", { defaultValue: "Other results" })}
                        </span>
                        <div className="h-px flex-1 bg-border/80" />
                      </div>,
                    );
                        ...
                        <IssueGroupHeader
                          ...
                          trailing={canCreateIssueInGroup ? (
                            <Button
                              ...
                              title={t("pages.inbox.newTaskInGroup", { group: group.label, defaultValue: "New task in {{group}}" })}
                              aria-label={t("pages.inbox.newTaskInGroup", { group: group.label, defaultValue: "New task in {{group}}" })}
                              onClick={(event) => {
                                event.stopPropagation();
                      ...
                      elements.push(
                        <div key={`today-divider-${group.key}-${index}`} className="my-2 flex items-center gap-3 px-4">
                          ...
                          <span className="shrink-0 text-[11px] font-medium uppercase tracking-wider text-zinc-500">
                            {t("pages.inbox.dividerEarlier", { defaultValue: "Earlier" })}
                          </span>
                        </div>,
                      );
                    }
        ...
        <>
          ...
          <div>
            <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">
              {t("pages.inbox.alertsHeading", { defaultValue: "Alerts" })}
            </h3>
              ...
              {showBudgetAlert && (
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      {t("pages.inbox.budgetAtPrefix", { defaultValue: "Budget at" })}{" "}
                      <span className="font-medium">{dashboard!.costs.monthUtilizationPercent}%</span>{" "}
                      {t("pages.inbox.budgetUtilizationSuffix", { defaultValue: "utilization this month" })}
                    </span>
                  </Link>
                  <button
                    type="button"

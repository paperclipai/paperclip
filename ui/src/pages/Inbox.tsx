...
function approvalStatusLabel(status: Approval["status"], t: (key: string) => string): string {
  const key = `inbox:approvalStatusLabels.${status}`;
  const translated = t(key);
  return translated !== key ? translated : status.replaceAll("_", " ");
}
          ...
          <span className="min-w-0 flex-1">
            <span className="line-clamp-2 text-sm font-medium sm:truncate sm:line-clamp-none">
              {issue ? (
                ...
                <>{t("inbox:labels.failedRun")}{linkedAgentName ? ` — ${linkedAgentName}` : ""}</>
              )}
            </span>
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              ...
              {requesterName ? <span>{t("inbox:labels.requestedBy", { name: requesterName })}</span> : null}
              <span>{t("inbox:labels.updated", { time: timeAgo(approval.updatedAt) })}</span>
            </span>
          </span>
        </Link>
      ...
      <div className="flex items-start gap-2 sm:items-center">
        ...
        <div className="flex min-w-0 flex-1 items-start gap-2">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              <span>{t("inbox:labels.requestedFromIp", { time: timeAgo(joinRequest.createdAt), ip: joinRequest.requestIp })}</span>
              {joinRequest.adapterType && <span>{t("inbox:labels.adapter", { type: joinRequest.adapterType })}</span>}
            </span>
          </span>
        </div>
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
...
export function Inbox() {
  ...
  const approveJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox:errors.failedToApproveJoin"));
    },
  });
  ...
  const rejectJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox:errors.failedToRejectJoin"));
    },
  });
  ...
  const retryRunMutation = useMutation({
    mutationFn: async (run: HeartbeatRun) => {
      ...
      if (!("id" in result)) {
        throw new Error(t("inbox:errors.retrySkipped"));
      }
  ...
  const archiveIssueMutation = useMutation({
    ...
    onError: (err, id) => {
      setActionError(err instanceof Error ? err.message : t("inbox:errors.failedToArchive"));
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <SelectTrigger className="h-8 w-[170px] text-xs">
              <SelectValue placeholder={t("inbox:labels.category")} />
            </SelectTrigger>
            <SelectContent>
          ...
          {showApprovalsCategory && (
            ...
            >
              <SelectTrigger className="h-8 w-[170px] text-xs">
                <SelectValue placeholder={t("inbox:labels.approvalStatus")} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t("inbox:approvalFilters.allStatuses")}</SelectItem>
          ...
          message={
            tab === "mine"
              ...
              : tab === "recent"
                ...
                : t("inbox:empty.noMatchingFilters")
          }
        />
      )}
                  ...
                  <IssueRow
                    ...
                    mobileMeta={
                      issue.lastExternalCommentAt
                        ? t("inbox:labels.commented", { time: timeAgo(issue.lastExternalCommentAt) })
                        : t("inbox:labels.updated", { time: timeAgo(issue.updatedAt) })
                    }
                    unreadState={
                ...
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      ...
                      {dashboard!.agents.error === 1 ? t("inbox:alerts.agentErrorSingular", { count: dashboard!.agents.error }) : t("inbox:alerts.agentErrorPlural", { count: dashboard!.agents.error })}
                    </span>
                  </Link>
                  <button
                    type="button"
              ...
              {showBudgetAlert && (
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      {t("inbox:alerts.budgetUtilization", { percent: dashboard!.costs.monthUtilizationPercent })}
                    </span>
                  </Link>
                  <button
                    type="button"

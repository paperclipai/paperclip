...
async function waitForTestProviderButtons(container: HTMLElement, expectedCount: number) {
  await waitForAssertion(() => {
    expect(testProviderButtons(container)).toHaveLength(expectedCount);
  });
  return testProviderButtons(container);
}
...
describe("CompanyEnvironments — test provider button", () => {
  ...
  it("shows the testing state only on the clicked environment's button", async () => {
    ...
    const buttonsBefore = await waitForTestProviderButtons(container, 2);
  ...
  it("explains that successful sandbox provider tests use a temporary sandbox", async () => {
    ...
    const [testButton] = await waitForTestProviderButtons(container, 1);
    await act(async () => {
  ...
  it("keeps the second environment's testing state when an earlier probe settles", async () => {
    ...
    let buttons = await waitForTestProviderButtons(container, 2);
    // Click both rows in quick succession while both probes are still pending.
    await act(async () => {
      buttons[0].dispatchEvent(new MouseEvent("click", { bubbles: true }));
    });
    await waitForAssertion(() => {
      expect(testProviderButtons(container)[0].textContent?.trim()).toBe("Testing...");
    });
    buttons = await waitForTestProviderButtons(container, 2);
    await act(async () => {
    ...
    await waitForAssertion(() => {
      const currentButtons = testProviderButtons(container);
      expect(currentButtons).toHaveLength(2);
      expect(currentButtons[1].textContent?.trim()).toBe("Testing...");
      expect(currentButtons[1].disabled).toBe(true);
    });
  });

  ...
  return (
    <div className="rounded-lg border border-border p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          {trigger.kind === "schedule" && trigger.nextRunAt
            ? t("routineDetail.triggerEditor.next", { time: new Date(trigger.nextRunAt).toLocaleString() })
            : trigger.kind === "webhook"
              ? t("routineDetail.triggerEditor.webhook")
              : t("routineDetail.triggerEditor.api")}
        </span>
      </div>

      <div className="grid gap-3 md:grid-cols-2">
        <div className="space-y-1.5">
          <Label className="text-xs">{t("routineDetail.triggerEditor.label")}</Label>
          <Input
            value={draft.label}
        ...
        {trigger.kind === "schedule" && (
          <div className="md:col-span-2 space-y-1.5">
            <Label className="text-xs">{t("routineDetail.triggerEditor.schedule")}</Label>
            <ScheduleEditor
        ...
        {trigger.kind === "webhook" && (
          <>
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routineDetail.triggerEditor.signingMode")}</Label>
              <Select
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routineDetail.triggerEditor.replayWindow")}</Label>
              <Input
      ...
      <div className="flex flex-wrap items-center gap-2">
        {trigger.lastResult && <span className="text-xs text-muted-foreground">{t("routineDetail.triggerEditor.last", { result: trigger.lastResult })}</span>}
        <div className="ml-auto flex items-center gap-2">
          {trigger.kind === "webhook" && (
            <Button variant="outline" size="sm" onClick={() => onRotate(trigger.id)}>
              <RefreshCw className="mr-1.5 h-3.5 w-3.5" />
              {t("routineDetail.triggerEditor.rotateSecret")}
            </Button>
          )}
          <Button
            variant="outline"
...
export function RoutineDetail() {
  ...
  useEffect(() => {
    if (!routine) return;
    setBreadcrumbs([{ label: t("routineDetail.breadcrumb"), href: "/routines" }, { label: routine.title }]);
  ...
  const copySecretValue = async (label: string, value: string) => {
    try {
      ...
      pushToast({ title: t("routineDetail.toasts.copied", { label }), tone: "success" });
    } catch (error) {
      pushToast({
        title: t("routineDetail.toasts.copyFailed", { label: label.toLowerCase() }),
        body: error instanceof Error ? error.message : t("routineDetail.toasts.clipboardDenied"),
        tone: "error",
      });
    }
  };
  ...
  const saveRoutine = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("routineDetail.toasts.saveFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("routineDetail.toasts.runFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onSuccess: async (_data, status) => {
      pushToast({
        title: t("routineDetail.toasts.saved"),
        body: status === "paused" ? t("routineDetail.toasts.automationPaused") : t("routineDetail.toasts.automationEnabled"),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("routineDetail.toasts.updateFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const createTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      if (result.secretMaterial) {
        setSecretMessage({
          title: t("routineDetail.secret.createdTitle"),
    ...
    onError: (error) => {
      pushToast({
        title: t("routineDetail.toasts.addTriggerFailed"),
        body: error instanceof Error ? error.message : t("routineDetail.toasts.addTriggerFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const updateTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("routineDetail.toasts.updateTriggerFailed"),
        body: error instanceof Error ? error.message : t("routineDetail.toasts.updateTriggerFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const deleteTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("routineDetail.toasts.deleteTriggerFailed"),
        body: error instanceof Error ? error.message : t("routineDetail.toasts.deleteTriggerFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const rotateTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      setSecretMessage({
        title: t("routineDetail.secret.rotatedTitle"),
        webhookUrl: result.secretMaterial.webhookUrl,
        webhookSecret: result.secretMaterial.webhookSecret,
      });
      await Promise.all([
    ...
    onError: (error) => {
      pushToast({
        title: t("routineDetail.toasts.rotateSecretFailed"),
        body: error instanceof Error ? error.message : t("routineDetail.toasts.rotateSecretFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const automationLabel = routine.status === "archived" ? t("routineDetail.automation.archived") : automationEnabled ? t("routineDetail.automation.active") : t("routineDetail.automation.paused");
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex shrink-0 items-center gap-3 pt-1">
          ...
          <button
            ...
            aria-label={automationEnabled ? t("routineDetail.automation.pauseAria") : t("routineDetail.automation.enableAria")}
      ...
      {secretMessage && (
        <div className="rounded-lg border border-blue-500/30 bg-blue-500/5 p-4 space-y-3 text-sm">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{t("routineDetail.secret.warning")}</p>
          </div>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Input value={secretMessage.webhookUrl} readOnly className="flex-1" />
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("routineDetail.secret.webhookUrlLabel"), secretMessage.webhookUrl)}>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("routineDetail.secret.webhookSecretLabel"), secretMessage.webhookSecret)}>
                <Copy className="h-3.5 w-3.5 mr-1" />
      ...
      <div className="overflow-x-auto overscroll-x-contain">
        <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
          <span>{t("routineDetail.assignment.for")}</span>
          <InlineEntitySelector
            ref={assigneeSelectorRef}
            value={editDraft.assigneeAgentId}
            options={assigneeOptions}
            placeholder={t("routineDetail.assignment.assigneePlaceholder")}
            noneLabel={t("routineDetail.assignment.noAssignee")}
            searchPlaceholder={t("routineDetail.assignment.searchAssignees")}
            emptyMessage={t("routineDetail.assignment.noAssigneesFound")}
            ...
            renderTriggerValue={(option) =>
              option ? (
                ...
                <span className="text-muted-foreground">{t("routineDetail.assignment.assigneePlaceholder")}</span>
              )
            }
            renderOption={(option) => {
          ...
          />
          <span>{t("routineDetail.assignment.in")}</span>
          <InlineEntitySelector
            ref={projectSelectorRef}
            value={editDraft.projectId}
            options={projectOptions}
            placeholder={t("routineDetail.assignment.projectPlaceholder")}
            noneLabel={t("routineDetail.assignment.noProject")}
            searchPlaceholder={t("routineDetail.assignment.searchProjects")}
            emptyMessage={t("routineDetail.assignment.noProjectsFound")}
            ...
            renderTriggerValue={(option) =>
              option && currentProject ? (
                ...
                <span className="text-muted-foreground">{t("routineDetail.assignment.projectPlaceholder")}</span>
              )
            }
            renderOption={(option) => {
              if (!option.id) return <span className="truncate">{option.label}</span>;
      ...
      <MarkdownEditor
        ...
        placeholder={t("routineDetail.instructionsPlaceholder")}
        bordered={false}
      ...
      <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
        <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
          <span className="text-sm font-medium">{t("routineDetail.advanced.heading")}</span>
          {advancedOpen ? <ChevronDown className="h-4 w-4 text-muted-foreground" /> : <ChevronRight className="h-4 w-4 text-muted-foreground" />}
        </CollapsibleTrigger>
        <CollapsibleContent className="pt-3">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routineDetail.advanced.concurrency")}</p>
              <Select
            ...
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routineDetail.advanced.catchUp")}</p>
              <Select
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          {t("routineDetail.actions.saveRoutine")}
        </Button>
      </div>
...
<TabsTrigger value="activity" className="gap-1.5">
        ...
        <TabsContent value="triggers" className="space-y-4">
          ...
          <div className="rounded-lg border border-border p-4 space-y-3">
            <p className="text-sm font-medium">{t("routineDetail.addTrigger.heading")}</p>
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routineDetail.addTrigger.kind")}</Label>
                  ...
                  <SelectContent>
                    {triggerKinds.map((kind) => (
                      <SelectItem key={kind} value={kind} disabled={kind === "webhook"}>
                        {kind}{kind === "webhook" ? t("routineDetail.addTrigger.comingSoon") : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
            ...
            <div className="flex items-center justify-end">
              <Button size="sm" onClick={() => createTrigger.mutate()} disabled={createTrigger.isPending}>
                {createTrigger.isPending ? t("routineDetail.addTrigger.adding") : t("routineDetail.addTrigger.add")}
              </Button>
            </div>
          </div>
          ...
          {routine.triggers.length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("routineDetail.empty.triggers")}</p>
          ) : (
        ...
        <TabsContent value="runs" className="space-y-4">
          ...
          {(routineRuns ?? []).length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("routineDetail.empty.runs")}</p>
          ) : (
        ...
        <TabsContent value="activity">
          {(activity ?? []).length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("routineDetail.empty.activity")}</p>
          ) : (
            <div className="border border-border rounded-lg divide-y divide-border">
              {(activity ?? []).map((event) => (

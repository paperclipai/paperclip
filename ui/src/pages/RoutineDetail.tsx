  ...
  return (
    <div className="rounded-lg border border-border p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          {trigger.kind === "schedule" && trigger.nextRunAt
            ? `${routineDetailPage.nextRunPrefix}${new Date(trigger.nextRunAt).toLocaleString()}`
            : trigger.kind === "webhook"
              ? routineDetailPage.webhook
              : routineDetailPage.api}
        </span>
      </div>

      <div className="grid gap-3 md:grid-cols-2">
        <div className="space-y-1.5">
          <Label className="text-xs">{routineDetailPage.label}</Label>
          <Input
            value={draft.label}
        ...
        {trigger.kind === "schedule" && (
          <div className="md:col-span-2 space-y-1.5">
            <Label className="text-xs">{routineDetailPage.schedule}</Label>
            <ScheduleEditor
        ...
        {trigger.kind === "webhook" && (
          <>
            <div className="space-y-1.5">
              <Label className="text-xs">{routineDetailPage.signingMode}</Label>
              <Select
            ...
            {!SIGNING_MODES_WITHOUT_REPLAY_WINDOW.has(draft.signingMode) && (
              <div className="space-y-1.5">
                <Label className="text-xs">{routineDetailPage.replayWindowSeconds}</Label>
                <Input
      ...
      <div className="flex flex-wrap items-center gap-2">
        {trigger.lastResult && <span className="text-xs text-muted-foreground">{routineDetailPage.lastPrefix}{trigger.lastResult}</span>}
        <div className="ml-auto flex items-center gap-2">
          {trigger.kind === "webhook" && (
            <Button variant="outline" size="sm" onClick={() => onRotate(trigger.id)}>
              <RefreshCw className="mr-1.5 h-3.5 w-3.5" />
              {routineDetailPage.rotateSecret}
            </Button>
          )}
          <Button
            variant="outline"
...
export function RoutineDetail() {
  ...
  const dirtyFields = useMemo<RoutineHistoryDirtyFieldDescriptor[]>(() => {
    ...
    if (editDraft.title !== routineDefaults.title) result.push({ key: "title", label: routineDetailPage.dirtyFieldTitle });
    if (editDraft.description !== routineDefaults.description) {
      result.push({ key: "description", label: routineDetailPage.dirtyFieldDescription });
    }
    if (editDraft.projectId !== routineDefaults.projectId) {
      result.push({ key: "projectId", label: routineDetailPage.dirtyFieldProject });
    }
    if (editDraft.assigneeAgentId !== routineDefaults.assigneeAgentId) {
      result.push({ key: "assigneeAgentId", label: routineDetailPage.dirtyFieldAssignee });
    }
    if (editDraft.priority !== routineDefaults.priority) {
      result.push({ key: "priority", label: routineDetailPage.dirtyFieldPriority });
    }
    if (editDraft.concurrencyPolicy !== routineDefaults.concurrencyPolicy) {
      result.push({ key: "concurrencyPolicy", label: routineDetailPage.dirtyFieldConcurrency });
    }
    if (editDraft.catchUpPolicy !== routineDefaults.catchUpPolicy) {
      result.push({ key: "catchUpPolicy", label: routineDetailPage.dirtyFieldCatchUp });
    }
  ...
  useEffect(() => {
    if (!routine) return;
    setBreadcrumbs([{ label: routineDetailPage.breadcrumb, href: "/routines" }, { label: routine.title }]);
    if (!routineDefaults) return;
  ...
  const copySecretValue = async (label: string, value: string) => {
    try {
      ...
      pushToast({ title: routinesShared.copyLabelCopied(label), tone: "success" });
    } catch (error) {
      pushToast({
        title: routinesShared.copyLabelFailed(label),
        body: error instanceof Error ? error.message : routinesShared.clipboardDenied,
        tone: "error",
      });
    }
  };
  ...
  const saveRoutine = useMutation({
    ...
    onError: (error) => {
      if (error instanceof ApiError && error.status === 409) {
        ...
        pushToast({
          title: routineDetailPage.routineChangedTitle,
          body: routineDetailPage.routineChangedBody,
          tone: "warn",
        });
        return;
      }
      pushToast({
        title: routineDetailPage.saveFailedTitle,
        body: error instanceof Error ? error.message : routineDetailPage.saveFailedBody,
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: routineDetailPage.runStartedTitle, tone: "success" });
    ...
    onError: (error) => {
      pushToast({
        title: routineDetailPage.runFailedTitle,
        body: error instanceof Error ? error.message : routineDetailPage.runFailedBody,
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onSuccess: async (_data, status) => {
      pushToast({
        title: routineDetailPage.routineSavedTitle,
        body: status === "paused" ? routineDetailPage.automationPaused : routineDetailPage.automationEnabled,
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        title: routineDetailPage.updateFailedTitle,
        body: error instanceof Error ? error.message : routineDetailPage.updateFailedBody,
        tone: "error",
      });
    },
  });
  ...
  const createTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      if (result.secretMaterial) {
        setSecretMessage({
          title: routineDetailPage.webhookCreatedTitle,
          entries: [{
        ...
        pushToast({
          title: routineDetailPage.triggerAddedTitle,
          body: routineDetailPage.triggerAddedBody,
          tone: "success",
        });
      }
      await Promise.all([
    ...
    onError: (error) => {
      pushToast({
        title: routineDetailPage.addTriggerFailedTitle,
        body: error instanceof Error ? error.message : routineDetailPage.addTriggerFailedBody,
        tone: "error",
      });
    },
  });
  ...
  const updateTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: routineDetailPage.triggerSavedTitle,
        body: routineDetailPage.triggerSavedBody,
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        title: routineDetailPage.updateTriggerFailedTitle,
        body: error instanceof Error ? error.message : routineDetailPage.updateTriggerFailedBody,
        tone: "error",
      });
    },
  });
  ...
  const deleteTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: routineDetailPage.triggerDeletedTitle,
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        title: routineDetailPage.deleteTriggerFailedTitle,
        body: error instanceof Error ? error.message : routineDetailPage.deleteTriggerFailedBody,
        tone: "error",
      });
    },
  });
  ...
  const rotateTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      setSecretMessage({
        title: routineDetailPage.secretRotatedTitle,
        entries: [{
          webhookUrl: result.secretMaterial.webhookUrl,
          webhookSecret: result.secretMaterial.webhookSecret,
        }],
    ...
    onError: (error) => {
      pushToast({
        title: routineDetailPage.rotateFailedTitle,
        body: error instanceof Error ? error.message : routineDetailPage.rotateFailedBody,
        tone: "error",
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={routineDetailPage.emptySelectCompany} />;
  }
  ...
  if (error || !routine) {
    return (
      <p className="pt-6 text-sm text-destructive">
        {error instanceof Error ? error.message : routineDetailPage.notFound}
      </p>
    );
  }
  ...
  const automationLabel = routine.status === "archived"
    ? routineDetailPage.statusArchived
    : !routine.assigneeAgentId
      ? routineDetailPage.statusDraft
      : automationEnabled
        ? routineDetailPage.statusActive
        : routineDetailPage.statusPaused;
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex items-start gap-4">
        <div className="min-w-0 flex-1 space-y-2">
          <textarea
            ...
            placeholder={routineDetailPage.routineTitlePlaceholder}
            rows={1}
          ...
          {routine.managedByPlugin ? (
            <Badge variant="outline" className="gap-1 text-xs text-muted-foreground">
              {routineDetailPage.managedBy(routine.managedByPlugin.pluginDisplayName)}
          ...
          <ToggleSwitch
            ...
            onCheckedChange={() => {
              if (!automationEnabled && !routine.assigneeAgentId) {
                pushToast({
                  title: routineDetailPage.defaultAgentRequiredTitle,
                  body: routineDetailPage.defaultAgentRequiredBody,
                  tone: "warn",
                });
                return;
              }
            ...
            aria-label={automationEnabled ? routineDetailPage.pauseTriggersAria : routineDetailPage.enableTriggersAria}
          />
      ...
      {secretMessage && (
        <div className="rounded-lg border border-blue-500/30 bg-blue-500/5 p-4 space-y-3 text-sm">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{routineDetailPage.secretSaveNow}</p>
          </div>
              ...
              <div key={`${entry.webhookUrl}-${index}`} className="space-y-2">
                {secretMessage.entries.length > 1 && (
                  <p className="text-xs font-medium text-muted-foreground">
                    {routineDetailPage.webhookTriggerOf(index + 1, secretMessage.entries.length)}
                  </p>
                )}
        ...
        <div className="rounded-md border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-1">
              <p className="font-medium text-amber-200">{routineDetailPage.outOfDateTitle}</p>
              <p className="text-xs text-muted-foreground">
                {routineDetailPage.outOfDateBody}
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Button
              ...
              >
                {routineDetailPage.reloadLatest}
              </Button>
            </div>
          </div>
        </div>
      ...
      {!routine.assigneeAgentId ? (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 text-sm text-amber-900 dark:text-amber-200">
          {routineDetailPage.draftAgentBanner}
        </div>
      ) : null}
      ...
      <div className="overflow-x-auto overscroll-x-contain">
        <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
          <span>{routinesShared.forLabel}</span>
          <InlineEntitySelector
            ref={assigneeSelectorRef}
            value={editDraft.assigneeAgentId}
            options={assigneeOptions}
            recentOptionIds={recentAssigneeIds}
            placeholder={routinesShared.assignee}
            noneLabel={routinesShared.noAssignee}
            searchPlaceholder={routinesShared.searchAssignees}
            emptyMessage={routinesShared.noAssigneesFound}
            ...
            renderTriggerValue={(option) =>
              option ? (
                ...
                <span className="text-muted-foreground">{routinesShared.assignee}</span>
              )
            }
            renderOption={(option) => {
          ...
          />
          <span>{routinesShared.inLabel}</span>
          <InlineEntitySelector
            ref={projectSelectorRef}
            value={editDraft.projectId}
            options={projectOptions}
            recentOptionIds={recentProjectIds}
            placeholder={routinesShared.project}
            noneLabel={routinesShared.noProject}
            searchPlaceholder={routinesShared.searchProjects}
            emptyMessage={routinesShared.noProjectsFound}
            onChange={(projectId) => {
            ...
            renderTriggerValue={(option) =>
              option && currentProject ? (
                ...
                <span className="text-muted-foreground">{routinesShared.project}</span>
              )
            }
            renderOption={(option) => {
              if (!option.id) return <span className="truncate">{option.label}</span>;
      ...
      <MarkdownEditor
        ...
        placeholder={routinesShared.addInstructionsPlaceholder}
        bordered={false}
      ...
      <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
        <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
          <span className="text-sm font-medium">{routinesShared.advancedDeliverySettings}</span>
          {advancedOpen ? <ChevronDown className="h-4 w-4 text-muted-foreground" /> : <ChevronRight className="h-4 w-4 text-muted-foreground" />}
        </CollapsibleTrigger>
        <CollapsibleContent className="pt-3">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{routinesShared.concurrency}</p>
              <Select
              ...
              >
                ...
                <SelectContent>
                  {concurrencyPolicies.map((value) => (
                    <SelectItem key={value} value={value}>{routinesShared.concurrencyPolicySelectLabel(value)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <p className="text-xs text-muted-foreground">{routinesShared.concurrencyPolicyDescriptions[editDraft.concurrencyPolicy as keyof typeof routinesShared.concurrencyPolicyDescriptions]}</p>
            </div>
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{routinesShared.catchUp}</p>
              <Select
              ...
              >
                ...
                <SelectContent>
                  {catchUpPolicies.map((value) => (
                    <SelectItem key={value} value={value}>{routinesShared.catchUpPolicySelectLabel(value)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <p className="text-xs text-muted-foreground">{routinesShared.catchUpPolicyDescriptions[editDraft.catchUpPolicy as keyof typeof routinesShared.catchUpPolicyDescriptions]}</p>
            </div>
          </div>
        </CollapsibleContent>
      </Collapsible>
      ...
      <div className="flex items-center justify-between">
        {isEditDirty ? (
          <span className="text-xs text-amber-600">{routineDetailPage.unsavedChanges}</span>
        ) : (
          <span />
        )}
        <Button
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-3">
        <TabsList variant="line" className="w-full justify-start gap-1">
          <TabsTrigger value="triggers" className="gap-1.5">
            ...
            {routineDetailPage.tabTriggers}
          </TabsTrigger>
          <TabsTrigger value="runs" className="gap-1.5">
            <Play className="h-3.5 w-3.5" />
            {routineDetailPage.tabRuns}
...
<TabsTrigger value="activity" className="gap-1.5">
        ...
        <TabsContent value="triggers" className="space-y-4">
          ...
          <div className="rounded-lg border border-border p-4 space-y-3">
            <p className="text-sm font-medium">{routineDetailPage.addTrigger}</p>
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{routineDetailPage.kind}</Label>
                  ...
                  <SelectContent>
                    {triggerKinds.map((kind) => (
                      <SelectItem key={kind} value={kind} disabled={kind === "webhook"}>
                        {kind}{kind === "webhook" ? routineDetailPage.webhookComingSoon : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {newTrigger.kind === "webhook" && (
                <>
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-muted-foreground">{routinesShared.signingModeDescriptions[newTrigger.signingMode as keyof typeof routinesShared.signingModeDescriptions]}</p>
                  </div>
            ...
            <div className="flex items-center justify-end">
              <Button size="sm" onClick={() => createTrigger.mutate()} disabled={createTrigger.isPending}>
                {createTrigger.isPending ? routineDetailPage.addingTrigger : routineDetailPage.addTriggerButton}
              </Button>
            </div>
          </div>
          ...
          {routine.triggers.length === 0 ? (
            <p className="text-xs text-muted-foreground">{routineDetailPage.noTriggers}</p>
          ) : (
            <div className="space-y-3">
        ...
        <TabsContent value="runs" className="space-y-4">
          ...
          {(routineRuns ?? []).length === 0 ? (
            <p className="text-xs text-muted-foreground">{routineDetailPage.noRuns}</p>
          ) : (
        ...
        <TabsContent value="activity">
          {(activity ?? []).length === 0 ? (
            <p className="text-xs text-muted-foreground">{routineDetailPage.noActivity}</p>
          ) : (
            <div className="border border-border rounded-lg divide-y divide-border">
              {(activity ?? []).map((event) => (
              ...
              if (response.secretMaterials.length > 0) {
                setSecretMessage({
                  title: response.secretMaterials.length === 1
                    ? routineDetailPage.webhookRestoredSingle
                    : routineDetailPage.webhookRestoredMany(response.secretMaterials.length),

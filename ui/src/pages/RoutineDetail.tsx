...
function useConcurrencyPolicyDescriptions(t: (key: string) => string): Record<string, string> {
  return {
    coalesce_if_active: t("concurrencyPolicyDescriptions.coalesce_if_active"),
    always_enqueue: t("concurrencyPolicyDescriptions.always_enqueue"),
    skip_if_active: t("concurrencyPolicyDescriptions.skip_if_active"),
  };
}
function useCatchUpPolicyDescriptions(t: (key: string) => string): Record<string, string> {
  return {
    skip_missed: t("catchUpPolicyDescriptions.skip_missed"),
    enqueue_missed_with_cap: t("catchUpPolicyDescriptions.enqueue_missed_with_cap"),
  };
}
function useSigningModeDescriptions(t: (key: string) => string): Record<string, string> {
  return {
    bearer: t("signingModeDescriptions.bearer"),
    hmac_sha256: t("signingModeDescriptions.hmac_sha256"),
    github_hmac: t("signingModeDescriptions.github_hmac"),
    none: t("signingModeDescriptions.none"),
  };
}
    ...
    <div className="rounded-lg border border-border p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          {trigger.kind === "schedule" && trigger.nextRunAt
            ? t("detail.nextRun", { time: new Date(trigger.nextRunAt).toLocaleString() })
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div className="space-y-1.5">
          <Label className="text-xs">{t("detail.label")}</Label>
          <Input
            value={draft.label}
        ...
        {trigger.kind === "schedule" && (
          <div className="md:col-span-2 space-y-1.5">
            <Label className="text-xs">{t("detail.schedule")}</Label>
            <ScheduleEditor
        ...
        {trigger.kind === "webhook" && (
          <>
            <div className="space-y-1.5">
              <Label className="text-xs">{t("detail.signingMode")}</Label>
              <Select
            ...
            {!SIGNING_MODES_WITHOUT_REPLAY_WINDOW.has(draft.signingMode) && (
              <div className="space-y-1.5">
                <Label className="text-xs">{t("detail.replayWindow")}</Label>
                <Input
      ...
      <div className="flex flex-wrap items-center gap-2">
        {trigger.lastResult && <span className="text-xs text-muted-foreground">{t("detail.lastResult", { result: trigger.lastResult })}</span>}
...
export function RoutineDetail() {
  ...
  const dirtyFields = useMemo<RoutineHistoryDirtyFieldDescriptor[]>(() => {
    ...
    if (editDraft.title !== routineDefaults.title) result.push({ key: "title", label: t("detail.dirtyFields.title") });
    if (editDraft.description !== routineDefaults.description) {
      result.push({ key: "description", label: t("detail.dirtyFields.description") });
    }
    if (editDraft.projectId !== routineDefaults.projectId) {
      result.push({ key: "projectId", label: t("detail.dirtyFields.project") });
    }
    if (editDraft.assigneeAgentId !== routineDefaults.assigneeAgentId) {
      result.push({ key: "assigneeAgentId", label: t("detail.dirtyFields.defaultAgent") });
    }
    if (editDraft.priority !== routineDefaults.priority) {
      result.push({ key: "priority", label: t("detail.dirtyFields.priority") });
    }
    if (editDraft.concurrencyPolicy !== routineDefaults.concurrencyPolicy) {
      result.push({ key: "concurrencyPolicy", label: t("detail.dirtyFields.concurrencyPolicy") });
    }
    if (editDraft.catchUpPolicy !== routineDefaults.catchUpPolicy) {
      result.push({ key: "catchUpPolicy", label: t("detail.dirtyFields.catchUpPolicy") });
    }
    if (JSON.stringify(editDraft.variables) !== JSON.stringify(routineDefaults.variables)) {
      result.push({ key: "variables", label: t("detail.dirtyFields.variables") });
    }
    return result;
  ...
  useEffect(() => {
    if (!routine) return;
    setBreadcrumbs([{ label: t("detail.breadcrumb"), href: "/routines" }, { label: routine.title }]);
    if (!routineDefaults) return;
  ...
  const copySecretValue = async (label: string, value: string) => {
    try {
      ...
      pushToast({ title: t("detail.copied", { label }), tone: "success" });
    } catch (error) {
      pushToast({
        title: t("detail.failedToCopy", { label: label.toLowerCase() }),
        body: error instanceof Error ? error.message : t("detail.clipboardDenied"),
        tone: "error",
      });
    }
  };
  ...
  const saveRoutine = useMutation({
    ...
    onError: (error) => {
      if (error instanceof ApiError && error.status === 409) {
        ...
        pushToast({
          title: t("detail.routineChanged"),
          body: t("detail.routineChangedDescription"),
          tone: "warn",
        });
        return;
      }
      pushToast({
        title: t("detail.failedToSaveRoutine"),
        body: error instanceof Error ? error.message : t("detail.failedToSaveRoutine"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: t("detail.routineRunStarted"), tone: "success" });
    ...
    onError: (error) => {
      pushToast({
        title: t("detail.routineRunFailed"),
        body: error instanceof Error ? error.message : t("errors.runFailed"),
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onSuccess: async (_data, status) => {
      pushToast({
        title: t("detail.routineSaved"),
        body: status === "paused" ? t("detail.automationPaused") : t("detail.automationEnabled"),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        title: t("detail.failedToUpdateRoutine"),
        body: error instanceof Error ? error.message : t("errors.updateFailed"),
        tone: "error",
      });
    },
  });
  ...
  const createTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      if (result.secretMaterial) {
        setSecretMessage({
          title: t("detail.webhookTriggerCreated"),
          entries: [{
        ...
        pushToast({
          title: t("detail.triggerAdded"),
          body: t("detail.triggerAddedDescription"),
          tone: "success",
        });
      }
      await Promise.all([
    ...
    onError: (error) => {
      pushToast({
        title: t("detail.failedToAddTrigger"),
        body: error instanceof Error ? error.message : t("detail.failedToAddTrigger"),
        tone: "error",
      });
    },
  });
  ...
  const updateTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t("detail.triggerSaved"),
        body: t("detail.triggerSavedDescription"),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        title: t("detail.failedToUpdateTrigger"),
        body: error instanceof Error ? error.message : t("detail.failedToUpdateTrigger"),
        tone: "error",
      });
    },
  });
  ...
  const deleteTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("detail.failedToDeleteTrigger"),
        body: error instanceof Error ? error.message : t("detail.failedToDeleteTrigger"),
        tone: "error",
      });
    },
  });
  ...
  const rotateTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      setSecretMessage({
        title: t("detail.webhookSecretRotated"),
        entries: [{
          webhookUrl: result.secretMaterial.webhookUrl,
          webhookSecret: result.secretMaterial.webhookSecret,
        }],
    ...
    onError: (error) => {
      pushToast({
        title: t("detail.failedToRotateWebhookSecret"),
        body: error instanceof Error ? error.message : t("detail.failedToRotateWebhookSecret"),
        tone: "error",
      });
    },
  });
  ...
  const signingModeDescriptions = useSigningModeDescriptions((key) => t(key));
  ...
  if (error || !routine) {
    return (
      <p className="pt-6 text-sm text-destructive">
        {error instanceof Error ? error.message : t("detail.routineNotFound")}
      </p>
    );
  }
  ...
  const automationLabel = routine.status === "archived"
    ? t("detail.status.archived")
    : !routine.assigneeAgentId
      ? t("detail.status.draft")
      : automationEnabled
    ...
    <div className="max-w-2xl space-y-6">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          ...
          {routine.managedByPlugin ? (
            <Badge variant="outline" className="gap-1 text-xs text-muted-foreground">
              {t("detail.managedBy")} {routine.managedByPlugin.pluginDisplayName}
          ...
          <ToggleSwitch
            ...
            onCheckedChange={() => {
              if (!automationEnabled && !routine.assigneeAgentId) {
                pushToast({
                  title: t("detail.defaultAgentRequiredTitle"),
                  body: t("detail.defaultAgentRequiredBody"),
                  tone: "warn",
                });
                return;
              }
            ...
            aria-label={automationEnabled ? t("detail.pauseAutomaticTriggers") : t("detail.enableAutomaticTriggers")}
          />
      ...
      {secretMessage && (
        <div className="rounded-lg border border-blue-500/30 bg-blue-500/5 p-4 space-y-3 text-sm">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{t("detail.saveSecretNow")}</p>
          </div>
              ...
              <div key={`${entry.webhookUrl}-${index}`} className="space-y-2">
                {secretMessage.entries.length > 1 && (
                  <p className="text-xs font-medium text-muted-foreground">
                    {t("detail.webhookTriggerCount", { current: index + 1, total: secretMessage.entries.length })}
                  </p>
                )}
                <div className="flex items-center gap-2">
                  <Input value={entry.webhookUrl} readOnly className="flex-1" />
                  <Button variant="outline" size="sm" onClick={() => copySecretValue(t("detail.webhookUrl"), entry.webhookUrl)}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="outline" size="sm" onClick={() => copySecretValue(t("detail.webhookSecret"), entry.webhookSecret)}>
                    <Copy className="h-3.5 w-3.5 mr-1" />
                    {t("detail.secret")}
                  </Button>
                </div>
        ...
        <div className="rounded-md border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-1">
              <p className="font-medium text-amber-200">{t("detail.outOfDate")}</p>
              <p className="text-xs text-muted-foreground">
                {t("detail.outOfDateDescription")}
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Button
      ...
      <div className="overflow-x-auto overscroll-x-contain">
        <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
          <span>{t("detail.for")}</span>
          <InlineEntitySelector
            ref={assigneeSelectorRef}
            value={editDraft.assigneeAgentId}
            options={assigneeOptions}
            recentOptionIds={recentAssigneeIds}
            placeholder={t("detail.assignee")}
            noneLabel={t("detail.noAssignee")}
            searchPlaceholder={t("detail.searchAssignees")}
            emptyMessage={t("detail.noAssigneesFound")}
          ...
          />
          <span>{t("detail.in")}</span>
          <InlineEntitySelector
            ref={projectSelectorRef}
            value={editDraft.projectId}
            options={projectOptions}
            recentOptionIds={recentProjectIds}
            placeholder={t("detail.project")}
            noneLabel={t("detail.noProject")}
            searchPlaceholder={t("detail.searchProjects")}
            emptyMessage={t("detail.noProjectsFound")}
            onChange={(projectId) => {
      ...
      <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
        <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
          <span className="text-sm font-medium">{t("detail.advancedDeliverySettings")}</span>
          {advancedOpen ? <ChevronDown className="h-4 w-4 text-muted-foreground" /> : <ChevronRight className="h-4 w-4 text-muted-foreground" />}
        </CollapsibleTrigger>
        <CollapsibleContent className="pt-3">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("detail.concurrency")}</p>
              <Select
            ...
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("detail.catchUp")}</p>
              <Select
      ...
      <div className="flex items-center justify-between">
        {isEditDirty ? (
          <span className="text-xs text-amber-600">{t("detail.unsavedChanges")}</span>
        ) : (
          <span />
        )}
        <Button
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-3">
        ...
        <TabsContent value="triggers" className="space-y-4">
          ...
          <div className="rounded-lg border border-border p-4 space-y-3">
            <p className="text-sm font-medium">{t("detail.addTrigger")}</p>
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("detail.kind")}</Label>
                  ...
                  <SelectContent>
                    {triggerKinds.map((kind) => (
                      <SelectItem key={kind} value={kind} disabled={kind === "webhook"}>
                        {kind}{kind === "webhook" ? t("detail.comingSoon") : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
            ...
            <div className="flex items-center justify-end">
              <Button size="sm" onClick={() => createTrigger.mutate()} disabled={createTrigger.isPending}>
                {createTrigger.isPending ? t("detail.adding") : t("detail.addTrigger")}
              </Button>
            </div>
          </div>
          ...
          {routine.triggers.length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("detail.noTriggersConfigured")}</p>
          ) : (
            <div className="space-y-3">
        ...
        <TabsContent value="runs" className="space-y-4">
          ...
          {(routineRuns ?? []).length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("detail.noRunsYet")}</p>
          ) : (
        ...
        <TabsContent value="activity">
          {(activity ?? []).length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("detail.noActivityYet")}</p>
          ) : (
            <div className="border border-border rounded-lg divide-y divide-border">
              {(activity ?? []).map((event) => (
              ...
              if (response.secretMaterials.length > 0) {
                setSecretMessage({
                  title: response.secretMaterials.length === 1
                    ? t("detail.webhookTriggerRestored")
                    : t("detail.webhookTriggersRestored", { count: response.secretMaterials.length }),

...
export function Workspaces() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("pages.workspaces.breadcrumb", { defaultValue: "Workspaces" }) }]);
  }, [setBreadcrumbs, t]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold">{t("pages.workspaces.heading", { defaultValue: "Workspaces" })}</h2>
      </div>

      {groups.length === 0 ? (
        <p className="text-sm text-muted-foreground">{t("pages.workspaces.emptyState", { defaultValue: "No workspace activity yet." })}</p>
      ) : (

...
export function InstanceGeneralSettings() {
  ...
  const signOutMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceGeneral.errors.signOut"));
    },
  });

  useEffect(() => {
    setBreadcrumbs([
      { label: t("instanceGeneral.tabs.settings"), href: "/company/settings" },
      { label: t("instanceGeneral.tabs.instanceSettings") },
      { label: t("instanceGeneral.tabs.general") },
    ]);
  }, [setBreadcrumbs]);
  ...
  const updateGeneralMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceGeneral.errors.updateSettings"));
    },
  });

  if (generalQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("instanceGeneral.text.loading")}</div>;
  }
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instanceGeneral.text.general")}</h1>
        </div>
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <h2 className="text-sm font-semibold">{t("instanceGeneral.text.deploymentAuth")}</h2>
            <ModeBadge
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <StatusBox
              label={t("instanceGeneral.labelsJsx.authReadiness")}
              value={healthQuery.data?.authReady ? "Ready" : "Not ready"}
            />
            <StatusBox
              label={t("instanceGeneral.labelsJsx.bootstrapStatus")}
              value={healthQuery.data?.bootstrapStatus === "bootstrap_pending" ? "Setup required" : "Ready"}
            />
            <StatusBox
              label={t("instanceGeneral.labelsJsx.bootstrapInvite")}
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("instanceGeneral.text.censorUsername")}</h2>
          ...
          <ToggleSwitch
            ...
            aria-label={t("instanceGeneral.labelsJsx.toggleUsernameLogCensoring")}
          />
        </div>
      </section>
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("instanceGeneral.text.keyboardShortcuts")}</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
          ...
          <ToggleSwitch
            ...
            disabled={updateGeneralMutation.isPending}
            aria-label={t("instanceGeneral.labelsJsx.toggleKeyboardShortcuts")}
          />
        </div>
      </section>
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="space-y-5">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("instanceGeneral.text.backupRetention")}</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
          ...
          <div className="space-y-1.5">
            <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t("instanceGeneral.text.daily")}</h3>
          ...
          <div className="space-y-1.5">
            <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t("instanceGeneral.text.weekly")}</h3>
            <div className="flex flex-wrap gap-2">
          ...
          <div className="space-y-1.5">
            <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t("instanceGeneral.text.monthly")}</h3>
            <div className="flex flex-wrap gap-2">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              {
                value: "allowed",
                label: t("instanceGeneral.options.alwaysAllow"),
                description: t("instanceGeneral.descriptions.shareVotedOutputs"),
              },
              {
                value: "not_allowed",
                label: t("instanceGeneral.options.dontAllow"),
                description: t("instanceGeneral.descriptions.keepVotedLocal"),
              },
            ].map((option) => {
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("instanceGeneral.text.signOut")}</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">

...
export function JoinRequestQueue() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("joinRequestQueue.breadcrumb.company"), href: "/dashboard" },
      { label: t("joinRequestQueue.breadcrumb.inbox"), href: "/inbox" },
      { label: t("joinRequestQueue.breadcrumb.joinRequests") },
    ]);
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("joinRequestQueue.toast.approved"), tone: "success" });
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: queryKeys.access.joinRequests(selectedCompanyId!, `${status}:${requestType}`) });
      pushToast({ title: t("joinRequestQueue.toast.rejected"), tone: "success" });
    },
  });
  ...
  if (requestsQuery.error) {
    const message =
      requestsQuery.error instanceof ApiError && requestsQuery.error.status === 403
        ? t("joinRequestQueue.permissionError")
        : requestsQuery.error instanceof Error
          ? requestsQuery.error.message
          : t("joinRequestQueue.loadError");
    ...
    <div className="max-w-6xl space-y-6">
      ...
      <div className="flex flex-wrap gap-3 rounded-xl border border-border bg-card p-4">
        <label className="space-y-2 text-sm">
          ...
          >
            <option value="pending_approval">{t("joinRequestQueue.statusOptions.pending_approval")}</option>
            <option value="approved">{t("joinRequestQueue.statusOptions.approved")}</option>
            <option value="rejected">{t("joinRequestQueue.statusOptions.rejected")}</option>
          </select>
        </label>
        <label className="space-y-2 text-sm">
          ...
          >
            <option value="all">{t("joinRequestQueue.typeOptions.all")}</option>
            <option value="human">{t("joinRequestQueue.typeOptions.human")}</option>
            <option value="agent">{t("joinRequestQueue.typeOptions.agent")}</option>
          </select>
        </label>
      </div>
            ...
            <div key={request.id} className="rounded-xl border border-border bg-card p-4">
                ...
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge variant={request.status === "pending_approval" ? "secondary" : request.status === "approved" ? "outline" : "destructive"}>
                      {t(`joinRequestQueue.statusOptions.${request.status}`)}
                    </Badge>
                    <Badge variant="outline">{t(`joinRequestQueue.typeOptions.${request.requestType}`)}</Badge>
                    {request.adapterType ? <Badge variant="outline">{request.adapterType}</Badge> : null}
                  </div>
                  <div>
                    <div className="text-base font-medium">
                      {request.requestType === "human"
                        ? request.requesterUser?.name || request.requestEmailSnapshot || request.requestingUserId || t("joinRequestQueue.unknownHuman")
                        : request.agentName || t("joinRequestQueue.unknownAgent")}
                    </div>
              ...
              <div className="mt-4 grid gap-3 text-sm text-muted-foreground md:grid-cols-2">
                ...
                <div className="rounded-lg border border-border bg-background px-3 py-2">
                  ...
                  <div className="mt-2">{t("joinRequestQueue.submitted", { date: new Date(request.createdAt).toLocaleString(i18n.language) })}</div>
                  <div>{t("joinRequestQueue.sourceIp", { ip: request.requestIp })}</div>

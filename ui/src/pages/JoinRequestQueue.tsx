...
export function JoinRequestQueue() {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("pages.joinRequestQueue.approvedToast", { defaultValue: "Join request approved" }), tone: "success" });
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: queryKeys.access.joinRequests(selectedCompanyId!, `${status}:${requestType}`) });
      pushToast({ title: t("pages.joinRequestQueue.rejectedToast", { defaultValue: "Join request rejected" }), tone: "success" });
    },
  });
  ...
  if (requestsQuery.error) {
    const message =
      requestsQuery.error instanceof ApiError && requestsQuery.error.status === 403
        ? t("pages.joinRequestQueue.noPermission", { defaultValue: "You do not have permission to review join requests for this company." })
        : requestsQuery.error instanceof Error
          ? requestsQuery.error.message
          : t("pages.joinRequestQueue.failedToLoad", { defaultValue: "Failed to load join requests." });
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("pages.joinRequestQueue.title", { defaultValue: "Join Request Queue" })}</h1>
        </div>
      ...
      <Card className="flex-row flex-wrap gap-3 p-4">
        <label className="space-y-2 text-sm">
          <span className="font-medium">{t("pages.joinRequestQueue.statusFilterLabel", { defaultValue: "Status" })}</span>
          <select
          ...
          >
            <option value="pending_approval">{t("pages.joinRequestQueue.pendingApproval", { defaultValue: "Pending approval" })}</option>
            <option value="approved">{t("pages.joinRequestQueue.approved", { defaultValue: "Approved" })}</option>
            <option value="rejected">{t("pages.joinRequestQueue.rejected", { defaultValue: "Rejected" })}</option>
          </select>
        </label>
        <label className="space-y-2 text-sm">
          ...
          >
            <option value="all">{t("pages.joinRequestQueue.all", { defaultValue: "All" })}</option>
            <option value="human">{t("pages.joinRequestQueue.human", { defaultValue: "Human" })}</option>
            <option value="agent">{t("pages.joinRequestQueue.agent", { defaultValue: "Agent" })}</option>
          </select>
        </label>
      </Card>
            ...
            <Card key={request.id} className="block p-4">
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                {request.status === "pending_approval" ? (
                  <div className="flex gap-2">
                    ...
                    >
                      {t("pages.joinRequestQueue.reject", { defaultValue: "Reject" })}
                    </Button>
                    <Button
                      onClick={() => approveMutation.mutate(request.id)}
                      disabled={approveMutation.isPending}
                    >
                      {t("pages.joinRequestQueue.approve", { defaultValue: "Approve" })}
                    </Button>
                  </div>
                ) : null}
              </div>
              ...
              <div className="mt-4 grid gap-3 text-sm text-muted-foreground md:grid-cols-2">
                <div className="rounded-lg border border-border bg-background px-3 py-2">
                  <div className="text-xs font-medium uppercase tracking-wide">{t("pages.joinRequestQueue.inviteContext", { defaultValue: "Invite context" })}</div>
                  <div className="mt-2">
                    {request.invite
                      ? request.invite.humanRole
                        ? t("pages.joinRequestQueue.inviteSummaryWithRole", {
                            defaultValue: "{{allowedJoinTypes}} join invite • default role {{humanRole}}",
                            allowedJoinTypes: request.invite.allowedJoinTypes,
                            humanRole: request.invite.humanRole,
                          })
                        : t("pages.joinRequestQueue.inviteSummary", {
                            defaultValue: "{{allowedJoinTypes}} join invite",
                            allowedJoinTypes: request.invite.allowedJoinTypes,
                          })
                ...
                <div className="rounded-lg border border-border bg-background px-3 py-2">
                  ...
                  <div className="mt-2">{t("pages.joinRequestQueue.submitted", { defaultValue: "Submitted" })} {new Date(request.createdAt).toLocaleString()}</div>
                  <div>{t("pages.joinRequestQueue.sourceIp", { defaultValue: "Source IP" })} {request.requestIp}</div>

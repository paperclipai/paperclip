...
export function JoinRequestQueue() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("joinRequestQueue.tabs.inbox"), href: "/inbox" },
      { label: t("joinRequestQueue.tabs.joinRequests") },
    ]);
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("joinRequestQueue.toasts.approved"), tone: "success" });
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: queryKeys.access.joinRequests(selectedCompanyId!, `${status}:${requestType}`) });
      pushToast({ title: t("joinRequestQueue.toasts.rejected"), tone: "success" });
    },
  });

  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("joinRequestQueue.text.selectCompany")}</div>;
  }
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("joinRequestQueue.text.queue")}</h1>
        </div>
      ...
      <div className="flex flex-wrap gap-3 rounded-xl border border-border bg-card p-4">
        <label className="space-y-2 text-sm">
          <span className="font-medium">{t("joinRequestQueue.text.status")}</span>
          <select
          ...
          >
            <option value="pending_approval">{t("joinRequestQueue.text.pending")}</option>
            <option value="approved">{t("joinRequestQueue.text.approved")}</option>
            <option value="rejected">{t("joinRequestQueue.text.rejected")}</option>
          </select>
        </label>
        <label className="space-y-2 text-sm">
          <span className="font-medium">{t("joinRequestQueue.text.requestType")}</span>
          <select
            className="rounded-md border border-border bg-background px-3 py-2"
            value={requestType}
            onChange={(event) =>
          ...
          >
            ...
            <option value="human">{t("joinRequestQueue.text.human")}</option>
            <option value="agent">{t("joinRequestQueue.text.agent")}</option>
          </select>
        </label>
      </div>
            ...
            <div key={request.id} className="rounded-xl border border-border bg-card p-4">
              ...
              <div className="mt-4 grid gap-3 text-sm text-muted-foreground md:grid-cols-2">
                <div className="rounded-lg border border-border bg-background px-3 py-2">
                  <div className="text-xs font-medium uppercase tracking-wide">{t("joinRequestQueue.text.inviteContext")}</div>
                  <div className="mt-2">
                    {request.invite
                ...
                <div className="rounded-lg border border-border bg-background px-3 py-2">
                  <div className="text-xs font-medium uppercase tracking-wide">{t("joinRequestQueue.text.requestDetails")}</div>

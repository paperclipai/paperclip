...
export function JoinRequestQueue() {
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("join-request-queue.select-an-organization-to-review-joi-13z")}</div>;
  }

  if (requestsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("join-request-queue.loading-join-requests-1un")}</div>;
  }
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("join-request-queue.join-request-queue-1vv")}</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("join-request-queue.review-human-and-agent-join-requests-450")}
        </p>
      </div>

      <Card className="flex-row flex-wrap gap-3 p-4">
        <label className="space-y-2 text-sm">
          <span className="font-medium">{t("join-request-queue.status-3pd")}</span>
          <select
          ...
          >
            <option value="pending_approval">{t("join-request-queue.pending-approval-1ku")}</option>
            <option value="approved">{t("join-request-queue.approved-1j3")}</option>
            <option value="rejected">{t("join-request-queue.rejected-1uo")}</option>
          </select>
        </label>
        <label className="space-y-2 text-sm">
          <span className="font-medium">{t("join-request-queue.request-type-wxd")}</span>
          <select
            className="rounded-md border border-border bg-background px-3 py-2"
            value={requestType}
            onChange={(event) =>
          ...
          >
            <option value="all">{t("join-request-queue.all-wnj")}</option>
            <option value="human">{t("join-request-queue.human-72u")}</option>
            <option value="agent">{t("join-request-queue.agent-1w5")}</option>
          </select>
        </label>
      </Card>
      ...
      <div className="space-y-4">
        {(requestsQuery.data ?? []).length === 0 ? (
          <div className="rounded-xl border border-dashed border-border px-4 py-8 text-sm text-muted-foreground">
            {t("join-request-queue.no-join-requests-match-the-current-f-160")}
          </div>
        ) : (
            ...
            <Card key={request.id} className="block p-4">
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                {request.status === "pending_approval" ? (
                  <div className="flex gap-2">
                    ...
                    >
                      {t("join-request-queue.reject-1ke")}
                    </Button>
                    <Button
                      onClick={() => approveMutation.mutate(request.id)}
                      disabled={approveMutation.isPending}
                    >
                      {t("join-request-queue.approve-1s2")}
                    </Button>
                  </div>
                ) : null}
              </div>
              ...
              <div className="mt-4 grid gap-3 text-sm text-muted-foreground md:grid-cols-2">
                <div className="rounded-lg border border-border bg-background px-3 py-2">
                  <div className="text-xs font-medium uppercase tracking-wide">{t("join-request-queue.invite-context-1yf")}</div>
                  <div className="mt-2">
                    {request.invite
                ...
                <div className="rounded-lg border border-border bg-background px-3 py-2">
                  <div className="text-xs font-medium uppercase tracking-wide">{t("join-request-queue.request-details-15l")}</div>
                  <div className="mt-2">{t("join-request-queue.submitted-12a")} {new Date(request.createdAt).toLocaleString()}</div>
                  <div>{t("join-request-queue.source-ip-1lv")} {request.requestIp}</div>

...
export function Agents() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("sidebar.agents", { defaultValue: "Agents" }) }]);
  }, [setBreadcrumbs, t]);
    ...
    <div className="space-y-4">
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">
          {t("agents.countAgents", { defaultValue: "{{count}} agents", count: filtered.length })}
        </p>
      )}
      ...
      {effectiveView === "list" && agents && agents.length > 0 && filtered.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-8">
          {t("agents.noFilterMatch", { defaultValue: "No agents match the selected filter." })}
        </p>
      )}
      ...
      {effectiveView === "org" && orgTree && orgTree.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-8">
          {t("agents.noOrg", { defaultValue: "No organizational hierarchy defined." })}
        </p>
      )}
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400">
        {liveCount > 1
          ? t("agents.liveWithCount", { defaultValue: "Live ({{count}})", count: liveCount })

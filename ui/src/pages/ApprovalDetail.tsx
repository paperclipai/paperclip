...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? t("approvals:breadcrumb") },
    ]);
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals:detail.approveFailed")),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals:detail.rejectFailed")),
  });
  ...
  if (!approval) return <p className="text-sm text-muted-foreground">{t("approvals:detail.approvalNotFound")}</p>;
      ...
      ? {
          label:
            (linkedIssues?.length ?? 0) > 1
              ? t("approvals:detail.reviewLinkedIssues")
              : t("approvals:detail.reviewLinkedIssue"),
  ...
  return (
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">{t("approvals:detail.approvalConfirmed")}</p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("approvals:detail.agentNotified")}
                </p>
              </div>
            </div>
            <Button
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          ...
          {approval.decisionNote && (
            <p className="text-xs text-muted-foreground">{t("approvals:detail.decisionNote", { note: approval.decisionNote })}</p>
          )}
        </div>
        {error && <p className="text-sm text-destructive">{error}</p>}
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            <p className="text-xs text-muted-foreground mb-1.5">{t("approvals:detail.linkedIssues")}</p>
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              {t("approvals:detail.linkedIssuesNote")}
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("approvals:detail.budgetResolveNote").split("<1>").map((part, i) => {
                if (i === 0) return part;
                const [linkText, rest] = part.split("</1>");
                return <span key={i}><Link to="/costs" className="underline underline-offset-2">{linkText}</Link>{rest}</span>;
              })}
            </p>
          )}
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm(t("approvals:detail.deleteConfirm"))) return;
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">{t("approvals:detail.comments", { count: comments?.length ?? 0 })}</h3>
            ...
            <div key={comment.id} className="border border-border/60 rounded-md p-3">
              <div className="flex items-center justify-between mb-1">
                {comment.authorAgentId ? (
                  ...
                  <Identity name={t("approvals:detail.board")} size="sm" />
                )}
        ...
        <Textarea
          ...
          placeholder={t("approvals:detail.addComment")}
          rows={3}
        />

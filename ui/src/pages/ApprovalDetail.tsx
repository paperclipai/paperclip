...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("pages.approvalDetail.breadcrumbApprovals", { defaultValue: "Approvals" }), href: "/approvals" },
      {
        label:
          approval?.id?.slice(0, 8) ??
          approvalId ??
          t("pages.approvalDetail.breadcrumbApproval", { defaultValue: "Approval" }),
      },
    ]);
    ...
    onError: (err) =>
      setError(
        err instanceof Error
          ? err.message
          : t("pages.approvalDetail.approveFailed", { defaultValue: "Approve failed" }),
      ),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) =>
      setError(
        err instanceof Error
          ? err.message
          : t("pages.approvalDetail.rejectFailed", { defaultValue: "Reject failed" }),
      ),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) =>
      setError(
        err instanceof Error
          ? err.message
          : t("pages.approvalDetail.revisionRequestFailed", {
              defaultValue: "Revision request failed",
            }),
      ),
  });
  ...
  const resubmitMutation = useMutation({
    ...
    onError: (err) =>
      setError(
        err instanceof Error
          ? err.message
          : t("pages.approvalDetail.resubmitFailed", { defaultValue: "Resubmit failed" }),
      ),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) =>
      setError(
        err instanceof Error
          ? err.message
          : t("pages.approvalDetail.commentFailed", { defaultValue: "Comment failed" }),
      ),
  });
  ...
  const deleteAgentMutation = useMutation({
    ...
    onError: (err) =>
      setError(
        err instanceof Error
          ? err.message
          : t("pages.approvalDetail.deleteFailed", { defaultValue: "Delete failed" }),
      ),
  });
  ...
  if (!approval)
    return (
      <p className="text-sm text-muted-foreground">
        {t("pages.approvalDetail.notFound", { defaultValue: "Approval not found." })}
      </p>
    );
    ...
    <div className="space-y-6 max-w-3xl">
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">
                  {t("pages.approvalDetail.approvalConfirmed", { defaultValue: "Approval confirmed" })}
                </p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("pages.approvalDetail.approvalConfirmedDescription", {
                    defaultValue:
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          {approval.requestedByAgentId && (
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground text-xs">
                {t("pages.approvalDetail.requestedBy", { defaultValue: "Requested by" })}
              </span>
              <Identity
          ...
          {approval.decisionNote && (
            <p className="text-xs text-muted-foreground">
              {t("pages.approvalDetail.decisionNote", {
                note: approval.decisionNote,
                defaultValue: "Decision note: {{note}}",
              })}
            </p>
          )}
        </div>
        ...
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            <p className="text-xs text-muted-foreground mb-1.5">
              {t("pages.approvalDetail.linkedTasks", { defaultValue: "Linked Tasks" })}
            </p>
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              {t("pages.approvalDetail.linkedTasksHint", {
                defaultValue:
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("pages.approvalDetail.budgetStopPrefix", {
                defaultValue: "Resolve this budget stop from the budget controls on ",
              })}
              <Link to="/costs" className="underline underline-offset-2">/costs</Link>
              {t("pages.approvalDetail.budgetStopSuffix", { defaultValue: "." })}
            </p>
          )}
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (
                  !window.confirm(
                    t("pages.approvalDetail.deleteDisapprovedAgentConfirm", {
                      defaultValue: "Delete this disapproved agent? This cannot be undone.",
                    }),
                  )
                )
                  return;
            ...
            >
              {t("pages.approvalDetail.deleteDisapprovedAgent", {
                defaultValue: "Delete disapproved agent",
              })}
            </Button>
          )}
        </div>
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">
          {t("pages.approvalDetail.commentsHeading", {
            ...
            <div key={comment.id} className="border border-border/60 rounded-md p-3">
              <div className="flex items-center justify-between mb-1">
                {comment.authorAgentId ? (
                  ...
                  <Identity
                    name={t("pages.approvalDetail.boardAuthor", { defaultValue: "Board" })}
                    size="sm"
                  />
                )}
        ...
        <Textarea
          ...
          placeholder={t("pages.approvalDetail.commentPlaceholder", {
        ...
        <div className="flex justify-end">
          ...
          >
            {addCommentMutation.isPending
              ? t("pages.approvalDetail.posting", { defaultValue: "Posting…" })
              : t("pages.approvalDetail.postComment", { defaultValue: "Post comment" })}
          </Button>
        </div>
      </div>
    </div>

...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("approvalDetail.breadcrumbRoot"), href: "/approvals" },
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? t("approvalDetail.breadcrumbFallback") },
    ]);
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvalDetail.errorApprove")),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvalDetail.errorReject")),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvalDetail.errorRevision")),
  });
  ...
  const resubmitMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvalDetail.errorResubmit")),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvalDetail.errorComment")),
  });
  ...
  const deleteAgentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvalDetail.errorDelete")),
  });

  if (isLoading) return <PageSkeleton variant="detail" />;
  if (!approval) return <p className="text-sm text-muted-foreground">{t("approvalDetail.notFound")}</p>;
      ...
      ? {
          label:
            (linkedIssues?.length ?? 0) > 1
              ? t("approvalDetail.ctaReviewLinkedIssues")
              : t("approvalDetail.ctaReviewLinkedIssue"),
          to: `/issues/${primaryLinkedIssue.identifier ?? primaryLinkedIssue.id}`,
        }
      : linkedAgentId
        ? {
            label: t("approvalDetail.ctaOpenHiredAgent"),
            to: `/agents/${linkedAgentId}`,
          }
        : {
            label: t("approvalDetail.ctaBackToApprovals"),
            to: "/approvals",
          };
  ...
  return (
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">{t("approvalDetail.confirmedTitle")}</p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("approvalDetail.confirmedBody")}
                </p>
              </div>
            </div>
            <Button
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          {approval.requestedByAgentId && (
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground text-xs">{t("approvalDetail.requestedBy")}</span>
              <Identity
          ...
          >
            ...
            {t("approvalDetail.seeFullRequest")}
          </button>
          {showRawPayload && (
          ...
          {approval.decisionNote && (
            <p className="text-xs text-muted-foreground">{t("approvalDetail.decisionNote", { note: approval.decisionNote })}</p>
          )}
        </div>
        {error && <p className="text-sm text-destructive">{error}</p>}
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            <p className="text-xs text-muted-foreground mb-1.5">{t("approvalDetail.linkedIssuesHeading")}</p>
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              {t("approvalDetail.linkedIssuesHint")}
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          {isActionable && !isBudgetApproval && (
            <>
              ...
              >
                {t("approvalDetail.actionApprove")}
              </Button>
              <Button
                variant="destructive"
                size="sm"
              ...
              >
                {t("approvalDetail.actionReject")}
              </Button>
            </>
          )}
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("approvalDetail.budgetStopBefore")} <Link to="/costs" className="underline underline-offset-2">/costs</Link>{t("approvalDetail.budgetStopAfter")}
            </p>
          )}
          {approval.status === "pending" && (
            ...
            >
              {t("approvalDetail.actionRequestRevision")}
            </Button>
          )}
          {approval.status === "revision_requested" && (
            ...
            >
              {t("approvalDetail.actionMarkResubmitted")}
            </Button>
          )}
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm(t("approvalDetail.confirmDeleteAgent"))) return;
                deleteAgentMutation.mutate(linkedAgentId);
              }}
              disabled={deleteAgentMutation.isPending}
            >
              {t("approvalDetail.actionDeleteDisapprovedAgent")}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">{t("approvalDetail.commentsHeading", { count: comments?.length ?? 0 })}</h3>
            ...
            <div key={comment.id} className="border border-border/60 rounded-md p-3">
              <div className="flex items-center justify-between mb-1">
                {comment.authorAgentId ? (
                  ...
                  <Identity name={t("approvalDetail.commentAuthorBoard")} size="sm" />
                )}
        ...
        <div className="flex justify-end">
          ...
          >
            {addCommentMutation.isPending ? t("approvalDetail.commentPosting") : t("approvalDetail.commentPost")}
          </Button>
        </div>
      </div>
    </div>

...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("pages.approvals.title", { defaultValue: "Approvals" }), href: "/approvals" },
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? t("pages.approvalDetail.title", { defaultValue: "Approval" }) },
    ]);
  }, [setBreadcrumbs, t, approval, approvalId]);
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("pages.approvalDetail.revisionRequestFailed", { defaultValue: "Revision request failed" })),
  });
  ...
  const resolvedCta =
    primaryLinkedIssue
      ? {
          label: t("pages.approvalDetail.reviewLinkedTasks", {
            defaultValue_one: "Review linked task",
            defaultValue_other: "Review linked tasks",
            count: linkedIssues?.length ?? 0,
          }),
  ...
  return (
    <div className="space-y-6 max-w-3xl">
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("pages.approvalDetail.approvalConfirmedDescription", { defaultValue: "Requesting agent was notified to review this approval and linked tasks." })}
                </p>
              </div>
            </div>
            <Button
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          ...
          {approval.decisionNote && (
            <p className="text-xs text-muted-foreground">{t("pages.approvalDetail.decisionNote", { defaultValue: "Decision note: {{note}}", note: approval.decisionNote })}</p>
          )}
        </div>
        ...
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            ...
            <p className="text-(length:--text-micro) text-muted-foreground mt-2">
              {t("pages.approvalDetail.linkedTasksHint", { defaultValue: "Linked tasks remain open until the requesting agent follows up and closes them." })}
            </p>
          </div>
        )}
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">{t("pages.approvalDetail.commentsCount", { defaultValue: "Comments ({{count}})", count: comments?.length ?? 0 })}</h3>
        ...
        <Textarea
          ...
          placeholder={t("pages.approvalDetail.addCommentPlaceholder", { defaultValue: "Add a comment..." })}
          rows={3}
        />

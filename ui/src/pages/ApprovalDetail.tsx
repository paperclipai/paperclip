...
export function ApprovalDetail() {
  ...
  if (!approval) return <p className="text-sm text-muted-foreground">{t("detail.notFound")}</p>;
      ...
      ? {
          label:
            (linkedIssues?.length ?? 0) > 1
              ? t("detail.reviewLinkedIssues")
              : t("detail.reviewLinkedIssue"),
          to: `/issues/${primaryLinkedIssue.identifier ?? primaryLinkedIssue.id}`,
        }
      : linkedAgentId
        ? {
            label: t("detail.openHiredAgent"),
            to: `/agents/${linkedAgentId}`,
          }
        : {
            label: t("detail.backToApprovals"),
            to: "/approvals",
          };
    ...
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">{t("detail.approvedBannerTitle")}</p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("detail.approvedBannerDescription")}
                </p>
              </div>
            </div>
            <Button
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          {approval.requestedByAgentId && (
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground text-xs">{t("detail.requestedBy")}</span>
              <Identity
          ...
          {approval.decisionNote && (
            <p className="text-xs text-muted-foreground">{t("detail.decisionNote", { note: approval.decisionNote })}</p>
          )}
        </div>
        {error && <p className="text-sm text-destructive">{error}</p>}
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            <p className="text-xs text-muted-foreground mb-1.5">{t("detail.linkedIssues")}</p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("detail.budgetResolve")} <Link to="/costs" className="underline underline-offset-2">/costs</Link>.
            </p>
          )}
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm(t("detail.deleteDisapprovedAgentConfirm"))) return;
                deleteAgentMutation.mutate(linkedAgentId);
              }}
              disabled={deleteAgentMutation.isPending}
            >
              {t("detail.deleteDisapprovedAgent")}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
            ...
            <div key={comment.id} className="border border-border/60 rounded-md p-3">
              <div className="flex items-center justify-between mb-1">
                {comment.authorAgentId ? (
                  ...
                  <Identity name={t("detail.board")} size="sm" />
                )}
        ...
        <div className="flex justify-end">
          ...
          >
            {addCommentMutation.isPending ? t("detail.posting") : t("detail.postComment")}
          </Button>
        </div>
      </div>
    </div>

...
export function ApprovalDetail() {
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "Falha ao solicitar revisão"),
  });
    ...
    <div className="space-y-6 max-w-3xl">
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  O agente solicitante foi avisado para revisar esta aprovação e as tarefas vinculadas.
                </p>
              </div>
            </div>
            <Button
        ...
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              As tarefas vinculadas permanecem abertas até o agente solicitante acompanhar e encerrá-las.
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              Resolva este bloqueio de orçamento nos controles em <Link to="/costs" className="underline underline-offset-2">/costs</Link>.
            </p>
          )}
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="flex justify-end">
          ...
          >
            {addCommentMutation.isPending ? "Enviando…" : "Publicar comentário"}
          </Button>
        </div>
      </div>
    </div>

...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? t("page.approvals.title") },
    ]);
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approval.approveFailed")),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approval.rejectFailed")),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approval.revisionFailed")),
  });
  ...
  const resubmitMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approval.resubmitFailed")),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approval.commentFailed")),
  });
  ...
  const deleteAgentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approval.deleteFailed")),
  });
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("approval.budgetResolve")} <Link to="/costs" className="underline underline-offset-2">/costs</Link>.
            </p>
          )}

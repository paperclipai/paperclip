...
function statusBadgeLabel(status: RemoteSecretImportCandidate["status"]) {
  switch (status) {
    case "duplicate":
      return t("pages.secrets.import.status.imported");
    case "conflict":
      return t("pages.secrets.import.status.conflict");
    case "ready":
    default:
...
function vaultStatusLabel(status: CompanySecretProviderConfig["status"]) {
  switch (status) {
    case "ready":
      return t("pages.secrets.status.ready");
    case "warning":
  ...
  switch (status) {
    case "imported":
      return (
        ...
        >
          <CheckCircle2 className="h-3 w-3" /> {t("pages.secrets.status.created")}
        </Badge>
      );
    case "skipped":
      return (
        ...
        >
          <Link2 className="h-3 w-3" /> {t("pages.secrets.status.skipped")}
        </Badge>
      );
    case "error":
    default:
      return (
        ...
        >
          <XCircle className="h-3 w-3" /> {t("pages.secrets.status.failed")}
        </Badge>
      );
  }
}
...
function validateDraftRow(
  ...
  if (!draft.name.trim()) return t("pages.secrets.import.validation.nameRequired");
  if (draft.name.length > 160) return t("pages.secrets.import.validation.nameTooLong");
  if (!draft.key.trim()) return t("pages.secrets.import.validation.keyRequired");
  if (!KEY_PATTERN.test(draft.key)) {
    return t("pages.secrets.import.validation.keyCharacters");
  }
  if (draft.key.length > 120) return t("pages.secrets.import.validation.keyTooLong");
  if (draft.description.length > 500) {
    return t("pages.secrets.import.validation.descriptionTooLong");
  }
  ...
  for (const existingSecret of existing) {
    if (existingSecret.name.trim().toLowerCase() === lowerName) {
      return t("pages.secrets.import.validation.nameInUse");
    }
    if (existingSecret.key.trim().toLowerCase() === lowerKey) {
      return t("pages.secrets.import.validation.keyInUse");
    }
  }
  ...
  for (const other of otherDrafts) {
    ...
    if (other.name.trim().toLowerCase() === lowerName) {
      return t("pages.secrets.import.validation.batchNameInUse");
    }
    if (other.key.trim().toLowerCase() === lowerKey) {
      return t("pages.secrets.import.validation.batchKeyInUse");
    }
  }
...
export function ImportFromVaultDialog({
  ...
  const importMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      if (result.errorCount === draftList.length && result.errorCount > 0) {
        toast.pushToast({
          title: t("pages.secrets.import.toasts.failed"),
          body: t("pages.secrets.import.toasts.noneImported", { vault: vaultName }),
          tone: "error",
        });
      } else {
        toast.pushToast({
          title:
            result.errorCount > 0
              ? t("pages.secrets.import.toasts.completedWithErrors")
              : t("pages.secrets.import.toasts.complete"),
          body: t("pages.secrets.import.resultSummary", {
  ...
  function handleLoadMore() {
    ...
    secretsApi
      ...
      .catch((error) => {
        toast.pushToast({
          title: t("pages.secrets.import.toasts.loadMoreFailed"),
          body: readableErrorMessage(error),
          tone: "error",
        });
      })
  ...
  function handleClose(force = false) {
    ...
    if (!force && step !== "result" && selection.size > 0 && !importResult) {
      const ok = window.confirm(t("pages.secrets.import.discardPending", { count: selection.size }));
      if (!ok) return;
    }
    onOpenChange(false);
  }
        ...
        <header className="flex items-start justify-between gap-3 border-b border-border/60 px-5 py-4">
          <div className="flex flex-col gap-1">
            <DialogTitle className="text-base font-semibold">
              {t("pages.secrets.import.title")}
            </DialogTitle>
            <DialogDescription className="text-xs text-muted-foreground">
              {t("pages.secrets.import.description")}
            </DialogDescription>
            <Stepper step={step} />
          </div>
          <button
        ...
        <footer className="flex items-center justify-between gap-3 border-t border-border/60 bg-muted/20 px-5 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {step === "select" && (
              ...
              >
                {t("pages.secrets.import.continueReview")}
              </Button>
            )}
            {step === "review" && (
              <Button
              ...
              >
                {importMutation.isPending ? (
                  ...
                  t("pages.secrets.import.importCount", { count: draftList.length })
                )}
              </Button>
            )}
            {step === "result" && (
              <Button size="sm" onClick={() => handleClose(true)}>
                {t("pages.secrets.actions.done")}
              </Button>
            )}
          </div>
        </footer>
...
function Stepper({ step }: { step: Step }) {
  const steps: { id: Step; label: string }[] = [
    ...
    { id: "result", label: t("pages.secrets.import.steps.result") },
  ];
  ...
  if (noEligibleVaults) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center p-6" data-testid="select-empty-vaults">
        <EmptyState
          icon={Cloud}
          message={t("pages.secrets.import.noVault")}
          action={onManageVaults ? t("pages.secrets.import.manageVaults") : undefined}
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-2 border-b border-border/60 px-5 py-3">
        ...
        {awsVaults.length === 1 && eligible.length === 1 ? (
          ...
          >
            <SelectTrigger
              size="sm"
              className="text-xs"
              aria-label={t("pages.secrets.import.selectVaultAria")}
            >
              <SelectValue placeholder={t("pages.secrets.import.selectVaultPlaceholder")} />
            </SelectTrigger>
            <SelectContent>
              {awsVaults.map((vault) => {
                  ...
                  >
                    <span className="flex items-center gap-2">
                      ...
                      {vault.status === "warning" && (
                        <Badge variant="outline" className="px-1 py-0 text-(length:--text-nano) text-amber-500 border-amber-500/40">
        ...
        <div className="relative ml-auto w-64">
          ...
          <Input
            ...
            placeholder={t("pages.secrets.import.searchPlaceholder")}
            className="pl-7 pr-7 text-xs"
            aria-label={t("pages.secrets.import.searchAria")}
        ...
        <Button
          ...
          aria-label={t("pages.secrets.import.refreshAria")}
        >
      ...
      {selectedNotVisible > 0 && (
        <div className="flex items-center justify-between border-b border-border/60 bg-muted/20 px-5 py-1.5 text-xs text-muted-foreground">
          <span>
            {t("pages.secrets.import.selectedNotVisible", {
          ...
          >
            {showOnlySelected
              ? t("pages.secrets.actions.showAll")
              : t("pages.secrets.actions.showSelected")}
          </Button>
        </div>
      )}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto" data-testid="vault-table-scroll">
              ...
              <tr>
                <th className="px-3 py-2 text-left">
                  <Checkbox
                    ...
                    aria-label={t("pages.secrets.import.selectAllLoadedAria", {
                ...
                <th className="px-2 py-2 text-left font-medium">
                  {t("pages.secrets.import.columns.remoteName")}
                </th>
                <th className="px-2 py-2 text-left font-medium">
                  {t("pages.secrets.import.columns.reference")}
                </th>
                <th className="px-2 py-2 text-left font-medium">
                  {t("pages.secrets.import.columns.lastChanged")}
                </th>
                <th className="px-2 py-2 text-left font-medium">
                  {t("pages.secrets.import.columns.suggestedName")}
                </th>
                <th className="px-2 py-2 text-left font-medium">
                  {t("pages.secrets.import.columns.state")}
                </th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                      <Checkbox
                        ...
                        aria-label={t("pages.secrets.import.selectSecretAria", {
                        ...
                        {candidate.status === "duplicate" &&
                          candidate.conflicts.find((c) => c.type === "exact_reference")?.existingSecretId && (
                            <span className="text-(length:--text-micro) text-muted-foreground">
                              {t("pages.secrets.import.alreadyImported")}
                            </span>
                          )}
                      </div>
        ...
        {hasNextPage && !previewError && (
          <div className="flex items-center justify-between border-t border-border/60 px-5 py-2 text-xs text-muted-foreground">
            <span>
              {t("pages.secrets.import.loaded", { count: candidates.length })}
              {selectableInLoaded.length > 0 && (
                <span>
                  {" · "}
                  {t("pages.secrets.import.selectable", { count: selectableInLoaded.length })}
                </span>
              )}
            </span>
            <Button
            ...
            >
              {pageLoading ? (
                <>
                  <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> {t("pages.secrets.status.loading")}
                </>
              ) : (
                t("pages.secrets.import.loadMore", { count: PAGE_SIZE })
              )}
            </Button>
          </div>
        )}
    ...
    >
      ...
      <div className="flex-1">
        <div className="font-medium">
          {isPermission
            ? t("pages.secrets.import.errors.awsDenied")
            : isThrottling
              ? t("pages.secrets.import.errors.awsThrottled")
              : t("pages.secrets.import.errors.loadRemoteFailed")}
        </div>
        <div className="mt-1 text-xs leading-relaxed text-destructive/80">
          {isPermission
            ? t("pages.secrets.import.errors.missingPermission")
            : message}
        </div>
        <div className="mt-2 flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={onRetry}>
            <RefreshCw className="mr-1.5 h-3.5 w-3.5" /> {t("pages.secrets.actions.retry")}
          </Button>
          {isPermission && (
            <a
            ...
            >
              {t("pages.secrets.import.iamReference")} <ExternalLink className="h-3 w-3" />
            </a>
          )}
        </div>
      </div>
...
function EmptyCandidates({ query }: { query: string }) {
  if (query) {
    return (
      <EmptyState
        icon={Search}
        message={t("pages.secrets.import.noMatch", { query })}
      />
    );
  }
  return (
    <EmptyState
      icon={Database}
      message={t("pages.secrets.import.noVisibleSecrets")}
    />
  );
}
  ...
  if (drafts.length === 0) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center p-6">
        <EmptyState
          icon={Info}
          message={t("pages.secrets.import.noSelection")}
        />
      </div>
    );
  }
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-3 border-b border-border/60 bg-muted/20 px-5 py-3 text-xs">
        <span className="font-medium">
          {t("pages.secrets.import.readyCount", { count: ready })}
        </span>
        {blocked > 0 && (
          <span className="text-amber-600 dark:text-amber-400">
            {t("pages.secrets.import.blockedCount", { count: blocked })}
          </span>
        )}
      </div>
            ...
            >
              <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">
                        {t("pages.secrets.import.paperclipName")}
                      </span>
                      <Input
                        value={draft.name}
                        onChange={(e) =>
                    ...
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">
                        {t("pages.secrets.fields.description")} {t("pages.secrets.common.optional")}
                      </span>
                      <Input
                ...
                <Button
                  ...
                  aria-label={t("pages.secrets.import.removeAria", {
...
function ResultStep({ result, draftList }: ResultStepProps) {
  ...
  const heading =
    result.errorCount === result.results.length && result.errorCount > 0
      ? t("pages.secrets.import.toasts.failed")
      : result.errorCount === 0 && result.skippedCount === 0
        ? t("pages.secrets.import.allImported", { count: result.importedCount })
        : t("pages.secrets.import.toasts.complete");
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="border-b border-border/60 px-5 py-3" data-testid="result-summary">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
          <span className="text-emerald-600 dark:text-emerald-400">
            ✓ {t("pages.secrets.import.createdCount", { count: result.importedCount })}
          </span>
          <span>⊘ {t("pages.secrets.import.skippedCount", { count: result.skippedCount })}</span>
          <span className="text-destructive">
            ⨯ {t("pages.secrets.import.failedCount", { count: result.errorCount })}
          </span>
        </div>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto">
        {grouped.created.length > 0 && (
          <ResultGroup
            label={t("pages.secrets.status.created")}
        ...
        {grouped.skipped.length > 0 && (
          <ResultGroup
            label={t("pages.secrets.status.skipped")}
        ...
        {grouped.failed.length > 0 && (
          <ResultGroup
            label={t("pages.secrets.status.failed")}
            rows={grouped.failed}
            draftLookup={draftLookup}
          />
        )}
  ...
  if (step === "select") {
    return (
      <div className="text-xs text-muted-foreground">
        {totalSelected === 0
          ? t("pages.secrets.import.selectPrompt")
          : t("pages.secrets.import.selectedCount", { count: totalSelected })}
      </div>
    );
  }
  if (step === "review") {
    return (
      <div className="text-xs text-muted-foreground">
        {t("pages.secrets.import.readyShort", { count: readyReviewCount })}
        {blockedReviewCount > 0 && (
          <span className="ml-2 text-amber-600 dark:text-amber-400">
            · {t("pages.secrets.import.blockedShort", { count: blockedReviewCount })}
          </span>
        )}
      </div>
    );
  ...
  if (result) {
    return (
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        <span>{t("pages.secrets.import.createdCount", { count: result.importedCount })}</span>
        <span>{t("pages.secrets.import.skippedCount", { count: result.skippedCount })}</span>
        <span>{t("pages.secrets.import.failedCount", { count: result.errorCount })}</span>
      </div>
    );
  }
  return null;

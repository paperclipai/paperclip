  ...
  if (error instanceof ApiError && error.status === 412) {
    return (
      <EmptyState
        ...
        message="Briefs needs a configured built-in agent before live content can load."
      />
    );
  }
    ...
    <div className="space-y-5">
      <div className="grid border-y border-border sm:grid-cols-3">
        {data.summaryItems.map((item) => (
          <div
            key={item.label}
            className="space-y-1 border-b border-border py-3 sm:border-b-0 sm:border-r sm:px-4 first:sm:pl-0 last:sm:border-r-0 last:sm:pr-0"
          >

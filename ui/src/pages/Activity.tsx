...
export function Activity() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("sidebar.activity", { defaultValue: "Activity" }) }]);
  }, [setBreadcrumbs, t]);
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={History} message={t("activity.selectCompany", { defaultValue: "Select a company to view activity." })} />;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-end">
        <Select value={filter} onValueChange={setFilter}>
          <SelectTrigger className="w-[140px] h-8 text-xs">
            <SelectValue placeholder={t("activity.filterByType", { defaultValue: "Filter by type" })} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">{t("activity.allTypes", { defaultValue: "All types" })}</SelectItem>
      ...
      {filtered && filtered.length === 0 && (
        <EmptyState icon={History} message={t("activity.empty", { defaultValue: "No activity yet." })} />
      )}

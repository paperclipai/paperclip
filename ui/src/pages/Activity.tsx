...
export function Activity() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("pages.activity.breadcrumb", { defaultValue: "Activity" }) }]);
  }, [setBreadcrumbs, t]);
  ...
  if (!selectedCompanyId) {
    return (
      <EmptyState
        icon={History}
        message={t("pages.activity.selectCompany", {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-end">
        <Select value={filter} onValueChange={setFilter}>
          <SelectTrigger className="w-[140px] h-8 text-xs">
            <SelectValue
              placeholder={t("pages.activity.filterByType", {
          ...
          <SelectContent>
            <SelectItem value="all">
              {t("pages.activity.allTypes", { defaultValue: "All types" })}
            </SelectItem>
      ...
      {filtered && filtered.length === 0 && (
        <EmptyState
          icon={History}
          message={t("pages.activity.noActivity", { defaultValue: "No activity yet." })}
        />
      )}

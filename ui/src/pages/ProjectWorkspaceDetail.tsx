      ...
      <div className="flex flex-wrap items-center gap-3">
        <Button variant="ghost" size="sm" asChild>
          <Link to={`/projects/${canonicalProjectRef}/workspaces`}>
            ...
            {t("project_workspace_detail.back_to_workspaces")}
          </Link>
        </Button>
        <div className="inline-flex items-center rounded-full border border-border bg-background px-2.5 py-1 text-xs text-muted-foreground">
          {workspace.isPrimary ? t("project_workspace_detail.primary_workspace") : t("project_workspace_detail.secondary_workspace")}
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
              <div className="space-y-2">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
                  {t("project_workspace_detail.project_workspace")}
                </div>
              ...
              {!workspace.isPrimary ? (
                ...
                >
                  ...
                  {t("project_workspace_detail.make_primary")}
                </Button>
              ) : (
                <div className="inline-flex items-center gap-2 rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-700 dark:text-emerald-300 sm:max-w-sm">
                  ...
                  {t("project_workspace_detail.primary_codebase_workspace")}
                </div>
              )}
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Field label={t("project_workspace_detail.workspace_name")}>
                <input
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                  value={form.name}
                  onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                  placeholder={t("project_workspace_detail.workspace_name")}
                />
              </Field>

              <Field label={t("project_workspace_detail.visibility")}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
            ...
            <div className="mt-4 grid gap-4">
              <Field label={t("project_workspace_detail.source_type")} hint={sourceTypeDescription ?? undefined}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto]">
                <Field label={t("project_workspace_detail.local_path")}>
                  <input
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("project_workspace_detail.repo_url")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                    value={form.repoUrl}
                ...
                <Field label={t("project_workspace_detail.repo_ref")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.repoRef}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("project_workspace_detail.default_ref")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                ...
                <Field label={t("project_workspace_detail.shared_workspace_key")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("project_workspace_detail.remote_provider")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                ...
                <Field label={t("project_workspace_detail.remote_workspace_ref")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("project_workspace_detail.setup_command")} hint={t("project_workspace_detail.setup_command_hint")}>
                  <textarea
                ...
                <Field label={t("project_workspace_detail.cleanup_command")} hint={t("project_workspace_detail.cleanup_command_hint")}>
                  <textarea
                    className="min-h-28 w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <Field label={t("project_workspace_detail.runtime_services_json")} hint={t("project_workspace_detail.runtime_services_json_hint")}>
                <textarea
            ...
            <div className="mt-5 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
              <Button className="w-full sm:w-auto" disabled={!isDirty || updateWorkspace.isPending} onClick={saveChanges}>
                ...
                {t("project_workspace_detail.save_changes")}
              </Button>
              <Button
                variant="outline"
                className="w-full sm:w-auto"
              ...
              >
                {t("project_workspace_detail.reset")}
              </Button>
              {errorMessage ? <p className="text-sm text-destructive">{errorMessage}</p> : null}
              {!errorMessage && runtimeActionMessage ? <p className="text-sm text-muted-foreground">{runtimeActionMessage}</p> : null}
              {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">{t("project_workspace_detail.no_unsaved_changes")}</p> : null}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="space-y-1">
              <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("project_workspace_detail.workspace_facts")}</div>
              <h2 className="text-lg font-semibold">{t("project_workspace_detail.current_state")}</h2>
            </div>
            <Separator className="my-4" />
            <DetailRow label={t("project_workspace_detail.project")}>
              <Link to={`/projects/${canonicalProjectRef}`} className="hover:underline">{project.name}</Link>
            </DetailRow>
            <DetailRow label={t("project_workspace_detail.workspace_id")}>
              <span className="break-all font-mono text-xs">{workspace.id}</span>
            </DetailRow>
            <DetailRow label={t("project_workspace_detail.local_path")}>
              <span className="break-all font-mono text-xs">{workspace.cwd ?? t("project_workspace_detail.none")}</span>
            </DetailRow>
            <DetailRow label={t("project_workspace_detail.repo")}>
              ...
              ) : t("project_workspace_detail.none")}
            </DetailRow>
            <DetailRow label={t("project_workspace_detail.default_ref")}>{workspace.defaultRef ?? t("project_workspace_detail.none")}</DetailRow>
            <DetailRow label={t("project_workspace_detail.updated")}>{new Date(workspace.updatedAt).toLocaleString()}</DetailRow>
          </div>

          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("project_workspace_detail.runtime_services")}</div>
                <h2 className="text-lg font-semibold">{t("project_workspace_detail.attached_services")}</h2>
                <p className="text-sm text-muted-foreground">
                  {t("project_workspace_detail.attached_services_hint")}
                </p>
              </div>
              <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap">
                ...
                >
                  ...
                  {t("project_workspace_detail.start")}
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                ...
                >
                  {t("project_workspace_detail.restart")}
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                ...
                >
                  {t("project_workspace_detail.stop")}
                </Button>
              </div>
            </div>
            <Separator className="my-4" />
            {workspace.runtimeServices && workspace.runtimeServices.length > 0 ? (
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div className="space-y-1">
                        ...
                        <div className="space-y-1 text-xs text-muted-foreground">
                          ...
                          <div>{service.command ?? t("project_workspace_detail.no_command_recorded")}</div>
              ...
              <p className="text-sm text-muted-foreground">
                {workspace.runtimeConfig?.workspaceRuntime
                  ? t("project_workspace_detail.no_runtime_services_running")
                  : t("project_workspace_detail.no_runtime_service_default")}
              </p>
            )}
          </div>
        </div>

  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? t("project.workspaceJobCompleted", { defaultValue: "Workspace job completed." })
          : request.action === "stop"
            ? t("project.workspaceServiceStopped", { defaultValue: "Workspace service stopped." })
            : request.action === "restart"
              ? t("project.workspaceServiceRestarted", { defaultValue: "Workspace service restarted." })
              : t("project.workspaceServiceStarted", { defaultValue: "Workspace service started." }),
      );
    },
    onError: (error) => {
          ...
          <div className="rounded-2xl border border-border bg-card p-5">
            ...
            <div className="mt-4 grid gap-4">
              ...
              <details className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-3 py-3">
                <summary className="cursor-pointer text-sm font-medium">{t("project.advancedRuntimeJson", { defaultValue: "Advanced runtime JSON" })}</summary>
                <p className="mt-2 text-sm text-muted-foreground">
                  {t("project.advancedRuntimeJsonDescription", { defaultValue: "Paperclip derives Services and Jobs from this JSON. Prefer editing named commands first; use raw JSON for advanced lifecycle, port, readiness, or environment settings." })}
                </p>
                <div className="mt-3">
                  <Field label={t("project.fieldWorkspaceCommandsJson", { defaultValue: "Workspace commands JSON" })} hint={t("project.hintWorkspaceCommandsJson", { defaultValue: "Execution workspaces inherit this config unless they override it. Legacy `services` arrays still work, but `commands` supports both services and jobs." })}>
                    <textarea
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("project.workspaceCommands", { defaultValue: "Workspace commands" })}</div>
                <h2 className="text-lg font-semibold">{t("project.servicesAndJobs", { defaultValue: "Services and jobs" })}</h2>
                <p className="text-sm text-muted-foreground">
                  {t("project.servicesAndJobsDescription", { defaultValue: "Long-running services stay supervised here, while one-shot jobs run on demand against this workspace. Execution workspaces inherit this config unless they override it." })}
                </p>
              </div>
            </div>
            <WorkspaceRuntimeControls
              ...
              serviceEmptyMessage={
                workspace.runtimeConfig?.workspaceRuntime
                  ? t("project.noServicesStarted", { defaultValue: "No services have been started for this workspace yet." })
                  : t("project.noWorkspaceCommandConfig", { defaultValue: "No workspace command config is defined for this workspace yet." })
              }
              jobEmptyMessage={t("project.noJobsConfigured", { defaultValue: "No one-shot jobs are configured for this workspace yet." })}
              disabledHint={t("project.disabledHint", { defaultValue: "Project workspaces need a working directory before local commands can run, and services also need runtime config." })}

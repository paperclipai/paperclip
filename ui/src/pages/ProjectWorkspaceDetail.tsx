...
function getProjectWorkspaceBaseTabItems(): OrderedProjectWorkspaceTabItem[] {
  return [
    {
      value: "configuration",
      label: t("pages.projectWorkspaceDetail.tabConfiguration", { defaultValue: "Configuration" }),
      order: 30,
    },
  ];
}
...
function getSourceTypeOptions(): Array<{ value: ProjectWorkspaceSourceType; label: string; description: string }> {
  return [
    {
      value: "local_path",
      label: t("pages.projectWorkspaceDetail.sourceTypeLocalPathLabel", { defaultValue: "Local git checkout" }),
      description: t("pages.projectWorkspaceDetail.sourceTypeLocalPathDescription", {
        defaultValue: "A local path Paperclip can use directly.",
      }),
    },
    {
      value: "non_git_path",
      label: t("pages.projectWorkspaceDetail.sourceTypeNonGitPathLabel", { defaultValue: "Local non-git path" }),
      description: t("pages.projectWorkspaceDetail.sourceTypeNonGitPathDescription", {
        defaultValue: "A local folder without git semantics.",
      }),
    },
    {
      value: "git_repo",
      label: t("pages.projectWorkspaceDetail.sourceTypeGitRepoLabel", { defaultValue: "Remote git repo" }),
      description: t("pages.projectWorkspaceDetail.sourceTypeGitRepoDescription", {
        defaultValue: "A repo URL with optional refs and local checkout.",
      }),
    },
    {
      value: "remote_managed",
      label: t("pages.projectWorkspaceDetail.sourceTypeRemoteManagedLabel", {
        defaultValue: "Remote-managed workspace",
      }),
      description: t("pages.projectWorkspaceDetail.sourceTypeRemoteManagedDescription", {
        defaultValue: "A hosted workspace tracked by external reference.",
      }),
    },
  ];
}

function getVisibilityOptions(): Array<{ value: ProjectWorkspaceVisibility; label: string }> {
  return [
    { value: "default", label: t("pages.projectWorkspaceDetail.visibilityDefault", { defaultValue: "Default" }) },
    { value: "advanced", label: t("pages.projectWorkspaceDetail.visibilityAdvanced", { defaultValue: "Advanced" }) },
  ];
}
...
function parseRuntimeConfigJson(value: string) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: t("pages.projectWorkspaceDetail.errorCommandsJsonMustBeObject", {
          defaultValue: "Workspace commands JSON must be a JSON object.",
        }),
      };
    }
    ...
    return {
      ...
      error: error instanceof Error
        ? error.message
        : t("pages.projectWorkspaceDetail.errorInvalidJson", { defaultValue: "Invalid JSON." }),
    };
  }
}
...
function validateWorkspaceForm(form: WorkspaceFormState) {
  ...
  if (form.sourceType === "remote_managed") {
    if (!remoteWorkspaceRef && !repoUrl) {
      return t("pages.projectWorkspaceDetail.validationRemoteManagedRequiresRef", {
        defaultValue: "Remote-managed workspaces require a remote workspace ref or repo URL.",
      });
    }
  } else if (!cwd && !repoUrl) {
    return t("pages.projectWorkspaceDetail.validationRequiresPathOrRepo", {
  ...
  if (cwd && (form.sourceType === "local_path" || form.sourceType === "non_git_path") && !isAbsolutePath(cwd)) {
    return t("pages.projectWorkspaceDetail.validationLocalPathAbsolute", {
  ...
  if (repoUrl) {
    try {
      ...
      return t("pages.projectWorkspaceDetail.validationRepoUrlInvalid", {
...
export function ProjectWorkspaceDetail() {
  ...
  const tabItems = useMemo(
    () => orderProjectWorkspaceTabItems([...getProjectWorkspaceBaseTabItems(), ...pluginTabItems]),
    [pluginTabItems],
  );
  ...
  useEffect(() => {
    if (!project) return;
    setBreadcrumbs([
      { label: t("pages.projectWorkspaceDetail.breadcrumbProjects", { defaultValue: "Projects" }), href: "/projects" },
      { label: project.name, href: `/projects/${canonicalProjectRef}` },
      {
        label: t("pages.projectWorkspaceDetail.breadcrumbWorkspaces", { defaultValue: "Workspaces" }),
    ...
    onError: (error) => {
      setErrorMessage(
        error instanceof Error
          ? error.message
          : t("pages.projectWorkspaceDetail.errorSaveWorkspace", { defaultValue: "Failed to save workspace." }),
      );
    },
  });
  ...
  const setPrimaryWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(
        error instanceof Error
          ? error.message
          : t("pages.projectWorkspaceDetail.errorUpdateWorkspace", { defaultValue: "Failed to update workspace." }),
      );
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? t("pages.projectWorkspaceDetail.runtimeJobCompleted", { defaultValue: "Workspace job completed." })
          : request.action === "stop"
            ? t("pages.projectWorkspaceDetail.runtimeServiceStopped", {
                defaultValue: "Workspace service stopped. Task execution is not paused.",
              })
            : request.action === "restart"
              ? t("pages.projectWorkspaceDetail.runtimeServiceRestarted", {
                  defaultValue: "Workspace service restarted. Task execution is not paused.",
                })
              : t("pages.projectWorkspaceDetail.runtimeServiceStarted", {
                  defaultValue: "Workspace service started.",
                }),
      );
    },
    onError: (error) => {
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : t("pages.projectWorkspaceDetail.errorControlCommands", {
              defaultValue: "Failed to control workspace commands.",
            }),
      );
    },
  });
  ...
  if (projectQuery.isLoading)
    return (
      <p className="text-sm text-muted-foreground">
        {t("pages.projectWorkspaceDetail.loadingWorkspace", { defaultValue: "Loading workspace…" })}
      </p>
    );
    ...
    return (
      <p className="text-sm text-destructive">
        {projectQuery.error instanceof Error
          ...
          : t("pages.projectWorkspaceDetail.errorLoadWorkspace", { defaultValue: "Failed to load workspace" })}
      </p>
    );
  }
  if (!project || !workspace || !form || !initialState) {
    return (
      <p className="text-sm text-muted-foreground">
        {t("pages.projectWorkspaceDetail.workspaceNotFound", { defaultValue: "Workspace not found for this project." })}
      </p>
    );
  }
  ...
  const sourceTypeDescription = getSourceTypeOptions().find((option) => option.value === form.sourceType)?.description ?? null;
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 space-y-2">
          <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
            {t("pages.projectWorkspaceDetail.eyebrowProjectWorkspace", { defaultValue: "Project workspace" })}
          </div>
        ...
        {!workspace.isPrimary ? (
          ...
          <div className="inline-flex items-center gap-2 rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-700 dark:text-emerald-300 sm:max-w-sm">
            ...
            {t("pages.projectWorkspaceDetail.primaryWorkspaceBanner", {
              defaultValue: "This is the project’s primary codebase workspace.",
            })}
          </div>
        )}
      </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <p className="max-w-2xl text-sm text-muted-foreground">
              {t("pages.projectWorkspaceDetail.configurationIntro", {
                defaultValue:
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Field label={t("pages.projectWorkspaceDetail.fieldWorkspaceName", { defaultValue: "Workspace name" })}>
                <input
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                  value={form.name}
                  onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                  placeholder={t("pages.projectWorkspaceDetail.placeholderWorkspaceName", { defaultValue: "Workspace name" })}
                />
              </Field>

              <Field label={t("pages.projectWorkspaceDetail.fieldVisibility", { defaultValue: "Visibility" })}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
            ...
            <div className="mt-4 grid gap-4">
              <Field label={t("pages.projectWorkspaceDetail.fieldSourceType", { defaultValue: "Source type" })} hint={sourceTypeDescription ?? undefined}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto]">
                <Field label={t("pages.projectWorkspaceDetail.fieldLocalPath", { defaultValue: "Local path" })}>
                  <input
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("pages.projectWorkspaceDetail.fieldRepoUrl", { defaultValue: "Repo URL" })}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                    value={form.repoUrl}
                ...
                <Field label={t("pages.projectWorkspaceDetail.fieldRepoRef", { defaultValue: "Repo ref" })}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.repoRef}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("pages.projectWorkspaceDetail.fieldDefaultRef", { defaultValue: "Default ref" })}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                ...
                <Field label={t("pages.projectWorkspaceDetail.fieldSharedWorkspaceKey", { defaultValue: "Shared workspace key" })}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("pages.projectWorkspaceDetail.fieldRemoteProvider", { defaultValue: "Remote provider" })}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                ...
                <Field label={t("pages.projectWorkspaceDetail.fieldRemoteWorkspaceRef", { defaultValue: "Remote workspace ref" })}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field
                  label={t("pages.projectWorkspaceDetail.fieldSetupCommand", { defaultValue: "Setup command" })}
                  hint={t("pages.projectWorkspaceDetail.hintSetupCommand", {
                    defaultValue: "Runs when this workspace needs custom bootstrap",
                  })}
                >
                  <textarea
                ...
                <Field
                  label={t("pages.projectWorkspaceDetail.fieldCleanupCommand", { defaultValue: "Cleanup command" })}
                  hint={t("pages.projectWorkspaceDetail.hintCleanupCommand", {
                    defaultValue: "Runs before project-level execution workspace teardown",
                  })}
                >
                  <textarea
                    className="min-h-28 w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <details className="rounded-xl border border-dashed border-border/70 bg-background px-3 py-3">
                ...
                <div className="mt-3">
                  <Field
                    label={t("pages.projectWorkspaceDetail.fieldWorkspaceCommandsJson", {
                      defaultValue: "Workspace commands JSON",
                    })}
                    hint={t("pages.projectWorkspaceDetail.hintWorkspaceCommandsJson", {
                      defaultValue:
                        "Execution workspaces inherit this config unless they override it. Legacy `services` arrays still work, but `commands` supports both services and jobs.",
                    })}
                  >
                    <textarea
            ...
            <div className="mt-5 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
              ...
              {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">{t("pages.projectWorkspaceDetail.noUnsavedChanges", { defaultValue: "No unsaved changes." })}</p> : null}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="space-y-1">
              <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("pages.projectWorkspaceDetail.workspaceFacts", { defaultValue: "Workspace facts" })}</div>
              <h2 className="text-lg font-semibold">{t("pages.projectWorkspaceDetail.currentState", { defaultValue: "Current state" })}</h2>
            </div>
            <Separator className="my-4" />
            <DetailRow label={t("pages.projectWorkspaceDetail.detailProject", { defaultValue: "Project" })}>
              <Link to={`/projects/${canonicalProjectRef}`} className="hover:underline">{project.name}</Link>
            </DetailRow>
            <DetailRow label={t("pages.projectWorkspaceDetail.detailWorkspaceId", { defaultValue: "Workspace ID" })}>
              <span className="break-all font-mono text-xs">{workspace.id}</span>
            </DetailRow>
            <DetailRow label={t("pages.projectWorkspaceDetail.detailLocalPath", { defaultValue: "Local path" })}>
              <span className="break-all font-mono text-xs">{workspace.cwd ?? t("pages.projectWorkspaceDetail.valueNone", { defaultValue: "None" })}</span>
            </DetailRow>
            <DetailRow label={t("pages.projectWorkspaceDetail.detailRepo", { defaultValue: "Repo" })}>
              ...
              ) : t("pages.projectWorkspaceDetail.valueNone", { defaultValue: "None" })}
            </DetailRow>
            <DetailRow label={t("pages.projectWorkspaceDetail.detailDefaultRef", { defaultValue: "Default ref" })}>{workspace.defaultRef ?? t("pages.projectWorkspaceDetail.valueNone", { defaultValue: "None" })}</DetailRow>
            <DetailRow label={t("pages.projectWorkspaceDetail.detailUpdated", { defaultValue: "Updated" })}>{new Date(workspace.updatedAt).toLocaleString()}</DetailRow>
          </div>

          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("pages.projectWorkspaceDetail.workspaceCommands", { defaultValue: "Workspace commands" })}</div>
            ...
            <WorkspaceRuntimeControls
              ...
              serviceEmptyMessage={
                workspace.runtimeConfig?.workspaceRuntime
                  ? t("pages.projectWorkspaceDetail.serviceEmptyNoServicesStarted", {
                      defaultValue: "No services have been started for this workspace yet.",
                    })
                  : t("pages.projectWorkspaceDetail.serviceEmptyNoConfig", {
                      defaultValue: "No workspace command config is defined for this workspace yet.",
                    })
              }
              jobEmptyMessage={t("pages.projectWorkspaceDetail.jobEmptyMessage", {
                defaultValue: "No one-shot jobs are configured for this workspace yet.",
              })}
              disabledHint={t("pages.projectWorkspaceDetail.disabledHint", {
                defaultValue:
                  "Project workspaces need a working directory before local commands can run, and services also need runtime config.",
              })}
      ...
      {isProjectWorkspacePluginTab(activeTab) ? (
        activePluginTab ? (
          ...
          <div className="rounded-lg border border-dashed border-border bg-background px-4 py-8 text-sm text-muted-foreground">
            {pluginDetailSlotsError ? pluginDetailSlotsError : t("pages.projectWorkspaceDetail.loadingWorkspacePlugin", { defaultValue: "Loading workspace plugin..." })}
          </div>
        ) : (

...
function ExpiryValue({ token }: { token: ToolMcpGatewayToken }) {
  ...
  if (!expiresAt) return <>{t("tokens-panel.no-expiry-vc8")}</>;
  if (expiresAt.getTime() <= Date.now()) {
    return <><span className="font-medium text-foreground">{t("tokens-panel.expired-1gc")}</span> <RelativeTime value={token.expiresAt} /></>;
  }
  return <>{t("tokens-panel.expires-1je")} <RelativeTime value={token.expiresAt} /></>;
}
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="max-w-2xl space-y-1">
          <p className="text-sm text-muted-foreground">
            {t("tokens-panel.issue-a-reusable-token-for-an-extern-usj")}
          </p>
          <p className="text-xs text-muted-foreground">
            {t("tokens-panel.paperclip-creates-a-fresh-one-hour-r-16w")}
          </p>
        </div>
        <Button size="sm" onClick={startIssuing}>
          <Plus className="mr-1.5 h-3.5 w-3.5" />
          {t("tokens-panel.issue-token-p33")}
        </Button>
      </div>
        ...
        <form className="space-y-3" onSubmit={submit}>
          <div className="grid gap-3 md:grid-cols-[1fr_auto]">
            <label className="space-y-1.5 text-sm">
              <span className="text-xs font-medium text-muted-foreground">{t("tokens-panel.token-name-9wq")}</span>
              <Input value={name} onChange={(event) => setName(event.target.value)} required autoFocus />
              <span className="text-xs text-muted-foreground">{t("tokens-panel.also-used-as-the-client-label-7b8")}</span>
            </label>
            <label className="space-y-1.5 text-sm">
              <span className="text-xs font-medium text-muted-foreground">{t("tokens-panel.expires-1je")}</span>
              <Input type="date" value={expiresAt} onChange={(event) => setExpiresAt(event.target.value)} required />
            </label>
          </div>
          <label className="block space-y-1.5 text-sm">
            <span className="text-xs font-medium text-muted-foreground">{t("tokens-panel.owner-note-optional-1ob")}</span>
            <Input
              value={ownerNote}
              onChange={(event) => setOwnerNote(event.target.value)}
              placeholder={t("tokens-panel.who-uses-this-token-or-why-it-exists-u7s")}
            />
          </label>
          <div className="flex justify-end gap-2">
            <Button type="button" variant="ghost" size="sm" onClick={() => setMinting(false)}>
              {t("tokens-panel.cancel-ew9")}
            </Button>
      ...
      {created ? (
        <div className="space-y-2 border-y border-border py-4">
          <div className="flex items-center justify-between gap-2">
            <div>
              <div className="text-sm font-semibold text-foreground">{t("tokens-panel.new-token-copy-now-3p8")}</div>
              <div className="text-xs text-muted-foreground">
                {t("tokens-panel.it-is-now-available-in-client-snippe-1cb")}
              </div>
            </div>
            <Button variant="ghost" size="sm" onClick={() => setCreated(null)} aria-label={t("tokens-panel.dismiss-new-token-1kv")}>
              {t("tokens-panel.dismiss-an1")}
            </Button>
          </div>
          <div className="flex items-center gap-2">
            ...
            {revealed ? (
              <Button variant="outline" size="sm" onClick={() => void copyToken(created.token)}>
                ...
                {t("tokens-panel.copy-s6g")}
              </Button>
            ) : (
              <Button variant="outline" size="sm" onClick={() => setRevealed(true)}>{t("tokens-panel.show-bi2")}</Button>
            )}
          </div>
        </div>
      ) : null}
      ...
      <Collapsible open={historyOpen} onOpenChange={setHistoryOpen}>
        <CollapsibleTrigger asChild>
          <Button variant="ghost" className="h-auto w-full justify-between px-0 py-1 hover:bg-transparent">
            <span className="flex items-center gap-2 text-sm font-semibold text-foreground">
              ...
              {t("tokens-panel.token-history-1o7")}
        ...
        <CollapsibleContent className="space-y-3 pt-3">
          {tokens.length === 0 ? (
            <p className="py-4 text-sm text-muted-foreground">{t("tokens-panel.no-tokens-have-been-issued-for-this-o32")}</p>
          ) : (
            <>
                ...
                <table className="w-full min-w-(--sz-44rem) text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/40 text-left text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
                      <th className="px-4 py-2.5">{t("tokens-panel.token-g5f")}</th>
                      <th className="px-4 py-2.5">{t("tokens-panel.owner-171")}</th>
                      <th className="px-4 py-2.5">{t("tokens-panel.created-2qk")}</th>
                      <th className="px-4 py-2.5">{t("tokens-panel.last-used-7s3")}</th>
                      <th className="px-4 py-2.5">{t("tokens-panel.expiry-r38")}</th>
                      <th className="px-4 py-2.5">{t("tokens-panel.status-3pd")}</th>
                  ...
                  return (
                    <div key={token.id} className="border-b border-border py-3 last:border-0">
                      ...
                      <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                        <TokenField label={t("tokens-panel.owner-171")} value={token.clientLabel || token.ownerNote || "—"} />
                        <TokenField label={t("tokens-panel.created-2qk")} value={<RelativeTime value={token.createdAt} />} />
                        <TokenField label={t("tokens-panel.last-used-7s3")} value={token.lastUsedAt ? <RelativeTime value={token.lastUsedAt} /> : "—"} />
                        <TokenField label={t("tokens-panel.expiry-r38")} value={<ExpiryValue token={token} />} />
                      </dl>
                      {status !== "revoked" ? (
                        <Button
                          variant="ghost"
              ...
              {pageCount > 1 ? (
                <div className="flex items-center justify-between gap-3">
                  <p className="text-xs text-muted-foreground">
                    {t("tokens-panel.page-1sf")} {historyPage} of {pageCount} · {tokens.length} tokens
                  </p>
                  <div className="flex items-center gap-1">
                    <Button
                      ...
                      aria-label={t("tokens-panel.previous-token-page-11h")}
                    >
                    ...
                    <Button
                      ...
                      aria-label={t("tokens-panel.next-token-page-7f8")}
                    >
      ...
      {confirmToken ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-md space-y-3 rounded-lg border border-border bg-card p-5 shadow-lg">
            <div>
              <h3 className="text-sm font-semibold text-foreground">{t("tokens-panel.revoke-this-token-12t")}</h3>
              <p className="mt-1 text-sm text-muted-foreground">
                {t("tokens-panel.any-client-using-1l1")} <span className="font-medium text-foreground">{confirmToken.name}</span> {t("tokens-panel.goes-silent-immediately-this-can-t-b-e5j")}
              </p>
            </div>
            <Input
              value={revokeName}
              ...
              aria-label={t("tokens-panel.type-the-token-name-to-confirm-wlo")}
              autoFocus
            />
            <div className="flex justify-end gap-2">
              <Button

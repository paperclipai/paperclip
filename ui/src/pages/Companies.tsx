...
export function Companies() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("companies.title", { defaultValue: "Companies" }) }]);
  }, [setBreadcrumbs, t]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-end">
        <Button size="sm" onClick={() => openOnboarding()}>
          ...
          {t("companies.newCompany", { defaultValue: "New Company" })}
        </Button>
      </div>
            ...
            >
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      >
                        ...
                        {t("companies.rename", { defaultValue: "Rename" })}
                      </DropdownMenuItem>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem
                        variant="destructive"
                      ...
                      >
                        ...
                        {t("companies.deleteCompany", { defaultValue: "Delete Company" })}
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
              ...
              <div className="flex items-center gap-3 sm:gap-5 mt-4 text-sm text-muted-foreground flex-wrap">
                <div className="flex items-center gap-1.5">
                  ...
                  <span>
                    {t("companies.agentCount", { defaultValue: "{{count}} agents", count: agentCount })}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                  <CircleDot className="h-3.5 w-3.5" />
                  <span>
                    {t("companies.issueCount", { defaultValue: "{{count}} issues", count: issueCount })}
                  </span>
                </div>
                <div className="flex items-center gap-1.5 tabular-nums">
                  ...
                  <span>
                    ...
                    {company.budgetMonthlyCents > 0
                      ...
                      : <span className="text-xs ml-1">{t("companies.unlimitedBudget", { defaultValue: "Unlimited budget" })}</span>}
                  </span>
                </div>
                <div className="flex items-center gap-1.5 ml-auto">
                  <Calendar className="h-3.5 w-3.5" />
                  <span>{t("companies.createdAt", { defaultValue: "Created {{when}}", when: relativeTime(company.createdAt) })}</span>
                </div>
              </div>
              ...
              {isConfirmingDelete && (
                ...
                >
                  <p className="text-sm text-destructive font-medium">
                    {t("companies.deleteConfirm", { defaultValue: "Delete this company and all its data? This cannot be undone." })}
                  </p>
                  <div className="flex items-center gap-2 ml-4 shrink-0">
                    ...
                    >
                      {deleteMutation.isPending
                        ? t("companies.deleting", { defaultValue: "Deleting…" })
                        : t("companies.delete", { defaultValue: "Delete" })}
                    </Button>
                  </div>
                </div>
              )}

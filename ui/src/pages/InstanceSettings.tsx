...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("instanceSettings.settings"), href: "/company/settings" },
      { label: t("instanceSettings.instanceSettings"), href: "/company/settings/instance/general" },
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("instanceSettings.unknownError");
        throw new Error(
          failures.length === 1
            ? t("instanceSettings.failedToDisableOne", { detail })
            : t("instanceSettings.failedToDisableMany", {
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("instanceSettings.activeCount", { count: activeCount })}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("instanceSettings.disabledCount", { count: disabledCount })}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? t("instanceSettings.companyOne") : t("instanceSettings.companyMany", { count: grouped.length })}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              if (enabledCount === 1) {
                if (!window.confirm(t("instanceSettings.confirmDisableOne", { count: enabledCount }))) {
                  return;
                }
              } else if (!window.confirm(t("instanceSettings.confirmDisableMany", { count: enabledCount }))) {
                return;
              }

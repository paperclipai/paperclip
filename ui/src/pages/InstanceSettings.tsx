...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("page.instanceSettings.nav.heartbeats", "Heartbeats") },
    ]);
  }, [setBreadcrumbs, t]);
    ...
    onError: (error) => {
      setActionError(
        error instanceof Error
          ? error.message
          : t("page.instanceSettings.heartbeats.errors.update_failed", "Failed to update heartbeat."),
      );
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("page.instanceSettings.heartbeats.errors.unknown_error", "Unknown error");
        throw new Error(
          failures.length === 1
            ? t("page.instanceSettings.heartbeats.errors.disable_one", "Failed to disable 1 timer heartbeat: {{detail}}", { detail })
            : t("page.instanceSettings.heartbeats.errors.disable_many", "Failed to disable {{failed}} of {{total}} timer heartbeats. First error: {{detail}}", {
    ...
    onError: (error) => {
      setActionError(
        error instanceof Error
          ? error.message
          : t("page.instanceSettings.heartbeats.errors.disable_all_failed", "Failed to disable all heartbeats."),
      );
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("page.instanceSettings.heartbeats.loading", "Loading scheduler heartbeats...")}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("page.instanceSettings.heartbeats.errors.load_failed", "Failed to load scheduler heartbeats.")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("page.instanceSettings.heartbeats.title", "Scheduler Heartbeats")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t(
            "page.instanceSettings.heartbeats.description",
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("page.instanceSettings.heartbeats.active", "active")}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("page.instanceSettings.heartbeats.disabled", "disabled")}</span>
        <span>
          <span className="font-semibold text-foreground">{grouped.length}</span>{" "}
          {grouped.length === 1
            ? t("page.instanceSettings.heartbeats.company_one", "company")
            : t("page.instanceSettings.heartbeats.company_other", "companies")}
        </span>
        {anyEnabled && (
          <Button
            variant="destructive"
            ...
            onClick={() => {
              const noun = enabledCount === 1
                ? t("page.instanceSettings.heartbeats.agent_one", "agent")
                : t("page.instanceSettings.heartbeats.agent_other", "agents");
              if (!window.confirm(t("page.instanceSettings.heartbeats.confirm_disable_all", "Disable timer heartbeats for all {{count}} enabled {{noun}}?", { count: enabledCount, noun }))) {
                return;
              }
          ...
          >
            {disableAllMutation.isPending
              ? t("page.instanceSettings.heartbeats.disabling", "Disabling...")
              : t("page.instanceSettings.heartbeats.disable_all", "Disable All")}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("page.instanceSettings.heartbeats.empty", "No scheduler heartbeats match the current criteria.")}
        />
      ) : (
                      ...
                      >
                        ...
                        >
                          {agent.lastHeartbeatAt
                            ...
                            : t("page.instanceSettings.heartbeats.never", "never")}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            to={buildAgentHref(agent)}
                            className="text-muted-foreground hover:text-foreground"
                            title={t("page.instanceSettings.heartbeats.full_agent_config", "Full agent config")}
                          >
                          ...
                          >
                            {saving
                              ...
                              : agent.heartbeatEnabled
                                ? t("page.instanceSettings.heartbeats.disable_timer", "Disable Timer Heartbeat")
                                : t("page.instanceSettings.heartbeats.enable_timer", "Enable Timer Heartbeat")}
                          </Button>
                        </span>
                      </div>
                    );

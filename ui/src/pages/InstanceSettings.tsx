...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("app.instanceSettings.settings", { defaultValue: "Settings" }), href: "/company/settings" },
      { label: t("app.instanceSettings.instanceSettings", { defaultValue: "Instance settings" }), href: "/company/settings/instance/general" },
      { label: t("app.instanceSettings.heartbeats", { defaultValue: "Heartbeats" }) },
    ]);
  }, [setBreadcrumbs]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("app.instanceSettings.failedToUpdateHeartbeat", { defaultValue: "Failed to update heartbeat." }));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("app.instanceSettings.unknownError", { defaultValue: "Unknown error" });
        throw new Error(
          failures.length === 1
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("app.instanceSettings.failedToDisableAllHeartbeats", { defaultValue: "Failed to disable all heartbeats." }));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("app.instanceSettings.loadingSchedulerHeartbeats", { defaultValue: "Loading scheduler heartbeats..." })}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("app.instanceSettings.failedToLoadSchedulerHeartbeats", { defaultValue: "Failed to load scheduler heartbeats." })}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("app.instanceSettings.schedulerHeartbeats", { defaultValue: "Scheduler Heartbeats" })}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("app.instanceSettings.agentsWithATimerHeartbeatEnabledAcrossAllOfYourCompanies", { defaultValue: "Agents with a timer heartbeat enabled across all of your companies." })}</p>
      </div>

      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("app.instanceSettings.active", { defaultValue: "active" })}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("app.instanceSettings.disabled", { defaultValue: "disabled" })}</span>
        ...
        {anyEnabled && (
          ...
          >
            {disableAllMutation.isPending ? t("app.instanceSettings.disabling", { defaultValue: "Disabling..." }) : t("app.instanceSettings.disableAll", { defaultValue: "Disable All" })}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("app.instanceSettings.noSchedulerHeartbeatsMatchTheCurrentCriteria", { defaultValue: "No scheduler heartbeats match the current criteria." })}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("app.instanceSettings.on", { defaultValue: "On" }) : t("app.instanceSettings.off", { defaultValue: "Off" })}
                        </Badge>
                        <Link
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            ...
                            title={t("app.instanceSettings.fullAgentConfig", { defaultValue: "Full agent config" })}
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("app.instanceSettings.disableTimerHeartbeat", { defaultValue: "Disable Timer Heartbeat" }) : t("app.instanceSettings.enableTimerHeartbeat", { defaultValue: "Enable Timer Heartbeat" })}
                          </Button>
                        </span>
                      </div>
                    );

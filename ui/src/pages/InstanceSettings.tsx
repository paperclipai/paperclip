    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("instanceSettings.heartbeats.loadFailed")}
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        ...
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? t("sidebar.company") : t("sidebar.companies")}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              ...
              if (!window.confirm(`${t("instanceSettings.heartbeats.disableConfirm", "Disable timer heartbeats for all {{count}} enabled agents?", { count: enabledCount })}`)) {
                return;
              }
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("instanceSettings.heartbeats.emptyMessage")}
        />
      ) : (

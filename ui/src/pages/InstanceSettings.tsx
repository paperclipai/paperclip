...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.updateFailed"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("common.unknown");
        throw new Error(
          failures.length === 1
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.disableAllFailed"));
    },
  });
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span>{t("instanceSettings.activeCount", { count: activeCount })}</span>
        <span>{t("instanceSettings.disabledCount", { count: disabledCount })}</span>
        <span>{t("instanceSettings.companyCount", { count: grouped.length })}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              if (!window.confirm(t("instanceSettings.disableAllConfirm", { count: enabledCount }))) {
                return;
              }
                          ...
                          >
                            {saving
                              ...
                              : agent.heartbeatEnabled
                                ? t("instanceSettings.disableTimerHeartbeat")
                                : t("instanceSettings.enableTimerHeartbeat")}
                          </Button>
                        </span>
                      </div>
                    );

...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.heartbeats.updateFailed"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("error.internal");
        throw new Error(
          failures.length === 1
            ? t("settings.heartbeats.disableOneFailed", { detail })
            : t("settings.heartbeats.disableManyFailed", {
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.heartbeats.disableAllFailed"));
    },
  });
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("settings.heartbeats.loadFailed")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        ...
        <span>
          ...
          {grouped.length === 1 ? t("settings.heartbeats.companySingular") : t("settings.heartbeats.companyPlural")}
        </span>
        {anyEnabled && (
          <Button
            variant="destructive"
            ...
            onClick={() => {
              const noun =
                enabledCount === 1 ? t("settings.heartbeats.agentSingular") : t("settings.heartbeats.agentPlural");
              if (!window.confirm(t("settings.heartbeats.disableConfirm", { count: enabledCount, noun }))) {
                return;
              }
                          ...
                          >
                            {saving
                              ...
                              : agent.heartbeatEnabled
                                ? t("settings.heartbeats.disableTimer")
                                : t("settings.heartbeats.enableTimer")}
                          </Button>
                        </span>
                      </div>
                    );

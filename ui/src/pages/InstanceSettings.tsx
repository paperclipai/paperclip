...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("instanceHeartbeats.breadcrumb.settings", "Instance Settings") },
      { label: t("instanceHeartbeats.breadcrumb.heartbeats", "Heartbeats") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceHeartbeats.error.update", "Failed to update heartbeat."));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        throw new Error(
          failures.length === 1
            ? t(
              "instanceHeartbeats.error.disableSingle",
              "Failed to disable 1 timer heartbeat: {{detail}}",
              { detail },
            )
            : t(
              "instanceHeartbeats.error.disableMultiple",
              "Failed to disable {{failed}} of {{enabled}} timer heartbeats. First error: {{detail}}",
              { failed: failures.length, enabled: enabled.length, detail },
            ),
        );
      }
      return enabled;
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceHeartbeats.error.disableAll", "Failed to disable all heartbeats."));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("instanceHeartbeats.loading", "Loading scheduler heartbeats...")}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("instanceHeartbeats.error.load", "Failed to load scheduler heartbeats.")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instanceHeartbeats.title", "Scheduler Heartbeats")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t(
            "instanceHeartbeats.subtitle",
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("instanceHeartbeats.summary.active", "active")}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("instanceHeartbeats.summary.disabled", "disabled")}</span>
        <span>
          <span className="font-semibold text-foreground">{grouped.length}</span>{" "}
          {grouped.length === 1
            ? t("instanceHeartbeats.summary.company", "company")
            : t("instanceHeartbeats.summary.companies", "companies")}
        </span>
        {anyEnabled && (
          <Button
            variant="destructive"
            ...
            onClick={() => {
              const noun = enabledCount === 1
                ? t("instanceHeartbeats.disableAll.confirm.agent", "agent")
                : t("instanceHeartbeats.disableAll.confirm.agents", "agents");
              if (!window.confirm(t(
                "instanceHeartbeats.disableAll.confirm.message",
                "Disable timer heartbeats for all {{count}} enabled {{noun}}?",
                { count: enabledCount, noun },
              ))) {
                return;
              }
          ...
          >
            {disableAllMutation.isPending
              ? t("instanceHeartbeats.disableAll.pending", "Disabling...")
              : t("instanceHeartbeats.disableAll.action", "Disable All")}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("instanceHeartbeats.empty", "No scheduler heartbeats match the current criteria.")}
        />
      ) : (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive
                            ? t("instanceHeartbeats.badge.on", "On")
                            : t("instanceHeartbeats.badge.off", "Off")}
                        </Badge>
                        <Link
                        ...
                        >
                          {agent.lastHeartbeatAt
                            ...
                            : t("instanceHeartbeats.lastHeartbeat.never", "never")}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            to={buildAgentHref(agent)}
                            className="text-muted-foreground hover:text-foreground"
                            title={t("instanceHeartbeats.agentConfig", "Full agent config")}
                          >
                          ...
                          >
                            {saving
                              ...
                              : agent.heartbeatEnabled
                                ? t("instanceHeartbeats.toggle.disable", "Disable Timer Heartbeat")
                                : t("instanceHeartbeats.toggle.enable", "Enable Timer Heartbeat")}
                          </Button>
                        </span>
                      </div>
                    );

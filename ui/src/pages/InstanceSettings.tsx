...
import { instanceHeartbeatsPage, sidebarNav } from "../lib/i18n";
...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: sidebarNav.instanceHeartbeats },
    ]);
  }, [setBreadcrumbs]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : instanceHeartbeatsPage.failedToUpdateHeartbeat);
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        throw new Error(
          failures.length === 1
            ? instanceHeartbeatsPage.failedToDisableSingle(1, detail)
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : instanceHeartbeatsPage.failedToDisableAll);
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{instanceHeartbeatsPage.loadingHeartbeats}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : instanceHeartbeatsPage.failedToLoadHeartbeats}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{instanceHeartbeatsPage.title}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {instanceHeartbeatsPage.subtitle}
        </p>
      </div>

      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {instanceHeartbeatsPage.active}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {instanceHeartbeatsPage.disabled}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? instanceHeartbeatsPage.company : instanceHeartbeatsPage.companies}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              const noun = enabledCount === 1 ? instanceHeartbeatsPage.company : instanceHeartbeatsPage.companies;
              if (!window.confirm(instanceHeartbeatsPage.confirmDisableAll(enabledCount))) {
                return;
              }
              disableAllMutation.mutate(agents);
            }}
          >
            {disableAllMutation.isPending ? instanceHeartbeatsPage.disabling : instanceHeartbeatsPage.disableAll}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={instanceHeartbeatsPage.noHeartbeats}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? instanceHeartbeatsPage.on : instanceHeartbeatsPage.off}
                        </Badge>
                        <Link
                        ...
                        >
                          {agent.lastHeartbeatAt
                            ...
                            : instanceHeartbeatsPage.never}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            to={buildAgentHref(agent)}
                            className="text-muted-foreground hover:text-foreground"
                            title={instanceHeartbeatsPage.fullAgentConfig}
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? instanceHeartbeatsPage.disableTimerHeartbeat : instanceHeartbeatsPage.enableTimerHeartbeat}
                          </Button>
                        </span>
                      </div>
                    );

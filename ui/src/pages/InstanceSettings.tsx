...
import { agentRoleKo, formatDateTime, isLangKo, relativeTime, relativeTimeKo } from "../lib/utils";
...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("instanceSettings.breadcrumbRoot") },
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.errorUpdateHeartbeat"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("instanceSettings.errorUnknown");
        throw new Error(
          failures.length === 1
            ? t("instanceSettings.errorDisableOne", { detail })
            : t("instanceSettings.errorDisableMany", { failed: failures.length, total: enabled.length, detail }),
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.errorDisableAll"));
    },
  });
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        ...
        {anyEnabled && (
          <Button
            ...
            onClick={() => {
              const confirmMsg = enabledCount === 1
                ? t("instanceSettings.confirmDisableAllOne", { count: enabledCount })
                : t("instanceSettings.confirmDisableAllMany", { count: enabledCount });
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("instanceSettings.emptyState")}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("instanceSettings.statusOn") : t("instanceSettings.statusOff")}
                        </Badge>
                        <Link
                        ...
                        >
                          ...
                          {isLangKo() && agent.lastHeartbeatAt && (
                            <span className="ml-1 text-muted-foreground/70">
                              ({relativeTimeKo(agent.lastHeartbeatAt)})
                            </span>
                          )}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            ...
                            title={t("instanceSettings.fullAgentConfigTitle")}
                          >

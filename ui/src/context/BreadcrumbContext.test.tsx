...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Brabrix Agent", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Brabrix Agent",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Brabrix Agent");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Brabrix Agent");
    expect(buildDocumentTitle([], null)).toBe("Brabrix Agent");
  });
});

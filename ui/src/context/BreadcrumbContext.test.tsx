...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before All Gas No Brakes", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • All Gas No Brakes",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • All Gas No Brakes");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • All Gas No Brakes");
    expect(buildDocumentTitle([], null)).toBe("All Gas No Brakes");
  });
});

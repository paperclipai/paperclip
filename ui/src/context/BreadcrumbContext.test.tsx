...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Odysseus", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Odysseus",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Odysseus");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Odysseus");
    expect(buildDocumentTitle([], null)).toBe("Odysseus");
  });
});

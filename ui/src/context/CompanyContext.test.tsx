...
import type { Company } from "@odysseus/shared";
...
describe("CompanyProvider", () => {
  ...
  it("does not expose a stale stored company id before companies load", async () => {
    localStorage.setItem("odysseus.selectedCompanyId", "stale-company");
  ...
  it("replaces a stale stored company id with the first loaded company", async () => {
    ...
    expect(localStorage.getItem("odysseus.selectedCompanyId")).toBe("company-1");
  });
});

...
## 12. i18n Guardrail (localization branch)

Branch `vib-1171-2652-2760-3582-localization` enforces a pre-push hook that blocks structural TSX changes from sneaking into i18n commits.
...
**What is checked:**
- New `.tsx` files are rejected unless the commit message contains `[stage-8-switcher]` AND the file is under `ui/src/components/LanguageSwitcher/`.
- Modified `.tsx` files: every added line must be `useTranslation` import/destructure or a `t("...")` call.
- New `.css` files: same bypass rule as `.tsx`.

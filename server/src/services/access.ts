...
export function accessService(db: Db) {
  ...
  async function hasProjectPermission(
    ...
    // Role preset fallback: check the principal's role in project_members against PROJECT_ROLE_PRESETS
    if (membership.role) {
      const preset = PROJECT_ROLE_PRESETS.find((p) => p.id === membership.role);
      if (preset?.permissions?.includes(permissionKey)) return true;
    }
    return false;
  }
  ...
  async function canUserAccessProject(
    ...
    if (await isCompanyOwner(project.companyId, userId)) return true;
  ...
  // Agent scope = owner's grants ∩ agent's invites.
  // An agent can access a project only if BOTH (a) its owner can access the
  // project AND (b) the agent is explicitly invited via project_agents.
  async function canAgentAccessProject(agentId: string, projectId: string): Promise<boolean> {
    const agent = await db
      .select({ ownerUserId: agents.ownerUserId })
      .from(agents)
      .where(eq(agents.id, agentId))
      .then((rows) => rows[0] ?? null);
    if (!agent || !agent.ownerUserId) return false;
    const [ownerHasAccess, agentInvited] = await Promise.all([
      canUserAccessProject(projectId, agent.ownerUserId),
      isAgentAssignedToProject(projectId, agentId),
    ]);
    return ownerHasAccess && agentInvited;
  }

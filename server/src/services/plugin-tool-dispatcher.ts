...
export interface PluginToolDispatcher {
   ...
   *
   * @param pluginId - The plugin's manifest key (used for tool namespacing)
   * @param manifest - The plugin manifest containing tool declarations
   * @param pluginDbId - The plugin's DB UUID (used for worker lookup via
   *   `workerManager.isRunning`). If omitted, falls back to `pluginId` —
   *   which breaks agent-side tool invocation because workers are keyed by
   *   DB UUID, not manifest key.
   */
  registerPluginTools(
    pluginId: string,

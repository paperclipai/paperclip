...
/**
 * Naming convention for npm-published AiTeamCorp plugins.
 * Packages matching this pattern are considered AiTeamCorp plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
...
export interface DiscoveredPlugin {
  ...
  manifest: AiTeamCorpPluginManifestV1 | null;
}
...
function getDeclaredPageRoutePaths(manifest: AiTeamCorpPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
   ...
   * Defaults to ~/.aiteamcorp/plugins/
   */
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: AiTeamCorpPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
   ...
   *
   * Returns null if the package is not a AiTeamCorp plugin.
   * Throws if the package is a AiTeamCorp plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<AiTeamCorpPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: AiTeamCorpPluginManifestV1;
    newManifest: AiTeamCorpPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
function getMinimumHostVersion(manifest: AiTeamCorpPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumAiTeamCorpVersion;
}
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: AiTeamCorpPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as AiTeamCorpPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a AiTeamCorp plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<AiTeamCorpPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a AiTeamCorp plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasAiTeamCorpPlugin = "aiteamcorpPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasAiTeamCorpPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<AiTeamCorpPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: AiTeamCorpPluginManifestV1;
      } | null;

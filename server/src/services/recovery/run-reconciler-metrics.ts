...
export function renderRunReconcilerPrometheusMetrics(now = new Date()) {
  ...
  for (const sample of latestSamples) {
    ...
    activeLines.push(`paperclip_run_active{adapter="${adapter}",agent="${agent}",runId="${runId}"} ${sample.active}`);
    ageLines.push(`paperclip_run_last_output_age_seconds{adapter="${adapter}",runId="${runId}"} ${sample.lastOutputAgeSeconds}`);
    pidLines.push(`paperclip_run_child_pid_alive{runId="${runId}"} ${sample.childPidAlive}`);
  }

  const lines = [
    "# HELP paperclip_run_active Whether a heartbeat run is currently active (1) or not (0).",
    "# TYPE paperclip_run_active gauge",
    ...(activeLines.length > 0 ? activeLines : ['paperclip_run_active{adapter="none",agent="none",runId="none"} 0']),

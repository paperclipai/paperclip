...
export function recoveryService(db: Db, deps: { enqueueWakeup: RecoveryWakeup }) {
  ...
  async function scanSilentActiveRuns(opts?: { now?: Date; companyId?: string }) {
    ...
    for (const run of candidates) {
      ...
      // LAM-948/LAM-950: pid-liveness check. When the pid is known, probe it before
      // deciding whether to raise a watchdog issue or auto-recover the run.
      if (typeof run.processPid === "number") {
        const pidAlive = isPidAlive(run.processPid) ||
          isProcessGroupAlive(run.processGroupId ?? undefined);
        if (pidAlive) {
          // LAM-950: emit a positive process.alive signal so the watchdog has a record
          // of the last confirmed-alive timestamp for this run.
          await appendRecoveryRunEvent(run, {
            level: "info",
            message: "process.alive",
            payload: { processPid: run.processPid, processGroupId: run.processGroupId, at: now.toISOString() },
          });
        }
        const pidDead = !pidAlive;
        if (pidDead) {
          const [nonTerminalChild] = await db
            .select({ id: issues.id })
            .from(issues)
            .where(
              and(
          ...
          if (!nonTerminalChild) {
            await db
              ...
              .set({
                ...
                resultJson: {
                  ...
                  zombieAutoRecovered: {
                    reason: "pid_dead_children_terminal",
            ...
            await appendRecoveryRunEvent(run, {
              level: "info",
              message: "Zombie auto-recovered: pid is dead and all spawned issues are terminal",

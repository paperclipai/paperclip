...
const RESTART_LANE_RECOVERY_POLL_INTERVAL_MS = 100;
...
export function recoveryService(db: Db, deps: { enqueueWakeup: RecoveryWakeup }) {
  ...
  async function waitForRestartLaneSuccessorRun(
    candidate: RestartLaneCandidate,
    successorRunId: string,
  ): Promise<{ kind: "recovered"; successorRunId: string } | { kind: "unrecoverable"; reason: string }> {
    const deadline = Date.now() + RESTART_LANE_RECOVERY_BATCH_TIMEOUT_MS;
    while (Date.now() < deadline) {
      const successor = await db
        .select({ status: heartbeatRuns.status, error: heartbeatRuns.error, errorCode: heartbeatRuns.errorCode })
        .from(heartbeatRuns)
        .where(and(
          eq(heartbeatRuns.id, successorRunId),
          eq(heartbeatRuns.companyId, candidate.agent.companyId),
          eq(heartbeatRuns.agentId, candidate.agent.id),
        ))
        .then((rows) => rows[0] ?? null);
      if (!successor) return { kind: "unrecoverable", reason: "successor_run_missing" };
      if (successor.status === "succeeded") return { kind: "recovered", successorRunId };
      if (UNSUCCESSFUL_HEARTBEAT_RUN_TERMINAL_STATUSES.includes(
        successor.status as typeof UNSUCCESSFUL_HEARTBEAT_RUN_TERMINAL_STATUSES[number],
      )) {
        return { kind: "unrecoverable", reason: `successor_run_${successor.status}` };
      }
      await new Promise((resolve) => setTimeout(resolve, RESTART_LANE_RECOVERY_POLL_INTERVAL_MS));
    }
    return { kind: "unrecoverable", reason: "batch_timeout" };
  }
  ...
  async function runRestartLaneRecoverySweep(): Promise<RestartLaneRecoverySweepResult> {
    ...
    for (let offset = 0; offset < candidates.length; offset += RESTART_LANE_RECOVERY_BATCH_SIZE) {
      ...
      const settled = await Promise.all(batch.map((candidate) => {
        ...
        const outcome = await waitForRestartLaneSuccessorRun(candidate, successor.id);
        return outcome.kind === "recovered"
          ? { candidate, attemptCount, ...outcome }
          : { candidate, attemptCount, ...outcome };
        })();
        return Promise.race([
          work,

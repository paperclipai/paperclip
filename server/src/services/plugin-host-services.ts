...
import type { CreateIssueThreadInteraction, IssueDocumentSummary } from "@odysseus/shared";
import { pluginOperationIssueOriginKind } from "@odysseus/shared";
 ...
 *
 * This implementation delegates to the core Odysseus domain services,
...
export function buildHostServices(
  ...
  options: { pluginWorkerManager?: PluginWorkerManager; manifest?: import("@odysseus/shared").OdysseusPluginManifestV1 } = {},
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@odysseus/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {

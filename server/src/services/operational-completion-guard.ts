...
const MISSING_RUNTIME_EVIDENCE_PATTERN =
  /blocked,? not done|disposition:\s*blocked|not done|no (?:green )?report\/sarif|no .*sarif artifact|no phase-\d|failed on .*429|too many requests|rate limit|capacity path|missing runtime evidence|acceptance evidence is still missing/i;
...
export function assessOperationalCompletionEvidenceGate(input: {
  ...
  const completionText = compactText([
    input.completionCommentBody ?? input.recentComments[0]?.body,
  ]);

...
async function createStaleTargetReplacementInteraction(
  ...
  args: {
    row: IssueThreadInteractionRow;
    interaction: TargetBoundInteraction;
    ...
    expireRequestConfirmationsSupersededByComment: async (
      ...
      for (const row of superseded) {
        const [updated] = await db
          .update(issueThreadInteractions)
          .set({
            // Only non-operator-facing kinds reach this loop (request_item_verdicts);
            // upstream labels comment-superseded remainders "expired".
            status: "expired",

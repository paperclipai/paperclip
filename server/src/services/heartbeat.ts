...
// Graduated orphan detection thresholds (30s heartbeat interval baseline)
const HEARTBEAT_WARN_THRESHOLD_MS = 2 * 30_000; // 60s  — 2x interval
const HEARTBEAT_STUCK_THRESHOLD_MS = 5 * 30_000; // 150s — 5x interval
const HEARTBEAT_ORPHANED_THRESHOLD_MS = 10 * 30_000; // 300s — 10x interval
...
export function heartbeatService(db: Db) {
  ...
  async function reapOrphanedRuns(opts?: { staleThresholdMs?: number }) {
    ...
    for (const { run, adapterType } of activeRuns) {
      ...
      // Graduated staleness logging before mark-as-failed
      const staleMs =
        now.getTime() - (run.updatedAt ? new Date(run.updatedAt).getTime() : 0);
      if (staleMs > HEARTBEAT_ORPHANED_THRESHOLD_MS) {
        logger.error(
          { runId: run.id, agentId: run.agentId, staleMs },
          "[orphan-reaper] ORPHANED — run stale >300s, marking failed and flagging for restart",
        );
      } else if (staleMs > HEARTBEAT_STUCK_THRESHOLD_MS) {
        logger.warn(
          { runId: run.id, agentId: run.agentId, staleMs },
          "[orphan-reaper] STUCK — run stale >150s",
        );
      } else if (staleMs > HEARTBEAT_WARN_THRESHOLD_MS) {
        logger.warn(
          { runId: run.id, agentId: run.agentId, staleMs },
          "[orphan-reaper] WARN — run stale >60s",
        );
      }

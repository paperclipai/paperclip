...
async function useEmptyProjectWorkspaceAfterRepoAccessFailure(input: {
  ...
  if (!isProjectWorkspaceRepoAccessError(input.error)) throw input.error;
...
async function repairInvalidProjectWorkspaceCheckout(input: {
  ...
  try {
    result = await materializeManagedProjectWorkspace(input.cwd, {
      repoUrl: input.repoUrl,
      resolveGitAuth: input.resolveGitAuth ?? null,
    });
  } catch (error) {
    result = await useEmptyProjectWorkspaceAfterRepoAccessFailure({
      repoUrl: input.repoUrl,
      cwd: input.cwd,
      label: input.label,
      error,
...
/**
 * Materialize a configured workspace path without accepting a different or
 * non-repository directory. Managed workspaces retain the fork's atomic clone
 * helper below; configured paths use the same credential-aware helper when a
 * clone is needed.
 */
export async function ensureProjectWorkspacePath(input: {
  ...
  if (!stats || (stats.isDirectory() && (await fs.readdir(cwd).catch(() => [] as string[])).length === 0)) {
    try {
      return await materializeManagedProjectWorkspace(cwd, {
        repoUrl,
        resolveGitAuth: input.resolveGitAuth ?? null,
      });
    } catch (error) {
...
export async function ensureManagedProjectWorkspace(input: {
  ...
  const attempt = ensureProjectWorkspacePath({
    cwd,
    repoUrl: input.repoUrl,
...
interface WakeupOptions {
  ...
  contextSnapshot?: Record<string, unknown>;
  /**
   * Explicit operator retry only: reap a non-terminal issue lock holder when
   * it no longer owns a live local process, so the requested retry can claim
   * a fresh execution path instead of coalescing into a ghost run.
   */
...
function isInvokableAgentStatus(status: string) {
...
export function buildExplicitResumeSessionOverride(input: {
  ...
  const runSessionParams = deserializeResumeParamsForCodec(
    input.sessionCodec,
    input.resumeRunSessionParams ?? null,
  );
  const resumeRunSessionParams =
    runSessionParams ??
    sessionParamsFromSessionId(input.sessionCodec, input.resumeRunSessionIdAfter) ??
    sessionParamsFromSessionId(input.sessionCodec, input.resumeRunSessionIdBefore);
  const desiredDisplayId = sessionDisplayIdForParams(input.sessionCodec, resumeRunSessionParams);
  const taskSessionParams = deserializeResumeParamsForCodec(
    input.sessionCodec,
    input.taskSession?.sessionParamsJson ?? null,
  );
  const taskSessionDisplayId = sessionDisplayIdForParams(input.sessionCodec, taskSessionParams);
...
/**
 * Board chat can select a model for exactly the run generated by that message.
 * Keep this defensive even though the issue route validates it: a heartbeat
 * context can also be written by internal callers, and must never override a
 * different agent's configuration or an adapter that lacks a model field.
 */
export function resolveRunScopedChatModelOverride(input: {
...
function validateOwnerBoundIssueMonitorDelivery(
  ...
  const expectedAssigneeAgentId = readNonEmptyString(contextSnapshot?.monitorExpectedAssigneeAgentId);
  ...
  if (!expectedAssigneeAgentId || !expectedStatus) {
    return { valid: false, reason: "Monitor wake is missing its owner/status delivery generation" };
  }
  if (issue.assigneeAgentId !== expectedAssigneeAgentId) {
    return { valid: false, reason: "Monitor wake is stale because issue ownership changed" };
  }
  if (issue.status !== expectedStatus) {
    return { valid: false, reason: "Monitor wake is stale because issue status changed" };
  }
  ...
  if (wakeReason === "issue_monitor_recovery") {
    ...
    return expectedClearedAt &&
      ...
      ? { valid: true }
      : { valid: false, reason: "Monitor recovery wake is stale because the cleared monitor generation changed" };
  }
...
function allowsIssueInteractionWake(
  ...
  const wakeReason = readNonEmptyString(contextSnapshot?.wakeReason);
  // A scheduled monitor is an explicitly bounded interaction with blocked
  // work: it checks the external dependency without reopening the source
  // issue. Its monitor claim metadata is the proof that this is not a normal
  // execution wake trying to bypass unresolved dependencies.
  if (
  ...
  if (
    (wakeReason === "issue_assigned" || wakeReason === "next_owner_handoff") &&
    contextSnapshot?.assignmentHandoff === true
  ) {
    return true;
  }
...
function isTrackedLocalChildProcessAdapter(adapterType: string) {
  try {
    ...
    return Boolean(adapter.sessionCodec || adapter.sessionManagement?.supportsSessionResume);
  } catch {
    return false;
  }
}
...
function buildDetachedProcessStalledMessage(
  ...
  const detail = silenceStartedAt ? ` since ${silenceStartedAt.toISOString()}` : "";
...
function normalizeResumeParamsForCodec(
  codec: AdapterSessionCodec,
  params: Record<string, unknown> | null | undefined,
) {
  const normalized = normalizeSessionParams(params);
  if (!normalized) return null;
  return normalizeSessionParams(codec.serialize(normalized));
}

function deserializeResumeParamsForCodec(codec: AdapterSessionCodec, raw: unknown) {
  const deserialized = normalizeSessionParams(codec.deserialize(raw));
  return deserialized ? normalizeResumeParamsForCodec(codec, deserialized) : null;
}

function sessionParamsFromSessionId(codec: AdapterSessionCodec, sessionId: string | null | undefined) {
  const normalizedSessionId = readNonEmptyString(sessionId);
  return normalizedSessionId
    ? deserializeResumeParamsForCodec(codec, { sessionId: normalizedSessionId })
    : null;
}

function sessionDisplayIdForParams(codec: AdapterSessionCodec, params: Record<string, unknown> | null | undefined) {
  const normalized = normalizeSessionParams(params);
  return truncateDisplayId(
    readNonEmptyString(normalized?.sessionId) ??
      (codec.getDisplayId ? codec.getDisplayId(normalized) : null),
  );
}

function readRunResultSessionParams(resultJson: Record<string, unknown>) {
  const explicitParams = parseObject(resultJson.sessionParams ?? resultJson.session_params);
  if (Object.keys(explicitParams).length > 0) return explicitParams;
  const sessionId = readNonEmptyString(resultJson.sessionId) ?? readNonEmptyString(resultJson.session_id);
...
export function resolveNextSessionState(input: {
  ...
  const previousSerializedParams = deserializeResumeParamsForCodec(codec, previousParams);
  const previousDisplayParams = sessionParamsFromSessionId(codec, previousDisplayId);
  const previousLegacyParams = sessionParamsFromSessionId(codec, previousLegacySessionId);
  const previousState = () => {
    const params = previousSerializedParams ?? previousDisplayParams ?? previousLegacyParams;
    const displayId = sessionDisplayIdForParams(codec, params);
    return {
      params,
      displayId,
      legacySessionId: readNonEmptyString(params?.sessionId) ?? displayId,
    };
  };

  const hasExplicitParams = adapterResult.sessionParams !== undefined;
  const hasExplicitSessionId = adapterResult.sessionId !== undefined;
  const hasExplicitDisplayId = adapterResult.sessionDisplayId !== undefined;
  if (!hasExplicitParams && !hasExplicitSessionId && !hasExplicitDisplayId) return previousState();

  const explicitParams = hasExplicitParams
    ? deserializeResumeParamsForCodec(codec, adapterResult.sessionParams)
    : null;
  const explicitSessionParams = hasExplicitSessionId
    ? sessionParamsFromSessionId(codec, adapterResult.sessionId)
    : null;
  const explicitDisplayParams = hasExplicitDisplayId
    ? sessionParamsFromSessionId(codec, adapterResult.sessionDisplayId)
    : null;
  const params = explicitParams ?? explicitSessionParams ?? explicitDisplayParams;

  if (!params) return previousState();

  const previous = previousState();
  if (input.outcome !== "succeeded" && previous.params) return previous;

  const displayId = sessionDisplayIdForParams(codec, params);
  const legacySessionId = readNonEmptyString(params.sessionId) ?? displayId;
...
export interface HeartbeatServiceOptions {
  ...
  /** Test/diagnostic seam after a monitor source is cleared before recovery. */
...
export function heartbeatService(db: Db, options: HeartbeatServiceOptions = {}) {
  ...
  async function resolveResponsibleUserIdForRun(input: {
    ...
    // Legacy/system-owned companies can legitimately have no human
    // responsible user. Keep the value nullable here; runtime secret
    // resolution enforces a responsible user only when a user-scoped secret
    // binding actually requires one.
  ...
  async function dispatchClaimedIssueMonitor(
    ...
    // A monitor claim is advisory until the exact claim token is consumed.
    // Re-read it before admitting work so a board reassignment/reschedule that
    // happens after the scheduler claim cannot be overwritten by the old
    // monitor generation.
    const claimToken = claimed.monitorWakeRequestedAt;
    if (!claimToken || !claimed.monitorNextCheckAt || !claimed.assigneeAgentId) {
      return { outcome: "skipped" as const, reason: "issue_monitor_claim_stale" };
    }
    const currentClaim = await db
      ...
      .where(and(
        ...
        eq(issues.status, claimed.status),
        eq(issues.monitorNextCheckAt, claimed.monitorNextCheckAt),
        eq(issues.monitorWakeRequestedAt, claimToken),
      ))
      .limit(1)
      .then((rows) => rows[0] ?? null);
    if (!currentClaim) return { outcome: "skipped" as const, reason: "issue_monitor_claim_stale" };
    ...
    const recordSkippedDispatch = async (reason: string) => {
      const permanentlyUnavailable =
        ...
        /\b(?:not invokable|paused|terminated|pending approval)\b/i.test(reason);
      if (permanentlyUnavailable || !monitor) {
      ...
      const updated = await db
        .update(issues)
        .set({ ...skippedPatch, updatedAt: input.now })
        .where(and(
          eq(issues.id, claimed.id),
          eq(issues.companyId, claimed.companyId),
          eq(issues.assigneeAgentId, claimed.assigneeAgentId!),
    ...
    try {
      const monitorIdempotencyKey = `issue-monitor:${claimed.id}:${scheduledAtIso}`;
      const delivery = await enqueueWakeupWithDisposition(targetAgentId, {
        source: input.source,
        triggerDetail: input.triggerDetail,
        reason: wakeReason,
        idempotencyKey: monitorIdempotencyKey,
        payload: {
          issueId: claimed.id,
          nextCheckAt: scheduledAtIso,
          monitorAttemptCount: nextAttemptCount,
      ...
      if (delivery.kind === "skipped") {
        return recordSkippedDispatch(delivery.reason);
      }
      ...
      await logActivity(db, {
        ...
        details: {
          ...
          ...monitorMetadata,
          source: input.activitySource,
          deliveryDisposition: delivery.kind,
          deliveryRunId: delivery.run?.id ?? null,
          wakeupRequestId: delivery.wakeupRequestId,
        },
      });
  ...
  async function tickDueIssueMonitors(now = new Date()) {
    ...
    const dueMonitors = await db
      .select(issueMonitorDispatchColumns)
      .from(issues)
      // Paused companies retain their monitor generation so they can be
      // re-armed when operations resume. Archived companies are terminal and
      // must never receive a scheduler wake.
    ...
    for (const due of dueMonitors) {
          ...
          .where(
            and(
              ...
              sql`exists (
                select 1
                from ${companies}
                where ${companies.id} = ${issues.companyId}
      ...
      try {
        ...
        if (result.outcome === "triggered") {
          triggered += 1;
          const deliveryDisposition = result.deliveryDisposition;
          if (
            deliveryDisposition === "queued" ||
            deliveryDisposition === "coalesced" ||
            deliveryDisposition === "deferred"
          ) {
            deliveries[deliveryDisposition] += 1;
          }
        }
  ...
  async function hasResolvablePriorSessionWorkspaceForWake(input: {
    ...
    const taskSessionParams = deserializeResumeParamsForCodec(codec, taskSession?.sessionParamsJson ?? null);
  ...
  async function resolveExplicitResumeSessionOverride(
    ...
    const sessionOverride = buildExplicitResumeSessionOverride({
      ...
      resumeRunSessionParams: readRunResultSessionParams(resumeRunResult),
    ...
    if (projectWorkspaceRows.length > 0) {
      ...
      for (const workspace of projectWorkspaceRows) {
        ...
        try {
          ...
          const configuredCwd = readNonEmptyString(workspace.cwd);
          if (configuredCwd && isProjectWorkspaceFilesystemPermissionError(error)) {
            try {
              ...
              return {
                ...
                warnings: [
                  preferredWorkspaceWarning,
                  `Configured project workspace path "${configuredCwd}" could not be created (${scrubbedError}). Using managed project workspace "${managedWorkspace.cwd}".`,
              ...
              const managedMessage = scrubGitCredentialText(
  ...
  async function evaluateQueuedRunStaleness(
    ...
    const wakeReason = readNonEmptyString(context.wakeReason);
    const isCommentWake = Boolean(wakeCommentId) && (
    ...
    for (const { run, adapterType, adapterConfig } of activeRuns) {
      ...
      if (processPidAlive) {
        ...
        if (
          ...
          // The stalled detached process has already been terminalized by this
          // reaper. Do not let the generic release path create a second retry.
  ...
  async function executeRun(runId: string) {
    ...
    const taskSession = taskKey
      ...
      : null;
    const taskSessionDecodedParams = deserializeResumeParamsForCodec(
      sessionCodec,
      taskSession?.sessionParamsJson ?? null,
    );
    const explicitResumeSessionParams = deserializeResumeParamsForCodec(
      sessionCodec,
      parseObject(context.resumeSessionParams),
    );
    ...
    const chatModelOverride = resolveRunScopedChatModelOverride({
      adapterType: agent.adapterType,
      agentId: agent.id,
      issueAssigneeAgentId: issueContext?.assigneeAgentId,
      contextSnapshot: context,
    ...
    if (chatModelOverride) {
      // This is intentionally applied after the task override: the board's
      // explicit chat choice controls this one response, but it is only stored
      // on the run context and never written back to the issue or agent.
      mergedConfig.model = chatModelOverride;
    ...
    // Expand company-catalog MCP references; inline agent definitions take
    // precedence and an expansion failure must not block the heartbeat.
    try {
    ...
    // before runtime-secret resolution so the adapter receives a fresh token.
    ...
    const previousSessionParams =
      ...
      sessionParamsFromSessionId(sessionCodec, explicitResumeSessionDisplayId) ??
      normalizeResumeParamsForCodec(
        sessionCodec,
    ...
    const runtimeSessionFallbackParams = taskKey || resetTaskSession
      ? null
      : sessionParamsFromSessionId(sessionCodec, runtime.sessionId);
    const runtimeSessionFallback =
      readNonEmptyString(runtimeSessionFallbackParams?.sessionId) ??
      sessionDisplayIdForParams(sessionCodec, runtimeSessionFallbackParams);
    let runtimeSessionParamsForAdapter = normalizeResumeParamsForCodec(
      sessionCodec,
      stripPaperclipSessionMetadataFromSessionParams(runtimeSessionParams),
    );
    const runtimeSessionDisplayId =
      explicitResumeSessionDisplayId ??
      sessionDisplayIdForParams(sessionCodec, previousSessionParams) ??
      sessionDisplayIdForParams(sessionCodec, runtimeSessionParamsForAdapter) ??
      runtimeSessionFallback;
    let previousSessionDisplayId = runtimeSessionDisplayId;
    let runtimeSessionIdForAdapter =
      readNonEmptyString(runtimeSessionParamsForAdapter?.sessionId) ?? runtimeSessionFallback;
    ...
    const promotionResult = await db.transaction(async (tx) => {
      ...
      while (true) {
        ...
        if (isOwnerBoundIssueMonitorWake(deferredContextSeed)) {
          const monitorDelivery = validateOwnerBoundIssueMonitorDelivery(issue, deferredContextSeed);
          if (!monitorDelivery.valid) {
            await tx
              ...
              .set({
                status: "cancelled",
                finishedAt: new Date(),
                error: `Deferred monitor wake cancelled because its generation changed: ${monitorDelivery.reason}`,
                updatedAt: new Date(),
              })
              .where(eq(agentWakeupRequests.id, deferred.id));
            continue;
  ...
  async function enqueueWakeup(agentId: string, opts: WakeupOptions = {}) {
    ...
    const resolveQueuedResponsibleUserId = () => {
      queuedResponsibleUserIdPromise ??= (async () => {
        ...
        // A system assignment handoff starts a normal issue execution path but
        // does not itself represent a human's credential-bearing action. Older
        // system-owned companies may therefore have no responsible user yet.
        // Keep that narrow path nullable; all other dispatches fail before a
        // run is persisted when no accountable user can be resolved.
        const isOwnerlessSystemAssignmentHandoff =
          source === "assignment" &&
          triggerDetail === "system" &&
          enrichedContextSnapshot.assignmentHandoff === true;
        // `heartbeat.invoke()` is a legacy server-internal seam whose callers
        // predate responsible-user attribution. API routes use `wakeup()` and
        // always supply an actor, so retaining this narrow actor-less fallback
        // does not make an external user-scoped dispatch unaccountable.
        const isOwnerlessLegacyServiceInvoke =
          opts.requestedByActorType == null && opts.requestedByActorId == null;
        if (
          !queuedResponsibleUserId &&
          !isOwnerlessSystemAssignmentHandoff &&
          !isOwnerlessLegacyServiceInvoke
        ) {
          throw unprocessable("Unable to resolve responsible user for heartbeat run dispatch", {
    ...
    if (issueId) {
      ...
      const outcome = await db.transaction(async (tx) => {
        ...
        // Manual Retry can clear only a genuinely orphaned execution owner.
        // A live process remains the sole owner, so a retry cannot create a
        // second cooldown/retry chain for the same issue.
        if (
          activeExecutionRun &&
          opts.forceClearStaleExecution &&
          !runningProcesses.has(activeExecutionRun.id) &&
          !isProcessAlive(activeExecutionRun.processPid) &&
          !(activeExecutionRun.processGroupId && isProcessGroupAlive(activeExecutionRun.processGroupId))
        ) {
          const now = new Date();
        ...
        if (activeExecutionRun) {
          ...
          const incomingIsGenerationBound =
            ...
            isSourceScopedRecoveryWake(enrichedContextSnapshot);
          ...
          const shouldQueueFollowupForRunningWake =
            ...
            (incomingIsGenerationBound && isSameExecutionAgent);
          ...
          if (activeExecutionRun) {
            ...
            const deferredCandidates = await tx
              ...
              .orderBy(asc(agentWakeupRequests.requestedAt))
              .limit(50);
            // Monitor and recovery signals are generation-bound. Never merge
            // them into a human follow-up (or vice versa): the stale monitor
            // can then be cancelled independently when its schedule changes.
            const existingDeferred = incomingIsGenerationBound
              ...
              : deferredCandidates.find((candidate) => {
                  ...
                  const candidateContext = parseObject(candidatePayload[DEFERRED_WAKE_CONTEXT_KEY]);
  ...
  /**
   * Preserve the public run-or-null wake API while exposing the durable
   * request disposition to scheduler/recovery callers. The supplied
   * idempotency key identifies exactly one wake request, including a request
   * that completed before the asynchronous dispatch function returns.
   */
  async function enqueueWakeupWithDisposition(
    agentId: string,
    opts: WakeupOptions = {},
  ): Promise<WakeupEnqueueDisposition> {
    const run = await enqueueWakeup(agentId, opts);
    const request = opts.idempotencyKey
      ? await db
        .select({
        ...
        .then((rows) => rows[0] ?? null)
      : null;
    const resolvedRun = run ?? (request?.runId
      ? await db
        .select()
        .from(heartbeatRuns)
        .where(eq(heartbeatRuns.id, request.runId))
        .then((rows) => rows[0] ?? null)
      : null);
    ...
    if (request?.status === "coalesced" && resolvedRun) {
      return {
        kind: "coalesced",
        run: resolvedRun,
        wakeupRequestId: request.id,
    ...
    if (resolvedRun) {
      return {
        kind: "queued",
        run: resolvedRun,
        wakeupRequestId: request?.id ?? resolvedRun.wakeupRequestId ?? null,
        reason: null,
      };
    }
    return {
      kind: "skipped",
      run: null,
      wakeupRequestId: request?.id ?? null,
      reason: request?.reason ?? "Wake was not admitted",
    };
  }
  ...
  async function cancelRunInternal(runId: string, reason = "Cancelled by control plane", options: CancelRunOptions = {}) {
    ...
    // Terminalize before sending SIGTERM. The adapter completion path can race
    // the grace interval; seeing a terminal cancellation prevents it from
    // recasting this deliberate stop as a failure and scheduling another retry.
  ...
  return {
    ...
    tickTimers: async (now = new Date()) => {
      ...
      for (const agent of allAgents) {
        ...
        // Generic timer wakes carry no issue-specific signal, so they must not
        // compete with a live/queued execution path for the same agent.
      ...
      return {
        ...
        // Preserve the delivery disposition separately from the aggregate
        // enqueue count. Consumers use it to distinguish a fresh monitor wake
        // from a coalesced/deferred monitor without inferring from run rows.
    ...
    cancelRun: (
      runId: string,
      reasonOrOptions?: string | (CancelRunOptions & { reason?: string }),
      explicitOptions?: CancelRunOptions,
    ) => {
      const reason =
        typeof reasonOrOptions === "string"
          ? reasonOrOptions
          : reasonOrOptions?.reason ?? "Cancelled by control plane";
      const options =
        typeof reasonOrOptions === "string"
          ? explicitOptions ?? {}
          : reasonOrOptions ?? {};
      return cancelRunInternal(runId, reason, {
        ...options,
        // An operator cancellation is a durable control-plane decision. A new
        // execution path must be explicitly requested by a later wake/retry.
    ...
    cancelActiveForAgent: (agentId: string, reason?: string, options?: { force?: boolean }) =>
      cancelActiveForAgentInternal(agentId, reason, "agent_paused", options),

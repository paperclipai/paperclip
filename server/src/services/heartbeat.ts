...
export function heartbeatService(db: Db) {
  ...
  async function releaseIssueExecutionAndPromote(run: typeof heartbeatRuns.$inferSelect) {
    ...
    type PromotionResult =
      | null
      | { kind: "released"; run: null; skippedTerminalWakes: TerminalSkipEntry[] }
      | {
          kind: "blocked";
          run: null;
          skippedTerminalWakes: TerminalSkipEntry[];
          issueId: string;
      ...
      | { kind: "promoted"; run: typeof heartbeatRuns.$inferSelect; skippedTerminalWakes: TerminalSkipEntry[] };
    ...
    const promotionResult: PromotionResult = await db.transaction(async (tx): Promise<PromotionResult> => {
      ...
      if (!issueNeedsImmediateRecovery) {
        return { kind: "released" as const, run: null, skippedTerminalWakes: terminalSkipWakes };
      }

...
// Routes and the scheduler construct separate heartbeatService instances, but they must agree on
// in-process adapter executions when reaping stale runs — so this is module-scoped, shared across
// instances rather than per-instance (fixes false process_lost on quiet long runs). (upstream #8776)

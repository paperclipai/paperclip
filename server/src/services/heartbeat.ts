...
export function heartbeatService(db: Db, options: HeartbeatServiceOptions = {}) {
  ...
  const externallyCancelledRunIds = new Set<string>();
  const budgetHooks = {
  ...
  async function executeRun(runId: string) {
    ...
    try {
      ...
      const externallyCancelled = externallyCancelledRunIds.has(run.id);
      const latestRunAfterAdapter = externallyCancelled
        ? await getRun(run.id).catch(() => null)
        : await getRun(run.id);
      if (externallyCancelled || latestRunAfterAdapter?.status === "cancelled") {
        try {
          ...
          if (latestRunAfterAdapter) {
            await db
              ...
              .where(eq(heartbeatRuns.id, run.id));
            await appendRunEvent(latestRunAfterAdapter, seq++, {
              eventType: "lifecycle",
              stream: "system",
              level: "warn",
              message: "run cancellation observed after adapter returned",
            });
          }
        } catch (err) {
          logger.warn({ err, runId: run.id }, "skipped late cancelled-run bookkeeping after adapter returned");
        }
        return;
      }
      ...
      if (agent.adapterType === "claude_local") {
        ...
        if (claudeAdapter.getQuotaWindows) {
          checkAndFireClaudeLocalQuotaAlert(db, agent.companyId, claudeAdapter.getQuotaWindows.bind(claudeAdapter)).catch(
            (err) => {
      ...
      await finalizeAgentStatus(agent.id, "failed");
          ...
          const skipPostCancelDbCleanup = externallyCancelledRunIds.delete(run.id);
          activeRunExecutions.delete(run.id);
          if (skipPostCancelDbCleanup) return;
          ...
          await releaseEnvironmentLeasesForRun({
            ...
            logger.warn({ err, runId: run.id }, "failed to release environment leases after heartbeat execution");
          });
          await releaseRuntimeServicesForRun(run.id).catch(() => undefined);
          await startNextQueuedRunForAgent(run.agentId).catch((err) => {
            logger.warn({ err, runId: run.id, agentId: run.agentId }, "failed to start next queued heartbeat run");
          });
        }
  }
  ...
  async function cancelRunInternal(runId: string, reason = "Cancelled by control plane") {
    ...
    if (activeRunExecutions.has(run.id)) {
      externallyCancelledRunIds.add(run.id);
    }
    ...
    if (cancelled) {
      ...
      await releaseEnvironmentLeasesForRun({
        ...
        logger.warn({ err, runId: cancelled.id }, "failed to release environment leases during run cancellation");
      });
      await releaseRuntimeServicesForRun(cancelled.id).catch(() => undefined);
    }

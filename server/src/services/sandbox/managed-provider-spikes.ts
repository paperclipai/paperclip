...
/**
 * LET-391 Phase 4A-S4: caller-facing reason on a billing-cap-side block. The
 * three-gate `evaluateLiveGate` (env/layer1/secret) returns
 * `gate: "env_flag" | "layer1_disabled" | "secret_unresolved"` in
 * PROVIDER_DISABLED details; the billing-cap-side reasons are reported under
 * `gate: "billing_cap_layers"` with this discriminator so the caller can
 * distinguish a hard-cap auto-disable from a manual operator pause.
 */
export type E2BBillingCapBlockReason = "billing-cap-monitor" | "operator-toggle";

export const E2B_BILLING_CAP_BLOCK_REASONS: readonly E2BBillingCapBlockReason[] = Object.freeze([
  "billing-cap-monitor",
  "operator-toggle",
]);

export interface E2BBillingCapLayerProbeInput {
  /** Lease scope. Production wiring resolves companyId from issueId (preferred)
   *  or heartbeatRunId; the integration test provides the snapshot directly. */
  issueId: string | null;
  heartbeatRunId: string;
  environmentId: string;
}

export interface E2BBillingCapLayerSnapshot {
  /** Persisted `sandbox_billing_cap_state.providerEnableLayerEnabled`. False
   *  when the cap monitor auto-disabled the provider after a hard-cap breach. */
  providerEnableLayerEnabled: boolean;
  /** Persisted `sandbox_billing_cap_state.operatorToggleEnabled`. False when an
   *  operator manually paused live acquisitions. */
  operatorToggleEnabled: boolean;
  /** Optional human-readable reason captured at the moment the layer flipped. */
...
export interface E2BSandboxProviderLiveDependencies {
  ...
  /**
   * LET-391: layer-1 read-side enforcement that reads
   * `sandbox_billing_cap_state` for the company × provider key written by the
   * LET-367 cap monitor. Returning `null` is treated as "no row yet, allow"
   * (matches the cap monitor's bootstrap default where no spend has been
   * polled). Returning a snapshot with either layer flag `false` causes
   * `acquireLease` to throw PROVIDER_DISABLED BEFORE any HTTP egress, before
   * the secret store is consulted, and before the live transport is
   * constructed. Errors thrown by the resolver fail closed.
   */
...
export class E2BSandboxProvider extends BaseManagedSandboxProvider {
  ...
  private readonly resolveBillingCapLayers:
    | ((input: E2BBillingCapLayerProbeInput) => Promise<E2BBillingCapLayerSnapshot | null>)
    | null;
  ...
  constructor(options: E2BSandboxProviderOptions = {}) {
    ...
    this.resolveBillingCapLayers = options.resolveBillingCapLayers ?? null;
  ...
  /**
   * LET-391 Phase 4A-S4 read-side enforcement of the billing-cap kill switch.
   *
   * The cap monitor (LET-367) persists `providerEnableLayerEnabled=false` on a
   * hard-cap breach and `operatorToggleEnabled=false` when an operator pauses
   * the provider. Concurrent live lease acquisitions must read this state and
   * fail closed — otherwise the monitor flips the switch but a racing lease
   * still spends.
   *
   * Behaviour:
   *  - No resolver wired → no-op (preserves test/dev defaults that never
   *    needed billing-cap state, and keeps the LET-366 three-gate semantics
   *    intact for the existing test matrix).
   *  - Resolver returns `null` → allow (no cap row yet for this company ×
   *    provider key; matches the monitor's bootstrap default before any
   *    counter tick has hit).
   *  - `providerEnableLayerEnabled === false` → PROVIDER_DISABLED with
   *    `reason: "billing-cap-monitor"`.
   *  - `operatorToggleEnabled === false` → PROVIDER_DISABLED with
   *    `reason: "operator-toggle"`.
   *  - Resolver throws → PROVIDER_DISABLED (fail closed; the cap-state DB
   *    being unreachable is treated as a paused-provider signal, not a
   *    pass-through, because spending under an unknown cap state is the
   *    failure mode the integration is meant to prevent).
   *
   * The check runs on every `acquireLease`, BEFORE `ensureActiveTransport`,
   * so a flipped layer takes effect on the very next acquisition even when
   * the live transport (and resolved API key) is already initialised.
   */
  private async evaluateBillingCapLayerGate(input: AcquireSandboxLeaseInput): Promise<void> {
    if (this.testTransportInjected) return;
    if (!this.resolveBillingCapLayers) return;
    let snapshot: E2BBillingCapLayerSnapshot | null;
    try {
      snapshot = await this.resolveBillingCapLayers({
        issueId: input.issueId,
        heartbeatRunId: input.heartbeatRunId,
        environmentId: input.environmentId,
      });
      ...
      throw this.providerDisabledError(
        "Billing-cap layer state could not be read; failing closed to avoid live spend under unknown cap state.",
        {
    ...
    if (!snapshot) return;
    if (snapshot.providerEnableLayerEnabled === false) {
      throw this.providerDisabledError(
        snapshot.providerEnableReason ??
          "E2B live provider is auto-disabled by the billing-cap monitor (hard cap breached).",
        {
          gate: "billing_cap_layers",
          reason: "billing-cap-monitor",
          providerEnableLayerEnabled: false,
          operatorToggleEnabled: snapshot.operatorToggleEnabled,
          providerEnableReason: snapshot.providerEnableReason ?? null,
        },
      );
    }
    if (snapshot.operatorToggleEnabled === false) {
      throw this.providerDisabledError(
        snapshot.operatorToggleReason ??
          "E2B live provider is paused by the operator-toggle kill switch.",
        {
          gate: "billing_cap_layers",
          reason: "operator-toggle",
          providerEnableLayerEnabled: snapshot.providerEnableLayerEnabled,
          operatorToggleEnabled: false,
          operatorToggleReason: snapshot.operatorToggleReason ?? null,
        },
      );
    }
  }
  ...
  override async acquireLease(input: AcquireSandboxLeaseInput): Promise<SandboxLeaseHandle> {
    // LET-391 read-side enforcement: check the persisted billing-cap layer
    // state BEFORE any HTTP egress, before the secret store is consulted,
    // and before the live transport is constructed. A hard-cap breach or a
    // tripped operator toggle must block subsequent live leases even when
    // all three LET-366 gates pass.
    await this.evaluateBillingCapLayerGate(input);

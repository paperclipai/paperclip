...
import { readOdysseusSkillSyncPreference } from "@odysseus/adapter-utils/server-utils";
import { claudeConfigDir, parseClaudeStreamJson } from "@odysseus/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@odysseus/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@odysseus/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveOdysseusInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "odysseus-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "odysseus-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "odysseus-feedback-v1";
const FEEDBACK_DESTINATION = "odysseus_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "odysseus_run" &&
    file.source !== "odysseus_run_events" &&
    file.source !== "odysseus_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.ODYSSEUS_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readOdysseusSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let odysseusRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      odysseusRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "odysseus/run.json",
        contentType: "application/json",
        source: "odysseus_run",
        contents: `${JSON.stringify(odysseusRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.odysseusRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "odysseus/run-events.json",
        contentType: "application/json",
        source: "odysseus_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "odysseus/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "odysseus_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.odysseusRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {

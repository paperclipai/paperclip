...
import { readValadrienOsSkillSyncPreference } from "@valadrien-os/adapter-utils/server-utils";
import { claudeConfigDir, parseClaudeStreamJson } from "@valadrien-os/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@valadrien-os/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@valadrien-os/adapter-opencode-local/server";
import {
...
const FEEDBACK_PAYLOAD_VERSION = "valadrien-os-feedback-v1";
const FEEDBACK_DESTINATION = "valadrien_os_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "valadrien_os_run" &&
    file.source !== "valadrien_os_run_events" &&
    file.source !== "valadrien_os_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.VALADRIEN_OS_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readValadrienOsSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let valadrienOsRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      valadrienOsRun = sanitizeFeedbackValue(
        {
          id: run.id,
        ...
        state,
        "bundle.valadrienOsRun",
        MAX_TRACE_FILE_CHARS,
      ) as Record<string, unknown>;

      files.push(makeBundleFile({
        path: "valadrien-os/run.json",
        contentType: "application/json",
        source: "valadrien_os_run",
        contents: `${JSON.stringify(valadrienOsRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.valadrienOsRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "valadrien-os/run-events.json",
        contentType: "application/json",
        source: "valadrien_os_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "valadrien-os/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "valadrien_os_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.valadrienOsRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {

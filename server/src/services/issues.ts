...
export function issueService(db: Db) {
  ...
  return {
    ...
    update: async (
      ...
      let receiptBaseline: typeof issues.$inferSelect | null = null;
      let previousLabelsByIssueId = new Map<string, IssueLabelRow[]>();
      ...
      if (
        ...
        patch.executionLockedAt = null;
          ...
          // The receipt baseline must be captured under the same row lock as
          // the write (runIssueMutation reads `existing` FOR UPDATE), before
          // label/relation syncs mutate the associated rows.
          receiptBaseline = existing;
          const [labelBaseline, relationBaseline] = await Promise.all([
          ...
          previousLabelsByIssueId = labelBaseline;
          previousRelationSummaries = relationBaseline;
      ...
      const changes = buildIssueChanges(
        (receiptBaseline ?? {}) as unknown as Record<string, unknown>,

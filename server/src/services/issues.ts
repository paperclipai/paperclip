...
export function issueService(db: Db) {
  ...
  >(comments: readonly T[]) {
    ...
    // Pass 2: only for comments still unresolved AND with >1 overlapping run
    // (where the window tier is ambiguous) do we read run logs to look for the
    // explicit `comment id:` post marker.

...
export function issueService(db: Db) {
  ...
  // Release the checkout lock when its checkoutRunId points at a terminal (or
  // missing) heartbeat run. Deliberately clears ONLY checkoutRunId and never the
  // execution lock cols: a review checkout must not displace a still-executing
  // lane (TSMC-10048). The paired clearExecutionRunIfTerminal — always invoked
  // first by callers — owns releasing a terminal execution lock.

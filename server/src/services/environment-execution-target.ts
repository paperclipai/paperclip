...
export async function resolveEnvironmentExecutionTarget(input: {
  ...
  // Preserve cancellation through the provider runner so user-initiated aborts
  // stop the underlying sandbox command, not merely its caller.
  signal?: AbortSignal;
              ...
              try {
                ...
                try {
                  result = await input.environmentRuntime!.execute({
                    ...
                    // Keep the fork's launch governance and cancellation
                    // propagation intact while retaining upstream streaming,
                    // session selection, and observability behavior.

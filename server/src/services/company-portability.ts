...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".aiteamcorp.yaml" || normalized === ".aiteamcorp.yml") return "extension";
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .aiteamcorp.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .aiteamcorp.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .aiteamcorp.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .aiteamcorp.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .aiteamcorp.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .aiteamcorp.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .aiteamcorp.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .aiteamcorp.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .aiteamcorp.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .aiteamcorp.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .aiteamcorp.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .aiteamcorp.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .aiteamcorp.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[aiteamcorpExtensionPath];
  if (selectedFiles.has(aiteamcorpExtensionPath) && typeof extensionEntry === "string") {
    filtered[aiteamcorpExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }
...
function findAiTeamCorpExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".aiteamcorp.yaml"] === "string") return ".aiteamcorp.yaml";
  if (typeof files[".aiteamcorp.yml"] === "string") return ".aiteamcorp.yml";
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "aiteamcorp_bundled") {
    ...
    return {
      ...
      commit,
      trackingRef: "master",
      url: `https://github.com/aiteamcorporated-collab/ai-team-coprorated/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "aiteamcorp_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.aiteamcorpSkillKey = skill.key;
  metadata.aiteamcorp = {
    ...(isPlainRecord(metadata.aiteamcorp) ? metadata.aiteamcorp : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const aiteamcorpExtension = aiteamcorpExtensionPath
    ...
    : {};
  const aiteamcorpCompany = isPlainRecord(aiteamcorpExtension.company) ? aiteamcorpExtension.company : {};
  const aiteamcorpSidebar = normalizePortableSidebarOrder(aiteamcorpExtension.sidebar);
  const aiteamcorpAgents = isPlainRecord(aiteamcorpExtension.agents) ? aiteamcorpExtension.agents : {};
  const aiteamcorpProjects = isPlainRecord(aiteamcorpExtension.projects) ? aiteamcorpExtension.projects : {};
  const aiteamcorpTasks = isPlainRecord(aiteamcorpExtension.tasks) ? aiteamcorpExtension.tasks : {};
  const aiteamcorpRoutines = isPlainRecord(aiteamcorpExtension.routines) ? aiteamcorpExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(aiteamcorpCompany.brandColor),
      logoPath: asString(aiteamcorpCompany.logoPath) ?? asString(aiteamcorpCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof aiteamcorpCompany.requireBoardApprovalForNewAgents === "boolean"
          ? aiteamcorpCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof aiteamcorpCompany.feedbackDataSharingEnabled === "boolean"
          ? aiteamcorpCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof aiteamcorpCompany.feedbackDataSharingConsentAt === "string"
          ? aiteamcorpCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(aiteamcorpCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(aiteamcorpCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: aiteamcorpSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(aiteamcorpAgents[slug]) ? aiteamcorpAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(aiteamcorpProjects[slug]) ? aiteamcorpProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(aiteamcorpTasks[slug]) ? aiteamcorpTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(aiteamcorpRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(aiteamcorpRoutines[slug]) ? aiteamcorpRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".aiteamcorp.yaml" ||
          relative === ".aiteamcorp.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const aiteamcorpAgentsOut: Record<string, Record<string, unknown>> = {};
    const aiteamcorpProjectsOut: Record<string, Record<string, unknown>> = {};
    const aiteamcorpTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const aiteamcorpRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        aiteamcorpAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      aiteamcorpProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      aiteamcorpTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      aiteamcorpRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const aiteamcorpExtensionPath = ".aiteamcorp.yaml";
    const aiteamcorpAgents = Object.fromEntries(
      Object.entries(aiteamcorpAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const aiteamcorpProjects = Object.fromEntries(
      Object.entries(aiteamcorpProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const aiteamcorpTasks = Object.fromEntries(
      Object.entries(aiteamcorpTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const aiteamcorpRoutines = Object.fromEntries(
      Object.entries(aiteamcorpRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[aiteamcorpExtensionPath] = buildYamlFile(
      {
        ...
        agents: Object.keys(aiteamcorpAgents).length > 0 ? aiteamcorpAgents : undefined,
        projects: Object.keys(aiteamcorpProjects).length > 0 ? aiteamcorpProjects : undefined,
        tasks: Object.keys(aiteamcorpTasks).length > 0 ? aiteamcorpTasks : undefined,
        routines: Object.keys(aiteamcorpRoutines).length > 0 ? aiteamcorpRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, aiteamcorpExtensionPath);

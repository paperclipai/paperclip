...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  async function prepareImportedAgentAdapter(
    ...
    const normalizedAdapterConfig = await secrets.normalizeAdapterConfigForPersistence(companyId, nextAdapterConfig, {
  ...
  function resolveImportedAssigneeAgentId(
    ...
    const assigneeAgentId = importedSlugToAgentId.get(assigneeSlug) ?? existingSlugToAgentId.get(assigneeSlug) ?? null;
    ...
    if (include.issues) {
      ...
      for (const manifestIssue of sourceManifest.issues) {
        ...
        if (manifestIssue.recurring) {
          if (!projectId) {
            throw unprocessable(
              `Recurring task ${manifestIssue.slug} is missing the project required to create a routine.`,
            );
          }
        ...
        if (!assigneeAgentId && issueStatus === "in_progress") {
          warnings.push(
            `Task ${manifestIssue.slug} was downgraded to todo because its assignee could not be imported as assignable work.`,
          );
          issueStatus = "todo";
        }

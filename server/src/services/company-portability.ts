...
import { requireOpenCodeModelId } from "@odysseus/adapter-opencode-local/server";
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".odysseus.yaml" || normalized === ".odysseus.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const odysseus = isPlainRecord(metadata?.odysseus) ? metadata?.odysseus as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.odysseusSkillKey)
    ?? asString(odysseus?.skillKey)
    ?? asString(odysseus?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "odysseus_bundled") {
    return `PossibLaw/odysseus/${slug}`;
  }
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  openclaw_gateway: [
    ...
    { path: ["sessionKey"], value: "odysseus" },
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .odysseus.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .odysseus.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .odysseus.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .odysseus.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Odysseus will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .odysseus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .odysseus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .odysseus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .odysseus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .odysseus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .odysseus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .odysseus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .odysseus.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .odysseus.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[odysseusExtensionPath];
  if (selectedFiles.has(odysseusExtensionPath) && typeof extensionEntry === "string") {
    filtered[odysseusExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findOdysseusExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".odysseus.yaml"] === "string") return ".odysseus.yaml";
  if (typeof files[".odysseus.yml"] === "string") return ".odysseus.yml";
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "odysseus_bundled") {
    ...
    return {
      ...
      commit,
      trackingRef: "master",
      url: `https://github.com/PossibLaw/odysseus/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "odysseus_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.odysseusSkillKey = skill.key;
  metadata.odysseus = {
    ...(isPlainRecord(metadata.odysseus) ? metadata.odysseus : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const odysseusExtensionPath = findOdysseusExtensionPath(normalizedFiles);
  const odysseusExtension = odysseusExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, odysseusExtensionPath) ?? "")
    : {};
  const odysseusCompany = isPlainRecord(odysseusExtension.company) ? odysseusExtension.company : {};
  const odysseusSidebar = normalizePortableSidebarOrder(odysseusExtension.sidebar);
  const odysseusAgents = isPlainRecord(odysseusExtension.agents) ? odysseusExtension.agents : {};
  const odysseusProjects = isPlainRecord(odysseusExtension.projects) ? odysseusExtension.projects : {};
  const odysseusTasks = isPlainRecord(odysseusExtension.tasks) ? odysseusExtension.tasks : {};
  const odysseusRoutines = isPlainRecord(odysseusExtension.routines) ? odysseusExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(odysseusCompany.brandColor),
      logoPath: asString(odysseusCompany.logoPath) ?? asString(odysseusCompany.logo),
      attachmentMaxBytes:
        typeof odysseusCompany.attachmentMaxBytes === "number" && Number.isFinite(odysseusCompany.attachmentMaxBytes)
          ? Math.max(1, Math.floor(odysseusCompany.attachmentMaxBytes))
          : null,
      requireBoardApprovalForNewAgents:
        typeof odysseusCompany.requireBoardApprovalForNewAgents === "boolean"
          ? odysseusCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof odysseusCompany.feedbackDataSharingEnabled === "boolean"
          ? odysseusCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof odysseusCompany.feedbackDataSharingConsentAt === "string"
          ? odysseusCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(odysseusCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(odysseusCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: odysseusSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(odysseusAgents[slug]) ? odysseusAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(odysseusProjects[slug]) ? odysseusProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(odysseusTasks[slug]) ? odysseusTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(odysseusRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(odysseusRoutines[slug]) ? odysseusRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  async function prepareImportedAgentAdapter(
    ...
    const nextAdapterConfig = writeOdysseusSkillSyncPreference(
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".odysseus.yaml" ||
          relative === ".odysseus.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const odysseusAgentsOut: Record<string, Record<string, unknown>> = {};
    const odysseusProjectsOut: Record<string, Record<string, unknown>> = {};
    const odysseusTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const odysseusRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readOdysseusSkillSyncPreference(
        ...
        odysseusAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      odysseusProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      odysseusTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      odysseusRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const odysseusExtensionPath = ".odysseus.yaml";
    const odysseusAgents = Object.fromEntries(
      Object.entries(odysseusAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const odysseusProjects = Object.fromEntries(
      Object.entries(odysseusProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const odysseusTasks = Object.fromEntries(
      Object.entries(odysseusTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const odysseusRoutines = Object.fromEntries(
      Object.entries(odysseusRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[odysseusExtensionPath] = buildYamlFile(
      {
        schema: "odysseus/v1",
        ...
        agents: Object.keys(odysseusAgents).length > 0 ? odysseusAgents : undefined,
        projects: Object.keys(odysseusProjects).length > 0 ? odysseusProjects : undefined,
        tasks: Object.keys(odysseusTasks).length > 0 ? odysseusTasks : undefined,
        routines: Object.keys(odysseusRoutines).length > 0 ? odysseusRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, odysseusExtensionPath);

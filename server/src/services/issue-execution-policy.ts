...
const EXECUTION_PENDING_STATUS: IssueExecutionState["status"] = "execution_pending";
  ...
  if (activeStage) {
    ...
    if (principalsEqual(currentParticipant, actor)) {
      if (requestedStatus === "done") {
        ...
        if (activeStage.onApprove === "return_to_executor" && nextStage) {
          if (!existingState?.returnAssignee) {
            throw unprocessable("Cannot return approved execution stage because the original executor is unknown");
          }
          patch.status = "in_progress";
          Object.assign(patch, patchForPrincipal(existingState.returnAssignee));

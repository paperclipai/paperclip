 ...
 */
import { pluginManifestV1Schema } from "@aiteamcorp/shared";
import type { AiTeamCorpPluginManifestV1 } from "@aiteamcorp/shared";
import { PLUGIN_API_VERSION } from "@aiteamcorp/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): AiTeamCorpPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as AiTeamCorpPluginManifestV1,
        };
      }

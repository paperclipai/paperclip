 ...
 */
import { pluginManifestV1Schema } from "@valadrien-os/shared";
import type { ValadrienOsPluginManifestV1 } from "@valadrien-os/shared";
import { PLUGIN_API_VERSION } from "@valadrien-os/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): ValadrienOsPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as ValadrienOsPluginManifestV1,
        };
      }

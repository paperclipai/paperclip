 ...
 */
import { pluginManifestV1Schema } from "@odysseus/shared";
import type { OdysseusPluginManifestV1 } from "@odysseus/shared";
import { PLUGIN_API_VERSION } from "@odysseus/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): OdysseusPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as OdysseusPluginManifestV1,
        };
      }

...
function extractQualifiedRefs(statement: string): SqlRef[] {
  ...
  const patterns: QualifiedRefPattern[] = [
    ...
    {
      // Accept optional schema qualification for the index name and `ON ONLY`,
      // while recording the qualified table reference rather than treating the
      // index name itself as a namespace object.
      pattern: /\bcreate\s+(?:unique\s+)?index\s+(?:concurrently\s+)?(?:if\s+not\s+exists\s+)?(?:"?[A-Za-z_][A-Za-z0-9_]*"?\.)?"?[A-Za-z_][A-Za-z0-9_]*"?\s+on\s+(?:only\s+)?"?([A-Za-z_][A-Za-z0-9_]*)"?\."?([A-Za-z_][A-Za-z0-9_]*)"?/gi,
      groups: "schema-table",

...
/**
 * MCP configs predate projection metadata. Keep their persisted secret-ref
 * shape stable by omitting default projection values, while retaining any
 * non-default classification supplied by the caller.
 */
function serializeMcpSecretRef(
  binding: Extract<CanonicalEnvBinding, { type: "secret_ref" }>,
): Extract<EnvBinding, { type: "secret_ref" }> {
  return {
    type: "secret_ref",
    secretId: binding.secretId,
    version: binding.version,
    ...(binding.projectionClass !== "unclassified"
...
export function secretService(db: Db) {
  ...
  async function assertBindingContext(
    ...
    if (!binding && context.consumerType === "agent" && context.configPath.startsWith("mcpServers.")) {
      binding = await db
        .select()
        .from(companySecretBindings)
        .where(
  ...
  async function normalizeMcpBinding(
    ...
    await assertSecretInCompany(companyId, binding.secretId);
    return serializeMcpSecretRef(binding);
  }
  ...
  async function normalizeMcpServersConfig(
    ...
    for (const [name, server] of Object.entries(parsed.data)) {
      ...
      let auth: McpServerAuth | undefined = server.auth;
      if (auth?.type === "bearer") {
        auth = {
          ...auth,
  ...
  async function listAdapterSchemaSecretFieldKeys(adapterType: string | null | undefined): Promise<string[]> {
    ...
    if (!adapter?.getConfigSchema) return [];
    try {
  ...
  return {
    ...
    resolveAdapterConfigForRuntime: async (
      ...
      const secretKeys = new Set<string>();
      const manifest: RuntimeSecretManifestEntry[] = [];
      const secretResolutionOptionsForPath = (
        configPath: string,
      ): SecretResolutionOptions | undefined => {
        if (!context) return undefined;
        if (ownerScoped) {
          return { accessContext: { ...context, configPath } };
        }
        return {
      ...
      const resolveBinding = async (
        ...
        configPath: string,
        fieldLabel: string,
      ): Promise<{ value: string | undefined; fromSecret: boolean }> => {
        const parsed = envBindingSchema.safeParse(rawBinding);
        if (!parsed.success) {
        ...
        if (binding.type === "secret_ref") {
          const secretResolution = await resolveSecretValueInternal(
            ...
            binding.secretId,
            binding.version,
            secretResolutionOptionsForPath(configPath),
          );
          manifest.push(secretResolution.manifestEntry);
          return { value: secretResolution.value, fromSecret: true };
        }
        if (opts?.skipUserSecrets) {
          return { value: undefined, fromSecret: false };
        }
        const secretResolution = await secretService(db).resolveUserSecretValue(
          companyId,
          {
      ...
      if (Object.prototype.hasOwnProperty.call(adapterConfig, "env")) {
        ...
        if (!record) {
          ...
          for (const [key, rawBinding] of Object.entries(record)) {
            ...
            if (resolution.value !== undefined) env[key] = resolution.value;
      ...
      if (
        ...
        for (const [name, rawServer] of Object.entries(record ?? {})) {
          ...
          for (const [key, rawBinding] of Object.entries(asRecord(server.headers) ?? {})) {
            ...
            if (resolution.value !== undefined) headers[key] = resolution.value;
          }
          const auth = asRecord(server.auth);
          if (auth?.type === "bearer") {
            const resolution = await resolveBinding(
            ...
            const secretResolution = await resolveSecretValueInternal(
              ...
              auth.secretId,
              version,
              secretResolutionOptionsForPath(`mcpServers.${name}.auth.secretId`),
            );
            manifest.push(secretResolution.manifestEntry);
      ...
      for (const key of secretFieldKeys) {
        ...
        const resolution = await resolveBinding(adapterConfig[key], key, key);

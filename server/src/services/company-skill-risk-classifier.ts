...
export function classifySkillRisk(input: CompanySkillRiskClassifierInput): CompanySkillRiskClassification {
  ...
  if (moneyMatches.length > 0) {
    return { tier: 2, rationale: { matchedRule: "money_tool_touch", matchedSignals: moneyMatches, summary: `Money-tool keyword(s) detected: ${moneyMatches.join(", ")}` } };
  }

  const writeSignals = [
    ...findMatches(corpus, WRITE_SCOPE_KEYWORDS),
    ...(WRITE_HTTP_METHOD_PATTERN.exec(corpus) ? [WRITE_HTTP_METHOD_PATTERN.exec(corpus)![0]] : []),
    ...(SQL_WRITE_PATTERN.exec(corpus) ? [SQL_WRITE_PATTERN.exec(corpus)![0]] : []),
  ];
  if (writeSignals.length > 0) {
    return { tier: 1, rationale: { matchedRule: "write_scope", matchedSignals: writeSignals, summary: `Write-scope signal(s) detected: ${writeSignals.join(", ")}` } };
  }

  return { tier: 0, rationale: { matchedRule: "read_only_default", matchedSignals: [], summary: "No money-tool or write-scope signals detected; treated as read-only/informational." } };
}

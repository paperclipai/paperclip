...
import type { AdapterRuntimeServiceReport } from "@aiteamcorp/adapter-utils";
import type { Db } from "@aiteamcorp/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@aiteamcorp/db";
import {
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.AITEAMCORP_WORKSPACE_CWD = input.worktreePath;
  env.AITEAMCORP_WORKSPACE_PATH = input.worktreePath;
  env.AITEAMCORP_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.AITEAMCORP_WORKSPACE_BRANCH = input.branchName;
  env.AITEAMCORP_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.AITEAMCORP_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.AITEAMCORP_WORKSPACE_SOURCE = input.base.source;
  env.AITEAMCORP_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.AITEAMCORP_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.AITEAMCORP_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.AITEAMCORP_PROJECT_ID = input.base.projectId ?? "";
  env.AITEAMCORP_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.AITEAMCORP_AGENT_ID = input.agent.id ?? "";
  env.AITEAMCORP_AGENT_NAME = input.agent.name;
  env.AITEAMCORP_COMPANY_ID = input.agent.companyId;
  env.AITEAMCORP_ISSUE_ID = input.issue?.id ?? "";
  env.AITEAMCORP_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.AITEAMCORP_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.AITEAMCORP_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.AITEAMCORP_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.AITEAMCORP_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.AITEAMCORP_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.AITEAMCORP_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.AITEAMCORP_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.AITEAMCORP_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.AITEAMCORP_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.AITEAMCORP_PROJECT_ID = input.workspace.projectId ?? "";
  env.AITEAMCORP_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.AITEAMCORP_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "AiTeamCorp", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,

...
import type { AdapterRuntimeServiceReport } from "@valadrien-os/adapter-utils";
import type { Db } from "@valadrien-os/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@valadrien-os/db";
import {
...
export function sanitizeRuntimeServiceBaseEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const key of Object.keys(env)) {
    if (key.startsWith("VALADRIEN_OS_")) {
      delete env[key];
    }
  }
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.VALADRIEN_OS_WORKSPACE_CWD = input.worktreePath;
  env.VALADRIEN_OS_WORKSPACE_PATH = input.worktreePath;
  env.VALADRIEN_OS_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.VALADRIEN_OS_WORKSPACE_BRANCH = input.branchName;
  env.VALADRIEN_OS_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.VALADRIEN_OS_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.VALADRIEN_OS_WORKSPACE_SOURCE = input.base.source;
  env.VALADRIEN_OS_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.VALADRIEN_OS_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.VALADRIEN_OS_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.VALADRIEN_OS_PROJECT_ID = input.base.projectId ?? "";
  env.VALADRIEN_OS_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.VALADRIEN_OS_AGENT_ID = input.agent.id ?? "";
  env.VALADRIEN_OS_AGENT_NAME = input.agent.name;
  env.VALADRIEN_OS_COMPANY_ID = input.agent.companyId;
  env.VALADRIEN_OS_ISSUE_ID = input.issue?.id ?? "";
  env.VALADRIEN_OS_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.VALADRIEN_OS_ISSUE_TITLE = input.issue?.title ?? "";
  env.VALADRIEN_OS_ISSUE_WORK_MODE = input.issue?.workMode ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.VALADRIEN_OS_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.VALADRIEN_OS_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.VALADRIEN_OS_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.VALADRIEN_OS_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.VALADRIEN_OS_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.VALADRIEN_OS_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.VALADRIEN_OS_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.VALADRIEN_OS_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.VALADRIEN_OS_PROJECT_ID = input.workspace.projectId ?? "";
  env.VALADRIEN_OS_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.VALADRIEN_OS_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".valadrien-os", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "ValadrienOs", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,

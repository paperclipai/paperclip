...
export async function repairOpenclawGatewayAgentAdapterConfig(
  ...
  try {
    await withAgentStartLock(agent.id, async () => {
      ...
      if (!fresh) return;
      if (!isOpenclawGatewayAgent(fresh)) return;
      if (!isEmptyAdapterConfig(fresh.adapterConfig)) return;
  ...
  logger.info(
    {
      ...
      beforeKeyCount: candidate.beforeConfig
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        sql`coalesce(jsonb_typeof(${agents.adapterConfig}), 'null') = 'object'`,
        sql`jsonb_object_keys(${agents.adapterConfig}) IS NULL`,
      ),
    )
    .limit(200);

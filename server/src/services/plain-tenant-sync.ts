...
// `threadDetails.tenantIdentifier.externalId`, but the tenant has to exist in
...
// fail-closed bridge: the session route only hands a tenant externalId to the
...
// to `tenant:read` + `tenant:create`. The key is bearer-grade for those
...
// Successful upserts per process, externalId → name. A hit means Plain
...
const ensuredTenants = new Map<string, string>();
 ...
 * workspace. Returns `true` only on a confirmed upsert; every failure path —
 * network error, timeout, non-200, GraphQL/mutation error — returns `false`
 * so the caller withholds tenant context rather than pointing the widget at
 * a tenant that may not exist. Never throws.
 */
export async function ensurePlainTenant(opts: EnsurePlainTenantOptions): Promise<boolean> {
  const { apiKey, externalId, name } = opts;
  if (ensuredTenants.get(externalId) === name) return true;
  ...
  try {
    ...
    ensuredTenants.set(externalId, name);
    return true;
  } catch (err) {

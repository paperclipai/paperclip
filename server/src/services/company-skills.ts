...
function skillMatchesImport(existing: CompanySkill, incoming: ImportedSkill, metadata: Record<string, unknown>) {
  return (
    ...
    existing.sourceLocator === incoming.sourceLocator &&
    ...
    stableJson(existing.fileInventory) === stableJson(serializeFileInventory(incoming.fileInventory)) &&
...
export function companySkillService(db: Db) {
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (existing && skillMatchesImport(existing, skill, metadata)) {
        out.push(existing);
        continue;
      }
      const row = existing

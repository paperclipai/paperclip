...
type RuntimeSkillEntry = PaperclipSkillEntry & {
  required: boolean;
...
function readBundledSkillRequired(markdown: string, key: string) {
  const { frontmatter } = parseFrontmatterMarkdown(markdown);
  if (frontmatter.required === true) return true;
  if (frontmatter.required === false) return false;
  return key === "paperclipai/paperclip/paperclip";
}
...
export function companySkillService(db: Db, options: CompanySkillServiceOptions = {}) {
    ...
    for (const skillsRoot of resolveBundledSkillsRoot()) {
      ...
      const bundledSkills = await readLocalSkillImports(companyId, skillsRoot)
        .then((skills) => skills.map((skill) => {
          ...
          const key = deriveCanonicalSkillKey(companyId, { ...skill, metadata });
          return {
            ...skill,
            key,
            metadata: {
              ...metadata,
              required: readBundledSkillRequired(skill.markdown, key),
            },
          };
        }))
  ...
  async function ensureSkillInventoryCurrent(companyId: string, input: { force?: boolean } = {}) {
    ...
    const refreshPromise = (async () => {
      ...
      await ensureBundledSkillReleases(companyId, bundled.skills);
  ...
  async function list(companyId: string, query: CompanySkillListQuery = {}): Promise<CompanySkillListItem[]> {
    ...
    // Folder membership is lightweight database state, not filesystem
    // inventory. Repair legacy/unfiled bundled rows even while the expensive
    // inventory scan is intentionally cached.
    await reconcilePaperclipSkillFolders(companyId);

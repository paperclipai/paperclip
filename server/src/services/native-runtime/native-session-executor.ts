...
const REMOTE_RUNNER_PROCESS_FINGERPRINT_SCRIPT =
  'process_fingerprint=$(test -r "/proc/$pid/stat" && test -r /proc/sys/kernel/random/boot_id || exit 4; IFS= read -r process_stat < "/proc/$pid/stat" || exit 4; IFS= read -r boot_id < /proc/sys/kernel/random/boot_id || exit 4; set -f; IFS=" "; set -- ${process_stat##*\\) }; test "$#" -ge 20 || exit 4; shift 19; start_ticks=$1; case "$start_ticks" in (""|*[!0-9]*) exit 4 ;; esac; test -n "$boot_id" || exit 4; printf "linux:%s:%s" "$boot_id" "$start_ticks")';

...
async function executePaperclipNativeSessionWithinScope(
  ...
  try {
    ...
    const remoteCleanupLease = input.runnerExecutionTarget?.kind === "remote" &&
      ...
      ? await input.db.select({ provider: environmentLeases.provider, resource: environmentLeases.providerLeaseId })
      ...
      async () => {
        ...
        const result = await trace.run(runnerSessionStartupScope, () =>
          executeNativeSession({
            ...
            remoteCleanupScope: remoteCleanupLease?.resource
              ? JSON.stringify([remoteCleanupLease.provider, remoteCleanupLease.resource]) : undefined,
            backend:
              input.backend ??
              runnerdBackend ??

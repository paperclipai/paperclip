...
const QA_BASELINE_BLOCK = [
  ...
  "- Every QA verdict comment must include one Smart Review summary line using exactly this token format: `[CQ:pass|warn|fail|na] [EH:pass|warn|fail|na] [TC:pass|warn|fail] [CM:pass|warn|fail|na] [DOC:pass|warn|fail|na]`.",
  "- `TC` must always be an explicit ship verdict (`pass`, `warn`, or `fail`) for QA ship comments; never use `TC:na`.",
  "- `DOC:na` is only for cases where docs were reviewed and no docs change is required.",

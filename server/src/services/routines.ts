...
// Recency window for the open-issue coalescing fallback (BUS-204). Long
// enough to bridge several hourly cron ticks while an issue sits open with a
// dead run; coalescing bumps `updated_at` so the window re-arms itself.
...
export function routineService(
  ...
  // routines duplicate themselves (BUS-174 report). This arm matches a still
  // open issue for the same routine updated within the recency window and is
  // refreshed on every coalesce, so the window stays open while the routine
  // keeps firing but reopens naturally once work goes stale.
    ...
    const run = await db.transaction(async (tx) => {
      ...
      try {
        ...
        if (activeIssue && input.routine.concurrencyPolicy !== "always_enqueue") {
          ...
          await txDb
            .update(issues)
            .set({ updatedAt: triggeredAt })
            .where(and(eq(issues.companyId, input.routine.companyId), eq(issues.id, activeIssue.id)));
        ...
        try {
          ...
          await txDb
            .update(issues)
            .set({ updatedAt: triggeredAt })
            .where(and(eq(issues.companyId, input.routine.companyId), eq(issues.id, existingIssue.id)));
          const updated = await finalizeRun(createdRun.id, {
            status,

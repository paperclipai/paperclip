...
import { agentApiKeys, agents, authUsers, companies, companyMemberships, instanceUserRoles } from "@valadrien-os/db";
...
async function resolveCloudTenantActor(db: Db, req: Request): Promise<Express.Request["actor"] | null> {
  const expectedToken = process.env.VALADRIEN_OS_CLOUD_TENANT_SERVER_TOKEN?.trim();
  if (!expectedToken) return null;

  const token = req.header("x-valadrien-os-cloud-tenant-token")?.trim();
  if (!token || !constantTimeStringEqual(token, expectedToken)) return null;

  const userId = requiredCloudHeader(req, "x-valadrien-os-cloud-user-id");
  const userEmail = requiredCloudHeader(req, "x-valadrien-os-cloud-user-email").toLowerCase();
  const stackId = requiredCloudHeader(req, "x-valadrien-os-cloud-stack-id");
  const stackRole = stackMembershipRole(req.header("x-valadrien-os-cloud-stack-role"));
  const userName = req.header("x-valadrien-os-cloud-user-name")?.trim() || userEmail;
  const valadrienOsCompanyId = req.header("x-valadrien-os-cloud-valadrien-os-company-id")?.trim();
  const companyId = cloudTenantCompanyId(stackId);
  const companyName = valadrienOsCompanyId || `${stackId} ValadrienOs`;
  const now = new Date();
  ...
  await db
    ...
    .values({
      id: companyId,
      name: companyName,
      description: `Provisioned by ValadrienOs Cloud for stack ${stackId}.`,
      status: "active",
...
function cloudTenantCompanyId(stackId: string): string {
  const bytes = createHash("sha256").update(`valadrien-os-cloud-tenant-company:${stackId}`).digest();

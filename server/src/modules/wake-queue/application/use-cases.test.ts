...
function createFakeIssueLock(reader: WakeQueueReader, writer: WakeQueueWriter, issue = ISSUE): IssueLockWriter {
  return {
...
describe("releaseIssueExecution", () => {
  it.each([true, false])(
    ...
    async (authorizedFailedChatRetry) => {
      ...
      const writer = createFakeWriter({
        claimNextDeferredWake: vi.fn(async () => queue.shift() ?? null),
      });
      const reader = createFakeReader();
      const release = createReleaseIssueExecution({
        issueLock: createFakeIssueLock(reader, writer, {
          ...ISSUE,
          status: "done",
        }),
      ...
      expect(writer.normalizeDeferredWakeCommentIds).not.toHaveBeenCalled();
      expect(writer.reopenIssue).not.toHaveBeenCalled();
      if (authorizedFailedChatRetry) {
        expect(writer.getQueuedCommentLiveness).not.toHaveBeenCalled();
        expect(writer.cancelDeferredWake).not.toHaveBeenCalled();
        expect(writer.finalizePromotedWake).toHaveBeenCalledWith(
        ...
        expect(writer.getQueuedCommentLiveness).toHaveBeenCalledTimes(1);
        expect(writer.cancelDeferredWake).toHaveBeenCalledTimes(1);
        expect(writer.finalizePromotedWake).not.toHaveBeenCalled();
  ...
  it.each([false, true])(
    ...
    async (hasDeferredInput) => {
      ...
      const release = createReleaseIssueExecution({
        issueLock: createFakeIssueLock(createFakeReader(), writer),
        recovery: createFakeRecovery(),
      });
      ...
      if (hasDeferredInput) {
        expect(writer.isImmediateRecoverySourceBlocked).not.toHaveBeenCalled();
        expect(result.outcome.kind).toBe("promoted");
      } else {
        expect(writer.isImmediateRecoverySourceBlocked).toHaveBeenCalledWith({
          companyId: RUN.companyId,
          runId: RUN.id,
        });

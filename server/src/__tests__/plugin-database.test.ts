...
const multiMigrationPluginKey = "valadrien-os.dbfixture";
const llmWikiPluginKey = "valadrien-os.plugin-llm-wiki";
...
describeEmbeddedPostgres("plugin database namespaces", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-plugin-db-");
    db = createDb(tempDb.connectionString);
  }, 20_000);

  afterEach(async () => {
    for (const pluginKey of ["valadrien-os.dbtest", "valadrien-os.escape", "valadrien-os.refresh", multiMigrationPluginKey, llmWikiPluginKey]) {
  ...
  async function createPluginPackage(manifest: ValadrienOsPluginManifestV1, migrationSql: string) {
    const packageRoot = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-plugin-package-"));
  ...
  function llmWikiManifest(): ValadrienOsPluginManifestV1 {
    return {
      id: llmWikiPluginKey,
      apiVersion: 1,
      version: "0.1.0",
  ...
  async function createInstallablePluginPackage(
    pluginManifest: ValadrienOsPluginManifestV1,
    migrationSql: string,
  ) {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        type: "module",
        valadrienOsPlugin: { manifest: "./manifest.js" },
      }),
      "utf8",
    );
    await writeFile(
  ...
  async function installPluginRecord(manifest: ValadrienOsPluginManifestV1) {
  ...
  function manifest(pluginKey = "valadrien-os.dbtest"): ValadrienOsPluginManifestV1 {
    return {
      id: pluginKey,
      apiVersion: 1,
      version: "1.0.0",
  ...
  it("applies the bundled LLM Wiki migrations through the production validator", async () => {
    ...
    expect(migrations.map((migration) => migration.migrationKey)).toEqual([
      "001_llm_wiki.sql",
      "002_valadrien_os_distillation.sql",
      "003_spaces.sql",
    ]);
    ...
    expect(constraints).not.toContain("valadrien_os_distillation_cursor_company_id_wiki_id_source_sco_key");
    expect(constraints).not.toContain("valadrien_os_distillation_work_i_company_id_wiki_id_idempotenc_key");
    expect(constraints).not.toContain("valadrien_os_page_bindings_company_id_wiki_id_page_path_key");
    expect(uniqueColumnSets).not.toContain("wiki_pages:company_id,wiki_id,path");
    expect(uniqueColumnSets).not.toContain("valadrien_os_distillation_cursors:company_id,wiki_id,source_scope,scope_key,source_kind");
    expect(uniqueColumnSets).not.toContain("valadrien_os_distillation_work_items:company_id,wiki_id,idempotency_key");
    expect(uniqueColumnSets).not.toContain("valadrien_os_page_bindings:company_id,wiki_id,page_path");
  });
  ...
  it("records a failed migration when SQL escapes the plugin namespace", async () => {
    const pluginManifest = manifest("valadrien-os.escape");
    const packageRoot = await createPluginPackage(
      pluginManifest,
  ...
  it("refreshes persisted manifests from disk before activation", async () => {
    const staleManifest = manifest("valadrien-os.refresh");
    const refreshedManifest: ValadrienOsPluginManifestV1 = {
      ...staleManifest,
      database: {

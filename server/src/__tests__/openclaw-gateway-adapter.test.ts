...
import { execute, testEnvironment } from "@odysseus/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@odysseus/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("odysseus:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("ODYSSEUS_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("ODYSSEUS_TASK_ID=task-123");
      expect(String(payload?.message ?? "")).toContain("## Odysseus Wake Payload");
      ...
      expect(payload?.odysseus).toMatchObject({
        wake: {
          latestCommentId: "comment-2",
          commentIds: ["comment-1", "comment-2"],
        },

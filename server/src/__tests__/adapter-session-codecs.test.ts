...
import { sessionCodec as claudeSessionCodec } from "@odysseus/adapter-claude-local/server";
import { sessionCodec as codexSessionCodec, isCodexUnknownSessionError } from "@odysseus/adapter-codex-local/server";
import {
...
import { sessionCodec as acpxSessionCodec } from "@odysseus/adapter-acpx-local/server";
...
describe("adapter session codecs", () => {
  ...
  it("preserves acpx session params required for compatibility checks", () => {
    const parsed = acpxSessionCodec.deserialize({
      sessionKey: "odysseus:company:agent:task:fingerprint",

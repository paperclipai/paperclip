...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "odysseus", LOGNAME: "odysseus", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "odysseus" }),
      },
    );

    expect(tokens).toEqual(["odysseus", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "odysseus" }, {
        userInfo: () => ({ username: "odysseus" }),
      });

      expect(tokens).toEqual(["odysseus", "custom-token"]);
    } finally {

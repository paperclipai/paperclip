...
describe("devin_local environment diagnostics", () => {
  ...
  it("honors XDG_CONFIG_HOME for the devin config check", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-devin-local-xdg-"));
    const fakeBin = await writeFakeDevin(root);
    const configHome = path.join(root, "xdg");
    const devinDir = path.join(configHome, "devin");
    ...
    try {
      ...
      const check = result.checks.find((c) => c.code === "devin_config_present");
      expect(check?.level).toBe("info");
      expect(check?.message).toContain(devinDir);
    } finally {
      vi.unstubAllEnvs();
      await fs.rm(root, { recursive: true, force: true });
    }
  ...
  it("reports a runtime-resolved working directory when no cwd is configured", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-devin-local-nocwd-"));
    const fakeBin = await writeFakeDevin(root);
    ...
    try {
      const result = await testEnvironment({
        companyId: "company-1",
        adapterType: "devin_local",
        config: { command: fakeBin },
      });

      const check = result.checks.find((c) => c.code === "cwd_runtime_resolved");
      expect(check?.level).toBe("info");
      expect(result.checks.some((c) => c.code === "agents_md_missing")).toBe(false);
      expect(result.checks.some((c) => c.code === "invalid_cwd")).toBe(false);
    } finally {
      await fs.rm(root, { recursive: true, force: true });
    }
  });

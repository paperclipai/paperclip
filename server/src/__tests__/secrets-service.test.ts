...
const externalSecretAdapter: ServerAdapterModule = {
  type: "external_secret_test",
  execute: async () => ({ exitCode: 0, signal: null, timedOut: false }),
  testEnvironment: async () => ({
    adapterType: "external_secret_test",
    status: "pass",
    checks: [],
    testedAt: new Date(0).toISOString(),
  }),
  getConfigSchema: () => ({
    fields: [{ key: "apiKey", label: "API key", type: "text", meta: { secret: true } }],
  }),
};
...
describeEmbeddedPostgres("secretService", () => {
  ...
  beforeAll(async () => {
    registerServerAdapter(externalSecretAdapter);
  ...
  afterAll(async () => {
    unregisterServerAdapter(externalSecretAdapter.type);
    await stopDb?.();
  ...
  it("can skip user-secret refs while resolving adapter config for non-runtime skill discovery", async () => {
    ...
    await expect(
      svc.resolveAdapterConfigForRuntime(companyId, adapterConfig, undefined, { adapterType: externalSecretAdapter.type }),
    ).rejects.toMatchObject({
      status: 422,
    ...
    const resolved = await svc.resolveAdapterConfigForRuntime(
      ...
      adapterConfig,
      undefined,
      { adapterType: externalSecretAdapter.type, skipUserSecrets: true },
    );
  ...
  it("reports missing adapter-config user secret refs before runtime resolution", async () => {
    ...
    const adapterConfig = {
      ...
      apiKey: { type: "user_secret_ref" as const, key: "plugin_api_key", version: "latest" as const },
    };
    ...
    await expect(
      ...
      expect.objectContaining({
        ...
        envKey: "apiKey",
        userSecretDefinitionId: definition.id,
        userSecretDefinitionKey: "plugin_api_key",
        responsibleUserId: "user-1",
        errorCode: "user_secret_missing",
      }),
    ]);
    ...
    await expect(
      ...
      expect.objectContaining({
        ...
        userSecretDefinitionId: definition.id,
        userSecretDefinitionKey: "plugin_api_key",
        userSecretDefinitionName: "Plugin API key",
        responsibleUserId: "user-1",
        errorCode: "user_secret_definition_inactive",
      }),
    ]);

...
describeEmbeddedPostgres("heartbeat bounded retry scheduling", () => {
  ...
  it("keeps unsafe workspace restore authoritative over launcher bootstrap evidence", async () => {
    const executionRecovery = { kind: "bootstrap" as const, providerWorkStarted: false as const,
      launcher: { version: 1 as const, outcome: "capacity_unavailable" as const } };
    const restored = applyWorkspaceRestoreFailure({ exitCode: 5, signal: null, timedOut: false,
      errorCode: "launcher_capacity_unavailable", executionRecovery,
      resultJson: { stdout: "", executionRecovery, workspaceRestoreFailure: "restore_unsafe_archive" } });
    expect(restored.errorCode).toBe("workspace_restore_failed");
    const runId = randomUUID(), companyId = randomUUID(), agentId = randomUUID();
    const now = new Date("2026-04-20T12:00:00.000Z");
    await seedRetryFixture({ runId, companyId, agentId, now,
      errorCode: restored.errorCode!, resultJson: restored.resultJson });
    await db.update(heartbeatRuns).set({ exitCode: restored.exitCode, signal: restored.signal })
      .where(eq(heartbeatRuns.id, runId));
    const run = await heartbeat.getRun(runId);
    expect(isLauncherCapacityFailure(run!)).toBe(false);

...
type AppOptions = { companyIds?: string[]; source?: string };

function isAppOptions(value: unknown): value is AppOptions {
  return typeof value === "object" && value !== null && (
    "companyIds" in value || "source" in value
  );
}

async function createApp(storage: StorageService, dbOrOptions: unknown = {}) {
  const options = isAppOptions(dbOrOptions) ? dbOrOptions : undefined;
  const db = options ? {} : dbOrOptions;

...
async function expectContainedWorkspaceBranchFailure(input: {
  ...
  expect(action).toMatchObject({
    ...
    wakePolicy: expect.objectContaining({
      // fork law: workspace_validation_failed stays manual_repair_required (owner-scoped), not board_escalation
...
async function expectForwardBranchReconciled(input: {
  ...
  // Some reconcile flows store the realpath'd worktree path and others store
  // it verbatim; on macOS the mkdtemp root under /var/folders is a symlink
  // into /private/var, so normalize BOTH sides before comparing.

...
import { execute } from "@odysseus/adapter-cursor-local/server";
...
describe("cursor execute", () => {
  it("injects odysseus env vars and prompt note by default", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-cursor-execute-"));
    ...
    try {
      ...
      expect(capture.argv).not.toContain("Follow the odysseus heartbeat.");
      ...
      expect(capture.prompt).toContain("Odysseus runtime note:");
      expect(capture.prompt).toContain("ODYSSEUS_API_KEY");
  ...
  it("passes --mode when explicitly configured", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-cursor-execute-mode-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("injects company-library runtime skills into the Cursor skills home before execution", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-cursor-execute-runtime-skill-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    ...
    const odysseusDir = await createSkillDir(runtimeSkillsRoot, "odysseus");
  ...
  it("prefers ~/.local/bin/cursor-agent for remote sandbox execution when using the default command", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-cursor-sandbox-execute-"));
  ...
  it("keeps explicit command overrides for remote sandbox execution", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-cursor-sandbox-explicit-"));
    const homeDir = path.join(root, "home");
    const workspace = path.join(root, "workspace");
    const remoteWorkspace = path.join(root, "remote-workspace");
    const capturePath = path.join(root, "capture.json");

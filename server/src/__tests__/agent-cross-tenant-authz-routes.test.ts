...
describe.sequential("agent cross-tenant route authorization", () => {
  ...
  it("restricted agent detail signals redaction and config presence instead of a silent empty object", async () => {
    resetMockDefaults();
    currentAccessCanUser = false;
    // Same-company sibling: passes the broad agent:read boundary (company access is
    // enforced separately by assertCompanyAccess) but lacks the agents:create
    // config-read grant -> the detail is returned redacted, not a 403.
    mockAccessService.decide.mockImplementation(async (input: { action?: string }) => ({
      allowed: input.action === "agent:read",
      action: input.action,
      reason: input.action === "agent:read" ? "allow_explicit_grant" : "deny_missing_grant",
      explanation: input.action === "agent:read" ? "Sibling may read agent detail." : "Missing permission.",
    }));
  ...
  it("restricted agent detail reports absence honestly when nothing is persisted", async () => {
    resetMockDefaults();
    currentAccessCanUser = false;
    // Same-company sibling: passes agent:read, lacks config-read (see sibling note above).
    mockAccessService.decide.mockImplementation(async (input: { action?: string }) => ({
      allowed: input.action === "agent:read",
      action: input.action,
      reason: input.action === "agent:read" ? "allow_explicit_grant" : "deny_missing_grant",

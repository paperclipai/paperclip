...
async function createApp(actor: Record<string, unknown>) {
  ...
  app.use("/api", briefsRoutes({} as never));
...
describe("briefs routes", () => {
  ...
  it("returns 412 until the Briefs agent is ready", async () => {
    const { HttpError } = await import("../errors.js");
    mockBriefsService.overview.mockRejectedValue(
      new HttpError(412, "Built-in agent is not configured: briefs", {

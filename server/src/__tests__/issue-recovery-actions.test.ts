...
describeEmbeddedPostgres("issue recovery actions", () => {
    ...
    while (Date.now() < deadline) {
      const wakeup = await db
        ...
        .then((rows) => rows.find((row) =>
          row.payload?.issueId === issueId &&
          row.payload?.mutation === "recovery_action_acceptance",
        ) ?? null);
      if (wakeup) {
        // A status-only recovery run already owns this issue's execution lock,
        // so acceptance must durably defer a normal-model assignment wake
        // instead of coalescing deliverable work into the recovery run.
  ...
  it("lets only the action owner in the matching recovery run accept a terminated-owner handoff", async () => {
    ...
    const assignmentWakeup = await waitForAssignmentWakeup(managerId, sourceIssueId);
    expect(assignmentWakeup).toMatchObject({
      ...
      payload: expect.objectContaining({
        ...
        _paperclipWakeContext: expect.objectContaining({
          issueId: sourceIssueId,
          source: "issue.recovery_action_acceptance",
          assignmentHandoff: true,
        }),
      }),
    });

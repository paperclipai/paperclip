...
describeEmbeddedPostgres("issueThreadInteractionService", () => {
  ...
  it("persists addressees without allowing them to bypass human-only governance", async () => {
    ...
    const input = {
      ...
      summary: "ASK: Which scope should this work take? WHY: The addressee fixture needs a pending operator ask. ACTION: Pick Phase 1.", // fork law: asks require an ASK/WHY/ACTION summary
  ...
  it("cancels addressed interactions before deleting the addressee", async () => {
    ...
    const created = await interactionsSvc.create(
      { id: issueId, companyId },
      {
        ...
        summary: "ASK: Answer the fixture question. WHY: The fixture needs a pending operator ask. ACTION: Reply with a choice.", // fork law: asks require an ASK/WHY/ACTION summary
  ...
  it("supersedes older pending confirmations from the same agent without crossing agent or kind", async () => {
    ...
    const older = await interactionsSvc.create({ id: issueId, companyId }, {
      ...
      summary: "ASK: Approve this request? WHY: The fixture needs a pending operator ask. ACTION: Accept or reject it.", // fork law: asks require an ASK/WHY/ACTION summary
  ...
  it("supersedes an agent's own older pending ask_user_questions without crossing agent, kind, or issue", async () => {
    ...
    const question = (prompt: string) => ({
      kind: "ask_user_questions" as const,
      summary: `ASK: ${prompt} WHY: The supersede fixture needs a pending operator ask. ACTION: Reply with a choice.`, // fork law: asks require an ASK/WHY/ACTION summary
      payload: {
        version: 1 as const,
        questions: [{
          id: "q",
  ...
  it("leaves exactly one pending ask_user_questions on the onboarding first task after probe cards and the real question arrive", async () => {
    ...
    const probe = (prompt: string) => ({
      kind: "ask_user_questions" as const,
      summary: `ASK: ${prompt} WHY: Throwaway schema probe from the onboarding agent. ACTION: Ignore; the real question follows.`, // fork law: asks require an ASK/WHY/ACTION summary
      payload: {
        version: 1 as const,
        questions: [{
          id: "q",
  ...
  it("does not supersede request confirmations for run-originated comments even under user auth", async () => {
    ...
    const created = await interactionsSvc.create({
      ...
      // Fork-local: operator asks must carry an ASK/WHY/ACTION summary
      // (requiredAskSummarySchema). Upstream's fixtures predate that rule.
      summary: "ASK: approve the draft. WHY: it is ready to ship. ACTION: confirm or reject.",
    }, {
      userId: "local-board",
    });

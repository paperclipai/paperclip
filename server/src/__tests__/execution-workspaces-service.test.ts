...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "Odysseus Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@odysseus.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("odysseus-execution-workspaces-service-");
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `odysseus-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "odysseus-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "odysseus-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      branchName: "odysseus-close-check",
      baseRef: "main",
      metadata: {

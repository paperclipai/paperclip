...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it("reaps a detached local run when the child pid stays alive without output", async () => {
    ...
    expect(runs.filter((candidate) => candidate.retryOfRunId === runId)).toHaveLength(0);
  });
  ...
  it("persists cancellation status before terminating the in-memory process", async () => {
    const { runId } = await seedRunFixture({
      agentStatus: "running",
      includeIssue: false,
    });
  ...
  it("does not immediately requeue assigned issue work after manual run cancellation", async () => {
    const { agentId, runId, issueId } = await seedRunFixture({
      agentStatus: "running",
    });
    const heartbeat = heartbeatService(db);
    ...
    const issueAfterCancel = await db
      ...
      .where(eq(issues.id, issueId))
      .then((rows) => rows[0] ?? null);
    expect(issueAfterCancel).toEqual({
      executionRunId: null,
    ...
    await db.insert(issueComments).values({
      ...
      issueId,
      authorUserId: "user-1",
      body: "Please continue after cancel.",
    });

    const followup = await heartbeat.wakeup(agentId, {
      source: "automation",
      triggerDetail: "system",
      reason: "issue_commented",
    ...
    await waitForRunToSettle(heartbeat, followup!.id, 5_000);
  });
  ...
  it("auto-resolves a parent recovery action when a child execution lane regains a live run", async () => {
    const { companyId, agentId, issueId } = await seedAssignedTodoNoRunFixture();
    ...
    await db.insert(issues).values({
      id: parentIssueId,
      companyId,
      title: "Parent waiting on child lane",
      status: "blocked",
      priority: "high",
      assigneeAgentId: agentId,
      issueNumber: 2,
    ...
    await db
      .update(issues)
      .set({ parentId: parentIssueId, requestDepth: 1 })
      .where(eq(issues.id, issueId));
    await db.insert(issueRecoveryActions).values({
      id: actionId,
      companyId,
      ...
      returnOwnerAgentId: agentId,
      cause: "stranded_assigned_issue",
      fingerprint: `test:${parentIssueId}`,
      evidence: { retryReason: "issue_continuation_needed" },
      nextAction: "Restore a live execution path.",
      attemptCount: 1,
      lastAttemptAt: new Date("2026-03-19T00:05:00.000Z"),
    ...
    expect(resolvedAction).toMatchObject({ status: "resolved", outcome: "restored" });
    expect(resolvedAction?.resolutionNote).toContain(issuePrefix);
    expect(resolvedAction?.resolutionNote).toContain(run!.id);
    ...
    const activity = await waitForValue(async () =>
      db
        .select()
        .from(activityLog)
        .where(and(eq(activityLog.entityId, parentIssueId), eq(activityLog.action, "issue.recovery_action_resolved")))
        .then((rows) => rows[0] ?? null),
    );
    expect(activity?.details).toMatchObject({
      recoveryActionId: actionId,
  ...
  it("fails orphaned deferred wakes before re-enqueueing stranded in-progress work", async () => {
    const { companyId, agentId, issueId, runId } = await seedStrandedIssueFixture({
      status: "in_progress",
      runStatus: "failed",
    });
    ...
    const result = await heartbeat.reconcileStrandedAssignedIssues();
    expect(result.orphanedDeferredWakesFailed).toBe(1);
    expect(result.continuationRequeued).toBe(1);
    expect(result.issueIds).toEqual([issueId]);
    ...
    expect(deferredWake?.error).toContain("orphaned");
  ...
  it("wakes the parent manager when a child execution lane escalates to recovery", async () => {
    ...
    expect(managerWake).toMatchObject({ reason: "issue_commented", source: "automation" });

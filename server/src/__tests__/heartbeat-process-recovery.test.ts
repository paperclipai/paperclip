...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it("blocks stranded in-progress work after a productive continuation retry was already used", async () => {
    ...
    const comments = await db
      ...
      .where(eq(issueComments.issueId, issueId));
    expect(comments).toHaveLength(1);
    // Union of both sides: upstream #14046 rerouted this fixture through
    // `escalateDispositionRepair`, so the comment text is the disposition-repair
    // notice. The KEE-250 invariant assertion is kept on top of that.

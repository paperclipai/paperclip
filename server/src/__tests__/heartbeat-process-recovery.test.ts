  ...
  it("blocks assigned todo work after the one automatic dispatch recovery was already used", async () => {
    const { companyId, agentId, issueId, runId } =
      await seedStrandedIssueFixture({
        ...
        // retry cap, and the cap escalates to an agent-owned action. Infra
        // kills take the wait-and-redispatch path instead and are covered by
        // the infra re-dispatch back-off tests below.
  ...
  it("blocks stranded in-progress work after the continuation retry was already used", async () => {
    const { companyId, agentId, issueId, runId } =
      await seedStrandedIssueFixture({
        ...
        runStatus: "failed",
        retryReason: "issue_continuation_needed",
        // An agent-side failure on purpose: this case is about the
        // continuation retry cap, and the cap escalates to an agent-owned
        // action. Infra kills take the wait-and-redispatch path instead and
        // are covered by the infra re-dispatch back-off tests below.
        runErrorCode: "adapter_exit_code",
      });
    const heartbeat = heartbeatService(db);

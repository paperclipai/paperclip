...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const odysseusDir = path.join(worktreeRoot, ".odysseus");
    const configPath = path.join(odysseusDir, "config.json");
    const envPath = path.join(odysseusDir, ".env");
    const sharedRoot = path.join(tempRoot, ".odysseus", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".odysseus-worktrees");

    await fs.mkdir(odysseusDir, { recursive: true });
    await fs.writeFile(configPath, JSON.stringify(buildLegacyConfig(sharedRoot), null, 2) + "\n", "utf8");
    await fs.writeFile(
      envPath,
      [
        "# Odysseus environment variables",
        "ODYSSEUS_IN_WORKTREE=true",
        "ODYSSEUS_WORKTREE_NAME=PAP-884-ai-commits-component",
        "ODYSSEUS_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.ODYSSEUS_IN_WORKTREE = "true";
    process.env.ODYSSEUS_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.ODYSSEUS_WORKTREES_DIR = isolatedHome;
    ...
    expect(repairedEnv).toContain(`ODYSSEUS_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('ODYSSEUS_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`ODYSSEUS_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`ODYSSEUS_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('ODYSSEUS_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.ODYSSEUS_HOME).toBe(isolatedHome);
    expect(process.env.ODYSSEUS_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# Odysseus environment variables",
        "ODYSSEUS_IN_WORKTREE=true",
        "ODYSSEUS_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.ODYSSEUS_IN_WORKTREE = "true";
    process.env.ODYSSEUS_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.ODYSSEUS_WORKTREES_DIR = isolatedHome;
    delete process.env.ODYSSEUS_HOME;
    delete process.env.ODYSSEUS_INSTANCE_ID;
    delete process.env.ODYSSEUS_CONFIG;
  ...
  it("does not persist transient runtime home overrides over repo-local worktree env", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-worktree-runtime-override-"));
    const isolatedHome = path.join(tempRoot, ".odysseus-worktrees");
    ...
    await fs.writeFile(
      ...
      [
        "# Odysseus environment variables",
        `ODYSSEUS_HOME=${JSON.stringify(isolatedHome)}`,
        `ODYSSEUS_INSTANCE_ID=${JSON.stringify(instanceId)}`,
        `ODYSSEUS_CONFIG=${JSON.stringify(configPath)}`,
        `ODYSSEUS_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`,
        'ODYSSEUS_IN_WORKTREE="true"',
        'ODYSSEUS_WORKTREE_NAME="PAP-989-multi-user-implementation-using-plan-from-pap-958"',
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.ODYSSEUS_IN_WORKTREE = "true";
    process.env.ODYSSEUS_WORKTREE_NAME = "PAP-989-multi-user-implementation-using-plan-from-pap-958";
    process.env.ODYSSEUS_HOME = transientHome;
    process.env.ODYSSEUS_INSTANCE_ID = instanceId;
    process.env.ODYSSEUS_CONFIG = configPath;
    ...
    expect(repairedEnv).toContain(`ODYSSEUS_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).not.toContain(`ODYSSEUS_HOME=${JSON.stringify(transientHome)}`);
    expect(process.env.ODYSSEUS_HOME).toBe(isolatedHome);
  });

  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".odysseus-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".odysseus", "worktrees");
    ...
    const odysseusDir = path.join(currentWorktreeRoot, ".odysseus");
    const configPath = path.join(odysseusDir, "config.json");
    const envPath = path.join(odysseusDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".odysseus", "config.json");
  ...
  it("persists runtime-selected worktree ports back into explicit-port auth URLs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.ODYSSEUS_IN_WORKTREE = "true";
    process.env.ODYSSEUS_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.ODYSSEUS_HOME = isolatedHome;
    process.env.ODYSSEUS_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.ODYSSEUS_CONFIG = configPath;
  ...
  it("does not rewrite no-port public auth URLs when persisting runtime-selected ports", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-worktree-public-ports-"));
    ...
    await fs.writeFile(
      ...
      JSON.stringify(
        {
          ...buildLegacyConfig(instanceRoot, "https://odysseus.example"),
          database: {
            mode: "embedded-postgres",
            embeddedPostgresDataDir: path.join(instanceRoot, "db"),
            embeddedPostgresPort: 54331,
    ...
    process.chdir(worktreeRoot);
    process.env.ODYSSEUS_IN_WORKTREE = "true";
    process.env.ODYSSEUS_WORKTREE_NAME = "PAP-125-public-base-url";
    process.env.ODYSSEUS_HOME = isolatedHome;
    process.env.ODYSSEUS_INSTANCE_ID = "pap-125-public-base-url";
    process.env.ODYSSEUS_CONFIG = configPath;
    ...
    expect(writtenConfig.server.port).toBe(3103);
    expect(writtenConfig.database.embeddedPostgresPort).toBe(54335);
    expect(writtenConfig.auth.publicBaseUrl).toBe("https://odysseus.example");
  });
  ...
  it("does not rewrite the in-memory config when auth URL has no explicit port", () => {
    const { config, changed } = applyRuntimePortSelectionToConfig(
      buildLegacyConfig("/tmp/shared", "https://odysseus.example"),
      {
        serverPort: 3104,
        databasePort: 54340,
        allowServerPortWrite: false,
    ...
    expect(config.server.port).toBe(3100);
    expect(config.database.embeddedPostgresPort).toBe(54340);
    expect(config.auth.publicBaseUrl).toBe("https://odysseus.example");
  });
});

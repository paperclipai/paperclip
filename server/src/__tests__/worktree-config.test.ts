...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    ...
    const aiteamcorpDir = path.join(worktreeRoot, ".aiteamcorp");
    const configPath = path.join(aiteamcorpDir, "config.json");
    const envPath = path.join(aiteamcorpDir, ".env");
    const sharedRoot = path.join(tempRoot, ".aiteamcorp", "instances", "default");
    ...
    await fs.writeFile(
      envPath,
      [
        "# AiTeamCorp environment variables",
  ...
  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    ...
    const isolatedHome = path.join(tempRoot, ".aiteamcorp-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".aiteamcorp", "worktrees");
    ...
    const aiteamcorpDir = path.join(currentWorktreeRoot, ".aiteamcorp");
    const configPath = path.join(aiteamcorpDir, "config.json");
    const envPath = path.join(aiteamcorpDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".aiteamcorp", "config.json");

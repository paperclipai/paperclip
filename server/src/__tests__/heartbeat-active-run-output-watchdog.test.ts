...
vi.mock("@odysseus/shared/telemetry", async () => {
  const actual = await vi.importActual<typeof import("@odysseus/shared/telemetry")>(
    "@odysseus/shared/telemetry",
  );
  return {
    ...actual,
...
describeEmbeddedPostgres("active-run output watchdog", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("odysseus-active-run-output-watchdog-");

...
describe("routine-owner cancel carve-out (GLA-1048)", () => {
  beforeEach(() => {
    vi.resetModules();
    ...
    mockRoutineService.get.mockResolvedValue({
      id: routineId,
      companyId,
      assigneeAgentId: opsReaperAgentId,
    });
  ...
  it("allows routine owner to PATCH status=cancelled on a leaked routine_execution issue", async () => {
  ...
  it("rejects routine owner attempting any field change beyond status=cancelled", async () => {
    const res = await request(await createApp(reaperActor()))
      .patch(`/api/issues/${issueId}`)
      .send({ status: "cancelled", title: "hijacked", priority: "low" });

    expect(res.status, JSON.stringify(res.body)).toBe(403);
    expect(res.body.error).toMatch(/Routine owner cancel carve-out/);
    expect(mockIssueService.update).not.toHaveBeenCalled();
  });

  it("rejects routine owner attempting status other than cancelled", async () => {
    const res = await request(await createApp(reaperActor()))
      .patch(`/api/issues/${issueId}`)
      .send({ status: "done" });
  ...
  it("rejects non-owner agent attempting carve-out", async () => {
    mockRoutineService.get.mockResolvedValue({
      id: routineId,
      companyId,
  ...
  it("rejects carve-out for non-routine_execution origin", async () => {

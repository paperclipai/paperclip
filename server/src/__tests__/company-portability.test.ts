...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "odysseus",
        name: "odysseus",
        description: "Odysseus coordination skill",
        markdown: "---\nname: odysseus\ndescription: Odysseus coordination skill\n---\n\n# Odysseus\n",
        sourceType: "github",
        sourceLocator: "https://github.com/PossibLaw/odysseus/tree/master/skills/odysseus",
  ...
  it("exports referenced skills as stubs by default with sanitized Odysseus extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Odysseus"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${odysseusKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/PossibLaw/odysseus/odysseus/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/PossibLaw/odysseus/odysseus/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/PossibLaw/odysseus/odysseus/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".odysseus.yaml"]);
    expect(extension).toContain('schema: "odysseus/v1"');
    ...
    expect(extension).not.toContain("odysseusSkillSync");
  ...
  it("exports hire approval policy only when approval is required", async () => {
    ...
    expect(asTextFile(exported.files[".odysseus.yaml"])).toContain("requireBoardApprovalForNewAgents: true");
  });
  ...
  it("exports default sidebar order into the Odysseus extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".odysseus.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/PossibLaw/odysseus/odysseus/SKILL.md"])).toContain("# Odysseus");
    expect(asTextFile(exported.files["skills/PossibLaw/odysseus/odysseus/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/PossibLaw/odysseus/odysseus/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/PossibLaw/odysseus/odysseus/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/PossibLaw/odysseus/odysseus/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .odysseus.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".odysseus.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-odysseus",
        companyId: "company-1",
        key: "PossibLaw/odysseus/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/PossibLaw/odysseus/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "odysseus_bundled",
          owner: "odysseus",
          repo: "odysseus",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/PossibLaw/odysseus/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/PossibLaw/odysseus/release-changelog/SKILL.md"])).toContain("PossibLaw/odysseus/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/odysseus",
            repoUrl: "https://github.com/PossibLaw/odysseus.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .odysseus-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/PossibLaw/odysseus.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/odysseus");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Odysseus",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/PossibLaw/odysseus.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".odysseus.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/PossibLaw/odysseus.git"');
    expect(extension).toContain('projectWorkspaceKey: "odysseus"');
  ...
  it("reads env inputs back from .odysseus.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("reads project env inputs back from .odysseus.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Odysseus routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "odysseus-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Odysseus" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Odysseus"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .odysseus.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Odysseus");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".odysseus.yaml"] = `${exported.files[".odysseus.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',

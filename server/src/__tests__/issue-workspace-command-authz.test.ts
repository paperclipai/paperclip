...
describe("issue workspace command authorization", () => {
  ...
  it("rejects agent callers that inject workspaceRuntime via executionWorkspaceSettings on create", async () => {
    const app = createApp({
      type: "agent",
      agentId: "agent-1",
      companyId: "company-1",
  ...
  it("rejects agent callers that inject workspaceRuntime via executionWorkspaceSettings on patch", async () => {
    mockIssueService.getById.mockResolvedValue(makeIssue());
    const app = createApp({
      type: "agent",
      agentId: "agent-1",

...
describe("agent local JWT", () => {
  const secretEnv = "ODYSSEUS_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "ODYSSEUS_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "ODYSSEUS_AGENT_JWT_ISSUER";
  const audienceEnv = "ODYSSEUS_AGENT_JWT_AUDIENCE";
  ...
  it("falls back to BETTER_AUTH_SECRET when ODYSSEUS_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "odysseus";
    process.env[audienceEnv] = "odysseus-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});

...
const capturePath = process.env.AITEAMCORP_TEST_CAPTURE_PATH;
const payload = {
  argv,
...
const capturePath = process.env.AITEAMCORP_TEST_CAPTURE_PATH;
const statePath = process.env.AITEAMCORP_TEST_STATE_PATH;
...
describe("claude execute", () => {
    ...
    try {
      await execute({
        ...
        config: {
          command: commandPath,
          cwd: workspace,
          env: { AITEAMCORP_TEST_CAPTURE_PATH: capturePath },
      ...
      const result = await execute({
        ...
        config: {
          ...
          env: {
            ...
            AITEAMCORP_TEST_STATE_PATH: statePath,
          },
          promptTemplate: "Do work.",
          instructionsFilePath: instructionsFile,
        },
  ...
  it("logs HOME, CLAUDE_CONFIG_DIR, and the resolved executable path in invocation metadata", async () => {
    ...
    try {
      ...
      expect(loggedEnv.AITEAMCORP_RESOLVED_COMMAND).toBe(commandPath);
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousPath === undefined) delete process.env.PATH;

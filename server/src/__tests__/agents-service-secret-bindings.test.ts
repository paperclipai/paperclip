...
describeEmbeddedPostgres("agent service secret binding sync", () => {
  ...
  it("converts external-plugin secret fields into persisted secret refs", async () => {
    const companyId = await seedCompany();
    const literalApiKey = `external-plugin-key-${randomUUID()}`;
    registerServerAdapter(externalSecretAdapter);
    try {
      const created = await agentService(db).create(companyId, {
        name: "External Plugin Agent",
        role: "engineer",
        status: "idle",
        adapterType: externalSecretAdapter.type,
        adapterConfig: {

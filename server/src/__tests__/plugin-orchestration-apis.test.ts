...
describeEmbeddedPostgres("plugin orchestration APIs", () => {
  ...
  afterEach(async () => {
    ...
    tempRoots.length = 0;
    // A comment-triggered wakeup can still be running when teardown begins.
    // Drain the shared heartbeat work first, then remove every fork/upstream
    // dependent in FK order.

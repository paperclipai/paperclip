...
describe("issue dependency wakeups in issue routes", () => {
  ...
  it("wakes dependents when the final blocker is soft-deleted (AGE-770)", async () => {
    mockIssueService.getById.mockResolvedValue({
      id: "issue-1",
      companyId: "company-1",
      identifier: "PAP-101",
      title: "Blocker to be deleted",
      description: null,
      status: "blocked",
      priority: "medium",
      parentId: null,
    ...
    mockIssueService.softDelete.mockResolvedValue({
      id: "issue-1",
      companyId: "company-1",
      identifier: "PAP-101",
      title: "Blocker to be deleted",
    ...
    const res = await request(await createApp()).delete("/api/issues/issue-1");
    expect(res.status).toBe(200);
    await vi.waitFor(() => {
      expect(mockWakeup).toHaveBeenCalledWith(
        "agent-2",

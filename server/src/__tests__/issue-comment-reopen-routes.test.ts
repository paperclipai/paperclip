...
describe.sequential("issue comment reopen routes", () => {
  ...
  // fork law: comments never implicitly reopen/resume closed or blocked issues — explicit reopen/resume intent required (4bde77244)
  it("explicitly reopens closed issues via the PATCH comment path when reassigning to an agent", async () => {
    ...
    // The PATCH route derives `previous`/`reopened` from the service's row-lock receipt
    // (`issue.changes`), so the mock must return a receipt, not a bare merged row.
    mockIssueService.update.mockImplementation(async (_id: string, patch: Record<string, unknown>) =>
  ...
  // fork law: comments never implicitly reopen/resume closed or blocked issues — explicit reopen/resume intent required (4bde77244)
  it("records board evidence comments on closed issues without reopening or waking the agent", async () => {
  ...
  it("moves assigned blocked issues back to todo via explicit POST resume", async () => {
  ...
  it("records comments on dependency-blocked issues without moving or waking them", async () => {
    mockIssueService.getById.mockResolvedValue(makeIssue("blocked"));
    mockIssueService.getDependencyReadiness.mockResolvedValue({
      issueId: "11111111-1111-4111-8111-111111111111",
  ...
  // fork law: comments never implicitly reopen/resume closed or blocked issues — explicit reopen/resume intent required (4bde77244).
  // Upstream's "still implicitly reopens a blocked issue via PATCH when the same request clears blockers" is
  // re-pinned here as the explicit-resume form plus its negative counterpart.
  it("does not implicitly reopen a blocked issue via PATCH when the same request clears blockers without explicit resume", async () => {
    mockIssueService.getById.mockResolvedValue(makeIssue("blocked"));
    mockIssueService.getRelationSummaries.mockResolvedValue({ blockedBy: [], blocks: [] });
    mockIssueService.getDependencyReadiness.mockResolvedValue({
      issueId: "11111111-1111-4111-8111-111111111111",
  ...
  it("moves a blocked issue to todo via explicit PATCH resume when the same request clears blockers", async () => {
    ...
    const res = await request(await installActor(createApp()))
      .patch("/api/issues/11111111-1111-4111-8111-111111111111")
      .send({ blockedByIssueIds: [], comment: "nothing left to wait on, please continue", resume: true });
  ...
  it("still implicitly reopens done issues via POST comments when the comment runId differs from the issue's owning run", async () => {
    ...
    const res = await request(await installActor(createApp(), {
      ...
      .post("/api/issues/11111111-1111-4111-8111-111111111111/comments")
      .send({ body: "Real human follow-up — please reopen", reopen: true });
  ...
  // fork law: comments never implicitly reopen/resume closed or blocked issues — explicit reopen/resume intent required (4bde77244)
  it("moves assigned blocked issues back to todo via explicit PATCH resume", async () => {
    const issue = makeIssue("blocked");
    mockIssueService.getById.mockResolvedValue(issue);
    // Receipt-shaped mock: the route reads `issue.changes` to decide `reopened`.
    mockIssueService.update.mockImplementation(async (_id: string, patch: Record<string, unknown>) =>
      makeIssueUpdateReceipt(issue, patch));

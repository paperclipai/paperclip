...
describeEmbeddedPostgres("heartbeat stale queued-run invalidation", () => {
  ...
  afterEach(async () => {
    ...
    // (Upstream introduced a cleanupHeartbeatInvalidationFixture(db) helper;
    // kept the explicit TRUNCATE here because the helper's per-table deletes
    // re-introduce the race PR #145 was specifically fixing.)

...
describeEmbeddedPostgres("heartbeat stale queued-run invalidation", () => {
  ...
  it("supersedes queued status-only recovery when a board unblock wakes its assignee", async () => {
    const { companyId, agentId } = await seedCompanyAndAgent();
    const issueId = randomUUID();
    await db.insert(issues).values({
      id: issueId,
      companyId,
      title: "Board-unblocked recovery source",
      status: "todo",
      priority: "high",
      assigneeAgentId: agentId,
    });
    ...
    expect(staleRun).toMatchObject({
      ...
      resultJson: {
        supersededRunWakeReason: "source_scoped_recovery_action",
        currentIssueStatus: "todo",
      },
    });
    // The issue-status wake is the intended replacement. The scheduler may
    // concurrently mint an equivalent normal `issue_assigned` wake, so accept
    // either normal wake as long as the cheap recovery is retired.
    expect(["issue_status_changed", "issue_assigned"]).toContain(
      (staleRun?.resultJson as { supersededByWakeReason?: string } | null)?.supersededByWakeReason,
    );
    expect(staleWakeup).toMatchObject({
      status: "cancelled",
      error: expect.stringMatching(/board unblock wake|fresh issue_assigned wake/),
    });
    expect(freshRun).toMatchObject({
      id: replacementRun?.id,

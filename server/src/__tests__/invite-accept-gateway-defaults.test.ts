...
describe("normalizeAgentDefaultsForJoin (external adapter)", () => {
  it("preserves a generic plugin payload without gateway-specific normalization", () => {
    const defaultsPayload = {
      endpoint: "https://adapter.example",
      apiKey: "plugin-key-1234567890",
    };
    const normalized = normalizeAgentDefaultsForJoin({
      adapterType: "external_secret_test",
      defaultsPayload,
    ...
    expect(normalized.normalized).toEqual(defaultsPayload);
  });
});

describeEmbeddedPostgres("prepareAgentDefaultsPayloadForJoinPersistence (external adapter)", () => {
  let stopDb: (() => Promise<void>) | null = null;
  let db!: ReturnType<typeof createDb>;
  const previousKeyFile = process.env.PAPERCLIP_SECRETS_MASTER_KEY_FILE;
  const secretsTmpDir = path.join(os.tmpdir(), `paperclip-external-join-defaults-${randomUUID()}`);

  beforeAll(async () => {
    registerServerAdapter(externalSecretAdapter);
    mkdirSync(secretsTmpDir, { recursive: true });
    process.env.PAPERCLIP_SECRETS_MASTER_KEY_FILE = path.join(secretsTmpDir, "master.key");
    const started = await startEmbeddedPostgresTestDatabase("external-join-defaults");
  ...
  it("stores a secret ref instead of the literal apiKey in join request defaults", async () => {
    ...
    await db.insert(joinRequests).values({
      ...
      requestIp: "127.0.0.1",
      agentName: "External Plugin Agent",
      adapterType: externalSecretAdapter.type,
      capabilities: "External plugin agent",

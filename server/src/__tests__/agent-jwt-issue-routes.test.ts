...
describeEmbeddedPostgres("system-attributed agent JWT issue authorization", () => {
  ...
  it("rejects an acceptance derived from a policy whose stage participants changed", async () => {
    const companyId = randomUUID();
    const coachId = randomUUID();
    const approverId = randomUUID();
    const replacementApproverId = randomUUID();
    const sourceRunId = randomUUID();
    const approverRunId = randomUUID();
    const issueId = randomUUID();
    const interactionId = randomUUID();
    ...
    await db.insert(companies).values({ id: companyId, name: "Stale Company", issuePrefix: "STALE" });
    await db.insert(agents).values([
      {
        id: coachId,
        companyId,
    ...
    const originalPolicy = {
      ...
      commentRequired: true,
      stages: [
        { id: approvalStageId, type: "approval" as const, approvalsNeeded: 1, participants: [{ type: "agent" as const, agentId: approverId }] },
      ],
    };
    const executionState = {
      ...
      currentParticipant: { type: "agent" as const, agentId: approverId },
      returnAssignee: { type: "agent" as const, agentId: coachId },
      reviewRequest: null,
      completedStageIds: [],
      lastDecisionId: null,
      lastDecisionOutcome: null,
          ...
          stages: [
            {
              ...
              type: "approval",
              approvalsNeeded: 1,
              participants: [{ type: "agent", agentId: replacementApproverId }],
            },
          ],
        },
      })
    ...
    await expect(issueThreadInteractionService(db).acceptInteraction(
      ...
      interactionId,
      {},
      { agentId: approverId, userId: null },
      {
        governedAcceptance: {
          ...
          decision: {
            ...
            stageType: "approval",
            outcome: "approved",
            body: `Accepted governed request_confirmation ${interactionId}`,
            createdByRunId: approverRunId,
          },
        },
      },
    )).rejects.toMatchObject({
      status: 409,
      message: "Issue execution policy changed while the decision was being applied",
    });
    ...
    expect(storedInteraction?.status).toBe("pending");
  });
});

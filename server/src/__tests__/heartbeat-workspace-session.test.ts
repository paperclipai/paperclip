...
describe("buildExplicitResumeSessionOverride", () => {
  ...
  it("truncates an over-long non-canonical run session id consistently in display and resume params", () => {
    // Contrived >128-char id on a non-Hermes (codex) codec. Real codex/claude
    // ids are UUIDs (<128) so this never truncates in practice; the canonical
    // session-id handling (upstream) keeps display and params in lockstep, and
    // full Hermes ids are preserved via the requiresCanonicalSessionIds path.
    ...
    expect(result?.sessionParams).toEqual({
      sessionId: longSessionId.slice(0, 128),
    });
  });

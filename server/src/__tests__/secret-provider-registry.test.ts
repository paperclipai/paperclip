...
describe("secret provider registry", () => {
  ...
  const previousMasterKey = process.env.ODYSSEUS_SECRETS_MASTER_KEY;
  ...
  afterEach(() => {
    ...
    if (previousMasterKey === undefined) {
      ...
      process.env.ODYSSEUS_SECRETS_MASTER_KEY = previousMasterKey;
    }
  ...
  it("warns when the local encrypted key file is readable by group or others", async () => {
    const dir = path.join(os.tmpdir(), `odysseus-secret-provider-${randomBytes(6).toString("hex")}`);
    tmpDirs.push(dir);
    ...
    process.env.ODYSSEUS_SECRETS_MASTER_KEY_FILE = keyFile;
    delete process.env.ODYSSEUS_SECRETS_MASTER_KEY;

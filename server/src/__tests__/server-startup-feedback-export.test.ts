...
const ORIGINAL_ODYSSEUS_RUNTIME_API_CANDIDATES_JSON = process.env.ODYSSEUS_RUNTIME_API_CANDIDATES_JSON;
...
function buildTestConfig(overrides: Record<string, unknown> = {}) {
  return {
    ...
    databaseUrl: "postgres://odysseus:odysseus@127.0.0.1:5432/odysseus",
    embeddedPostgresDataDir: "/tmp/odysseus-test-db",
    embeddedPostgresPort: 54329,
    databaseBackupEnabled: false,
    databaseBackupIntervalMinutes: 60,
    databaseBackupRetentionDays: 30,
    databaseBackupDir: "/tmp/odysseus-test-backups",
    serveUi: false,
    uiDevMiddleware: false,
    secretsProvider: "local_encrypted",
    secretsStrictMode: false,
    secretsMasterKeyFilePath: "/tmp/odysseus-master.key",
    storageProvider: "local_disk",
    storageLocalDiskBaseDir: "/tmp/odysseus-storage",
    storageS3Bucket: "odysseus-test",
...
describe("startServer ODYSSEUS_API_URL handling", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    loadConfigMock.mockReturnValue(buildTestConfig());
    process.env.BETTER_AUTH_SECRET = "test-secret";
  ...
  afterEach(() => {
    ...
    if (ORIGINAL_ODYSSEUS_RUNTIME_API_CANDIDATES_JSON === undefined) {
      delete process.env.ODYSSEUS_RUNTIME_API_CANDIDATES_JSON;
    } else {
      process.env.ODYSSEUS_RUNTIME_API_CANDIDATES_JSON = ORIGINAL_ODYSSEUS_RUNTIME_API_CANDIDATES_JSON;
    }
  ...
  it("uses the externally set ODYSSEUS_API_URL when provided", async () => {
    process.env.ODYSSEUS_API_URL = "http://custom-api:3100";

    const started = await startServer();

    expect(started.apiUrl).toBe("http://custom-api:3100");
    expect(process.env.ODYSSEUS_API_URL).toBe("http://custom-api:3100");
    expect(JSON.parse(process.env.ODYSSEUS_RUNTIME_API_CANDIDATES_JSON ?? "[]")).toEqual(
      expect.arrayContaining(["http://custom-api:3100"]),
    );
    expect(JSON.parse(process.env.ODYSSEUS_RUNTIME_API_CANDIDATES_JSON ?? "[]")[0]).toBe("http://custom-api:3100");
  });

  it("falls back to host-based URL when ODYSSEUS_API_URL is not set", async () => {
    const started = await startServer();

    expect(started.apiUrl).toBe("http://127.0.0.1:3210");
    expect(process.env.ODYSSEUS_API_URL).toBe("http://127.0.0.1:3210");
  });
  ...
  it("rewrites explicit-port auth public URLs when detect-port selects a new port", async () => {
    ...
    expect(process.env.ODYSSEUS_RUNTIME_API_URL).toBe("http://my-host.ts.net:3110");
  });
  ...
  it("keeps no-port auth public URLs stable when detect-port selects a new port", async () => {
    loadConfigMock.mockReturnValueOnce(buildTestConfig({
      port: 3100,
      authBaseUrlMode: "explicit",
      authPublicBaseUrl: "https://odysseus.example",
    }));
    detectPortMock.mockResolvedValueOnce(3110);
    ...
    expect(started.listenPort).toBe(3110);
    expect(started.apiUrl).toBe("https://odysseus.example");
    expect(process.env.ODYSSEUS_RUNTIME_API_URL).toBe("https://odysseus.example");
  });
});

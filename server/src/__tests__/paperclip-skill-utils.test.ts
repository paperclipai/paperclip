...
describe("paperclip skill utils", () => {
  ...
  it("keeps MCP approval requests in the Paperclip control-plane workflow", async () => {
    ...
    expect(paperclip).toMatchObject({ runtimeName: "paperclip" });
    expect(paperclip).not.toHaveProperty("required");

    const markdown = await fs.readFile(path.join(paperclip!.source, "SKILL.md"), "utf8");
    expect(markdown).toContain("## MCP Tool Approval Gates");
    expect(markdown).toContain("approval_required");
    expect(markdown).toContain("approve and run");
    expect(markdown).toContain("approval_path_missing");
    expect(markdown).not.toContain("/mcp-servers");
  });

...
const payload = {
  ...
  aiteamcorpWakePayloadJson: process.env.AITEAMCORP_WAKE_PAYLOAD_JSON || null,
...
describe("codex execute", () => {
  it("uses a Paperclip-managed CODEX_HOME outside worktree mode while preserving shared auth and config", async () => {
    ...
    delete process.env.AITEAMCORP_IN_WORKTREE;
  ...
  it("injects structured AiTeamCorp wake payloads into env and prompt", async () => {
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.aiteamcorpEnvKeys).toContain("AITEAMCORP_WAKE_PAYLOAD_JSON");
  ...
  it("uses a worktree-isolated CODEX_HOME while preserving shared auth and config", async () => {
    ...
    process.env.HOME = root;
    process.env.AITEAMCORP_HOME = aiteamcorpHome;
    process.env.AITEAMCORP_INSTANCE_ID = "worktree-1";

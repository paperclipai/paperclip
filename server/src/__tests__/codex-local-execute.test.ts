...
async function writeFakeCodexCommand(basePath: string): Promise<string> {
...
`;
  ...
  if (process.platform === "win32") {
    const wrapperPath = `${basePath}.cmd`;
...
describe("codex execute", () => {
  ...
  it("falls back to a writable Codex home when managed home cannot be written", async () => {
    ...
    const fallbackCodexHome = path.join(
      workspace,
      ".paperclip",
      "codex-home-fallback",
      "company-1",
    ...
    const mkdirSpy = vi.spyOn(fs, "mkdir").mockImplementation(async (target, options) => {
      if (target === managedCodexHome) {
        throw Object.assign(new Error(`EACCES: permission denied, mkdir '${managedCodexHome}'`), {
          code: "EACCES",
          errno: -4048,
          syscall: "mkdir",
      ...
      return originalMkdir(target as string, options as Parameters<typeof fs.mkdir>[1]);
    });
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.codexHome).toBe(fallbackCodexHome);
      expect((await fs.lstat(path.join(fallbackCodexHome, "auth.json"))).isFile()).toBe(true);
      expect(await fs.readFile(path.join(fallbackCodexHome, "config.toml"), "utf8")).toBe('model = "codex-mini-latest"\n');
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining(`switching to writable Codex home "${fallbackCodexHome}"`),
        }),
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;

...
async function insertIssue(
  ...
  await db.insert(issues).values({
    id: issueId,
    companyId: input.companyId,
    title: input.title ?? "Review implementation handoff",
...
async function insertEvidenceComment(
  db: ReturnType<typeof createDb>,
  input: {
    companyId: string;
    issueId: string;
  ...
  await db.insert(issueComments).values({
    ...
    body: "Verification evidence: focused tests passed and PR is linked.",
    createdAt: input.createdAt ?? NOW,
...
describeEmbeddedPostgres("agent service health", () => {
  ...
  it("flags stale agent-owned in-review issues without active runs, evidence, or pending wakeups", async () => {
    const { companyId, agentId } = await insertCompanyAgent(db);
    await insertRun(db, { companyId, agentId, status: "succeeded" });
    const issueId = await insertIssue(db, {
      companyId,
      agentId,
  ...
  it("does not flag recent, execution-managed, active-run, evidence-backed, or wakeup-backed in-review issues", async () => {
    const { companyId, agentId } = await insertCompanyAgent(db);
    await insertRun(db, { companyId, agentId, status: "succeeded" });
    ...
    const activeIssueId = await insertIssue(db, {
      companyId,
      agentId,
      updatedAt: new Date(NOW.getTime() - 16 * 60 * 1000),
      identifier: "RUN-1",
    ...
    const evidenceIssueId = await insertIssue(db, {
      companyId,
      agentId,
      updatedAt: new Date(NOW.getTime() - 16 * 60 * 1000),
      identifier: "EVIDENCE-1",
    });
    await insertEvidenceComment(db, {
      companyId,
      agentId,
      issueId: evidenceIssueId,
      createdAt: new Date(NOW.getTime() - 2 * 60 * 1000),
    });
    const wakeupBackedIssueId = await insertIssue(db, {
      companyId,
      agentId,
      updatedAt: new Date(NOW.getTime() - 16 * 60 * 1000),
      identifier: "WAKE-1",
  ...
  it("flags recently done agent-owned issues without visible evidence comments", async () => {
    const { companyId, agentId } = await insertCompanyAgent(db);
    await insertRun(db, { companyId, agentId, status: "succeeded" });
    const issueId = await insertIssue(db, {
      companyId,
    ...
    expect(health.status).toBe("down");
    expect(health.reason).toBe("agent_completion_gaps");
    expect(health.counts.completionGapIssueCount).toBe(1);
    expect(health.boardIssueWarnings).toEqual([
      expect.objectContaining({
        kind: "completion_gap",
        issueId,
  ...
  it("does not flag done claims with agent evidence comments", async () => {
    const { companyId, agentId } = await insertCompanyAgent(db);
    await insertRun(db, { companyId, agentId, status: "succeeded" });
    const issueId = await insertIssue(db, {
      companyId,
    ...
    await insertEvidenceComment(db, { companyId, agentId, issueId });
    ...
    expect(health.status).toBe("healthy");
    expect(health.reason).toBeNull();
    expect(health.counts.completionGapIssueCount).toBe(0);
  });

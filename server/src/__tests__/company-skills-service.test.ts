...
describeEmbeddedPostgres("companySkillService.list", () => {
  ...
  it("keeps unchanged bundled inventory rows stable and scopes required runtime skills", async () => {
    const companyId = randomUUID();
    await db.insert(companies).values({
      id: companyId,
      name: "Paperclip",
    ...
    await svc.list(companyId);
    const paperclip = await db
      .select()
      .from(companySkills)
      .where(eq(companySkills.key, "paperclipai/paperclip/paperclip"))
      .then((rows) => rows.find((row) => row.companyId === companyId) ?? null);
    expect(paperclip?.metadata).toMatchObject({
      sourceKind: "paperclip_bundled",
      required: true,
    });
    const paperclipCreateAgent = await db
      .select()
      .from(companySkills)
      .where(eq(companySkills.key, "paperclipai/paperclip/paperclip-create-agent"))
      .then((rows) => rows.find((row) => row.companyId === companyId) ?? null);
    expect(paperclipCreateAgent?.metadata).toMatchObject({
      sourceKind: "paperclip_bundled",
      required: false,
    });
    ...
    await db
      ...
      .where(eq(companySkills.id, paperclip!.id));
    ...
    expect(runtimeEntries.find((entry) => entry.key === paperclip!.key)).toMatchObject({
      required: true,
      requiredReason: expect.any(String),
    });
    expect(runtimeEntries.find((entry) => entry.key === paperclipCreateAgent!.key)).toMatchObject({
      required: false,
      requiredReason: null,
    });

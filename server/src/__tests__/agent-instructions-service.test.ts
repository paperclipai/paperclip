...
describe("agent instructions service", () => {
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const valadrienOsHome = await makeTempDir("valadrien-os-agent-instructions-home-");
    const externalRoot = await makeTempDir("valadrien-os-agent-instructions-external-");
    cleanupDirs.add(valadrienOsHome);
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("valadrien-os-agent-instructions-new-external-");
    cleanupDirs.add(valadrienOsHome);
    cleanupDirs.add(externalRoot);
    process.env.VALADRIEN_OS_HOME = valadrienOsHome;
    process.env.VALADRIEN_OS_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("valadrien-os-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const valadrienOsHome = await makeTempDir("valadrien-os-agent-instructions-recover-");
    cleanupDirs.add(valadrienOsHome);
    process.env.VALADRIEN_OS_HOME = valadrienOsHome;
    process.env.VALADRIEN_OS_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const valadrienOsHome = await makeTempDir("valadrien-os-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("valadrien-os-agent-instructions-stale-root-");
    cleanupDirs.add(valadrienOsHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const valadrienOsHome = await makeTempDir("valadrien-os-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("valadrien-os-agent-instructions-heal-write-stale-");
    cleanupDirs.add(valadrienOsHome);
    cleanupDirs.add(staleRoot);
    process.env.VALADRIEN_OS_HOME = valadrienOsHome;
    process.env.VALADRIEN_OS_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const valadrienOsHome = await makeTempDir("valadrien-os-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("valadrien-os-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(valadrienOsHome);
    cleanupDirs.add(staleRoot);
    process.env.VALADRIEN_OS_HOME = valadrienOsHome;
    process.env.VALADRIEN_OS_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const valadrienOsHome = await makeTempDir("valadrien-os-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("valadrien-os-agent-instructions-partial-root-");
    cleanupDirs.add(valadrienOsHome);
    cleanupDirs.add(staleRoot);
    process.env.VALADRIEN_OS_HOME = valadrienOsHome;
    process.env.VALADRIEN_OS_INSTANCE_ID = "test-instance";

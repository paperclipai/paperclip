...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ironworks-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "ironworks@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "Ironworks Test"]);
...
afterEach(async () => {
  ...
  delete process.env.IRONWORKS_WORKTREES_DIR;
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".ironworks", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$IRONWORKS_WORKSPACE_BRANCH\" > .ironworks-provision-branch",
        "printf '%s\\n' \"$IRONWORKS_WORKSPACE_BASE_CWD\" > .ironworks-provision-base",
        "printf '%s\\n' \"$IRONWORKS_WORKSPACE_CREATED\" > .ironworks-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".ironworks-provision-branch"), "utf8")).resolves.toBe(
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".ironworks-provision-created"), "utf8")).resolves.toBe("false\n");
  });

  it("writes an isolated repo-local Ironworks config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    const previousCwd = process.cwd();
    const ironworksHome = await fs.mkdtemp(path.join(os.tmpdir(), "ironworks-worktree-home-"));
    const isolatedWorktreeHome = await fs.mkdtemp(path.join(os.tmpdir(), "ironworks-worktrees-"));
    const instanceId = "worktree-base";
    const sharedConfigDir = path.join(ironworksHome, "instances", instanceId);
    const sharedConfigPath = path.join(sharedConfigDir, "config.json");
    const sharedEnvPath = path.join(sharedConfigDir, ".env");

    process.env.IRONWORKS_HOME = ironworksHome;
    process.env.IRONWORKS_INSTANCE_ID = instanceId;
    process.env.IRONWORKS_WORKTREES_DIR = isolatedWorktreeHome;
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".ironworks", "config.json");
      const envPath = path.join(workspace.cwd, ".ironworks", ".env");
      ...
      expect(envContents).toContain(`IRONWORKS_HOME=${JSON.stringify(isolatedWorktreeHome)}`);
...
describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ironworks-runtime-workspace-"));
  ...
  it("does not leak parent Ironworks instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ironworks-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "ironworksConfig: process.env.IRONWORKS_CONFIG ?? null,",
          "ironworksHome: process.env.IRONWORKS_HOME ?? null,",
          "ironworksInstanceId: process.env.IRONWORKS_INSTANCE_ID ?? null,",
    ...
    process.env.IRONWORKS_CONFIG = "/tmp/base-ironworks-config.json";
    process.env.IRONWORKS_HOME = "/tmp/base-ironworks-home";
    process.env.IRONWORKS_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/ironworks";
    ...
    expect(captured.ironworksConfig).toBeNull();
    expect(captured.ironworksHome).toBeNull();
    expect(captured.ironworksInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ironworks-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "ironworks-runtime-sibling-"));

...
describeEmbeddedPostgres("companySearchService", () => {
  ...
  it("searches issue documents and returns document metadata for snippets", async () => {
    ...
    await db.insert(documents).values({
      id: documentId,
      companyId,
      title: "External Adapter Parser Plan",
    ...
    const result = await svc.search(companyId, companySearchQuerySchema.parse({ q: "external parser", scope: "documents" }));
    ...
    expect(result.results[0]?.snippets[0]).toMatchObject({ field: "document", label: "External Adapter Parser Plan" });

...
function seedKnowledgeEntry(
  ...
  const lines = [
    ...
    `domain: ${entry.domain}`,
    `outcome: done`,
    `summary: "${entry.yamlSummary ?? entry.summary}"`,
    `decided_at: ${entry.decidedAt}`,
    `digest_model: test-model`,
    `digest_version: 1`,
    `source: manual`,
  ];
  const yamlAntiPatterns = entry.yamlAntiPatterns ?? entry.antiPatterns;
  if (yamlAntiPatterns && yamlAntiPatterns.length > 0) {
    lines.push("anti_patterns:");
    for (const ap of yamlAntiPatterns) lines.push(`  - "${ap}"`);
  }
...
describe.sequential("heartbeat-context priorRunKnowledge — contract", () => {
  ...
  it("returns 5 priorRunKnowledge entries for SSI Director, newest-first", async () => {
    ...
    const entries = [
      ...
      { taskId: "33333333-0000-4000-8000-000000000003", identifier: "SAG-1003", domain: "ssi-hp", summary: "Index summary is stale", yamlSummary: "YAML canonical summary", decidedAt: "2026-05-01T00:00:00.000Z", antiPatterns: ["Index AP is stale"], yamlAntiPatterns: ["YAML canonical AP"] },
    ...
    expect(entry3.summary).toBe("YAML canonical summary");
    expect(entry3.antiPatterns).toEqual(["YAML canonical AP"]);
  });
  ...
  it("falls back to JSONL metadata for absent fields and reads writer block scalars", () => {
    const identifier = "SAG-2001";
    const decidedAt = "2026-09-01T00:00:00.000Z";
    seedKnowledgeEntry(tmpDir, {
      taskId: "aaaaaaaa-0000-4000-8000-000000002001",
      identifier,
      domain: "ssi-hp",
      summary: "Index fallback summary",
      antiPatterns: ["Index fallback anti-pattern"],
      decidedAt,
    });

    const yamlPath = path.join(tmpDir, "tasks", "2026", "09", `${identifier}.yaml`);
    fs.writeFileSync(
      yamlPath,
      [
        "task_id: aaaaaaaa-0000-4000-8000-000000002001",
    ...
    expect(
      readPriorRunKnowledge("company-1", "ssi_director", "ssi-hp", "SAG-9999", {
        knowledgeDir: tmpDir,
        limit: 1,
      }),
    ).toEqual([
      {
        ...
        antiPatterns: ["Index fallback anti-pattern"],
        decisions: ["Choose the first path before changing scope"],
        link: "/SAG/issues/SAG-2001",
      },
    ]);
  });
});
...
describe.sequential("heartbeat-context priorRunKnowledge — performance", () => {
  ...
  it("reads up to 5 entries from 100-entry seed in < 100ms delta", async () => {
    ...
    const endpoint = "/api/issues/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa/heartbeat-context";
    const sampleCount = 9;
    const median = (values: number[]) => values.sort((a, b) => a - b)[Math.floor(values.length / 2)];
    const measure = async (app: express.Express) => {
      const values: number[] = [];
    ...
    // Warm both route and filesystem paths before collecting repeated samples.
    await request(baselineApp).get(endpoint);
    mockIssueService.getById.mockResolvedValue(ssiDirectorIssue());
    mockProjectService.getById.mockResolvedValue(ssiProject());
    await request(fullApp).get(endpoint);

    const baselineSamples = await measure(baselineApp);
    const fullSamples = await measure(fullApp);
    const p50Delta = median(fullSamples) - median(baselineSamples);
    const res = await request(fullApp).get(endpoint);

    expect(res.status).toBe(200);
    expect(res.body.priorRunKnowledge).toHaveLength(5); // top 5 only
    console.info(`SAG-2189 priorRunKnowledge p50 delta: ${p50Delta.toFixed(2)}ms`);
    expect(p50Delta).toBeLessThan(100);
  });
});

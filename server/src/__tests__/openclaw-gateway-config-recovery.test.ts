...
import {
  ...
  shouldRecoverEmptyAdapterConfig,
...
describeEmbeddedPostgres("openclaw_gateway adapterConfig wedge recovery (PHA-3517)", () => {
  ...
  it("shouldRecoverEmptyAdapterConfig returns true only for unknown source", () => {
    // Wedge path: unknown source means the race condition fired and
    // recovery should restore the last known-good config.
    expect(shouldRecoverEmptyAdapterConfig("unknown")).toBe(true);

    // Operator paths: any known clear/revoke/replace source means the
    // empty config is intentional and recovery must NOT revert it.
    expect(shouldRecoverEmptyAdapterConfig("operator_replace")).toBe(false);
    expect(shouldRecoverEmptyAdapterConfig("agent_self_clear")).toBe(false);
    expect(shouldRecoverEmptyAdapterConfig("revocation")).toBe(false);
    expect(shouldRecoverEmptyAdapterConfig("test_fixture")).toBe(false);
  });
});

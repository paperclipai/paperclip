...
describeEmbeddedPostgres("heartbeat plugin environments", () => {
  ...
  it("acquires plugin environment leases through the heartbeat execution path", async () => {
    ...
    const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), "odysseus-plugin-env-heartbeat-"));
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey,
      packageName: "@acme/odysseus-environments",
      version: "1.0.0",
      apiVersion: 1,
  ...
  it("ignores stale non-reused workspace environment config in favor of the issue selection", async () => {
    ...
    const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), "odysseus-plugin-env-issue-"));
    tempRoots.push(workspaceRoot);
    const workerManager = {
      isRunning: vi.fn((id: string) => id === pluginId),

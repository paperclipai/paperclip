...
describe.sequential("issue thread interaction routes", () => {
  beforeEach(() => {
    ...
    const cancelledQuestionInteraction = {
      id: "interaction-2",
      companyId: "company-1",
      issueId: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",
      kind: "ask_user_questions",
    ...
    mockInteractionService.cancelInteraction.mockResolvedValue(cancelledQuestionInteraction);
    mockInteractionService.cancelQuestions.mockResolvedValue(cancelledQuestionInteraction);
  ...
  it("rejects watchdog-scoped and low-trust agents from cancelling interactions", async () => {
    mockResolveTaskWatchdogMutationScope.mockResolvedValueOnce({
      kind: "watchdog",
      watchdogId: "watchdog-1",
      companyId: "company-1",
    ...
    const watchdogApp = await createApp({
      type: "agent",
      agentId: ASSIGNEE_AGENT_ID,
      companyId: "company-1",
      runId: "run-watchdog-cancel",
    });
    const watchdog = await request(watchdogApp)
      .post("/api/issues/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa/interactions/interaction-2/cancel")
      .send({ reason: "No longer needed" });
    expect(watchdog.status).toBe(403);
    expect(watchdog.body.error).toContain("Task-watchdog");

    mockResolveCoreTrustPreset.mockReturnValueOnce({ kind: "low_trust_review" });
    const lowTrustApp = await createApp({
      type: "agent",
      agentId: ASSIGNEE_AGENT_ID,
      companyId: "company-1",
      runId: "run-low-trust-cancel",
    });

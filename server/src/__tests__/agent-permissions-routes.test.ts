...
import { DEFAULT_OPENCODE_LOCAL_MODEL } from "@odysseus/adapter-opencode-local";
...
describe.sequential("agent permission routes", () => {
  beforeEach(() => {
    ...
    vi.doUnmock("@odysseus/adapter-opencode-local/server");
    registerModuleMocks();
    vi.resetAllMocks();
    ...
    const res = await requestApp(app, (baseUrl) => request(baseUrl)
      ...
      .send({
        adapterConfig: {
          workspaceStrategy: {
            type: "git_worktree",
            provisionCommand: "touch /tmp/odysseus-rce",
          },
        },
      }));

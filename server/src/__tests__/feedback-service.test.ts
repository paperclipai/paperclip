...
import { writeValadrienOsSkillSyncPreference } from "@valadrien-os/adapter-utils/server-utils";
import {
  agents,
  companies,
  companySkills,
...
describe("feedbackService.saveIssueVote", () => {
  ...
  beforeAll(async () => {
    const started = await startEmbeddedPostgresTestDatabase("valadrien-os-feedback-service-");
  ...
  async function seedIssueWithRichAgentComment() {
    ...
    const instructionsDir = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-feedback-instructions-"));
    ...
    await db.insert(agents).values({
      ...
      status: "active",
      adapterType: "codex_local",
      adapterConfig: writeValadrienOsSkillSyncPreference(
        {
          model: "gpt-5.4",
  ...
  it("builds a detailed sanitized shared bundle with issue and agent context", async () => {
    ...
    expect(trace?.schemaVersion).toBe("valadrien-os-feedback-envelope-v2");
    expect(trace?.bundleVersion).toBe("valadrien-os-feedback-bundle-v2");
  ...
  it("keeps earlier local votes local when a later vote enables sharing", async () => {
    ...
    expect(localTrace?.payloadVersion).toBe("valadrien-os-feedback-v1");
    ...
    expect(sharedTrace?.payloadVersion).toBe("valadrien-os-feedback-v1");
  });

  it("captures Claude project session artifacts as full traces", async () => {
    const claudeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-feedback-claude-"));
  ...
  it("captures OpenCode message and part files as full traces", async () => {
    const opencodeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-feedback-opencode-"));
    ...
    vi.stubEnv("VALADRIEN_OS_OPENCODE_STORAGE_DIR", opencodeRoot);
    const uploadTraceBundle = vi.fn().mockResolvedValue({ objectKey: "feedback-traces/test.json" });
    const flushingSvc = feedbackService(db, {
      shareClient: {
        uploadTraceBundle,
  ...
  it("flushes pending shared traces into configured object storage and marks them sent", async () => {
    ...
    expect(uploadTraceBundle.mock.calls[0]?.[0]).toMatchObject({
      ...
      envelope: {
        destination: "valadrien_os_labs_feedback_v1",
        exportId: traces[0]?.exportId,
      },
    });
  });

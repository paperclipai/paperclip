...
import { execute } from "@odysseus/adapter-gemini-local/server";
...
const payload = {
  ...
  odysseusEnvKeys: Object.keys(process.env)
    .filter((key) => key.startsWith("ODYSSEUS_"))
    .sort(),
};
if (capturePath) {
...
type CapturePayload = {
  argv: string[];
  odysseusEnvKeys: string[];
};

describe("gemini execute", () => {
  it("passes prompt via --prompt and injects odysseus env vars", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-gemini-execute-"));
    ...
    try {
      ...
      expect(promptArg).toContain("Follow the odysseus heartbeat.");
      expect(promptArg).toContain("Odysseus runtime note:");
      expect(capture.odysseusEnvKeys).toEqual(
      ...
      expect(invocationPrompt).toContain("Odysseus runtime note:");
      expect(invocationPrompt).toContain("ODYSSEUS_API_URL");
      expect(invocationPrompt).toContain("Odysseus API access note:");
  ...
  it("always passes --approval-mode yolo", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-gemini-yolo-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("normalizes turn-limit exhaustion into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-gemini-max-turns-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("normalizes Gemini exit code 53 as max-turn exhaustion", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-gemini-exit-53-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingGeminiCommand(commandPath, {
  ...
  it("does not normalize unstructured turn-limit text into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-gemini-max-turn-text-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingGeminiCommand(commandPath, {
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-gemini-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(promptArg).toContain("## Odysseus Resume Delta");
      ...
      expect(promptArg).not.toContain("Follow the odysseus heartbeat.");
    } finally {
      if (previousHome === undefined) {
        delete process.env.HOME;
      } else {

...
import { isCursorUnknownSessionError, parseCursorJsonl } from "@aiteamcorp/adapter-cursor-local/server";
import { parseCursorStdoutLine } from "@aiteamcorp/adapter-cursor-local/ui";
import { printCursorStreamEvent } from "@aiteamcorp/adapter-cursor-local/cli";
...
describe("cursor ui stdout parser", () => {
  ...
  it("compacts shellToolCall and shell tool result for run log", () => {
    const ts = "2026-03-05T00:00:00.000Z";
    const longCommand = "curl -s -X POST \"$AITEAMCORP_API_URL/api/issues/abc/checkout\" -H \"Authorization: Bearer $AITEAMCORP_API_KEY\"";

...
const mockIssueService = vi.hoisted(() => ({
  ...
  // Board-action requirements (local feature) — the issue list route hydrates
  // these for every row; default to an empty map / passthrough title so route
  // tests that don't exercise board actions still succeed.
  getBoardActionRequirements: vi.fn(async () => new Map()),
  applyBoardActionTitlePrefix: vi.fn((title: string) => title),
}));

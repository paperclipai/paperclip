...
function createRunContextDb(
  ...
  const buildTransactionQuery = (selection: Record<string, unknown>) => {
    const keys = Object.keys(selection);
    const rows = keys.includes("role") || keys.includes("capabilities")
      ? [{ id: ownerAgentId, status: "idle", role: "engineer", capabilities: {} }]
      : keys.length === 0
        ? [{
            id: recoveryActionId,
            companyId,
    ...
    lockable.limit.mockImplementation(() => lockable);
    const query = {
      innerJoin: vi.fn(() => query),
      leftJoin: vi.fn(() => query),
      where: vi.fn(() => ({
        ...lockable,
        orderBy: vi.fn(() => lockable),
      })),
    };
    return query;
  };
  return {
    transaction: async (callback: (tx: { select: (selection?: Record<string, unknown>) => unknown }) => Promise<unknown>) => callback({
      select: (selection: Record<string, unknown> = {}) => ({
        from: () => buildTransactionQuery(selection),
      }),
    }),
...
describe("agent issue mutation checkout ownership", () => {
  beforeEach(() => {
    ...
    mockIssueThreadInteractionService.cancelPendingForTerminalIssue.mockReset();
    ...
    // Root listing returns the issue, while the coordination-lane guard's
    // direct-child query returns no child rows for this fixture.
    mockIssueService.list.mockImplementation(async (_companyId: string, options?: { parentId?: string }) =>
      options?.parentId ? [] : [makeIssue()],
    );

...
function manifest(): OdysseusPluginManifestV1 {
  return {
    id: "odysseus.managed-agents-test",
    apiVersion: 1,
    version: "0.1.0",
...
describeEmbeddedPostgres("plugin-managed agents", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("odysseus-plugin-managed-agents-");
  ...
  async function seedCompanyAndPlugin(options: { requireApproval?: boolean; manifest?: OdysseusPluginManifestV1 } = {}) {
    ...
    await db.insert(plugins).values({
      ...
      packageName: "@odysseus/plugin-managed-agents-test",
  ...
  it("materializes declared managed agent instructions with local folder paths", async () => {
    const previousHome = process.env.ODYSSEUS_HOME;
    const previousInstance = process.env.ODYSSEUS_INSTANCE_ID;
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-managed-agent-home-"));
    const wikiRoot = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-managed-agent-wiki-")));
    process.env.ODYSSEUS_HOME = tempHome;
    process.env.ODYSSEUS_INSTANCE_ID = "test";
    try {
      const pluginManifest = manifest();
      ...
      if (previousHome === undefined) delete process.env.ODYSSEUS_HOME;
      else process.env.ODYSSEUS_HOME = previousHome;
      if (previousInstance === undefined) delete process.env.ODYSSEUS_INSTANCE_ID;
      else process.env.ODYSSEUS_INSTANCE_ID = previousInstance;
  ...
  it("respects board approval policy for new managed agents", async () => {
    ...
    expect(approval?.payload).toMatchObject({
      agentId: created.agentId,
      sourcePluginKey: "odysseus.managed-agents-test",

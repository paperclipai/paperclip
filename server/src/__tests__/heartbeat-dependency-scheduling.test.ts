...
import { issueTreeControlService } from "../services/issue-tree-control.ts";
...
describeEmbeddedPostgres("heartbeat dependency-aware queued run selection", () => {
  ...
  it("rejects a child cancel beneath an ancestor cancel, then safely restores and wakes the child", async () => {
    const companyId = randomUUID();
    const agentId = randomUUID();
    const rootIssueId = randomUUID();
    ...
    await db.insert(companies).values({
      id: companyId,
      name: "Cancel restore wake company",
      issuePrefix: `T${companyId.replace(/-/g, "").slice(0, 6).toUpperCase()}`,
      requireBoardApprovalForNewAgents: false,
    });
    await db.insert(agents).values({
    ...
    const ancestorCancel = await treeControl.createHold(companyId, rootIssueId, { mode: "cancel", actor });
    await expect(treeControl.createHold(companyId, childIssueId, { mode: "cancel", actor }))
    ...
    await treeControl.createHold(companyId, rootIssueId, { mode: "restore", actor });
    await expect(treeControl.getActiveCancelHoldGate(companyId, childIssueId)).resolves.toBeNull();

    const wake = await heartbeat.wakeup(agentId, {
      source: "assignment",
      triggerDetail: "system",
      reason: "issue_tree_restored",
      payload: { issueId: childIssueId, rootIssueId },
      contextSnapshot: {
    ...
    expect(mockAdapterExecute.mock.calls.some(([context]) => context?.runId === wake!.id)).toBe(true);
  });

  it("denies every wake reason, including comment interaction, under an active cancel hold", async () => {
    const companyId = randomUUID();
    const agentId = randomUUID();
    const rootIssueId = randomUUID();
    const childIssueId = randomUUID();
    ...
    await db.insert(companies).values({
      id: companyId,
      name: "Cancel hold wake gate company",
      issuePrefix: `T${companyId.replace(/-/g, "").slice(0, 6).toUpperCase()}`,
      requireBoardApprovalForNewAgents: false,
    });
    await db.insert(agents).values({
    ...
    await db.insert(issueComments).values({
      ...
      issueId: childIssueId,
      authorUserId: "board-user",
      body: "This comment must not bypass cancellation.",
    });
    ...
    const wake = await heartbeat.wakeup(agentId, {
      ...
      triggerDetail: "system",
      reason: "issue_commented",
      payload: { issueId: childIssueId, commentId },
      requestedByActorType: "user",
      requestedByActorId: "board-user",
      contextSnapshot: {
        issueId: childIssueId,
    ...
    const skipped = await db
      ...
      .then((rows) => rows[0] ?? null);
    expect(skipped).toMatchObject({ status: "skipped", reason: "issue_tree_cancel_hold_active" });
    expect(await issueTreeControlService(db).getActiveCancelHoldGate(companyId, childIssueId))
      .toMatchObject({ holdId: hold.id, rootIssueId, mode: "cancel" });
    await expect(db
      .select({ id: heartbeatRuns.id })
      .from(heartbeatRuns)
      .where(sql`${heartbeatRuns.contextSnapshot} ->> 'issueId' = ${childIssueId}`))

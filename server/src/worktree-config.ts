...
import type { OdysseusConfig } from "@odysseus/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Odysseus worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.ODYSSEUS_IN_WORKTREE !== "true") return null;

  const configPath = resolveOdysseusConfigPath(overrideConfigPath);
  ...
  const worktreeName =
    nonEmpty(persistedEnv.ODYSSEUS_WORKTREE_NAME) ??
    nonEmpty(env.ODYSSEUS_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(persistedEnv.ODYSSEUS_INSTANCE_ID) ??
    nonEmpty(env.ODYSSEUS_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(persistedEnv.ODYSSEUS_HOME) ??
      nonEmpty(env.ODYSSEUS_HOME) ??
      nonEmpty(env.ODYSSEUS_WORKTREES_DIR) ??
      "~/.odysseus-worktrees",
  );
...
function writeConfigFile(configPath: string, config: OdysseusConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".odysseus", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as OdysseusConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: OdysseusConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: OdysseusConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.ODYSSEUS_HOME = context.homeDir;
  process.env.ODYSSEUS_INSTANCE_ID = context.instanceId;
  process.env.ODYSSEUS_CONFIG = context.configPath;
  process.env.ODYSSEUS_CONTEXT = context.contextPath;
  process.env.ODYSSEUS_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as OdysseusConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    ODYSSEUS_HOME: context.homeDir,
    ODYSSEUS_INSTANCE_ID: context.instanceId,
    ODYSSEUS_CONFIG: context.configPath,
    ODYSSEUS_CONTEXT: context.contextPath,
    ODYSSEUS_IN_WORKTREE: "true",
    ODYSSEUS_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: OdysseusConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as OdysseusConfig;
  } catch {
    return;
  }

...
export const PRP_V2_EVENT_TYPES = new Set([
  ...[...PRP_V1_EVENT_TYPES].filter((type) => type !== "runner.backpressure" && type !== "semantic_tool.reconciled"),
...
function isKnownPrpEventDiscriminator(
  ...
  return (
    key === "eventType" &&
    typeof value === "string" &&
    ((container.schema === "paperclip.prp.event.v1" && container.schemaVersion === 1 && PRP_V1_EVENT_TYPES.has(value)) ||
      (container.schema === "paperclip.prp.event.v2" && container.schemaVersion === 2 && PRP_V2_EVENT_TYPES.has(value)))
  );
}

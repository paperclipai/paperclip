...
function createFaviconDataUrl(background: string, foreground: string): string {
  const svg = [
    ...
    `<path d="M12 5L4 11v8a1.5 1.5 0 001.5 1.5h13A1.5 1.5 0 0020 19v-8L12 5z" fill="${foreground}" fill-opacity="0.15" stroke="${foreground}" stroke-width="1.5" stroke-linejoin="round"/>`,
    `<circle cx="12" cy="14.5" r="2.5" fill="${foreground}"/>`,
    "</svg>",
  ].join("");

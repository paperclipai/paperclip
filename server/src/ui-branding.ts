const FAVICON_BLOCK_START = "<!-- ODYSSEUS_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- ODYSSEUS_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- ODYSSEUS_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- ODYSSEUS_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.ODYSSEUS_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.ODYSSEUS_WORKTREE_NAME) ?? nonEmpty(env.ODYSSEUS_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.ODYSSEUS_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="odysseus-worktree-enabled" content="true" />',
    `<meta name="odysseus-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="odysseus-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="odysseus-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}

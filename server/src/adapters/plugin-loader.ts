...
function extractUiParserSource(
  ...
  const contractVersion = pkg.odysseus?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no odysseus.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }

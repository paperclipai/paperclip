...
function extractUiParserSource(
  ...
  const contractVersion = pkg.aiteamcorp?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no aiteamcorp.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }

 ...
 * records attributes, and properly handles errors.
 */
...
export async function withHeartbeatSpan<T>(
  ...
  return tracer.startActiveSpan("heartbeat.execute", async (span) => {
...
export async function withAdapterSpan<T>(
  ...
  const tracer = getTracer();
  return tracer.startActiveSpan("adapter.execute", async (span) => {
    span.setAttributes({
...
export async function withToolCallSpan<T>(
  ...
  const tracer = getTracer();
  return tracer.startActiveSpan("plugin.tool.execute", async (span) => {
    const startTime = performance.now();
...
export async function withCostEventSpan<T>(
  ...
  const tracer = getTracer();
  return tracer.startActiveSpan("cost.record_event", async (span) => {
    span.setAttributes({

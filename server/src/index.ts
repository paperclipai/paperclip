...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.ODYSSEUS_SECRETS_PROVIDER === undefined) {
    process.env.ODYSSEUS_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.ODYSSEUS_SECRETS_STRICT_MODE === undefined) {
    process.env.ODYSSEUS_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.ODYSSEUS_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.ODYSSEUS_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.ODYSSEUS_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.ODYSSEUS_MIGRATION_PROMPT === "never") return false;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set ODYSSEUS_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@odysseus.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.ODYSSEUS_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://odysseus:odysseus@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "odysseus");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://odysseus:odysseus@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "odysseus");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: odysseus");
    }
  
    const embeddedConnectionString = `postgres://odysseus:odysseus@127.0.0.1:${port}/odysseus`;
  ...
  const configuredApiUrl = process.env.ODYSSEUS_API_URL?.trim() || runtimeApiUrl;
  ...
  process.env.ODYSSEUS_LISTEN_HOST = runtimeListenHost;
  process.env.ODYSSEUS_LISTEN_PORT = String(listenPort);
  process.env.ODYSSEUS_RUNTIME_API_URL = runtimeApiUrl;
  process.env.ODYSSEUS_RUNTIME_API_CANDIDATES_JSON = JSON.stringify(runtimeApiCandidates);
  process.env.ODYSSEUS_API_URL = configuredApiUrl;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.ODYSSEUS_OPEN_ON_LISTEN === "true") {
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Odysseus server failed to start");
    process.exit(1);
  });
}

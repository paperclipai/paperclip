...
import { resolveOdysseusConfigPath, resolveOdysseusEnvPath } from "./paths.js";
import type { BindMode, DeploymentExposure, DeploymentMode } from "@odysseus/shared";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.ODYSSEUS_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.ODYSSEUS_AGENT_JWT_SECRET === "string" ? parsed.ODYSSEUS_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm odysseus onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveOdysseusEnvPath();

...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const heartbeatFailureWebhook = opts.heartbeatFailureWebhookUrl
    ? color("configured", "green")
    : color("not-configured", "yellow");
  ...
  const lines = [
    ...
    row("HB Failure Webhook", heartbeatFailureWebhook),

...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.AITEAMCORP_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.AITEAMCORP_AGENT_JWT_SECRET === "string" ? parsed.AITEAMCORP_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm aiteamcorp onboard`)",
  };
}

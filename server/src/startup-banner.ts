...
import { resolveValadrienOsConfigPath, resolveValadrienOsEnvPath } from "./paths.js";
import type { BindMode, DeploymentExposure, DeploymentMode } from "@valadrien-os/shared";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.VALADRIEN_OS_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.VALADRIEN_OS_AGENT_JWT_SECRET === "string" ? parsed.VALADRIEN_OS_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm valadrien-os onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveValadrienOsEnvPath();

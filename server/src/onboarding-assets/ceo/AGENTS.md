...
- For `[UI]`, `[DATA]`, `[GAP]`, or `[NO-CODE]` issues (no-code kind), the eventual closure comment goes through the closure-gate escape hatch; have the confirmation reference `doc/closure-gate.md` so reviewers understand the marker convention.

...
- On work assigned to you, an operator-facing confirmation/question/verdict must carry `payload.humanCategory` (`credential`, `identity`, `spend`, `oauth`, or `g_class`) and a one-line `payload.humanJustification`. Otherwise complete or reassign the work; Paperclip refuses the board card and logs the attempt.

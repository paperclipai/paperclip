...
function jwtConfig() {
  const secret = process.env.AITEAMCORP_AGENT_JWT_SECRET?.trim() || process.env.BETTER_AUTH_SECRET?.trim();
  ...
  return {
    ...
    audience: process.env.AITEAMCORP_AGENT_JWT_AUDIENCE ?? "aiteamcorp-api",
  };
}

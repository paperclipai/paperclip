...
function hasScheduledMonitor(input: {
  ...
  const isLive = (value: Date) => {
    ...
    return (
      input.requireNextCheckAfter === undefined ||
      value.getTime() > input.requireNextCheckAfter
    );
  };
  const candidates: Date[] = [];
  if (input.patchMonitorNextCheckAt instanceof Date)
    candidates.push(input.patchMonitorNextCheckAt);
  else if (

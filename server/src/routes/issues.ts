...
export function issueRoutes(
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const actor = getActorInfo(req);
      const result = await issueThreadInteractionService(db).rejectInteraction(issue, interactionId, req.body, {
        agentId: actor.agentId,
        userId: actor.actorType === "user" ? actor.actorId : null,
      });
      const interaction = "continuationIssue" in result ? result.interaction : result;
      const continuationIssue = "continuationIssue" in result ? result.continuationIssue : null;
      const continuationWakeIssue = continuationIssue ?? issue;

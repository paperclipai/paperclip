...
async function publishCommittedIssueCreateActivities(input: {
  ...
  for (const publication of input.publications) {
    await runCommittedIssueCreateSideEffect({
      ...
      message:
        "The issue was created and its activity was recorded, but a live update could not be published.",
    ...
    async (req, res) => {
      ...
      const createInput = {
        ...
        activityInputFactory: async (
          ...
          referenceSummary =
            await issueReferencesSvc.listIssueReferenceSummary(
              createdIssue.id,
              dbOrTx,
            );
          const referenceDiff = issueReferencesSvc.diffIssueReferenceSummary(
              ...
              details: {
                ...
                ...summarizeIssueReferenceActivityDetails({
                  addedReferencedIssues:
                    referenceDiff.addedReferencedIssues.map(
                      summarizeIssueRelationForActivity,
                    ),
                  removedReferencedIssues:
                    referenceDiff.removedReferencedIssues.map(
                      summarizeIssueRelationForActivity,
                    ),
                  currentReferencedIssues:
                    referenceDiff.currentReferencedIssues.map(
                      summarizeIssueRelationForActivity,
                    ),
                }),
              },
      ...
      if (deduplicationReason) {
        const referenceSummary = await runCommittedIssueCreateSideEffect({
          ...
          message:
            "The existing issue was returned, but related-work references may be incomplete.",
          warnings: postCommitWarnings,
          operation: () =>
            issueReferencesSvc.listIssueReferenceSummary(issue.id),
  ...
  router.post(
    ...
    async (req, res) => {
          ...
          const activityInputs: Parameters<typeof logActivity>[1][] = [
            {
              ...
              details: {
                ...
                ...(Array.isArray(req.body.blockedByIssueIds)
                  ? { blockedByIssueIds: req.body.blockedByIssueIds }
                  : {}),
                ...(createBody.blockParentUntilDone
                  ? { parentBlockerAdded: true }
                  : {}),
                ...(serializationContext
                  ? {
                      watchdogFollowUpsSerialized: true,
                      serializedBehindIssueId:
                        currentSerializedChild?.id ?? null,
                    }
                  : {}),
              },
            },
      ...
      await runPostCommit({
        ...
        message:
          "The child issue was created, but watchdog follow-up serialization could not be completed.",
        operation: async () => {
          await blockWatchdogParentOnCurrentChild({
            actor,
            watchdogParentIssueId:
              serializationContext?.watchdogParentIssueId,

...
import {
  assertIssueCloseEvidenceSatisfied,
...
import { resolveIssueRunForCloseGate } from "../services/issue-close-run-context.js";
...
async function queueResolvedInteractionContinuationWakeup(input: {
  ...
  // Local: previously skipped on "expired", which left an assignee unaware that a
  // board-action card it created had been superseded — so it re-asserted a stale
  // pending approval. Expired resolutions wake the assignee too; the resolution
...
export function issueRoutes(
  ...
  async function assertInReviewReviewPath(input: {
    ...
    if (await hasTypedGateKeeperReviewPath({ access, issue: input.existing, nextAssigneeAgentId })) return null;
  ...
  async function assertAgentIssueCommentAllowed(
    ...
    if (!boundaryDecision.allowed) {
      // Gate-keeper comment override (local): a gate-keeper agent scoped to this
      // issue's review labels may comment even when the issue is outside its base
      // authorization boundary. The tasks:gate_keeper_write capability gates it.
      if (options?.gateKeeperOverride) {
        const override = await hasGateKeeperWriteOverride(issue, {
          actorAgentId,
  ...
  async function resolveActiveIssueAssigneeAgentReference(
    companyId: string,
    rawAssigneeAgentId: string | null | undefined,
    options: { preserveFallbackSisterAssignee?: boolean; actorType?: string } = {},
  ) {
  ...
  router.get("/companies/:companyId/issues", async (req, res) => {
    ...
    const coordinated = await coordinateIssueListGet({
      ...
      compute: async () => {
        ...
        const [handoffStates, recoveryActionByIssue, boardActionByIssue] = await Promise.all([
          listSuccessfulRunHandoffStates(db, companyId, issueIds),
          recoveryActionsSvc.listActiveForIssues(companyId, issueIds),
          // Local (board-asks): surface pending operator decisions on list rows.
          svc.getBoardActionRequirements(companyId, result),
        ]);
        const actor = getActorInfo(req);
        await Promise.all(result.map(async (issue) => {
          const activeRecoveryAction = recoveryActionByIssue.get(issue.id) ?? null;
  ...
  router.patch("/issues/:id", validateIssueMutationBody(updateIssueRouteSchema), async (req, res) => {
    ...
    await assertIssueCloseEvidenceSatisfied({
      ...
      issueRun: nextStatus === "done"
        ? await resolveIssueRunForCloseGate(db, existing, heartbeat.getRun, {
  ...
  router.post("/issues/:id/checkout", validate(checkoutIssueSchema), async (req, res) => {
    ...
    try {
      updated = await svc.checkout(id, req.body.agentId, req.body.expectedStatuses, checkoutRunId, { checkoutType });
    } catch (error) {
  ...
  router.post("/issues/:id/comments", validate(addIssueCommentSchema), async (req, res) => {
    ...
    void (async () => {
      ...
      const skipWake =
        ...
        || wakeIssueSnapshot.status === "blocked"
        || suppressDuplicateBlockedCommentWake;

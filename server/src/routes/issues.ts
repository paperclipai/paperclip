...
function sourceDispositionRecoveryOutcome(input: {
  ...
  if (input.status === "done" || input.status === "in_review" || input.status === "cancelled") {
    return "restored";
  }
  return null;
}
...
export function issueRoutes(
  ...
  router.patch("/issues/:id", validate(updateIssueRouteSchema), async (req, res) => {
    ...
    if (sourceDispositionWritten) {
      ...
      if (recoveryOutcome && activeRecoveryActionBeforeUpdate) {
        autoResolvedRecoveryAction = await recoveryActionsSvc.resolveActiveForIssue({
          companyId: issue.companyId,
          sourceIssueId: issue.id,
          actionId: activeRecoveryActionBeforeUpdate.id,
          status: "resolved",

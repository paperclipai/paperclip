...
export function issueRoutes(
  ...
  function agentCanCreateIssues(agent: { permissions: Record<string, unknown> | null | undefined; role: string }) {
    if (agent.role === "ceo") return true;
    if (!agent.permissions || typeof agent.permissions !== "object") return false;
    return Boolean((agent.permissions as Record<string, unknown>).canCreateIssues);
  }
  ...
  async function assertCanAssignTasks(req: Request, companyId: string) {
    ...
    if (req.actor.type === "agent") {
      ...
      if (actorAgent && actorAgent.companyId === companyId && agentCanCreateIssues(actorAgent)) return;

...
export function executionWorkspaceRoutes(db: Db, opts: { pluginWorkerManager?: PluginWorkerManager } = {}) {
  ...
  router.patch("/execution-workspaces/:id", validate(updateExecutionWorkspaceSchema), async (req, res) => {
    ...
    if (req.body.status === "archived" && existing.status !== "archived") {
      ...
      const shouldCleanupWorkspaceArtifacts = readiness.plannedActions.some((action) =>
        action.kind !== "archive_record" && action.kind !== "stop_runtime_services"
      );
      ...
      try {
        ...
        if (shouldCleanupWorkspaceArtifacts) {
          const projectWorkspace = existing.projectWorkspaceId
            ? await db
                .select({
                  cwd: projectWorkspaces.cwd,

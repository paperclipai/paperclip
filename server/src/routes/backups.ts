...
function backupActorId(req: Parameters<typeof assertBoard>[0]): string | null {
...
export function backupRoutes(backupManager: BackupManager) {
  ...
  router.use("/backups", (req, _res, next) => {
    ...
    if (isCloudManagedInstance()) {
      throw forbidden("Portable backups are platform-managed on cloud-managed instances", {
        code: "portable_backups_platform_managed",
      });
    }
    next();
  });
  ...
  router.post("/backups/run", async (req, res) => {
    assertInstanceAdmin(req);
    res.status(202).json(await backupManager.createManualBackup(backupActorId(req)));
  });

  router.patch("/backups/settings", validate(updateBackupSettingsSchema), async (req, res) => {
    assertInstanceAdmin(req);
    res.json(await backupManager.updateSettings(req.body, backupActorId(req)));
  });
  ...
  router.post("/backups/import", async (req, res) => {
    ...
    try {
      const imported = await backupManager.importBackupArchive(
        file.path,
  ...
  router.post("/backups/recovery/rollback", validate(rollbackRestoreRecoverySchema), async (req, res) => {
    assertInstanceAdmin(req);
    res.status(202).json(await backupManager.rollbackInterruptedRestore(backupActorId(req)));
  });
  ...
  router.get("/backups/:backupId/download", async (req, res, next) => {
    ...
    try {
      ...
      const download = await backupManager.getDownloadDescriptor(backupId, backupActorId(req));
      const tar = spawn("tar", ["-czf", "-", "-C", download.bundleDirectory, download.bundleName], {
        stdio: ["ignore", "pipe", "pipe"],
      });
      let stderr = "";
      const onRequestClose = () => tar.kill("SIGTERM");
  ...
  router.post("/backups/:backupId/restore", validate(restoreBackupSchema), async (req, res) => {
    assertInstanceAdmin(req);
    const backupId = Array.isArray(req.params.backupId) ? req.params.backupId[0] : req.params.backupId;
    res.status(202).json(await backupManager.restoreBackup(backupId, backupActorId(req)));
  });

  router.post("/backups/:backupId/archive", validate(archiveBackupSchema), async (req, res) => {
    assertInstanceAdmin(req);
    const backupId = Array.isArray(req.params.backupId) ? req.params.backupId[0] : req.params.backupId;
    res.json(await backupManager.archiveBackup(backupId, backupActorId(req)));
  });

  router.post("/backups/:backupId/unarchive", validate(unarchiveBackupSchema), async (req, res) => {
    assertInstanceAdmin(req);
    const backupId = Array.isArray(req.params.backupId) ? req.params.backupId[0] : req.params.backupId;
    res.json(await backupManager.unarchiveBackup(backupId, backupActorId(req)));
  });

  router.post("/backups/:backupId/delete", validate(deleteBackupSchema), async (req, res) => {
    assertInstanceAdmin(req);
    const backupId = Array.isArray(req.params.backupId) ? req.params.backupId[0] : req.params.backupId;
    res.json(await backupManager.deleteBackup(backupId, backupActorId(req)));
  });

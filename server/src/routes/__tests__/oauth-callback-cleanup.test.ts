...
import { exchangeToken } from "../../oauth/http.js";
...
describe("OAuth callback cleanup", () => {
  ...
  it("omits code_verifier when provider does not support PKCE", async () => {
    vi.mocked(exchangeToken).mockClear();
    const stateRow = {
      id: "state-1",
      providerId: "github",
      consumedAt: new Date(),
    ...
    const db = {
      ...
      query: {
        ...
        oauthConnections: { findFirst: vi.fn() },
      },
      transaction: vi.fn(),
    };
    ...
    app.use(
      ...
      oauthCallbackRoute({
        ...
        registry: {
          get: () => ({
            ...provider,
            config: { ...provider.config, pkce: "unsupported" },
            shape: {},
          }),
        } as unknown as never,
    ...
    const call = vi.mocked(exchangeToken).mock.calls[0]?.[0];
    expect(call?.params).toMatchObject({
      grant_type: "authorization_code",
      code: "code",
      redirect_uri: stateRow.redirectUri,
    });
    expect(call?.params).not.toHaveProperty("code_verifier");

...
import { agents as agentsTable, companies, heartbeatRuns, issues as issuesTable } from "@odysseus/db";
...
import { trackAgentCreated } from "@odysseus/shared/telemetry";
...
export function agentRoutes(
   ...
   *   adapter probes the Odysseus host (legacy behavior).
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeOdysseusSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readOdysseusSkillSyncPreference(

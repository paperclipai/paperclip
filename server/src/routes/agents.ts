...
import { agents as agentsTable, companies, heartbeatRuns, issues as issuesTable } from "@aiteamcorp/db";
...
import { trackAgentCreated } from "@aiteamcorp/shared/telemetry";
...
import { runClaudeLogin } from "@aiteamcorp/adapter-claude-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeAiTeamCorpSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readAiTeamCorpSkillSyncPreference(

...
import type { WorkspaceRuntimeDesiredState, WorkspaceRuntimeServiceStateMap } from "@odysseus/shared";
import { trackProjectCreated } from "@odysseus/shared/telemetry";
...
export function projectRoutes(db: Db) {
  ...
  async function handleProjectWorkspaceRuntimeCommand(req: Request, res: Response) {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Project workspace needs a local path before Odysseus can run workspace commands" });
      return;
    }

...
// Mirrors `importTransferManifestSchema` in companies.ts: the declared
// byte-range part layout of the caller's existing company package .zip.
const importTransferDeclarationSchema = z.object({
  totalBytes: z.number().int().min(1),
  zipSha256: z.string().describe("sha256 of the whole .zip, 64 lowercase hex characters"),
  partSizeBytes: z.number().int().min(1),
  parts: z.array(
    z.object({
      index: z.number().int().min(0),
      byteSize: z.number().int().min(1),
      sha256: z.string().describe("sha256 of this part, 64 lowercase hex characters"),
    }),
  ),
});
...
registry.registerPath({
  method: "post",
  path: "/api/companies/import/transfers",
  tags: ["companies"],
  ...
  description:
    ...
    "still missing.",
  request: { body: jsonBody(importTransferDeclarationSchema) },
  responses: { 200: r.ok(), 400: r.badRequest, 401: r.unauthorized, 422: r.unprocessable },
});

registry.registerPath({
  method: "put",
  path: "/api/companies/import/transfers/{transferId}/parts/{partIndex}",
  tags: ["companies"],
...
registry.registerPath({
  method: "get",
  path: "/api/companies/import/transfers/{transferId}",
  tags: ["companies"],

...
const WORKSPACE_RUNTIME_ELIGIBLE_ISSUE_STATUSES: string[] = ["backlog", "todo", "in_progress", "in_review", "blocked"];
  ...
  const linkedIssue = await db
    ...
    .where(
      and(
        ...
        workspaceScopeConditions.length === 1 ? workspaceScopeConditions[0]! : or(...workspaceScopeConditions),
      ),
    )
    .then((rows) => rows[0] ?? null);

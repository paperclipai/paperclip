...
const IMPORT_TRANSFER_MAX_PARTS = 4096;
const SHA256_HEX_RE = /^[0-9a-f]{64}$/;

const importTransferDeclaredPartSchema = z.object({
  index: z.number().int().min(0),
  byteSize: z.number().int().min(1),
  sha256: z.string().regex(SHA256_HEX_RE, "sha256 must be 64 lowercase hex characters"),
});

const importTransferManifestSchema = z.object({
  totalBytes: z.number().int().min(1),
  zipSha256: z.string().regex(SHA256_HEX_RE, "sha256 must be 64 lowercase hex characters"),
  partSizeBytes: z.number().int().min(1),
  parts: z.array(importTransferDeclaredPartSchema).min(1).max(IMPORT_TRANSFER_MAX_PARTS),
});

type ImportTransferManifest = z.infer<typeof importTransferManifestSchema>;
...
export function companyRoutes(db: Db, storage?: StorageService, options?: CompanyRoutesOptions) {
  ...
  router.put(
    "/import/transfers/:transferId/parts/:partIndex",
    ...
    async (req, res) => {
      ...
      if (run.completedParts.includes(partName)) {
        ...
        if (sizeOnDisk === declared.byteSize) {
          res.json({ ok: true, index: partIndex, alreadyCompleted: true });
          return;
        }
      }
      ...
      res.json({ ok: true, index: partIndex, alreadyCompleted: false });
    },
  );

  // Resume polling: which parts are done, which are still missing.
  router.get("/import/transfers/:transferId", async (req, res) => {
    assertBoard(req);
    const run = await requireImportTransferRun(req);
    const manifest = storedImportTransferManifest(run);

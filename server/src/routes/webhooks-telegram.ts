  ...
  router.post("/webhooks/telegram", async (req, res) => {
    ...
    try {
      ...
      if (msg.entities && msg.entities.length > 0) {
        ...
        if (botMention) {
          const before = text.slice(0, botMention.offset);
          const after = text.slice(botMention.offset + botMention.length);
          const trimmed = (before + after).trim();
          if (trimmed) {
            logger.info({ trimmed, chatId }, "Telegram @mention → /ask");

...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    Then set AITEAMCORP_API_KEY and AITEAMCORP_API_URL from the saved token field for every heartbeat run.
    ...
    Be sure to prepend your AITEAMCORP_API_URL to the top of your skill and note the path to your AITEAMCORP_API_URL
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@aiteamcorp.local";

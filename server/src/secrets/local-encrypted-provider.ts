...
function resolveMasterKeyFilePath() {
  const fromEnv = process.env.VALADRIEN_OS_SECRETS_MASTER_KEY_FILE;
...
function loadOrCreateMasterKey(): Buffer {
  const envKeyRaw = process.env.VALADRIEN_OS_SECRETS_MASTER_KEY;
  if (envKeyRaw && envKeyRaw.trim().length > 0) {
    const fromEnv = decodeMasterKey(envKeyRaw);
    if (!fromEnv) {
      throw badRequest(
        "Invalid VALADRIEN_OS_SECRETS_MASTER_KEY (expected 32-byte base64, 64-char hex, or raw 32-char string)",
      );
    }
    return fromEnv;
  }
  ...
  if (envKeyRaw && envKeyRaw.trim().length > 0) {
    if (!decodeMasterKey(envKeyRaw)) {
      return {
        ...
        status: "error",
        message:
          "VALADRIEN_OS_SECRETS_MASTER_KEY is invalid; expected 32-byte base64, 64-char hex, or raw 32-char string",
      };
    }
    return {
      provider: "local_encrypted",
      status: "ok",
      message: "Local encrypted provider is using VALADRIEN_OS_SECRETS_MASTER_KEY",
      backupGuidance: [
...
export const localEncryptedProvider: SecretProviderModule = {
  ...
  async deleteOrArchive() {
    // Secret metadata deletion is handled in ValadrienOs DB; the local key is shared and must remain.
  },
  async healthCheck() {

...
export interface AdapterExecutionTargetOdysseusBridgeHandle {
  env: Record<string, string>;
  stop(): Promise<void>;
}
...
function resolveDefaultOdysseusApiUrl(): string {
  const runtimeHost = resolveHostForUrl(
    process.env.ODYSSEUS_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  // 3100 matches the default Odysseus dev server port when the runtime does not provide one.
...
function isBridgeDebugEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.ODYSSEUS_BRIDGE_DEBUG?.trim().toLowerCase();
...
export function adapterExecutionTargetUsesOdysseusBridge(
  target: AdapterExecutionTarget | null | undefined,
): boolean {
  return target?.kind === "remote";
}
  ...
  if (detectCommand) {
    ...
    if (!recheck.timedOut && recheck.exitCode === 0) {
      if (input.onLog) {
        ...
        await input.onLog(
          "stderr",
          `[odysseus] Install command ${reason} (${installCommand}) but ${detectCommand} is on PATH; continuing.\n`,
        );
      }
      return;
    }
 ...
 * the directory state inside the environment, not on the Odysseus host.
 *
...
export function runtimeAssetDir(
  ...
  return prepared.assetDirs[key] ?? path.posix.join(fallbackRemoteCwd, ".odysseus-runtime", key);
}
...
export async function startAdapterExecutionTargetOdysseusBridge(input: {
  runId: string;
  target: AdapterExecutionTarget | null | undefined;
...
}): Promise<AdapterExecutionTargetOdysseusBridgeHandle | null> {
  if (!adapterExecutionTargetUsesOdysseusBridge(input.target)) {
    return null;
  }
  ...
  if (hostApiToken.length === 0) {
    throw new Error("Sandbox bridge mode requires a host-side Odysseus API token.");
  }

  const runtimeRootDir =
    input.runtimeRootDir?.trim().length
      ? input.runtimeRootDir.trim()
      : path.posix.join(target.remoteCwd, ".odysseus-runtime", input.adapterKey);
  const bridgeRuntimeDir = path.posix.join(runtimeRootDir, "odysseus-bridge");
  ...
  const hostApiUrl =
    ...
    process.env.ODYSSEUS_RUNTIME_API_URL?.trim() ||
    process.env.ODYSSEUS_API_URL?.trim() ||
    resolveDefaultOdysseusApiUrl();
  ...
  await onLog(
    "stdout",
    `[odysseus] Starting sandbox callback bridge for ${input.adapterKey} in ${bridgeRuntimeDir}.\n`,
  );
  ...
  try {
    ...
    // ODYSSEUS_BRIDGE_DEBUG opts into verbose stdout logs of every bridge
    ...
    worker = await startSandboxCallbackBridgeWorker({
      ...
      handleRequest: async (request) => {
        ...
        if (bridgeDebugEnabled) {
          await onLog(
            "stdout",
            `[odysseus] Bridge proxy ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
          );
        }
        ...
        headers.set("x-odysseus-run-id", input.runId);
        ...
        if (bridgeDebugEnabled) {
          await onLog(
            "stdout",
            `[odysseus] Bridge proxy response ${response.status} for ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
          );
        }
        return {
  ...
  return {
    env: {
      ODYSSEUS_API_URL: server.baseUrl,
      ODYSSEUS_API_KEY: bridgeToken,

  ...
  if (target?.kind === "remote" && target.transport === "sandbox") {
    ...
    try {
      const result = await runner.execute({
        ...
        acquireLaunchPermit: options.acquireLaunchPermit,
...
export async function startAdapterExecutionTargetProcessSessionBridge(input: {
  ...
  let stdinSeq = 0;
  // A single proxy connection can send stdin and stdinEnd in the same socket
  // chunk. Each remote write is an independent sandbox exec, so fire-and-
  // forget writes could complete out of order and close the child before its
  // preceding input arrived. Keep the durable file queue strictly ordered.
  ...
  const enqueueStdinWrite = (stdinPayload: { type: "stdin"; data: string } | { type: "stdinEnd" }) => {
    stdinSeq += 1;
  ...
  const server = net.createServer((nextSocket) => {
    ...
    nextSocket.on("data", (chunk) => {
      ...
      for (const line of split.lines) {
        ...
        const stdinPayload: { type: "stdin"; data: string } | { type: "stdinEnd" } | null =
          message.type === "stdin" && typeof message.data === "string"
            ...
            : message.type === "stdinEnd"
              ...
              : null;
        if (stdinPayload) {
          void enqueueStdinWrite(stdinPayload).catch((error) => {
  ...
  return {
    ...
    stop: async () => {
      ...
      // Do not race the shutdown marker with a final queued stdin write; the
      // remote wrapper consumes lexically sorted files and treats stdinEnd as
      // terminal.
      await stdinWriteChain.catch(() => undefined);
      if (!stdinWriteFailed) {
        await enqueueStdinWrite({ type: "stdinEnd" }).catch(() => undefined);
      }

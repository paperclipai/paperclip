...
describe("resolvePaperclipDesiredSkillNames", () => {
  ...
  // The legacy resolver forwards the option, so a hire-capable agent on a
  // legacy adapter gets the create-agent skill alongside the operational skill
  // that resolver already guarantees.
  it("forwards always-include through the legacy resolver, keeping the operational skill", () => {
    const result = resolveLegacyPaperclipDesiredSkillNames({}, [createAgentEntry, otherEntry], {
      alwaysIncludeSkillKeys: [PAPERCLIP_CREATE_AGENT_SKILL_KEY],
    });
    expect(result).toContain(createAgentEntry.key);
    expect(result).toContain(PAPERCLIP_OPERATIONAL_SKILL_KEY);
  });
});

...
async function acquireDirectoryMergeLock(lockDir: string): Promise<() => Promise<void>> {
  ...
  while (true) {
    try {
      ...
      // the materializeValadrienOsSkillCopy lock pattern — remove and retry.
...
export async function withDirectoryMergeLock<T>(
  ...
  const releaseLock = await acquireDirectoryMergeLock(`${targetDir}.valadrien-os-restore.lock`);
  try {
    return await fn();
  } finally {
    await releaseLock();

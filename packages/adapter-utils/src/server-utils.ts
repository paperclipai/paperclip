...
const DEFAULT_ODYSSEUS_INSTANCE_ID = "default";
const PATH_SEGMENT_RE = /^[a-zA-Z0-9_-]+$/;
const SENSITIVE_ENV_KEY = /(key|token|secret|password|passwd|authorization|cookie)/i;
const REDACTED_LOG_VALUE = "***REDACTED***";
const ODYSSEUS_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];
const MATERIALIZED_SKILL_SENTINEL = ".odysseus-materialized-skill.json";
...
export function resolveOdysseusInstanceRootForAdapter(input: {
  homeDir?: string;
  instanceId?: string;
  env?: NodeJS.ProcessEnv;
} = {}): string {
  const env = input.env ?? process.env;
  const homeRaw = input.homeDir?.trim() || env.ODYSSEUS_HOME?.trim();
  const homeDir = path.resolve(homeRaw ? expandHomePrefix(homeRaw) : path.resolve(os.homedir(), ".odysseus"));
  const instanceId = input.instanceId?.trim() || env.ODYSSEUS_INSTANCE_ID?.trim() || DEFAULT_ODYSSEUS_INSTANCE_ID;
  if (!PATH_SEGMENT_RE.test(instanceId)) throw new Error(`Invalid ODYSSEUS_INSTANCE_ID '${instanceId}'.`);
  return path.resolve(homeDir, "instances", instanceId);
}

export const DEFAULT_ODYSSEUS_AGENT_PROMPT_TEMPLATE = [
  "You are agent {{agent.id}} ({{agent.name}}). Continue your Odysseus work.",
  "",
  "Execution contract:",
...
export interface OdysseusSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
export interface MaterializedOdysseusSkillCopyResult {
  copiedFiles: number;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: OdysseusSkillEntry[];
...
type OdysseusWakeExecutionPrincipal = {
  type: "agent" | "user" | null;
  agentId: string | null;
  userId: string | null;
};

type OdysseusWakeExecutionStage = {
  wakeRole: "reviewer" | "approver" | "executor" | null;
  stageId: string | null;
  stageType: string | null;
  currentParticipant: OdysseusWakeExecutionPrincipal | null;
  returnAssignee: OdysseusWakeExecutionPrincipal | null;
  reviewRequest: {
    instructions: string;
  } | null;
...
type OdysseusWakeContinuationSummary = {
  key: string | null;
  title: string | null;
  body: string;
  bodyTruncated: boolean;
...
type OdysseusWakeLivenessContinuation = {
  attempt: number | null;
...
type OdysseusWakeChildIssueSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type OdysseusWakeBlockerSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type OdysseusWakeTreeHoldSummary = {
  holdId: string | null;
...
type OdysseusWakePayload = {
  reason: string | null;
  issue: OdysseusWakeIssue | null;
  checkedOutByHarness: boolean;
  dependencyBlockedInteraction: boolean;
  treeHoldInteraction: boolean;
  activeTreeHold: OdysseusWakeTreeHoldSummary | null;
  unresolvedBlockerIssueIds: string[];
  unresolvedBlockerSummaries: OdysseusWakeBlockerSummary[];
  executionStage: OdysseusWakeExecutionStage | null;
  continuationSummary: OdysseusWakeContinuationSummary | null;
  livenessContinuation: OdysseusWakeLivenessContinuation | null;
  interactionKind: string | null;
  interactionStatus: string | null;
  childIssueSummaries: OdysseusWakeChildIssueSummary[];
  childIssueSummaryTruncated: boolean;
  commentIds: string[];
  latestCommentId: string | null;
  comments: OdysseusWakeComment[];
...
function normalizeOdysseusWakeIssue(value: unknown): OdysseusWakeIssue | null {
...
function normalizeOdysseusWakeComment(value: unknown): OdysseusWakeComment | null {
...
function normalizeOdysseusWakeContinuationSummary(value: unknown): OdysseusWakeContinuationSummary | null {
...
function normalizeOdysseusWakeLivenessContinuation(value: unknown): OdysseusWakeLivenessContinuation | null {
...
function normalizeOdysseusWakeChildIssueSummary(value: unknown): OdysseusWakeChildIssueSummary | null {
...
function normalizeOdysseusWakeBlockerSummary(value: unknown): OdysseusWakeBlockerSummary | null {
...
function normalizeOdysseusWakeTreeHoldSummary(value: unknown): OdysseusWakeTreeHoldSummary | null {
...
function normalizeOdysseusWakeExecutionPrincipal(value: unknown): OdysseusWakeExecutionPrincipal | null {
...
function normalizeOdysseusWakeExecutionStage(value: unknown): OdysseusWakeExecutionStage | null {
  ...
  const allowedActions = Array.isArray(stage.allowedActions)
    ...
    : [];
  const currentParticipant = normalizeOdysseusWakeExecutionPrincipal(stage.currentParticipant);
  const returnAssignee = normalizeOdysseusWakeExecutionPrincipal(stage.returnAssignee);
...
export function normalizeOdysseusWakePayload(value: unknown): OdysseusWakePayload | null {
  const payload = parseObject(value);
  const comments = Array.isArray(payload.comments)
    ? payload.comments
        .map((entry) => normalizeOdysseusWakeComment(entry))
        .filter((entry): entry is OdysseusWakeComment => Boolean(entry))
    : [];
  ...
  const commentIds = Array.isArray(payload.commentIds)
    ? payload.commentIds
        ...
        .map((entry) => entry.trim())
    : [];
  const executionStage = normalizeOdysseusWakeExecutionStage(payload.executionStage);
  const continuationSummary = normalizeOdysseusWakeContinuationSummary(payload.continuationSummary);
  const livenessContinuation = normalizeOdysseusWakeLivenessContinuation(payload.livenessContinuation);
  const childIssueSummaries = Array.isArray(payload.childIssueSummaries)
    ? payload.childIssueSummaries
        .map((entry) => normalizeOdysseusWakeChildIssueSummary(entry))
        .filter((entry): entry is OdysseusWakeChildIssueSummary => Boolean(entry))
    : [];
  ...
  const unresolvedBlockerSummaries = Array.isArray(payload.unresolvedBlockerSummaries)
    ? payload.unresolvedBlockerSummaries
        .map((entry) => normalizeOdysseusWakeBlockerSummary(entry))
        .filter((entry): entry is OdysseusWakeBlockerSummary => Boolean(entry))
    : [];

  const activeTreeHold = normalizeOdysseusWakeTreeHoldSummary(payload.activeTreeHold);
  if (comments.length === 0 && commentIds.length === 0 && childIssueSummaries.length === 0 && unresolvedBlockerIssueIds.length === 0 && unresolvedBlockerSummaries.length === 0 && !activeTreeHold && !executionStage && !continuationSummary && !livenessContinuation && !normalizeOdysseusWakeIssue(payload.issue)) {
    return null;
  }

  return {
    reason: asString(payload.reason, "").trim() || null,
    issue: normalizeOdysseusWakeIssue(payload.issue),
...
export function stringifyOdysseusWakePayload(value: unknown): string | null {
  const normalized = normalizeOdysseusWakePayload(value);
  if (!normalized) return null;
  return JSON.stringify(normalized);
}

export function readOdysseusIssueWorkModeFromContext(value: unknown): string | null {
  const context = parseObject(value);
  const issue = parseObject(context.odysseusIssue);
  const direct = asString(issue.workMode, "").trim();
  if (direct) return direct;
  const wake = normalizeOdysseusWakePayload(context.odysseusWake);
  return wake?.issue?.workMode ?? null;
}

export function renderOdysseusWakePrompt(
  value: unknown,
  ...
  const principalLabel = (principal: OdysseusWakeExecutionPrincipal | null) => {
  ...
  const lines = resumedSession
      ? [
        "## Odysseus Resume Delta",
        "",
        "You are resuming an existing Odysseus session.",
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "ODYSSEUS_RESOLVED_COMMAND"] = redactCommandTextForLogs(resolvedCommand);
  }

  return redactEnvForLogs(merged);
}

export function buildOdysseusEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    ODYSSEUS_AGENT_ID: agent.id,
    ODYSSEUS_COMPANY_ID: agent.companyId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.ODYSSEUS_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.ODYSSEUS_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl =
    process.env.ODYSSEUS_RUNTIME_API_URL ??
    process.env.ODYSSEUS_API_URL ??
    `http://${runtimeHost}:${runtimePort}`;
  vars.ODYSSEUS_API_URL = apiUrl;
  return vars;
}

export function applyOdysseusWorkspaceEnv(
  env: Record<string, string>,
  input: {
  ...
  const mappings = [
    ["ODYSSEUS_WORKSPACE_CWD", input.workspaceCwd],
    ["ODYSSEUS_WORKSPACE_SOURCE", input.workspaceSource],
    ["ODYSSEUS_WORKSPACE_STRATEGY", input.workspaceStrategy],
    ["ODYSSEUS_WORKSPACE_ID", input.workspaceId],
    ["ODYSSEUS_WORKSPACE_REPO_URL", input.workspaceRepoUrl],
    ["ODYSSEUS_WORKSPACE_REPO_REF", input.workspaceRepoRef],
    ["ODYSSEUS_WORKSPACE_BRANCH", input.workspaceBranch],
    ["ODYSSEUS_WORKSPACE_WORKTREE_PATH", input.workspaceWorktreePath],
...
export function shapeOdysseusWorkspaceEnvForExecution(input: {
  workspaceCwd?: string | null;
  workspaceWorktreePath?: string | null;
  ...
  if (executionCwd === null) {
    ...
    console.warn(
      "[odysseus] shapeOdysseusWorkspaceEnvForExecution called with executionCwd=null on a remote target; " +
...
export function refreshOdysseusWorkspaceEnvForExecution(input: {
  env: Record<string, string>;
  ...
  delete input.env.ODYSSEUS_WORKSPACE_CWD;
  delete input.env.ODYSSEUS_WORKSPACE_WORKTREE_PATH;
  delete input.env.ODYSSEUS_WORKSPACES_JSON;

  applyOdysseusWorkspaceEnv(input.env, {
  ...
  if (shapedWorkspaceEnv.workspaceHints.length > 0) {
    input.env.ODYSSEUS_WORKSPACES_JSON = JSON.stringify(shapedWorkspaceEnv.workspaceHints);
  }
...
export function sanitizeInheritedOdysseusEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = { ...baseEnv };
  for (const key of Object.keys(env)) {
    if (!key.startsWith("ODYSSEUS_")) continue;
    if (key === "ODYSSEUS_RUNTIME_API_URL") continue;
    if (key === "ODYSSEUS_LISTEN_HOST") continue;
    if (key === "ODYSSEUS_LISTEN_PORT") continue;
    delete env[key];
  }
  return env;
}
...
export async function resolveOdysseusSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...ODYSSEUS_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listOdysseusSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<OdysseusSkillEntry[]> {
  const root = await resolveOdysseusSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return Promise.all(dirs.map(async (entry) => {
      ...
      return {
        key: `PossibLaw/odysseus/${entry.name}`,
        runtimeName: entry.name,
        source: skillDir,
        required,
        requiredReason: required
          ? "Bundled Odysseus skills are always available for local adapters."
          : null,
      };
    }));
  } catch {
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Odysseus skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Odysseus cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredOdysseusRuntimeSkills(value: unknown): OdysseusSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: OdysseusSkillEntry[] = [];
...
export async function readOdysseusRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<OdysseusSkillEntry[]> {
  const configuredEntries = normalizeConfiguredOdysseusRuntimeSkills(config.odysseusRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listOdysseusSkillEntries(moduleDir, additionalCandidates);
}

export async function readOdysseusSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
...
export function readOdysseusSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.odysseusSkillSync;
...
function canonicalizeDesiredOdysseusSkillReference(
  reference: string,
...
export function resolveOdysseusDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readOdysseusSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredOdysseusSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeOdysseusSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.odysseusSkillSync;
  const current =
  ...
  next.odysseusSkillSync = current;
  return next;
}

export async function ensureOdysseusSkillSymlink(
  source: string,
  target: string,
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error(`Timed out waiting for Odysseus skill materialization lock at ${lockDir}`);
      }
...
export async function materializeOdysseusSkillCopy(
  source: string,
  target: string,
): Promise<MaterializedOdysseusSkillCopyResult> {
  ...
  if (!rootStat.isDirectory()) {
    throw new Error("Odysseus skills must be directories.");
  }

  const result: MaterializedOdysseusSkillCopyResult = {
    copiedFiles: 0,
    skippedSymlinks: [],
  };
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    const rawMerged: NodeJS.ProcessEnv = {
      ...sanitizeInheritedOdysseusEnv(process.env),
      ...opts.env,
    };

    // Strip Claude Code nesting-guard env vars so spawned `claude` processes
    // don't refuse to start with "cannot be launched inside another session".
    // These vars leak in when the Odysseus server itself is started from
    // within a Claude Code session (e.g. `npx odysseus run` in a terminal

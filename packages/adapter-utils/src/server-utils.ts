...
export function renderPaperclipWakePrompt(
  ...
  const lines = resumedSession
      ? [
        ...
        "When calling HTTP APIs from the shell, never pipe `curl`/`wget` output directly into `python`, `python3`, `bash`, `sh`, `node`, or any interpreter.",
        "For JSON inspection, prefer direct API support, `jq` when available, or save the response to a temporary file before reading it.",
        "",

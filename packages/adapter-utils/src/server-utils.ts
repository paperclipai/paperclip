...
const SERVER_ONLY_AGENT_ENV_KEYS = new Set([
...
// Runtime API identity and server-only signing keys are never accepted from
// adapter or user config. Other PAPERCLIP_*-named config keys are allowed as
// long as Paperclip has not assigned the same key for the run.
export function isForbiddenConfigEnvKey(key: string): boolean {
  return key === "PAPERCLIP_API_KEY" || SERVER_ONLY_AGENT_ENV_KEYS.has(key);
}

export function sanitizeAgentSpawnEnv<T extends Record<string, string | undefined>>(env: T): T {
  const sanitized = { ...env } as T;
  for (const key of SERVER_ONLY_AGENT_ENV_KEYS) {
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    const explicitEnv = sanitizeAgentSpawnEnv(opts.env);
    ...
    void resolveSpawnTarget(command, args, opts.cwd, mergedEnv, {
      ...
      .then((target) => {
        const childEnv = sanitizeAgentSpawnEnv({ ...mergedEnv, ...target.env });

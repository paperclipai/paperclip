...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- To ask for that input, create an interaction on the current issue with POST /api/issues/{issueId}/interactions using kind suggest_tasks, ask_user_questions, request_confirmation, or record_context. Use continuationPolicy wake_assignee when you need to resume after a response; for request_confirmation this resumes only after acceptance.",
  "- To save a durable, keyed note for future runs (yours or another agent's), create a record_context interaction with kind record_context and a payload of {key, title?, body, tags?}; the memory entry is written once the interaction is accepted, not at creation time.",

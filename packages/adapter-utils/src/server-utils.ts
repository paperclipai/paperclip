 ...
 *
 * Adapters that call external HTTP APIs (e.g. Copilot, future OpenAI-compat
 * endpoints) should use this instead of bare `fetch()` to handle transient
 ...
 * Usage:
 * ```ts
 * const response = await fetchWithRetry("https://api.example.com/v1/chat", {
 *   method: "POST",
 *   headers: { ... },
 *   body: JSON.stringify({ ... }),
 * }, {
 ...
 *     console.log(`Retry ${attempt} after ${status}, waiting ${delayMs}ms`),
 * });
 * ```
 */

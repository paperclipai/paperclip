...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- Write issue comments short and human: lead with the update, don't restate the request, and keep only the operational essentials (status, next owner/action, blockers, verification, decision asks, links).",
...
export function renderPaperclipWakePrompt(
  ...
  const lines = resumedSession
      ? [
        ...
        "",
        "Execution contract: take concrete action in this heartbeat when the issue is actionable; do not stop at a plan unless planning was requested. Leave durable progress and then give the issue a clear final disposition before ending the heartbeat: `done`, `in_review` with a real reviewer/approval/interaction path, `blocked` with first-class blockers or a named unblock owner/action, delegated follow-up issues with blockers, or `in_progress` only when a live continuation path exists. Use child issues for long or parallel delegated work instead of polling. Keep issue comments short and human — lead with the update, don't restate the request, keep the operational essentials (status, next owner/action, blockers, verification, decision asks, links). Comments, documents, screenshots, work products, and `Remaining` bullets are evidence, not valid liveness paths by themselves.",
        "",

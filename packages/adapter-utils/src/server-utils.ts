...
export interface RunProcessSpawnMetadata {
  pid: number;
...
export interface RunProcessJournalOptions {
  spoolDir: string;
  stdoutOffset?: number;
  stderrOffset?: number;
  processPreservable?: boolean;
...
let systemdScopeSupport: boolean | null = null;
const JOURNAL_LAUNCHER_SCRIPT = String.raw`
const fs = require("node:fs");
const { spawn } = require("node:child_process");
const config = JSON.parse(process.argv[1]);
const stdoutFd = fs.openSync(config.stdoutPath, "a");
const stderrFd = fs.openSync(config.stderrPath, "a");
...
const child = spawn(config.command, config.args, {
  ...
  detached: false,
  shell: false,
  stdio: [config.hasStdin ? "pipe" : "ignore", stdoutFd, stderrFd],
});
if (config.hasStdin && child.stdin) process.stdin.pipe(child.stdin);
for (const signal of ["SIGINT", "SIGTERM", "SIGHUP"]) {
  process.on(signal, () => {
    if (child.exitCode === null && child.signalCode === null) child.kill(signal);
  });
}
const writeExit = (code, signal, error) => {
  ...
  fs.writeFileSync(config.exitTempPath, body, "utf8");
  fs.renameSync(config.exitTempPath, config.exitPath);
};
child.once("error", (error) => {
  writeExit(null, null, error.message);
  process.exitCode = 1;
});
child.once("exit", (code, signal) => {
  writeExit(code, signal, null);
  process.exitCode = code ?? (signal ? 1 : 0);
});
`;

export function buildJournalLauncherSpawnTarget(input: {
  runId: string;
  launcherConfig: string;
  systemdScopeSupported: boolean;
}) {
  const launcherArgs = ["-e", JOURNAL_LAUNCHER_SCRIPT, input.launcherConfig];
  if (!input.systemdScopeSupported) {
    return { command: process.execPath, args: launcherArgs, systemdScopeUnit: null };
  }

  const systemdScopeUnit = `paperclip-run-${input.runId}`;
  return {
    command: "systemd-run",
    args: [
      "--scope",
...
function canLaunchSystemdScopes(env: NodeJS.ProcessEnv) {
  if (process.platform !== "linux" || env.PAPERCLIP_DISABLE_SYSTEMD_SCOPES === "true") {
    return false;
  }
  if (systemdScopeSupport !== null) return systemdScopeSupport;

  const probeUnit = `paperclip-run-probe-${process.pid}-${randomUUID()}`;
  const probe = spawnSync(
    "systemd-run",
    ["--scope", "--quiet", "--collect", `--unit=${probeUnit}`, process.execPath, "-e", ""],
    { env, stdio: "ignore", timeout: 2_000 },
  );
  systemdScopeSupport = probe.status === 0 && !probe.error;
  return systemdScopeSupport;
}
...
async function runJournaledChildProcess(
  runId: string,
  command: string,
  opts: {
    cwd: string;
    ...
    onSpawn?: (meta: RunProcessSpawnMetadata) => Promise<void>;
    terminalResultCleanup?: TerminalResultCleanupOptions;
    stdin?: string;
    journal: RunProcessJournalOptions;
  },
  target: SpawnTarget,
  mergedEnv: NodeJS.ProcessEnv,
): Promise<RunProcessResult> {
  await fs.mkdir(opts.journal.spoolDir, { recursive: true });
  const stdoutPath = path.join(opts.journal.spoolDir, "stdout.log");
  const stderrPath = path.join(opts.journal.spoolDir, "stderr.log");
  const exitPath = path.join(opts.journal.spoolDir, "exit.json");
  const exitTempPath = path.join(opts.journal.spoolDir, `exit.${process.pid}.${randomUUID()}.tmp`);
  await Promise.all([
  ...
  const launcherConfig = JSON.stringify({
    ...
    exitPath,
    exitTempPath,
    hasStdin: opts.stdin != null,
  });
  const launcherTarget = buildJournalLauncherSpawnTarget({
    runId,
    launcherConfig,
    systemdScopeSupported: canLaunchSystemdScopes(childEnv),
  });
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    const child = spawn(launcherTarget.command, launcherTarget.args, {
      ...
      shell: false,
      stdio: [opts.stdin != null ? "pipe" : "ignore", "ignore", "ignore"],
    ...
    const spawnPersistPromise =
      typeof child.pid === "number" && child.pid > 0 && opts.onSpawn
        ? opts.onSpawn({
            ...
            startedAt,
            ioMode: "journal",
            processPreservable: opts.journal.processPreservable ?? opts.stdin == null,
          }).catch((err) => {
            opts.onLogError(err, runId, "failed to record journaled child process metadata");
          })
        : Promise.resolve();
    ...
    let tailChain: Promise<void> = Promise.resolve();
    ...
    const maybeArmTerminalResultCleanup = () => {
      ...
      if (!terminalCleanup || terminalCleanupStarted || timedOut || terminalResultSeen) return;
      try {
        terminalResultSeen = terminalCleanup.hasTerminalResult({ stdout, stderr });
      } catch (err) {
        opts.onLogError(err, runId, "failed to inspect journaled terminal adapter output");
      }
      ...
      }, Math.max(0, terminalCleanup.graceMs ?? 5_000));
    };
    ...
    const tail = async () => {
      while (!tailStopped) {
        await drainAvailable();
        await new Promise((tailResolve) => setTimeout(tailResolve, JOURNAL_TAIL_INTERVAL_MS));
      }
      await drainAvailable();
    };
    tailChain = tail().catch((err) => {
      opts.onLogError(err, runId, "failed to tail journaled child output");
    });
...
export async function runChildProcess(
  ...
  opts: {
    ...
    remoteExecution?: RemoteExecutionSpec | null;
    localProcessSandbox?: LocalProcessSandboxOptions | null;
    journal?: RunProcessJournalOptions | null;
  },
): Promise<RunProcessResult> {
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    void resolveSpawnTarget(command, args, opts.cwd, mergedEnv, {
      ...
      .then((target) => {
        if (opts.journal && !opts.remoteExecution) {
          return runJournaledChildProcess(
            runId,
            command,
            {
              ...opts,

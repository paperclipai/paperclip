...
> {
  if (entry.required) {
    return {
      ...
      originLabel: "Required by Standard Power",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Standard Power",
    readOnly: false,
  };
}
...
export function renderPaperclipWakePrompt(
  ...
  const lines = resumedSession
      ? [
        "<!-- PAPERCLIP_WAKE_PAYLOAD_START -->",
        "## Resume Delta",
        "",
        "You are resuming an existing Standard Power session.",
  ...
  lines.push("<!-- PAPERCLIP_WAKE_PAYLOAD_END -->");
  return lines.join("\n").trim();
}
    ...
    return Promise.all(dirs.map(async (entry) => {
      ...
      return {
        ...
        requiredReason: required
          ? "Bundled Standard Power skills are always available for local adapters."
          : null,
      };
    }));
  } catch {
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Standard Power skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Standard Power cannot find this skill in the local runtime skills directory.",
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error(`Timed out waiting for skill materialization lock at ${lockDir}`);
      }
...
export async function materializePaperclipSkillCopy(
  ...
  if (!rootStat.isDirectory()) {
    throw new Error("Standard Power skills must be directories.");
  }
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the Standard Power server itself is started from

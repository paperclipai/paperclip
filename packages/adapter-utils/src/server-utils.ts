...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- Compose API requests through the normalized base, never by appending to the raw value: in the shell run `PAPERCLIP_API_BASE=\"${PAPERCLIP_API_URL%/}\"; PAPERCLIP_API_BASE=\"${PAPERCLIP_API_BASE%/api}\"`, then call `$PAPERCLIP_API_BASE/api/...`. `PAPERCLIP_API_URL` is the server root and may or may not already end in `/api`; every endpoint path named in this contract starts with `/api`. A request without the prefix returns the web app's HTML, not JSON.",

...
export function renderPaperclipChatWakePrompt(
  ...
  const lines = resumedSession
    ...
    : [
        ...
        "- When the operator asks for task details missing from the pack, use **Paperclip MCP tools** (`paperclipGetIssue`, `paperclipGetDocument`, `paperclipListComments`, `paperclipGetHeartbeatContext`, `paperclipListIssues`).",
        "- Prefer MCP reads over inbox/checkout workflow — this is chat mode, not a heartbeat run.",
        ...
        "",
        "**Do not:**",
        "- run heartbeat Steps 1–4 (`/api/agents/me`, inbox, pick tasks from queue)",
        "- checkout or mutate issues unless the operator explicitly asks to delegate or update work",
        "",
        `- session: ${normalized.threadId}`,
        `- run: ${normalized.runId ?? "unknown"}`,
      ];

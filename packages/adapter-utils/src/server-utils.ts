...
export function buildPaperclipEnv(agent: { id: string; companyId: string }): Record<string, string> {
  const isLoopbackHostname = (host: string | null | undefined): boolean => {
  ...
  // Agents run in-container next to the server, so they must reach it over a
  // loopback URL. NEVER inherit the boot PAPERCLIP_API_URL / RUNTIME_API_URL
  // when it points at a remote (Tailscale/DNS) hostname — it fails DNS from
  // inside Railway. Fall back to the local listen host/port.
  const configuredApiUrl = (process.env.PAPERCLIP_API_URL ?? "").trim();
  const runtimeApiUrl = (process.env.PAPERCLIP_RUNTIME_API_URL ?? "").trim();
  const loopbackCandidate = [configuredApiUrl, runtimeApiUrl]
    .find((url) => {
      try {
  ...
  const apiUrl = loopbackCandidate ?? `http://${runtimeHost}:${runtimePort}`;

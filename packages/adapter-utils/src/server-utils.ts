...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- Call the Paperclip API at `$PAPERCLIP_API_URL/api/...`: `PAPERCLIP_API_URL` is the server root and does not include the `/api` prefix, and endpoint paths named in this contract already start with `/api`. A request without the prefix returns the web app's HTML, not JSON.",

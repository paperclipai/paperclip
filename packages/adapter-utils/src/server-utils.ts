...
const AITEAMCORP_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];
...
> {
  ...
  return {
    ...
    originLabel: "Managed by AiTeamCorp",
    readOnly: false,
  };
}
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "AITEAMCORP_RESOLVED_COMMAND"] = resolvedCommand;
  }
...
export function buildAiTeamCorpEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    AITEAMCORP_AGENT_ID: agent.id,
    AITEAMCORP_COMPANY_ID: agent.companyId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.AITEAMCORP_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.AITEAMCORP_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.AITEAMCORP_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.AITEAMCORP_API_URL = apiUrl;
  return vars;
}
...
export async function resolveAiTeamCorpSkillsDir(
  ...
  const candidates = [
    ...AITEAMCORP_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listAiTeamCorpSkillEntries(
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `aiteamcorporated-collab/ai-team-coprorated/${entry.name}`,
...
export function readAiTeamCorpSkillSyncPreference(config: Record<string, unknown>): {
  ...
  const raw = config.aiteamcorpSkillSync;
...
export function writeAiTeamCorpSkillSyncPreference(
  ...
  const next = { ...config };
  const raw = next.aiteamcorpSkillSync;
  const current =
  ...
  next.aiteamcorpSkillSync = current;
  return next;
}

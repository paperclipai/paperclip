...
describe("sandbox adapter execution targets", () => {
  ...
  async function runProxyWithInput(command: string, input: string): Promise<{ stdout: string; stderr: string; code: number | null }> {
    ...
    const code = await new Promise<number | null>((resolve, reject) => {
      ...
      // `exit` can precede the final stdout/stderr chunks. Wait for `close` so
      // assertions observe the complete proxy stream under parallel test load.
  ...
  describe("streamed output (streamOutputViaSession)", () => {
    ...
    it("keeps the agent command on the persistent session and forces bridge control execs off it", async () => {
      ...
      const bridge = await startAdapterExecutionTargetProcessSessionBridge({
        // Source and compiled test artifacts run side by side in the package
        // suite. Keep their transient process registry keys distinct.
        runId: `run-stream-isolation-${randomUUID()}`,
        target,
        runtimeRootDir: path.posix.join(rootDir, ".paperclip-runtime", "acpx"),
        adapterKey: "acpx",
        command: process.execPath,

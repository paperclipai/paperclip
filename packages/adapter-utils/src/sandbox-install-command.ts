...
// @google/gemini-cli. The bootstrap also sets VALADRIEN_OS_NPM_BOOTSTRAPPED=1
...
const ENSURE_NPM_PREAMBLE =
  ...
  "VALADRIEN_OS_NPM_BOOTSTRAPPED=1; " +
  "fi;";
...
export function buildSandboxNpmInstallCommand(packageName: string): string {
  ...
  return [
    ENSURE_NPM_PREAMBLE,
    'if [ -n "$VALADRIEN_OS_NPM_BOOTSTRAPPED" ]; then',

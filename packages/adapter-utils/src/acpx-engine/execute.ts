    ...
    try {
      ...
      const stepTurnFinalize = async (
        ...
        capturedResult = {
          ...
          // A quota refusal is classified in `classifyAcpxError`; carry its family
          // and stated reset through, otherwise recovery sees a bare
          // `acpx_turn_failed` and retries immediately instead of waiting.
          ...(timedOut
            ? {}
            : {
                errorFamily: emitted?.classified.errorFamily ?? null,
                retryNotBefore: emitted?.classified.retryNotBefore ?? null,
              }),
          ...billingFields,
          ...referencedProjectStagingFailuresField,
          model: prepared.requestedModel || null,

    ...
    sandboxSite = createSandboxRunSite({
      ...
      startPaperclipBridge: (runtimeRootDir) =>
        startAdapterExecutionTargetPaperclipBridge({
          ...
          // Fork (TSMC-20877): standard net-fetch door, scoped to the company.
...
export function createAcpxEngineExecutor(deps: AcpxEngineExecutorOptions = {}) {
  ...
  return async function executeAcpxEngine(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
    ...
    try {
      ...
      // Fork: the full output text (non-thought deltas plus the in-run
      // disposition re-ask reply). It feeds the PAPERCLIP_DISPOSITION extraction
      // and the opaque-error limit-text surfacing; `outputSegments` still drive
      // the run summary.
      const textParts: string[] = [];
      // Fork: per-run token budget (weighted accounting, TSMC-20840). A hard
      // run ceiling: cancel the turn before the next model step can overshoot it.
      ...
      const stepEventRelay = async (started: StartedTurn): Promise<AcpRuntimeTurnResult> => {
        const turn = activeTurn as AcpRuntimeTurn;
        const toolTitles = new Map<string, string>();
        // Fork: cancel the turn once the weighted budget is reached, or once one
        // more model step (the largest observed, at least the minimum reserve)
        // could no longer fit under it.
      ...
      const stepTurnFinalize = async (
        ...
        if (
          ...
          try {
            ...
            // The re-ask reply feeds only the disposition extraction (textParts).
            // It must NOT become an output segment: the run summary is the last
            // output segment of the agent's own turn, and a trailing re-ask
            // segment (at best the bare marker line, at worst replayed prose)
            // would displace it.
        ...
        runtimeSettlement = {
          ...
          // Fork: a token-budget cancel keeps the persistent session state so a
          // later, separately-budgeted run can resume it.
          discardPersistentState: (terminal.status === "cancelled" && !tokenBudgetExhausted) || timedOut || channelLost,
          dropWarmEntry: false,
        ...
        const terminalStopReason = tokenBudgetExhausted
          ...
          : terminal.status === "failed"
            ...
            : terminal.stopReason;
        // Fork: the final PAPERCLIP_DISPOSITION record rides the result; the
        // marker is stripped from the summary segments.
        const { disposition } = extractPaperclipDisposition(textParts.join("").trim());
        const summarySegments = disposition
          ? outputSegments.map((segment) => extractPaperclipDisposition(segment).cleanedText)
          : outputSegments;
        await emitAcpxLog(ctx, {
        ...
        capturedResult = {
          ...
          summary: buildAcpxRunSummary({
            outputSegments: summarySegments,
        ...
        if (terminal.status === "cancelled") {
          return {
            ...
            cause: {
              kind: "turn_cancelled",
              reason: tokenBudgetExhausted ? "token_budget_exhausted" : (terminal.stopReason ?? "cancelled"),
            },
            resources: emptyConsumed,
          };
        }

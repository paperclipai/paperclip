...
async function ensureSymlink(target: string, source: string): Promise<void> {
  ...
  // Two runs for the same company can prepare the managed home at once. Treat
  // a target appearing between lstat and symlink as a normal contention case,
  // then re-check its final link rather than leaking EEXIST to the run.
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    if (!existing) {
      ...
      try {
        ...
        if (isErrnoException(err, "EEXIST")) continue;
        throw err;
      }
    }
  ...
  throw new Error(`Unable to prepare managed symlink after concurrent updates: ${target}`);
}

      ...
      createRuntime: () =>
        turnRuntime({
          onStartTurn: (turnInput) => {
            // fork law: the in-run disposition re-ask issues a second startTurn when
            // the agent stated no PAPERCLIP_DISPOSITION; characterize the first (real) turn.
            if (!captured) captured = turnInput;
          },
          events: async function* () {
            yield { type: "done", stopReason: "end_turn" };
          },
    ...
    const firstExecute = createAcpxEngineExecutor({
      createRuntime: () =>
        turnRuntime({
          ...
          events: async function* () {
            // fork law: a stated disposition prevents the in-run re-ask's extra startTurn.
            yield { type: "text_delta", text: 'PAPERCLIP_DISPOSITION {"status":"done","hasBlocker":false}', stream: "output", tag: "agent_message_chunk" };
            yield { type: "done", stopReason: "end_turn" };
          },
          result: Promise.resolve({ status: "completed", stopReason: "end_turn" }),
        }) as never,

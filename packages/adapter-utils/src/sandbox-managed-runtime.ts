...
export async function prepareSandboxManagedRuntime(input: {
  ...
  const baselineSnapshot = await captureDirectorySnapshot(input.workspaceLocalDir, {
    exclude: [...new Set([".valadrien-os-runtime", ...(input.preserveAbsentOnRestore ?? []), ...(input.workspaceExclude ?? [])])],
  });
  ...
  await withTempDir("valadrien-os-sandbox-sync-", async (tempDir) => {
    ...
    const preservedNames = new Set([".valadrien-os-runtime", ...(input.preserveAbsentOnRestore ?? [])]);

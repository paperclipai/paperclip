...
import { createPhase2BrowserMiddleware } from "./phase2-browser-server.mjs";
...
function fakeRunner() {
  ...
  return {
    ...
    async load() {
      return {
        async startPhase2Scenario({ scenario }) {
          startCalls += 1;
    ...
    const request = httpRequest(
      {
        hostname: "127.0.0.1",
        port,
        path: options.path ?? "/api/phase2/runs",
...
describe("Phase 2 browser transport hardening", () => {
  ...
  it("denies cross-origin starts and applies the guard before route lookup", async () => {
    ...
    const unknown = await sendRequest(port, {
      method: "GET",
      path: "/api/phase2/runs/missing/events",
      origin: "https://attacker.example",
    });
  ...
  it("caps active runs and evicts the oldest completed run at the retention bound", async () => {
    ...
    const evicted = await sendRequest(port, {
      method: "GET",
      path: `/api/phase2/runs/${first.body.id}/events`,
    });

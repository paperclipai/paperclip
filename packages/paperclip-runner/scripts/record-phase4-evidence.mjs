...
async function recordPhase4Evidence() {
  const scratchRoot =
  ...
  const deniedHostPaths = createPhase4DeniedHostPaths({
    hostHome,
  ...
  try {
    ...
    if (run.error) {
      throw new Error(`Phase 4 tracer could not start: ${run.error.message}`);
    }
    ...
    try {
      ...
      throw new Error(
        `Phase 4 tracer did not produce a readable trace (status ${run.status ?? "unknown"}): ${detail}`,
      );
    }
    assertPhase4TraceReadyForOutputs(trace);
    if (run.status !== 0) {
      throw new Error(
        `Phase 4 tracer exited with status ${run.status ?? "unknown"} after reporting an accepted \`done\` result`,
      );
    }
    const hello = await readRequiredPhase4Output(join(workspace, "hello.txt"), "hello.txt");
    if (hello !== "hello from phase 4") {
      throw new Error("Phase 4 safe task produced unexpected hello.txt content");
    }
    const isolationProof = await readRequiredPhase4Output(
...
try {
  await recordPhase4Evidence();
} catch (error) {
  const message = error instanceof Error ? error.message : String(error);
  process.stderr.write(`Phase 4 evidence recording failed: ${message}\n`);
  process.exitCode = 1;
}

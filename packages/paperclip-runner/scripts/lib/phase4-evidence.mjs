...
export function createPhase4DeniedHostPaths({
  hostHome,
...
export function assertPhase4TraceReadyForOutputs(trace) {
  ...
  if (decision !== "accepted" || disposition !== "done") {
    const summary = phase4TraceSummary(trace);
    throw new Error(
      "Phase 4 recorder requires an accepted `done` result before reading workspace outputs; " +
  ...
  if (failedAssertions.length > 0) {
    throw new Error(
      `Phase 4 tracer reported failed assertions: ${failedAssertions.join(", ")}`,
    );
  }
}

export async function readRequiredPhase4Output(path, displayName) {
  try {
    return await readFile(path, "utf8");
  } catch (error) {
    if (error && typeof error === "object" && error.code === "ENOENT") {
      throw new Error(
        `Phase 4 recorder expected ${displayName} after an accepted \`done\` result, but the file was not created.`,
      );
    }
    const detail = error instanceof Error ? error.message : String(error);
    throw new Error(`Phase 4 recorder could not read ${displayName}: ${detail}`);
  }
}

function phase4TraceSummary(trace) {

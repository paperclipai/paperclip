...
| `reassign_task` | `optional_agent_tool` | `delegation:tasks:assign` | `standard`<br>`skill_test` | `company_write` | `required` | no | `semantic_command:reassign_task` | `scenario` + `live`<br>`live_codex` | `PaperclipRunnerToolAuthority.reassign_task`<br>semantic-operation item event plus company-entity state diff and audit record<br>catalog PRP status: `bound` |

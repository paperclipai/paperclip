...
fn assigned_gateway_binding_reaches_qualified_sidecar_without_unrelated_secrets() {
    ...
    if std::env::var_os(CHILD).is_none() {
        let status = std::process::Command::new(std::env::current_exe().unwrap())
            .args(["--exact", "assigned_gateway_binding_reaches_qualified_sidecar_without_unrelated_secrets", "--nocapture"])
            .env(CHILD, "1")
            .env("PAPERCLIP_NATIVE_MCP_NAME", "paperclip-assigned")
            .env("PAPERCLIP_NATIVE_MCP_URL", "http://127.0.0.1:3100/mcp")
            .env("PAPERCLIP_NATIVE_MCP_TOKEN", "fixture-token-never-returned-in-test-output")
    ...
    for agent in ["claude", "codex"] {
        let mut sidecar = AcpxSidecarTransport::start_for_agent(&AcpxSidecarTransportConfig {
            command: PathBuf::from(env!("CARGO_BIN_EXE_fake-acpx-sidecar")),
        ...
        let response = sidecar.request(GeneratedAcpxSidecarCommand::Initialize, json!({})).unwrap();
        assert_eq!(response["name"], "paperclip-assigned");
        assert_eq!(response["url"], "http://127.0.0.1:3100/mcp");
        assert_eq!(response["hasToken"], true, "assigned gateway credential was dropped");

...
export const PHASE0_FIXTURE_SCHEMA = "paperclip.runner.phase0.fixture.v1";

export interface Phase0Fixture {
  schemaVersion: typeof PHASE0_FIXTURE_SCHEMA;
  run: NativeRunIdentity;
  events: NativeRunEvent[];
  result: NativeRunResult;
}

export const phase0FixtureUrl = new URL(
  "../../protocol/fixtures/phase-00-minimal-run.json",
  import.meta.url,
);
...
function parseResult(value: unknown, runId: string): NativeRunResult {
  ...
  if (status !== "succeeded") {
    throw new Error("the Phase 0 fixture result must be succeeded");
  }
  return {
    runId,
    status,
...
export function validatePhase0Fixture(value: unknown): Phase0Fixture {
  const fixture = asRecord(value, "fixture");
  if (fixture.schemaVersion !== PHASE0_FIXTURE_SCHEMA) {
    throw new Error(`schemaVersion must be ${PHASE0_FIXTURE_SCHEMA}`);
  }
  const run = parseRun(fixture.run);
  return {
    schemaVersion: PHASE0_FIXTURE_SCHEMA,
    run,
...
export async function loadPhase0Fixture(
  url: URL = phase0FixtureUrl,
): Promise<Phase0Fixture> {
  const contents = await readFile(url, "utf8");
  return validatePhase0Fixture(JSON.parse(contents) as unknown);
}

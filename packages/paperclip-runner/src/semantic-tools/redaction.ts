...
export const PHASE7_REDACTED = "[REDACTED]";
...
export function redactSemanticValue(value: unknown, key = ""): Phase7JsonValue {
  if (SENSITIVE_KEY.test(key)) return PHASE7_REDACTED;
  if (typeof value === "string") {
    return value
      .replace(SECRET_VALUE, PHASE7_REDACTED)
      .replace(SECRET_QUERY, `$1${PHASE7_REDACTED}`);
  }

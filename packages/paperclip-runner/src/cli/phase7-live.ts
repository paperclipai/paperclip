...
function operationRevision(session: Phase7LiveSession): number | null {
...
async function main(): Promise<void> {
  ...
  const service = new Phase7LiveSessionService({
    transportOptions: {
      onDiagnostic: (message) => process.stderr.write(`[phase7 runnerd] ${message}\n`),
    },
  });
  let session: Phase7LiveSession | null = null;
  try {
    ...
    const first = await session.sendMessage([
      "Call report_progress exactly once with idempotencyKey phase7-live-smoke-1",
    ...
    await service.shutdown(session.id, "Phase 7 live smoke complete");
    ...
    const output = {
      schema: "paperclip.phase7.live-smoke.v1",
      providerThreadId,
    ...
    if (session !== null && session.snapshot().status !== "closed") {
      await service.shutdown(session.id, "Phase 7 live smoke cleanup");
    }
  }
}

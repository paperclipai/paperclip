...
import {
  InMemoryPhase7LiveSessionStore,
...
interface FakeProviderState {
  ...
  transports: FakePhase7CodexTransport[];
}

class FakePhase7CodexTransport implements CodexAppServerTransport {
...
function fakeTransportFactory(state: FakeProviderState): Phase7LiveTransportFactory {
  return (options) => {
    const evidence = {
      ...
      diagnostics: ["paperclip-runnerd: phase7 codex proxy started"],
    };
    const transport = new FakePhase7CodexTransport(state, () => {
...
function providerState(): FakeProviderState {
  return {
    threadId: "codex-thread-phase7",
    providerSessionId: "codex-provider-phase7",
    nextTurn: 0,
...
describe("Phase 7 live runnerd and Codex session", () => {
  it("returns typed mock results to the same multi-turn Codex thread without Paperclip network calls", async () => {
    const state = providerState();
    const fetchSpy = vi.spyOn(globalThis, "fetch");
    const service = new Phase7LiveSessionService({ transportFactory: fakeTransportFactory(state) });
  ...
  it("restores transcript, mock state, and pending interactions before resuming the same Codex thread", async () => {
    const state = providerState();
    const store = new InMemoryPhase7LiveSessionStore();
    const firstService = new Phase7LiveSessionService({
      store,
    ...
    const resumedService = new Phase7LiveSessionService({
      store,
      transportFactory: fakeTransportFactory(state),
    });
    const resumed = await resumedService.restore(first.id);
    expect(resumed.snapshot().providerThreadId).toBe("codex-thread-phase7");

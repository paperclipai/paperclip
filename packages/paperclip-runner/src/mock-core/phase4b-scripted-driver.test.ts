...
import { validatePrpEvent, type PrpEvent } from "../protocol/phase1-contract.js";
import {
  applyPrpEvent,
...
import {
  PHASE4B_DEMO_MANIFESTS,
  findPhase4bDemoManifest,
  phase4bDemoManifestCatalogue,
} from "./phase4b-demo-manifests.js";
import { Phase4bScriptedDriver } from "./phase4b-scripted-driver.js";
...
async function openSession(manifestId: string, chunkDelayMs = 0) {
  const driver = new Phase4bScriptedDriver({ manifestId, chunkDelayMs });
...
describe("Phase 4b demo manifests", () => {
  it("gives every manifest a unique id and at least one expected observation", () => {
    const ids = PHASE4B_DEMO_MANIFESTS.map((manifest) => manifest.id);
    expect(new Set(ids).size).toBe(ids.length);
    for (const manifest of PHASE4B_DEMO_MANIFESTS) {
  ...
  it("publishes a catalogue that carries no beat scripts to the browser", () => {
    const catalogue = phase4bDemoManifestCatalogue();
    expect(catalogue).toHaveLength(PHASE4B_DEMO_MANIFESTS.length);
    expect(JSON.stringify(catalogue)).not.toContain("beats");
  });

  it("rejects an unknown manifest id", () => {
    expect(() => findPhase4bDemoManifest("nope")).toThrow(/unknown Phase 4b demo manifest/);
  });
});

describe("Phase 4b scripted driver", () => {

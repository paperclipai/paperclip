...
import type { Phase7FixtureSeed } from "./phase7-control-plane-types.js";
import {
  Phase7MockControlPlaneAdapter,
  Phase7MockControlPlaneError,
} from "./phase7-mock-control-plane-adapter.js";
...
function seeded(seed: Phase7FixtureSeed = {}) {
  return new Phase7MockControlPlaneAdapter({
    workspaceServices: [
      {
        id: "service-1",
        taskId: "task-1",
...
describe("Phase7MockControlPlaneAdapter", () => {
  ...
  it("uses stable typed errors for retryable scripted failures", async () => {
    ...
    expect(error).toBeInstanceOf(Phase7MockControlPlaneError);

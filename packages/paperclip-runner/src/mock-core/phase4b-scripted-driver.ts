...
import {
  findPhase4bDemoManifest,
  type Phase4bDemoBeat,
  type Phase4bDemoManifest,
} from "./phase4b-demo-manifests.js";

const DRIVER_KIND = "phase4b-scripted-demo";
...
export interface Phase4bScriptedDriverOptions {
  manifestId?: string;
  manifest?: Phase4bDemoManifest;
...
class Phase4bScriptedSession implements HarnessSession {
  ...
  async #runTurn(turnId: string, beats: readonly Phase4bDemoBeat[]): Promise<void> {
  ...
  async #runBeat(turnId: string, beat: Phase4bDemoBeat): Promise<boolean> {
  ...
  #emitResult(turnId: string, failed: boolean): void {
    ...
    const result: PrpStructuredRunResult = {
      ...
      verification: [
        {
          commandOrCheck: `phase4b demo manifest ${manifest.id}`,
 ...
 */
export class Phase4bScriptedDriver implements HarnessDriver {
  readonly #manifest: Phase4bDemoManifest;
  readonly #chunkDelayMs: number;
  readonly #now: () => Date;
  #session: Phase4bScriptedSession | null = null;

  constructor(options: Phase4bScriptedDriverOptions = {}) {
    this.#manifest = options.manifest ??
      findPhase4bDemoManifest(options.manifestId ?? "completion");
    this.#chunkDelayMs = options.chunkDelayMs ?? 45;
    this.#now = options.now ?? (() => new Date());
  }

  get manifest(): Phase4bDemoManifest {
  ...
  async descriptor(): Promise<HarnessDriverDescriptor> {
    return {
      kind: DRIVER_KIND,
      displayName: `Phase 4b demo — ${this.#manifest.name}`,
      version: "1",
      protocolVersion: "phase4b-demo-v1",
  ...
  async openSession(input: OpenHarnessSessionInput): Promise<HarnessSession> {
    const session = new Phase4bScriptedSession({
      ...input,

...
pnpm --filter @paperclipai/paperclip-runner record:capability:ui
...
pnpm --filter @paperclipai/paperclip-runner record:capability:ui:live
```
...
`knowledge/evidence/capability/ui/index.md`, so `check:capability:ui` needs that same
...
`knowledge/evidence/capability/ui-live/` and is excluded from the determinism gate.

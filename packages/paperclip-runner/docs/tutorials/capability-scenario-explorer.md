...
`knowledge/evidence/capability/eval-parity-report.{json,md}`. It is a
...
pnpm --filter @paperclipai/paperclip-runner record:capability

# Documentation links and the OKF knowledge bundle (run with no generated
# report present).
...
- **The parity report breaks the OKF bundle if left in place.**
  `report:capability-evals` writes `eval-parity-report.md` into the knowledge
  bundle without OKF frontmatter, so `docs:validate` fails while it is present.
  Delete it before validating. A clean checkout never contains it. This is
  recorded for the Capability QA gate.
...
- Plan: Capability plan, issue `PAP-16897` (see its `#document-plan` document).
- Child issues: 7A capability inventory (PAP-16898), 7B UX map (PAP-16899),
  7C mock adapter (PAP-16900), 7D semantic tools and authorization (PAP-16901,
  security gate PAP-16902, remediation PAP-16909), 7E conformance suite
  (PAP-16903), 7F browser explorer (PAP-16904, UX gate PAP-16905), 7G docs and
  evidence (PAP-16906), 7H clean-room QA (PAP-16907), Capability checkpoint
  (PAP-16908).

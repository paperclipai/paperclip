...
The report is written to `knowledge/evidence/capability/eval-parity-report.{json,md}`.
...
`knowledge/evidence/capability/ui/`. Every mobile capture asserts no horizontal
...
The frames land in `knowledge/evidence/capability/ui-live/`. They are intentionally

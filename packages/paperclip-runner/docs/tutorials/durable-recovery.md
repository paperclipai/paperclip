# Durable recovery: Break It on Purpose

## What this phase is

Durable recovery adds a durable outbound WebSocket connection.
...
```sh
pnpm --filter @paperclipai/paperclip-runner trace:durable-recovery -- --fault socket-drop
```
...
```sh
pnpm --filter @paperclipai/paperclip-runner trace:durable-recovery -- --fault runner-restart
```
...
```sh
pnpm --filter @paperclipai/paperclip-runner trace:durable-recovery -- --fault harness-restart
```
...
```sh
pnpm --filter @paperclipai/paperclip-runner trace:durable-recovery -- --fault duplicate-command
```
...
```sh
pnpm --filter @paperclipai/paperclip-runner trace:durable-recovery -- --fault storage-pressure
```
...
```sh
pnpm --filter @paperclipai/paperclip-runner trace:durable-recovery -- --fault lease-expiry
pnpm --filter @paperclipai/paperclip-runner trace:durable-recovery -- --fault drain
pnpm --filter @paperclipai/paperclip-runner trace:durable-recovery -- --fault revoke
```
...
to run these cases with **Run Durable recovery**:
...
conformance, replay, local execution, and durable recovery.

## Evidence

- [Durable recovery fault matrix](../../knowledge/evidence/durable-recovery-fault-matrix.json)
- [Durable recovery trace](../../knowledge/evidence/durable-recovery-trace.json)
- [Durable recovery transport reference](../durable-recovery.md)

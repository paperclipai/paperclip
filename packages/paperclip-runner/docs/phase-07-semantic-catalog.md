# Phase 7 semantic catalog and authorization

Phase 7D adds a transport-neutral tool boundary over the deterministic mock
...
```ts
import {
  Phase7SemanticDispatcher,
  createPhase7ProviderNeutralBinding,
} from "@paperclipai/paperclip-runner";

const dispatcher = new Phase7SemanticDispatcher(mockPort, {
  scenario: {
...
`createPhase7ProviderNeutralBinding("fake")` and
`createPhase7ProviderNeutralBinding("live_codex")` return identical contract
...
`generated/phase-07/semantic-tool-contracts.json`. Phase 7A traceability remains
under `generated/phase-07/` and is checked with `check:phase-07-contract`.

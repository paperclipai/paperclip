...
| Screenshot acceptance set | `record:capability` | 24 deterministic images |
...
| Clean-room screenshots | `record:capability:cleanroom` | 7 images; intentionally not byte-stable |
...
- **The parity report is generated on demand.** `report:capability-evals` writes
  `knowledge/evidence/capability/eval-parity-report.{json,md}`, which are not
  ...
  **before** generating the report, or delete the report first. This dirty-tree
  interaction is tracked for the Capability QA gate (`PAP-16907`).

# Phase 4b Component Decision Record — shadcn/ui and Vercel AI Elements
...
for the authorized Phase 4b browser surface.
...
3. **Dependency budget: zero new runtime dependencies** for the demo app
   ...
   Rationale: Phase 4b is a proof under `packages/paperclip-runner/` that
   Phase 5 will freeze into an SDK — every dependency added here becomes an
...
5. **Token gaps are system changes.** Phase 4b needs a few new tokens (see
...
**REJECT (with reasons; revisit in Phase 5 where noted):**

| Primitive | Reason |
|---|---|
| `Response` (streamed markdown) | pulls `streamdown`/markdown pipeline; tracer renders exact text — markdown prettification can hide protocol truth. Phase 5 candidate for the SDK console. |
| `CodeBlock` | needs `shiki` highlighting dep; `<pre>` + mono token suffices for a tracer. Phase 5 candidate. |
...
| `Context` (token/cost meter) | usage belongs in the inspector Session tab as plain data; a persistent meter overweights cost in a tracer. Phase 5 candidate. |
| `Artifact`, `WebPreview`, `Image`, `Attachments`, `OpenIn`, `Suggestion`, `Queue` | no protocol counterpart in Phase 4b; adopting them would invent affordances the runner cannot honor (fabricated-controls rule). |
...
| Primitive | Reason |
|---|---|
| `Command` (cmdk palette) | the goal surface has five fixed verbs — a searchable palette adds a dep and search UI for nothing (Hick's law works in our favor with a plain menu). Revisit only if goal/command vocabulary grows in Phase 5+. |
...
| `Sheet`/`Drawer`, `Popover`, `Select`, `Switch`, `Progress`, `Skeleton`, `Toast`/`Sonner` | no Phase 4b surface needs them; toasts specifically are rejected because failures must live in the transcript record, not ephemeral notifications (map §1.5). |
...
- Do not restyle existing Phase 1–3 surfaces beyond shared-token additions;
...
- [x] Interaction map cross-references: every ADAPT row maps to a surface in
      the [interaction map](phase-4b-interaction-map.md); every surface has a
      component owner.
- [x] Annotated mockups rendered at 1440×900 and 390×844 — see
      [`mockups/`](mockups/).

...
recorded by `pnpm run record:scenario-chat`.
...
(annotated desktop + mobile renders in `knowledge/evidence/capability/`,
`scenario-chat-*` slugs).
...
Evidence lands in `knowledge/evidence/capability/` as
...
Re-acceptance: re-run `record:scenario-chat` and hand back only the two
...
Reproducibility spot-check in the review run: `record:scenario-chat` re-run

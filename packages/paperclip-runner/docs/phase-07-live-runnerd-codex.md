# Phase 7 live runnerd and Codex loop

Phase 7E binds the provider-neutral semantic catalog to a real package-local
...
```text
Phase7LiveSession -> paperclip-runnerd -> codex app-server
                  -> Phase7SemanticDispatcher -> ControlPlanePort mock
```
...
Import the package root and construct `Phase7LiveSessionService`:

```ts
import { Phase7LiveSessionService } from "@paperclipai/paperclip-runner";

const service = new Phase7LiveSessionService({ store });
...
Supply a durable implementation of `Phase7LiveSessionStore` for refresh and
...
the active Codex turn. Their typed `Phase7SemanticToolResult` is serialized into

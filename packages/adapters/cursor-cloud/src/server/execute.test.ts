...
describe("cursor_cloud execute", () => {
  ...
  it("creates a fresh Cursor agent and injects Odysseus env without CURSOR_API_KEY", async () => {
    ...
    expect(createMock.mock.calls[0]?.[0]).toMatchObject({
      ...
      name: "Odysseus Cursor Cloud Agent",
      model: { id: "gpt-5.4" },
      cloud: {
        env: { type: "cloud" },
    ...
    expect(createMock.mock.calls[0]?.[0]?.cloud?.envVars).toMatchObject({
      EXTRA_FLAG: "1",
      ODYSSEUS_RUN_ID: "run-heartbeat-1",
      ODYSSEUS_TASK_ID: "issue-1",
      ODYSSEUS_WAKE_REASON: "issue_commented",
      ODYSSEUS_API_KEY: "odysseus-run-jwt",
    });
  ...
  it("maps non-finished Cursor results to failing Odysseus runs", async () => {

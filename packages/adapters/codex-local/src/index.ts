...
- On local Linux/WSL runs, the managed curl launcher relays same-origin PAPERCLIP_API_URL requests through a run-scoped FIFO when the Codex command sandbox blocks loopback sockets. The bridge preserves the restricted network policy and never stores the run API token in launcher files.

...
- A Paperclip-managed CODEX_HOME is seeded by copying config.toml out of the host home ($CODEX_HOME, else ~/.codex), but the host's root-level model and model_provider keys are never inherited: Paperclip selects the model itself (--model, from adapterConfig.model) and the provider itself (the managed model_provider block from PAPERCLIP_CODEX_PROVIDERS), so a host pin would silently outrank both. [model_providers.*] definitions are kept; only the selection is dropped. An already-seeded managed home is repaired on the next seed. The host config is never modified, and an explicit external CODEX_HOME override is left entirely alone.

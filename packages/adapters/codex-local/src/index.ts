...
- A host-level OPENAI_API_KEY is stripped from the spawned Codex process at run time unless the adapter config explicitly provides one, so no empty OPENAI_API_KEY slot is persisted in agent config. Explicit CODEX_HOME overrides must not point at the shared company codex-home, $CODEX_HOME, or ~/.codex.

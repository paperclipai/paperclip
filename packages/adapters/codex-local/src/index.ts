...
- Managed homes inherit host authentication only. Host config, MCP servers, apps, plugins, global instructions, and unselected $HOME/.agents/skills are disabled by default; selected Paperclip skills and governed tool gateways are added per agent. An external CODEX_HOME is the explicit opt-in to a self-managed Codex profile.
- New and updated codex_local agents persist an empty OPENAI_API_KEY override by default so a host-level OPENAI_API_KEY cannot leak into Codex runs through process inheritance.

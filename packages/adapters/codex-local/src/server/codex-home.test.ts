...
describe("per-agent managed Codex homes", () => {
  it("uses an isolated home when agentId is set and retains the company home otherwise", () => {
    const env = { PAPERCLIP_HOME: "/srv/pc", PAPERCLIP_INSTANCE_ID: "inst" } as NodeJS.ProcessEnv;

    expect(resolveManagedCodexHomeDir(env, "company-1", "agent-1")).toBe(
    ...
    expect(resolveManagedCodexHomeDir(env, "company-1")).toBe(
      path.resolve("/srv/pc", "instances", "inst", "companies", "company-1", "codex-home"),
    );
  });

  it("seeds an isolated agent home from the live shared Codex login", async ({ skip }) => {
    if (!(await symlinkSupported())) {
      skip();
      return;
    }

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-codex-home-agent-"));
    try {
      const sharedHome = path.join(root, "shared-codex");
      await fs.mkdir(sharedHome, { recursive: true });
      await fs.writeFile(
        path.join(sharedHome, "auth.json"),
        JSON.stringify({ tokens: { account_id: "account", access_token: "test-access-token" } }),
        "utf8",
      );

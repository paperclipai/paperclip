...
import { resolveOdysseusInstanceRootForAdapter } from "@odysseus/adapter-utils/server-utils";
...
function isWorktreeMode(env: NodeJS.ProcessEnv): boolean {
  return TRUTHY_ENV_RE.test(env.ODYSSEUS_IN_WORKTREE ?? "");
}
...
export async function prepareManagedCodexHome(
  ...
  if (seedFromShared) {
    ...
    await onLog(
      "stdout",
      `[odysseus] Using ${isWorktreeMode(env) ? "worktree-isolated" : "Odysseus-managed"} Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
    );
  }
  ...
  if (apiKey) {
    ...
    await onLog(
      "stdout",
      `[odysseus] Wrote API-key auth.json into Codex home "${targetHome}" from configured OPENAI_API_KEY.\n`,
    );
  }

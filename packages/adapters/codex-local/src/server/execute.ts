...
async function pruneBrokenUnavailablePaperclipSkillSymlinks(
  ...
  for (const entry of entries) {
    ...
    await onLog(
      "stdout",
      `[aiteamcorp] Removed stale Codex skill "${entry.name}" from ${skillsHome}\n`,
    );
  }
}
    ...
    try {
      ...
      if (existing?.isSymbolicLink()) {
        ...
        if (
          ...
          await onLog(
            "stdout",
            `[aiteamcorp] Repaired Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
          );
          continue;
        }
      }
      ...
      await onLog(
        "stdout",
        `[aiteamcorp] ${result === "repaired" ? "Repaired" : "Injected"} Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[aiteamcorp] Failed to inject Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[aiteamcorp] Codex session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[aiteamcorp] Codex resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );

...
export function isCodexTransientUpstreamError(input: {
  ...
  // failure shape, plus ordinary API rate-limit errors that can be relieved by
  // rotating to another selected OpenAI credential. Provider quota is handled
  // separately so callers can preserve its retry/reset metadata.
  return (

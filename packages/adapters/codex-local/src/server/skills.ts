...
async function buildCodexSkillSnapshot(
  ...
  const desiredSkills = resolveOdysseusDesiredSkillNames(config, availableEntries);
  ...
  const entries: AdapterSkillEntry[] = availableEntries.map((entry) => ({
    ...
    originLabel: entry.required ? "Required by Odysseus" : "Managed by Odysseus",
    readOnly: false,
...
export function resolveCodexDesiredSkillNames(
  ...
  return resolveOdysseusDesiredSkillNames(config, availableEntries);
}

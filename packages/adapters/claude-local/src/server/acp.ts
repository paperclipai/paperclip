...
export async function resolveClaudeExecutionEngineForRun(
  ...
  // `command` is the legacy Claude CLI executable setting. ACP uses its own
  // `agentCommand` setting, so an auto-configured run that still carries a
  // CLI command must stay on the CLI lane. This preserves existing agents
  // while allowing newly configured agents without that legacy field to use
  // ACP by default.
  if (!selection.explicit && firstNonEmptyString(input.config.command)) {

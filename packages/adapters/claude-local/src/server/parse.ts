...
// Quota phrasing and reset-time parsing now live in adapter-utils so the ACPX
// engine classifies the same refusals this adapter does; keeping a second copy
...
export function extractClaudeRetryNotBefore(
  ...
  return extractProviderQuotaRetryNotBefore(buildClaudeTransientHaystack(input), now);
}
...
export function isClaudeProviderQuotaError(input: {
  ...
  return isProviderQuotaMessage(buildClaudeTransientHaystack(input));
}

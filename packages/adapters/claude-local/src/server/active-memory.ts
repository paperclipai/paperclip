/**
 * Active Memory Enforcement (VOG-5736)
 *
 * Reads agent memory files tagged with `enforcement: always-check` and
...
export type MemoryEnforcement = "always-check" | "triggered" | "optional";
...
async function parseMemoryFile(filePath: string): Promise<ActiveMemory | null> {
  try {
    ...
    const rawEnforcement = fm["enforcement"] ?? "optional";
    const enforcement =
      rawEnforcement === "always-check" || rawEnforcement === "triggered"
        ? (rawEnforcement as MemoryEnforcement)
        : "optional";
    const body = bodyAfterFrontmatter(content);
    const howToApplyMatch = HOW_TO_APPLY_RE.exec(body);
    const howToApply = howToApplyMatch?.[1]?.trim() ?? null;
    return { name, description, enforcement, howToApply };
  } catch {
    return null;
  }
}
...
/**
 * Load all memories tagged `enforcement: always-check` from the agent's memory directory.
...
export async function loadActiveMemories(cwd: string): Promise<ActiveMemory[]> {
  ...
  return parsed.filter((m): m is ActiveMemory => m !== null && m.enforcement === "always-check");
}

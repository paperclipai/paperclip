...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  } else if (runtimeSessionId && !hasMatchingWorkspace) {
    await onLog(
      "stdout",
      `[paperclip] Claude session "${runtimeSessionId}" was saved for workspace "${runtimeSessionWorkspaceId || "unknown"}" and will not be resumed in workspace "${workspaceId ?? "unknown"}".\n`,
    );
  }

...
import type { AdapterExecutionContext, AdapterExecutionResult } from "@aiteamcorp/adapter-utils";
...
async function buildClaudeRuntimeConfig(input: ClaudeExecutionInput): Promise<ClaudeRuntimeConfig> {
  ...
  const workspaceHints = Array.isArray(context.aiteamcorpWorkspaces)
    ...
    : [];
  const runtimeServiceIntents = Array.isArray(context.aiteamcorpRuntimeServiceIntents)
    ? context.aiteamcorpRuntimeServiceIntents.filter(
        (value): value is Record<string, unknown> => typeof value === "object" && value !== null,
      )
    : [];
  const runtimeServices = Array.isArray(context.aiteamcorpRuntimeServices)
    ? context.aiteamcorpRuntimeServices.filter(
        (value): value is Record<string, unknown> => typeof value === "object" && value !== null,
      )
    : [];
  const runtimePrimaryUrl = asString(context.aiteamcorpRuntimePrimaryUrl, "");
  ...
  if (wakeReason) {
    env.AITEAMCORP_WAKE_REASON = wakeReason;
  }
  if (wakeCommentId) {
    env.AITEAMCORP_WAKE_COMMENT_ID = wakeCommentId;
  }
  if (approvalId) {
    env.AITEAMCORP_APPROVAL_ID = approvalId;
  }
  if (approvalStatus) {
    env.AITEAMCORP_APPROVAL_STATUS = approvalStatus;
  }
  if (linkedIssueIds.length > 0) {
    env.AITEAMCORP_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  }
  if (wakePayloadJson) {
    env.AITEAMCORP_WAKE_PAYLOAD_JSON = wakePayloadJson;
  }
  if (effectiveWorkspaceCwd) {
    env.AITEAMCORP_WORKSPACE_CWD = effectiveWorkspaceCwd;
  }
  if (workspaceSource) {
    env.AITEAMCORP_WORKSPACE_SOURCE = workspaceSource;
  }
  if (workspaceStrategy) {
    env.AITEAMCORP_WORKSPACE_STRATEGY = workspaceStrategy;
  }
  if (workspaceId) {
    env.AITEAMCORP_WORKSPACE_ID = workspaceId;
  }
  if (workspaceRepoUrl) {
    env.AITEAMCORP_WORKSPACE_REPO_URL = workspaceRepoUrl;
  }
  if (workspaceRepoRef) {
    env.AITEAMCORP_WORKSPACE_REPO_REF = workspaceRepoRef;
  }
  if (workspaceBranch) {
    env.AITEAMCORP_WORKSPACE_BRANCH = workspaceBranch;
  }
  if (workspaceWorktreePath) {
    env.AITEAMCORP_WORKSPACE_WORKTREE_PATH = workspaceWorktreePath;
  }
  if (agentHome) {
    env.AGENT_HOME = agentHome;
  }
  if (workspaceHints.length > 0) {
    env.AITEAMCORP_WORKSPACES_JSON = JSON.stringify(workspaceHints);
  }
  if (runtimeServiceIntents.length > 0) {
    env.AITEAMCORP_RUNTIME_SERVICE_INTENTS_JSON = JSON.stringify(runtimeServiceIntents);
  }
  if (runtimeServices.length > 0) {
    env.AITEAMCORP_RUNTIME_SERVICES_JSON = JSON.stringify(runtimeServices);
  }
  if (runtimePrimaryUrl) {
    env.AITEAMCORP_RUNTIME_PRIMARY_URL = runtimePrimaryUrl;
  }
  ...
  if (!hasExplicitApiKey && authToken) {
    env.AITEAMCORP_API_KEY = authToken;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const claudeSkillEntries = await readAiTeamCorpRuntimeSkillEntries(config, __moduleDir);
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[aiteamcorp] Claude session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  if (runtimeSessionId && runtimePromptBundleKey.length > 0 && runtimePromptBundleKey !== promptBundle.bundleKey) {
    await onLog(
      "stdout",
      `[aiteamcorp] Claude session "${runtimeSessionId}" was saved for prompt bundle "${runtimePromptBundleKey}" and will not be resumed with "${promptBundle.bundleKey}".\n`,
    );
  }
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[aiteamcorp] Claude resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );

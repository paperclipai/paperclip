...
## Scope
This plan is now **gateway-only**. Valadrien OS supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Valadrien OS skill.
...
```json
{
  "valadrienOsApiUrl": "http://host.docker.internal:3100",
...
3. In `/new` OpenClaw session, OpenClaw can still create a Valadrien OS task.
...
```bash
pnpm --filter @valadrien-os/server test:run -- openclaw-gateway
pnpm --filter @valadrien-os/server typecheck
pnpm --filter @valadrien-os/ui typecheck
pnpm --filter valadrien-os typecheck
```

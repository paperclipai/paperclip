...
Use when:
- You want Ironworks to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Ironworks server.
...
- workspaceRuntime (object, optional): desired runtime service intents; Ironworks forwards these in a standardized ironworks.workspaceRuntime block for remote execution environments
- timeoutSec (number, optional): adapter timeout in seconds (default 120)
- waitTimeoutMs (number, optional): agent.wait timeout override (default timeoutSec * 1000)
- autoPairOnFirstConnect (boolean, optional): on first "pairing required", attempt device.pair.list/device.pair.approve via shared auth, then retry once (default true)
- ironworksApiUrl (string, optional): absolute Ironworks base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default ironworks)

Standard outbound payload additions:
- ironworks (object): standardized Ironworks context added to every gateway agent request
- ironworks.workspace (object, optional): resolved execution workspace for this run
- ironworks.workspaces (array, optional): additional workspace hints Ironworks exposed to the run
- ironworks.workspaceRuntime (object, optional): normalized runtime service intent config for the workspace

...
Use when:
- You want Odysseus to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Odysseus server.
...
- odysseusApiUrl (string, optional): absolute Odysseus base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/odysseus-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default odysseus)

Standard outbound payload additions:
- odysseus (object): standardized Odysseus context added to every gateway agent request
- odysseus.workspace (object, optional): resolved execution workspace for this run
- odysseus.workspaces (array, optional): additional workspace hints Odysseus exposed to the run
- odysseus.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution

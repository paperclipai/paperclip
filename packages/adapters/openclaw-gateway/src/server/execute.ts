...
function buildWakeText(
  ...
  const lines = [
    ...
    "   - Leave durable progress with a clear next action. Use direct child issues only from main parent issues for bounded execution lanes; if this issue already has parentId, coordinate inside this issue and never create grandchildren.",
    "   - Create direct child execution lanes only when you know what needs to be done and the current issue is a main parent; use POST /api/issues/{issueId}/interactions with kind suggest_tasks, ask_user_questions, or request_confirmation when the board/user must choose, answer, or confirm before you can continue.",
    "   - For plan approval, update the plan document first, then create request_confirmation targeting the latest plan revision with idempotencyKey confirmation:{issueId}:plan:{revisionId}; wait for acceptance before creating direct child execution lanes.",

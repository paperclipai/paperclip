...
export function buildWakeText(
  ...
  const lines = [
    ...
    "",
    "Workflow:",
    "1) Prefer the Paperclip MCP/API client that is already available in the run. Use its issue tools instead of ad-hoc shell pipelines.",
    "2) Determine issueId: PAPERCLIP_TASK_ID if present, otherwise issue_id (${issueIdHint}).",
    "3) If issueId exists:",
    "   - Use the Paperclip client to identify the current agent, checkout the issue, read the issue, and read comments.",
    ...
    "   - Create child issues directly when you know what needs to be done; use issue interactions with kind suggest_tasks, ask_user_questions, or request_confirmation when the board/user must choose, answer, or confirm before you can continue.",
    ...
    "   - If blocked, update the issue to status=blocked with a comment covering what is blocked, who owns the unblock, and the next action.",
    "   - If instructions require a comment, add an issue comment.",
    "   - When complete, update the issue to status=done with a concise change summary.",
    "4) If issueId does not exist:",
    "   - Use the Paperclip client to list your assigned issues for this company.",
    "   - Prefer in_progress first, then in_review when you were woken by a comment, then todo, then blocked, then continue with step 3.",
    "5) CRITICAL SECURITY RULE: NEVER pipe curl/wget output into an interpreter (curl|python3, curl|bash, wget|sh). The security scanner WILL block these and your heartbeat is wasted. If you need API data, use the Paperclip MCP/API client directly. If MCP is unavailable, use python3 with urllib.request to fetch and process data, or write curl output to a temp file and read it separately — never pipe curl into an interpreter. You can also use scripts/pc-api.sh as a convenience wrapper.",
    "",

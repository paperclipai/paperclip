...
function buildWakeText(
  ...
  const lines = [
    ...
    "- Never pipe downloaded/streamed content directly into interpreters/shells (no curl|python, curl|bash, wget|sh, etc.).",
    "- If you need HTTP response processing, download to a local file first, inspect briefly, then parse from the local file.",
    "- Prefer jq for JSON filtering when available.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",

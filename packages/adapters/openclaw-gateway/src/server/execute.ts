...
const DEFAULT_CLIENT_VERSION = "valadrien-os";
...
export function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "valadrien-os";
  if (input.strategy === "run") {
    return prefixSessionKeyForAgent(`valadrien-os:run:${input.runId}`, input.agentId);
  }
  if (input.strategy === "issue" && input.issueId) {
    return prefixSessionKeyForAgent(`valadrien-os:issue:${input.issueId}`, input.agentId);
  }
...
function resolveValadrienOsApiUrlOverride(value: unknown): string | null {
...
const DEFAULT_CLAIMED_API_KEY_PATH = "~/.openclaw/workspace/valadrien-os-claimed-api-key.json";

function resolveClaimedApiKeyPath(value: unknown): string {
  return nonEmpty(value) ?? DEFAULT_CLAIMED_API_KEY_PATH;
}

function buildValadrienOsEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const valadrienOsApiUrlOverride = resolveValadrienOsApiUrlOverride(ctx.config.valadrienOsApiUrl);
  const valadrienOsEnv: Record<string, string> = {
    ...buildValadrienOsEnv(ctx.agent),
    VALADRIEN_OS_RUN_ID: ctx.runId,
  };

  if (valadrienOsApiUrlOverride) {
    valadrienOsEnv.VALADRIEN_OS_API_URL = valadrienOsApiUrlOverride;
  }
  if (wakePayload.taskId) valadrienOsEnv.VALADRIEN_OS_TASK_ID = wakePayload.taskId;
  const issueWorkMode = readValadrienOsIssueWorkModeFromContext(ctx.context);
  if (issueWorkMode) valadrienOsEnv.VALADRIEN_OS_ISSUE_WORK_MODE = issueWorkMode;
  if (wakePayload.wakeReason) valadrienOsEnv.VALADRIEN_OS_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) valadrienOsEnv.VALADRIEN_OS_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) valadrienOsEnv.VALADRIEN_OS_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) valadrienOsEnv.VALADRIEN_OS_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    valadrienOsEnv.VALADRIEN_OS_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return valadrienOsEnv;
}

function buildWakeText(
  payload: WakePayload,
  valadrienOsEnv: Record<string, string>,
  structuredWakePrompt: string,
): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/valadrien-os-claimed-api-key.json";
  const orderedKeys = [
    "VALADRIEN_OS_RUN_ID",
    "VALADRIEN_OS_AGENT_ID",
    "VALADRIEN_OS_COMPANY_ID",
    "VALADRIEN_OS_API_URL",
    "VALADRIEN_OS_TASK_ID",
    "VALADRIEN_OS_WAKE_REASON",
    "VALADRIEN_OS_WAKE_COMMENT_ID",
    "VALADRIEN_OS_APPROVAL_ID",
    "VALADRIEN_OS_APPROVAL_STATUS",
    "VALADRIEN_OS_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = valadrienOsEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = valadrienOsEnv.VALADRIEN_OS_API_URL ?? "<set VALADRIEN_OS_API_URL>";

  const lines = [
    "ValadrienOs wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `VALADRIEN_OS_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load VALADRIEN_OS_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $VALADRIEN_OS_API_KEY on every API call.",
    "- Use X-ValadrienOs-Run-Id: $VALADRIEN_OS_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: VALADRIEN_OS_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$VALADRIEN_OS_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$VALADRIEN_OS_COMPANY_ID/issues?assigneeAgentId=$VALADRIEN_OS_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
function buildStandardValadrienOsPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  valadrienOsEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateValadrienOs = parseObject(payloadTemplate.valadrienOs);
  const workspace = asRecord(ctx.context.valadrienOsWorkspace);
  const workspaces = Array.isArray(ctx.context.valadrienOsWorkspaces)
    ? ctx.context.valadrienOsWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.valadrienOsRuntimeServiceIntents)
    ? ctx.context.valadrienOsRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardValadrienOs: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: valadrienOsEnv.VALADRIEN_OS_API_URL ?? null,
  };
  const structuredWake = parseObject(ctx.context.valadrienOsWake);
  if (Object.keys(structuredWake).length > 0) {
    standardValadrienOs.wake = structuredWake;
  }

  if (workspace) {
    standardValadrienOs.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardValadrienOs.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardValadrienOs.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "valadrien-os-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const valadrienOsEnv = buildValadrienOsEnvForWake(ctx, wakePayload);
  const structuredWakePrompt = renderValadrienOsWakePrompt(ctx.context.valadrienOsWake);
  const structuredWakeJson = stringifyValadrienOsWakePayload(ctx.context.valadrienOsWake);
  ...
  const valadrienOsPayload = buildStandardValadrienOsPayload(ctx, wakePayload, valadrienOsEnv, payloadTemplate);
  ...
  agentParams.valadrienOs = valadrienOsPayload;

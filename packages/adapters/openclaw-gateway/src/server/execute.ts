...
const DEFAULT_CLIENT_VERSION = "odysseus";
...
export function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "odysseus";
  if (input.strategy === "run") {
    return prefixSessionKeyForAgent(`odysseus:run:${input.runId}`, input.agentId);
  }
  if (input.strategy === "issue" && input.issueId) {
    return prefixSessionKeyForAgent(`odysseus:issue:${input.issueId}`, input.agentId);
  }
...
function resolveOdysseusApiUrlOverride(value: unknown): string | null {
...
const DEFAULT_CLAIMED_API_KEY_PATH = "~/.openclaw/workspace/odysseus-claimed-api-key.json";

function resolveClaimedApiKeyPath(value: unknown): string {
  return nonEmpty(value) ?? DEFAULT_CLAIMED_API_KEY_PATH;
}

function buildOdysseusEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const odysseusApiUrlOverride = resolveOdysseusApiUrlOverride(ctx.config.odysseusApiUrl);
  const odysseusEnv: Record<string, string> = {
    ...buildOdysseusEnv(ctx.agent),
    ODYSSEUS_RUN_ID: ctx.runId,
  };

  if (odysseusApiUrlOverride) {
    odysseusEnv.ODYSSEUS_API_URL = odysseusApiUrlOverride;
  }
  if (wakePayload.taskId) odysseusEnv.ODYSSEUS_TASK_ID = wakePayload.taskId;
  const issueWorkMode = readOdysseusIssueWorkModeFromContext(ctx.context);
  if (issueWorkMode) odysseusEnv.ODYSSEUS_ISSUE_WORK_MODE = issueWorkMode;
  if (wakePayload.wakeReason) odysseusEnv.ODYSSEUS_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) odysseusEnv.ODYSSEUS_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) odysseusEnv.ODYSSEUS_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) odysseusEnv.ODYSSEUS_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    odysseusEnv.ODYSSEUS_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(
  payload: WakePayload,
  odysseusEnv: Record<string, string>,
  structuredWakePrompt: string,
): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/odysseus-claimed-api-key.json";
  const orderedKeys = [
    "ODYSSEUS_RUN_ID",
    "ODYSSEUS_AGENT_ID",
    "ODYSSEUS_COMPANY_ID",
    "ODYSSEUS_API_URL",
    "ODYSSEUS_TASK_ID",
    "ODYSSEUS_WAKE_REASON",
    "ODYSSEUS_WAKE_COMMENT_ID",
    "ODYSSEUS_APPROVAL_ID",
    "ODYSSEUS_APPROVAL_STATUS",
    "ODYSSEUS_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = odysseusEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = odysseusEnv.ODYSSEUS_API_URL ?? "<set ODYSSEUS_API_URL>";

  const lines = [
    "Odysseus wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `ODYSSEUS_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load ODYSSEUS_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $ODYSSEUS_API_KEY on every API call.",
    "- Use X-Odysseus-Run-Id: $ODYSSEUS_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: ODYSSEUS_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$ODYSSEUS_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$ODYSSEUS_COMPANY_ID/issues?assigneeAgentId=$ODYSSEUS_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
function buildStandardOdysseusPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  odysseusEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateOdysseus = parseObject(payloadTemplate.odysseus);
  const workspace = asRecord(ctx.context.odysseusWorkspace);
  const workspaces = Array.isArray(ctx.context.odysseusWorkspaces)
    ? ctx.context.odysseusWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.odysseusRuntimeServiceIntents)
    ? ctx.context.odysseusRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardOdysseus: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: odysseusEnv.ODYSSEUS_API_URL ?? null,
  };
  const structuredWake = parseObject(ctx.context.odysseusWake);
  if (Object.keys(structuredWake).length > 0) {
    standardOdysseus.wake = structuredWake;
  }

  if (workspace) {
    standardOdysseus.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardOdysseus.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardOdysseus.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "odysseus-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const odysseusEnv = buildOdysseusEnvForWake(ctx, wakePayload);
  const structuredWakePrompt = renderOdysseusWakePrompt(ctx.context.odysseusWake);
  const structuredWakeJson = stringifyOdysseusWakePayload(ctx.context.odysseusWake);
  ...
  const odysseusPayload = buildStandardOdysseusPayload(ctx, wakePayload, odysseusEnv, payloadTemplate);
  ...
  agentParams.odysseus = odysseusPayload;

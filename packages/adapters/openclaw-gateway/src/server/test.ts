...
import { asString, parseObject } from "@aiteamcorp/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the AiTeamCorp server host.",
        });
      }
    } catch (err) {
      checks.push({

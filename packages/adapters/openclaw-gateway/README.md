...
This document describes how `@valadrien-os/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into ValAdrien OS logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (ValAdrien OS `runId`)

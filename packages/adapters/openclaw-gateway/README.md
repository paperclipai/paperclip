...
This document describes how `@ironworksai/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into Ironworks logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (Ironworks `runId`)

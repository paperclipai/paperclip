...
async function injectPluginToolMcpSettings(input: {
  ...
  const mcpPath = path.join(input.cwd, ".cursor", "mcp.json");
  ...
  await fs.mkdir(path.join(input.cwd, ".cursor"), { recursive: true });
  await fs.writeFile(mcpPath, `${JSON.stringify(next, null, 2)}\n`, { encoding: "utf-8", mode: 0o600 });
  await input.onLog("stdout", `[paperclip] Plugin tool MCP bridge: injected ${Object.keys(bridgeEnv).length > 0 ? "with credentials" : "without credentials"} into ${mcpPath}.\n`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (!executionTargetIsRemote) {
    await injectPluginToolMcpSettings({ cwd: effectiveExecutionCwd, env: effectiveEnv, onLog });
  }

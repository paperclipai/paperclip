...
Use when:
- You want Ironworks to run Cursor Agent CLI locally as the agent runtime
...
- Ironworks auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$ironworks" and related skills on local runs.
- Ironworks auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;

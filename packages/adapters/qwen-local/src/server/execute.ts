...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // ---- Build CLI args ----
  // Qwen headless mode: positional argument = prompt, --approval-mode yolo, --max-session-turns
  const args: string[] = [
    "-o", outputFormat,
  ...
  if (instructionsFilePath && !sessionId) {
    args.push("--system-prompt", `Instructions file: ${instructionsFilePath}`);
  }
  ...
  // Positional prompt goes last (or via stdin)
  // Qwen supports reading from stdin when no positional argument is given,
  // but for reliability we pass it as the last positional argument.
  args.push("--");

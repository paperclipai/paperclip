...
function resolveQwenBillingType(env: Record<string, string>): "api" | "subscription" {
  return hasNonEmptyEnvValue(env, "DASHSCOPE_API_KEY") ? "api" : "subscription";
}

async function buildQwenRuntimeConfig(input: QwenExecutionInput): Promise<QwenRuntimeConfig> {
  ...
  const loggedEnv = buildInvocationEnvForLogs(env, {
    runtimeEnv,
    includeRuntimeKeys: ["HOME", "QWEN_CONFIG_DIR"],
    resolvedCommand,
  });
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const runtimeConfig = await buildQwenRuntimeConfig({
    runId,
    agent,
    config,
    context,
  ...
  const buildQwenArgs = (resumeSessionId: string | null) => {
  ...
  const parseFallbackErrorMessage = (proc: RunProcessResult) => {
    ...
    if ((proc.exitCode ?? 0) === 0) {
      return "Failed to parse Qwen JSON output";
    }

    return stderrLine
      ? `Qwen Code exited with code ${proc.exitCode ?? -1}: ${stderrLine}`
      : `Qwen Code exited with code ${proc.exitCode ?? -1}`;
  };

  const runAttempt = async (resumeSessionId: string | null) => {
    const args = buildQwenArgs(resumeSessionId);
    if (onMeta) {
      await onMeta({
    ...
    const parsedStream = parseQwenStreamJson(proc.stdout);
  ...
  const toAdapterResult = (
    attempt: {
      ...
      parsedStream: ReturnType<typeof parseQwenStreamJson>;
    ...
    if (!parsed) {
      return {
        ...
        errorCode: loginMeta.requiresLogin ? "qwen_auth_required" : null,
        errorMeta,
        resultJson: {
          stdout: proc.stdout,
          stderr: proc.stderr,
    ...
    const usage =
      ...
      (() => {
        ...
        return {
          ...
          cachedInputTokens: asNumber(usageObj.cache_read_input_tokens, 0) || asNumber(usageObj.cached_tokens, 0),
    ...
    const resolvedSessionId =
      ...
      (asString(parsed.session_id, opts.fallbackSessionId ?? "") ||
        asString(parsed.sessionId, opts.fallbackSessionId ?? "") ||
    ...
    const resolvedSessionParams = resolvedSessionId
      ...
      : null;
    const clearSessionForMaxTurns = isQwenMaxTurnsResult(parsed);
    ...
    return {
      ...
      errorMessage:
        (proc.exitCode ?? 0) === 0
          ? null
          : describeQwenFailure(parsed) ?? `Qwen Code exited with code ${proc.exitCode ?? -1}`,
      errorCode: loginMeta.requiresLogin ? "qwen_auth_required" : null,
      errorMeta,
      usage,
      sessionId: resolvedSessionId,
      ...
      resultJson: parsed,
      summary: parsedStream.summary || asString(parsed.response, ""),
  ...
  if (
    ...
    initial.parsed &&
    isQwenUnknownSessionError(initial.parsed)
  ) {
    await onLog(
      "stdout",

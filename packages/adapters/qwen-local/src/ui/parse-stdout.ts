...
/**
 * Parse Qwen Code stream-json output for UI transcript rendering.
 *
 * Mirrors the structure of Claude's parseStdoutLine but adapted for
 * Qwen Code's JSON event format.
 */
export function parseQwenStdoutLine(line: string, ts: string): TranscriptEntry[] {
  ...
  if (type === "result") {
    ...
    const cachedTokens = asNumber(usage.cache_read_input_tokens) || asNumber(usage.cached_tokens);
    const costUsd = asNumber(parsed.total_cost_usd) || asNumber(parsed.cost_usd);

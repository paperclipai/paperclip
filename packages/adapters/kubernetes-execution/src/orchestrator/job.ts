...
export interface BuildJobInput {
  ...
  /** Key in the env Secret that contains AdapterRuntimeCommandSpec JSON for the shim. */
  runtimeCommandSecretKey: string;
...
export function buildAgentJob(input: BuildJobInput): V1Job {
  ...
  const mainContainer: V1Container = {
    ...
    volumeMounts: [
      ...
      {
        name: "env",
        mountPath: "/run/paperclip/runtime-command.json",
        subPath: input.runtimeCommandSecretKey,
        readOnly: true,
      },
    ],

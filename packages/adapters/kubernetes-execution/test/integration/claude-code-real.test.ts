...
const BASE_TAG = BASE_IMAGE.includes(":") ? BASE_IMAGE.split(":").pop()! : "latest";
...
describe.skipIf(!process.env["K8S_INTEGRATION"] || !process.env["ANTHROPIC_API_KEY"])(
  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      execSync(
        `docker build -t ${BASE_IMAGE} -f docker/agent-runtime/Dockerfile.base .`,
        { cwd: repoRoot, stdio: "inherit" },
      );
      // eslint-disable-next-line no-console
      console.log("[claude-code-real] building agent-runtime-claude...");
      execSync(
        `docker build -t ${REAL_CLAUDE_IMAGE} --build-arg BASE_TAG=${BASE_TAG} -f docker/agent-runtime/Dockerfile.claude .`,
        { cwd: repoRoot, stdio: "inherit" },
      );
      // eslint-disable-next-line no-console

...
describe("ensureTenantNamespace", () => {
  ...
  it("folds tenant Cilium DSL into the baseline CNP when non-empty", async () => {
    const client = makeFakeClient();
    ...
    // log to ciliumnetworkpolicies paths and verify the tenant allowlist is
    // folded into the single baseline CNP.
    ...
    const putCall = requestCalls.find((c) =>
      c[0] === "PUT" &&
      typeof c[1] === "string" &&
      c[1].includes("paperclip-agent-egress-l7")
    );
    const body = putCall?.[2] as { spec?: { egress?: Array<{ toFQDNs?: Array<{ matchName?: string }> }> } };

...
const HERMES_DEFAULT_PROMPT_TEMPLATE = [
  ...
  "- Authenticate every API request, but never as a command-line argument: process arguments are world-readable (/proc/<pid>/cmdline), so a bearer token in an argument is exposed to every process on this host while the request runs. Feed the header to curl on stdin with `-H @-`, or from a process substitution (`-H @<(printf 'Authorization: Bearer %s' \"$PAPERCLIP_API_KEY\")`) when stdin already carries the body.",
  "- Include `-H \"X-Paperclip-Run-Id: $PAPERCLIP_RUN_ID\"` on mutating issue requests (run ids are not secrets and may stay on the command line).",

...
/**
 * Resolve the optional profile-to-preset binding without consulting any
 * global/default MOA state. A malformed or absent binding is deliberately a
 * no-op, preserving the adapter's configured single-model route.
 */
...
/**
 * Selection point for Hermes' local model argv.
 *
 * `moa:<preset>` and the virtual `moa` provider are Hermes runtime concepts;
 * this function only emits them for the exact configured profile binding.
 */

...
describe("hermes adapter argv security (TRA-257)", () => {
  ...
  it("task id sentinel does not appear in argv", async () => {
    ...
    // The task/issue ID must not be visible in argv (it may appear in env but
    // that's not visible in ps aux in the same way).
  ...
  it("company-A payload does not appear in company-B run's argv", async () => {
    ...
    // Company B's payload may appear in stdin (it's their own session) but
  ...
  it("all argv elements after the query sentinel are non-prompt flags only", async () => {
    ...
    // not a paragraph of text.  We test this by checking the query element
    // specifically.
    const qIdx = args.indexOf("-q");
    expect(qIdx).toBeGreaterThanOrEqual(0);
    expect(args[qIdx + 1]).toBe("-"); // sentinel, never the prompt body

    // No arg should be longer than 256 characters (flags and model names are
    // short; the prompt is thousands of characters).

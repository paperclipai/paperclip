...
/**
 * Lightweight environment diagnostic. Runs frequently from the UI, so it
 * deliberately does NOT make a network call — we just verify that the
 * configured baseUrl is HTTPS and that an API key is present in the env.
 */
export async function testEnvironment(
  ...
  const baseUrl = String(ctx.config?.baseUrl ?? "https://openrouter.ai/api/v1");
  const apiKey =

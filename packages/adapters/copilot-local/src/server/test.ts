...
import { parseCopilotJsonl, isCopilotAuthError } from "./parse.js";
...
export async function testEnvironment(
  ...
  const tokenEnvVars = ["COPILOT_GITHUB_TOKEN", "GH_TOKEN", "GITHUB_TOKEN"] as const;
  const foundToken = tokenEnvVars.find(
    (key) => isNonEmpty(env[key]) || isNonEmpty(process.env[key]),
  );
  if (foundToken) {
    const source = isNonEmpty(env[foundToken]) ? "adapter config env" : "server environment";
    checks.push({
      code: "copilot_github_token_found",
      level: "info",
      message: `GitHub token detected via ${foundToken}.`,
      detail: `Detected in ${source}.`,
    });
  } else {
    checks.push({
      code: "copilot_no_token",
      level: "info",
      message: "No explicit GitHub token env var found; Copilot CLI will use its own OAuth session.",
      hint: "Run `copilot login` to authenticate, or set COPILOT_GITHUB_TOKEN.",
    });
  }

  // Model configuration info
  const modelConfig = asString(config.model, "").trim();
  if (modelConfig) {
    checks.push({
      code: "copilot_model_configured",
      level: "info",
      message: `Model configured: ${modelConfig}`,
    });
  }
  ...
  if (canRunProbe) {
    if (!commandLooksLike(command, "copilot")) {
      ...
      const args = ["-p", "Respond with hello", "--allow-all-tools", "--output-format", "json", "--no-auto-update"];
      if (modelConfig) args.push("--model", modelConfig);
      ...
      const isAuth = isCopilotAuthError(probe.stdout, probe.stderr);
      ...
      if (probe.timedOut) {
        ...
        checks.push({
          ...
          ...(detail ? { detail } : {}),
          hint: "Run `copilot login` to complete OAuth sign-in, or set COPILOT_GITHUB_TOKEN, then retry.",
        });
        ...
        checks.push({
          ...
          ...(hasHello
            ? {}
            : {
                hint: "Try the probe manually (`copilot -p \"Respond with hello\" --allow-all-tools --output-format json --no-auto-update`).",
              }),
        });
      } else {
        checks.push({
          ...
          ...(detail ? { detail } : {}),
          hint: "Run `copilot -p \"Respond with hello\" --allow-all-tools --output-format json --no-auto-update` manually in this directory to debug.",
        });
      }
    }
  }

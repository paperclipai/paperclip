...
import { resolveHermesHome } from "./home.js";
...
function resolveHermesHomeLocal(config: Record<string, unknown>, companyId?: string): string {
  const env =
    typeof config.env === "object" && config.env !== null && !Array.isArray(config.env)
      ? (config.env as Record<string, unknown>)
      : {};
  return resolveHermesHome(env, companyId);
}
...
async function buildHermesSkillSnapshot(config: Record<string, unknown>, companyId?: string): Promise<AdapterSkillSnapshot> {
  const home = resolveHermesHomeLocal(config, companyId);

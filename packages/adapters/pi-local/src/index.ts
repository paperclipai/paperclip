...
Use when:
- You want AiTeamCorp to run Pi (the AI coding agent) locally as the agent runtime
...
- AiTeamCorp requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/aiteamcorp-sessions/ and resumed with --session.

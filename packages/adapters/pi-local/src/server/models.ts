...
import type { AdapterModel } from "@aiteamcorp/adapter-utils";
import { asString, runChildProcess } from "@aiteamcorp/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.AITEAMCORP_PI_COMMAND === "string" &&
    process.env.AITEAMCORP_PI_COMMAND.trim().length > 0
      ? process.env.AITEAMCORP_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["AITEAMCORP_", "npm_", "NPM_"] as const;

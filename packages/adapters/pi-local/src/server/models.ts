...
import { asString, mergeAllowlistedHostEnvWith, runChildProcess } from "@paperclipai/adapter-utils/server-utils";
...
export async function discoverPiModels(input: {
  ...
  const runtimeEnv = normalizeEnv(mergeAllowlistedHostEnvWith(env));

...
export function readResumeBaseline(value: unknown): ResumeBaseline | null {
  ...
  const values = [r.totalSteps, r.totalPromptTokens, r.totalCompletionTokens, r.totalCachedTokens];
  if (!values.every((counter): counter is number => typeof counter === 'number' && Number.isSafeInteger(counter) && counter >= 0)) return null;
  const [totalSteps, totalPromptTokens, totalCompletionTokens, totalCachedTokens] = values as [number, number, number, number];
  if (totalCachedTokens > totalPromptTokens) return null;
  return { totalSteps, totalPromptTokens, totalCompletionTokens, totalCachedTokens };
}
...
export async function resolveRunUsageAndCost(options: {
  ...
  const cumulative = readResumeBaseline({
    totalSteps,
  ...
  if (options.resumeBaseline != null && (
    !resumeBaseline || !cumulative ||
    cumulative.totalSteps < resumeBaseline.totalSteps ||
    cumulative.totalPromptTokens < resumeBaseline.totalPromptTokens ||
    cumulative.totalCompletionTokens < resumeBaseline.totalCompletionTokens ||
    cumulative.totalCachedTokens < resumeBaseline.totalCachedTokens
  )) {
    const actualModel = resolveActualModel(requestedModel, rawMetrics);
    return {
      ...empty,
      sessionId,
      model: actualModel,
      resultJson: {
        ...
        devinResumeBaselineInvalid: true,
        ...(cumulative ? { devinCumulative: cumulative } : {}),
      },
    };
  }

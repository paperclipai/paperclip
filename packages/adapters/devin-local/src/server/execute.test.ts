...
import { chmodSync, mkdirSync, mkdtempSync, readdirSync, rmSync, writeFileSync, readFileSync, lstatSync, readlinkSync } from 'node:fs';
...
vi.mock('@paperclipai/adapter-utils/server-utils', async () => {
  ...
  readRuntimeSkillEntriesMock.mockImplementation(actual.readPaperclipRuntimeSkillEntries);
  return { ...actual, runChildProcess, readPaperclipRuntimeSkillEntries: readRuntimeSkillEntriesMock };
});
...
describe('buildDevinPrompt', () => {
  ...
  const assignmentWake = () => ({
    ...
    issue: {
      ...
      identifier: 'ISS-7',
      title: 'Do the thing',
      description: 'ISSUE-DESC-MARKER: detail only in the wake payload',
  ...
  it('includes the task-context markdown after the wake section in conversation mode and suppresses the duplicated issue description', () => {
    const { prompt, promptMetrics } = buildDevinPrompt({
      ...basePromptOpts(),
      wake: assignmentWake(),
      context: {
        conversationMode: true,
        paperclipTaskMarkdown: 'TASK-MD-MARKER: chat mode brief',
      },
    });
    const wakePos = prompt.indexOf('issue: ISS-7 Do the thing');
    const taskPos = prompt.indexOf('TASK-MD-MARKER');
    expect(wakePos).toBeGreaterThanOrEqual(0);
    expect(taskPos).toBeGreaterThan(wakePos);
    expect(prompt).not.toContain('ISSUE-DESC-MARKER');
    expect(promptMetrics.taskContextChars).toBeGreaterThan(0);
  });

  it('ignores the task-context markdown outside conversation mode and keeps the issue description', () => {
    const { prompt, promptMetrics } = buildDevinPrompt({
      ...basePromptOpts(),
      wake: assignmentWake(),
      context: {
    ...
    expect(prompt).not.toContain('TASK-MD-MARKER');
    expect(prompt).toContain('ISSUE-DESC-MARKER');
    expect(promptMetrics.taskContextChars).toBe(0);
  });
});
...
describe('execute', () => {
  ...
  it('still runs and cleans temp files when skill staging fails outright', async () => {
    ...
    expect(
      logs.some(
        (l) =>
          l.stream === 'stderr' &&
          l.chunk.includes('Failed to stage Devin skills: catalog exploded'),
      ),
    ).toBe(true);
  ...
  it('stages desired skills in <cwd>/.devin/skills during the run and removes them at exit', async () => {
    ...
    let duringLink: string | undefined;
    runChildProcess.mockImplementationOnce(async () => {
      const stat = lstatSync(target, { throwIfNoEntry: false });
      duringLink = stat?.isSymbolicLink() ? readlinkSync(target) : undefined;
      return {
        exitCode: 0,
        signal: null,
        timedOut: false,
    ...
    try {
      ...
      expect(duringLink).toBe(reviewSource);
      expect(lstatSync(target, { throwIfNoEntry: false })).toBeUndefined();
      expect(
        lstatSync(path.join(cwd, '.devin', 'skills'), { throwIfNoEntry: false }),
      ).toBeUndefined();
      expect(
        logs.some(
          (l) =>
            l.stream === 'stdout' &&
            l.chunk.includes('[paperclip] Staged 1 Devin skill(s) in') &&
  ...
  function leaseFileFor(cwd: string, runId: string): string {
    ...
    return path.join(
      tmpdir(),
      'paperclip-devin-skill-leases',
      hash,
      `${runId}.json`,
    );
  }

  it('records the spawned Devin child pid in the skill lease', async () => {
    const cwd = path.join(tmp, `devin-lease-pid-${Date.now()}`);
    const reviewSource = path.join(cwd, 'runtime-skills', 'review');
    mkdirSync(reviewSource, { recursive: true });
    writeFileSync(path.join(reviewSource, 'SKILL.md'), '---\nname: review\n---\n');
    const leaseFile = leaseFileFor(cwd, 'run-1');
    let seenChildPid: number | undefined;
    runChildProcess.mockImplementationOnce(
      async (_runId, _cmd, _args, opts) => {
        await opts.onSpawn?.({
          pid: 4242,
          processGroupId: null,
          startedAt: new Date().toISOString(),
        });
        seenChildPid = JSON.parse(readFileSync(leaseFile, 'utf8')).childPid;
        return {
          exitCode: 0,
          signal: null,
          timedOut: false,
    ...
    try {
      ...
      expect(seenChildPid).toBe(4242);
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  });

  it('updates the lease child pid when the unknown-session retry spawns again', async () => {
    const cwd = path.join(tmp, `devin-lease-pid-retry-${Date.now()}`);
    const reviewSource = path.join(cwd, 'runtime-skills', 'review');
    mkdirSync(reviewSource, { recursive: true });
    writeFileSync(path.join(reviewSource, 'SKILL.md'), '---\nname: review\n---\n');
    const leaseFile = leaseFileFor(cwd, 'run-1');
    const seenChildPids: number[] = [];
    const attempt = (pid: number, exitCode: number, stderr: string) =>
      async (_runId: string, _cmd: string, _args: string[], opts: {
        ...
        seenChildPids.push(
          JSON.parse(readFileSync(leaseFile, 'utf8')).childPid,
        );
        return {
          exitCode,
          signal: null,
    ...
    runChildProcess
      .mockImplementationOnce(attempt(1111, 1, "Error: No session found matching 'gone'"))
      .mockImplementationOnce(attempt(2222, 0, ''));
    try {
      await execute(
        baseCtx({
          runtime: { sessionParams: { sessionId: 'gone', cwd } },
          config: {
            cwd,
            paperclipRuntimeSkills: [
              { key: 'company/review', runtimeName: 'review', source: reviewSource },
      ...
      expect(seenChildPids).toEqual([1111, 2222]);
      expect(runChildProcess).toHaveBeenCalledTimes(2);
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  ...
  it('removes staged skill links even when the child exits non-zero', async () => {
    const cwd = path.join(tmp, `devin-run-skills-fail-${Date.now()}`);
    const reviewSource = path.join(cwd, 'runtime-skills', 'review');
    mkdirSync(reviewSource, { recursive: true });
    writeFileSync(path.join(reviewSource, 'SKILL.md'), '---\nname: review\n---\n');
    const target = path.join(cwd, '.devin', 'skills', 'review');
  ...
  it('mounts only the operational skill when nothing is configured (legacy local rule)', async () => {
    ...
    let duringEntries: string[] | undefined;
    runChildProcess.mockImplementationOnce(async () => {
      duringEntries = readdirSync(home);
      return {
        exitCode: 0,
        signal: null,
        timedOut: false,
    ...
    try {
      ...
      if (duringEntries) {
        ...
        // skill when available; nothing else may be linked, and the staged
        // links are removed when the run ends.
        expect(duringEntries).toEqual(['paperclip']);
        expect(logs.some((l) => l.chunk.includes('Staged 1 Devin skill(s)'))).toBe(true);
        expect(lstatSync(home, { throwIfNoEntry: false })).toBeUndefined();
      }
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  ...
  it('executes in the resolved project workspace instead of a different configured cwd', async () => {
    ...
    runChildProcess.mockImplementationOnce(async () => {
      ...
      duringLink = stat?.isSymbolicLink() ? readlinkSync(link) : undefined;
      return {
        exitCode: 0,
        signal: null,
        timedOut: false,
    ...
    try {
      ...
      // Staged in the workspace during the run, removed at exit.
      expect(duringLink).toBe(reviewSource);
      expect(
        lstatSync(path.join(projectDir, '.devin', 'skills'), {
  ...
  it('uses the conversation prompt template in conversation mode when none is configured', async () => {
    let metaPrompt = '';
    const ctx = {
      ...baseCtx({ context: { conversationMode: true } }),
      onMeta: async (meta: { prompt: string }) => {
        metaPrompt = meta.prompt;
      },
    };
    await execute(ctx as never);
    expect(metaPrompt).toContain('Continue your Paperclip conversation using the supplied chat mode directive');
    expect(metaPrompt).not.toContain('Execution contract:');
  });

  it('lets a configured promptTemplate win in conversation mode', async () => {
    let metaPrompt = '';
    const ctx = {
      ...baseCtx({
        config: { promptTemplate: 'CUSTOM-TEMPLATE-MARKER for {{agent.id}}' },
    ...
    await execute(ctx as never);
    expect(metaPrompt).toContain('CUSTOM-TEMPLATE-MARKER');
    expect(metaPrompt).not.toContain('Continue your Paperclip conversation');
  });

  it('prepends the initial communication guidance on a fresh run but not on a resume', async () => {
    let metaPrompt = '';
    const onMeta = async (meta: { prompt: string }) => {
      metaPrompt = meta.prompt;
    };
    await execute({
      ...baseCtx({ context: { paperclipTaskCommunicationGuidance: 'GUIDANCE-MARKER' } }),
      onMeta,
    } as never);
    expect(metaPrompt.startsWith('GUIDANCE-MARKER')).toBe(true);

    metaPrompt = '';
    await execute({
      ...baseCtx({
        context: { paperclipTaskCommunicationGuidance: 'GUIDANCE-MARKER' },
        runtime: { sessionParams: { sessionId: 'prev-session', cwd: tmp } },
      }),
      onMeta,
    } as never);
    expect(metaPrompt).not.toContain('GUIDANCE-MARKER');
  });

  it('restores the initial communication guidance in the prompt file for an unknown-session fresh retry', async () => {
    const promptFile = path.join(tmp, 'devin-prompt-run-1.txt');
    const promptsAtSpawn: string[] = [];
    runChildProcess.mockImplementation(async () => {
      promptsAtSpawn.push(readFileSync(promptFile, 'utf8'));
      const fresh = promptsAtSpawn.length > 1;
      return {
        exitCode: fresh ? 0 : 1,
        signal: null,
        timedOut: false,
        stdout: fresh ? 'ok' : '',
        stderr: fresh ? '' : "Error: No session found matching 'gone-session'",
        pid: 1,
        startedAt: new Date().toISOString(),
      };
    });
    ...
    expect(runChildProcess).toHaveBeenCalledTimes(2);
    expect(promptsAtSpawn[0]).not.toContain('GUIDANCE-MARKER');
    expect(promptsAtSpawn[1].startsWith('GUIDANCE-MARKER')).toBe(true);
  });

  it('rebuilds the prompt fresh-shaped for an unknown-session retry instead of reusing the resume prompt', async () => {
    const promptFile = path.join(tmp, 'devin-prompt-run-1.txt');
    const promptsAtSpawn: string[] = [];
    runChildProcess.mockImplementation(async () => {
      promptsAtSpawn.push(readFileSync(promptFile, 'utf8'));
    ...
    await execute(
      baseCtx({
        context: {
          conversationMode: true,
          paperclipTaskMarkdown: 'FULL-TASK-MD: the complete brief',
          paperclipTaskMarkdownCompact: 'COMPACT-TASK-MD: resume delta brief',
          paperclipTaskCommunicationGuidance: 'GUIDANCE-MARKER',
          paperclipWake: {
            reason: 'comment',
            issue: { id: 'issue-1', identifier: 'ISS-3', title: 'Chat task' },
            comments: [{ id: 'c1', body: 'a new comment arrived' }],
          },
        },
        runtime: { sessionParams: { sessionId: 'gone-session', cwd: tmp } },
      }) as never,
    ...
    expect(runChildProcess).toHaveBeenCalledTimes(2);
    // First attempt: compact resume shape (delta markdown, no guidance, no
    // rendered conversation template).
    expect(promptsAtSpawn[0]).toContain('COMPACT-TASK-MD');
    expect(promptsAtSpawn[0]).not.toContain('FULL-TASK-MD');
    expect(promptsAtSpawn[0]).not.toContain('GUIDANCE-MARKER');
    expect(promptsAtSpawn[0]).not.toContain('Continue your Paperclip conversation');
    // Fresh retry: full task markdown, guidance, and the rendered template.
    expect(promptsAtSpawn[1].startsWith('GUIDANCE-MARKER')).toBe(true);
    expect(promptsAtSpawn[1]).toContain('FULL-TASK-MD');
    expect(promptsAtSpawn[1]).toContain('Continue your Paperclip conversation using the supplied chat mode directive');
  });

  it('removes the failed resume attempt ATIF before the fresh retry spawns', async () => {
    const exportPath = path.join(tmp, 'export.atif');
    const atifFile = path.join(tmp, 'export-run-1.atif');
    let atifPresentAtRetry: boolean | undefined;
    runChildProcess
      .mockImplementationOnce(async () => {
        writeFileSync(
          atifFile,
          JSON.stringify({ session_id: 'gone-session' }),
        );
        return {
          exitCode: 1,
          signal: null,
      ...
      .mockImplementationOnce(async () => {
        atifPresentAtRetry = lstatSync(atifFile, {
          throwIfNoEntry: false,
        }) !== undefined;
        return {
          exitCode: 0,
    ...
    expect(runChildProcess).toHaveBeenCalledTimes(2);
    expect(atifPresentAtRetry).toBe(false);
  });

  it('removes the failed resume attempt ATIF even when the fresh retry is skipped', async () => {
    if (process.getuid?.() === 0) return;
    const exportPath = path.join(tmp, 'export.atif');
    const atifFile = path.join(tmp, 'export-run-1.atif');
    const promptFile = path.join(tmp, 'devin-prompt-run-1.txt');
    runChildProcess.mockImplementationOnce(async () => {
      writeFileSync(atifFile, JSON.stringify({ session_id: 'gone-session' }));
      chmodSync(promptFile, 0o400);
      return {
        exitCode: 1,
        signal: null,
        timedOut: false,
    ...
    try {
      ...
      if (lstatSync(promptFile, { throwIfNoEntry: false })) {
        chmodSync(promptFile, 0o600);
      }
    }
  });

  it('skips the fresh retry when the prompt file cannot be refreshed and still drops the dead session', async () => {
    if (process.getuid?.() === 0) return;
    const promptFile = path.join(tmp, 'devin-prompt-run-1.txt');
    runChildProcess.mockImplementationOnce(async () => {
      chmodSync(promptFile, 0o400);
    ...
    try {
      ...
      expect(
        logs.some(
          (l) =>
            l.stream === 'stderr' &&
            l.chunk.includes('skipping the fresh retry'),
        ),
      ).toBe(true);
    } finally {
      if (lstatSync(promptFile, { throwIfNoEntry: false })) {
  ...
  it('does not export PAPERCLIP_WAKE_PAYLOAD_JSON while the wake context still reaches the prompt', async () => {
    let metaPrompt = '';
    const ctx = {
      ...baseCtx({
        config: { env: { PAPERCLIP_WAKE_PAYLOAD_JSON: 'configured' } },
        context: {
          ...
          paperclipWake: {
            ...
            issue: {
              id: 'issue-1',
              identifier: 'ISS-9',
              title: 'WAKE-TITLE-MARKER',
              description: 'wake detail',
              descriptionTruncated: false,
              status: 'in_progress',
            },
            commentWindow: { requestedCount: 0, includedCount: 0, missingCount: 0 },
    ...
    expect(opts.env.PAPERCLIP_WAKE_REASON).toBe('issue_assigned');
    expect(metaPrompt).toContain('WAKE-TITLE-MARKER');
  });
});

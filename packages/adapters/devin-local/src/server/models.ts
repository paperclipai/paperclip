...
export function buildDiscoveredModels(
  ...
  if (!Array.isArray(families)) {
    throw new Error('malformed devin models catalog: families must be an array');
  }
  const details = new Map<string, DetailedModel>();
  const freeModelIds: string[] = [];
  const malformed = (what: string, at: string) =>
    new Error(`malformed devin models catalog: ${what} at ${at}`);

  for (const [fi, family] of families.entries()) {
    if (!family || typeof family !== 'object' || Array.isArray(family)) {
      throw malformed('family entry must be an object', `families[${fi}]`);
    }
    if (!Array.isArray(family.variants)) {
      throw malformed(
        'family variants must be an array',
        `families[${fi}] (${String(family.family_uid ?? 'unknown')})`,
      );
    }
    const fusionFamily =
      String(family.family_uid ?? '').trim().toLowerCase() === 'fusion';
    for (const [vi, variant] of family.variants.entries()) {
      const at = `families[${fi}].variants[${vi}]`;
      if (!variant || typeof variant !== 'object' || Array.isArray(variant)) {
        throw malformed('variant entry must be an object', at);
      }
      ...
      if (
        ...
        uid.trim().length === 0 ||
        /\p{Cc}/u.test(uid) ||
        fusionSelectionError(uid) !== null
      ) {
        throw malformed(
          'variant model_uid must be a nonempty string without control characters',
          at,
        );
      }
      const rawLabel = variant.label;
      const hasLabel = typeof rawLabel === 'string' && rawLabel.trim().length > 0;
      const isFusion = fusionFamily || isFusionModelId(uid);
      const cost = parseCostSummary(
        typeof variant.cost_tier === 'string' ? variant.cost_tier : undefined,
        typeof variant.cost_summary === 'string' ? variant.cost_summary : null,
      );
      const label = hasLabel
        ? isFusion
          ? rawLabel.trim()
          : formatModelLabel(variant)
        : uid;
      const model: DetailedModel = {
        id: uid,
        label,
        familyUid: String(family.family_uid ?? ''),
        familyLabel: String(family.family_label ?? ''),
        slug: String(family.slug ?? ''),
        costTier: typeof variant.cost_tier === 'string' ? variant.cost_tier : '',
        maxContextTokens:
          typeof variant.max_context_tokens === 'number'
            ? variant.max_context_tokens
            : 0,
        maxOutputTokens:
          typeof variant.max_output_tokens === 'number'
            ? variant.max_output_tokens
            : 0,
        cost,
        isNew: variant.is_new === true,
        isBeta: variant.is_beta === true,
        ...(isFusion
          ? {
              fusion: hasLabel
                ? parseFusionVariant(
                    uid,
                    rawLabel,
                    typeof variant.cost_summary === 'string'
                      ? variant.cost_summary
                      : null,
                  )
                : parseFusionVariant(uid, uid, null),
            }
          : {}),
      };
      const existing = details.get(model.id);
      if (!existing) {
      ...
      if (existing.fusion || model.fusion) {
        const sameMetadata =
          existing.label === model.label &&
          JSON.stringify(existing.fusion ?? null) ===
            JSON.stringify(model.fusion ?? null);
        if (sameMetadata) continue;
        const freeIdx = freeModelIds.indexOf(model.id);
        if (freeIdx >= 0) freeModelIds.splice(freeIdx, 1);
  ...
  const labelsByModelKey = new Map<string, Set<string>>();
  for (const d of details.values()) {
    if (!d.fusion?.components) continue;
    for (const c of [
      d.fusion.components.orchestrator,
      d.fusion.components.worker,
    ]) {
      const set = labelsByModelKey.get(c.modelKey) ?? new Set<string>();
      set.add(c.modelLabel);
      labelsByModelKey.set(c.modelKey, set);
    }
  }
  for (const d of details.values()) {
    if (!d.fusion?.components) continue;
    for (const c of [
      ...
      if ((labelsByModelKey.get(c.modelKey)?.size ?? 0) > 1) {
        c.modelLabel = c.modelKey;
      }
    }
  }

  const fusionLeaves: AdapterModel[] = [...details.values()]
    .filter((d) => d.fusion)
    .map((d) => ({ id: d.id, label: d.label, fusion: d.fusion }))
    .sort(
...
export async function resolveDevinModelUid(
  ...
  if (!model) return '';
  const error = fusionSelectionError(model);
...
export function resolveModelUidFrom(
  ...
  const error = fusionSelectionError(model);
  if (error) throw new Error(error);
  if (isFusionModelId(model) || discovered.details.get(model)?.fusion) {
    if (discovered.details.get(model)?.fusion) {
      return model;
    }
    throw new Error(
      `model "${model}" is not a discovered Fusion combination; refresh models or choose an offered combination`,
    );
  }
...
export function resolveDetectedBaseModelId(
  ...
  if (isFusionModelId(value) || discovered.details.get(value)?.fusion) {
    return value;
  }

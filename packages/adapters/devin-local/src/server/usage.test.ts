...
import { resolveRunUsageAndCost, computeCostUsd, readResumeBaseline } from './usage.js';

describe('readResumeBaseline', () => {
  const valid = { totalSteps: 2, totalPromptTokens: 100, totalCompletionTokens: 50, totalCachedTokens: 10 };

  it('preserves complete nonnegative integer counters, including zero', () => {
    expect(readResumeBaseline(valid)).toEqual(valid);
    expect(readResumeBaseline({ totalSteps: 0, totalPromptTokens: 0, totalCompletionTokens: 0, totalCachedTokens: 0 })).toEqual({ totalSteps: 0, totalPromptTokens: 0, totalCompletionTokens: 0, totalCachedTokens: 0 });
  });

  it.each(Object.keys(valid))('rejects an invalid %s instead of guessing a counter', (key) => {
    for (const value of [undefined, null, -1, 0.5, Number.NaN, Number.POSITIVE_INFINITY, '1', Number.MAX_SAFE_INTEGER + 1]) {
      expect(readResumeBaseline({ ...valid, [key]: value })).toBeNull();
    }
  });

  it('rejects cached tokens greater than the cache-inclusive prompt count', () => {
    expect(readResumeBaseline({ ...valid, totalCachedTokens: 101 })).toBeNull();
  });
});
...
describe('resolveRunUsageAndCost', () => {
  ...
  it.each([
    ...
    { totalSteps: -1, totalPromptTokens: 50, totalCompletionTokens: 20, totalCachedTokens: 5 },
    { totalSteps: 2, totalPromptTokens: Number.NaN, totalCompletionTokens: 20, totalCachedTokens: 5 },
    { totalSteps: 5, totalPromptTokens: 80, totalCompletionTokens: 30, totalCachedTokens: 5 },
    { totalSteps: 2, totalPromptTokens: 101, totalCompletionTokens: 30, totalCachedTokens: 5 },
    { totalSteps: 2, totalPromptTokens: 80, totalCompletionTokens: 51, totalCachedTokens: 5 },
    { totalSteps: 2, totalPromptTokens: 80, totalCompletionTokens: 30, totalCachedTokens: 11 },
  ])('does not rebill cumulative history when resume counters are unusable: %j', async (resumeBaseline) => {
    const atif = writeAtif({
      session_id: 'session-abc',
      final_metrics: {
        total_steps: 4,
    ...
    const result = await resolveRunUsageAndCost({
      atifPath: atif,
      requestedModel: 'swe-1-7',
      resumeBaseline: resumeBaseline as never,
    });
    expect(result.usage).toBeUndefined();
    expect(result.costUsd).toBeNull();
    expect(result.billingType).toBe('unknown');
    expect(result.resultJson.devinResumeBaselineInvalid).toBe(true);
    expect(result.resultJson.devinCumulative).toEqual({
      totalSteps: 4,
      totalPromptTokens: 100,
      totalCompletionTokens: 50,
    ...
    expect(getModelCost).not.toHaveBeenCalled();
  });

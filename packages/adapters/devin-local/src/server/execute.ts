...
export function buildDevinPrompt(opts: {
  ...
  const renderedBootstrapPrompt =
    bootstrapPromptTemplate.trim().length > 0
      ...
      : '';
  const conversationMode = context.conversationMode === true;
  const taskContextNote = conversationMode
 ...
 * Returns the resolved task/comment ids for reuse in prompt +
 * meta context.
 */
...
export async function execute(
  ...
  const preferConfigured =
    workspace.source === 'agent_home' && configuredCwd.length > 0;
  const cwd = (preferConfigured ? '' : workspaceCwd) || configuredCwd;
  if (!cwd)
    throw new Error(
      'Devin Local requires an absolute working directory or a resolved Paperclip workspace.',
    );
  ...
  const configuredTimeoutSec = asNumber(config.timeoutSec, 1800);
  const timeoutSec =
    configuredTimeoutSec === 0
      ? 0
      : clampNumber(configuredTimeoutSec, 1, 86_400);
  ...
  const legacyInstructionsPath = asString(config.instructionsFilePath, '');
  ...
  const instructionsEntry = instructionsEntryFile
    ...
    : legacyInstructionsPath
      ? path.resolve(cwd, legacyInstructionsPath)
      : '';
  ...
  const storedUsageBaseline = storedSession?.resumeBaseline;
  ...
  const sessionHandoffMarkdown = asString(
    context.paperclipSessionHandoffMarkdown,
    '',
  );
  const promptForAttempt = (sessionId: string | null) => {
    const attempt = buildDevinPrompt({
      instructionsPrefix,
      promptTemplate,
      bootstrapPromptTemplate,
      templateData,
    ...
    return {
      prompt: joinPromptSections([
        selectInitialCommunicationGuidance(ctxBag, {
          resumedSession: Boolean(sessionId),
        }),
        attempt.prompt,
      ]),
      promptMetrics: attempt.promptMetrics,
    };
  };
  const firstAttempt = promptForAttempt(resumeSessionId);
  const firstAttemptPrompt = firstAttempt.prompt;
  const promptMetrics = firstAttempt.promptMetrics;
  ...
  try {
    ...
    writeFileSync(promptFile, firstAttemptPrompt, {
      encoding: 'utf8',
      mode: 0o600,
    });
  } catch (err) {
  ...
  // Skill links live in <cwd>/.devin/skills only while this run needs them;
  // release runs first in the finally below so every exit path is covered.
  const skillLinks = await acquireDevinSkillLinks({
    config: { ...config, cwd },
    cwd,
    runId,
    onLog,
  ...
  try {
    ...
    const runAttempt = (sessionId: string | null, attemptTimeoutSec?: number) =>
      runChildProcess(runId, command, buildArgs(sessionId), {
        ...
        // Every spawn (first attempt and the unknown-session retry) points
        // the lease at the live Devin child so a server restart cannot mark
        // this run's links stale.
        onSpawn: async (meta) => {
          await skillLinks.recordChildPid(meta.pid);
    ...
    if (
      ...
      const remainingTimeoutSec = timeoutSec === 0 ? 0 : Math.max(1, timeoutSec - elapsedSec);
      const freshRetryPrompt = promptForAttempt(null).prompt;
      let freshPromptReady = true;
      if (freshRetryPrompt !== firstAttemptPrompt) {
        try {
          writeFileSync(promptFile, freshRetryPrompt, {
            encoding: 'utf8',
            mode: 0o600,
          });
        } catch (err) {
          ...
          await onLog(
            'stderr',
            `[adapter] could not refresh prompt file for fresh retry: ${reason}; skipping the fresh retry\n`,
          );
          freshPromptReady = false;
        }
      }
      if (freshPromptReady) result = await runAttempt(null, remainingTimeoutSec);
      clearSession = true;

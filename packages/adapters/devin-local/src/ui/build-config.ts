...
import { fusionSelectionError, isFusionModelId } from '../fusion.js';
...
export function buildDevinLocalConfig(
  ...
  const bareError = fusionSelectionError(v.model ?? '');
  if (bareError) throw new Error(bareError);

  const ac: Record<string, unknown> = {};
  if (v.cwd) ac.cwd = v.cwd;
  if (v.model) ac.model = v.model;

  if (!isFusionModelId(v.model ?? '')) {

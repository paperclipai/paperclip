...
describe('fusion selection', () => {
  it('rejects a bare fusion value in any case or padding', () => {
    for (const model of ['fusion', ' Fusion ', 'FUSION']) {
      expect(() => buildDevinLocalConfig(makeValues({ model }))).toThrow(
        'Choose an explicit Fusion combination; select an orchestrator and worker.',
      );
    }
  });

  it('writes the exact fusion UID and omits generic axes', () => {
    const config = buildDevinLocalConfig(
      makeValues(
        { model: 'fusion-alpha-high-sidekick-beta-low', thinkingEffort: 'high' },
        { contextSize: '1m', fastMode: true, priority: true },
      ),
    );
    expect(config.model).toBe('fusion-alpha-high-sidekick-beta-low');
    for (const key of ['thinkingEffort', 'contextSize', 'fastMode', 'priority']) {
      expect(config).not.toHaveProperty(key);
    }
  });

  it('keeps generic axes for single models', () => {
    const config = buildDevinLocalConfig(
      makeValues(
        { model: 'swe-1.7', thinkingEffort: 'high' },
        { contextSize: '1m', fastMode: true, priority: true },
      ),
    );

...
function renderOdysseusEnvNote(env: Record<string, string>): string {
  const odysseusKeys = Object.keys(env)
    .filter((key) => key.startsWith("ODYSSEUS_"))
    .sort();
  if (odysseusKeys.length === 0) return "";
  return [
    "Odysseus runtime note:",
    `The following ODYSSEUS_* environment variables are available in this run: ${odysseusKeys.join(", ")}`,
...
function renderApiAccessNote(env: Record<string, string>): string {
  if (!hasNonEmptyEnvValue(env, "ODYSSEUS_API_URL") || !hasNonEmptyEnvValue(env, "ODYSSEUS_API_KEY")) return "";
  return [
    "Odysseus API access note:",
    "Use run_shell_command with curl to make Odysseus API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer $ODYSSEUS_API_KEY\\" \\"$ODYSSEUS_API_URL/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $ODYSSEUS_API_KEY\\" -H 'Content-Type: application/json' -H \\"X-Odysseus-Run-Id: $ODYSSEUS_RUN_ID\\" -d '{...}' \\"$ODYSSEUS_API_URL/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}
...
/**
 * Inject Odysseus skills directly into `~/.gemini/skills/` via symlinks.
...
async function ensureGeminiSkillsInjected(
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[odysseus] Failed to prepare Gemini skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[odysseus] Removed maintainer-only Gemini skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[odysseus] ${result === "repaired" ? "Repaired" : "Linked"} Gemini skill: ${entry.key}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[odysseus] Failed to link Gemini skill "${entry.key}": ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function buildGeminiSkillsDir(
  ...
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-gemini-skills-"));
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const geminiSkillEntries = await readOdysseusRuntimeSkillEntries(config, __moduleDir);
  const desiredGeminiSkillNames = resolveOdysseusDesiredSkillNames(config, geminiSkillEntries);
  ...
  if (executionTargetIsRemote) {
    try {
      ...
      await onLog(
        "stdout",
        `[odysseus] Syncing workspace and Gemini runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
  ...
  if (executionTargetIsRemote && adapterExecutionTargetUsesOdysseusBridge(executionTarget)) {
  ...
  if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[odysseus] Gemini session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
    );
  } else if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[odysseus] Gemini session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  ...
  const odysseusEnvNote = renderOdysseusEnvNote(env);
  ...
  const promptMetrics = {
    ...
    runtimeNoteChars: odysseusEnvNote.length + apiAccessNote.length,
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[odysseus] Gemini resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );

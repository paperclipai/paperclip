...
export function isGeminiUnknownSessionError(stdout: string, stderr: string): boolean {
  ...
  return /unknown\s+session|invalid\s+session|session\s+.*\s+not\s+found|resume\s+.*\s+not\s+found|checkpoint\s+.*\s+not\s+found|cannot\s+resume|failed\s+to\s+resume|no\s+previous\s+sessions\s+found/i.test(
    haystack,
  );
}

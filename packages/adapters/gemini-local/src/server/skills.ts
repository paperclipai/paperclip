...
async function buildGeminiSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  return buildPersistentSkillSnapshot({
    ...
    externalDetail: "Installed outside Odysseus management.",
  });
}
...
export async function syncGeminiSkills(
  ...
  const availableEntries = await readOdysseusRuntimeSkillEntries(ctx.config, __moduleDir);
  ...
  for (const available of availableEntries) {
    ...
    await ensureOdysseusSkillSymlink(available.source, target);
  }

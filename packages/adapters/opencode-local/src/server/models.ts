...
function resolveOpenCodeCommand(input: unknown): string {
  const envOverride =
    typeof process.env.VALADRIEN_OS_OPENCODE_COMMAND === "string" &&
    process.env.VALADRIEN_OS_OPENCODE_COMMAND.trim().length > 0
      ? process.env.VALADRIEN_OS_OPENCODE_COMMAND.trim()
      : "opencode";

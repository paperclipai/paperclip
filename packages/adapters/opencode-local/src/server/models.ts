...
function resolveOpenCodeCommand(input: unknown): string {
  const envOverride =
    typeof process.env.AITEAMCORP_OPENCODE_COMMAND === "string" &&
    process.env.AITEAMCORP_OPENCODE_COMMAND.trim().length > 0
      ? process.env.AITEAMCORP_OPENCODE_COMMAND.trim()
      : "opencode";

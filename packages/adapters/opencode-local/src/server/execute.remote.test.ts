...
describe("opencode remote execution", () => {
  ...
  it("prepares the workspace, syncs OpenCode skills, and restores workspace changes for remote SSH execution", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "odysseus-opencode-remote-"));
    ...
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.odysseus-runtime/opencode/xdgConfig`,
    }));
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.odysseus-runtime/opencode/skills`,
      followSymlinks: true,
    }));
    ...
    expect(modelProbeCall?.[3].env.XDG_CONFIG_HOME).toBe(
      `${managedRemoteWorkspace}/.odysseus-runtime/opencode/xdgConfig`,
    );
    ...
    expect(call?.[3].env.XDG_CONFIG_HOME).toBe(`${managedRemoteWorkspace}/.odysseus-runtime/opencode/xdgConfig`);
  ...
  it("fails before the remote run when the configured model is unavailable on the SSH target", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "odysseus-opencode-remote-model-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
    ...
    expect(startAdapterExecutionTargetOdysseusBridge).not.toHaveBeenCalled();
  });

  it("resumes saved OpenCode sessions for remote SSH execution only when the identity matches", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "odysseus-opencode-remote-resume-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");

...
const expectedPaperclipApiUrl =
  process.env.PAPERCLIP_RUNTIME_API_URL ?? process.env.PAPERCLIP_API_URL ?? "http://localhost:3100";
  ...
  it.each(["done", "cancelled"] as const)(
    ...
    async (status) => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        `${expectedPaperclipApiUrl}/api/issues/issue-123`,

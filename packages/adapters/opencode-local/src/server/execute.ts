...
import { inferOpenAiCompatibleBiller, type AdapterExecutionContext, type AdapterExecutionResult } from "@odysseus/adapter-utils";
import {
...
import { removeMaintainerOnlySkillSymlinks } from "@odysseus/adapter-utils/server-utils";
...
async function ensureOpenCodeSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[odysseus] Removed maintainer-only OpenCode skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  for (const entry of selectedEntries) {
    ...
    try {
      const result = await ensureOdysseusSkillSymlink(entry.source, target);
      if (result === "skipped") continue;
      await onLog(
        "stderr",
        `[odysseus] ${result === "repaired" ? "Repaired" : "Injected"} OpenCode skill "${entry.key}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[odysseus] Failed to inject OpenCode skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function buildOpenCodeSkillsDir(config: Record<string, unknown>): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "odysseus-opencode-skills-"));
  const target = path.join(tmp, "skills");
  await fs.mkdir(target, { recursive: true });
  const availableEntries = await readOdysseusRuntimeSkillEntries(config, __moduleDir);
  const desiredNames = new Set(resolveOdysseusDesiredSkillNames(config, availableEntries));
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const openCodeSkillEntries = await readOdysseusRuntimeSkillEntries(config, __moduleDir);
  const desiredOpenCodeSkillNames = resolveOdysseusDesiredSkillNames(config, openCodeSkillEntries);
  ...
  try {
    ...
    if (executionTarget?.kind === "remote") {
      ...
      await onLog(
        "stdout",
        `[odysseus] Syncing workspace and OpenCode runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
    ...
    if (executionTargetIsRemote && adapterExecutionTargetUsesOdysseusBridge(runtimeExecutionTarget)) {
      odysseusBridge = await startAdapterExecutionTargetOdysseusBridge({
        ...
        adapterKey: "opencode",
        timeoutSec,
        hostApiToken: preparedRuntimeConfig.env.ODYSSEUS_API_KEY,
        onLog,
      });
      if (odysseusBridge) {
        Object.assign(preparedRuntimeConfig.env, odysseusBridge.env);
    ...
    if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[odysseus] OpenCode session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
      );
    } else if (runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[odysseus] OpenCode session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
      );
    }
    ...
    if (resolvedInstructionsFilePath) {
      try {
        ...
        await onLog(
          "stdout",
          `[odysseus] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
        );
      }
    }
    ...
    try {
      ...
      if (
        ...
        await onLog(
          "stdout",
          `[odysseus] OpenCode session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
        );

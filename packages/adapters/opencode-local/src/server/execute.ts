...
async function ensureOpenCodeSkillsInjected(
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      const result = await ensureIronworksSkillSymlink(entry.source, target);
      if (result === "skipped") continue;
      await onLog(
        "stderr",
        `[ironworks] ${result === "repaired" ? "Repaired" : "Injected"} OpenCode skill "${entry.key}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[ironworks] Failed to inject OpenCode skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const linkedIssueIds = Array.isArray(context.issueIds)
    ...
    : [];
  if (wakeTaskId) env.IRONWORKS_TASK_ID = wakeTaskId;
  if (wakeReason) env.IRONWORKS_WAKE_REASON = wakeReason;
  if (wakeCommentId) env.IRONWORKS_WAKE_COMMENT_ID = wakeCommentId;
  if (approvalId) env.IRONWORKS_APPROVAL_ID = approvalId;
  if (approvalStatus) env.IRONWORKS_APPROVAL_STATUS = approvalStatus;
  if (linkedIssueIds.length > 0) env.IRONWORKS_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  if (effectiveWorkspaceCwd) env.IRONWORKS_WORKSPACE_CWD = effectiveWorkspaceCwd;
  if (workspaceSource) env.IRONWORKS_WORKSPACE_SOURCE = workspaceSource;
  if (workspaceId) env.IRONWORKS_WORKSPACE_ID = workspaceId;
  if (workspaceRepoUrl) env.IRONWORKS_WORKSPACE_REPO_URL = workspaceRepoUrl;
  if (workspaceRepoRef) env.IRONWORKS_WORKSPACE_REPO_REF = workspaceRepoRef;
  if (agentHome) env.AGENT_HOME = agentHome;
  if (workspaceHints.length > 0) env.IRONWORKS_WORKSPACES_JSON = JSON.stringify(workspaceHints);
  ...
  try {
    ...
    if (runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[ironworks] OpenCode session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
      );
    }
    ...
    if (resolvedInstructionsFilePath) {
      try {
        ...
        await onLog(
          "stdout",
          `[ironworks] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
        );
      }
    }

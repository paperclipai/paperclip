...
export function printOllamaStreamEvent(raw: string, debug: boolean): void {
  ...
  if (type === "system" && parsed.subtype === "init") {
    ...
    console.log(pc.blue(`Ollama initialized (model: ${model}${sessionId ? `, session: ${sessionId}` : ""})`));
    return;
  }
  ...
  if (type === "result") {
    ...
    if (subtype.startsWith("error") || isError || errors.length > 0) {
      console.log(pc.red(`ollama_result: subtype=${subtype || "unknown"} is_error=${isError ? "true" : "false"}`));
      if (errors.length > 0) {
        console.log(pc.red(`ollama_errors: ${errors.join(" | ")}`));
      }
    }
    console.log(
      pc.blue(

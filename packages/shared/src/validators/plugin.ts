...
/**
 * Zod schema for {@link AiTeamCorpPluginManifestV1} — the complete runtime
 ...
 * | `minimumAiTeamCorpVersion`| string?    | legacy alias of `minimumHostVersion`         |
 ...
 *
 * @see PLUGIN_SPEC.md §10.1 — Manifest shape
 * @see {@link AiTeamCorpPluginManifestV1} — the inferred TypeScript type
 */
export const pluginManifestV1Schema = z.object({
  ...
  minimumAiTeamCorpVersion: z.string().regex(
    /^\d+\.\d+\.\d+(-[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?(\+[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?$/,
    "minimumAiTeamCorpVersion must follow semver (e.g. 1.0.0)",
  ).optional(),
  ...
  if (
    ...
    && manifest.minimumAiTeamCorpVersion
    && manifest.minimumHostVersion !== manifest.minimumAiTeamCorpVersion
  ) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "minimumHostVersion and minimumAiTeamCorpVersion must match when both are declared",

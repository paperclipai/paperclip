...
export const issueExecutionPolicySchema = z.object({
  ...
  externalWait: issueExternalWaitSchema.optional().nullable(),
...
export const resolveIssueRecoveryActionSchema = z.object({
  ...
  sourceIssueStatus: z.enum(["todo", "in_progress", "done", "in_review", "blocked", "cancelled"]),
  ...
  if (value.outcome === "false_positive" || value.outcome === "cancelled") {
    if (
      ...
      value.sourceIssueStatus !== "done" &&
      value.sourceIssueStatus !== "in_review" &&
      !(value.outcome === "cancelled" && value.sourceIssueStatus === "cancelled")
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "This recovery outcome requires sourceIssueStatus to be in_progress, done, or in_review; cancelled outcomes may also cancel the source",
        path: ["sourceIssueStatus"],
      });
    }
    return;
...
export const askUserQuestionsResultSchema = z.object({
  ...
  expirationReason: z.enum(["superseded_by_comment", "stale_issue_state", "superseded_by_newer_interaction"]).optional(),
...
export const requestConfirmationPayloadSchema = z.object({
  version: z.literal(1, {
    error: "payload.version must be literal 1 (request_confirmation minimal contract)",
  }),
  prompt: z
    .string({
      error: (issue) =>
        issue.input === undefined
          ? "payload.prompt is required (request_confirmation minimal contract)"
          : "payload.prompt must be a non-empty string (request_confirmation minimal contract)",
    })
    .trim()
...
export const requestItemVerdictsResultSchema = z.object({
  version: z.literal(1),
  outcome: z.enum(["resolved", "superseded_by_comment", "stale_target", "stale_issue_state", "cancelled", "withdrawn", "issue_closed", "addressee_deleted"]),
  reason: z.string().trim().max(4000).nullable().optional(),

...
describe("routine variable helpers", () => {
  ...
  it("deduplicates placeholder names across the routine title and description", () => {
    expect(extractRoutineVariableNames(["Triage {{repo}}", "Review {{repo}} for {{priority}} bugs"])).toEqual([
      "repo",
      "priority",
    ]);
  });
  ...
  it("preserves existing metadata when syncing variables from a template", () => {
    expect(
      syncRoutineVariablesWithTemplate(
        ["Triage {{repo}}", "Review {{repo}} and {{priority}}"],
  ...
  it("excludes built-in variables from syncRoutineVariablesWithTemplate", () => {
    const result = syncRoutineVariablesWithTemplate("Daily report for {{date}} — {{repo}}", []);
  ...
  it("interpolates built-in date variable alongside user variables", () => {
    ...
    expect(interpolateRoutineTemplate("Report for {{date}} on {{repo}}", allVars)).toBe(
      `Report for ${builtins.date} on paperclip`,
    );
  });
});

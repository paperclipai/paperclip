...
function makeInstance(prefix: string, instanceId: string) {
  const cwd = /* fork: macOS os.tmpdir() is a symlink alias; canonicalize so the registered base workspace is canonical */ fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), prefix)));
  cleanup.push(cwd);
...
function makeInstanceRoot(instanceId: string) {
  const home = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "paperclip-seed-home-")));
  cleanup.push(home);
...
function makePlainCheckout() {
  const cwd = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "paperclip-seed-checkout-")));
  cleanup.push(cwd);
  return cwd;
}

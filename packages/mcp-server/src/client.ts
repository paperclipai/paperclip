import type { AiTeamCorpMcpConfig } from "./config.js";

export class AiTeamCorpApiError extends Error {
...
export class AiTeamCorpApiClient {
  constructor(private readonly config: AiTeamCorpMcpConfig) {}
  ...
  resolveCompanyId(companyId?: string | null): string {
    ...
    if (!resolved) {
      throw new Error("companyId is required because AITEAMCORP_COMPANY_ID is not set");
    }
    return resolved;
  }
  ...
  resolveAgentId(agentId?: string | null): string {
    ...
    if (!resolved) {
      throw new Error("agentId is required because AITEAMCORP_AGENT_ID is not set");
    }
    return resolved;
  }

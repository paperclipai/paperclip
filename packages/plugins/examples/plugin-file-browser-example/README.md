...
Example AiTeamCorp plugin that demonstrates:
...
```bash
pnpm --filter @aiteamcorp/plugin-file-browser-example build
pnpm aiteamcorp plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm aiteamcorp plugin uninstall aiteamcorp-file-browser-example --force
```
...
- Optional: use `aiteamcorp-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.

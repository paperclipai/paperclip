...
Example Ironworks plugin that demonstrates:
...
```bash
pnpm --filter @ironworksai/plugin-file-browser-example build
pnpm ironworksai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm ironworksai plugin uninstall ironworks-file-browser-example --force
```
...
- Optional: use `ironworks-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.

# @odysseus/plugin-hello-world-example
...
```bash
pnpm --filter @odysseus/plugin-hello-world-example build
pnpm odysseus plugin install ./packages/plugins/examples/plugin-hello-world-example
```
...
- **Reinstall after pulling.** If you installed a plugin by local path before the server stored `package_path`, the plugin may show status **error** (worker not found). Uninstall and install again so the server persists the path and can activate the plugin:  
  `pnpm odysseus plugin uninstall odysseus.hello-world-example --force` then  
  `pnpm odysseus plugin install ./packages/plugins/examples/plugin-hello-world-example`.

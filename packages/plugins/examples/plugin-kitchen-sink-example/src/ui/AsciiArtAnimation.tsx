...
type ValadrienOsSprite = (typeof VALADRIEN_OS_SPRITES)[number];
...
function spriteSize(sprite: ValadrienOsSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = VALADRIEN_OS_SPRITES[(baseCol + baseRow) % VALADRIEN_OS_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = VALADRIEN_OS_SPRITES[Math.floor(Math.random() * VALADRIEN_OS_SPRITES.length)]!;

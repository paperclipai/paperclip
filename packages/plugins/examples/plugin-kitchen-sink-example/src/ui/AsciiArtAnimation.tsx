...
type OdysseusSprite = (typeof ODYSSEUS_SPRITES)[number];
...
function spriteSize(sprite: OdysseusSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = ODYSSEUS_SPRITES[(baseCol + baseRow) % ODYSSEUS_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = ODYSSEUS_SPRITES[Math.floor(Math.random() * ODYSSEUS_SPRITES.length)]!;

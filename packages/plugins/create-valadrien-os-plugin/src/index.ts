...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".valadrien-os-sdk");
...
/**
 * Generate a complete ValadrienOs plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A ValadrienOs plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "valadrien-os-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["valadrien-os", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@valadrien-os/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@valadrien-os/plugin-sdk": sdkDependency,
...
import { createPluginBundlerPresets } from "@valadrien-os/plugin-sdk/bundlers";
...
`,
    ...
    writeFile(
      ...
      `import { definePlugin, runWorker } from "@valadrien-os/plugin-sdk";
import type {
...
`,
    ...
    writeFile(
      ...
      `import { usePluginData, type PluginWidgetProps } from "@valadrien-os/plugin-sdk/ui";
...
} from "@valadrien-os/plugin-sdk/testing";
...
`,
    ...
    writeFile(
      path.join(outputDir, "src", "ui", "index.tsx"),
      `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@valadrien-os/plugin-sdk/ui";
...
When this package is installed from a local path, ValadrienOs watches that rebuilt
output and reloads the plugin worker. Local installs run trusted code from this
folder on your machine.

${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@valadrien-os/plugin-sdk\` and \`@valadrien-os/shared\` from a local ValadrienOs checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.valadrien-os-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}

## Install Into ValadrienOs

\`\`\`bash
valadrien-os plugin install ${shellQuote(toPosixPath(outputDir))}
\`\`\`
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-valadrien-os-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <valadrien-os-sdk-path>]");
    process.exit(1);
  }

...
export const PLUGIN_ID = "odysseus.plugin-llm-wiki";
...
export const ODYSSEUS_DISTILL_SKILL_KEY = "odysseus-distill";
...
export const DEFAULT_MAX_ODYSSEUS_ISSUE_SOURCE_CHARS = 12000;
export const DEFAULT_MAX_ODYSSEUS_CURSOR_WINDOW_CHARS = 60000;
export const DEFAULT_MAX_ODYSSEUS_ROUTINE_RUN_CHARS = 120000;
export const DEFAULT_ODYSSEUS_COST_CENTS_PER_1K_CHARS = 1;
...
function canonicalSkillKey(skillKey: string) {
  return `plugin/odysseus-plugin-llm-wiki/${skillKey}`;
}
...
const CURSOR_WINDOW_ROUTINE_DESCRIPTION = `Process bounded Odysseus issue-history windows into the LLM Wiki.

Run procedure:
Target space: default (slug: default). Odysseus-derived indexing currently writes only into the default space, so this routine never sweeps other spaces. Per-space Odysseus ingestion profiles are a later phase; until they ship, treat any prompt to operate on a non-default space here as a bug and stop.
...
5. For ambiguous contradictions or major rewrites, leave the pages unchanged and create or comment a follow-up Odysseus issue with the exact files and evidence.
...
const manifest: OdysseusPluginManifestV1 = {
  ...
  localFolders: [
    {
      ...
      description: "Company-scoped local folder that stores raw sources, wiki pages, Odysseus project standups under wiki/projects/, AGENTS.md, IDEA.md, wiki/index.md, and wiki/log.md.",
      access: "readWrite",
      requiredDirectories: [
        "raw",
        "wiki",
  ...
  skills: [
    ...
    {
      skillKey: ODYSSEUS_DISTILL_SKILL_KEY,
      displayName: "Odysseus Distill",
      slug: ODYSSEUS_DISTILL_SKILL_KEY,
      description: "Turn Odysseus cursor-window, distill, or backfill source bundles into wiki-insightful project knowledge.",
      markdown: skillMarkdown(ODYSSEUS_DISTILL_SKILL_KEY)
    },
    {

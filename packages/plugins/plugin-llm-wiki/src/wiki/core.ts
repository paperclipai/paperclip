...
import type { Agent, AgentSessionEvent, Issue, IssueComment, PluginContext, PluginEvent, PluginLocalFolderEntry, Project, ToolResult } from "@odysseus/plugin-sdk";
import type { IssueDocument, PluginIssueOriginKind, PluginManagedRoutineResolution, PluginManagedSkillResolution } from "@odysseus/plugin-sdk/types";
import {
  DEFAULT_MAX_SOURCE_BYTES,
  DEFAULT_MAX_ODYSSEUS_CURSOR_WINDOW_CHARS,
  DEFAULT_MAX_ODYSSEUS_ISSUE_SOURCE_CHARS,
  DEFAULT_MAX_ODYSSEUS_ROUTINE_RUN_CHARS,
  DEFAULT_ODYSSEUS_COST_CENTS_PER_1K_CHARS,
  PLUGIN_ID,
...
const MAX_ODYSSEUS_INGESTION_PROFILE_SOURCE_COUNT = 3;
const MAX_ODYSSEUS_DISTILLATION_FAN_OUT = 25;
const MAX_ODYSSEUS_PROFILE_SELECTED_PROJECTS = 25;
const MAX_ODYSSEUS_PROFILE_ROOT_ISSUES = 25;
...
export type OdysseusDistillationScope = "company" | "project" | "root_issue";
export type OdysseusDistillationWorkItemKind = "manual" | "retry" | "backfill" | "priority_override" | "review_patch";
...
export type OdysseusIngestionSourceScope =
...
export type OdysseusIngestionProfileV1 = {
  version: 1;
  enabled: boolean;
  sourceScopes: OdysseusIngestionSourceScope[];
...
export type OdysseusIngestionProfileEffectiveState =
  | "enabled"
  | "disabled"
  | "policy_blocked"
  | "pending_approval"
...
export type OdysseusIngestionProfileRead = {
  wikiId: string;
  space: Pick<WikiSpace, "id" | "slug" | "displayName" | "accessScope" | "status">;
  profile: OdysseusIngestionProfileV1;
  effectiveState: OdysseusIngestionProfileEffectiveState;
  policyBlocks: string[];
...
type OdysseusIngestionPolicyPurpose =
  | "profile_read"
  | "profile_update"
  | "candidate_search"
  | "queue"
...
type OdysseusIngestionPolicyDecision =
...
type OdysseusIngestionCandidatesInput = SpaceInput & {
  query?: string | null;
};
...
type OdysseusSourceBundleInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  projectId?: string | null;
...
type OdysseusSourceBundle = {
  markdown: string;
  sourceRefs: OdysseusSourceRef[];
  sourceHash: string;
...
type OdysseusDistillationRunInput = OdysseusSourceBundleInput;

type OdysseusDistillationOutcomeInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  runId: string;
...
type OdysseusDistillationWorkItemInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  kind: OdysseusDistillationWorkItemKind;
  projectId?: string | null;
  rootIssueId?: string | null;
...
type OdysseusProjectPageDistillationInput = OdysseusSourceBundleInput & {
  autoApply?: boolean;
...
type OdysseusDistillationPatchOperation =
  | "standup_update"
  ...
  | "index_refresh"
  | "log_append";
type OdysseusDistillationPatch = {
  pagePath: string;
  operationType: OdysseusDistillationPatchOperation;
...
type OdysseusEventIngestResult =
...
async function requireOdysseusIngestionPolicy(
  ctx: PluginContext,
  input: { companyId: string; wikiId: string; spaceSlug?: string | null },
  purpose: OdysseusIngestionPolicyPurpose,
  ...
  const decision = evaluateOdysseusProfilePolicy({
    space,
    profile,
    purpose,
...
function assertOdysseusSourceScopePayload(input: { projectId?: string | null; rootIssueId?: string | null }) {
  if (input.projectId && input.rootIssueId) {
    throw new Error("Odysseus source scope must specify either projectId or rootIssueId, not both.");
  }
}
...
function assertRequestedCharacterLimit(name: string, value: unknown, max: number) {
  ...
  if (Math.floor(value) > max) {
    throw new Error(`${name} exceeds the hard Odysseus ingestion cap of ${max} characters.`);
  }
}
...
function normalizeCostRate(value: unknown): number {
  if (typeof value !== "number" || !Number.isFinite(value)) return DEFAULT_ODYSSEUS_COST_CENTS_PER_1K_CHARS;
  return Math.max(0, value);
}

type OdysseusDistillationLimits = {
  maxCharacters: number;
  maxCharactersPerSource: number;
...
type DistillationSourceProtectionResult = {
  body: string;
  warning: string | null;
  refPatch: Pick<OdysseusSourceRef, "redactionStatus" | "redactionReasons">;
};
...
function protectDistillationSourceBody(input: {
  ...
  return {
    body: [
      ...
      "- Review the original Odysseus source directly if a human needs the unredacted material.",
    ].join("\n"),
...
async function resolveOdysseusDistillationLimits(
  ctx: PluginContext,
  input: Pick<OdysseusSourceBundleInput, "maxCharacters" | "maxCharactersPerSource" | "routineRun">,
): Promise<OdysseusDistillationLimits> {
  assertRequestedCharacterLimit("maxCharacters", input.maxCharacters, DEFAULT_MAX_ODYSSEUS_CURSOR_WINDOW_CHARS);
  assertRequestedCharacterLimit("maxCharactersPerSource", input.maxCharactersPerSource, DEFAULT_MAX_ODYSSEUS_ISSUE_SOURCE_CHARS);
  const config = await ctx.config.get() as Record<string, unknown>;
  const maxCharactersPerSource = Math.min(
    normalizeBundleLimit(input.maxCharactersPerSource, DEFAULT_MAX_ODYSSEUS_ISSUE_SOURCE_CHARS),
    normalizeBundleLimit(config.maxOdysseusIssueSourceCharacters, DEFAULT_MAX_ODYSSEUS_ISSUE_SOURCE_CHARS),
  );
  const cursorWindowCap = normalizeBundleLimit(
    config.maxOdysseusCursorWindowCharacters,
    DEFAULT_MAX_ODYSSEUS_CURSOR_WINDOW_CHARS,
  );
  const routineRunCap = normalizeBundleLimit(
    config.maxOdysseusRoutineRunCharacters,
    DEFAULT_MAX_ODYSSEUS_ROUTINE_RUN_CHARS,
  );
  ...
  return {
    ...
    costCentsPerThousandSourceCharacters: normalizeCostRate(config.odysseusCostCentsPerThousandSourceCharacters),
  };
}

async function resolveOdysseusDistillationLimitsForSpace(
  ctx: PluginContext,
  input: Pick<OdysseusSourceBundleInput, "companyId" | "maxCharacters" | "maxCharactersPerSource" | "routineRun"> & { space: WikiSpace },
): Promise<OdysseusDistillationLimits> {
  const [base, profile] = await Promise.all([
    resolveOdysseusDistillationLimits(ctx, input),
...
function defaultOdysseusIngestionProfile(input: {
  space: Pick<WikiSpace, "slug">;
  legacySettings?: WikiEventIngestionSettings | null;
}): OdysseusIngestionProfileV1 {
  ...
  return {
    ...
    cursor: {
      maxWindowCharacters: DEFAULT_MAX_ODYSSEUS_CURSOR_WINDOW_CHARS,
      maxCharactersPerSource: DEFAULT_MAX_ODYSSEUS_ISSUE_SOURCE_CHARS,
...
function normalizeOdysseusIngestionSourceScope(value: unknown): OdysseusIngestionSourceScope | null {
  ...
  if (kind === "active_projects") {
    ...
    return {
      kind,
      limit: normalizeLimit(record.limit, 3, MAX_ODYSSEUS_PROFILE_SELECTED_PROJECTS),
      ...(statuses && statuses.length > 0 ? { statuses: [...new Set(statuses)] } : {}),
    };
  }
  if (kind === "selected_projects") {
    return { kind, projectIds: stringArray(record.projectIds).slice(0, MAX_ODYSSEUS_PROFILE_SELECTED_PROJECTS) };
  }
  if (kind === "root_issues") {
    return { kind, issueIds: stringArray(record.issueIds).slice(0, MAX_ODYSSEUS_PROFILE_ROOT_ISSUES) };
  }
  if (kind === "company_all") {
...
function normalizeOdysseusIngestionProfile(
  value: unknown,
  input: { space: Pick<WikiSpace, "slug">; legacySettings?: WikiEventIngestionSettings | null },
): OdysseusIngestionProfileV1 {
  const fallback = defaultOdysseusIngestionProfile(input);
  ...
  return {
    ...
    sourceScopes: Array.isArray(record.sourceScopes)
      ? record.sourceScopes.map(normalizeOdysseusIngestionSourceScope).filter((scope): scope is OdysseusIngestionSourceScope => Boolean(scope))
    ...
    cursor: {
      maxWindowCharacters: normalizeLimit(cursor.maxWindowCharacters, fallback.cursor.maxWindowCharacters, DEFAULT_MAX_ODYSSEUS_CURSOR_WINDOW_CHARS),
      maxCharactersPerSource: normalizeLimit(cursor.maxCharactersPerSource, fallback.cursor.maxCharactersPerSource, DEFAULT_MAX_ODYSSEUS_ISSUE_SOURCE_CHARS),
...
async function profileForSpace(ctx: PluginContext, companyId: string, space: WikiSpace): Promise<OdysseusIngestionProfileV1> {
  const legacySettings = space.slug === DEFAULT_SPACE_SLUG ? await getEventIngestionSettings(ctx, companyId) : null;
  return normalizeOdysseusIngestionProfile(space.settings.odysseusIngestion, { space, legacySettings });
}
...
function evaluateOdysseusProfilePolicy(input: {
  space: WikiSpace;
  profile?: OdysseusIngestionProfileV1 | null;
  purpose: OdysseusIngestionPolicyPurpose;
  requireEnabledProfile?: boolean;
}): OdysseusIngestionPolicyDecision {
  ...
  if (space.status !== "active") {
    return {
      ...
      message: `Odysseus ingestion policy denied ${purpose}: space "${space.slug}" is ${space.status}.`,
    };
  }
  if (space.accessScope !== "shared") {
    return {
      ...
      message: `Odysseus ingestion policy denied ${purpose}: ${space.accessScope} spaces cannot ingest Odysseus sources until host permissions are enforced.`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && !profile?.enabled) {
    return {
      ...
      message: `Odysseus ingestion policy denied ${purpose}: Odysseus ingestion is not enabled for space "${space.slug}".`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && profile?.enabled && profile.sourceScopes.length === 0) {
    return {
      ...
      message: `Odysseus ingestion policy denied ${purpose}: space "${space.slug}" has no source scopes configured.`,
    };
  }
  return { allowed: true, space };
}

export async function getOdysseusIngestionProfile(
  ctx: PluginContext,
  input: { companyId: string; wikiId?: string | null; spaceSlug?: string | null },
): Promise<OdysseusIngestionProfileRead> {
  ...
  const profile = await profileForSpace(ctx, input.companyId, space);
  const policy = evaluateOdysseusProfilePolicy({ space, profile, purpose: "profile_read" });
  const historicalPageCount = await countOdysseusHistoricalPages(ctx, {
    companyId: input.companyId,
    wikiId,
    spaceId: space.id,
  });
  const overlapCount = await countOdysseusProfileOverlaps(ctx, {
    companyId: input.companyId,
    wikiId,
    space,
    profile,
  });
  const effectiveState: OdysseusIngestionProfileEffectiveState = !policy.allowed
    ? "policy_blocked"
...
async function countOdysseusHistoricalPages(ctx: PluginContext, input: { companyId: string; wikiId: string; spaceId: string }): Promise<number> {
...
function scopeIdentity(scope: OdysseusIngestionSourceScope): string[] {
...
async function countOdysseusProfileOverlaps(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  space: WikiSpace;
  profile: OdysseusIngestionProfileV1;
...
async function validateOdysseusIngestionProfile(ctx: PluginContext, input: {
  companyId: string;
  space: WikiSpace;
  profile: OdysseusIngestionProfileV1;
}) {
  const policy = evaluateOdysseusProfilePolicy({
    space: input.space,
  ...
  if (input.profile.enabled && input.profile.sourceScopes.length === 0) {
    throw new Error("Odysseus ingestion profile must include at least one source scope before it can be enabled.");
  }
  if (input.profile.sourceScopes.length > MAX_ODYSSEUS_INGESTION_PROFILE_SOURCE_COUNT) {
    throw new Error(`Odysseus ingestion profile sources exceed the hard cap of ${MAX_ODYSSEUS_INGESTION_PROFILE_SOURCE_COUNT}.`);
  }
  for (const scope of input.profile.sourceScopes) {
    ...
    if (scope.kind === "selected_projects") {
      if (scope.projectIds.length > MAX_ODYSSEUS_PROFILE_SELECTED_PROJECTS) {
        throw new Error(`selected_projects exceeds the hard cap of ${MAX_ODYSSEUS_PROFILE_SELECTED_PROJECTS}.`);
      }
    ...
    if (scope.kind === "root_issues") {
      if (scope.issueIds.length > MAX_ODYSSEUS_PROFILE_ROOT_ISSUES) {
        throw new Error(`root_issues exceeds the hard cap of ${MAX_ODYSSEUS_PROFILE_ROOT_ISSUES}.`);
      }
...
export async function updateOdysseusIngestionProfile(ctx: PluginContext, input: {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  profile: unknown;
  ...
  const profile = normalizeOdysseusIngestionProfile(input.profile, { space, legacySettings: space.slug === DEFAULT_SPACE_SLUG ? await getEventIngestionSettings(ctx, input.companyId) : null });
  await validateOdysseusIngestionProfile(ctx, { companyId: input.companyId, space, profile });
  await updateSpace(ctx, {
    companyId: input.companyId,
    wikiId,
    spaceSlug: space.slug,
    settings: { odysseusIngestion: profile },
  });
  ...
  await ctx.activity.log({
    companyId: input.companyId,
    message: `Updated Odysseus ingestion profile for ${space.displayName}`,
    entityType: "llm_wiki_space",
    entityId: space.id,
    metadata: {
      type: "plugin.llm_wiki.odysseus_ingestion_profile_updated",
      wikiId,
      spaceSlug: space.slug,
  ...
  return getOdysseusIngestionProfile(ctx, { companyId: input.companyId, wikiId, spaceSlug: space.slug });
}

export async function listOdysseusIngestionCandidates(ctx: PluginContext, input: OdysseusIngestionCandidatesInput): Promise<{
  ...
  const wikiId = normalizeWikiId(input.wikiId);
  await requireOdysseusIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "candidate_search");
...
export async function updateEventIngestionSettings(
  ...
  await requireOdysseusIngestionPolicy(ctx, {
    companyId: input.companyId,
  ...
  if (sourceKeys.length > MAX_ODYSSEUS_INGESTION_PROFILE_SOURCE_COUNT) {
    throw new Error(`Odysseus ingestion profile sources exceed the hard cap of ${MAX_ODYSSEUS_INGESTION_PROFILE_SOURCE_COUNT}.`);
  }
  ...
  const profile = normalizeOdysseusIngestionProfile(
    {
      ...defaultOdysseusIngestionProfile({ space: defaultSpace, legacySettings: next }),
      ...
      cursor: {
        ...defaultOdysseusIngestionProfile({ space: defaultSpace, legacySettings: next }).cursor,
...
function assertWikiPath(path: string, options: { allowMetadata?: boolean } = {}): string {
  ...
  if (
    ...
    !(options.allowMetadata && trimmed.startsWith(".odysseus/"))
  ) {
...
function distillationCursorTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "odysseus_distillation_cursors");
}

function distillationRunTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "odysseus_distillation_runs");
}

function sourceSnapshotTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "odysseus_source_snapshots");
}

function distillationWorkItemTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "odysseus_distillation_work_items");
}

function pageBindingTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "odysseus_page_bindings");
}
...
function operationPromptWithSpaceContext(input: OperationSpaceContext): string {
  const odysseusDerived = input.operationType === "distill" || input.operationType === "backfill";
  return [
    ...
    odysseusDerived
      ? "- Odysseus-derived distill/backfill operations are default-space-only in Phase 1. Stop and comment if asked to write Odysseus-derived pages into a non-default space."
...
export async function createOperationIssue(ctx: PluginContext, input: OperationInput) {
  ...
  const space = input.operationType === "distill" || input.operationType === "backfill"
    ? await requireOdysseusIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "queue", { requireEnabledProfile: true })
...
function odysseusDistillationScope(input: { projectId?: string | null; rootIssueId?: string | null }): OdysseusDistillationScope {
  if (input.rootIssueId) return "root_issue";
  if (input.projectId) return "project";
  return "company";
}

function odysseusCursorScopeMetadata(input: { projectId?: string | null; rootIssueId?: string | null }) {
  const sourceScope = odysseusDistillationScope(input);
...
async function upsertOdysseusDistillationCursor(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  spaceId: string;
  projectId?: string | null;
  ...
  const scope = odysseusCursorScopeMetadata(input);
  await ctx.db.execute(
    `INSERT INTO ${distillationCursorTable(ctx)} AS odysseus_distillation_cursors
     ...
     VALUES ($1, $2, $3, $11, $4, $5, $6, $7, 'odysseus_issue_history', $8::timestamptz, $9, $10::jsonb)
     ON CONFLICT (company_id, wiki_id, space_id, source_scope, scope_key, source_kind)
     DO UPDATE SET last_observed_at = GREATEST(
                       COALESCE(odysseus_distillation_cursors.last_observed_at, EXCLUDED.last_observed_at),
                       COALESCE(EXCLUDED.last_observed_at, odysseus_distillation_cursors.last_observed_at)
                     ),
                   pending_event_count = odysseus_distillation_cursors.pending_event_count + EXCLUDED.pending_event_count,
                   metadata = odysseus_distillation_cursors.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      cursorId,
      input.companyId,
  ...
  const rows = await ctx.db.query<{ id: string }>(
    `SELECT id
      ...
      WHERE company_id = $1
        ...
        AND source_scope = $4
        AND scope_key = $5
        AND source_kind = 'odysseus_issue_history'
      LIMIT 1`,
...
export async function enableActiveProjectDistillation(ctx: PluginContext, input: {
  ...
  const wikiId = normalizeWikiId(input.wikiId);
  const space = await requireOdysseusIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "candidate_search", { requireEnabledProfile: true });
  if (typeof input.limit === "number" && Number.isFinite(input.limit) && Math.floor(input.limit) > MAX_ODYSSEUS_DISTILLATION_FAN_OUT) {
    throw new Error(`Odysseus ingestion fan-out exceeds the hard cap of ${MAX_ODYSSEUS_DISTILLATION_FAN_OUT} enabled profiles.`);
  }
  ...
  for (const project of activeProjects) {
    ...
    const cursorId = await upsertOdysseusDistillationCursor(ctx, {
      companyId: input.companyId,
      wikiId,
      spaceId: space.id,
...
function appendBoundedSection(input: {
  ...
  title: string;
  body: string;
  refs: OdysseusSourceRef[];
  ref: OdysseusSourceRef;
...
function sourceRefUpdatedAt(ref: OdysseusSourceRef): string | null {
  return ref.updatedAt ?? ref.createdAt ?? null;
}

function issueInBackfillWindow(issue: Issue, input: Pick<OdysseusSourceBundleInput, "backfillStartAt" | "backfillEndAt">): boolean {
...
async function listOdysseusBundleIssues(ctx: PluginContext, input: OdysseusSourceBundleInput): Promise<Issue[]> {
...
export async function assembleOdysseusSourceBundle(ctx: PluginContext, input: OdysseusSourceBundleInput): Promise<OdysseusSourceBundle> {
  const wikiId = normalizeWikiId(input.wikiId);
  assertOdysseusSourceScopePayload(input);
  const space = await requireOdysseusIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
  const limits = await resolveOdysseusDistillationLimitsForSpace(ctx, { ...input, space });
  ...
  const issues = await listOdysseusBundleIssues(ctx, input);
  const scope = odysseusCursorScopeMetadata(input);
  const sourceRefs: OdysseusSourceRef[] = [];
  const warnings: string[] = [];
  const lines = [
    `# Odysseus source bundle`,
    "",
    "## Bundle Metadata",
    "",
...
export async function createOdysseusDistillationRun(ctx: PluginContext, input: OdysseusDistillationRunInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  assertOdysseusSourceScopePayload(input);
  const space = await requireOdysseusIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
  const scope = odysseusCursorScopeMetadata(input);
  ...
  const bundle = await assembleOdysseusSourceBundle(ctx, input);
...
export async function recordOdysseusDistillationOutcome(ctx: PluginContext, input: OdysseusDistillationOutcomeInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  const space = await requireOdysseusIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
...
export async function createOdysseusDistillationWorkItem(ctx: PluginContext, input: OdysseusDistillationWorkItemInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  assertOdysseusSourceScopePayload(input);
  const space = await requireOdysseusIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "queue", { requireEnabledProfile: true });
  ...
  await ctx.db.execute(
    `INSERT INTO ${distillationWorkItemTable(ctx)} AS odysseus_distillation_work_items
     ...
     DO UPDATE SET priority = EXCLUDED.priority,
                   metadata = odysseus_distillation_work_items.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      itemId,
      input.companyId,
...
function sourceRefLabel(ref: OdysseusSourceRef): string {
  const issue = ref.issueIdentifier ? issueReference(ref.issueIdentifier) : (ref.title ?? "Odysseus source");
  if (ref.kind === "document") return `${issue} document:${ref.documentKey ?? "unknown"}`;
  if (ref.kind === "comment") return `${issue} comment`;
  return issue;
}

function sourceRefMarkdown(ref: OdysseusSourceRef): string {
  const metadata = [
...
function issueSourceRef(issue: Issue): OdysseusSourceRef {
  return {
    kind: "issue",
    issueId: issue.id,
    issueIdentifier: issue.identifier ?? null,
...
function projectPageSlug(input: { project: Project | null; rootIssue: Issue | null }): string {
  return slugify(input.project?.name ?? input.rootIssue?.title ?? "odysseus-project");
}
...
function hasDurableSignal(bundle: OdysseusSourceBundle, issues: Issue[]): boolean {
...
function standupPageContents(input: {
  ...
  issues: Issue[];
  bundle: OdysseusSourceBundle;
  pagePath: string;
  durablePagePath: string;
}): string {
  const currentAsOf = input.bundle.sourceWindowEnd ?? new Date().toISOString();
  const title = input.project?.name ?? input.rootIssue?.title ?? "Odysseus Project";
  ...
  return [
    ...
    "",
    lead
      ? `The current center of gravity is **${issueConcept(lead)}** (${issueReferenceFor(lead)}). ${input.bundle.clipped ? "The source window was clipped, so treat this as a bounded readout rather than the full live state." : "This is a high-level readout of the meaningful Odysseus work in the current source window."}`
...
function projectPageContents(input: {
  ...
  return [
    ...
    "## Overview",
    "",
    description ? excerpt(description, 700) : `This page synthesizes Odysseus issue history into a stable project brief for ${title}.`,
    "",
    "## Current Direction",
    "",
    activeIssues.length
    ...
    input.bundle.clipped ? "\nThe source window was clipped, so verify Odysseus before treating this as complete state." : null,
    "",
    "## Workstreams",
    "",
    ...(recentIssues.length
...
function decisionsPageContents(input: { project: Project | null; rootIssue: Issue | null; issues: Issue[]; bundle: OdysseusSourceBundle }): string {
  const title = input.project?.name ?? input.rootIssue?.title ?? "Odysseus Project";
...
function historyPageContents(input: { project: Project | null; rootIssue: Issue | null; issues: Issue[]; bundle: OdysseusSourceBundle }): string {
  const title = input.project?.name ?? input.rootIssue?.title ?? "Odysseus Project";
...
function appendProjectLogContents(current: string | null, input: { standupPath: string; pagePath: string; runId: string; sourceHash: string; status: string; warnings: string[] }): string {
  ...
  const warningLines = input.warnings.length
    ...
    : ["- warnings: none"];
  const entry = [
    `## [${new Date().toISOString().slice(0, 10)}] odysseus-distill | ${input.status}`,
...
async function upsertPageBinding(ctx: PluginContext, input: {
  ...
  await ctx.db.execute(
    `INSERT INTO ${pageBindingTable(ctx)} AS odysseus_page_bindings
     ...
     DO UPDATE SET last_applied_source_hash = EXCLUDED.last_applied_source_hash,
                   ...
                   metadata = odysseus_page_bindings.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      randomUUID(),
      input.companyId,
...
export function getDistillationAutoApplyRestriction(): DistillationAutoApplyRestriction {
  const rawMode = process.env.ODYSSEUS_DEPLOYMENT_MODE;
  const rawExposure = process.env.ODYSSEUS_DEPLOYMENT_EXPOSURE;
...
export async function distillOdysseusProjectPage(ctx: PluginContext, input: OdysseusProjectPageDistillationInput) {
  if (!input.projectId && !input.rootIssueId) {
  ...
  const run = await createOdysseusDistillationRun(ctx, input);
  ...
  if (!hasDurableSignal(bundle, issues)) {
    await recordOdysseusDistillationOutcome(ctx, {
      companyId: input.companyId,
      wikiId,
      spaceSlug: space.slug,
      runId: run.runId,
      ...
      warning: "Skipped low-signal Odysseus source window.",
    });
    return {
      status: "skipped",
      reason: "low_signal",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped low-signal Odysseus source window."],
      patches: [] as OdysseusDistillationPatch[],
    };
  }
  ...
  if (existingBinding?.last_applied_source_hash === bundle.sourceHash) {
    await recordOdysseusDistillationOutcome(ctx, {
      ...
      sourceHash: bundle.sourceHash,
      sourceWindowEnd: bundle.sourceWindowEnd,
      warning: "Skipped unchanged Odysseus source hash.",
    });
    return {
      status: "skipped",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped unchanged Odysseus source hash."],
      patches: [] as OdysseusDistillationPatch[],
    };
  }
  ...
  const title = project?.name ?? rootIssue?.title ?? "Odysseus Project";
  ...
  const patches: OdysseusDistillationPatch[] = [
  ...
  for (const patch of patches) {
    await writeWikiPage(ctx, {
      ...
      summary: `Odysseus distillation ${patch.operationType} from ${bundle.sourceHash}`,
...
function rawPathForOdysseusEvent(input: {
  ...
  return assertRawPath(`raw/odysseus/${input.sourceKind}/${eventDate}-${slugify(identifier)}-${slugify(input.label)}-${contentHash(input.contents).slice(0, 8)}.md`);
}

function formatIssueEventSource(issue: Issue, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Odysseus issue: ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
...
function formatCommentEventSource(issue: Issue, comment: IssueComment, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Odysseus comment on ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
...
function formatDocumentEventSource(issue: Issue, document: IssueDocument, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Odysseus document: ${document.title ?? document.key}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
...
async function recordOdysseusCursorObservation(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  space: WikiSpace;
  sourceKind: WikiEventIngestionSource;
  ...
  issue: Issue;
  event: PluginEvent;
}): Promise<Extract<OdysseusEventIngestResult, { status: "recorded" }>> {
  const cursorId = await upsertOdysseusDistillationCursor(ctx, {
    companyId: input.companyId,
    wikiId: input.wikiId,
    spaceId: input.space.id,
...
async function odysseusProfileIncludesIssue(ctx: PluginContext, input: {
  companyId: string;
  issue: Issue;
  profile: OdysseusIngestionProfileV1;
}): Promise<boolean> {
...
async function routeOdysseusCursorObservation(ctx: PluginContext, input: {
  companyId: string;
  sourceKind: WikiEventIngestionSource;
  sourceId: string;
  issue: Issue;
  event: PluginEvent;
}): Promise<OdysseusEventIngestResult> {
  const { spaces } = await listSpaces(ctx, { companyId: input.companyId, wikiId: DEFAULT_WIKI_ID });
  const recorded: Array<Extract<OdysseusEventIngestResult, { status: "recorded" }>> = [];
  let eligibleProfileCount = 0;
  for (const space of spaces) {
    const profile = await profileForSpace(ctx, input.companyId, space);
    if (!profile.enabled) continue;
    const policy = evaluateOdysseusProfilePolicy({ space, profile, purpose: "event_routing", requireEnabledProfile: true });
    if (!policy.allowed) continue;
    if (!profile.sourceKinds[input.sourceKind]) continue;
    if (!(await odysseusProfileIncludesIssue(ctx, { companyId: input.companyId, issue: input.issue, profile }))) continue;
    eligibleProfileCount += 1;
    if (eligibleProfileCount > MAX_ODYSSEUS_DISTILLATION_FAN_OUT) {
      throw new Error(`Odysseus ingestion fan-out exceeds the hard cap of ${MAX_ODYSSEUS_DISTILLATION_FAN_OUT} enabled profiles.`);
    }
    if (await ctx.state.get(eventIngestionDedupKey(input.companyId, space.wikiId, space.id, input.sourceKind, input.sourceId))) {
      continue;
    }
    recorded.push(await recordOdysseusCursorObservation(ctx, {
      ...input,
      wikiId: space.wikiId,
      space,
    }));
...
export async function handleOdysseusEventIngestion(ctx: PluginContext, event: PluginEvent): Promise<OdysseusEventIngestResult> {
  ...
  if (event.eventType === "issue.created" || event.eventType === "issue.updated") {
    ...
    return routeOdysseusCursorObservation(ctx, {
      companyId,
      sourceKind: "issues",
      sourceId,
      issue,

...
| [`valadrien-os-distill`](./valadrien-os-distill/SKILL.md) | Cursor-window, distill, or backfill operation on Valadrien OS activity — write a wiki-insightful project page, decisions log, and history note. |
...
- Each skill names the input it expects (e.g. an operation issue with `originKind` ending in `:ingest`, a captured `raw/` path, a Valadrien OS source bundle).

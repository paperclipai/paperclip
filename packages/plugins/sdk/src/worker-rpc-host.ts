...
import type { AiTeamCorpPluginManifestV1 } from "@aiteamcorp/shared";

import type { AiTeamCorpPlugin } from "./define-plugin.js";
import type {
 ...
 * import { startWorkerRpcHost } from "@aiteamcorp/plugin-sdk";
 *
 * startWorkerRpcHost({ plugin });
 * ```
 *
...
export function startWorkerRpcHost(options: WorkerRpcHostOptions): WorkerRpcHost {
  ...
  let manifest: AiTeamCorpPluginManifestV1 | null = null;
  ...
  const toolHandlers = new Map<string, {
    declaration: Pick<import("@aiteamcorp/shared").PluginToolDeclaration, "displayName" | "description" | "parametersSchema">;

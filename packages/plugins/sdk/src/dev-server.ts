 ...
 *
 * Endpoint summary:
 * - `GET /__valadrien_os__/health` for diagnostics
 * - `GET /__valadrien_os__/events` for hot-reload stream
...
export async function startPluginDevServer(options: PluginDevServerOptions = {}): Promise<PluginDevServer> {
  ...
  const handleRequest = async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (url === "/__valadrien_os__/health") {
      sendJson(res, { ok: true, rootDir, uiDir });
      return;
    }

    if (url === "/__valadrien_os__/events") {
      res.writeHead(200, {

# `@ironworksai/plugin-sdk`

Official TypeScript SDK for Ironworks plugin authors.

- **Worker SDK:** `@ironworksai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@ironworksai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@ironworksai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@ironworksai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@ironworksai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@ironworksai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@ironworksai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@ironworksai/plugin-sdk/ui/hooks` | Hooks only |
| `@ironworksai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@ironworksai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@ironworksai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@ironworksai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@ironworksai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@ironworksai/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @ironworksai/plugin-sdk
```
...
- Plugin UI bundles run as same-origin JavaScript inside the main Ironworks app. They can call ordinary Ironworks HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Ironworks instance at runtime.
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@ironworksai/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Ironworks information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@ironworksai/plugin-sdk`.
...
import { usePluginAction, type PluginBridgeError } from "@ironworksai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@ironworksai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@ironworksai/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@ironworksai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@ironworksai/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@ironworksai/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@ironworksai/plugin-sdk/ui";
...
```bash
ironworks-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@ironworksai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__ironworks__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__ironworks__/events` streams `reload` SSE events on UI build changes

# `@aiteamcorp/plugin-sdk`

Official TypeScript SDK for AiTeamCorp plugin authors.

- **Worker SDK:** `@aiteamcorp/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@aiteamcorp/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@aiteamcorp/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@aiteamcorp/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@aiteamcorp/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@aiteamcorp/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@aiteamcorp/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@aiteamcorp/plugin-sdk/ui/hooks` | Hooks only |
| `@aiteamcorp/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@aiteamcorp/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@aiteamcorp/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@aiteamcorp/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@aiteamcorp/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@aiteamcorp/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @aiteamcorp/plugin-sdk
```
...
- Plugin UI bundles run as same-origin JavaScript inside the main AiTeamCorp app. They can call ordinary AiTeamCorp HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the AiTeamCorp instance at runtime.
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@aiteamcorp/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core AiTeamCorp information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@aiteamcorp/plugin-sdk`.
...
import { usePluginAction, type PluginBridgeError } from "@aiteamcorp/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@aiteamcorp/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@aiteamcorp/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@aiteamcorp/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@aiteamcorp/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@aiteamcorp/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@aiteamcorp/plugin-sdk/ui";
...
```ts
import { startPluginDevServer } from "@aiteamcorp/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__aiteamcorp__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__aiteamcorp__/events` streams `reload` SSE events on UI build changes

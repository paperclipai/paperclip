...
export function createWikiService(client: WeKnoraClient, config: WeKnoraConfig) {
  return {
    async listPages(input: { knowledgeBaseId: string; page?: number; pageSize?: number }) {
      ...
      return { pages: result.pages.slice(0, size), page, pageSize: size, hasMore: result.total != null ? page * size < result.total : result.pages.length >= size };
    },
    async readPage(input: { knowledgeBaseId: string; slug: string }) {
      ...
      const content = result.page.content.length > maxChars ? result.page.content.slice(0, maxChars) : result.page.content;
      return { page: { ...result.page, content, truncated: content.length < result.page.content.length } };
    },
    async search(input: { knowledgeBaseId: string; query: string; limit?: number }) {
      ...
      return { results: result.results.slice(0, limit) };
    },
  };
}

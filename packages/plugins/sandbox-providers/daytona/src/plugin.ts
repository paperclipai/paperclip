...
function buildSandboxLabels(input: {
  ...
  return {
    "odysseus-provider": "daytona",
    "odysseus-company-id": input.companyId,
    "odysseus-environment-id": input.environmentId,
    "odysseus-reuse-lease": input.reuseLease ? "true" : "false",
    ...(input.runId ? { "odysseus-run-id": input.runId } : {}),
  };
}
...
async function resolveSandboxWorkingDirectory(sandbox: Sandbox): Promise<string> {
  const root = (await sandbox.getWorkDir())?.trim()
    ...
    || "/home/daytona";
  const remoteCwd = path.posix.join(root, "odysseus-workspace");
...
async function executeOneShot(
  ...
  const stdinPath = params.stdin != null ? `/tmp/odysseus-stdin-${randomUUID()}` : null;

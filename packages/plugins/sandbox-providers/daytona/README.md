...
Active-work cancellation stops the sandbox before waiting for admitted execute
and file-transfer requests. It confirms termination only after those requests
settle and the provider is stopped again. A settlement timeout leaves the lease
closed to new work and reports failure, so the host retains recovery ownership.

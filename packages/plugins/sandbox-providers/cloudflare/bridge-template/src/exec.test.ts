...
describe("bridge exec", () => {
  it("invokes target.exec with a single shell command string and no args option", async () => {
    ...
    await executeInSandbox({
      ...
      env: { VALADRIEN_OS_TEST_FLAG: "1" },
      sessionStrategy: "named",
      sessionId: "valadrien-os",
      timeoutMs: 12_345,
    });
    ...
    expect(commandArg).toContain("/workspace/valadrien-os");
    expect(commandArg).toContain("VALADRIEN_OS_TEST_FLAG");
  ...
  it("stages stdin through a sandbox temp file and redirects from it", async () => {
    ...
    expect(stdinPath).toMatch(/^\/tmp\/\.valadrien-os-bridge-stdin-/);

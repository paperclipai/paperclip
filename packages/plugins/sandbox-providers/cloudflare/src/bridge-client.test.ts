...
const baseConfig: CloudflareDriverConfig = {
  ...
  sleepAfter: "10m",
  normalizeId: true,
  requestedCwd: "/workspace/valadrien-os",
...
describe("Cloudflare bridge client timeouts", () => {
  ...
  it("keeps the configured timeout for non-exec requests", () => {
    expect(resolveRequestTimeoutMs(baseConfig, "/api/valadrien-os-sandbox/v1/probe", {
      method: "POST",
  ...
  it("extends exec requests to the command timeout when needed", () => {
    expect(resolveRequestTimeoutMs(baseConfig, "/api/valadrien-os-sandbox/v1/exec", {
      method: "POST",

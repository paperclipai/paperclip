...
describe("Cloudflare sandbox provider plugin", () => {
  ...
  it("maps acquire lease responses from the bridge", async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse({
        ...
        metadata: {
          provider: "cloudflare",
          remoteCwd: "/workspace/valadrien-os",
          resumedLease: false,
        },
      }),
    );
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "https://bridge.example.workers.dev/api/valadrien-os-sandbox/v1/leases/acquire",
      expect.objectContaining({
        method: "POST",
        headers: expect.any(Headers),
      }),
    );
    expect(requestHeadersAt().get("X-ValadrienOs-Run-Id")).toBe("run-1");
    expect(requestHeadersAt().get("X-ValadrienOs-Environment-Id")).toBe("env-1");
    expect(requestHeadersAt().get("X-ValadrienOs-Issue-Id")).toBe("issue-1");
  ...
  it("defaults the sleepAfter passed to the bridge to 1h so long runs don't idle out", async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse({
        providerLeaseId: "pc-run-1-abcd1234",
        metadata: { provider: "cloudflare", remoteCwd: "/workspace/valadrien-os", resumedLease: false },
      }),
    );
  ...
  it("returns expired lease semantics when resume reports lost state", async () => {
    ...
    const lease = await plugin.definition.onEnvironmentResumeLease?.({
      ...
      leaseMetadata: { remoteCwd: "/workspace/valadrien-os" },
      config: {
        bridgeBaseUrl: "https://bridge.example.workers.dev",
        bridgeAuthToken: "resolved-token",
      },
  ...
  it("routes bridge-channel execute calls through a dedicated session", async () => {
    ...
    await plugin.definition.onEnvironmentExecute?.({
      ...
      cwd: "/workspace/valadrien-os",
      env: {
        VALADRIEN_OS_SANDBOX_EXEC_CHANNEL: "bridge",
        KEEP_ME: "visible",
      },
      config: {
        bridgeBaseUrl: "https://bridge.example.workers.dev",
    ...
    expect(requestBodyAt()).toMatchObject({
      ...
      sessionId: "valadrien-os-bridge",
      env: {
        KEEP_ME: "visible",
      },
    });
    expect(requestBodyAt().env).not.toHaveProperty("VALADRIEN_OS_SANDBOX_EXEC_CHANNEL");
  ...
  it("wraps realizeWorkspace bridge failures and forwards the issue header", async () => {
    ...
    await expect(plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      lease: {
        providerLeaseId: "pc-run-1-abcd1234",
        metadata: { remoteCwd: "/workspace/valadrien-os" },
      },
      workspace: {
    ...
    })).rejects.toThrow("Failed to prepare Cloudflare sandbox workspace at /workspace/valadrien-os: mkdir: permission denied");

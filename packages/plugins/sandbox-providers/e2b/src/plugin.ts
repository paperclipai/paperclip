...
async function resolveSandboxWorkingDirectory(sandbox: Sandbox): Promise<string> {
  ...
  const remoteCwd = path.posix.join(cwd.length > 0 ? cwd : "/", "odysseus-workspace");
...
const plugin = definePlugin({
  ...
  async onEnvironmentRealizeWorkspace(
    ...
    const remoteCwd =
      ...
      params.lease.metadata.remoteCwd.trim().length > 0
        ...
        : params.workspace.remotePath ?? params.workspace.localPath ?? "/odysseus-workspace";
  ...
  async onEnvironmentExecute(
    ...
    if (params.stdin != null) {
      stagedStdinPath = `/tmp/odysseus-stdin-${randomUUID()}`;
      try {

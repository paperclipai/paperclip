...
// has passwordless sudo. The Odysseus sandbox callback bridge is a Node script, so
// every Odysseus workload on this provider needs node on PATH before the bridge can
// start. When the operator hasn't supplied their own setup script, install Node 20 via
// nodesource so the VM comes up ready for Odysseus out of the box.
...
function normalizeNamePrefix(value: string | null): string {
  const normalized = (value ?? "odysseus")
    .toLowerCase()
    .replace(/[^a-z0-9-]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .replace(/-{2,}/g, "-");
  return normalized.length > 0 ? normalized.slice(0, 24) : "odysseus";
}
...
async function prepareSshIdentity(config: ExeDevDriverConfig): Promise<{
  ...
  const tempDir = await mkdtemp(path.join(tmpdir(), "odysseus-exe-dev-ssh-"));
...
function formatSshFailure(
  ...
  if (
    ...
    return [
      `Failed to ${action} exe.dev VM ${vmName}: the Odysseus host SSH key is not registered with exe.dev.`,
...
async function buildLease(
  ...
  const remoteCwd = requestedCwd?.trim() || path.posix.join(remote.homeDir, "odysseus-workspace");
...
const plugin = definePlugin({
  ...
  async onEnvironmentValidateConfig(
    ...
    warnings.push(
      "The Odysseus host must have SSH access to the created exe.dev VM, and its SSH key must be registered with exe.dev. The API token only covers provisioning.",
    );

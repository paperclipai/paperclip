...
describe("exe.dev sandbox provider plugin", () => {
  ...
  it("acquires a lease by creating a VM and preparing the SSH workspace", async () => {
    fetchMock.mockResolvedValueOnce(
      new Response(JSON.stringify({
        vm_name: "odysseus-env-run",
        ssh_dest: "odysseus-env-run.exe.xyz",
        https_url: "https://odysseus-env-run.exe.xyz",
        status: "running",
      }), { status: 200 }),
    );
    ...
    expect(lease).toMatchObject({
      providerLeaseId: "odysseus-env-run",
      metadata: {
        provider: "exe-dev",
        vmName: "odysseus-env-run",
        sshDest: "odysseus-env-run.exe.xyz",
  ...
  it("surfaces exe.dev SSH onboarding guidance during lease acquisition", async () => {
    ...
    await expect(plugin.definition.onEnvironmentAcquireLease?.({
      ...
      "the Odysseus host SSH key is not registered with exe.dev",
    );

    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'odysseus-env-run'");
  });
  ...
  it("returns exe.dev SSH onboarding guidance for command execution failures", async () => {
    ...
    expect(String(result?.stderr ?? "")).toContain("the Odysseus host SSH key is not registered with exe.dev");
  ...
  it("probes by creating and then deleting a VM after SSH verification", async () => {
    fetchMock
      .mockResolvedValueOnce(
        new Response(JSON.stringify({
          vm_name: "odysseus-probe",
          ssh_dest: "odysseus-probe.exe.xyz",
          status: "running",
        }), { status: 200 }),
      )
    ...
    expect(result).toMatchObject({
      ok: true,
      summary: "Connected to exe.dev VM odysseus-probe.",
      metadata: {
        provider: "exe-dev",
        vmName: "odysseus-probe",
        sshDest: "odysseus-probe.exe.xyz",
        shellCommand: "bash",
      },
    });
    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'odysseus-probe'");
  });
  ...
  it("returns onboarding guidance when probe hits exe.dev SSH registration", async () => {
    ...
    expect(String(result?.metadata?.error ?? "")).toContain("the Odysseus host SSH key is not registered with exe.dev");
    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'odysseus-probe'");
  });
  ...
  it("realizes a workspace by mkdir-ing the remote cwd over SSH when VM metadata is present", async () => {
    ...
    const result = await plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      lease: {
        ...
        metadata: {
          sshDest: "vm-1.exe.xyz",
          remoteCwd: "/srv/odysseus/run-1",
        },
      },
      workspace: {
        localPath: "/local/odysseus",
    ...
    expect(sshCommand).toContain("/srv/odysseus/run-1");
    expect(result).toMatchObject({
      cwd: "/srv/odysseus/run-1",
      metadata: {
        provider: "exe-dev",
        remoteCwd: "/srv/odysseus/run-1",
      },
  ...
  it("falls back through workspace.remotePath then workspace.localPath when lease.metadata.remoteCwd is missing", async () => {
    ...
    const result = await plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      workspace: {
        localPath: "/local/odysseus",
        remotePath: "/srv/odysseus/remote-fallback",
      },
    });

    expect(result?.cwd).toBe("/srv/odysseus/remote-fallback");
  });
  ...
  it("skips ensureRemoteWorkspace and returns the resolved cwd when no VM metadata is available", async () => {
    ...
    expect(result?.cwd).toBe("/srv/odysseus/no-vm");
  });
});

...
describe("exe.dev sandbox provider plugin", () => {
  ...
  it("normalizes config and emits SSH guidance warnings", async () => {
    ...
    const result = await plugin.definition.onEnvironmentValidateConfig?.({
      ...
      config: {
        ...
        namePrefix: " ValadrienOs Sandbox ",
        image: " ubuntu:22.04 ",
        cpu: "4.8",
        memory: " 8GB ",
        disk: " 50GB ",
    ...
    expect(result).toEqual({
      ...
      normalizedConfig: {
        ...
        namePrefix: "valadrien-os-sandbox",
        image: "ubuntu:22.04",
        command: null,
        cpu: 4,
        memory: "8GB",
  ...
  it("acquires a lease by creating a VM and preparing the SSH workspace", async () => {
    fetchMock.mockResolvedValueOnce(
      new Response(JSON.stringify({
        vm_name: "valadrien-os-env-run",
        ssh_dest: "valadrien-os-env-run.exe.xyz",
        https_url: "https://valadrien-os-env-run.exe.xyz",
        status: "running",
      }), { status: 200 }),
    );
    ...
    expect(lease).toMatchObject({
      providerLeaseId: "valadrien-os-env-run",
      metadata: {
        provider: "exe-dev",
        vmName: "valadrien-os-env-run",
        sshDest: "valadrien-os-env-run.exe.xyz",
  ...
  it("surfaces exe.dev SSH onboarding guidance during lease acquisition", async () => {
    ...
    await expect(plugin.definition.onEnvironmentAcquireLease?.({
      ...
      "the ValadrienOs host SSH key is not registered with exe.dev",
    );

    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'valadrien-os-env-run'");
  });
  ...
  it("returns exe.dev SSH onboarding guidance for command execution failures", async () => {
    ...
    expect(String(result?.stderr ?? "")).toContain("the ValadrienOs host SSH key is not registered with exe.dev");
  ...
  it("probes by creating and then deleting a VM after SSH verification", async () => {
    fetchMock
      .mockResolvedValueOnce(
        new Response(JSON.stringify({
          vm_name: "valadrien-os-probe",
          ssh_dest: "valadrien-os-probe.exe.xyz",
          status: "running",
        }), { status: 200 }),
      )
    ...
    expect(result).toMatchObject({
      ok: true,
      summary: "Connected to exe.dev VM valadrien-os-probe.",
      metadata: {
        provider: "exe-dev",
        vmName: "valadrien-os-probe",
        sshDest: "valadrien-os-probe.exe.xyz",
        shellCommand: "bash",
      },
    });
    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'valadrien-os-probe'");
  });
  ...
  it("returns onboarding guidance when probe hits exe.dev SSH registration", async () => {
    ...
    expect(String(result?.metadata?.error ?? "")).toContain("the ValadrienOs host SSH key is not registered with exe.dev");
    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'valadrien-os-probe'");
  });
  ...
  it("realizes a workspace by mkdir-ing the remote cwd over SSH when VM metadata is present", async () => {
    ...
    const result = await plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      lease: {
        ...
        metadata: {
          sshDest: "vm-1.exe.xyz",
          remoteCwd: "/srv/valadrien-os/run-1",
        },
      },
      workspace: {
        localPath: "/local/valadrien-os",
    ...
    expect(sshCommand).toContain("/srv/valadrien-os/run-1");
    expect(result).toMatchObject({
      cwd: "/srv/valadrien-os/run-1",
      metadata: {
        provider: "exe-dev",
        remoteCwd: "/srv/valadrien-os/run-1",
      },
  ...
  it("falls back through workspace.remotePath then workspace.localPath when lease.metadata.remoteCwd is missing", async () => {
    ...
    const result = await plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      workspace: {
        localPath: "/local/valadrien-os",
        remotePath: "/srv/valadrien-os/remote-fallback",
      },
    });

    expect(result?.cwd).toBe("/srv/valadrien-os/remote-fallback");
  });
  ...
  it("skips ensureRemoteWorkspace and returns the resolved cwd when no VM metadata is available", async () => {
    const result = await plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      lease: {
        providerLeaseId: null,
        metadata: {
          remoteCwd: "/srv/valadrien-os/no-vm",
        },
      },
      workspace: {
        localPath: "/local/valadrien-os",
    ...
    expect(result?.cwd).toBe("/srv/valadrien-os/no-vm");
  });
});

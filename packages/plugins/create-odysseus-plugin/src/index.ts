...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".odysseus-sdk");
...
/**
 * Generate a complete Odysseus plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Odysseus plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "odysseus-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["odysseus", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@odysseus/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@odysseus/plugin-sdk": sdkDependency,
...
`,
    ...
    writeFile(
      ...
      `import { usePluginData, type PluginWidgetProps } from "@odysseus/plugin-sdk/ui";
...
When this package is installed from a local path, Odysseus watches that rebuilt
output and reloads the plugin worker. Local installs run trusted code from this
folder on your machine.

${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@odysseus/plugin-sdk\` and \`@odysseus/shared\` from a local Odysseus checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.odysseus-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}

## Install Into Odysseus

\`\`\`bash
odysseus plugin install ${shellQuote(toPosixPath(outputDir))}
\`\`\`
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-odysseus-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <odysseus-sdk-path>]");
    process.exit(1);
  }

...
// We found /issues in server/src/routes/issues.ts without /v1/
...
const gooseJsonTraining = `
--- JSON Training for Goose ---
...
Instruction for Goose:
1. The 'description' field contains your primary instructions. Read it carefully.
2. The 'identifier' (e.g. ENG-123) is the human-readable ID. Always reference it when logging or updating work.
3. The 'title' gives you the broad context of the task.
`;
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      {
        name: "list_issues",
        description: "List issues in the Paperclip workspace. Use this to find work assigned to you. Returns a JSON array of issues. " + gooseJsonTraining,
        inputSchema: {
          type: "object",
          properties: {
      ...
      {
        name: "get_issue",
        description: "Retrieve a single issue by ID or identifier (e.g. ENG-123). " + gooseJsonTraining,
        inputSchema: {
          type: "object",
          properties: {
...
async function main() {
  ...
  console.error("Paperclip Goose MCP server running on stdio");
}

...
describeEmbeddedPostgres("company secret proposals migration", () => {
  ...
  it("can be reapplied after its migration journal entry is removed", async () => {
    ...
    // Embedded Postgres may need to replay the schema journal under the
    // serialized workspace suite, so retain the migration assertion while
    // allowing the database process its normal startup window.
  }, 20_000);
});

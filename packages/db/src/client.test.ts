...
describeEmbeddedPostgres("applyPendingMigrations", () => {
  ...
  it(
    ...
    async () => {
      ...
      const sql = postgres(connectionString, { max: 1, onnotice: () => {} });
      try {
        const liveWakeIdempotencyHash = await migrationHash("0182_agent_wakeup_live_idempotency.sql");
      ...
      expect(pendingState).toMatchObject({
        status: "needsMigrations",
        pendingMigrations: ["0182_agent_wakeup_live_idempotency.sql"],
        reason: "pending-migrations",
      });
      ...
      try {
        ...
        expect(rows[1].error).toContain("0182_agent_wakeup_live_idempotency.sql coalesced");

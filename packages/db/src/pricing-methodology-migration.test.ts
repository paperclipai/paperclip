...
const MIGRATION_FILE = "0276_pricing_methodology.sql";
...
describe("0276_pricing_methodology migration", () => {
  ...
  it("adds rate_card_cents nullable and with no column default", async () => {
    const sql = await readMigration();
    // Nullable: a row with no measurable rate records NULL, not 0. No DEFAULT:
    // a lingering `DEFAULT 0` would let a future omitted-column insert silently
    // write 0 and masquerade as measured.
    expect(sql).toMatch(
      /ALTER TABLE\s+"cost_events"\s+ADD COLUMN\s+IF NOT EXISTS\s+"rate_card_cents"\s+integer\s*;/i,
    );
    expect(sql).not.toMatch(/"rate_card_cents"\s+integer\s+DEFAULT/i);
  });
  ...
  it("documents a backout path in the migration comment", async () => {
    ...
    expect(sql).toMatch(
      new RegExp(
        String.raw`--\s*ALTER TABLE\s+"cost_events"${commentLine}DROP COLUMN\s+IF EXISTS\s+"rate_card_cents"`,
        "i",
      ),
    );
    expect(sql).toMatch(

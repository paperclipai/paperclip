-- paperclip:migration-safety-ignore large-create-index-not-concurrently: Drizzle migrations run transactionally, so CONCURRENTLY is unavailable, and this partial unique index backstops the advisory-lock-guarded issue_children_completed wake claim with no forward-only alternative.

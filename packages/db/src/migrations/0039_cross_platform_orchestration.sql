-- P0: Add adapterType to cost_events for cross-platform cost tracking
ALTER TABLE "cost_events" ADD COLUMN "adapter_type" text;
CREATE INDEX IF NOT EXISTS "cost_events_company_adapter_occurred_idx" ON "cost_events" ("company_id", "adapter_type", "occurred_at");

-- Backfill adapter_type from agent table
UPDATE "cost_events" ce
SET "adapter_type" = a."adapter_type"
FROM "agents" a
WHERE ce."agent_id" = a."id" AND ce."adapter_type" IS NULL;

-- P2: Workflow runs table for multi-platform workflow chains

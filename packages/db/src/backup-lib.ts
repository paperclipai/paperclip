...
const STATEMENT_BREAKPOINT = "-- valadrien-os statement breakpoint 69f6f3f1-42fd-46a6-bf17-d1d85f8f3900";
...
async function restoreWithPsql(opts: RunDatabaseRestoreOptions, connectTimeout: number): Promise<void> {
  const psqlBin = process.env.VALADRIEN_OS_PSQL_PATH || "psql";
  const child = spawn(
    psqlBin,
    [
      `--dbname=${opts.connectionString}`,
...
export async function runDatabaseBackup(opts: RunDatabaseBackupOptions): Promise<RunDatabaseBackupResult> {
  const filenamePrefix = opts.filenamePrefix ?? "valadrien-os";
  ...
  try {
    ...
    emit("-- ValadrienOs database backup");

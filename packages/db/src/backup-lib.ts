...
function parseRestoreCopyBlock(statement: string): { command: string; data: string } | null {
  const lines = statement.split(/\r?\n/);
  const copyLineIndex = lines.findIndex((line) => /^\s*COPY\s+.+\s+FROM\s+STDIN\s*;\s*$/i.test(line));
  if (copyLineIndex === -1) return null;

  const leadingLines = lines.slice(0, copyLineIndex);
  if (leadingLines.some((line) => {
  ...
  const copyLines = lines.slice(copyLineIndex + 1);
  if (copyLines.at(-1) !== "\\.") {
    throw new Error("Unsupported COPY block in fallback restore");
  }

  const dataLines = copyLines.slice(0, -1);
  return {
    command: lines[copyLineIndex]!.trim(),
...
export async function runDatabaseRestore(opts: RunDatabaseRestoreOptions): Promise<void> {
  ...
  try {
    ...
    for await (const statement of readRestoreStatements(opts.backupFile)) {
      const copyBlock = parseRestoreCopyBlock(statement);
      if (copyBlock) {
        const copyStream = await sql.unsafe(copyBlock.command).writable();
        await pipeline(Readable.from(copyBlock.data.length > 0 ? [copyBlock.data] : []), copyStream);
      } else {

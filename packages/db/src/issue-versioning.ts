...
async function runInTransaction<T>(
  tx: DbTransaction,
  { issueId, companyId, expectedVersion, now, mutate }: RunIssueMutationInput<T>,
): Promise<IssueMutationResult<T> | null> {
  const issueFilter = companyId
  ...
  const updated = await tx
    ...
    .where(and(issueFilter, eq(issues.version, current.version)))
    .returning()
    .then((rows) => rows[0] ?? null);

...
export const issues = pgTable(
  "issues",
  {
    ...
    planRootIssueId: uuid("plan_root_issue_id").references((): AnyPgColumn => issues.id),
    ...
    // GitHub PR URL opened for this issue's branch (dev-team A3 pipeline).
    prUrl: text("pr_url"),
  ...
  (table) => ({
    ...
    planRootIdx: index("issues_company_plan_root_idx").on(table.companyId, table.planRootIssueId),

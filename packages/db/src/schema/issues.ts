...
export const issues = pgTable(
  "issues",
  {
    ...
    closeContract: jsonb("close_contract").$type<Record<string, unknown>>(),
  ...
  (table) => ({
    ...
    openFallbackMonitorRoutineExecutionIdx: uniqueIndex("issues_open_fallback_monitor_execution_uq")
      .on(
        table.companyId,
        sql`lower(regexp_replace(btrim(${table.title}), '\\s+', ' ', 'g'))`,
      )
      .where(
        sql`${table.originKind} = 'routine_execution'
          and ${table.hiddenAt} is null
          and ${table.status} in ('backlog', 'todo', 'in_progress', 'in_review', 'blocked')
          and lower(regexp_replace(btrim(${table.title}), '\\s+', ' ', 'g')) = 'fallback-monitor'`,
      ),

...
export const agentWakeupRequests = pgTable(
  ...
  (table) => ({
    ...
    feedbackDeliveryIdempotencyUq: uniqueIndex("agent_wakeup_requests_feedback_delivery_idempotency_uq")
      .on(table.companyId, table.idempotencyKey)
      .where(sql`${table.idempotencyKey} LIKE 'feedback_delivery:%' AND ${table.status} NOT IN ('skipped', 'cancelled')`),

...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://odysseus:odysseus@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "odysseus");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://odysseus:odysseus@127.0.0.1:${preferredPort}/odysseus`,
  ...
  if (runningPid) {
    ...
    return {
      connectionString: `postgres://odysseus:odysseus@127.0.0.1:${port}/odysseus`,
  ...
  const adminConnectionString = `postgres://odysseus:odysseus@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "odysseus");

  return {
    connectionString: `postgres://odysseus:odysseus@127.0.0.1:${selectedPort}/odysseus`,

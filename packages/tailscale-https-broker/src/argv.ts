...
function assertMutablePort(port: number): number {
 ...
 */
export function buildExposeArgv(binPath: string, port: number): string[] {
  assertAbsolute(binPath);
  const p = assertMutablePort(port);
  return [binPath, "serve", "--bg", `--https=${p}`, loopbackTarget(p)];
}

/** Remove exactly one HTTPS listener by port. Never `reset`, never `off` all. */
export function buildRemoveArgv(binPath: string, port: number): string[] {
  assertAbsolute(binPath);
  const p = assertMutablePort(port);

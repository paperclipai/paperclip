...
- **IMMEDIATELY cancel any issue with `originKind: "stale_active_run_evaluation"`** — `PATCH /api/issues/{id}` with `{"status":"cancelled"}`. Do not investigate them; cancel on sight. Board policy: these waste tokens.

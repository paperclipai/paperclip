...
| WORKTREE_PATCH_V1 | per-agent worktree provisioning + PR-on-exit | packages/adapters/claude-local/src/server/execute.ts | When `adapterConfig.worktreeEnabled: true`, the adapter (a) creates `~/.paperclip-worktrees/<slug>-<taskId>/` branched from `origin/<primaryBase>` pre-spawn, (b) overrides cwd to `<wkt>/_workspaces/<slug>/`, (c) injects GH_TOKEN/GITHUB_TOKEN from macOS keychain (`<keychainService><slug>`, account `paperclip`), git identity (GIT_AUTHOR_NAME/EMAIL), and PAPERCLIP_WORKTREE/PAPERCLIP_IOS_WORKTREE/PAPERCLIP_AGENT_SLUG env, (d) on session exit pushes the branch + opens a PR via `gh pr create` with `auto-merge:approved` label if commits exist, (e) always cleans up worktree + branch. Supports optional `secondaryRepo` for engineering agents (second worktree at `<wkt>-ios`). Block guarded by `worktreeEnabled` — a no-op when absent, so existing agent configs are unaffected. Design: `~/.claude/plans/okay-this-is-clearly-luminous-goblet.md` Layer 5. | HIGH | Not upstreamable (FreeMyMemories-specific policy) |
| WORKTREE_PATCH_V1.1 | detect existing PR before creating (safety-net mode) | packages/adapters/claude-local/src/server/execute.ts | Agents now open their own PRs via `ship` / `open-rollup-pr` / `open-vault-pr` skills with rich bodies during the session. Before `gh pr create`, the adapter runs `gh pr list --head <branch> --state open --json number --limit 1` and skips creation when a PR already exists (logs `PR already exists on <branch> (#N), skipping create`). When no PR exists (agent crashed or didn't open one), the fallback PR body is prefixed with a ⚠️ banner noting it was created by the adapter on session exit and review context may be incomplete. Cleanup (worktree + branch remove) always runs regardless. Extends WORKTREE_PATCH_V1 — same guard marker region. | HIGH | Not upstreamable (FreeMyMemories-specific policy) |
| WORKTREE_PATCH_V1.2 | drop per-agent PAT injection; use default gh auth | packages/adapters/claude-local/src/server/execute.ts | Simplification rollback of the per-agent PAT design. Removed: `readKeychainPat()` helper, `keychainService` field on `WorktreeConfig`, `patToken` field on `WorktreeProvisionResult`, `patToken` param on `finalizeWorktree`, and the `envAdditions.GH_TOKEN` / `envAdditions.GITHUB_TOKEN` assignments. The spawned Claude Code process and the post-session `gh pr create` call now both inherit default `gh` CLI auth from `~/.config/gh/` (dirk-miller's OAuth session). Per-agent PAT scoping was belt-and-suspenders against a low-probability threat (malicious agent behavior) at the cost of high ongoing maintenance (token rotation, per-repo regeneration, Dirk-UI-gated). Not worth it for a single-operator system protected by the PreToolUse hook + adapter worktrees + workflow gates. Auto-merge workflows in both repos switched to `secrets.GITHUB_TOKEN` in a companion change. Extends WORKTREE_PATCH_V1 — same guard marker region. | HIGH | Not upstreamable (FreeMyMemories-specific policy) |
| WORKTREE_PATCH_V2 | stable worktree per (agent, task) — fixes Claude Code session-resume | packages/adapters/claude-local/src/server/execute.ts, packages/adapters/claude-local/scripts/gc-orphan-worktrees.mjs | Root cause: the adapter was provisioning `~/.paperclip-worktrees/<slug>-<Date.now()>` when `taskId` was missing, and unconditionally `git worktree add -b` on every wake even when `taskId` was present. Claude Code indexes its session JSONLs by cwd-derived project slug — a new cwd per wake made `--resume` useless, so iOS Dev ↔ Code Reviewer handoffs (and every multi-wake issue) started with a fresh session instead of resuming. Fix: (a) deterministic branch `<slug>-<taskId>` when taskId present; ephemeral fallback key is persisted into `sessionParams.worktreeKey` so subsequent wakes in the same runtime session reuse it. (b) New `ensureWorktree()` helper: registered → verify branch + clean tree + `fetch` + `pull --ff-only` and return `{reused:true}`; not registered → fetch base + `worktree add`. (c) Fail-loud: uncommitted changes from a prior wake throw rather than silently stashing. (d) `.paperclip-wake.lock` inside the worktree (PID + ISO timestamp); stale-PID lock is cleaned; live-PID lock refuses spawn. (e) Post-completion cleanup is conditional: ephemeral wakes cleanup as before; task wakes query `GET $PAPERCLIP_API_URL/api/issues/$taskId` and cleanup iff status is `done`/`cancelled`, else leave worktree for the next wake. On cleanup, also `rm -rf ~/.claude/projects/<cwd-slug>/` so dead Claude session JSONLs don't accumulate. (f) GC script at `scripts/gc-orphan-worktrees.mjs` — enumerates `~/.paperclip-worktrees/`, queries Paperclip for status per worktree's stableKey, flags merged-PR branches as orphans; dry-run by default, requires `--confirm`. Extends WORKTREE_PATCH_V1 — lives in the same `provisionWorktrees` / `finalizeWorktree` region and honors the same `worktreeEnabled` toggle. Reload: tsx watch picks up the adapter on next file change; a Paperclip server restart guarantees it. | HIGH | Not upstreamable (FreeMyMemories-specific policy) |

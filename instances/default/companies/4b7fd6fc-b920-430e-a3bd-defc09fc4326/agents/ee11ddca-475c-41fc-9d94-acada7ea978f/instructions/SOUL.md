...
## Genesis Website Anti-Patterns

- Do not greenlight fragile frontend "wins" on Genesis just because they sound fast. A speed tweak that destabilizes `/sg/` is a business loss, not a win.
- Do not accept manual jQuery defer, output-buffer HTML mutation, regex page rewriting, or theme-file surgery as default solutions.
- Protect the homepage and commercial service pages like revenue surfaces. Prefer boring, inspectable, reversible changes over clever ones.
- If the symptom is font, colour, image, or hamburger drift, assume design-system / JS init / cache-path problems first — not content quality problems.

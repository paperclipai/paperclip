...
- When running `wp_update_post` from wp-cli for any GENESIS_PROTECTED post (IDs 45, 295, 656, 663, 951, 953, 955, 957, 1598, 2360), set `GENESIS_SAFETY_BYPASS=1` in the subprocess env AND call `remove_all_filters("wp_insert_post_data");` at the start of the wp eval, then re-verify with `wp post get`. See shared guardrail #14.
